...
// 
Функция ТекстЗапроса_ЗатратыНаРемонт_МониторНП() Экспорт
	// Затраты на ремонт.
	ТекстЗапроса =	
		"ВЫБРАТЬ
		...
		|	ЕСТЬNULL(НалоговаяАмортизация.БалансоваяСтоимостьНаКонецГода, 0) КАК БалансоваяСтоимостьНаКонецПрошлыйГод,
		|	ЕСТЬNULL(НалоговаяАмортизация.АмортизацияЗаГод, 0) КАК АмортизацияЗаПрошлыйГод
		|ИЗ
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.НалоговаяАмортизация КАК НалоговаяАмортизация
		|		ПО ХозрасчетныйОбороты.Субконто1 = НалоговаяАмортизация.ОсновноеСредство";
...
Функция ТекстЗапроса_ВременнаяТаблицаНалоговаяАмортизация()
	...
	ТекстЗапроса = 
		...
		//|	НалоговаяАмортизация.СуммаПоступленияНУ КАК СуммаПоступленияНУ,
		//|	НалоговаяАмортизация.СуммаВыбытияНУ КАК СуммаВыбытияНУ
...
Функция ТекстЗапроса_ВременнаяТаблицаКоличествоВГруппах()
	...
	ТекстЗапроса = 
		...
		|			И (НЕ СостоянияОССрезПоследних.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийОС.СнятоСУчета))
		|ГДЕ
		...
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ВременнаяТаблицаСписокОС.ГруппаНУ) КАК КоличествоВГруппах,
...
Функция ТекстЗапроса_ВременнаяТаблицаСписокГрупп_МониторНП()
	ТекстЗапроса = 	
		...
		|	ВременнаяТаблицаДвижениеОС.СуммаПоступленияНУ КАК СуммаПоступленияНУ,
		|	ВременнаяТаблицаДвижениеОС.СуммаВыбытияНУ КАК СуммаВыбытияНУ
		//|	ВременнаяТаблицаДвижениеОС.КоличествоПолныхМесяцев КАК КоличествоПолныхМесяцев
		|ПОМЕСТИТЬ ВременнаяТаблицаСписокГрупп
		|ИЗ
		|	ВременнаяТаблицаДвижениеОС КАК ВременнаяТаблицаДвижениеОС";	
...
Функция ТекстЗапроса_ДляМонитораНП()
	...
	ТекстЗапроса =  
		...
		|	ВременнаяТаблицаСписокГрупп.ГруппаНУ,
		|	ВременнаяТаблицаСписокГрупп.ОсновноеСредство,
		|	ВременнаяТаблицаСписокГрупп.Регистратор
		|
		|УПОРЯДОЧИТЬ ПО
		|	ГруппаНУ,
		|	ОсновноеСредство,
