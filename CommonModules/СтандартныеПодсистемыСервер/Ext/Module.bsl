...
Процедура ПередЗапускомПрограммы()
	...
	МинимальнаяВерсияПлатформы = "8.3.11.3133";
	...
	ПоддерживаемаяВерсияПлатформы = "8.3.11";
	...
	Иначе
		...
		НеПоддерживаемаяВерсияПлатформы = ?(СтрЗаканчиваетсяНа(Строка(ТекущийРежим), "8_3_11"), "", Строка(ТекущийРежим));
	КонецЕсли;
