...
Функция РасчетСуммыОтчетаНДС(ВидСуммы, Знач Параметры) Экспорт
	...
	ИначеЕсли ВидСуммы = "КоэффициентОсвобожденныхПоставокРасчетный" Тогда
		...
		Сумма053 = Параметры.Сумма053 - (Параметры.СуммаАвансы - Параметры.СуммаНДСАвансы - Параметры.СуммаНСПАвансы) 
...
Функция ВсеСведенияОРеализации(Организация, НачалоМесяца, КонецМесяца) Экспорт
	...
	Запрос.Текст = 
		...
		|	СчетаФактурыВыписанные.СуммаБезНДС + СчетаФактурыВыписанные.СуммаНДС КАК Всего,
...
Функция ВсеСведенияОПоступлении(Организация, НачалоМесяца, КонецМесяца) Экспорт
	...
	Запрос.Текст = 
		...
		|	ВЫБОР
		|		КОГДА СведенияОПоступлении.Регистратор ССЫЛКА Документ.ГТДПоИмпорту
		|			ТОГДА ДанныеДляОтчетаРеестрВвезенных.Контрагент.ПризнакСтраны
		|		ИНАЧЕ СведенияОПоступлении.ПризнакСтраны
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрНакопления.ДанныеДляОтчетаРеестрВвезенных.Обороты(, , Авто, Организация = &Организация) КАК ДанныеДляОтчетаРеестрВвезенных
		|		ПО СведенияОПоступлении.Регистратор = ДанныеДляОтчетаРеестрВвезенных.Регистратор
		|ГДЕ
