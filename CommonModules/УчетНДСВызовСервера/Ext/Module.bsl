...
Функция РасчетСуммыОтчетаНДС(ВидСуммы, Знач Параметры) Экспорт
	...
	ИначеЕсли ВидСуммы = "КоэффициентОсвобожденныхПоставокРасчетный" Тогда
		...
		Если КоэффициентОсвобожденныхПоставок <= Параметры.ПороговыйПроцентОсвобожденныхПоставок 
	...
	ИначеЕсли ВидСуммы = "НДСКРНеРазрешенныйКЗачету" Тогда
		РезультатРасчета = Параметры.КфОПР * Параметры.НДСДляРаспределенияКР + Параметры.НДСКРСебестоимость;
		
	ИначеЕсли ВидСуммы = "НДСЕАЭСНеРазрешенныйКЗачету" Тогда
		РезультатРасчета = Параметры.КфОПР * Параметры.НДСДляРаспределенияЕАЭС + Параметры.НДСЕАЭССебестоимость;
		
	ИначеЕсли ВидСуммы = "НДСНеКРНеРазрешенныйКЗачету" Тогда
		РезультатРасчета = Параметры.КфОПР * Параметры.НДСДляРаспределенияЕАЭС + Параметры.НДСНеКРСебестоимость;
	...
	ИначеЕсли ВидСуммы = "НДСРаспределениеНеЗачет" Тогда		
		...
		Если Параметры.УказыватьПризнакЗачетаНДСПриПоступлении Тогда 			
			Для каждого СтрокаТаблицыЗначений Из Параметры.Таблица Цикл
				Если СтрокаТаблицыЗначений.ЗачетНДС = Перечисления.ВидыЗачетаНДС.Распределение Тогда
					СуммаРаспределения = СтрокаТаблицыЗначений.СуммаНДС - Окр(СтрокаТаблицыЗначений.СуммаНДС * Параметры.КоэффициентОсвобожденныхПоставокРасчетный, 2);
		...
		Иначе
			...
			Для каждого СтрокаТаблицыЗначений Из Параметры.Таблица Цикл
				РезультатРасчета = РезультатРасчета + СтрокаТаблицыЗначений.СуммаНДС - Окр(СтрокаТаблицыЗначений.СуммаНДС * Параметры.КоэффициентОсвобожденныхПоставокРасчетный, 2);
				СтрокаРасшифровки = ТаблицаРасшифровка.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаРасшифровки, СтрокаТаблицыЗначений);
			КонецЦикла;
	...
	ИначеЕсли ВидСуммы = "Сумма053" Тогда
		...
		Для каждого СтрокаТаблицыЗначений Из Параметры.Таблица Цикл
			...
			Если МассивСтавок.Количество() <> 0 Тогда
				...
				СтрокаРасшифровки = ТаблицаРасшифровка.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаРасшифровки, СтрокаТаблицыЗначений);	
			ИначеЕсли СтрокаТаблицыЗначений.СтавкаНДС = Справочники.СтавкиНДС.Нулевая Тогда
				РезультатРасчета = РезультатРасчета - СтрокаТаблицыЗначений.СуммаБезНДС;
				СтрокаРасшифровки = ТаблицаРасшифровка.Добавить();
				ЗаполнитьЗначенияСвойств(СтрокаРасшифровки, СтрокаТаблицыЗначений);				
			КонецЕсли;
	...
	ИначеЕсли ВидСуммы = "Сумма065" Тогда
		...
		Иначе
			...
			РезультатРасчета = КфНП * Параметры.Сумма064;
		КонецЕсли;
...
Функция ПолучитьСтавкиНДССНулевымиЗначениями(Период)
	...
	Запрос.Текст = 
		...
		|			&Период,
		|			СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Нулевая)
		|				И Ставка = 0) КАК СтавкиНДССрезПоследних";
