...
Процедура ОбработатьУведомлениеИзмененияСостояния(Знач ДанныеУведомления) Экспорт
	...
	Пояснение = СтрШаблон(НСтр("ru = 'Расширение ""%1"" %2, необходимо перезапустить приложение';
								|en = 'The extension ""%1"" %2, the application must be restarted'"), ДанныеУведомления.Расширение, СостояниеДоп);
...
Процедура ОбработатьПерезапуск(Знач ДанныеУведомления) Экспорт 
	...
	Вопрос = СтрШаблон(НСтр("ru = 'Расширение ""%1"" %2, перезапустить приложение?';
							|en = 'The extension ""%1"" %2, restart the application?'"), ДанныеУведомления.Расширение, СостояниеДоп);
