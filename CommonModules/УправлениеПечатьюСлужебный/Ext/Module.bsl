...
Процедура СобратьКонтейнерДокументаDOCX(Знач ПолноеИмяФайла, Знач ПутьКСтруктуреФайла)
	...
	МаскаУпаковкиФайлов = ПутьКСтруктуреФайла + ?(Прав(ПутьКСтруктуреФайла, 1) = "\", "", "\") + "*";
...
Функция СобратьФайлДокументаDOCX(ПечатнаяФорма)
	...
	ИзменяемыеФайлы.Вставить("СвязиКонтента", ПечатнаяФорма.ИмяКаталога + "\word\_rels\document.xml.rels");
	ИзменяемыеФайлы.Вставить("ТипыКонтента",  ПечатнаяФорма.ИмяКаталога + "\[Content_Types].xml");
	ИзменяемыеФайлы.Вставить("Документ",      ПечатнаяФорма.ИмяКаталога + "\word\document.xml");
	...
	Для Каждого Раздел Из ПечатнаяФорма.СтруктураДокумента.Разделы Цикл
		...
		Для Каждого ЭлементКолонтитула Из Раздел.Значение.Колонтитулы Цикл
			...
			ИмяФайла = ПечатнаяФорма.ИмяКаталога + "\word\" + Колонтитул.ИмяВнутр + ".xml";
...
Процедура ИнициализироватьСтруктуруМакета(Макет)
	...
	МаскаТипов = "[Content_Types].xml";
	ФайлТипов = НайтиФайлы(ИмяКаталога, МаскаТипов);
	Если ФайлТипов.Количество() = 1 Тогда
		Чтение = Новый ЧтениеТекста(ФайлТипов[0].ПолноеИмя, КодировкаТекста.UTF8);
		ТекстФайла = Чтение.Прочитать();
		СтруктураДокумента.ТипыКонтента = ТекстФайла;
	КонецЕсли;
	
	КаталогФайлаСвязей = ИмяКаталога + "\word\_rels";
	МаскаСвязей = "document.xml.rels";
	ФайлСвязей = НайтиФайлы(КаталогФайлаСвязей, МаскаСвязей);
	
	Если ФайлСвязей.Количество() = 1 Тогда
		
		ЧтениеXML = Новый ЧтениеТекста(ФайлСвязей[0].ПолноеИмя, КодировкаТекста.UTF8);
	...
	КаталогСоСтруктуройФайла = ИмяКаталога + "\word";
	СтруктурныеФайлы = НайтиФайлы(КаталогСоСтруктуройФайла, "document.xml");
	
	Для Каждого ТекФайл Из СтруктурныеФайлы Цикл
		ЧтениеXML        = ИнициализироватьЧтениеXML(ТекФайл.ПолноеИмя, 1);
	...
	Для Каждого ТекФайл Из СтруктурныеФайлы Цикл
		...
		Если НЕ (Лев(ТекФайл.ИмяБезРасширения, 6) = "header") И НЕ (Лев(ТекФайл.ИмяБезРасширения, 6) = "footer") Тогда
			Продолжить;
		КонецЕсли;
		
		Колонтитул = СтруктураДокумента.Колонтитулы.Получить(ТекФайл.Имя);
		
		Если Колонтитул = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		СтруктураДокумента.Колонтитулы.Удалить(ТекФайл.Имя);
...
Процедура ИнициализироватьСтруктуруПечатнойФормы(ПечатнаяФорма, Макет)
	...
	СтруктураДокумента.КаталогКартинок        = ИмяКаталога + "\word\media\";
	СтруктураДокумента.ТаблицаСвязейКонтента = Макет.СтруктураДокумента.ТаблицаСвязейКонтента.Скопировать();
	
	МаскаТипов = "[Content_Types].xml";
	ФайлТипов  = НайтиФайлы(ИмяКаталога, МаскаТипов);
	Если ФайлТипов.Количество() = 1 Тогда
		ЧтениеXML = Новый ЧтениеТекста(ФайлТипов[0].ПолноеИмя,КодировкаТекста.UTF8);
		ТекстФайла = ЧтениеXML.Прочитать();
		СтруктураДокумента.ТипыКонтента = ТекстФайла;
	КонецЕсли;
	...
	Для Каждого ТекФайл Из СтруктурныеФайлы Цикл
		...
		Если ТекФайл.ИмяБезРасширения = "document" Тогда
			ЗаписьXML = Новый ЗаписьТекста(ТекФайл.ПолноеИмя, КодировкаТекста.UTF8);
			ЗаписьXML.Записать("");
		КонецЕсли;
		
		Если Лев(ТекФайл.ИмяБезРасширения, 6) = "header" Тогда
			ЗаписьXML = Новый ЗаписьТекста(ТекФайл.ПолноеИмя,КодировкаТекста.UTF8);
			ЗаписьXML.Записать("");
		КонецЕсли;
		
		Если Лев(ТекФайл.ИмяБезРасширения, 6) = "footer" Тогда
			ЗаписьXML = Новый ЗаписьТекста(ТекФайл.ПолноеИмя, КодировкаТекста.UTF8);
			ЗаписьXML.Записать("");
		КонецЕсли;
