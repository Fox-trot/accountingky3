...
Функция СчетаРасчетовСКонтрагентами(Тип) Экспорт
	...
	Возврат БухгалтерскийУчетСервер.СчетаРасчетовСКонтрагентами(ИсключенныеСчета);
	
КонецФункции	

// Получает дату последнего обновления данных монитора руководителя по организации
// Если в организации указана пустая ссылка или неопределено то вернет наибольшую дату по всем организациям
// Параметры
...
// ВариантОкругления - Число - 1 - округлять до целых, 1000 - до тысяч
// Возвращаемое значение:
...
// Параметры:
//   Параметры - Структура - Структура с параметрами переданная в фоновое задание
//         *ПолучатьПрошлыйПериод - Булево - Нужно ли рассчитывать данные прошлого периода.
//	ВременноеХранилищеРезультата - Строка - путь к временному хранилищу (не используется в рамках данной процедуры)	
...
Функция НовыйДанныеМонитораРуководителя()
	...
	Данные.Колонки.Добавить("СуммаНаСегодня",     МетаданныеРегистра.Ресурсы.Сумма.Тип);
...
Функция ЗапросКДаннымРазделаМонитора(ПолучатьПрошлыйПериод, ОтбиратьПоОрганизации, ПолучатьДанныеПодразделов)
	...
	|	ДанныеМонитораРуководителя.СуммаВВалюте КАК СуммаВВалюте,
	|	ДанныеМонитораРуководителя.СуммаВВалюте * КурсыВалютСрезПоследних.Курс / КурсыВалютСрезПоследних.Кратность / &ВариантОкругления КАК СуммаНаСегодня,
	...
	|		ПО ДанныеМонитораРуководителя.Валюта = КурсыВалютСрезПоследних.Валюта
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДанныеМонитораРуководителя.РазделМонитора,
	|	ДанныеМонитораРуководителя.ПодразделМонитора,
...
//		*Организация - СправочникСсылка.Организации - организация по которой нужно обновить данные монитора
//									Если не заполнено данные будут обновлены по всем доступным организациям
...
Функция ДанныеОстаткиДенежныхСредств(Параметры) Экспорт
	...
	// Банк
	Отбор.Вставить("Порядок", Отчеты.ОстаткиДенежныхСредств.ПорядокСомовыхСчетовВМониторе());
	НайденныеСтроки = ДанныеРазделовМонитора.НайтиСтроки(Отбор);
	Если НайденныеСтроки.Количество() > 0 Тогда
...
Функция ДанныеКурсыВалют(Параметры) Экспорт
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсыВалютСегодня
		|ИЗ
		|	РегистрСведений.КурсыВалют.СрезПоследних(, Валюта = &Валюта0) КАК КурсыВалютСрезПоследних
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ИЗ
		|	РегистрСведений.КурсыВалют.СрезПоследних(, Валюта = &Валюта1) КАК КурсыВалютСрезПоследних
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	КурсыВалютСрезПоследних.Курс
		|ИЗ
		|	РегистрСведений.КурсыВалют.СрезПоследних(, Валюта = &Валюта2) КАК КурсыВалютСрезПоследних
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	КурсыВалютСрезПоследних.Курс
		|ИЗ
		|	РегистрСведений.КурсыВалют.СрезПоследних(, Валюта = &Валюта3) КАК КурсыВалютСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	КурсыВалютСрезПоследних.Валюта КАК Валюта,
		|	КурсыВалютСрезПоследних.Курс КАК Курс
		|ПОМЕСТИТЬ ВременнаяТаблицаКурсыВалютВчера
		|ИЗ
		|	РегистрСведений.КурсыВалют.СрезПоследних(&ПериодВчера, Валюта = &Валюта0) КАК КурсыВалютСрезПоследних
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	КурсыВалютСрезПоследних.Курс
		|ИЗ
		|	РегистрСведений.КурсыВалют.СрезПоследних(&ПериодВчера, Валюта = &Валюта1) КАК КурсыВалютСрезПоследних
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	КурсыВалютСрезПоследних.Курс
		|ИЗ
		|	РегистрСведений.КурсыВалют.СрезПоследних(&ПериодВчера, Валюта = &Валюта2) КАК КурсыВалютСрезПоследних
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	КурсыВалютСрезПоследних.Курс
		|ИЗ
		|	РегистрСведений.КурсыВалют.СрезПоследних(&ПериодВчера, Валюта = &Валюта3) КАК КурсыВалютСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаКурсыВалютСегодня.Период КАК Период,
		|	ВременнаяТаблицаКурсыВалютСегодня.Валюта КАК Валюта,
		|	ВременнаяТаблицаКурсыВалютСегодня.Валюта.Наименование КАК ВалютаНаименование,
		|	ВременнаяТаблицаКурсыВалютСегодня.Курс КАК Курс,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаКурсыВалютВчера.Курс = 0
		|			ТОГДА 0
		|		ИНАЧЕ (ВременнаяТаблицаКурсыВалютСегодня.Курс / ВременнаяТаблицаКурсыВалютВчера.Курс - 1) * 100
		|	КОНЕЦ КАК Отклонение
		|ИЗ
		|	ВременнаяТаблицаКурсыВалютСегодня КАК ВременнаяТаблицаКурсыВалютСегодня
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаКурсыВалютВчера КАК ВременнаяТаблицаКурсыВалютВчера
		|		ПО (ВременнаяТаблицаКурсыВалютВчера.Валюта = ВременнаяТаблицаКурсыВалютСегодня.Валюта)";
	Запрос.УстановитьПараметр("ПериодВчера", НачалоДня(ТекущаяДатаСеанса())-1);
	Запрос.УстановитьПараметр("Валюта0", Справочники.Валюты.НайтиПоКоду("840"));
	Запрос.УстановитьПараметр("Валюта1", Справочники.Валюты.НайтиПоКоду("643"));
	Запрос.УстановитьПараметр("Валюта2", Справочники.Валюты.НайтиПоКоду("978"));
	Запрос.УстановитьПараметр("Валюта3", Справочники.Валюты.НайтиПоКоду("398"));
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		ДанныеКурсыВалют = Новый Структура;
		ДанныеКурсыВалют.Вставить("Период", "");
		ДанныеКурсыВалют.Вставить("Валюта", "");
		ДанныеКурсыВалют.Вставить("ВалютаНаименование", "");
		ДанныеКурсыВалют.Вставить("Курс", 0);
		ДанныеКурсыВалют.Вставить("Отклонение", 0);
		ЗаполнитьЗначенияСвойств(ДанныеКурсыВалют, ВыборкаДетальныеЗаписи);
		
		Данные.Добавить(ДанныеКурсыВалют);		
	КонецЦикла;
