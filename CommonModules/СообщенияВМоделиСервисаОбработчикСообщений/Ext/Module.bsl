...
Процедура ОбработатьСообщение(Знач КаналСообщений, Знач ТелоСообщения, Знач Отправитель) Экспорт
	...
	Попытка
		...
		Если Обработчик = Неопределено Тогда
			Шаблон = НСтр("ru = 'Не удалось определить обработчик канала сообщений в модели сервиса %1';
							|en = 'Cannot define the message channel handler in SaaS %1'");
	...
	Исключение
		...
		Событие = НСтр("ru = 'Обмен сообщениями';
						|en = 'Message exchange'", ОбщегоНазначения.КодОсновногоЯзыка());
		Шаблон = НСтр("ru = 'Ошибка обработки сообщения %1: %2';
						|en = 'An error occurred when processing message %1: %2'", ОбщегоНазначения.КодОсновногоЯзыка());
