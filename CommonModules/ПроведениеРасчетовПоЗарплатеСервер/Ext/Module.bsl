...
Процедура РассчитатьЗаписиРегистраРасчета(ИмяРегистра, НаборЗаписейРегистра, ТабличнаяЧастьДокумента, Ошибки = Неопределено, МассивСтрокДляУдаления = Неопределено) Экспорт 
	...
	Пока ВыборкаКатегорий.Следующий() Цикл 
		...
		ДанныеДляРасчета = ПолучитьДанныеДляРасчета(ИмяРегистра, ВыборкаКатегорий.КатегорияРасчета, НеобходимыеДанные, Регистратор, ТабличнаяЧастьДокумента);
		
		РассчитатьЗаписиНабора(ИмяРегистра, ДанныеДляРасчета.ИсходныеДанные, НаборЗаписейРегистра, ТабличнаяЧастьДокумента, ДанныеДляРасчета.ТаблицаРазрезы, Ошибки, МассивСтрокДляУдаления);
...
Функция ПолучитьДанныеДляРасчета(ИмяРегистра, КатегорияРасчета, НеобходимыеДанные, Регистратор, ТабличнаяЧастьДокумента) Экспорт 
	...
	ДанныеДляРасчета.Вставить("ТаблицаРазрезы", ?(НеобходимыеДанные.ТребуетсяРасчетБазыВРазрезе, МассивРезультатов[1].Выгрузить(), Неопределено));
...
//
Процедура РассчитатьЗаписиНабора(ИмяРегистра, ИсходныеДанные, НаборЗаписейРегистра, ТабличнаяЧастьДокумента, ТаблицаРазрезы, Ошибки, МассивСтрокДляУдаления) Экспорт 
	...
	Для Каждого СтрокаДвижений ИЗ НаборЗаписейРегистра Цикл 
		...
		Если ИсходныеДанные.НайтиСледующий(СтруктураПоиска) Тогда
			...
			РассчитатьЗаписьРегистраРасчета(ИмяРегистра, СтрокаДвижений, ИсходныеДанные, ТаблицаРазрезы, Ошибки, ДополнительныеПараметры);
...
//  Ошибки			 - 	 - 
//
Процедура РассчитатьЗаписьРегистраРасчета(ИмяРегистра, СтрокаДвижений, ИсходныеДанные, ТаблицаРазрезы, Ошибки, ДополнительныеПараметры) Экспорт 
	...
	Если ИмяРегистра = "Начисления" Тогда 
		...
		ИначеЕсли СпособРасчета = ПредопределенноеЗначение("Перечисление.СпособыРасчетаНачислений.ПроизвольнаяФормула") Тогда 
			...
			ИначеЕсли ИсходныеДанные.Формула = "" Тогда 
				Результат = 0;
			Иначе 
				Результат = РасчетПоПроизвольнойФормуле(СтрокаДвижений, ИсходныеДанные, НормаДней, НормаЧасов, ФактДней, ФактЧасов, ТаблицаРазрезы, ШаблонТекстаСообщения);
			КонецЕсли;	
		...
		ИначеЕсли СпособРасчета = ПредопределенноеЗначение("Перечисление.СпособыРасчетаНачислений.ПоМесячнойСтавкеОтОбратногоПоДням") Тогда
			Если НормаДней = 0 Или ФактДней = 0 Тогда 
				ШаблонТекстаСообщения = НСтр("ru = 'Нет рабочих дней в заданном периоде для вида расчета ""%1""; сотрудник ""%2"".'");
			Иначе 
				ШаблонТекстаСобщения = НСтр("ru = 'Расчет от обратного в разработке; вида расчета ""%1""; сотрудник ""%2"".'");
				Результат = 0;
			КонецЕсли;
		КонецЕсли;	
...
Функция СведенияОСтажеСотрудника(Период, Организация, ФизЛицо) Экспорт 
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	СтажиСотрудников.Организация = &Организация
		|	И СтажиСотрудников.ФизЛицо = &ФизЛицо";
