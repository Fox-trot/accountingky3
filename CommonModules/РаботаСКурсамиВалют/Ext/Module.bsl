...
Функция КурсыАктуальны() Экспорт
	...
	|			И (ВЫБОР
	|				КОГДА Валюты.Наименование = ""USD""
	|						ИЛИ Валюты.Наименование = ""EUR""
	|						ИЛИ Валюты.Наименование = ""RUB""
	|						ИЛИ Валюты.Наименование = ""KZT""
	|					ТОГДА КурсыВалют.Период = &ТекущаяДата
	|				ИНАЧЕ РАЗНОСТЬДАТ(КурсыВалют.Период, &ТекущаяДата, ДЕНЬ) <= 7
	|			КОНЕЦ)
	// Конец БПКР
	|ГДЕ
