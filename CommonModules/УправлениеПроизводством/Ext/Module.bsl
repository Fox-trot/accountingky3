...
Функция ТекстЗапросаВременнаяТаблицаЗатратыСырья() Экспорт
	...
	// Статья затрат заполняется, так как большинство из документов, где требуется функция, не заполняют ее в ходе
	// определения счетов учета. После перевода этих документов на механизмы СчетаУчетаВДокументах следует пересмотреть
	// заполнения поля СтатьяЗатрат.
	Возврат
	"ВЫБРАТЬ ПЕРВЫЕ 1
	...
	|	ЕСТЬNULL(СтатьяЗатратСписаниеМатериалов.СтатьяЗатрат, ЗНАЧЕНИЕ(Справочник.СтатьиЗатрат.ПустаяСсылка)) КАК СтатьяЗатрат,
	|	ВЫБОР
