	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.УменьшаемогоОстатка Тогда
			...
			Иначе 
				...
				РасчетнаяДата = ?(ЗначениеЗаполнено(Выборка.ДатаМодернизации), Выборка.ДатаМодернизации, Выборка.ДатаВводаВЭксплуатацию);
				...
				РасчетнаяДата = ДобавитьМесяц(РасчетнаяДата, КоличествоПолныхЛет * 12);
				...
				Запрос.УстановитьПараметр("Период", РасчетнаяДата);
				...
				Если Выборка.СрокСлужбы <> 0 Тогда
				 	СуммаАмортизации = (Выборка.СтоимостьДляРасчетаАмортизации - НакопленныйИзносНаНачалоРасчетногоГода) / Выборка.СрокСлужбы * Выборка.КоэффициентУскорения;
				КонецЕсли;
			КонецЕсли;	
