...
Функция РасчетАмортизации(ПериодСтоимости, ПериодОстатков, Организация, ОсновноеСредство, НачислениеАмортизацииОСПоМесяцам, СуммаМодернизации = 0) Экспорт
	...
	"ВЫБРАТЬ
		...
		//|						ТОГДА ВЫРАЗИТЬ((ПараметрыУчетаОССрезПоследних.СтоимостьДляРасчетаАмортизации + &СуммаМодернизации - ПараметрыУчетаОССрезПоследних.ЛиквидационнаяСтоимость) / ПараметрыУчетаОССрезПоследних.СрокСлужбы / ДЕНЬ(КОНЕЦПЕРИОДА(&ПериодОстатков, МЕСЯЦ)) * &КоличествоДней КАК ЧИСЛО(15, 2))
		//|					ИНАЧЕ ВЫРАЗИТЬ((ПараметрыУчетаОССрезПоследних.СтоимостьДляРасчетаАмортизации - ПараметрыУчетаОССрезПоследних.ЛиквидационнаяСтоимость) / ПараметрыУчетаОССрезПоследних.СрокСлужбы / ДЕНЬ(КОНЕЦПЕРИОДА(&ПериодОстатков, МЕСЯЦ)) * &КоличествоДней КАК ЧИСЛО(15, 2))
		|				КОНЕЦ
		|		ИНАЧЕ 0
		...
		|				ИЛИ Регистратор Ссылка Документ.ЗакрытиеМесяца)) КАК ПараметрыУчетаОССрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Процедура ЗаполнитьДанныеОсновныхСредствВТабличнойЧасти(ДокументСсылка, Период, Организация, ТабличнаяЧасть, МассивОсновныхСредств = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|		ИНАЧЕ ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		|	КОНЕЦ КАК АмортизацияЗаМесяц,
		|	ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) - ВЫБОР
		|		КОГДА ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) >= ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ВременнаяТаблицаПараметрыУчетаОС.ЛиквидационнаяСтоимость
		|			ТОГДА 0
		|		КОГДА ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) < ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		|			ТОГДА ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0)
		|		ИНАЧЕ ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл  
		...
		Для Каждого СтрокаТабличнойЧасти Из СтрокиТабличнойЧасти Цикл
			...
			//Если ДокументРеализации Тогда
				//СтрокаТабличнойЧасти.НакопленнаяАмортизация = СтрокаТабличнойЧасти.НакопленнаяАмортизация + СтрокаТабличнойЧасти.АмортизацияЗаМесяц;	
			//КонецЕсли;	
		КонецЦикла;		
	КонецЦикла;
