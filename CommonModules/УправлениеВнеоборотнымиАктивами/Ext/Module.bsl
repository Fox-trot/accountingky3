...
//
Функция РасчетАмортизации(ПериодСтоимости, ПериодОстатков, Организация, ОсновноеСредство, НачислениеАмортизацииОСПоМесяцам, СуммаМодернизации = 0) Экспорт
	...
	"ВЫБРАТЬ
		...
		|				ВЫБОР
		|					КОГДА &СуммаМодернизации <> 0
		|						ТОГДА ВЫРАЗИТЬ((ПараметрыУчетаОССрезПоследних.СтоимостьДляРасчетаАмортизации + &СуммаМодернизации) / ПараметрыУчетаОССрезПоследних.СрокСлужбы / ДЕНЬ(КОНЕЦПЕРИОДА(&ПериодОстатков, МЕСЯЦ)) * &КоличествоДней КАК ЧИСЛО(15, 2))
		|					ИНАЧЕ ВЫРАЗИТЬ((ПараметрыУчетаОССрезПоследних.СтоимостьДляРасчетаАмортизации) / ПараметрыУчетаОССрезПоследних.СрокСлужбы / ДЕНЬ(КОНЕЦПЕРИОДА(&ПериодОстатков, МЕСЯЦ)) * &КоличествоДней КАК ЧИСЛО(15, 2))
		|				КОНЕЦ
		|		ИНАЧЕ 0
	...
	Если НачислениеАмортизацииОСПоМесяцам Тогда
		Запрос.УстановитьПараметр("ПериодСтоимости", 	НачалоМесяца(ПериодОстатков));
	Иначе
	...
	Если НачислениеАмортизацииОСПоМесяцам Тогда
		Запрос.УстановитьПараметр("КоличествоДней", День(КонецМесяца(ПериодОстатков)));	
	ИначеЕсли Месяц(ПериодСтоимости) = Месяц(ПериодОстатков) Тогда
		Запрос.УстановитьПараметр("КоличествоДней", День(ПериодОстатков) - День(ПериодСтоимости));
	Иначе
		Запрос.УстановитьПараметр("КоличествоДней", День(ПериодОстатков) - 1);
	КонецЕсли;
...
Процедура ЗаполнитьДанныеОсновныхСредствВТабличнойЧасти(ДокументСсылка, Период, Организация, ТабличнаяЧасть, МассивОсновныхСредств = Неопределено) Экспорт
	...
	ДанныеУчетнойПолитики = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьДанныеУчетнойПолитикиОрганизаций(Период, Организация);
	
	// 1. Параметры учета
	// 2. Накопленная амортизация на дату документа
	...
	Запрос.Текст = 
		...
		|						ТОГДА ВЫРАЗИТЬ(ПараметрыУчетаОССрезПоследних.СтоимостьДляРасчетаАмортизации / ПараметрыУчетаОССрезПоследних.СрокСлужбы КАК ЧИСЛО(15, 2))
		|					ИНАЧЕ ВЫРАЗИТЬ(ПараметрыУчетаОССрезПоследних.СтоимостьДляРасчетаАмортизации / ПараметрыУчетаОССрезПоследних.СрокСлужбы / ДЕНЬ(КОНЕЦПЕРИОДА(&Период, МЕСЯЦ)) * (ДЕНЬ(&Период) - 1) КАК ЧИСЛО(15, 2))
		|				  КОНЕЦ
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК АмортизацияЗаМесяц,
		...
		// АмортизацияЗаМесяц
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) >= ВременнаяТаблицаПараметрыУчетаОС.ПервоначальнаяСтоимость
		|			ТОГДА 0
		|		КОГДА ВременнаяТаблицаПараметрыУчетаОС.ПервоначальнаяСтоимость - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) < ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		|			ТОГДА ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0)
		|		ИНАЧЕ ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		|	КОНЕЦ КАК АмортизацияЗаМесяц,
		
		// НакопленнаяАмортизацияДокумента (накопленная + за месяц)
		|	ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) +
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) >= ВременнаяТаблицаПараметрыУчетаОС.ПервоначальнаяСтоимость
		|				ТОГДА 0
		|			КОГДА ВременнаяТаблицаПараметрыУчетаОС.ПервоначальнаяСтоимость - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) < ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		|				ТОГДА ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0)
		|			ИНАЧЕ ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		|		КОНЕЦ КАК НакопленнаяАмортизацияДокумента,
		
		// ОстаточнаяСтоимость
		|	ВременнаяТаблицаПараметрыУчетаОС.ПервоначальнаяСтоимость - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) -
		|		ВЫБОР
		|			КОГДА ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) >= ВременнаяТаблицаПараметрыУчетаОС.ПервоначальнаяСтоимость
		|				ТОГДА 0
		|			КОГДА ВременнаяТаблицаПараметрыУчетаОС.ПервоначальнаяСтоимость - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) < ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		...
		//|			ТОГДА 0
		//|		КОГДА ВременнаяТаблицаПараметрыУчетаОС.ПервоначальнаяСтоимость - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) < ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		//|			ТОГДА ВременнаяТаблицаПараметрыУчетаОС.ПервоначальнаяСтоимость - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0)
		//|		ИНАЧЕ ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		//|	КОНЕЦ КАК ОстаточнаяСтоимость,
		//
		//|	ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) + ВЫБОР
		//|		КОГДА ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) >= ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации
		//|			ТОГДА 0
		//|		КОГДА ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0) < ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		//|			ТОГДА ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ЕСТЬNULL(ВременнаяТаблицаОстатки.НакопленнаяАмортизация, 0)
		//|		ИНАЧЕ ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		//|	КОНЕЦ КАК НакопленнаяАмортизация,
		//
		//|	ЕСТЬNULL(ВременнаяТаблицаАмортизацияНаНачалоМесяца.НакопленнаяАмортизация, 0) + ВЫБОР
		//|		КОГДА ЕСТЬNULL(ВременнаяТаблицаАмортизацияНаНачалоМесяца.НакопленнаяАмортизация, 0) >= ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации
		//|			ТОГДА 0
		//|		КОГДА ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ЕСТЬNULL(ВременнаяТаблицаАмортизацияНаНачалоМесяца.НакопленнаяАмортизация, 0) < ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		//|			ТОГДА ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ЕСТЬNULL(ВременнаяТаблицаАмортизацияНаНачалоМесяца.НакопленнаяАмортизация, 0)
		//|		ИНАЧЕ ВременнаяТаблицаПараметрыУчетаОС.АмортизацияЗаМесяц
		//|	КОНЕЦ КАК НакопленнаяАмортизацияНаНачалоМесяца
		|ИЗ
		|	ВременнаяТаблицаПараметрыУчетаОС КАК ВременнаяТаблицаПараметрыУчетаОС
	...
	ДокументРеализации 	= ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг");
	ДокументСписания 	= ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.СписаниеОС");
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл  
		...
		Для Каждого СтрокаТабличнойЧасти Из СтрокиТабличнойЧасти Цикл
			...
			Если ДокументРеализации ИЛИ ДокументСписания Тогда
				СтрокаТабличнойЧасти.НакопленнаяАмортизацияНаНачалоМесяца = ВыборкаДетальныеЗаписи.НакопленнаяАмортизация;	
			КонецЕсли;	
		КонецЦикла;		
	КонецЦикла;
