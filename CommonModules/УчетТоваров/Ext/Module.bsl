...
Процедура СформироватьДвиженияСписаниеТоваров(ДополнительныеСвойства, Движения, Отказ) Экспорт

	ТаблицаСписанныеТовары = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаСписанныеТовары;
	...
	Для Каждого СтрокаПартии Из Параметры.ТаблицаСписанныеТовары Цикл
		...
		Если СтрокаПартии.КорСчетСписания = ПланыСчетов.Хозрасчетный.ОсновноеПроизводство 
		...
		Если (ВедетсяУчетПоПартиям ИЛИ СвойстваСчета.Забалансовый) И НЕ ЗаказНаПроизводство Тогда
			БухгалтерскийУчетСервер.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт,
		...
		Если ВедетсяУчетПоПартиям ИЛИ СвойстваСчета.Забалансовый 
			ИЛИ (ДокументРеализации И Проводка.СчетКт = ПланыСчетов.Хозрасчетный.ГотоваяПродукция) Тогда
...
Процедура СформироватьДвиженияПеремещениеТоваров(ДополнительныеСвойства, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаПартии Из Параметры.ТаблицаСписанныеТовары Цикл
		...
		БухгалтерскийУчетСервер.УстановитьСубконто(Проводка.СчетДт, Проводка.СубконтоДт, "Склады", Реквизиты.СкладПолучатель);
...
Процедура СформироватьДвиженияВозвратТоваровПоставщику(ДополнительныеСвойства, Движения, Отказ, ПрямыеПроводки) Экспорт
	
	ТаблицаСписанныеТовары = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаСписанныеТовары;
	ТаблицаТовары = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаТовары;		
	ТаблицаРеквизиты = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРеквизиты;
...
Функция ПодготовитьТаблицуВозвращенныеСписанныеТовары(ТаблицаТовары, ТаблицаСчетаУчетаТоваров, ТаблицаСписокНоменклатуры, ТаблицаРеквизиты, Отказ) Экспорт
	...
	|	ВременнаяТаблицаСписанныеТовары.Партия,
	|	ВременнаяТаблицаСписанныеТовары.СчетУчета,
	...
	|	СУММА(ВременнаяТаблицаСписанныеТовары.Количество) КАК Количество,
	|	СУММА(ВременнаяТаблицаСписанныеТовары.КоличествоОтгружено) КАК КоличествоОтгружено,
	|	СУММА(ВременнаяТаблицаСписанныеТовары.КоличествоВозвращеноРанее) КАК КоличествоВозвращеноРанее,
	|	СУММА(ВременнаяТаблицаСписанныеТовары.СуммаВозвращеноРанее) КАК СуммаВозвращеноРанее
	|ИЗ
	...
	|			И ВременнаяТаблицаСписанныеТовары.Партия = РеквизитыДокументаПартии.Документ
	|
	|СГРУППИРОВАТЬ ПО
	|	ВременнаяТаблицаСписанныеТовары.Номенклатура,
	|	ВременнаяТаблицаСписанныеТовары.СчетУчета,
	...
	|
	|ИМЕЮЩИЕ
	|	СУММА(ВременнаяТаблицаСписанныеТовары.Количество) > 0
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВременнаяТаблицаСписанныеТовары.Номенклатура,
	|	ДатаПартии УБЫВ";
...
Функция ПодготовитьПараметрыТаблицыВозвращенныеСписанныеТовары(ТаблицаТовары, ТаблицаСчетаУчетаТоваров, ТаблицаСписокНоменклатуры, ТаблицаРеквизиты)
	...
	+ "ДатаДокументаРеализации";                 // <ДокументСсылка>
...
Процедура СформироватьДвиженияПоступлениеТоваров(ДополнительныеСвойства, Движения, Отказ, ПоступлениеАО = Ложь) Экспорт
	
	ТаблицаТовары = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаТовары;
	ТаблицаРеквизиты = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРеквизиты;
	
	Параметры = ПодготовитьПараметрыПоступлениеТоваров(ТаблицаТовары, ТаблицаРеквизиты, ПоступлениеАО);
...
Функция ПодготовитьПараметрыПоступлениеТоваров(ТаблицаТовары, ТаблицаРеквизиты, ПоступлениеАО)
	...
	+ "СуммаАкциза,"			// <Число,15,2> - сумма акциза в валюте регламентированного учета
	+ "Содержание";          	// <Строка,150> - содержание проводки
...
Процедура СформироватьДвиженияОприходованиеТоваров(ДополнительныеСвойства, Движения, Отказ) Экспорт
...
Процедура СформироватьДвиженияГотоваяПродукция(ДополнительныеСвойства, Движения, Отказ) Экспорт

	ТаблицаГотоваяПродукция = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаГотоваяПродукция;
	ТаблицаРеквизитыГП = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРеквизитыГП;
