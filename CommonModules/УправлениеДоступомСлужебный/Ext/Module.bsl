...
Функция ОграничиватьДоступНаУровнеЗаписейУниверсально(БезУчетаКонстантыОграничениеДоступаНаУровнеЗаписей = Ложь) Экспорт
	
	УстановитьОтключениеБезопасногоРежима(Истина);
	УстановитьПривилегированныйРежим(Истина);
	
	Если БезУчетаКонстантыОграничениеДоступаНаУровнеЗаписей Тогда
...
Процедура УстановитьОбновлениеДоступа(Использование, БезПроверкиВыполненияОбновленияИБ = Ложь) Экспорт
	...
	Если Не БезПроверкиВыполненияОбновленияИБ
	...
	Если Не ВключитьЗадание Или ОбщегоНазначения.РазделениеВключено() Тогда
		Возврат;
	КонецЕсли;
...
//                        - Структура - см. ПараметрыПланированияОбновленияДоступа().
//
...
//
Функция ПараметрыПланированияОбновленияДоступа() Экспорт
	...
	Результат.Вставить("ДляВнешнихПользователей", Константы.ИспользоватьВнешнихПользователей.Получить());
...
Процедура УстановкаПараметровСеанса(ИмяПараметра, УстановленныеПараметры) Экспорт
	...
	ПараметрыСеанса.ОграничениеДоступаНаУровнеЗаписейУниверсально =
		ОграничиватьДоступНаУровнеЗаписей И УниверсальноеОграничение;
...
Функция СвойстваВидовДоступа()
	...
	Если Параметры.ОпределяемыеТипыЗначенийДоступа.Получить(
		...
		ТекстОшибки = Параметры.ЗаголовокОшибки + ОписаниеОшибки;
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
...
Процедура ПроверитьДоступПередЗаписьюИсточника(Источник, Отказ, ЭтоНаборЗаписей, Замещение)
	...
	Если ЭтоПолноправныйПользователь Тогда
		...
		Если Источник.ДополнительныеСвойства.Свойство("ПропуститьОбновлениеКлючейДоступа") Тогда
			// Ключи доступа будут обновляться разработчиком самостоятельно.
			Возврат;
		КонецЕсли;
	КонецЕсли;
...
Процедура ПроверитьДоступКИсточнику(Источник, ПередЗаписью, ЭтоНаборЗаписей, Замещение, ЭтоПолноправныйПользователь)
	...
	Если ПараметрыОграничения.ОграничениеОтключено Тогда
		Если Не ПередЗаписью
		   И ПараметрыОграничения.Свойство("ТекстЗапросаЭлементовДанныхСУстаревшимРазрешеннымКлючом") Тогда
	...
	Если ПараметрыОграничения.ДоступЗапрещен Тогда
		Если ЭтоПолноправныйПользователь Тогда
			Возврат;
		ИначеЕсли ПараметрыОграничения.ДляВнешнихПользователей Тогда
			ШаблонОшибки =
	...
	Если ЭтоНаборЗаписей Тогда
		ПроверитьДоступКНаборуЗаписей(Источник,
			ПередЗаписью, Замещение, ПараметрыОграничения, ЭтоПолноправныйПользователь);
	Иначе
		ПроверитьДоступКОбъекту(Источник,
			ПередЗаписью, ПараметрыОграничения, ЭтоПолноправныйПользователь);
	КонецЕсли;
	
КонецПроцедуры

// Для процедуры ПроверитьДоступКИсточнику.
Процедура ПроверитьДоступКОбъекту(Источник, ПередЗаписью, ПараметрыОграничения, ЭтоПолноправныйПользователь)
	...
	СообщитьОбОшибкеДоступа(Данные, ПередЗаписью, ПравоЧтения);
	
КонецПроцедуры

// Для процедуры ПроверитьДоступКИсточнику.
Процедура ПроверитьДоступКНаборуЗаписей(Источник, ПередЗаписью, Замещение, ПараметрыОграничения, ЭтоПолноправныйПользователь)
	
	Если    ПередЗаписью И (Не Замещение Или ЭтоПолноправныйПользователь)
	 Или Не ПередЗаписью И Источник.Количество() = 0
	 Или ЭтоПолноправныйПользователь
	   И ПараметрыОграничения.ИспользуетсяОграничениеПоВладельцу Тогда
	...
	Если Не ПередЗаписью
	   И Не ПараметрыОграничения.ИспользуетсяОграничениеПоВладельцу Тогда
...
Функция ПредставлениеДанных(Данные)
	...
	Иначе
		...
		Свойства = Новый Структура("РасширенноеПредставлениеОбъекта, ПредставлениеОбъекта");
		ЗаполнитьЗначенияСвойств(Свойства, ОбъектМетаданных);
		
		ПредставлениеМетаданных = Свойства.РасширенноеПредставлениеОбъекта;
		Если Не ЗначениеЗаполнено(ПредставлениеМетаданных) Тогда
			ПредставлениеМетаданных = Свойства.ПредставлениеОбъекта;
		КонецЕсли;
		Если Не ЗначениеЗаполнено(ПредставлениеМетаданных) Тогда
...
Процедура ОбновитьКлючиДоступаЭлементовДанныхПриЗаписи(ОписаниеЭлементовДанных, ПараметрыОграничения,
			ИдентификаторТранзакции, ОбновитьПраваНаКлючи = Ложь, ЕстьИзмененияПрав = Ложь) Экспорт
	...
	Иначе
		...
		КонтекстОбновления.Вставить("ЗапросЗначенийКлючей",
			Новый Запрос(ПараметрыОграничения.ТекстЗапросаЗначенийИзКлючейДоступаДляСравнения));
...
// Для процедур ЗапланироватьОбновлениеЗависимыхУстаревшихКлючейДоступа.
...
Процедура ЗапланироватьОбновлениеУстаревшихКлючейДоступа(СпискиДляОбновления, ИдентификаторТранзакции)
	...
	Контекст.Вставить("ИдентификаторТранзакции", ИдентификаторТранзакции);
	Контекст.Вставить("НаборЗаписей",            НаборЗаписей);
	Контекст.Вставить("КлючУникальности",        Новый УникальныйИдентификатор);
	Контекст.Вставить("ТекущаяДата",             ТекущаяДатаСеанса());
	Контекст.Вставить("МаксимальнаяДата",        МаксимальнаяДата());
	...
	Для Каждого ОписаниеСписка Из СпискиДляОбновления Цикл
		Контекст.ПолноеИмя = ОписаниеСписка.Ключ;
		Контекст.Список = ОбщегоНазначения.ИдентификаторОбъектаМетаданных(Контекст.ПолноеИмя);
		
		Если ОписаниеСписка.Значение.ДляПользователей Тогда
			ЗапланироватьОбновлениеУстаревшихКлючейДоступаДляВидаПользователей(Контекст, Ложь)
		КонецЕсли;
		
		Если ОписаниеСписка.Значение.ДляВнешнихПользователей Тогда
			ЗапланироватьОбновлениеУстаревшихКлючейДоступаДляВидаПользователей(Контекст, Истина)
		КонецЕсли;
	КонецЦикла;
...
// Для процедуры ЗапланироватьОбновлениеУстаревшихКлючейДоступа.
Процедура ЗапланироватьОбновлениеУстаревшихКлючейДоступаДляВидаПользователей(Контекст, ДляВнешнихПользователей)
	
	ПараметрыОграничения = ПараметрыОграничения(Контекст.ПолноеИмя,
		Контекст.ИдентификаторТранзакции, ДляВнешнихПользователей);
	...
	НоваяЗапись = Контекст.НаборЗаписей.Добавить();
	НоваяЗапись.КлючУникальности                   = Контекст.КлючУникальности;
	НоваяЗапись.Список                             = Контекст.Список;
	НоваяЗапись.ДляВнешнихПользователей            = ДляВнешнихПользователей;
	НоваяЗапись.ДатаПоследнегоОбновленногоЭлемента = Контекст.МаксимальнаяДата;
	НоваяЗапись.ДатаИзмененияЗаписиРегистра        = Контекст.ТекущаяДата;
...
Процедура ЗапланироватьОбновлениеДоступаПриИзмененииУчастниковГруппыДоступа(ГруппаДоступа, НовыеУчастники, СтарыеУчастники) Экспорт
	...
	Если ТипЗнч(СтарыеУчастники) = Тип("ТаблицаЗначений") Тогда
		Для Каждого Строка Из СтарыеУчастники Цикл
	...
	ЗапланироватьОбновлениеПользователейКлючейДоступа(Таблицы,
		ДляПользователей, ДляВнешнихПользователей, Истина);
...
// Для процедур ПослеДобавленияИзмененияПользователяИлиГруппы, ПослеОбновленияСоставовГруппПользователей.
Процедура ЗапланироватьОбновлениеДоступаПриКосвенномИзмененииУчастниковГруппыДоступа(ИзмененныеУчастники)
	...
	ИзмененныеВнешниеПользователи = Новый Массив;
	...
	Для Каждого Участник Из ИзмененныеУчастники Цикл
		Если ТипЗнч(Участник) = Тип("СправочникСсылка.Пользователи")
		 Или ТипЗнч(Участник) = Тип("СправочникСсылка.ГруппыПользователей") Тогда
			ДляПользователей = Истина;
		КонецЕсли;
		Если ТипЗнч(Участник) = Тип("СправочникСсылка.ВнешниеПользователи")
		...
		Если ТипЗнч(Участник) = Тип("СправочникСсылка.Пользователи") Тогда
			ИзмененныеПользователи.Добавить(Участник);
		КонецЕсли;
		Если ТипЗнч(Участник) = Тип("СправочникСсылка.ВнешниеПользователи") Тогда
			ИзмененныеВнешниеПользователи.Добавить(Участник);
		КонецЕсли;
	КонецЦикла;
	
	Если ИзмененныеПользователи.Количество() > 0 Тогда
		ОбновитьПользователейРазрешенногоКлючаДоступа(ИзмененныеПользователи);
	КонецЕсли;
	
	Если ИзмененныеВнешниеПользователи.Количество() > 0 Тогда
		ОбновитьВнешнихПользователейРазрешенногоКлючаДоступа(ИзмененныеВнешниеПользователи);
	КонецЕсли;
...
//
Процедура ЗапланироватьОбновлениеПользователейКлючейДоступа(ОписаниеСписков, ДляПользователей = Истина,
				ДляВнешнихПользователей = Истина, ПриИзмененииУчастниковГруппыДоступа = Ложь)
	
	Если ОписаниеСписков.Количество() = 0 Тогда
		Возврат;
	КонецЕсли;
	
	НаборЗаписей = СлужебныйНаборЗаписей(РегистрыСведений.ОбновлениеКлючейДоступаПользователей);
	...
	Иначе
		...
		ОбъектыМетаданныхСписков = Справочники.ИдентификаторыОбъектовМетаданных.ОбъектыМетаданныхПоИдентификаторам(
			ПолныеИмена, Истина);
	КонецЕсли;
	...
	Для Каждого ОписаниеСписка Из ОписаниеСписков Цикл
		...
		Если ДляПользователей Тогда
			ЗапланироватьОбновлениеПользователейКлючейДоступаДляВидаПользователей(Контекст, Ложь)
		КонецЕсли;
		Если ДляВнешнихПользователей Тогда
			ЗапланироватьОбновлениеПользователейКлючейДоступаДляВидаПользователей(Контекст, Истина)
		КонецЕсли;
	КонецЦикла;
...
Процедура ЗапланироватьОбновлениеПользователейКлючейДоступаДляВидаПользователей(Контекст, ДляВнешнихПользователей)
	
	Если ЗначениеЗаполнено(Контекст.ПолноеИмя) Тогда
		ДействующиеПараметры = ДействующиеПараметрыОграниченияДоступа(Контекст.ИдентификаторТранзакции);
		Если ДляВнешнихПользователей Тогда
...
Процедура ОбновлениеДоступаНаУровнеЗаписей() Экспорт
	...
	Если ТекущийСеанс.ИмяПриложения <> "BackgroundJob" Тогда
		ТекстОшибки = НСтр("ru = 'Обновление доступа может выполняться только в фоновом задании.'");
		ВызватьИсключение ТекстОшибки;
	КонецЕсли;
	...
	Попытка
		...
		КонецЕсли;
		Если ОграничиватьДоступНаУровнеЗаписейУниверсально() Тогда
			ПроверитьОбновитьДействующиеПараметрыОграниченияДоступа(ПоследнееОбновлениеДоступа);
	...
	Исключение
		...
		ЗарегистрироватьОшибкуОбновленияДоступа(ИнформацияОбОшибке);
	КонецПопытки;
...
// Для процедуры ОбновлениеДоступаНаУровнеЗаписей.
Процедура ПроверитьОбновитьДействующиеПараметрыОграниченияДоступа(ПоследнееОбновлениеДоступа)
	...
	ДействующиеПараметрыОграниченияДоступа(Неопределено);
	...
	Если ДатаОбновленияВерсииРасширений                         > ДатаАктуальностиПараметровОграниченияДоступа
	 ...
	 Или ДатаПоследнегоЗаполненияВсехПараметровРаботыРасширений > ДатаАктуальностиПараметровОграниченияДоступа Тогда
		
		ДействующиеПараметрыОграниченияДоступа(Неопределено, Ложь, Истина);
	КонецЕсли;
...
//
Процедура ВыполнитьОбновлениеДоступа()
	
	ИсполняющиеФоновыеЗадания = Неопределено;
	Если Не ОбщегоНазначения.ИнформационнаяБазаФайловая() Тогда
		...
		Если КоличествоПотоков > 1 Тогда
			...
			Для Счетчик = 2 По КоличествоПотоков Цикл
				ОписаниеЗадания = Новый Структура("ЭтоОбновлениеКлючейДоступаКДанным,
				|Задание, Список, ДляВнешнихПользователей, АдресРезультата");
	...
	|	МАКСИМУМ(Списки.ДатаИзмененияЗаписиРегистра) КАК ДатаИзменения
	|ИЗ
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	МАКСИМУМ(Списки.ДатаИзмененияЗаписиРегистра) УБЫВ";
	
	ОкончаниеВыполнения = ТекущаяУниверсальнаяДата()
	...
	Пока Истина Цикл
		...
		Если РезультатыЗапроса[0].Пустой()
		   И РезультатыЗапроса[1].Пустой() Тогда
			...
			Попытка
				ОтключитьРегламентноеЗаданиеЕслиНетНовыхЗаданий(Истина, ОшибкаОжиданияБлокировкиДанных);
			Исключение
		...
		МаксимальнаяДата = МаксимальнаяДата();
		ОбщиеПараметрыОбновления = Новый Структура("ЭтоОбновлениеКлючейДоступаКДанным,
		|ИдентификаторСписка, ДляВнешнихПользователей, ДатаНачала, ДатаОкончания");
		
		ПервыйИндекс = 0;
		ОбновлениеКлючейДоступаКДанным = ТаблицаОбновленияКлючейДоступаКДанным(РезультатыЗапроса[0],
			МаксимальнаяДата, ОбщиеПараметрыОбновления, ИсполняющиеФоновыеЗадания = Неопределено);
		
		Отбор = Новый Структура("ПоследняяДата", МаксимальнаяДата);
		ОбновлятьВсе = ОбновлениеКлючейДоступаКДанным.НайтиСтроки(Отбор).Количество() = 0;
		
		ВторойИндекс = 0;
		ОбновлениеКлючейДоступаПользователей = РезультатыЗапроса[1].Выгрузить();
		
		ОбработкаЗавершена = Истина;
		Пока Истина Цикл
			Если ТекущаяУниверсальнаяДата() > ОкончаниеВыполнения
			 Или ПервыйИндекс >= ОбновлениеКлючейДоступаКДанным.Количество()
			   И ВторойИндекс >= ОбновлениеКлючейДоступаПользователей.Количество() Тогда
				Прервать;
			КонецЕсли;
			Если ПервыйИндекс < ОбновлениеКлючейДоступаКДанным.Количество() Тогда
				Строка = ОбновлениеКлючейДоступаКДанным[ПервыйИндекс];
				ПервыйИндекс = ПервыйИндекс + 1;
				Если Не ОбновлятьВсе И Строка.ПоследняяДата <> МаксимальнаяДата Тогда
					Продолжить;
				КонецЕсли;
				ОбщиеПараметрыОбновления.ЭтоОбновлениеКлючейДоступаКДанным = Истина;
				ОбщиеПараметрыОбновления.ИдентификаторСписка = Строка.Список;
				ОбщиеПараметрыОбновления.ДляВнешнихПользователей = Строка.ДляВнешнихПользователей;
				ЗапуститьОбновлениеДоступаСписка(ИсполняющиеФоновыеЗадания, ОбщиеПараметрыОбновления, ОбработкаЗавершена);
			КонецЕсли;
			Если ВторойИндекс < ОбновлениеКлючейДоступаПользователей.Количество() Тогда
				Строка = ОбновлениеКлючейДоступаПользователей[ВторойИндекс];
				ВторойИндекс = ВторойИндекс + 1;
				ОбщиеПараметрыОбновления.ЭтоОбновлениеКлючейДоступаКДанным = Ложь;
				ОбщиеПараметрыОбновления.ИдентификаторСписка = Строка.Список;
				ОбщиеПараметрыОбновления.ДляВнешнихПользователей = Строка.ДляВнешнихПользователей;
				ЗапуститьОбновлениеДоступаСписка(ИсполняющиеФоновыеЗадания, ОбщиеПараметрыОбновления, ОбработкаЗавершена);
			КонецЕсли;
		...
		Если ИсполняющиеФоновыеЗадания <> Неопределено Тогда
...
Функция ТаблицаОбновленияКлючейДоступаКДанным(РезультатЗапроса, МаксимальнаяДата, ОбщиеПараметрыОбновления, ОбновлениеВЭтомСеансе)
	...
	Если ОбновлениеВЭтомСеансе Тогда
		ИдентификаторыСписков = Новый Массив;
		Если Не ОбщиеПараметрыОбновления.Свойство("ОбъектыМетаданныхПоИдентификаторам") Тогда
			ОбщиеПараметрыОбновления.Вставить("ОбъектыМетаданныхПоИдентификаторам", Новый Соответствие);
		КонецЕсли;
	КонецЕсли;
	...
	Для Каждого Строка Из Выгрузка Цикл
		...
		КонецЕсли;
		Если ОбновлениеВЭтомСеансе
		   И ОбщиеПараметрыОбновления.ОбъектыМетаданныхПоИдентификаторам.Получить(Строка.Список) = Неопределено Тогда
			
			ИдентификаторыСписков.Добавить(Строка.Список);
		КонецЕсли;
	...
	КонецЦикла;
	Выгрузка.Сортировать("УровеньЗависимости Возр, ПоследняяДата Убыв, ДатаИзменения Убыв");
	
	Если ОбновлениеВЭтомСеансе И ИдентификаторыСписков.Количество() > 0 Тогда
		ОбъектыМетаданныхПоИдентификаторам =
			Справочники.ИдентификаторыОбъектовМетаданных.ОбъектыМетаданныхПоИдентификаторам(ИдентификаторыСписков, Истина);
		Для Каждого КлючИЗначение Из ОбъектыМетаданныхПоИдентификаторам Цикл
			ОбщиеПараметрыОбновления.ОбъектыМетаданныхПоИдентификаторам.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
		КонецЦикла;
	КонецЕсли;
...
Процедура УстановитьНедоступныеСписки(РезультатЗапроса, НедоступныеСписки)
	...
	ОбъектыМетаданныхПоИдентификаторам =
		Справочники.ИдентификаторыОбъектовМетаданных.ОбъектыМетаданныхПоИдентификаторам(
			РезультатЗапроса.Выгрузить().ВыгрузитьКолонку("Список"), Истина);
...
// Для процедуры ВыполнитьОбновлениеДоступа.
Процедура ЗапуститьОбновлениеДоступаСписка(ИсполняющиеФоновыеЗадания, ОбщиеПараметрыОбновления, ОбработкаЗавершена)
	
	Если ИсполняющиеФоновыеЗадания = Неопределено Тогда
		ВыполнитьОбновлениеДоступаСписка(ОбщиеПараметрыОбновления, ОбработкаЗавершена);
	Иначе
		Пока Не ЗапущеноОбновлениеДоступаСпискаВФоне(ИсполняющиеФоновыеЗадания,
					ОбщиеПараметрыОбновления, ОбработкаЗавершена) Цикл
			Попытка
...
// Для процедуры ВыполнитьОбновлениеДоступа.
Функция ЗапущеноОбновлениеДоступаСпискаВФоне(ИсполняющиеФоновыеЗадания, ОбщиеПараметрыОбновления, ОбработкаЗавершена)
...
Процедура ЗавершитьФоновыеЗадания(ИсполняющиеФоновыеЗадания, ОбработкаЗавершена)
	...
	Для Каждого ОписаниеЗадания Из ИсполняющиеФоновыеЗадания Цикл
		...
		Если ОписаниеЗадания.Задание.Состояние = СостояниеФоновогоЗадания.Активно Тогда
			...
			Исключение
				ЗарегистрироватьОшибкуОбновленияДоступа(ИнформацияОбОшибке(), ОписаниеЗадания);
				Продолжить;
			КонецПопытки;
		КонецЕсли;
		ОбработатьЗавершенноеФоновоеЗадание(ОписаниеЗадания, ОбработкаЗавершена);
...
// Для функции ЗапущеноОбновлениеДоступаСпискаВФоне и процедуры ЗавершитьФоновыеЗадания.
Процедура ОбработатьЗавершенноеФоновоеЗадание(ОписаниеЗадания, ОбработкаЗавершена)
	...
	Если Задание.Состояние = СостояниеФоновогоЗадания.ЗавершеноАварийно Тогда
		ЗарегистрироватьОшибкуОбновленияДоступа(Задание.ИнформацияОбОшибке, ОписаниеЗадания);
	КонецЕсли;
	
КонецПроцедуры

// Для функции ЗапущеноОбновлениеДоступаСпискаВФоне.
Процедура ВыполнитьОбновлениеДоступаСпискаВФоне(ОбщиеПараметрыОбновления, АдресРезультата) Экспорт
	
	ОбработкаЗавершена = Истина;
	Попытка
		ВыполнитьОбновлениеДоступаСписка(ОбщиеПараметрыОбновления, ОбработкаЗавершена);
	Исключение
		ЗарегистрироватьОшибкуОбновленияДоступа(ИнформацияОбОшибке(), ОбщиеПараметрыОбновления);
		ОбработкаЗавершена = Ложь;
	КонецПопытки;
...
// ОбработатьЗавершенноеФоновоеЗадание, ВыполнитьОбновлениеДоступаСпискаВФоне.
//
Процедура ЗарегистрироватьОшибкуОбновленияДоступа(ИнформацияОбОшибке, ОписаниеЗадания = Неопределено)
	...
	ИначеЕсли ОписаниеЗадания.ЭтоОбновлениеКлючейДоступаКДанным Тогда
		
		Если ОписаниеЗадания.ДляВнешнихПользователей Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обновить порцию ключей доступа к данным списка
	...
	Иначе // ОбновлениеКлючейДоступаПользователей.
		...
		Если ОписаниеЗадания.ДляВнешнихПользователей Тогда
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обновить порцию ключей доступа внешних пользователей
				           |для списка ""%1""
				           |по причине:
				           |%2'"),
				Строка(ОписаниеЗадания.Список),
		...
		Иначе
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
				НСтр("ru = 'Не удалось обновить порцию ключей доступа пользователей
				           |для списка ""%1""
				           |по причине:
				           |%2'"),
				Строка(ОписаниеЗадания.Список),
...
// Для функции ЗапущеноОбновлениеДоступаСпискаВФоне.
Процедура ВыполнитьОбновлениеДоступаСписка(ОбщиеПараметрыОбновления, ОбработкаЗавершена)
	
	Если ОбщиеПараметрыОбновления.Свойство("ОбъектыМетаданныхПоИдентификаторам") Тогда
		ОбъектМетаданных = ОбщиеПараметрыОбновления.ОбъектыМетаданныхПоИдентификаторам.Получить(
	...
	Иначе
		ОбъектМетаданных = Справочники.ИдентификаторыОбъектовМетаданных.ОбъектМетаданныхПоИдентификатору(
			ОбщиеПараметрыОбновления.ИдентификаторСписка, Истина);
	КонецЕсли;
	...
	Если ОбщиеПараметрыОбновления.ЭтоОбновлениеКлючейДоступаКДанным Тогда
		ОбновитьКлючиДоступаКДаннымСписка(ОбъектМетаданных, ОбщиеПараметрыОбновления, ОбработкаЗавершена);
	Иначе
		ОбновитьКлючиДоступаПользователейСписка(ОбъектМетаданных, ОбщиеПараметрыОбновления, ОбработкаЗавершена);
	КонецЕсли;
	
КонецПроцедуры

// Для процедур ВыполнитьОбновлениеДоступаСписка.
Процедура ОбновитьКлючиДоступаКДаннымСписка(ОбъектМетаданных, ОбщиеПараметрыОбновления, ОбработкаЗавершена)
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Список", ОбщиеПараметрыОбновления.ИдентификаторСписка);
	Запрос.УстановитьПараметр("ДляВнешнихПользователей", ОбщиеПараметрыОбновления.ДляВнешнихПользователей);
	Запрос.Текст =
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	...
	Если ТипЗнч(ОбъектМетаданных) <> Тип("ОбъектМетаданных") Тогда
		...
		ПустойНабор = СлужебныйНаборЗаписей(РегистрыСведений.ОбновлениеКлючейДоступаКДанным);
		ПустойНабор.Отбор.Список.Установить(ОбщиеПараметрыОбновления.ИдентификаторСписка);
	...
	Иначе
		...
		ПланОбновленияКлючей = СлужебныйНаборЗаписей(РегистрыСведений.ОбновлениеКлючейДоступаКДанным);
		ПланОбновленияКлючей.Отбор.Список.Установить(ОбщиеПараметрыОбновления.ИдентификаторСписка);
		ПланОбновленияКлючей.Отбор.ДляВнешнихПользователей.Установить(ОбщиеПараметрыОбновления.ДляВнешнихПользователей);
		ПланОбновленияКлючей.Отбор.КлючУникальности.Установить(
			Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000"));
		
		Запись = ПланОбновленияКлючей.Добавить();
		Запись.Список                  = ОбщиеПараметрыОбновления.ИдентификаторСписка;
		Запись.ДляВнешнихПользователей = ОбщиеПараметрыОбновления.ДляВнешнихПользователей;
	...
	ПараметрыОграничения = ПараметрыОграничения(ПолноеИмя, ИдентификаторТранзакции, ОбщиеПараметрыОбновления.ДляВнешнихПользователей);
	
	ПараметрыОбновления = Новый Структура(ПараметрыОграничения);
	ПараметрыОбновления.Вставить("ИдентификаторСписка", ОбщиеПараметрыОбновления.ИдентификаторСписка);
	ПараметрыОбновления.Вставить("ИдентификаторТранзакции", ИдентификаторТранзакции);
	ИмяСвойстваВидаПользователей = ?(ОбщиеПараметрыОбновления.ДляВнешнихПользователей,
		"ДляВнешнихПользователей", "ДляПользователей");
	...
	ПараметрыОбновления.Вставить("ДатаНачала",    ОбщиеПараметрыОбновления.ДатаНачала);
	ПараметрыОбновления.Вставить("ДатаОкончания", ОбщиеПараметрыОбновления.ДатаОкончания);
...
// Для процедуры ОбновитьЭлементыДанныхСпискаСУстаревшимРазрешеннымКлючом.
Процедура ОбновитьПраваНаРазрешенныйКлючДоступа(ЕстьИзменения = Ложь)
	...
	ОбновитьПользователейРазрешенногоКлючаДоступа(, ЕстьИзменения);
	ОбновитьВнешнихПользователейРазрешенногоКлючаДоступа(, ЕстьИзменения);
...
Процедура ОбновитьГруппыДоступаРазрешенногоКлючаДоступа(ГруппыДоступа = Неопределено, ЕстьИзменения = Ложь) Экспорт
	...
	|	&РазрешенныйКлючДоступа КАК КлючДоступа,
	...
	Поля.Добавить(Новый Структура("КлючДоступа",   "&УсловиеОтбораРазрешенногоКлючаДоступа"));
	Поля.Добавить(Новый Структура("Изменение"));
	
	РазрешенныйКлючДоступа = УправлениеДоступомСлужебныйПовтИсп.РазрешенныйКлючДоступа();
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("РазрешенныйКлючДоступа", РазрешенныйКлючДоступа);
	Запрос.Текст = ТекстЗапросаВыбораИзменений(ТекстЗапроса,
		Поля, "РегистрСведений.КлючиДоступаГруппДоступа", ТекстЗапросовВременныхТаблиц);
	
	УстановитьУсловиеОтбораВЗапросе(Запрос, ГруппыДоступа, "ГруппыДоступа",
		"&УсловиеОтбораГруппДоступа1:ГруппыДоступа.Ссылка
		|&УсловиеОтбораГруппДоступа2:СтарыеДанные.ГруппаДоступа");
		
	УстановитьУсловиеОтбораВЗапросе(Запрос, РазрешенныйКлючДоступа, "КлючДоступа",
		"&УсловиеОтбораРазрешенногоКлючаДоступа:СтарыеДанные.КлючДоступа");
	...
	Попытка
		...
		Данные = Новый Структура;
		Данные.Вставить("МенеджерРегистра",      РегистрыСведений.КлючиДоступаГруппДоступа);
		Данные.Вставить("ИзмененияСоставаСтрок", Запрос.Выполнить().Выгрузить());
		Данные.Вставить("ИзмеренияОтбора",       ИзмеренияОтбора);
		Данные.Вставить("ФиксированныйОтбор",    Новый Структура("КлючДоступа", РазрешенныйКлючДоступа));
...
// Для процедуры ОбновитьПраваНаРазрешенныйКлючДоступа.
Процедура ОбновитьПользователейРазрешенногоКлючаДоступа(Пользователь = Неопределено, ЕстьИзменения = Ложь)
	...
	// Подготовка выбираемых полей с необязательным отбором.
	Поля = Новый Массив; 
	Поля.Добавить(Новый Структура("Пользователь", "&УсловиеОтбораПользователей2"));
	Поля.Добавить(Новый Структура("КлючДоступа",  "&УсловиеОтбораРазрешенногоКлючаДоступа"));
	Поля.Добавить(Новый Структура("Изменение"));
	...
	Запрос.УстановитьПараметр("РазрешенныйКлючДоступа", РазрешенныйКлючДоступа);
	Запрос.Текст = ТекстЗапросаВыбораИзменений(ТекстЗапроса,
		Поля, "РегистрСведений.КлючиДоступаПользователей", ТекстЗапросовВременныхТаблиц);
	
	УстановитьУсловиеОтбораВЗапросе(Запрос, Пользователь, "Пользователь",
		"&УсловиеОтбораПользователей1:СоставыГруппПользователей.Пользователь
		|&УсловиеОтбораПользователей2:СтарыеДанные.Пользователь");
	...
	Попытка
		Блокировка.Заблокировать();
		Если Пользователь <> Неопределено Тогда
			ИзмеренияОтбора = "Пользователь";
		Иначе
			ИзмеренияОтбора = Неопределено;
		КонецЕсли;
		
		Данные = Новый Структура;
		Данные.Вставить("МенеджерРегистра",      РегистрыСведений.КлючиДоступаПользователей);
		Данные.Вставить("ИзмененияСоставаСтрок", Запрос.Выполнить().Выгрузить());
		Данные.Вставить("ИзмеренияОтбора",       ИзмеренияОтбора);
		Данные.Вставить("ФиксированныйОтбор",    Новый Структура("КлючДоступа", РазрешенныйКлючДоступа));
...
// Для процедуры ОбновитьПраваНаРазрешенныйКлючДоступа.
Процедура ОбновитьВнешнихПользователейРазрешенногоКлючаДоступа(ВнешнийПользователь = Неопределено, ЕстьИзменения = Ложь)
	...
	ТекстЗапроса =
	"ВЫБРАТЬ
	|	НовыеДанные.ВнешнийПользователь,
	|	НовыеДанные.КлючДоступа,
	|	НовыеДанные.Изменение,
	|	&ПодстановкаПоляВидИзмененияСтроки
	|ИЗ
	...
	// Подготовка выбираемых полей с необязательным отбором.
	Поля = Новый Массив; 
	Поля.Добавить(Новый Структура("ВнешнийПользователь", "&УсловиеОтбораВнешнихПользователей2"));
	Поля.Добавить(Новый Структура("КлючДоступа",         "&УсловиеОтбораРазрешенногоКлючаДоступа"));
	Поля.Добавить(Новый Структура("Изменение"));
	...
	Запрос.УстановитьПараметр("РазрешенныйКлючДоступа", РазрешенныйКлючДоступа);
	Запрос.Текст = ТекстЗапросаВыбораИзменений(ТекстЗапроса,
		Поля, "РегистрСведений.КлючиДоступаВнешнихПользователей", ТекстЗапросовВременныхТаблиц);
	
	УстановитьУсловиеОтбораВЗапросе(Запрос, ВнешнийПользователь, "ВнешнийПользователь",
		"&УсловиеОтбораВнешнихПользователей1:СоставыГруппПользователей.Пользователь
		|&УсловиеОтбораВнешнихПользователей2:СтарыеДанные.ВнешнийПользователь");
	...
	Попытка
		...
		Если ВнешнийПользователь <> Неопределено Тогда
			ИзмеренияОтбора = "ВнешнийПользователь";
		Иначе
			ИзмеренияОтбора = Неопределено;
		КонецЕсли;
		
		Данные = Новый Структура;
		Данные.Вставить("МенеджерРегистра",      РегистрыСведений.КлючиДоступаВнешнихПользователей);
		Данные.Вставить("ИзмененияСоставаСтрок", Запрос.Выполнить().Выгрузить());
		Данные.Вставить("ИзмеренияОтбора",       ИзмеренияОтбора);
		Данные.Вставить("ФиксированныйОтбор",    Новый Структура("КлючДоступа", РазрешенныйКлючДоступа));
...
Процедура ОбновитьЭлементыДанныхСпискаСУстаревшимиКлючами(ЭлементыДанных, ПараметрыОбновления)
	...
	ЗапросЗначенийКлючей.Текст = ПараметрыОбновления.ТекстЗапросаЗначенийИзКлючейДоступаДляСравнения;
...
Процедура ОбновитьКлючиДоступаПорцииЭлементовДанныхСписка(ПараметрыОбновления, Контекст)
	...
	ПараметрыОбновления.Вставить("ДопустимыеТипыЗначенийКлючейДоступа",
		УправлениеДоступомСлужебныйПовтИсп.ДопустимыеТипыЗначенийКлючейДоступа());
	
	НовыеКлючиДоступа = Новый Массив;
	Попытка
		...
		Для Каждого ОписаниеКлюча Из ТребуемыеКлючиДоступа Цикл
			Если ОписаниеКлюча.КлючДоступа <> Неопределено Тогда
				Продолжить;
			КонецЕсли;
			ПроверитьТипЗначенийКлючаДоступа(ОписаниеКлюча, ПараметрыОбновления);
			НовыйКлючДоступа = НовыйКлючДоступа(ОписаниеКлюча, ПараметрыОбновления);
			НовыеКлючиДоступа.Добавить(НовыйКлючДоступа);
			ОписаниеКлюча.КлючДоступа = НовыйКлючДоступа;
		КонецЦикла;
	Исключение
		Если Не ТранзакцияАктивна() Тогда
			ПараметрыПланирования = ПараметрыПланированияОбновленияДоступа();
			ПараметрыПланирования.КлючиДоступаКДанным = Ложь;
			ЗапланироватьОбновлениеДоступа(ПараметрыОбновления.Список, ПараметрыПланирования);
		КонецЕсли;
		ВызватьИсключение;
	КонецПопытки;
	...
	Если ПараметрыОбновления.Свойство("ОбновитьПраваНаКлючи")
	   И ПараметрыОбновления.ОбновитьПраваНаКлючи Тогда
		...
		ВсеКлючиДоступаПорции = Новый Массив;
		Для Каждого ОписаниеКлюча Из ТребуемыеКлючиДоступа Цикл
			ВсеКлючиДоступаПорции.Добавить(ОписаниеКлюча.КлючДоступа);
		КонецЦикла;
		ОбновитьПраваНаКлючиДоступа(ВсеКлючиДоступаПорции, ПараметрыОбновления,
			НовыеКлючиДоступа.Количество() > 0);
		
	ИначеЕсли НовыеКлючиДоступа.Количество() > 0 Тогда
		ОбновитьПраваНаКлючиДоступа(НовыеКлючиДоступа, ПараметрыОбновления, Истина);
	КонецЕсли;
...
Процедура ОбновитьПраваНаКлючиДоступа(КлючиДоступа, ПараметрыОбновления, НовыеКлючи = Ложь)
	...
	Запрос.Текст = СтрЗаменить(ПараметрыОбновления.ТекстЗапросаЗначенийИзКлючейДоступаДляРасчетаПрав,
		"И КлючиДоступа.Ссылка > &ПоследнийКлючДоступа",
		"И КлючиДоступа.Ссылка В (&КлючиДоступа)");
	
	Попытка
		ОбновитьПользователейПорцииКлючейДоступаСписка(ПараметрыОбновления, Запрос);
	Исключение
		Если НовыеКлючи И Не ТранзакцияАктивна() Тогда
			ПараметрыПланирования = ПараметрыПланированияОбновленияДоступа();
			ПараметрыПланирования.КлючиДоступаКДанным = Ложь;
			ЗапланироватьОбновлениеДоступа(ПараметрыОбновления.Список, ПараметрыПланирования);
		КонецЕсли;
...
// Для процедуры ОбновитьКлючиДоступаПорцииЭлементовДанных.
Процедура ПроверитьТипЗначенийКлючаДоступа(ОписаниеКлюча, ПараметрыОбновления)
	
	ДопустимыеТипыЗначенийКлючейДоступа = ПараметрыОбновления.ДопустимыеТипыЗначенийКлючейДоступа;
	...
	Для Каждого ЗначенияКолонокТаблицы Из ЗначенияКолонокТаблиц Цикл
		Для Каждого ЗначенияКолонкиТаблицы Из ЗначенияКолонокТаблицы Цикл
			Для Каждого Значение Из ЗначенияКолонкиТаблицы Цикл
				Если Не ДопустимыеТипыЗначенийКлючейДоступа.СодержитТип(ТипЗнч(Значение)) Тогда
					ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
...
// Для процедур ВыполнитьОбновлениеДоступаСписка.
Процедура ОбновитьКлючиДоступаПользователейСписка(ОбъектМетаданных, ОбщиеПараметрыОбновления, ОбработкаЗавершена)
	...
	"ВЫБРАТЬ ПЕРВЫЕ 1000
	|	КлючиУникальности.КлючУникальности КАК КлючУникальности,
	|	КлючиУникальности.ПоследнийОбновленныйКлючДоступа КАК ПоследнийОбновленныйКлючДоступа
	|ИЗ
	...
	Если ТипЗнч(ОбъектМетаданных) <> Тип("ОбъектМетаданных") Тогда
		// Очистка записей не существующих объектов метаданных.
		ПустойНабор = СлужебныйНаборЗаписей(РегистрыСведений.ОбновлениеКлючейДоступаПользователей);
		ПустойНабор.Отбор.Список.Установить(ОбщиеПараметрыОбновления.ИдентификаторСписка);
		ПустойНабор.Записать();
		Возврат;
	КонецЕсли;
	...
	Если Выгрузка.Количество() = 1
	   И Не ЗначениеЗаполнено(Выгрузка[0].КлючУникальности) Тогда
		...
		ПоследнийОбновленныйКлючДоступа = Выгрузка[0].ПоследнийОбновленныйКлючДоступа.Получить();
	Иначе
		ПоследнийОбновленныйКлючДоступа = Неопределено;
		
		ПланОбновленияКлючей = СлужебныйНаборЗаписей(РегистрыСведений.ОбновлениеКлючейДоступаПользователей);
		ПланОбновленияКлючей.Отбор.Список.Установить(ОбщиеПараметрыОбновления.ИдентификаторСписка);
		ПланОбновленияКлючей.Отбор.ДляВнешнихПользователей.Установить(ОбщиеПараметрыОбновления.ДляВнешнихПользователей);
		ПланОбновленияКлючей.Отбор.КлючУникальности.Установить(
			Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000"));
	...
	СвойстваСпискаКакВедущего = СвойстваСпискаКакВедущего(ПолноеИмя, ИдентификаторТранзакции);
	ПараметрыОграничения = ПараметрыОграничения(ПолноеИмя, ИдентификаторТранзакции,
		ОбщиеПараметрыОбновления.ДляВнешнихПользователей);
	...
	КонецЕсли;
	ПараметрыОбновления.Вставить("Кэш", ОбщиеПараметрыОбновления.Кэш);
	ИмяСвойстваВидаПользователей = ?(ОбщиеПараметрыОбновления.ДляВнешнихПользователей,
		"ДляВнешнихПользователей", "ДляПользователей");
	...
	ПараметрыОбновления.Вставить("ПоследнийОбновленныйКлючДоступа", ПоследнийОбновленныйКлючДоступа);
	
	ОбновитьПользователейКлючейДоступаСписка(ПараметрыОбновления);
	...
	Если ПараметрыОбновления.Свойство("НовыйПоследнийОбновленныйКлючДоступа") Тогда
		Запись = ПланОбновленияКлючей.Добавить();
		Запись.Список                          = ОбщиеПараметрыОбновления.ИдентификаторСписка;
		Запись.ДляВнешнихПользователей         = ОбщиеПараметрыОбновления.ДляВнешнихПользователей;
		Запись.ДатаИзмененияЗаписиРегистра     = ТекущаяДатаСеанса();
		Запись.ПоследнийОбновленныйКлючДоступа = Новый ХранилищеЗначения(
			ПараметрыОбновления.НовыйПоследнийОбновленныйКлючДоступа);
...
// Для процедуры ОбновитьКлючиДоступаКДаннымСписка.
Процедура ОбновитьПользователейКлючейДоступаСписка(ПараметрыОбновления)
	
	Если ПараметрыОбновления.ОграничениеОтключено
	 Или ПараметрыОбновления.ДоступЗапрещен
	 Или ПараметрыОбновления.ИспользуетсяОграничениеПоВладельцу Тогда
		
		УдалитьПорциюКлючейДоступаСписка(ПараметрыОбновления);
		Возврат;
	КонецЕсли;
	...
	НовыйПоследнийОбновленныйКлючДоступа = Неопределено;
	Если ОбщегоНазначения.ЭтоСсылка(ТипЗнч(ПараметрыОбновления.ПоследнийОбновленныйКлючДоступа)) Тогда
		Запрос.УстановитьПараметр("ПоследнийКлючДоступа", ПараметрыОбновления.ПоследнийОбновленныйКлючДоступа);
	Иначе
		Запрос.УстановитьПараметр("ПоследнийКлючДоступа", Неопределено);
	КонецЕсли;
...
// Для процедуры ОбновитьКлючиДоступаКДаннымСписка.
Процедура ОбновитьПользователейПорцииКлючейДоступаСписка(ПараметрыОбновления, Запрос)
	
	Запрос.УстановитьПараметр("Список", ПараметрыОбновления.ИдентификаторСписка);
	Запрос.УстановитьПараметр("ИдентификаторТаблицыНастроекПрав", ПараметрыОбновления.ИдентификаторТаблицыНастроекПрав);
	Запрос.УстановитьПараметр("ПустойУникальныйИдентификатор",
		Новый УникальныйИдентификатор("00000000-0000-0000-0000-000000000000"));
	
	Если ПараметрыОбновления.ВладельцыСПравамиДля <> Неопределено Тогда
		УстановитьПараметрВладельцевСПравами(Запрос, "ВладельцыСПравамиПользователей",
			ПараметрыОбновления.ВладельцыСПравамиДля.Пользователей, ПараметрыОбновления.Кэш);
		
		УстановитьПараметрВладельцевСПравами(Запрос, "ВладельцыСПравамиГруппДоступа",
			ПараметрыОбновления.ВладельцыСПравамиДля.ГруппДоступа, ПараметрыОбновления.Кэш);
	КонецЕсли;
	...
	Для ИндексКлюча = 0 По ИндексПоследнегоКлюча Цикл
		...
		ОбновитьПраваКлючаДоступаСписка(ПараметрыОбновления, ЗначенияТаблицКлюча, КлючДоступа);
	КонецЦикла;
	
	ПоследнийКлючДоступа = ЗначенияПервойТаблицы.Строки[ИндексПоследнегоКлюча].Ссылка;
	
	ПараметрыОбновления.Вставить("НовыйПоследнийОбновленныйКлючДоступа", ПоследнийКлючДоступа);
	
КонецПроцедуры

// Для процедуры ОбновитьПользователейПорцииКлючейДоступаСписка.
Процедура УстановитьПараметрВладельцевСПравами(Запрос, ИмяПараметра, ПолныеИмена, Кэш)
	
	Если Не Кэш.Свойство(ИмяПараметра) Тогда
		ИдентификаторыПоПолнымИменам =
			Справочники.ИдентификаторыОбъектовМетаданных.ИдентификаторыОбъектовМетаданных(ПолныеИмена, Истина);
		...
		Кэш.Вставить(ИмяПараметра, Идентификаторы);
	КонецЕсли;
	
	Запрос.УстановитьПараметр(ИмяПараметра, Кэш[ИмяПараметра]);
	
КонецПроцедуры

// Для процедуры ОбновитьПользователейКлючейДоступаСписка.
Процедура УдалитьПорциюКлючейДоступаСписка(ПараметрыОбновления)
	...
	|		ГДЕ
	|			КлючиДоступаГруппДоступа.КлючДоступа = КлючиДоступа.Ссылка) КАК ОчиститьКлючиДоступаГруппДоступа,
	|	ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	|		ГДЕ
	|			КлючиДоступаПользователей.КлючДоступа = КлючиДоступа.Ссылка) КАК ОчиститьКлючиДоступаПользователей,
	|	ИСТИНА В
	|		(ВЫБРАТЬ ПЕРВЫЕ 1
	|			ИСТИНА
	|		ИЗ
	...
	|		ГДЕ
	|			КлючиДоступаВнешнихПользователей.КлючДоступа = КлючиДоступа.Ссылка) КАК ОчиститьКлючиДоступаВнешнихПользователей
	|ИЗ
	|	Справочник.КлючиДоступа КАК КлючиДоступа
	|ГДЕ
	...
	|
	|УПОРЯДОЧИТЬ ПО
	|	КлючиДоступа.ПометкаУдаления УБЫВ,
	|	КлючиДоступа.Ссылка";
	...
	Контекст = Новый Структура;
	Контекст.Вставить("КлючиДоступаГруппДоступа",
		СлужебныйНаборЗаписей(РегистрыСведений.КлючиДоступаГруппДоступа));
	Контекст.Вставить("КлючиДоступаПользователей",
		СлужебныйНаборЗаписей(РегистрыСведений.КлючиДоступаПользователей));
	Контекст.Вставить("КлючиДоступаВнешнихПользователей",
		СлужебныйНаборЗаписей(РегистрыСведений.КлючиДоступаВнешнихПользователей));
	
	ВсеУдалено = Истина;
	ПорцияКлючей = Новый Массив;
	Для Каждого Строка Из Выгрузка Цикл
		Если ПорцияКлючей.Количество() < 50 Тогда
			ПорцияКлючей.Добавить(Строка);
			Продолжить;
		КонецЕсли;
		УдалитьТекущуюПорциюКлючейДоступаСписка(Контекст, ПорцияКлючей, ВсеУдалено);
		ПорцияКлючей = Новый Массив;
	КонецЦикла;
	
	Если ПорцияКлючей.Количество() > 0 Тогда
		УдалитьТекущуюПорциюКлючейДоступаСписка(Контекст, ПорцияКлючей, ВсеУдалено);
	КонецЕсли;
	
	Если Не ВсеУдалено Или Выгрузка.Количество() = 1000 Тогда
		ПараметрыОбновления.Вставить("НовыйПоследнийОбновленныйКлючДоступа", Неопределено);
	КонецЕсли;
	
КонецПроцедуры

// Для процедуры УдалитьПорциюКлючейДоступаСписка.
Процедура УдалитьТекущуюПорциюКлючейДоступаСписка(Контекст, ПорцияКлючей, ВсеУдалено)
	
	Блокировка = Новый БлокировкаДанных;
	Для Каждого ОписаниеКлюча Из ПорцияКлючей Цикл
		ЭлементБлокировки = Блокировка.Добавить("Справочник.КлючиДоступа");
		ЭлементБлокировки.УстановитьЗначение("Ссылка", ОписаниеКлюча.Ссылка);
		Если ОписаниеКлюча.ОчиститьКлючиДоступаГруппДоступа Тогда
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.КлючиДоступаГруппДоступа");
			ЭлементБлокировки.УстановитьЗначение("КлючДоступа", ОписаниеКлюча.Ссылка);
		КонецЕсли;
		Если ОписаниеКлюча.ОчиститьКлючиДоступаПользователей Тогда
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.КлючиДоступаПользователей");
			ЭлементБлокировки.УстановитьЗначение("КлючДоступа", ОписаниеКлюча.Ссылка);
		КонецЕсли;
		Если ОписаниеКлюча.ОчиститьКлючиДоступаВнешнихПользователей Тогда
			ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.КлючиДоступаВнешнихПользователей");
			ЭлементБлокировки.УстановитьЗначение("КлючДоступа", ОписаниеКлюча.Ссылка);
		КонецЕсли;
	КонецЦикла;
	...
	Попытка
		...
		БлокировкаУстановлена = Истина;
		Для Каждого ОписаниеКлюча Из ПорцияКлючей Цикл
			КлючДоступаОбъект = ОписаниеКлюча.Ссылка.ПолучитьОбъект();
			Если КлючДоступаОбъект <> Неопределено Тогда
				ОбновлениеИнформационнойБазы.УдалитьДанные(КлючДоступаОбъект, Ложь, Ложь);
			КонецЕсли;
			Если ОписаниеКлюча.ОчиститьКлючиДоступаГруппДоступа Тогда
				Контекст.КлючиДоступаГруппДоступа.Отбор.КлючДоступа.Установить(ОписаниеКлюча.Ссылка);
				Контекст.КлючиДоступаГруппДоступа.Записать();
			КонецЕсли;
			Если ОписаниеКлюча.ОчиститьКлючиДоступаПользователей Тогда
				Контекст.КлючиДоступаПользователей.Отбор.КлючДоступа.Установить(ОписаниеКлюча.Ссылка);
				Контекст.КлючиДоступаГруппДоступа.Записать();
			КонецЕсли;
			Если ОписаниеКлюча.ОчиститьКлючиДоступаВнешнихПользователей Тогда
				Контекст.КлючиДоступаВнешнихПользователей.Отбор.КлючДоступа.Установить(ОписаниеКлюча.Ссылка);
				Контекст.КлючиДоступаГруппДоступа.Записать();
			КонецЕсли;
		КонецЦикла;
		ЗафиксироватьТранзакцию();
...
Процедура ЗаполнитьПраваИУчастниковГруппДоступаСписка(ПараметрыОбновления)
	...
	|	РегистрСведений.ТаблицыГруппДоступа КАК ТаблицыГруппДоступа
	|ГДЕ
	|	ТаблицыГруппДоступа.Таблица = &Таблица";
	...
	|ИЗ
	|	РегистрСведений.ТаблицыГруппДоступа КАК ТаблицыГруппДоступа
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ГруппыДоступа.Пользователи КАК ПользователиГруппыДоступа
	|		ПО (ПользователиГруппыДоступа.Ссылка = ТаблицыГруппДоступа.ГруппаДоступа)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоставыГруппПользователей КАК ПользователиГруппПользователей
	|		ПО (ПользователиГруппПользователей.ГруппаПользователей = ПользователиГруппыДоступа.Пользователь)
	|			И (ПользователиГруппПользователей.Используется)
	|			И (ПользователиГруппПользователей.Пользователь.ИдентификаторПользователяИБ <> &ПустойУникальныйИдентификатор)
	|ГДЕ
	|	ТаблицыГруппДоступа.Таблица = &Таблица
	|	И НЕ ПользователиГруппыДоступа.Ссылка ЕСТЬ NULL
	|	И НЕ ПользователиГруппПользователей.Пользователь ЕСТЬ NULL
	|ИТОГИ ПО
	|	ГруппаДоступа
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ПользователиГруппПользователей.ГруппаПользователей КАК ГруппаПользователей,
	|	ПользователиГруппПользователей.Пользователь КАК Пользователь
	|ИЗ
	|	РегистрСведений.ТаблицыГруппДоступа КАК ТаблицыГруппДоступа
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ГруппыДоступа.Пользователи КАК ПользователиГруппыДоступа
	|		ПО (ПользователиГруппыДоступа.Ссылка = ТаблицыГруппДоступа.ГруппаДоступа)
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоставыГруппПользователей КАК ПользователиГруппПользователей
	|		ПО (ПользователиГруппПользователей.ГруппаПользователей = ПользователиГруппыДоступа.Пользователь)
	|			И (ПользователиГруппПользователей.Пользователь <> ПользователиГруппыДоступа.Пользователь)
	|			И (ПользователиГруппПользователей.Используется)
	|			И (ПользователиГруппПользователей.Пользователь.ИдентификаторПользователяИБ <> &ПустойУникальныйИдентификатор)
	|ГДЕ
	|	ТаблицыГруппДоступа.Таблица = &Таблица
	...
	Пока Выборка.Следующий() Цикл
		ПраваГруппДоступаСписка.Вставить(Выборка.ГруппаДоступа,
			Новый Структура("Изменение, ЧтениеБезОграничения, ИзменениеБезОграничения",
				Выборка.Изменение, Выборка.ЧтениеБезОграничения, Выборка.ИзменениеБезОграничения));
	КонецЦикла;
	...
	ПользователиГруппПользователей = Новый Соответствие;
	
	Если ПараметрыОбновления.РассчитыватьПраваПользователей Тогда
		Дерево = РезультатыЗапроса[2].Выгрузить(ОбходРезультатаЗапроса.ПоГруппировкам);
		Для Каждого Строка Из Дерево.Строки Цикл
		...
		КонецЦикла;
	КонецЕсли;
	ПараметрыОбновления.Вставить("ПользователиГруппПользователей", ПользователиГруппПользователей);
...
// Для процедуры ОбновитьПользователейКлючейДоступаСписка.
Процедура ОбновитьПраваКлючаДоступаСписка(ПараметрыОбновления, ЗначенияТаблицКлюча, КлючДоступа)
	
	ПраваКлючаДоступа = Новый Соответствие;
	...
	Для Каждого ОписаниеПрав Из ПараметрыОбновления.ПраваГруппДоступаСписка Цикл
		...
		Если ПраваГруппыДоступа.ЧтениеБезОграничения Тогда
			ПравоЧтение = "Истина";
		Иначе
		...
		ИначеЕсли ПраваГруппыДоступа.Изменение Тогда
			Если ПараметрыОбновления.ЕстьОграничениеИзменения Тогда
		...
		Если Контекст.РассчитыватьПраваПользователей Тогда
			ДобавитьПраваПользователейКлючаДоступа(ПраваКлючаДоступа, ПравоЧтение, ПравоИзменение, Контекст);
		Иначе
			ПраваКлючаДоступа.Вставить(ГруппаДоступа, ПравоИзменение <> "Ложь");
		КонецЕсли;
	КонецЦикла;
	...
	Если Не ПараметрыОбновления.РассчитыватьПраваПользователей Тогда
		...
		ИмяПоляВладельцаПрав = "ГруппаДоступа";
		ЭлементБлокировки = Блокировка.Добавить("РегистрСведений.КлючиДоступаГруппДоступа");
		НаборЗаписей = СлужебныйНаборЗаписей(РегистрыСведений.КлючиДоступаГруппДоступа);
		
	ИначеЕсли ПараметрыОбновления.ДляВнешнихПользователей Тогда
		Запрос.Текст =
		"ВЫБРАТЬ
	...
	Попытка
		...
		Выборка = Запрос.Выполнить().Выбрать();
		ЕстьИзменения = Ложь;
		НаборЗаписей.Отбор[ИмяПоляВладельцаПрав].Использование = Истина;
		Пока Выборка.Следующий() Цикл
			ПравоИзменение = ПраваКлючаДоступа.Получить(Выборка[ИмяПоляВладельцаПрав]);
			Если ПравоИзменение = Неопределено Тогда
				НаборЗаписей.Отбор[ИмяПоляВладельцаПрав].Значение = Выборка[ИмяПоляВладельцаПрав];
				НаборЗаписей.Записать();
				ЕстьИзменения = Истина;
			ИначеЕсли Выборка.Изменение = ПравоИзменение Тогда
				ПраваКлючаДоступа.Вставить(Выборка[ИмяПоляВладельцаПрав], Null);
			КонецЕсли;
		КонецЦикла;
		ОднаЗапись = НаборЗаписей.Добавить();
		ОднаЗапись.КлючДоступа = КлючДоступа;
		Для Каждого ОписаниеПрава Из ПраваКлючаДоступа Цикл
			Если ОписаниеПрава.Значение = Null Тогда
				Продолжить;
			КонецЕсли;
			НаборЗаписей.Отбор[ИмяПоляВладельцаПрав].Значение = ОписаниеПрава.Ключ;
			ОднаЗапись[ИмяПоляВладельцаПрав] = ОписаниеПрава.Ключ;
			ОднаЗапись.Изменение = ОписаниеПрава.Значение;
			НаборЗаписей.Записать();
			ЕстьИзменения = Истина;
		КонецЦикла;
		Если ЕстьИзменения И ПараметрыОбновления.ЗависимыеСпискиПоКлючамДоступа.Количество() > 0 Тогда
			ЗапланироватьОбновлениеПользователейКлючейДоступа(ПараметрыОбновления.ЗависимыеСпискиПоКлючамДоступа,
				Не ПараметрыОбновления.ДляВнешнихПользователей, ПараметрыОбновления.ДляВнешнихПользователей);
		КонецЕсли;
		ЗафиксироватьТранзакцию();
	Исключение
		ОтменитьТранзакцию();
	...
	Если ПараметрыОбновления.Свойство("ЕстьИзмененияПрав") Тогда
		ПараметрыОбновления.ЕстьИзмененияПрав = ЕстьИзменения;
	КонецЕсли;
	
КонецПроцедуры

// Для процедуры ОбновитьПраваКлючаДоступаСписка.
Процедура ДобавитьПраваПользователейКлючаДоступа(ПраваКлючаДоступа, ПравоЧтение, ПравоИзменение, Контекст)
	
	Если ТипЗнч(ПравоЧтение) <> Тип("Соответствие") Тогда
		ПравоИзменениеКакЧтение = ТипЗнч(ПравоИзменение) <> Тип("Соответствие") И ПравоИзменение <> "Ложь";
		ДобавитьУчастникамГруппыДоступаПравоНаКлючДоступа(ПраваКлючаДоступа, ПравоИзменениеКакЧтение, Контекст);
		Если ПравоИзменениеКакЧтение Тогда
			Возврат;
		КонецЕсли;
	Иначе
		ДобавитьПользователямПравоНаКлючДоступа(ПраваКлючаДоступа, ПравоЧтение, Ложь)
	КонецЕсли;
	
	Если ПравоИзменение = "Ложь"
	 Или ТипЗнч(ПравоИзменение) = Тип("Соответствие")
	   И ПравоИзменение.Количество() = 0 Тогда
		
		Возврат;
	КонецЕсли;
	
	Если ТипЗнч(ПравоИзменение) <> Тип("Соответствие") Тогда
		ДобавитьПользователямПравоНаКлючДоступа(ПраваКлючаДоступа, ПраваКлючаДоступа, Истина)
	Иначе
		ДобавитьПользователямПравоНаКлючДоступа(ПраваКлючаДоступа, ПравоИзменение, Истина)
	КонецЕсли;
	
КонецПроцедуры

// Для процедуры ДобавитьПраваПользователейКлючаДоступа.
Процедура ДобавитьУчастникамГруппыДоступаПравоНаКлючДоступа(ПраваКлючаДоступа, ПравоИзменение, Контекст)
	...
	Если ПравоИзменение Тогда
		Для Каждого КлючИЗначение Из УчастникиТекущейГруппыДоступа Цикл
			Если ТипЗнч(КлючИЗначение.Ключ) = Контекст.ТипПользователя Тогда
				ПраваКлючаДоступа.Вставить(КлючИЗначение.Ключ, Истина);
				
			ИначеЕсли ТипЗнч(КлючИЗначение.Ключ) = Контекст.ТипГруппыПользователей Тогда
				ПользователиГруппы = Контекст.ПользователиГруппПользователей.Получить(КлючИЗначение.Ключ);
				Если ПользователиГруппы <> Неопределено Тогда
					Для Каждого КлючИЗначение Из ПользователиГруппы Цикл
						ПраваКлючаДоступа.Вставить(КлючИЗначение.Ключ, Истина);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
	...
	Иначе
		Для Каждого КлючИЗначение Из УчастникиТекущейГруппыДоступа Цикл
			Если ТипЗнч(КлючИЗначение.Ключ) = Контекст.ТипПользователя Тогда
				ТекущееПраво = ПраваКлючаДоступа.Получить(КлючИЗначение.Ключ);
				НовоеПраво = ?(ТекущееПраво = Неопределено, Ложь, ТекущееПраво);
				ПраваКлючаДоступа.Вставить(КлючИЗначение.Ключ, НовоеПраво);
...
// Для процедуры ДобавитьПраваПользователейКлючаДоступа.
Процедура ДобавитьПользователямПравоНаКлючДоступа(ПраваКлючаДоступа, СоставПользователей, ПравоИзменение)
...
// Для процедуры ОбновитьГруппыДоступаКлючаДоступаСписка и функции РассчитанноеУсловие.
Функция РассчитанноеУсловиеДляСтрок(Контекст, Условие, УзелРеквизитов = Неопределено, ДляЛюбойИзСтрок = Истина)
	...
	Пока Истина Цикл
		...
		Иначе
			...
			Иначе
				ДобавитьТекущийРезультат(Результат, ТекущийРезультат, ДляЛюбойИзСтрок);
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
...
Процедура ДобавитьТекущийРезультат(Результат, ТекущийРезультат, ДляЛюбойИзСтрок)
	...
	Иначе
		...
		Для Каждого КлючИЗначение Из МеньшийРезультат Цикл
			Если БольшийРезультат.Получить(КлючИЗначение.Ключ) <> Неопределено Тогда
...
Процедура УстановитьОбратныйРезультат(Результат, Контекст)
	...
	ПолныйРезультат = Новый Соответствие;
	ДобавитьУчастникамГруппыДоступаПравоНаКлючДоступа(ПолныйРезультат, Истина, Контекст);
	
	ТекущийРезультат = Результат;
	Результат = Новый Соответствие;
	
	Для Каждого КлючИЗначение Из ПолныйРезультат Цикл
		Если ТекущийРезультат.Получить(КлючИЗначение.Ключ) = Неопределено Тогда
			Результат.Вставить(КлючИЗначение.Ключ, КлючИЗначение.Значение);
		КонецЕсли;
	КонецЦикла;
...
Функция ТекущиеЗначенияТаблицКлюча(Контекст, ТребуемыеРеквизитыТабличныхЧастейКлюча)
	...
	Для Каждого ОписаниеТаблиц Из ТребуемыеРеквизитыТабличныхЧастейКлюча Цикл
		ИмяТаблицыКлюча = ОписаниеТаблиц.Ключ;
		Реквизиты       = ОписаниеТаблиц.Значение;
...
Функция РассчитанноеУсловие(Контекст, Условие)
	...
	Если Условие.Узел = "Поле" Тогда
		Результат = ?(Контекст.ТекущиеСтрокиТаблицКлюча[Условие.Таблица][Условие.Реквизит]
			= Перечисления.ДополнительныеЗначенияДоступа.Истина, "Истина", "Ложь");
		...
		Для Каждого Аргумент Из Условие.Аргументы Цикл
			...
			ИначеЕсли ТипЗнч(ТекущийРезультат) = Тип("Соответствие") Тогда
				...
				Если ТипЗнч(Результат) = Тип("Строка") Тогда
					Результат = ТекущийРезультат;
				Иначе
					ДобавитьТекущийРезультат(Результат, ТекущийРезультат, Ложь);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		Если Результат = "Неопределено" Тогда
		...
		Для Каждого Аргумент Из Условие.Аргументы Цикл
			...
			ИначеЕсли ТипЗнч(ТекущийРезультат) = Тип("Соответствие") Тогда
				...
				ИначеЕсли ТипЗнч(Результат) = Тип("Строка") Тогда
					Результат = ТекущийРезультат;
				Иначе
					ДобавитьТекущийРезультат(Результат, ТекущийРезультат, Истина);
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
		Если Результат = "Неопределено" Тогда
	...
	ИначеЕсли Условие.Узел = "ЗначениеРазрешено"
		...
		ИначеЕсли Условие.Узел = "ЗначениеРазрешено" Тогда
			Если Значение = Перечисления.ДополнительныеЗначенияДоступа.Null
			 Или Значение = Перечисления.ДополнительныеЗначенияДоступа.Неопределено
			 Или Контекст.ЗначенияГруппыДоступа = Неопределено Тогда
				Результат = "Ложь";
			Иначе
				...
				Иначе
					...
					ИначеЕсли ТипЗначения = Контекст.ТипГруппыПользователей Тогда
						ПользователиГруппы = Контекст.ПользователиГруппПользователей.Получить(Значение);
						
						Если ПользователиГруппы <> Неопределено Тогда
							Результат = Новый Соответствие;
							Для Каждого ОписаниеПользователя Из ПользователиГруппы Цикл
								ЗаполнитьРезультатДляПользователя(Результат, ОписаниеПользователя.Ключ, Контекст);
							КонецЦикла;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
		...
		ИначеЕсли ТипЗнч(Значение) = Тип("СправочникСсылка.КлючиДоступа") Тогда
			...
			ИначеЕсли Контекст.РассчитыватьПраваПользователей Тогда
				Если Условие.Узел = "ЧтениеОбъектаРазрешено" Тогда
					Результат = Новый Соответствие(ПраваНаВедущийКлючДоступа);
...
Процедура ЗаполнитьРезультатДляПользователя(Результат, Пользователь, Контекст)
	...
	Если ТипЗнч(Пользователь) <> Контекст.ТипПользователя Тогда
		Возврат;
	КонецЕсли;
	...
	Для Каждого ОписаниеУчастника Из УчастникиГруппыДоступа Цикл
		Участник = ОписаниеУчастника.Ключ;
		Если Участник = Пользователь Тогда
			Результат.Вставить(Пользователь, Истина);
			Прервать;
		ИначеЕсли ТипЗнч(Участник) = Контекст.ТипГруппыПользователей Тогда
			ПользователиГруппы = Контекст.ПользователиГруппПользователей.Получить(Участник);
			
			Если ПользователиГруппы <> Неопределено
			   И ПользователиГруппы.Получить(Пользователь) <> Неопределено Тогда
...
Функция СлужебныйЭлемент(МенеджерСправочника)
...
Функция МаксимальноеКоличествоЭлементовДанныхПорции()
	
	Минут = Цел(МаксимальноеКоличествоМинутВыполненияФоновогоЗаданияОбновленияДоступа());
	
	Если Минут < 2 Тогда
		Минут = 1;
	ИначеЕсли Минут >= 20 Тогда
		Минут = 20;
	КонецЕсли;
...
Функция СвойстваСпискаКакВедущего(ПолноеИмя, ИдентификаторТранзакции = Неопределено)
...
//
Функция ОбщийКонтекстРасчетаПараметровОграничения(ВсеВидыДоступаИспользуются = Ложь, ЗаполнитьСпискиСОграничением = Истина)
	...
	ВнешниеПользователиВключены = ВсеВидыДоступаИспользуются
		Или Константы.ИспользоватьВнешнихПользователей.Получить();
...
Функция РезультатПроверкиОграниченияДоступаОбъекта(ПолноеИмя, Знач ДополнительныеПараметры)
	
	Если Не ЗначениеЗаполнено(ДополнительныеПараметры) Тогда
		ДополнительныеПараметры = Новый Структура;
		ДополнительныеПараметры.Вставить("Текст", Неопределено);
		ДополнительныеПараметры.Вставить("ТекстДляВнешнихПользователей", Неопределено);
		ДополнительныеПараметры.Вставить("УчитыватьЗависимости", Ложь);
	КонецЕсли;
	...
	Если ОписаниеОграничения <> Неопределено Тогда
		...
		Если ДополнительныеПараметры.Текст = Неопределено Тогда
			ДляПользователей.ПроверяемоеОграничение = ДляПользователей.ОграничениеВМодуле;
		Иначе
			ДляПользователей.ПроверяемоеОграничение = ДополнительныеПараметры.Текст;
			ОписаниеОграничения.Текст               = ДополнительныеПараметры.Текст;
		КонецЕсли;
		
		Если ДополнительныеПараметры.ТекстДляВнешнихПользователей = Неопределено Тогда
			ДляВнешнихПользователей.ПроверяемоеОграничение = ДляВнешнихПользователей.ОграничениеВМодуле;
		Иначе
			ДляВнешнихПользователей.ПроверяемоеОграничение   = ДополнительныеПараметры.ТекстДляВнешнихПользователей;
			ОписаниеОграничения.ТекстДляВнешнихПользователей = ДополнительныеПараметры.ТекстДляВнешнихПользователей;
		КонецЕсли;
	КонецЕсли;
	...
	Контекст.Вставить("УчитыватьЗависимости",     ДополнительныеПараметры.УчитыватьЗависимости);
	...
	Если ДополнительныеПараметры.УчитыватьЗависимости Тогда
...
Процедура УстановитьНастройкиВнедрения(НастройкиВнедрения, Данные, ТипыТаблицПоИменам)
	...
	Для Каждого КлючИЗначение Из Данные.ТипыИзмеренийРегистровКлючей Цикл
		ИмяРегистраКлючей = СтрРазделить(КлючИЗначение.Ключ, ".", Ложь)[1];
		
		Если ИмяРегистраКлючей = "КлючиДоступаКРегистрам" Тогда
			ДобавитьТипыТребуемыеВОпределяемомТипе(НастройкиВнедрения, ТипыТаблицПоИменам, Истина,
				КлючИЗначение.Значение, "ПолеРегистраКлючейДоступаКРегистрам");
		Иначе
			ИзмеренияРегистра = Метаданные.РегистрыСведений[ИмяРегистраКлючей].Измерения;
			СписокТипов = "";
			НомерПоля = 1;
			Для Каждого ОписаниеПоля Из КлючИЗначение.Значение Цикл
...
Процедура ПроверитьОграничениеДляВидаПользователей(Контекст, Результат, ДляВнешнихПользователей, ДополнительныйКонтекст)
	...
	ДобавитьДополнительныйКонтекст(Контекст.ПолноеИмя,
		ДополнительныйКонтекст, Контекст.ОписаниеОграничения, Ложь);
	...
	Попытка
		ПараметрыОграничения = ПараметрыОграниченияПоСтруктуреОграничения(Контекст.ПолноеИмя,
			СтруктураОграничения, Ложь, Контекст.ОбщийКонтекст, ДополнительныйКонтекст);
	Исключение
		ИнформацияОбОшибке = ИнформацияОбОшибке();
	...
	Если ПараметрыОграничения <> Неопределено Тогда
		...
		ПараметрыОграничения.Контекст.СпискиСЗаписьюКлючейДоступа = Новый Соответствие;
...
Функция ОписаниеОграниченияДанных(ОбщийКонтекст, ПолноеИмя, БезВызоваИсключения = Ложь)
	...
	Если Не ОбщийКонтекст.ВнешниеПользователиВключены Тогда
...
Функция ВерсияСтруктурыКэша()
	...
	Возврат "4"; // Число нужно увеличивать при внесении изменений в состав параметров кэша.
...
// Для функции ДействующиеПараметрыОграниченияДоступа.
Процедура УстановитьВерсиюПараметров(ВерсияПараметров, ИдентификаторТранзакции, ПовторныйВызов = Ложь)
	...
	Если ТипЗнч(ВерсияПараметров.ДляЗаписиОбъектовИПроверкиПрав) = Тип("ХранилищеЗначения") Тогда
		...
		Если ТипЗнч(ДляЗаписиОбъектовИПроверкиПрав) = Тип("Структура")
		 ...
		 Или ДляЗаписиОбъектовИПроверкиПрав.ВерсияСтруктурыКэша <> ВерсияСтруктурыКэша() Тогда
			...
			ВерсияПараметров = НоваяВерсияПараметровОграниченияДоступа();
			УстановитьВерсиюПараметров(ВерсияПараметров, ИдентификаторТранзакции, Истина);
			Возврат;
		КонецЕсли;
	Иначе
...
// Для функции НоваяВерсияПараметровОграниченияДоступа.
Функция ХранимыеПараметрыОграниченияДоступа(ВерсииОграниченийСписков = Неопределено, ОбщийКонтекст = Неопределено)
...
Процедура ДобавитьХранимыеПараметрыОграниченияДляВидаПользователей(Контекст)
	...
	ТаблицаСвойств.Колонки.Добавить("ВладельцыСПравамиПользователей",  Новый ОписаниеТипов("Массив"));
	ТаблицаСвойств.Колонки.Добавить("ВладельцыСПравамиГруппДоступа",   Новый ОписаниеТипов("Массив"));
		...
		Если ВедущиеСписки.ПоКлючамДоступа.Количество() > 0 Тогда
			...
			Для Каждого КлючИЗначение Из ВедущиеСписки.ПоКлючамДоступа Цикл
				...
				Если СвойстваВедущего = Неопределено Или ТипЗнч(СвойстваВедущего) = Тип("Булево") Тогда
					...
					Если СвойстваВедущего.Параметры.ОграничениеОтключено Тогда
		...
		Для Каждого СвойстваСписка Из СвойстваСписков Цикл
			...
			Для Каждого Ведущий Из СвойстваСписка.Ведущие Цикл
				...
				Если СвойстваВедущего.КлючиДоступаПользователей Тогда
					...
					Свойства.ВладельцыСПравамиПользователей.Добавить(Ведущий);
				Иначе
					КлючиДоступаГруппДоступа = Истина;
					Свойства.ВладельцыСПравамиГруппДоступа.Добавить(Ведущий);
				КонецЕсли;
			КонецЦикла;
	...
	Для Каждого СвойстваСписка Из ТаблицаСвойств Цикл
		...
		Если Не СвойстваСписка.ОграничениеПоВладельцуВключено
		   И СвойстваСписка.КлючиДоступаПользователей Тогда
			
			Если Не СвойстваСписка.Параметры.РассчитыватьПраваПользователей Тогда
				ОбновитьВерсиюПараметров = Истина;
			КонецЕсли;
			УстановитьСвойствоОграничения(ПолноеИмя, "РассчитыватьПраваПользователей", Истина, Контекст);
			Если СвойстваСписка.ВладельцыСПравамиПользователей.Количество() > 0
			   И СвойстваСписка.ВладельцыСПравамиГруппДоступа.Количество() > 0 Тогда
				
				ВладельцыСПравамиДля = Новый Структура;
				ВладельцыСПравамиДля.Вставить("Пользователей", СвойстваСписка.ВладельцыСПравамиПользователей);
				ВладельцыСПравамиДля.Вставить("ГруппДоступа",  СвойстваСписка.ВладельцыСПравамиГруппДоступа);
				УстановитьСвойствоОграничения(ПолноеИмя, "ВладельцыСПравамиДля", ВладельцыСПравамиДля, Контекст);
				ОбновитьВерсиюПараметров = Истина;
			КонецЕсли;
		КонецЕсли;
		...
		КонецЕсли;
		Если СвойстваСписка.ОграничениеПоВладельцуВключено Тогда
			Если Не СвойстваСписка.Параметры.ИспользуетсяОграничениеПоВладельцу Тогда
				ОбновитьВерсиюПараметров = Истина;
			КонецЕсли;
			УстановитьСвойствоОграничения(ПолноеИмя,
		...
		Если ОбновитьВерсиюПараметров Тогда
			СвойстваСписка.Параметры = ПараметрыОграниченияДляВидаПользователей(СвойстваСписка.ПолноеИмя, Контекст);
		КонецЕсли;
...
Функция СвойстваОграниченияСписка(ПолноеИмя, Контекст, ДобавлятьВКоллекцию = Ложь)
	...
	Если Свойства = Неопределено Тогда
		...
		Свойства.Вставить("ВладельцыСПравамиДля", Неопределено);
...
Функция ПараметрыОграниченияПоСтруктуреОграничения(Список, СтруктураОграничения,
	...
	// Состав владельцев с разным расчетом прав (для пользователей и для групп пользователей) или Неопределено.
	Результат.Вставить("ВладельцыСПравамиДля", Неопределено);
	...
	Если Свойства <> Неопределено Тогда
		...
		Результат.ВладельцыСПравамиДля               = Свойства.ВладельцыСПравамиДля;
	КонецЕсли;
	...
	Контекст.Вставить("ВладельцыСПравамиДля",               Результат.ВладельцыСПравамиДля);
	...
	Если Контекст.СпискиСЗаписьюКлючейДоступа = Неопределено Тогда
	...
	Если Результат.ВладельцыСПравамиДля = Неопределено Тогда
		ДобавитьСвойствоВерсии(Контекст, Контекст, "ВладельцыСПравамиДля");
	Иначе
		ДобавитьЭлементВерсии(Контекст, "ВладельцыСПравамиДля.Пользователей",
			Контекст.ВладельцыСПравамиДля.Пользователей);
		ДобавитьЭлементВерсии(Контекст, "ВладельцыСПравамиДля.ГруппДоступа",
			Контекст.ВладельцыСПравамиДля.ГруппДоступа);
	КонецЕсли;
...
Процедура ЗаполнитьНаличиеВедущихКлючейИСписковИВладельцевНастроекПрав(Результат, Контекст)
	...
	Для Каждого СвойстваПоля Из Контекст.СвойстваПолей Цикл
		...
		Для Каждого Тип Из СвойстваПоля.НеиспользуемыеТипыЗначенийДоступа Цикл
			Контекст.НеиспользуемыеТипыЗначенийДоступа.Добавить(Тип);
		КонецЦикла;
	КонецЦикла;
	...
	Если Родитель.Узел = "ЗначениеРазрешено" Тогда
		...
		Для Каждого Тип Из Свойства.ТипКонечногоПоля.Типы() Цикл
			...
			Если Контекст.ИспользуемыеТипыЗначений.Получить(СвойстваВидаДоступа.ТипЗначений) = Неопределено Тогда
				...
				Если УточнениеТипа(Родитель, "Отключено") = "Ложь" Тогда
					Свойства.ТипыСохраненияТипаЗапрещенный.Добавить(Тип);
				Иначе
					Свойства.ТипыСохраненияТипаРазрешенный.Добавить(Тип);
				КонецЕсли;
...
Процедура ДобавитьТекстыЗапросовВПараметрыОграничения(Результат)
	...
	Результат.Вставить("ТекстЗапросаЗначенийИзКлючейДоступаДляСравнения");
...
Процедура ДобавитьТекстЗапросаЭлементовДанныхДляОчисткиКлючей(Результат, Контекст)
	...
	Количество = Формат(МаксимальноеКоличествоЭлементовДанныхПорции(), "ЧГ=");
...
Процедура ЗаполнитьШаблоныЧастейЗапросаПроверкиРегистра(Контекст)
	...
	// Установка шаблона запроса.
	Контекст.Вставить("ТекстЗапросаПроверки");
	
	ТекстПолногоЗапросаПроверки =
	"ВЫБРАТЬ РАЗЛИЧНЫЕ ПЕРВЫЕ " + Количество + "
	|	" + ОпорныеПоля.ДляВыбора + "
	|ИЗ (
	...
	ТекстПолногоЗапросаПроверки = СтрЗаменить(ТекстПолногоЗапросаПроверки,
		"ТекущийСписок.", "ЧастиТекущегоСписка.");
	
	ТекстОберткиЧастиЗапросаПроверки =
	"ВЫБРАТЬ
	|	" + ОпорныеПоля.ДляВыбора + "
	|ИЗ
	|	(#ЗапросЧастиОбъектов) КАК ЧастьТекущегоСписка";
	
	ТекстОберткиЧастиЗапросаПроверки = СтрЗаменить(ТекстОберткиЧастиЗапросаПроверки,
		"ТекущийСписок.", "ЧастьТекущегоСписка.");
	
	ТекстЗапросаЧастиОбъектов = СтрЗаменить(ТекстОберткиЧастиЗапросаПроверки,
		"#ЗапросЧастиОбъектов", ТекстСОтступом(ТекстЗапросаНовыхКомбинаций, "	"));
	
	ТекстЗапросаЧастиОбъектов = ТекстЗапросаЧастиОбъектов + "
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|";
	
	ТекстЗапросаЧастиОбъектов = ТекстЗапросаЧастиОбъектов + СтрЗаменить(ТекстОберткиЧастиЗапросаПроверки,
		"#ЗапросЧастиОбъектов", ТекстСОтступом(ТекстЗапросаПроверкиКомбинаций, "	"));
	
	Контекст.ТекстЗапросаПроверки = СтрЗаменить(ТекстПолногоЗапросаПроверки,
		"#ЗапросыОбъектовПоЧастям", ТекстСОтступом(ТекстЗапросаЧастиОбъектов, "	"));
...
Процедура СобратьЧастиЗапросаПроверки(Результат, Контекст)
	...
	Контекст.ТекстЗапросаПроверки = СтрЗаменить(Контекст.ТекстЗапросаПроверки,
		"#Условие", ТекстСОтступом(Условие, ?(Контекст.ЭтоСсылочныйТип, "	", "			")));
...
Процедура СобратьЧастиЗапросовЗаполнения(Результат, Контекст)
	...
	ТекстЗапроса = СтрСоединить(Контекст.ЧастиЗапросаЗначенийИзКлючейДляСравнения,
		ОбщегоНазначения.РазделительПакетаЗапросов());
	ПодставитьОбщиеПараметрыВЗапрос(ТекстЗапроса, Контекст);
	Результат.ТекстЗапросаЗначенийИзКлючейДоступаДляСравнения = ТекстЗапроса;
...
Процедура ДобавитьЧастьЗапросаВыбораПравВедущихКлючейДоступа(Контекст)
	...
	Если Не Контекст.РассчитыватьПраваПользователей Тогда
		...
		|ИТОГИ ПО
		|	КлючДоступа";
	ИначеЕсли Контекст.ВладельцыСПравамиДля = Неопределено Тогда
		ТекстЗапроса =
		"ВЫБРАТЬ
		|	ПраваНаКлючиДоступа.КлючДоступа КАК КлючДоступа,
		|	ПраваНаКлючиДоступа.Пользователь КАК ВладелецПрав,
		|	ПраваНаКлючиДоступа.Изменение КАК Изменение
		|ИЗ
	...
	Иначе
		...
		"ВЫБРАТЬ
		|	ПраваНаКлючиДоступа.КлючДоступа КАК КлючДоступа,
		|	ПраваНаКлючиДоступа.ВладелецПрав КАК ВладелецПрав,
		|	ПраваНаКлючиДоступа.Изменение КАК Изменение
		|ИЗ
		|	(ВЫБРАТЬ
		|		ПраваНаКлючиДоступа.КлючДоступа КАК КлючДоступа,
		...
		|		РегистрСведений.КлючиДоступаПользователей КАК ПраваНаКлючиДоступа
		|	ГДЕ
		|		ПраваНаКлючиДоступа.КлючДоступа.Список В(&ВладельцыСПравамиПользователей)
		|		И &УсловиеОтбора
		|	
		|	ОБЪЕДИНИТЬ ВСЕ
		|	
		...
		|		ПользователиГруппПользователей.Пользователь,
		|		МАКСИМУМ(ПраваНаКлючиДоступа.Изменение)
		|	ИЗ
		|		РегистрСведений.КлючиДоступаГруппДоступа КАК ПраваНаКлючиДоступа
		|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ГруппыДоступа.Пользователи КАК ПользователиГруппыДоступа
		|			ПО (ПользователиГруппыДоступа.Ссылка = ПраваНаКлючиДоступа.ГруппаДоступа)
		|				И (ПраваНаКлючиДоступа.КлючДоступа.Список В (&ВладельцыСПравамиГруппДоступа))
		|				И (&УсловиеОтбора)
		|			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СоставыГруппПользователей КАК ПользователиГруппПользователей
		|			ПО (ПользователиГруппПользователей.ГруппаПользователей = ПользователиГруппыДоступа.Пользователь)
		|				И (ПользователиГруппПользователей.Используется)
		...
		|	СГРУППИРОВАТЬ ПО
		|		ПраваНаКлючиДоступа.КлючДоступа,
		|		ПользователиГруппПользователей.Пользователь) КАК ПраваНаКлючиДоступа
		|ИТОГИ ПО
		|	КлючДоступа";
	КонецЕсли;
	...
	Иначе
		УсловиеОтбора = "(" + СтрСоединить(Контекст.ЧастиУсловияВыбораПравВедущихКлючейДоступа,
			Символы.ПС + "			ИЛИ ") + ")";
	КонецЕсли;
	УсловиеОтбора = СтрЗаменить(УсловиеОтбора, "#ПроверяемоеПоле", "ПраваНаКлючиДоступа.КлючДоступа");
...
Процедура ДобавитьЧастьЗапросаВыбораПравВедущихСписков(Контекст)
	...
	Если Контекст.РассчитыватьПраваПользователей Тогда
		...
		|	ПользователиГруппПользователей.Пользователь КАК ВладелецПрав,
		|	МАКСИМУМ(ПраваНаСписки.Изменение) КАК Изменение
		|ИЗ
		...
		|		ПО (ПользователиГруппыДоступа.Ссылка = ПраваНаСписки.ГруппаДоступа)
		|			И (&УсловиеОтбора)
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоставыГруппПользователей КАК ПользователиГруппПользователей
		|		ПО (ПользователиГруппПользователей.ГруппаПользователей = ПользователиГруппыДоступа.Пользователь)
		|			И (ПользователиГруппПользователей.Используется)
		|			И (ПользователиГруппПользователей.Пользователь.ИдентификаторПользователяИБ <> &ПустойУникальныйИдентификатор)
		|
		|СГРУППИРОВАТЬ ПО
		|	ПраваНаСписки.Таблица,
		|	ПользователиГруппПользователей.Пользователь
		|ИТОГИ ПО
		|	Список";
	Иначе
...
Процедура ДобавитьЧастьЗапросаВыбораПравПоВладельцамНастроекПрав(Контекст)
	...
	|	МАКСИМУМ(НастройкиПрав.ИзменениеРазрешено) = ИСТИНА
	|		И МАКСИМУМ(НастройкиПрав.ИзменениеЗапрещено) = ЛОЖЬ КАК Изменение
	|ИЗ
	|	(ВЫБРАТЬ
	...
	|				И (&УсловиеОтбора)
	|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.СоставыГруппПользователей КАК СоставыГруппПользователей
	|			ПО (СоставыГруппПользователей.ГруппаПользователей = НастройкиПрав.Пользователь)) КАК НастройкиПрав
	|
	|СГРУППИРОВАТЬ ПО
	...
	|
	|ИМЕЮЩИЕ
	|	МАКСИМУМ(НастройкиПрав.ЧтениеРазрешено) = ИСТИНА И
	|	МАКСИМУМ(НастройкиПрав.ЧтениеЗапрещено) = ЛОЖЬ
	|ИТОГИ ПО
...
Процедура ЗаполнитьЗапросыПроверкиПравЧтениеИзменение(Результат, Контекст)
	...
	Если Контекст.ЭтоСсылочныйТип
	 Или Результат.ИспользуетсяОграничениеПоВладельцу Тогда
		...
		Если Свойства.ОграничениеВШаблонахЧерезКлючиДоступаПользователейИГруппДоступа Тогда
			...
			Иначе
				...
				|					И РазрешенныеКлючиДоступа.Изменение))";
			КонецЕсли;
		ИначеЕсли Свойства.ОграничениеВШаблонахЧерезКлючиДоступаПользователей Тогда
			Если Контекст.ДляВнешнихПользователей Тогда
				ТекстЗапроса =
				"ВЫБРАТЬ ПЕРВЫЕ 1
				|	ИСТИНА КАК ЗначениеИстина
...
Процедура ДобавитьУсловиеОтбораПравДляШапкиКлюча(Контекст, ГруппаПолей, НомерШапки, НазначениеУсловия)
	...
	Для Каждого СвойстваПоля Из ГруппаПолей Цикл
		...
		Если НомерШапки = 0 Тогда
			...
			"#ПроверяемоеПоле В
			|				(ВЫБРАТЬ
			|					Шапка0." + СвойстваПоля.ИмяРеквизитаГруппыПолейКлючаДоступа + "
			|				ИЗ
			|					ПорцияКлючей КАК ПорцияКлючей
			|						ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиДоступа КАК Шапка0
			|						ПО
		...
		Иначе
			...
			"#ПроверяемоеПоле В
			|				(ВЫБРАТЬ
			|					Шапка?." + СвойстваПоля.ИмяРеквизитаГруппыПолейКлючаДоступа + "
			|				ИЗ
			|					ПорцияКлючей КАК ПорцияКлючей
			|						ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиДоступа.Шапка КАК Шапка?
			|						ПО
...
Процедура ДобавитьУсловиеОтбораПравДляТабличнойЧастиКлюча(Контекст, ГруппаПолей,
	...
	Для Каждого СвойстваПоля Из ГруппаПолей Цикл
		...
		"#ПроверяемоеПоле В
		|				(ВЫБРАТЬ
		|					ТабличнаяЧасть?." + СвойстваПоля.ИмяРеквизитаГруппыПолейКлючаДоступа + "
		|				ИЗ
		|					ПорцияКлючей КАК ПорцияКлючей
		|						ЛЕВОЕ СОЕДИНЕНИЕ Справочник.КлючиДоступа.ТабличнаяЧасть? КАК ТабличнаяЧасть?
		|						ПО
...
Процедура ОтметитьНекорректныеАргументыИЗапрещенныеУзлы(Условие, ДоступныеУзлы, Контекст, Родитель)
	...
	Если Условие.Узел = "Поле" Тогда
		...
		Если Не Контекст.ЭтоУсловиеСоединения Тогда
			...
			ПолеКлючаДоступа.Вставить("Родители", Новый ФиксированныйМассив(Контекст.Родители));
...
Процедура ОбновитьПрогрессВФоне(Контекст, АдресРезультата) Экспорт
	...
	|ВЫБРАТЬ
	|	Списки.Список КАК Список,
	|	ОбновлениеКлючейДоступаДляПользователей.ПоследнийОбновленныйКлючДоступа КАК ПоследнийОбновленныйКлючДоступа,
	|	ОбновлениеКлючейДоступаДляВнешнихПользователей.ПоследнийОбновленныйКлючДоступа КАК ПоследнийОбновленныйКлючДоступаДляВнешнихПользователей
	|ИЗ
	|	(ВЫБРАТЬ
	|		ОбновлениеКлючейДоступаПользователей.Список КАК Список,
	|		МАКСИМУМ(ВЫБОР
	...
	Для Каждого КлючИЗначение Из СтрокиСписков Цикл
		...
		Если СписокОбновления = Неопределено Или Не СписокОбновления.ОбновлениеКлючейДоступа Тогда
			СвойстваСписка.ПоследнийОбновленныйКлючДоступа = Неопределено;
			СвойстваСписка.ПоследнийОбновленныйКлючДоступаДляВнешнихПользователей = Неопределено;
			Если Контекст.РассчитыватьПоКоличествуДанных Тогда
...
Функция НоваяСтрокаСписка(Контекст, Список, ИмяТаблицы)
	...
	Свойства.Вставить("ПоследнийОбновленныйКлючДоступа", Неопределено);
	...
	Свойства.Вставить("ПоследнийОбновленныйКлючДоступаДляВнешнихПользователей", Неопределено);
	...
	Для Каждого Строка Из Контекст.СтрокиОбновленияКоличестваОбработанныхКлючейДоступа Цикл
		...
		Иначе
			...
			Для ВидПользователей = 0 По 1 Цикл
				...
				КонецЕсли;
				Если ПоследнийОбновленныйКлючДоступа = Неопределено Тогда
...
Процедура ДобавитьТекстЗапросаКоличестваОбработанныхКлючейДоступа(Запрос, Строка, Индекс, Контекст, ДляВнешнихПользователей)
	...
	СвойстваСписка = Контекст.СвойстваСписков.Получить(Строка.Список);
	Если ДляВнешнихПользователей Тогда
		ПоследнийКлючДоступа = СвойстваСписка.ПоследнийОбновленныйКлючДоступаДляВнешнихПользователей;
	Иначе
		ПоследнийКлючДоступа = СвойстваСписка.ПоследнийОбновленныйКлючДоступа;
	КонецЕсли;
...
Функция ИдентификаторыСписковСОграничением(ДействующиеПараметры)
	...
	Возврат Справочники.ИдентификаторыОбъектовМетаданных.ИдентификаторыОбъектовМетаданных(Списки, Истина);
