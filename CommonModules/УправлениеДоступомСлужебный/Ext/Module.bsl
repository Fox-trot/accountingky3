	...
	Если ЗначениеЗаполнено(Контекст.ПолноеИмя) Тогда
		...
		Если ДополнительныйКонтекст.СпискиСОтключеннымОграничением.Получить(Контекст.ПолноеИмя) <> Неопределено
		 ...
		 Или Контекст.ПриИзмененииУчастниковГруппыДоступа
		   И Не (    СвойстваОграничения.ОграничениеЧерезКлючиДоступаПользователей
		         Или СвойстваОграничения.ОграничениеЧерезКлючиДоступаПользователейИГруппДоступа) Тогда
...
Процедура ОбновитьКлючиДоступаПорцииЭлементовДанныхСписка(ПараметрыОбновления, Контекст)
	...
	Исключение
		Если Не ТранзакцияАктивна() Тогда
			ЗапланироватьОбновлениеДоступа(ПараметрыОбновления.Список, Ложь, Истина);
		КонецЕсли;
		ВызватьИсключение;
	КонецПопытки;
...
Процедура ЗаполнитьПраваИУчастниковГруппДоступаСписка(ПараметрыОбновления)
	...
	Если ПараметрыОбновления.ЕстьОграничениеПоПользователям Тогда
...
Процедура ОбновитьПраваКлючаДоступаСписка(ПараметрыОбновления, ЗначенияТаблицКлюча, КлючДоступа)
	...
	Контекст.Вставить("ЕстьОграничениеПоПользователям", ПараметрыОбновления.ЕстьОграничениеПоПользователям);
	...
	Для Каждого ОписаниеПрав Из ПараметрыОбновления.ПраваГруппДоступаСписка Цикл
		...
		Если Контекст.ЕстьОграничениеПоПользователям Тогда
	...
	Если Не ПараметрыОбновления.ЕстьОграничениеПоПользователям Тогда
		Запрос.Текст =
		"ВЫБРАТЬ
	...
	Попытка
		...
		Для Каждого ОписаниеПрава Из ПраваКлючаДоступа Цикл
			Если ОписаниеПрава.Значение = Null Тогда
				Продолжить;
			КонецЕсли;
			НаборЗаписей.Отбор[ИмяПоляВладельцаПрав].Установить(ОписаниеПрава.Ключ);
...
Функция РассчитанноеУсловие(Контекст, Условие)
	...
	ИначеЕсли Условие.Узел = "ЗначениеРазрешено"
		...
		ИначеЕсли ТипЗнч(Значение) = Тип("СправочникСсылка.КлючиДоступа") Тогда
			...
			ИначеЕсли Контекст.ЕстьОграничениеПоПользователям Тогда
...
Функция ОписаниеОграниченияДанных(ОбщийКонтекст, ПолноеИмя, БезВызоваИсключения = Ложь)
	...
	Ограничение.Вставить("УсловиеБезНастройки", Ложь);
	Ограничение.Вставить("УсловиеБезНастройкиДляВнешнихПользователей", Ложь);
...
Функция ВерсияСтруктурыКэша()
	...
	Возврат "3"; // Число нужно увеличивать при внесении изменений в состав параметров кэша.
...
Процедура ДобавитьДополнительныйКонтекст(ПолноеИмя, ДополнительныйКонтекст,
	...
	Если ДляВнешнихПользователей Тогда
		...
		УсловиеБезНастройки = ОписаниеОграничения.УсловиеБезНастройкиДляВнешнихПользователей;
		ПоВладельцу         = ОписаниеОграничения.ПоВладельцуБезЗаписиКлючейДоступаДляВнешнихПользователей;
	Иначе
		Текст               = ОписаниеОграничения.Текст;
		УсловиеБезНастройки = ОписаниеОграничения.УсловиеБезНастройки;
	...
	СокращенноеОписаниеОграничения.Вставить("УсловиеБезНастройки", УсловиеБезНастройки);
...
Процедура УдалитьПоляНеиспользуемыхВидовДоступа(Результат, Контекст)
	...
	Если Не Результат.ЕстьВедущиеКлючиДоступа
	   ...
	   И Не Контекст.ОписанияОграничений.Получить(Контекст.Список).УсловиеБезНастройки Тогда
...
Процедура ДобавитьЧастьЗапросаВыбораПравВедущихСписков(Контекст)
	...
	Если Контекст.ЕстьОграничениеПоПользователям Тогда
		...
		|			И (ПользователиГруппПользователей.Пользователь.ИдентификаторПользователяИБ <> &ПустойУникальныйИдентификатор)
		|ГДЕ
		|	НЕ ПользователиГруппыДоступа.Ссылка ЕСТЬ NULL
		|	И НЕ ПользователиГруппПользователей.Пользователь ЕСТЬ NULL
		|	И &УсловиеОтбора
		|ИТОГИ ПО
		|	Список";
...
Процедура ЗаполнитьЗапросыПроверкиПравЧтениеИзменение(Результат, Контекст)
	...
	Если Свойства.ПолеВладельца <> Неопределено Тогда
		Если Контекст.ЭтоСсылочныйТип Тогда
			ПолеОбъекта = "ВЫРАЗИТЬ(&Объект КАК " + Контекст.Список + ")." + Свойства.ПолеВладельца.Имя;
		Иначе
			ПолеОбъекта = "ТекущаяТаблица." + Свойства.ПолеВладельца.Имя;
		КонецЕсли;
	...
	ИначеЕсли Не Контекст.ЭтоСсылочныйТип Тогда
		...
		Если Свойства.ОграничениеЧерезКлючиДоступаПользователей Тогда
			Если Контекст.ДляВнешнихПользователей Тогда
				ТекстЗапроса =
				"ВЫБРАТЬ ПЕРВЫЕ 1
				|	ИСТИНА КАК ЗначениеИстина
