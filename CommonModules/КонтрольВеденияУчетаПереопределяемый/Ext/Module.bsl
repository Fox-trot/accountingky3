...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	ГруппаПроверок = ГруппыПроверок.Добавить();
	ГруппаПроверок.Наименование    = НСтр("ru='Соответствие'");
	ГруппаПроверок.Идентификатор   = "Соответствие";
	
	Проверка = Проверки.Добавить();
	Проверка.ИдентификаторГруппы	      = "Соответствие";
	Проверка.ИдентификаторРодителя        = "Соответствие";
	...
	Проверка.КонтекстПроверокВеденияУчета = "Соответствие";
КонецПроцедуры
...
Процедура ПроверкаПарныхСчетов(Проверка, ПараметрыПроверки) Экспорт
	...
	Запрос.Текст =
		...
		|	И НЕ (ВременнаяТаблицаПарныеСчета.КодСчета = ""1750""
		|		И ВременнаяТаблицаПарныеСчета.КодПарногоСчета = ""1795"")
		|	И НЕ (ВременнаяТаблицаПарныеСчета.КодСчета = ""1795""
		|		И ВременнаяТаблицаПарныеСчета.КодПарногоСчета = ""1750"")
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
