...
//
Функция ПолучитьСписокЗадачБухгалтера(ВернутьТаблицуЗначений) Экспорт
	...
	Запрос.Текст = 
		...
		|	ЗадачиБухгалтера.ПризнакЗаполнения КАК ПризнакЗаполнения
		|ИЗ
		...
		ИначеЕсли ВыборкаСправочник.Периодичность = 4 Тогда
			...
			Если ДатаСрока >= ТекущаяДата И ДатаСрока <= КонецМесяца(КонецМесяца(ТекущаяДата) + 1) Тогда
				...
				Если МассивСтрок.Количество() > 0 И ВернутьТаблицуЗначений Тогда
					...
					НоваяСтрокаТаблицы.Комментарий = СтрокаИзРегистра.Комментарий;
					НоваяСтрокаТаблицы.ОписаниеЗадачи = СтрШаблон(НСтр("ru = '%1 ""за %2 %3 г.""'"), 
							ВыборкаСправочник.Наименование, МассивМесяцев[Месяц(ДатаСрока) - 1], Формат(Год(ДатаСрока), "ЧГ=0"));
...
//
// Параметры:
//  ПараметрыУчета				 - Струтктура	 - Содержит настроки параметров учета до изменения.
//  ДополнительныеПараметры		 - Струтктура	 - Определяет стратегию изменения.
...
Процедура ПрименитьПараметрыУчета(ПараметрыУчета, ДополнительныеПараметры, ИзмененыПараметрыСубконто = Ложь, Отказ = Ложь) Экспорт
	...
	ДействияИзмененияСубконто = ПолучитьДействияИзмененияСубконто(ПараметрыУчета, ДополнительныеПараметры); // Иерархическая коллекция: на первом уровне действия с субконто на каждом из счетов, на втором - действия с призаками учета.
...
//
// Параметры:
//  ПараметрыУчетаФормы			 - Струтктура	 - Содержит настроки параметров учета до изменения.
//  ДополнительныеПараметры		 - Струтктура	 - Определяет стратегию изменения.
// 
// Возвращаемое значение:
...
Функция ПолучитьДействияИзмененияСубконто(ПараметрыУчетаФормы, ДополнительныеПараметры) Экспорт
	...
	ПараметрыСубконто = ПолучитьСоответствиеСубконтоПараметрамУчета(ДополнительныеПараметры);
...
//
Функция ПолучитьСоответствиеСубконтоПараметрамУчета(ДополнительныеПараметры)
	...
	ИначеЕсли ДополнительныеПараметры.УчетДС Тогда 
		...
		СтрокаСчета = Счета.Добавить();
		СтрокаСчета.Счет = ПланыСчетов.Хозрасчетный.СчетаВИностараннойВалютеВМестныхБанках;
		СтрокаСчета.СПодчиненными = Истина;
...
// Получает струтуру параметров учета по счету.
//
// Параметры:
//  ПараметрыУчета			 - Струтктура	 - Содержит настроки параметров учета до изменения.
//  ПараметрыГруппыСчетов	 - Струтктура	 - 
//  Счет					 - ПланСчетовСсылка.Хозрасчетый	 - 
// 
// Возвращаемое значение:
//   Результат - Структура.
//
