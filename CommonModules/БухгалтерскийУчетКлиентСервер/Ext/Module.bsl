...
//
// Параметры:
//  СтруктураПараметров	 - Обязательные ключи структуры	 - 
//		* ИНН - Строка
//		* ЭтоЮрЛицо - Булево
//		* ПроверитьИНН - Булево
// 
...
//
Процедура ЗаписатьСправочникОбъект(СправочникОбъект, Сообщать = Ложь, Оповещать = Ложь, Отказ = Ложь, ОбменДаннымиЗагрузка = Ложь) Экспорт
...
Процедура НастроитьСвязьСчетаИСубконтоВТЧ(Форма, Суффикс, ИмяСчета, ИмяТЧ, СтрокаТабличнойЧасти, ПоляОбъекта = Неопределено) Экспорт
	...
	КонецЕсли;
	ПараметрыДокумента = СписокПараметровВыбораСубконто(Форма, СтрокаТабличнойЧасти, "Субконто" + Суффикс + "%Индекс%", ИмяСчета);
...
// Процедура - При изменении счета
// Определение необходимости очистики ранее заполненных значений субконто
//
// Параметры:
//  Счет			 - 	 - 
//  Объект			 - 	 - 
...
//   - 
//
Функция СписокПараметровВыбораСубконто(Форма, Объект, ШаблонИмяПоляОбъекта, ИмяРеквизитаСчетаУчета) Экспорт
	...
	// Счет.
	СписокПараметров.Вставить("СчетУчета", Объект[ИмяРеквизитаСчетаУчета]);
	...
	// Вид договора.
	ВидОперации = ?(Форма.Объект.Свойство("ВидОперации"), Форма.Объект.ВидОперации, Неопределено);
	МассивПараметровВыбора = ПолучитьМассивПараметровВыбораДоговора(Форма.Объект.Ссылка, ВидОперации);  
	СписокПараметров.Вставить("ВидДоговора", Новый ФиксированныйМассив(МассивПараметровВыбора));
...
Функция ПолучитьМассивПараметровВыбораДоговора(Документ, ВидОперации = Неопределено) Экспорт 
	...
	ИначеЕсли ТипЗнч(Документ) = Тип("ДокументСсылка.ПлатежноеПоручениеВходящее") Тогда
		...
		Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийППВ.ОплатаОтПокупателя") Тогда
			НовыйМассив.Добавить(ПредопределенноеЗначение("Перечисление.ВидыДоговоровКонтрагентов.СПокупателем"));
