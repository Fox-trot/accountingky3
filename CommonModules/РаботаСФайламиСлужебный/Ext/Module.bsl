...
Процедура ПроверкаСсылокНаНесуществующиеФайлыВТоме(Проверка, ПараметрыПроверки) Экспорт
	...
	Для Каждого ВидМетаданных Из МассивВидовОбъектовМетаданных Цикл
		Для Каждого ОбъектМетаданных Из ВидМетаданных Цикл
			Если МодульКонтрольВеденияУчетаСлужебный.ЭтоНеразделенныеДанные(ОбъектМетаданных) Тогда
				Продолжить;
			КонецЕсли;
...
Процедура ПриПолученииФайла(ЭлементДанных, ПолучениеЭлемента, Отправитель = Неопределено)
	...
	Если ОбработатьПолученныеФайлы Тогда
		...
		Если Отправитель <> Неопределено И ПланыОбмена.ИзменениеЗарегистрировано(Отправитель.Ссылка, ЭлементДанных) Тогда
				// Коллизия объекта (изменения зарегистрированы и на главном узле и на подчинённом).
				ПолучениеЭлемента = ПолучениеЭлементаДанных.Игнорировать;
				Возврат;
		КонецЕсли;
...
Функция ВыбратьДанныеПоПравилам(УчетнаяЗапись)
...
Функция СведенияОСинхронизации(ВладелецФайла = Неопределено) Экспорт
	...
	Запрос.Текст = 
		...
		|	СтатусыСинхронизацииФайловСОблачнымСервисомКорень.ДатаСинхронизацииНачало КАК ДатаСинхронизации,
		|	СтатусыСинхронизацииФайловСОблачнымСервисомКорень.НомерСеанса КАК НомерСеанса,
		|	СтатусыСинхронизацииФайловСОблачнымСервисомКорень.Синхронизирован КАК Синхронизирован,
		|	СтатусыСинхронизацииФайловСОблачнымСервисомКорень.ДатаСинхронизацииЗавершение КАК ДатаСинхронизацииЗавершение,
		|	СтатусыСинхронизацииФайловСОблачнымСервисом.Href КАК Href,
		...
		|	И СтатусыСинхронизацииФайловСОблачнымСервисом.Синхронизирован = ЛОЖЬ";
