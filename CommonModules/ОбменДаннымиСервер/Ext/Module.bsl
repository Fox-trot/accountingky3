...
Функция ЕстьПодключениеККорреспонденту(Знач Корреспондент, Знач СтруктураНастроек, СообщениеПользователю = "") Экспорт
	...
	Если ВерсияКорреспондента_2_1_1_7
		...
		ИначеЕсли ВерсияКорреспондента_3_0_1_1 Тогда
		...
		Попытка
			...
			КодКорреспондента = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(
				ОбменДаннымиПовтИсп.ПолучитьЭтотУзелПланаОбменаПоСсылке(Корреспондент), "Код");
			...
			Если Не ЕстьПодключение
				...
				Для Каждого ИмяСтарогоПланаОбмена Из ОбменДаннымиПовтИсп.ПланыОбменаБСП() Цикл
					...
					ИмяНовогоПланаОбмена = ЗначениеНастройкиПланаОбмена(ИмяСтарогоПланаОбмена,
						"ИмяПланаОбменаДляПереходаНаНовыйОбмен", ВариантНастройки);
					
					Если ИмяНовогоПланаОбмена = ИмяПланаОбмена Тогда
						ЕстьПодключение = WSПрокси.TestConnection(
