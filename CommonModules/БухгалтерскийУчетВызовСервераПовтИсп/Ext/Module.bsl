...
// Все активные счета учета с субконто "Основные средства"
//
// Возвращаемое значение:
...
Функция СчетаУчетаОсновныхСредствИНематериальныхАктивов() Экспорт 
	...
	Запрос.Текст = 
		...
		|	ПланСчетов.Хозрасчетный КАК Хозрасчетный
		|ГДЕ
		|	Хозрасчетный.Родитель = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ОсновныеСредства)
		...
		|	ПланСчетов.Хозрасчетный КАК Хозрасчетный
		|ГДЕ
		|	Хозрасчетный.Родитель = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НематериальныеАктивы)
		|	И Хозрасчетный.Вид = ЗНАЧЕНИЕ(ВидСчета.Активный)
		|	И НЕ Хозрасчетный.ПометкаУдаления
		|	И НЕ Хозрасчетный.ЗапретитьИспользоватьВПроводках";
	Возврат Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Счет");
...
Функция ПолучитьСведенияОбОрганизации(Знач Организация, Знач ДатаЗначения = Неопределено, Знач СписокПоказателей = Неопределено) Экспорт
	...
	Если СписокПоказателей = Неопределено Тогда
		...
		СписокПоказателей.Добавить("ПолныйАдрФакт");
	...
	Для Каждого Показатель Из СписокПоказателей Цикл
		...
		ИначеЕсли ИмяПоказателя = "ПолныйАдрФакт" Тогда
		...
		ИначеЕсли ИмяПоказателя = "Страна" Тогда
			...
			Если Результат <> Неопределено Тогда
				...
				Если НЕ ЗначениеЗаполнено(СоответствиеАдреса["Страна"]) Тогда 
					СформироватьАдрес(Результат.ЗначенияПолей, СоответствиеАдреса);
				КонецЕсли;	
...
Функция АдресВФормате9Запятых(Запись) Экспорт
	...
	Если СтрЧислоСтрок(Запись.ЗначенияПолей) = 1
	   И СтрЧислоВхождений(Запись.ЗначенияПолей, "КонтактнаяИнформация") = 2 Тогда
		...
		КодСтраны = СтруктураАдресаЗаписи.КодСтраны + ",";
	...
	Иначе
		...
		Возврат КодСтраны + АдресБезКодаСтраны;
...
Процедура СформироватьАдрес(КонтактнаяИнформация, СоответствиеАдреса) Экспорт
	...
	Для Каждого Элемент Из СоответствиеАдреса Цикл
		СоответствиеАдреса[Элемент.Ключ] = "";
	КонецЦикла;
	...
	Для НомСтр = 1 По СтрЧислоСтрок(ЗначенияПолей) Цикл
		...
		Если ТипЗнч(СоответствиеАдреса) = Тип("Соответствие") И ЗначениеЗаполнено(ПредставлениеСтр) И НЕ СоответствиеАдреса.Получить(ПредставлениеСтр) = Неопределено Тогда
			СоответствиеАдреса[ПредставлениеСтр] = ЗначениеСтр;
		ИначеЕсли ТипЗнч(СоответствиеАдреса) = Тип("Структура") И ЗначениеЗаполнено(ПредставлениеСтр) И СоответствиеАдреса.Свойство(ПредставлениеСтр) Тогда
			СоответствиеАдреса[ПредставлениеСтр] = ЗначениеСтр;
		КонецЕсли;
...
// Функция возвращает основной вид деятельности.
//
Функция ПолучитьОсновнойВидДеятельности() Экспорт

	Возврат Перечисления.ВидыДеятельностиЕН.ПереработкаПроизводствоТорговля;	

КонецФункции // ПолучитьОсновнойВидДеятельностиПоОрганизации()
