...
Процедура ПрочитатьНаборЗаписей(Форма, ИмяРегистра, ВедущийОбъект) Экспорт
	...
	Если ИмяРегистра = "КвалификацияФизическихЛиц" И ТипЗнч(ВедущийОбъект) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
		ИмяИзмерения = Метаданные.РегистрыСведений[ИмяРегистра].Измерения[1].Имя;
	Иначе	
		ИмяИзмерения = Метаданные.РегистрыСведений[ИмяРегистра].Измерения[0].Имя;
	КонецЕсли;
...
Процедура ПроверитьЗаполнениеПоляЗаписиРегистраВФорме(МенеджерЗаписи, ИмяРегистра, ОписаниеПоля, Синоним, Отказ, ПутьКРеквизитуФормы = "")
	...
	Если ОписаниеПоля.ПроверкаЗаполнения = ПроверкаЗаполнения.ВыдаватьОшибку И Не ЗначениеЗаполнено(МенеджерЗаписи[ОписаниеПоля.Имя]) Тогда
		ТекстСообщения = СтрШаблон(НСтр("ru = '%1: не заполнено поле ""%2"".'"), Синоним, ?(ЗначениеЗаполнено(ОписаниеПоля.Синоним), ОписаниеПоля.Синоним, ОписаниеПоля.Имя));
...
Процедура ЗаписатьНаборЗаписей(Форма, ИмяРегистра, ВедущийОбъект, ИсключаемыеИменаКолонок = "", ДополнительныеСвойства = Неопределено) Экспорт
	...
	ТаблицаЗаписейФормы = Форма[ИмяРегистра + "НаборЗаписей"].Выгрузить();
	ТаблицаЗаписейФормы.Колонки.Удалить("ИсходныйНомерСтроки");
	ТаблицаЗаписейФормы.ЗаполнитьЗначения(ВедущийОбъект, ИмяИзмерения);
	...
	ТаблицаЗаписейБазыДанных = РегистрыСведений[ИмяРегистра].СоздатьНаборЗаписей();
	ТаблицаЗаписейБазыДанных.Отбор[ИмяИзмерения].Установить(ВедущийОбъект);
	ТаблицаЗаписейБазыДанных.Прочитать();
	
	Если НЕ ОбщегоНазначения.КоллекцииИдентичны(ТаблицаЗаписейФормы, ТаблицаЗаписейБазыДанных) Тогда
		ТаблицаЗаписейБазыДанных.Загрузить(ТаблицаЗаписейФормы);
		ТаблицаЗаписейБазыДанных.Записать();
	КонецЕсли; 
