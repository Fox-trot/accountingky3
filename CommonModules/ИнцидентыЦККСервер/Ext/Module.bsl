	...
	Если Результат <> "OK" Тогда
		ЗаписьЖурналаРегистрации(
			НСтр("ru = 'ИнтеграцияЦКК';
				|en = 'QCCIntegration'", ОбщегоНазначения.КодОсновногоЯзыка()),
			УровеньЖурналаРегистрации.Ошибка,,, 
			НСтр("ru = 'Ошибка при передаче счетчика в ЦКК';
				|en = 'An error occurred while transferring counter to QCC'", ОбщегоНазначения.КодОсновногоЯзыка()));
	КонецЕсли;
	...
	Если Ответ.КодСостояния<>200 Тогда
		ЗаписьЖурналаРегистрации(
			...
			НСтр("ru = 'Ошибка (код %3) команды %1 сервиса инцидентов ЦКК: %2';
				|en = 'QCC incident service command %1 (code %3) failed: %2'", ОбщегоНазначения.КодОсновногоЯзыка()), 
