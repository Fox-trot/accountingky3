...
Процедура ОткрытьПодбор(ФормаВладелец, ИмяТабличнойЧасти) Экспорт
	...
	Если НЕ Отказ Тогда
		ОписаниеОповещенияПриЗакрытииПодбора = Новый ОписаниеОповещения("ПриЗакрытииПодбора", УправлениеВнеоборотнымиАктивамиКлиент.ЭтотОбъект);
...
Процедура ЗаполнитьПараметрСтруктурнаяЕдиница(ФормаВладелец, ПараметрыПодбора)
	...
	Если СтрНайти(ФормаВладелец.ИмяФормы, "ПоступлениеТоваровУслуг") > 0 
		ИЛИ СтрНайти(ФормаВладелец.ИмяФормы, "ПринятиеКУчетуОС") > 0 Тогда
...
Процедура ЗаполнитьДополнительныеПараметры(ФормаВладелец, ПараметрыПодбора)
	...
	Если ФормаВладелец.ИмяФормы = "Документ.ПоступлениеТоваровУслуг.Форма.ФормаДокумента" 
	...
	ИначеЕсли ФормаВладелец.ИмяФормы = "Документ.ПередачаОС.Форма.ФормаДокумента"
		Или ФормаВладелец.ИмяФормы = "Документ.СписаниеОС.Форма.ФормаДокумента" Тогда
