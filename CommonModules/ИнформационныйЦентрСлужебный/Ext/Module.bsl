	...
	Пока ЧтениеИнформационныхСсылок.ТипУзла = ТипУзлаXML.НачалоЭлемента Цикл
		...
		Иначе
			...
			"." + НСтр("ru = 'Информационные ссылки';
						|en = 'Information links'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция ЕстьФормаПоПолномуПути(ПолныйПутьКФорме, ЗаписыватьОшибкуВЖР = Истина)
	...
	Если ЗаписыватьОшибкуВЖР Тогда
		ИмяСобытияЖР = ИнформационныйЦентрСервер.ПолучитьИмяСобытияДляЖурналаРегистрации()
			+ "."
			+ НСтр("ru = 'Информационные ссылки';
					|en = 'Information links'", ИнтеграцияПодсистемБТС.КодОсновногоЯзыка());
		...
		ЗаписьЖурналаРегистрации(ИмяСобытияЖР,
			...
			НСтр("ru = 'При обновлении списка форм, в конфигурации не найдена в форма с полным путем:';
				|en = 'Form with the full path was not found in the configuration when updating the form list:'", ИнтеграцияПодсистемБТС.КодОсновногоЯзыка())
				+ ПолныйПутьКФорме);
	КонецЕсли;
