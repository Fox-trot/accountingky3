...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБП.ЗаполнитьКоличествоДнейДействияВДокументеДоверенность";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение Количества дней действия в документе Доверенность.'");
	Обработчик.НачальноеЗаполнение = Ложь;	
	
	Обработчик = Обработчики.Добавить();
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Версия = "3.1.7.13";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБП.ОчиститьСубконтоСчета5130";
	Обработчик.Комментарий = НСтр("ru = 'Очищение субконто счета 5130.'");
	Обработчик.НачальноеЗаполнение = Ложь;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Версия = "3.1.7.18";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБП.ОпредлитьОтчетыВРазныеГНС";
	Обработчик.Комментарий = НСтр("ru = 'Определение нужно ли сдавать отчеты в разные ГНС и заполнение подразделения.'");
	Обработчик.НачальноеЗаполнение = Ложь;
		
	Обработчик = Обработчики.Добавить();
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Версия = "3.1.7.21";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБП.ЗаполнитьКодыПоставокНДС";
	Обработчик.Комментарий = НСтр("ru = 'Заполнение справочника Коды поставок НДС.'");
	Обработчик.НачальноеЗаполнение = Ложь;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Версия = "3.1.7.22";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБП.ЗарегестрироватьДляДоступаКСервисам";
	Обработчик.Комментарий = НСтр("ru = 'Зарегестрировать для доступа к сервисам'");
	Обработчик.НачальноеЗаполнение = Ложь;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Версия = "3.1.7.25";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБП.ЗаполнитьГНС";
	Обработчик.Комментарий = НСтр("ru = 'Добавление новых элементов в справочник ГНС'");
	Обработчик.НачальноеЗаполнение = Ложь;
	
	Обработчик = Обработчики.Добавить();
	Обработчик.РежимВыполнения = "Оперативно";
	Обработчик.Версия = "3.1.7.26";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБП.ОбновитьОтветственныеЛица";
	Обработчик.Комментарий = НСтр("ru = 'Обновление ответственных лиц.'");
	Обработчик.НачальноеЗаполнение = Ложь;
КонецПроцедуры
...
Процедура УстановитьДатуТекущейВерсии() Экспорт
	...
	Константы.ДатаТекущейВерсии.Установить(Дата(2020,02,19));
КонецПроцедуры
...
Процедура ДозаполнитьСчетаФактурыВыписанные(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Для Каждого СтрокаТабличнойЧасти Из ДокументОбъект.Товары Цикл
			СтрокаТабличнойЧасти.СтавкаНДС = СтрокаТабличнойЧасти.ДокументОснование.СтавкаНДС;
			СтрокаТабличнойЧасти.СтавкаНСП = СтрокаТабличнойЧасти.ДокументОснование.СтавкаНСП;
		КонецЦикла;
...
Процедура ОбновитьОтветственныеЛица() Экспорт
			...
			Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.ДокументОснование) Тогда
				...
				Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
					Если СтрокаТабличнойЧасти.Номенклатура.Услуга Тогда
						СтрокаТабличнойЧасти.СтавкаНСП = СтрокаТабличнойЧасти.ДокументОснование.СтавкаНСПУслуги;
					Иначе
						СтрокаТабличнойЧасти.СтавкаНСП = СтрокаТабличнойЧасти.ДокументОснование.СтавкаНСП;
					КонецЕсли;	
				КонецЕсли;	
...
#Область Обновление_3_1_7

// Вызывается при переходе на версию конфигурации 3.1.7.8.
//
Процедура ЗаполнитьКоличествоДнейДействияВДокументеДоверенность() Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = 
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	Доверенность.Ссылка КАК Ссылка,
		|	ВЫБОР
		|		КОГДА Доверенность.удалитьДатаДействия = ДАТАВРЕМЯ(1, 1, 1)
		|			ТОГДА 10
		|		ИНАЧЕ РАЗНОСТЬДАТ(Доверенность.Дата, Доверенность.удалитьДатаДействия, ДЕНЬ)
		|	КОНЕЦ КАК КоличествоДнейДействия
		|ИЗ
		|	Документ.Доверенность КАК Доверенность
		|ГДЕ
		|	Доверенность.КоличествоДнейДействия = 0
	...
	Пока Выборка.Следующий() Цикл				
		ДокументОбъект = Выборка.Ссылка.ПолучитьОбъект();
		ДокументОбъект.КоличествоДнейДействия = Выборка.КоличествоДнейДействия;
		Попытка
		 	ДокументОбъект.Записать();
		Исключение
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось заполнить Количество дней действия в доверенности: %1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
			ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,, ТекстСообщения);			
			ВызватьИсключение;		
		КонецПопытки;		
	КонецЦикла;
...
Процедура ОчиститьСубконтоСчета5130() Экспорт

	Попытка
		СчетОбъект = ПланыСчетов.Хозрасчетный.ПрочийУставныйКапитал.ПолучитьОбъект();
		СчетОбъект.ВидыСубконто.Очистить();
		ОбновлениеИнформационнойБазы.ЗаписатьОбъект(СчетОбъект);			
	Исключение
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Не удалось очистить субконто на счете 5130 по причине: %1'"), ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(НСтр("ru = 'Ошибка'", ОбщегоНазначения.КодОсновногоЯзыка()), УровеньЖурналаРегистрации.Ошибка,,, ТекстСообщения);
...
// Вызывается при переходе на версию конфигурации 3.1.7.18.
//
Процедура ОпредлитьОтчетыВРазныеГНС() Экспорт

	// Определение по записям регистра сведений ПределыПоПодразделениям
	НаборЗаписей = РегистрыСведений.ПределыПоПодразделениям.СоздатьНаборЗаписей();
	НаборЗаписей.Прочитать();
	
	Если НаборЗаписей.Количество() > 0 Тогда 
		Константы.ФункциональнаяОпцияОтчетыВРазныеГНС.Установить(Истина);		
	КонецЕсли;
	
	Для Каждого ЗаписьНабора Из НаборЗаписей Цикл 
		СправочникОбъект = ЗаписьНабора.Подразделение.ПолучитьОбъект();	
		СправочникОбъект.ГНС = ЗаписьНабора.УГНС;
...
// Вызывается при переходе на версию конфигурации 3.1.7.21.
//
Процедура ЗаполнитьКодыПоставокНДС() Экспорт
	
	// Ранее были предопределенные элементы
	// они удалены, поэтому нужно снять пометку удаления
	// и заполнить Наименование из макета.
	// Заполнение реквизита "Владелец"
	СправочникМенеджер = Справочники.КодыПоставокНДС;
	...
	Для Каждого СтрокаТаблицыЗначений Из КлассификаторТаблица Цикл 
		...
		ВидПоставки = Справочники.ВидыПоставокНДС[СтрокаТаблицыЗначений.ВидПоставки];
		...
		СправочникОбъект.ВидПоставки = ВидПоставки;
...
// Вызывается при переходе на версию конфигурации 3.1.7.22.
//
Процедура ЗарегестрироватьДляДоступаКСервисам() Экспорт
	...
	СтандартныеПодсистемыСервер.ПометитьЗаписиКэшаВерсийНеактуальными();
	
КонецПроцедуры

// Вызывается при переходе на версию конфигурации 3.1.7.25.
//
Процедура ЗаполнитьГНС() Экспорт
	
	СпрОбъект = Справочники.ГНС.СоздатьЭлемент();
	СпрОбъект.Код = "997";
	СпрОбъект.Наименование = "УККН Юг";
	ОбновлениеИнформационнойБазы.ЗаписатьДанные(СпрОбъект);
	
	СпрОбъект = Справочники.ГНС.СоздатьЭлемент();
	СпрОбъект.Код = "998";
	СпрОбъект.Наименование = "СЭЗ Бишкек";
	ОбновлениеИнформационнойБазы.ЗаписатьДанные(СпрОбъект);
