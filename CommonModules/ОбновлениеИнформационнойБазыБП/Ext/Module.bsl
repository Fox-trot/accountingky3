...
Процедура УстановитьДатуТекущейВерсии() Экспорт
	Константы.ДатаТекущейВерсии.Установить(Дата(2019,09,04));
КонецПроцедуры

Процедура УстановкаИУдалениеИсправлений() Экспорт
	Возврат;
	
	ОбновитьИсправления = Ложь;
	...
	Для Каждого Расширение Из ВсеРасширения Цикл
		Если ОбновлениеКонфигурации.ЭтоИсправление(Расширение) Тогда
			ОбновитьИсправления = Истина;
			Прервать;
		КонецЕсли;
	КонецЦикла;

	Если ОбновитьИсправления Тогда 
		ОбновлениеКонфигурации.ОбновитьИсправленияИзСкрипта(, "b2b78783-a249-48c0-8cff-0e6f470c785e");
	КонецЕсли;	
КонецПроцедуры
