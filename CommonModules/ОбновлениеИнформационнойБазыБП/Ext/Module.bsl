...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.РежимВыполнения = "Отложенно";
	Обработчик.Версия = "3.0.17.14";
	Обработчик.Процедура = "ОбновлениеИнформационнойБазыБП.ЗаполнитьКурсКратностьНБКР_ВПоступлении";
	Обработчик.Идентификатор = Новый УникальныйИдентификатор("8db104ca-ec0c-4288-8fb3-7ce11067cba8");
	Обработчик.Комментарий = НСтр("ru = 'Обновление проверок учета.'");
...
Процедура УстановитьДатуТекущейВерсии() Экспорт
	Константы.ДатаТекущейВерсии.Установить(Дата(2019,09,24));
КонецПроцедуры
...
Процедура ЗаполнитьСуммыВВалютеРегламентированногоУчетаВВозврате(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл	
		Документобъект = Выборка.Ссылка.ПолучитьОбъект();
		Документобъект.РасчитатьСуммыВВалютеРеглУчета = Истина;
		Документобъект.Записать();
	КонецЦикла;
...
#Область Обновление_3_0_17_14

Процедура ЗаполнитьКурсКратностьНБКР_ВПоступлении(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл	
		...
		Структура = РаботаСКурсамиВалют.ПолучитьКурсВалюты(Выборка.ВалютаДокумента, Выборка.Дата);
		
		Документобъект.КурсНБКР      = ?(Структура.Курс = 0, 1, Структура.Курс);
		Документобъект.КратностьНБКР = ?(Структура.Кратность = 0, 1, Структура.Кратность); 
...
Процедура ЗаполнениеМетодовРасчетаВСтатусах(Параметры) Экспорт
	...
	Пока Выборка.Следующий() Цикл	
		...
		Если Выборка.Категория = Справочники.КатегорииСотрудников.Арендодатель Тогда 
