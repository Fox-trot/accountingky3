...
Процедура ПослеПопыткиПодключенияВнешнейКомпоненты1СДляСбербанка(Результат, Параметры) Экспорт
	...
	Если Результат Тогда
		...
		Исключение
			...
			ПодробноеПредставлениеОшибки = ПодробноеПредставлениеОшибки(ИнформацияОбОшибке());
			ОбработатьОшибку(Операция, ПодробноеПредставлениеОшибки, ТекстСообщения);
			ВыполнитьОбработкуОповещения(Параметры.ОповещениеПослеПодключенияКомпоненты1СДляСбербанка, Неопределено);
			Возврат;
		КонецПопытки;
		Параметры.Вставить("ПодключаемыйМодуль", ПодключаемыйМодуль);
		УстановитьТаймаут(Параметры);
	Иначе
...
Процедура УстановитьТаймаут(Параметры)
	
	Оповещение = Новый ОписаниеОповещения("ПослеУстановкиТаймаута", ЭтотОбъект, Параметры);
	
	Параметры.ПодключаемыйМодуль.НачатьВызовУстановитьТаймаут(Оповещение, 100000);
	
КонецПроцедуры

Процедура ПослеУстановкиТаймаута(РезультатВызова, ПараметрыВызова, ДополнительныеПараметры) Экспорт
	
	ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПослеПодключенияКомпоненты1СДляСбербанка,
		ДополнительныеПараметры.ПодключаемыйМодуль);
...
Процедура ПроверитьАктуальностьВерсииВКПослеПолученияСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ПоддерживаетсяВерсияКомпонентыСбербанк(Версия) Тогда
		ВыполнитьОбработкуОповещения(
...
Процедура ОтправитьДокументыВБанк(ДанныеАутентификации, Параметры) Экспорт
	...
	Если НЕ Параметры.ТребуетсяАвторизация ИЛИ ЗначениеЗаполнено(ДанныеАутентификации) Тогда
		...
		ИначеЕсли РеквизитыНастройкиОбмена.ПрограммаБанка = ПредопределенноеЗначение("Перечисление.ПрограммыБанка.ОбменЧерезДопОбработку") Тогда
			...
			ОтправитьДокументыЧерезДополнительнуюОбработку(
				Оповещение, Параметры.НастройкаОбмена, РеквизитыНастройкиОбмена.ИмяВнешнегоМодуля, ДанныеДляОтправкиПоНастройке);
...
Процедура ОтправитьДокументыЧерезДополнительнуюОбработку(Оповещение, НастройкаОбмена, ИмяМодуля, ДанныеДляОтправки)
	...
	ПолучитьВнешнийМодульЧерезДополнительнуюОбработку(ОбработчикПослеПодключения, ИмяМодуля);
