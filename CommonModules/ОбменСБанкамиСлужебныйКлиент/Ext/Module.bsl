...
//  НастройкаОбмена - СправочникСсылка.НастройкиОбменСБанками - текущая настройка обмена с банком.
//
Процедура АутентифицироватьсяПоСертификатуСбербанк(Оповещение, ИдентификаторВК, НастройкаОбмена = Неопределено) Экспорт
	...
	ОповещениеПослеПодключенияВК = Новый ОписаниеОповещения(
		"АутентифицироватьсяПоСертификатуПослеПодключенияВКСбербанк", ЭтотОбъект, ДополнительныеПараметры);
...
Процедура ОпределитьСертификатПодписиСбербанк(Оповещение, ИдентификаторВК, НастройкаОбмена, ОграничениеПоСертификатам = Неопределено) Экспорт
	...
	ОповещениеПослеПодключенияВК = Новый ОписаниеОповещения("ОпределитьСертификатПодписиПослеПодключенияВКСбербанк", ЭтотОбъект, ДополнительныеПараметры);
	ПодключитьКомпоненту(ОповещениеПослеПодключенияВК, ИдентификаторВК);
...
Процедура ПровестиАутентификациюПослеПолученияФродПараметровСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Оповещение = Новый ОписаниеОповещения(
		"ВызватьАутентификациюПослеПодключенияВК1ССбербанк", ЭтотОбъект, ДополнительныеПараметры);
	
	ПодключитьВнешнююКомпоненту1СДляСбербанка(Оповещение);
	
КонецПроцедуры

Процедура ВызватьАутентификациюПослеПодключенияВК1ССбербанк(ПодключаемыйМодуль1С, ДополнительныеПараметры) Экспорт
	...
	ПодключаемыйМодуль1С.НачатьВызовАутентификация(Оповещение, ДополнительныеПараметры.ИдентификаторСессии,
		ДополнительныеПараметры.ПодписьСоли, ДополнительныеПараметры.СтрокаФрод, ИдентификаторСессии, КодВозврата);
...
//  ПодключаемыйМодуль - AddIn - внешняя компонента банка.
//
Процедура ПолучитьСертификатСТокенаСбербанк(Оповещение, ПодключаемыйМодуль)
...
Процедура АутентифицироватьсяПоСертификатуПослеПодключенияВКСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат.Подключено Тогда
		...
		ПолучитьСертификатСТокенаСбербанк(ОповещениеПослеУстановкиКанала, Результат.ПодключаемыйМодуль);
	Иначе
		Если НЕ ПустаяСтрока(Результат.ОписаниеОшибки) Тогда
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(Результат.ОписаниеОшибки);
		КонецЕсли;
