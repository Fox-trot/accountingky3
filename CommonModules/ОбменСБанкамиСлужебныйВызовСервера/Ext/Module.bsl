...
Функция НазваниеБизнесСистемыСбербанк() Экспорт
	...
	Если ИспользуетсяТестовыйРежим Тогда
		НазваниеБизнесСистемы = "sbbolsbt_upg2_int";
	КонецЕсли;
...
Функция МаркерБанкаПоSMS(Знач АдресСервера, Знач ИдентификаторОрганизации, Знач ИдентификаторСессии, Знач ОдноразовыйПароль, Знач ВерсияAPI, Знач НастройкаОбмена = Неопределено) Экспорт
	...
	// БПКР
	Заголовки.Вставить("SMS", ОдноразовыйПароль);
	...
	Результат = ОбменСБанкамиСлужебный.ОтправитьPOSTЗапрос(АдресСервера, "v1/ms-api-1c/logonSMS", Заголовки, ТелоЗапроса, , 15);
	// Конец БПКР
	Если Не Результат.Статус Тогда
		Если ЗначениеЗаполнено(Результат.КодСостояния) Тогда
...
Процедура СформироватьЗапросЗонд(Знач НастройкаОбмена, Знач МассивОтпечатковСертификатов, НовоеСообщениеОбмена, НастройкиОбмена) Экспорт
	...
	Попытка
		...
		// БПКР
		ОбменСБанкамиСлужебный.ЗаполнитьСвойствоXDTO(Отправитель, "kpp", "770501001", , ТекстОшибки);
		// Конец БПКР
...
Функция БазоваяАутентификация(Знач АдресСервера, Знач ИдентификаторОрганизации, Знач ДанныеАутентификации, Знач ВерсияAPI, Знач НастройкаОбмена = Неопределено) Экспорт
	...
	// БПКР
	Заголовки.Вставить("Cookie", "rib_web_session=;rib_int_session=;");
	...
	Результат = ОбменСБанкамиСлужебный.ОтправитьPOSTЗапрос(АдресСервера, "v1/ms-api-1c/logon", Заголовки, ТелоЗапроса, , 50);
	// Конец БПКР
