	...
	Если ТипЗнч(Хранилище) = Тип("ХранилищеЗначения") Тогда
		...
		Если ТипЗнч(Настройки) = Тип("Структура") Тогда
			Если Не Настройки.Свойство("ВерхнийКолонтитул") 
				ИЛИ Не Настройки.Свойство("НижнийКолонтитул")
				ИЛИ Не Настройки.Свойство("Отступ") Тогда
				Настройки = Неопределено;
			Иначе
				ДополнитьНастройкиКолонтитула(Настройки.ВерхнийКолонтитул);
				ДополнитьНастройкиКолонтитула(Настройки.НижнийКолонтитул);
				ДополнитьНастройкиКолонтитула(Настройки.Отступ);
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Процедура УстановитьКолонтитулы(ТабличныйДокумент, НазваниеОтчета = "", Пользователь = Неопределено) Экспорт
	...
	ТабличныйДокумент.ПолеСлева   = НастройкиКолонтитулов.Отступ.ПолеСлева;
	ТабличныйДокумент.ПолеСправа  = НастройкиКолонтитулов.Отступ.ПолеСправа;
	ТабличныйДокумент.ПолеСверху  = НастройкиКолонтитулов.Отступ.ПолеСверху;
	ТабличныйДокумент.ПолеСнизу  = НастройкиКолонтитулов.Отступ.ПолеСнизу;
...
Функция ПолучитьСтруктуруНастроек()
	...
	Отступ = Новый Структура();
	Отступ.Вставить("ПолеСверху", 10);
	Отступ.Вставить("ПолеСлева", 10);
	Отступ.Вставить("ПолеСнизу", 10);
	Отступ.Вставить("ПолеСправа", 10);
	
	Настройки = Новый Структура("ВерхнийКолонтитул,НижнийКолонтитул, Отступ", ВерхнийКолонтитул, НижнийКолонтитул, Отступ);
...
Процедура ДополнитьНастройкиКолонтитула(НастройкиКолонтитула)
	...
	Если Не НастройкиКолонтитула.Свойство("ПолеСверху")
		ИЛИ ТипЗнч(НастройкиКолонтитула.ПолеСверху) <> Тип("Число") Тогда
		НастройкиКолонтитула.Вставить("ПолеСверху", 10);
	КонецЕсли;
	Если Не НастройкиКолонтитула.Свойство("ПолеСлева")
		ИЛИ ТипЗнч(НастройкиКолонтитула.ПолеСлева) <> Тип("Число") Тогда
		НастройкиКолонтитула.Вставить("ПолеСлева", 10);
	КонецЕсли;
	Если Не НастройкиКолонтитула.Свойство("ПолеСнизу")
		ИЛИ ТипЗнч(НастройкиКолонтитула.ПолеСнизу) <> Тип("Число") Тогда
		НастройкиКолонтитула.Вставить("ПолеСнизу", 10);
	КонецЕсли;
	Если Не НастройкиКолонтитула.Свойство("ПолеСправа")
		ИЛИ ТипЗнч(НастройкиКолонтитула.ПолеСправа) <> Тип("Число") Тогда
		НастройкиКолонтитула.Вставить("ПолеСправа", 10);
	КонецЕсли;
