...
Процедура ПодготовитьОбластьДанныхДляМиграции(КодОбластиДанных, Параметры) Экспорт
	...
	Если Не ИнтеграцияПодсистемБТС.ПодсистемаСуществует("ТехнологияСервиса.МиграцияПриложений") Тогда
		ТекстСообщения = НСтр("ru = 'Миграция приложений не поддерживается';
								|en = 'Application migration is not supported'", ОбщегоНазначения.КодОсновногоЯзыка());
		ВызватьИсключение ТекстСообщения;
	КонецЕсли;
...
Функция СобытиеЖурналаРегистрацииУдаленноеАдминистрированиеУстановитьПараметры() Экспорт
	...
	Возврат НСтр("ru = 'Удаленное администрирование в модели сервиса.Установить параметры';
				|en = 'Remote administration in SaaS.Set options'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЖурналаРегистрацииУдаленноеАдминистрированиеУдалитьОбласть() Экспорт
	...
	Возврат НСтр("ru = 'Удаленное администрирование в модели сервиса.Удалить область';
				|en = 'Remote administration in SaaS.Delete area'", ОбщегоНазначения.КодОсновногоЯзыка());
