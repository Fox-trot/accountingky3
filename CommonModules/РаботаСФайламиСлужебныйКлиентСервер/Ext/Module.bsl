...
#Если Не ВебКлиент Тогда
	...
	ЧислоСекундТекущейДаты = ТекущаяДата() - '00010101';
	ГенераторСлучая = Новый ГенераторСлучайныхЧисел(ЧислоСекундТекущейДаты);
#КонецЕсли
...
#Иначе
		...
		// поэтому приведение к ТекущаяДатаСеанса не требуется.
		ЧислоСекундТекущейДаты = ТекущаяДата() - '00010101';
		НомерКаталога = ЧислоСекундТекущейДаты % КоличествоСлучайныхВариантов;
#КонецЕсли
