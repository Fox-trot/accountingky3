...
Функция ПроверитьДанныеОбщая(Проверки, Исправлять, СохранятьРезультат, Адрес = Неопределено)
	...
	Комментарий = СтрШаблон(НСтр("ru = 'Количество проверок: %1.';
								|en = 'Number of checks: %1.'", ОбщегоНазначения.КодОсновногоЯзыка()), КоличествоПроверок);
	...
	Если Не ТребуетсяЗаполнитьНастройки(Проверки, Исправлять, Обработчики) Тогда
		...
		Для Каждого Проверка Из Проверки Цикл
			...
			Комментарий = НСтр("ru = 'Начало выполнения проверки.';
								|en = 'Check start.'", ОбщегоНазначения.КодОсновногоЯзыка());
			...
			Комментарий = СтрШаблон(НСтр("ru = 'Результат проверки: %1.';
										|en = 'Check result: %1.'", ОбщегоНазначения.КодОсновногоЯзыка()), РезультатПроверки.ПредставлениеРезультата);
			ЗаписьЖурналаРегистрации(ИмяСобытия, УровеньЖурналаРегистрации.Информация, , , Комментарий);
	...
	Если Результат.Выполнено Тогда
		Комментарий = НСтр("ru = 'Проверки выполнены.';
							|en = 'Checks executed.'", ОбщегоНазначения.КодОсновногоЯзыка());
	Иначе
		Комментарий = НСтр("ru = 'Требуется заполнение настроек';
							|en = 'It is required to fill in settings'", ОбщегоНазначения.КодОсновногоЯзыка());
	КонецЕсли;
...
Функция ШаблонОшибкиВыполненияПроверки()
	...
	Возврат НСтр("ru = 'Не удалось выполнить проверку по причине: %1';
				|en = 'Cannot check due to: %1'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЗапускЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Проверка и корректировка данных.Запуск';
				|en = 'Check and correct data.Start '", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЗавершениеЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Проверка и корректировка данных.Завершение';
				|en = 'Check and correct data. Complete'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеЗаписьИсторииЖурналаРегистрации()
	...
	Возврат НСтр("ru = 'Проверка и корректировка данных.Запись истории';
				|en = 'Check and correct data.Save history'", ОбщегоНазначения.КодОсновногоЯзыка());
...
Функция СобытиеПроверкиЖурналаРегистрации(Проверка)
	...
	Возврат СтрШаблон(НСтр("ru = 'Проверка и корректировка данных.%1';
							|en = 'Check and correct data.%1'", ОбщегоНазначения.КодОсновногоЯзыка()), Проверка);
