	...
	Пока ВыборкаПорядок.Следующий() Цикл
		...
		ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.БазыРаспределенияКосвенныхРасходов.МатериальныеЗатраты Тогда 
			...
			Текст = Текст + 
				...
				|			ТОГДА 0
				|		ИНАЧЕ СпецификацииНоменклатурыИсходныеКомплектующие.Количество / СпецификацииНоменклатурыИсходныеКомплектующие.Ссылка.Количество * ВременнаяТаблицаВыпускМассивПродукции.КоличествоОборот
