...
Процедура КорректировкаСебестоимостиПродукции(ТаблицаРеквизиты, Движения, Отказ) Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаУровней Цикл 
		...
		НайденныеСтроки[0].Порядок = СтрокаТаблицы.Уровень;
	КонецЦикла;	
	...
	СчетаУчетаПрямыхРасходов = УчетПроизводства.ПредопределенныеСчетаПрямыхРасходов();
	...
	Запрос.Текст = 
		...
		|	ТаблицаПродукция.Порядок КАК Порядок
		|ПОМЕСТИТЬ ВременнаяТаблицаПорядок
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаПорядок.Продукция КАК Продукция,
		|	ВременнаяТаблицаПорядок.Порядок КАК Порядок
		|ИЗ
		|	ВременнаяТаблицаПорядок КАК ВременнаяТаблицаПорядок
		|
		|УПОРЯДОЧИТЬ ПО
		|	ВременнаяТаблицаПорядок.Порядок УБЫВ
		|ИТОГИ ПО
		|	ВременнаяТаблицаПорядок.Порядок";
	...
	Пока ВыборкаПорядок.Следующий() Цикл
		...
		ТекстЗапросаФрагмент =
			...
			|	ХозрасчетныйОстатки.СуммаОстатокДт КАК СуммаКРаспределению
			|ПОМЕСТИТЬ ВременнаяТаблицаКРаспределению
			|ИЗ
			|	РегистрБухгалтерии.Хозрасчетный.Остатки(
			|			&КонецПериода,
		...
		Если Реквизиты.БазаРаспределения = Перечисления.БазыРаспределенияКосвенныхРасходов.ПлановаяСебестоимость Тогда
			...
			ТекстЗапросаФрагмент =
				...
				|	ВременнаяТаблицаКРаспределению.СуммаКРаспределению КАК СуммаКРаспределению
				|ИЗ
				|	ВременнаяТаблицаКРаспределению КАК ВременнаяТаблицаКРаспределению
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаВыпускМассивПродукции КАК ВременнаяТаблицаВыпускМассивПродукции
				|		ПО ВременнаяТаблицаКРаспределению.СчетЗатрат = ВременнаяТаблицаВыпускМассивПродукции.СчетЗатрат
				|			И ВременнаяТаблицаКРаспределению.Подразделение = ВременнаяТаблицаВыпускМассивПродукции.Подразделение
				|			И ВременнаяТаблицаКРаспределению.НоменклатурнаяГруппа = ВременнаяТаблицаВыпускМассивПродукции.НоменклатурнаяГруппа
				|ИТОГИ
			...
			// Определяется количество выпусказа текущий месяц и остаток на начало месяца.
			ТекстЗапросаФрагмент =
				"ВЫБРАТЬ
				|	ВременнаяТаблицаВыпускМассивПродукции.СчетСписания КАК СчетУчета,
				|	ВременнаяТаблицаВыпускМассивПродукции.Номенклатура КАК Номенклатура,
		...
		ИначеЕсли Реквизиты.БазаРаспределения = Перечисления.БазыРаспределенияКосвенныхРасходов.МатериальныеЗатраты Тогда 
			...
			Если СчетаУчетаНПС.Количество() > 0 Тогда
				...
				|	ХозрасчетныйОбороты.СуммаОборотКт КАК ПоказательРаспределения,
				|	ВременнаяТаблицаКРаспределению.СуммаКРаспределению КАК СуммаКРаспределению
				|ИЗ
				...
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаКРаспределению КАК ВременнаяТаблицаКРаспределению
				|		ПО ХозрасчетныйОбороты.Счет = ВременнаяТаблицаКРаспределению.СчетЗатрат
				|			И ХозрасчетныйОбороты.Субконто1 = ВременнаяТаблицаКРаспределению.Подразделение
				|			И ХозрасчетныйОбороты.Субконто2 = ВременнаяТаблицаКРаспределению.НоменклатурнаяГруппа
				|ГДЕ
			...
			Иначе 
				...
				|			&ВидысубконтоНС) КАК ХозрасчетныйОбороты
				|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаКРаспределению КАК ВременнаяТаблицаКРаспределению
				|		ПО ХозрасчетныйОбороты.Счет = ВременнаяТаблицаКРаспределению.СчетЗатрат
				|			И ХозрасчетныйОбороты.Субконто1 = ВременнаяТаблицаКРаспределению.Подразделение
				|			И ХозрасчетныйОбороты.Субконто2 = ВременнаяТаблицаКРаспределению.НоменклатурнаяГруппа
			...
			ТекстЗапроса = ТекстЗапроса + Текст + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
		КонецЕсли;		
		...
		ТекстЗапросаФрагмент =
			...
			|УНИЧТОЖИТЬ ВременнаяТаблицаКРаспределению";
		ТекстЗапроса = ТекстЗапроса + ТекстЗапросаФрагмент + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
		...
		ТаблицаДанныеДляКорректировкиСписания = МассивРезультатов[3].Выгрузить();
		ТаблицаСписанныеТовары = МассивРезультатов[4].Выгрузить();
		
		ВыборкаСчетЗатрат = МассивРезультатов[2].Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
		Пока ВыборкаСчетЗатрат.Следующий() Цикл
			...
			Пока ВыборкаПодразделение.Следующий() Цикл
				...
				Пока ВыборкаНоменклатурнаяГруппа.Следующий() Цикл
					СуммаКорректировки = ВыборкаНоменклатурнаяГруппа.СуммаКРаспределению;
					...
					МассивСуммКорректировки = ОбщегоНазначенияКлиентСервер.РаспределитьСуммуПропорциональноКоэффициентам(СуммаКорректировки, МассивКоэффициентов);
					...
					Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
						...
						СуммаКорректировки = МассивСуммКорректировки[ИндексВМассиве];
						...
						//НайденныеСтрокиСписание = ТаблицаСписанныеТовары.НайтиСтроки(СтруктураОтбора);
						...
						Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл 
							КоличествоКорректировки = НайденнаяСтрока.КоличествоОстаток + НайденнаяСтрока.КоличествоВыпуска;
						КонецЦикла;	
						...
						//СуммаКорректировкиОстаток = СуммаКорректировки;
						
						Для Каждого НайденнаяСтрока Из ТаблицаСписанныеТовары Цикл 
							Проводка = Движения.Хозрасчетный.Добавить();
							Проводка.Период = Реквизиты.Период;
							Проводка.Организация = Реквизиты.Организация;
							...
							//СуммаКорректировкиОстаток = СуммаКорректировкиОстаток - Проводка.Сумма;
							//КоличествоКорректировки = КоличествоКорректировки - НайденнаяСтрока.Количество;
						КонецЦикла;	
						
						//// Распределение пограшности округления корректировки до копеек.
						//// Копейка относится к завершающей проводке.
						//Если КоличествоКорректировки = 0
						//	И НЕ СуммаКорректировкиОстаток = 0 Тогда 
						//	Проводка.Сумма = Проводка.Сумма + СуммаКорректировкиОстаток;							
						//КонецЕсли;	
					КонецЦикла;
				КонецЦикла;
			КонецЦикла;
	...
	// то остаток отнесем на завершаюшую проводку списания со счета.
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
