...
Процедура ОткрытьПодбор(ФормаВладелец) Экспорт
	...
	Если НЕ Отказ Тогда
		ОписаниеОповещенияПриЗакрытииПодбора = Новый ОписаниеОповещения("ПриЗакрытииПодбораДокументов", РаботаСКонтрагентамиБПКлиент.ЭтотОбъект);
...
Процедура ЗаполнитьЗначенияПараметровПодбора(ФормаВладелец, ПараметрыПодбора)
	...
	ПараметрыПодбора.Вставить("ДокументВозврата", ФормаВладелец.ЭтотОбъект.Объект.Возврат);
	ПараметрыПодбора.Вставить("НачалоПериода", НачалоПериода);
	ПараметрыПодбора.Вставить("КонецПериода", КонецПериода);
	ПараметрыПодбора.Вставить("ВидПодбора", "ПодборДокументовПоКонтрагентуСФВ");
