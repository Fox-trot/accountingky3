...
//
// Возвращаемое значение:
//  Строка - XML в соответствии со структурой XDTO Контактная информация и Адрес.
//
Функция АдресПоИдентификатору(ИдентификаторАдреса, ДополнительнаяИнформацияАдреса = Неопределено) Экспорт
	...
	Если ДополнительнаяИнформацияАдреса.Свойство("ДополнительнаяИнформация") Тогда
		Результат.Комментарий = ДополнительнаяИнформацияАдреса.ДополнительнаяИнформация;
	КонецЕсли;
	
	Результат.Состав = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "Адрес"));
	Адрес = Результат.Состав;
	
	Если ДополнительнаяИнформацияАдреса.Свойство("Страна") Тогда
		Адрес.Страна = Строка(ДополнительнаяИнформацияАдреса.Страна);
	Иначе
		Адрес.Страна = Строка(РаботаСАдресамиКлиентСервер.ОсновнаяСтрана());
	КонецЕсли;
	...
	АдресРФИзСервиса = МодульАдресныйКлассификаторСлужебный.АктуальныеАдресныеСведения(ИдентификаторАдреса).Данные;
	
	// Переносим сведения
	Для каждого ЗначениеСвойства Из АдресРФИзСервиса.Свойства() Цикл
		Если ЗначениеСвойства.Имя = "СвРайМО" И АдресРФИзСервиса[ЗначениеСвойства.Имя] <> Неопределено Тогда
			АдресРФ.СвРайМО = ФабрикаXDTO.Создать(АдресРФ.Свойства().Получить("СвРайМО").Тип);
			АдресРФ.СвРайМО.Район = АдресРФИзСервиса[ЗначениеСвойства.Имя].Район;
			АдресРФ.СвРайМО.СвМО = АдресРФИзСервиса[ЗначениеСвойства.Имя].СвМО;
		ИначеЕсли ЗначениеСвойства.Имя <> "ДопАдрЭл" И АдресРФИзСервиса[ЗначениеСвойства.Имя] <> Неопределено Тогда
			АдресРФ[ЗначениеСвойства.Имя] = АдресРФИзСервиса[ЗначениеСвойства.Имя];
		ИначеЕсли ЗначениеСвойства.Имя = "ДопАдрЭл" Тогда
			Для каждого ЗначениеДополнительногоАдресногоЭлемента Из АдресРФИзСервиса.ДопАдрЭл Цикл
				Если ЗначениеДополнительногоАдресногоЭлемента.ТипАдрЭл <> Неопределено Тогда
					Запись = АдресРФ.ДопАдрЭл.Добавить(ФабрикаXDTO.Создать(АдресРФ.ДопАдрЭл.ВладеющееСвойство.Тип));
					Запись.Значение = ЗначениеДополнительногоАдресногоЭлемента.Значение;
					Запись.ТипАдрЭл = ЗначениеДополнительногоАдресногоЭлемента.ТипАдрЭл;
				КонецЕсли;
			КонецЦикла;
		КонецЕсли;
	КонецЦикла;
	...
	ТаблицаЗданий = Новый ТаблицаЗначений();
	ТаблицаЗданий.Колонки.Добавить("Тип", ОписаниеСтроки);
	ТаблицаЗданий.Колонки.Добавить("Значение", ОписаниеСтроки);
	ТаблицаЗданий.Колонки.Добавить("ПутьXPath", ОписаниеСтроки);
	
	ТаблицаПомещений = Новый ТаблицаЗначений();
	ТаблицаПомещений.Колонки.Добавить("Тип", ОписаниеСтроки);
	ТаблицаПомещений.Колонки.Добавить("Значение", ОписаниеСтроки);
	ТаблицаПомещений.Колонки.Добавить("ПутьXPath", ОписаниеСтроки);
	...
	Если ДополнительнаяИнформацияАдреса.Свойство("НомерДома") И ЗначениеЗаполнено(ДополнительнаяИнформацияАдреса.НомерДома) Тогда
		...
		СтрокаЗдания.Тип      = ОписаниеДома.Тип;
		СтрокаЗдания.Значение = ОписаниеДома.Номер;
	...
	Если ДополнительнаяИнформацияАдреса.Свойство("НомерСтроения") И ЗначениеЗаполнено(ДополнительнаяИнформацияАдреса.НомерСтроения) Тогда
		...
		СтрокаЗдания.Тип      = ОписаниеСтроения.Тип;
		СтрокаЗдания.Значение = ОписаниеСтроения.Номер;
	КонецЕсли;
	...
	Если ДополнительнаяИнформацияАдреса.Свойство("НомерОфиса") И ЗначениеЗаполнено(ДополнительнаяИнформацияАдреса.НомерОфиса) Тогда
		СтрокаПомещений = ТаблицаПомещений.Вставить(0);
		ОписаниеПомещения = РазделитьДомаСтроения(ДополнительнаяИнформацияАдреса.НомерОфиса, НСтр("ru='Офис'"));
		СтрокаПомещений.Тип      = ОписаниеПомещения.Тип;
		СтрокаПомещений.Значение = ОписаниеПомещения.Номер;
	КонецЕсли;
	...
	Если ДополнительнаяИнформацияАдреса.Свойство("АбонентскийЯщик") И ЗначениеЗаполнено(ДополнительнаяИнформацияАдреса.АбонентскийЯщик) Тогда
		СтрокаПомещений = ТаблицаПомещений.Вставить(0);
		ОписаниеПомещения = РазделитьДомаСтроения(ДополнительнаяИнформацияАдреса.АбонентскийЯщик, НСтр("ru='А/Я'"));
		СтрокаПомещений.Тип      = ОписаниеПомещения.Тип;
		СтрокаПомещений.Значение = ОписаниеПомещения.Номер;
	КонецЕсли;
	...
	// Индекс
	Если ДополнительнаяИнформацияАдреса.Свойство("ПочтовыйИндекс") И ДополнительнаяИнформацияАдреса.Свойство("ПочтовыйИндекс") Тогда
		Обработки.РасширенныйВводКонтактнойИнформации.ПочтовыйИндексАдреса(АдресРФ, ДополнительнаяИнформацияАдреса.ПочтовыйИндекс);
	КонецЕсли;
	
	// Представление и свободный ввод адреса.
	ВидИнформации = Новый Структура();
	ВидИнформации.Вставить("ФорматАдреса", "ФИАС");
	...
	Возврат УправлениеКонтактнойИнформациейСлужебный.КонтактнаяИнформацияXDTOВXML(Результат);
