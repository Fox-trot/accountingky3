...
Процедура ПриПоказеФайла(ДополнительныеПараметры) Экспорт
	...
	ТипСтруктура         = Тип("Структура");
	ТипТекстовыйДокумент = Тип("ТекстовыйДокумент");
...
// Процедура запускается после открытия файла средствами операционной системы.
//
// Параметры:
//  КодВозврата - Число, Неопределено - код возврата.
//  ДополнительныеПараметры - Структура - структура с ключами:
//   * ПолноеИмяФайла - Строка - Полное имя файла, который необходимо открыть средствами операционной системы.
//
Процедура ПослеВыбораФайла(КодВозврата, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ДополнительныеПараметры) = ТипСтруктура Тогда
		Если (ДополнительныеПараметры.Свойство("ПолноеИмяФайла"))
				И (НЕ ПустаяСтрока(ДополнительныеПараметры.ПолноеИмяФайла)) Тогда
			Если КодВозврата = Неопределено Тогда
				// Открытие файла без ожидания.
			ИначеЕсли ТипЗнч(КодВозврата) = ТипЧисло Тогда
				// Открытие файла с ожиданием завершения.
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
