...
Процедура ПриОпределенииПроверок(ГруппыПроверок, Проверки) Экспорт
	...
	Проверка.ОбработчикПроверки           = "КонтрольВеденияУчетаПереопределяемыйБП.ПроверкаСчетовВНастройкахНалоговойДекларации";
...
Процедура ПроверкаПризнакаВалютный(Проверка, ПараметрыПроверки) Экспорт
	...
	Запрос.Текст =
		...
		|			ИЛИ Хозрасчетный.ВидыСубконто.ВидСубконто = ЗНАЧЕНИЕ(ПланВидовХарактеристик.ВидыСубконтоХозрасчетные.Контрагенты)
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаДетали.Следующий() Цикл
			...
			Если ВыборкаДетали.Счет = Неопределено ИЛИ ВыборкаДетали.Счет = ПланыСчетов.Хозрасчетный.ПустаяСсылка() Тогда
				...
				Проблема.УточнениеПроблемы = СтрШаблон(НСтр("ru = 'У строки %1 не заполнен счет в настройке.
