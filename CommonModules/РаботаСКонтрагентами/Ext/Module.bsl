...
Функция ПроверитьДоступКСервисам() Экспорт
	...
	ИдентификаторСервиса = "РаботаСКонтрагентами/ПроверкаДоступа";
	
	ЗаписатьИнформациюВЖурналРегистрации(НСтр("ru = 'Проверка доступа'"), 
	...
	URL = "https://fresh.1c-cloud.kg/a/ak/hs/serv/ping";
...
Процедура ДополнитьПараметрыПолученияДаннымиСтатистики(ПараметрыПолучения) Экспорт 
	...
	ПараметрыПолученияФайла.Заголовки.Вставить("OrganizationName", ОрганизацияПоУмолчанию.НаименованиеПолное);
	ПараметрыПолученияФайла.Заголовки.Вставить("OrganizationINN", ОрганизацияПоУмолчанию.ИНН);
