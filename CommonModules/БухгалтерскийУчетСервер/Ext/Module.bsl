...
//
Процедура ОтразитьРеализацияТоваров(ДополнительныеСвойства, Движения, Отказ) Экспорт
	
	ТаблицаДляЗагрузки = ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРеализацияТоваров;
	...
	ДвиженияПоРегистру = Движения.РеализацияТоваров;
	ДвиженияПоРегистру.Записывать = Истина;
	ДвиженияПоРегистру.Загрузить(ТаблицаДляЗагрузки);	
...
// Выполняет движения по РС "ОтразитьБланкиСчетовФактур"
//
		...
		Если СтруктураДанных.Свойство("СуммаВключаетНалоги")
			И ((СтруктураДанных.СуммаВключаетНалоги И НЕ Выборка.ЦенаВключаетНалоги)
				...
				Цена = УчетНДСКлиентСервер.ПересчитатьЦенуПриИзмененииФлаговНалогов(Цена,
												...
												УчетНДСПовтИсп.ПолучитьСтавкуНДС(СтруктураДанных.СтавкаНДС),
												УчетНДСПовтИсп.ПолучитьСтавкуНСП(СтруктураДанных.СтавкаНСП));
		КонецЕсли;
...
// Устанавливает значение параметра сеанса "ТипПоследнегоОткрытогоДокумента",
// для определения вида договора при его создании
//
// Параметры:
...
Процедура ОпределитьТипПоследнегоОткрытогоДокумента(Объект, СПокупателем = Ложь) Экспорт 
	...
	Если Объект.Свойство("ВидОперации") Тогда  //значит это один из платежных доков
 		Если Найти(НРег(Объект.ВидОперации),"покупат") > 0  Тогда 
			ПараметрыСеанса.ТипПоследнегоОткрытогоДокумента = "С покупателем";			
		ИначеЕсли 
			Найти(НРег(Объект.ВидОперации),"проч") > 0 Или Найти(НРег(Объект.ВидОперации),"займ") > 0 Тогда  
			ПараметрыСеанса.ТипПоследнегоОткрытогоДокумента = "Прочее";
			
		ИначеЕсли  Найти(НРег(Объект.ВидОперации),"поставщ") > 0  Тогда 
