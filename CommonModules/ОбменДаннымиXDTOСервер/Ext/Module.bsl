...
Функция ИнициализироватьКомпонентыОбмена(НаправлениеОбмена) Экспорт
	...
	Иначе
		...
		ТаблицаПоддерживаемыеОбъекты = Новый ТаблицаЗначений;
		ИнициализироватьТаблицуПоддерживаемыхОбъектовФормата(ТаблицаПоддерживаемыеОбъекты, "ОтправкаПолучение");
		...
		КомпонентыОбмена.НастройкиXDTOКорреспондента.Вставить("ПоддерживаемыеОбъекты", ТаблицаПоддерживаемыеОбъекты);
...
Процедура ОткрытьФайлВыгрузки(КомпонентыОбмена, ИмяФайлаОбмена = "") Экспорт
	...
	Если КомпонентыОбмена.ЭтоОбменЧерезПланОбмена Тогда
		...
		Для Каждого ВерсияФормата Из ВерсииФорматаОбменаМассив(ЗаписьСообщения.Получатель) Цикл
			Header.AvailableVersion.Добавить(ВерсияФормата);
		КонецЦикла;
		
		ВыгрузитьПоддерживаемыеОбъектыФормата(Header, ЗаписьСообщения.Получатель);
	...
	Для Каждого Версия Из ВерсииФормата Цикл
		...
		Если СтрНайти(Режим, "Отправка") Тогда
			...
			Для Каждого ПравилоКонвертации Из КомпонентыОбмена.ПравилаКонвертацииОбъектов Цикл
				...
				Отбор.Вставить("Версия", Версия.Ключ);
...
Функция ПоддерживаемыеОбъектыФорматаКорреспондента(УзелОбмена, Режим = "ОтправкаПолучение") Экспорт
	...
	Если Выборка.Следующий() Тогда
		НастройкиКорреспондента = Выборка.НастройкиКорреспондента.Получить();
...
Процедура ЗаполнитьПоддерживаемыеОбъектыXDTO(КомпонентыОбмена) Экспорт
	...
	Если КомпонентыОбмена.НаправлениеОбмена = "Отправка" Тогда
		...
		ТаблицаОбъектыБазы = ПоддерживаемыеОбъектыФормата(ИмяПланаОбмена, "Отправка", КомпонентыОбмена.УзелКорреспондента);
	...
	ИначеЕсли КомпонентыОбмена.НаправлениеОбмена = "Получение" Тогда
		...
		ТаблицаОбъектыБазы = ПоддерживаемыеОбъектыФормата(ИмяПланаОбмена, "Получение", КомпонентыОбмена.УзелКорреспондента);
	...
	ТаблицаОбъектыБазыПоВерсии = ТаблицаОбъектыБазы.Скопировать(ОтборПоВерсии);
...
Процедура ВыгрузитьПоддерживаемыеОбъектыФормата(Header, УзелКорреспондента)
	...
	ТаблицаПоддерживаемыеОбъекты = ПоддерживаемыеОбъектыФормата(
		ОбменДаннымиПовтИсп.ПолучитьИмяПланаОбмена(УзелКорреспондента));
	...
	СтруктураЗаписи.Вставить("Настройки", Новый ХранилищеЗначения(ТаблицаПоддерживаемыеОбъекты, Новый СжатиеДанных(9)));
...
Процедура ОбновитьНастройкиXDTOКорреспондента(КомпонентыОбмена) Экспорт
	...
	СтруктураЗаписи.Вставить("НастройкиКорреспондента",
		Новый ХранилищеЗначения(КомпонентыОбмена.НастройкиXDTOКорреспондента.ПоддерживаемыеОбъекты, Новый СжатиеДанных(9)));
