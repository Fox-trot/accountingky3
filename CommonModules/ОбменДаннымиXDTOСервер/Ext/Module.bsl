	...
	Пока НомерИтерации = 0 Цикл
		...
		Если ЭтоОбменЧерезПланОбмена Тогда
			...
			Для Каждого ДоступнаяВерсияОтправителя Из ДоступныеВерсииОтправителя Цикл
				...
				Если ДоступнаяВерсияОтправителя = ПроверяемаяВерсия Тогда
					КомпонентыОбмена.Вставить("КорреспондентПоддерживаетИдентификаторОбменаДанными", Истина);
				ИначеЕсли НЕ КомпонентыОбмена.КорреспондентПоддерживаетИдентификаторОбменаДанными Тогда
					РезультатСравненияВерсий = СравнитьВерсии(СокрЛП(ДоступнаяВерсияОтправителя), ПроверяемаяВерсия);
