...
Процедура ПолучитьНовыеДокументыИзБанка(Параметры, АдресРезультата = Неопределено) Экспорт
	...
	ApiKey = "Basic ZmhneTRPZGVmZWZtVEZaZWRmZzM0ZQ==";
	Заголовки.Вставить("ProxyAuthorization", ApiKey);
	
	НачальнаяДатаПолученияЭД = ДанныеОСостоянии.ПоследняяДатаПолученияЭД;
	ПоследняяДатаПолученияЭД = НачальнаяДатаПолученияЭД;
	
	ДатаСтрокой = Формат(НачальнаяДатаПолученияЭД, "ДФ=yyyy-MM-dd%20HH:mm:ss");
	...
	АдресРесурса = "v1/ms-api-1c/getPackList" + ПараметрЗапроса;
	...
	Для Каждого Идентификатор Из МассивИдентификаторов Цикл
		АдресРесурса = "v1/ms-api-1c/getPack?id=" + Идентификатор;
	...
	Для Каждого ПакетОбменСБанками Из МассивПакетов Цикл
		...
		Если РеквизитыНастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен Тогда
			Если ИдентификаторСессии = Неопределено Тогда
				Продолжить;
			КонецЕсли;
			Настройки.Вставить("Ресурс", "v1/ms-api-1c/sendPack");
		...
		// Если РеквизитыНастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.РосинБанкОнлайн Тогда
		Настройки.Вставить("ApiKey", "ZmhneTRPZGVmZWZtVEZaZWRmZzM0ZQ==");
...
Функция ПодтвердитьПлатеж(confirmId, ПакетОбменСБанками, ИдентификаторСессии)
	...
	// тут должна быть криптография
	
	Настройки = Новый Структура("Адрес", РеквизитыНастройкиОбмена.АдресСервера);
		
	Если РеквизитыНастройкиОбмена.ПрограммаБанка = Перечисления.ПрограммыБанка.АсинхронныйОбмен Тогда
		Настройки.Вставить("Ресурс", "v1/ms-api-1c/confirmPack");
	...
	//Настройки.Вставить("confirm", confirmId);
	...
	ApiKey = "ZmhneTRPZGVmZWZtVEZaZWRmZzM0ZQ==";//"fhgy4OdefefmTFZedfg34e";
	
	ВременныйФайл = ПолучитьИмяВременногоФайла();
	ЗаписьТекста = Новый ЗаписьТекста(ВременныйФайл, "CESU-8");
	ЗаписьТекста.Записать(confirmId);
	ЗаписьТекста.Закрыть();
	ДД_Файла = Новый ДвоичныеДанные(ВременныйФайл);
	УдалитьФайлы(ВременныйФайл);

	confirmId = Base64Строка(ДД_Файла);
	
	ОбъектXML.ЗаписатьБезОбработки("<?xml version='1.0'?>
	|<Request>
	|  <apikey>" + ApiKey + "</apikey>
	|  <confirm>" + confirmId + "</confirm>
	|</Request>");
	ОбъектXML.Закрыть();
	
	ТелоЗапроса = Новый ДвоичныеДанные(ИмяФайлаОтправки);
	
	ОтветБанка = Неопределено;
	ТекстОшибки = "";
			
	ОбменСБанкамиСлужебный.ОтправитьВБанк(Настройки, ТелоЗапроса, ОтветБанка, ТекстОшибки);
...
Процедура СоздатьЭДПакетаAsync(Конверт, СвойстваСертификатов = Неопределено) Экспорт
	...
	Попытка
		...
		ПолноеНаименованиеОрганизации = Сред(СведенияОбОрганизации.НаименованиеПолное, 1, 160);
		...
		ЗаполнитьСвойствоXDTO(CustomerPartyType, "kpp", "770501001", , ТекстОшибки);
		...
		ЗаполнитьСвойствоXDTO(Packet, "oper_type_id", 1, , ТекстОшибки);
		...
		ЗаполнитьСвойствоXDTO(Packet, "oper_type_id", СообщениеОбмена.ТипОперации, , ТекстОшибки);
...
Процедура ЗаполнитьСвойствоXDTO(ОбъектXDTO, ИмяСвойства, Значение, Обязательное = Ложь, ТекстОшибки = "", УстанавливатьПустыеЗначения = Ложь) Экспорт
	...
	Иначе
		Если Обязательное ИЛИ ЗначениеЗаполнено(Значение) ИЛИ УстанавливатьПустыеЗначения ИЛИ ИмяСвойства = "Priority" Тогда
			УстановитьЗначениеXDTO(ОбъектXDTO, ИмяСвойства, Значение, ТекстОшибки);
		КонецЕсли;
	КонецЕсли;
...
Процедура СформироватьПлатежноеТребование(СсылкаНаОбъект, НастройкиОбменаЭД, ДеревоДокумента, СтруктураПараметров) Экспорт
	...
	// Если НастройкиОбменаЭД.ПрограммаБанка = Перечисления.ПрограммыБанка.РосинбанкОнлайн Тогда
	СтруктураЭД.Вставить("ТипОперации", СсылкаНаОбъект.ТипОперации);	
...
Функция МассивЗапросовВыпискиАсинхронныйОбмен(НастройкаОбмена, ДатаНачала, ДатаОкончания, МассивБанковскихСчетов, СозданАвтоматически = Ложь) Экспорт
	...
	Для Каждого НомерСчета Из МассивБанковскихСчетов Цикл
		...
		Попытка
			...
			ЗаполнитьСвойствоXDTO(Отправитель, "kpp", "770501001", , ТекстОшибки);
...
Процедура СформироватьПлатежноеТребованиеAsync(СсылкаНаДокумент, ДеревоДанных, АдресФайлаВоВременномХранилище)
	...
	ЗаполнитьСвойствоXDTO(РеквизитыПлательщика, "PayerType", 1, , ТекстОшибки);
	ЗаполнитьСвойствоXDTO(РеквизитыПлательщика, "Currency", "KGS", , ТекстОшибки);
	...
	ЗаполнитьСвойствоXDTO(РеквизитыПолучателя, "PayerType", 1, , ТекстОшибки);
	ЗаполнитьСвойствоXDTO(РеквизитыПолучателя, "Currency", "KGS", , ТекстОшибки);
...
Процедура СформироватьПлатежноеПоручениеAsync(СсылкаНаДокумент, ДеревоДанных, АдресФайлаВоВременномХранилище)
	...
	userAgent = ОбменСБанкамиСлужебныйПовтИсп.ВерсияПрограммыКлиентаДляБанка();
	ЗаполнитьСвойствоXDTO(ЭД, "userAgent", userAgent, , ТекстОшибки);
	//Если СсылкаНаДокумент.Свойство("ТипОперации") Тогда
	ЗаполнитьСвойствоXDTO(ЭД, "oper_type_id", СсылкаНаДокумент.ТипОперации, , ТекстОшибки);
	//КонецЕсли;	
	...
	ЗаполнитьСвойствоXDTO(ДанныеПлатежа, "Sum", Сумма, Истина, ТекстОшибки);
	//Директ
	ЗаполнитьСвойствоXDTO(ДанныеПлатежа, "Course", "", Истина, ТекстОшибки);
...
Процедура ОтправитьВБанк(Настройки, Данные, Результат, ТекстОшибки)
	...
	Если Настройки.Свойство("ApiKey") Тогда
		Заголовки.Вставить("ProxyAuthorization", "Basic " + Настройки.ApiKey);
	КонецЕсли;
