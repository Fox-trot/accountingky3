...
Процедура ПроверкаУникальностиЗаданныхЭлементов(ГруппыПроверок, Проверки)
	...
	Пока Результат.Следующий() Цикл
		...
		Пока ДетальныйРезультат.Следующий() Цикл
			ТекстИсключения = ТекстИсключения + Символы.ПС + СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = '- Наименование элемента: ""%1""'"), ДетальныйРезультат.Наименование);
		КонецЦикла;
	...
	Если ЗначениеЗаполнено(ТекстИсключения) Тогда
		ВызватьИсключение СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Обновление справочника проверок прервано по причине дублирования идентификаторов элементов:
		|%1'"), ТекстИсключения);
	КонецЕсли;
