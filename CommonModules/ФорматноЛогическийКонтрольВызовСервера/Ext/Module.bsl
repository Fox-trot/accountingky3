...
Функция ВыполненаПроверкаОбязательностиИПравильностиЗаполненияТэгов(Параметры, ИдентификаторУстройства, ОписаниеОшибки) Экспорт
	...
	Для Каждого ПозицияЧека Из Параметры.ПозицииЧека Цикл
		Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
			...
			//			ОписаниеОшибки = НСтр("ru = 'ИНН поставщика для предмета расчета в строке №'") + НомерПозицииСтрока + НСтр("ru = ' не указан.'") ;
			//		Возврат Ложь;
			//	КонецЕсли;
			//КонецЕсли;
			//
...
Функция ПроверяемыеРеквизитыПозицийЧека(МассивФФД, СоответствиеРеквизитов)
	...
	Для Каждого ФорматФД Из МассивФФД Цикл
		...
		//	СоответствиеРеквизитов.Вставить("Сумма", НСтр("ru = 'Сумма позиции чека'"));
...
Перем ЗначениеДанных;
	...
	Для Каждого ПозицияЧека Из ПозицииЧека Цикл
		Если ПозицияЧека.Свойство("ФискальнаяСтрока") Тогда
			...
			// Реквизиты «признак агента по предмету расчета» (тег 1222), «данные агента» (тег 1223), «данные поставщика» (тег 1224) 
			// и «ИНН поставщика» (тег 1226) включаются в состав реквизита «предмет расчета» (тег 1059) в случае, 
			// если указанный реквизит содержит сведения об осуществлении расчетов пользователем в качестве платежного агента (субагента),
			// банковского платежного агента (субагента), комиссионера, поверенного или иного агента.
		//	Если Не ПустаяС  НомерПозиции.ПризнакАгентаПоПредметуРасчета 
