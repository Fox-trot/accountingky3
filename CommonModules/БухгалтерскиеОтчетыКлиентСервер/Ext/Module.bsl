...
Функция ПолучитьНаименованиеЗаданияВыполненияОтчета(Форма) Экспорт
	...
	НаименованиеЗадания = СтрШаблон(НСтр("ru = 'Выполнение отчета: %1'"), ИмяОтчета);
...
Процедура ОбработатьОтборПриСменеСчета(СчетСтарый, СчетНовый, КомпоновщикНастроек, ПоддержкаРаботыСоСтруктурнымиПодразделениями) Экспорт
	...
	Если Тип("ПланСчетовСсылка.Хозрасчетный") = ТипЗнч(СчетНовый.Ссылка) Тогда
		ПланСчетовХозрасчетный = Истина;
	Иначе
		ПланСчетовХозрасчетный = Ложь;
	КонецЕсли;
	
	Если ПланСчетовХозрасчетный Тогда
...
// Возвращает значение найденного элемента отбора только в случае ВидСравненияКомпоновкиДанных.Равно
//
// Параметры:
//		ЭлементСтруктуры - элемент структуры
//		ИмяПоля      	 - Строка - имя поля для поиска
//		Использование    - Неопределено, Булево - признак использования отбора (по умолчанию: Неопределено).
//                         Если Неопределено, то отбираются все элементы, независимо от их использования
//
Функция НайтиЭлементОтбора(ЭлементСтруктуры, ИмяПоля, Использование = Неопределено) Экспорт
...
// Процедура - Заполнить дополнительные параметры печати
//
// Параметры:
//  ДокументРезультат	 		- ТабличныйДокумент - таб.док. куда будет добавлен номер страницы
//  ДополнительныеПараметры	 	- Структура	 - структура с дополнительными параметрами
Процедура ЗаполнитьДополнительныеПараметрыПечати(ДокументРезультат, Знач ДополнительныеПараметры = Неопределено) Экспорт 
	...
	ДокументРезультат.НижнийКолонтитул.ТекстСправа  	= "[&НомерСтраницы] из [&СтраницВсего]";
	
	// Не выводить	
	Если ДополнительныеПараметры.Свойство("НЕВыводитьНомераСтраницы") 
		И ДополнительныеПараметры.НЕВыводитьНомераСтраницы Тогда 
		
		ДокументРезультат.НижнийКолонтитул.ТекстСправа  = "";
	КонецЕсли;	

	Если ДополнительныеПараметры.Свойство("НЕВыводитьДатыФормирования") 
		И ДополнительныеПараметры.НЕВыводитьДатыФормирования Тогда 
		
		ДокументРезультат.НижнийКолонтитул.ТекстСлева  = "";
	КонецЕсли;	

	Если ДополнительныеПараметры.Свойство("НЕВыводитьОрганизации") 
		И ДополнительныеПараметры.НЕВыводитьОрганизации Тогда 
		
		ДокументРезультат.ВерхнийКолонтитул.ТекстСправа  = "";
	КонецЕсли;	

	// Проверка вывода колонтитулов
	Если ДокументРезультат.НижнийКолонтитул.ТекстСправа = ""
		И ДокументРезультат.НижнийКолонтитул.ТекстСлева = "" Тогда 
		
		ДокументРезультат.НижнийКолонтитул.Выводить = Ложь;
	КонецЕсли;	
	
	Если ДокументРезультат.ВерхнийКолонтитул.ТекстСправа = ""
		И ДокументРезультат.ВерхнийКолонтитул.ТекстСлева = "" Тогда 
		
		ДокументРезультат.ВерхнийКолонтитул.Выводить = Ложь;
	КонецЕсли;	
	
КонецПроцедуры // ЗаполнитьДополнительныеПараметрыПечати()

// Процедура добавляет номер страницы в нижний колонтитул
//
// Параметры:
//  ДокументРезультат	 - ТабличныйДокумент - таб.док. куда будет добавлен номер страницы
Процедура ДобавитьВыводНомераСтраницы(ДокументРезультат) Экспорт 
	ДокументРезультат.НижнийКолонтитул.Выводить 	= Истина;
	ДокументРезультат.НижнийКолонтитул.ТекстСправа  = "[&НомерСтраницы] из [&СтраницВсего]";
КонецПроцедуры // ДобавитьВыводНомераСтраницы()

// Процедура добавляет дату формирования в нижний колонтитул
//
// Параметры:
//  ДокументРезультат	 - ТабличныйДокумент - таб.док. куда будет добавлен номер страницы
Процедура ДобавитьВыводДатыФормирования(ДокументРезультат) Экспорт 
	ДокументРезультат.НижнийКолонтитул.Выводить 	= Истина;
	ДокументРезультат.НижнийКолонтитул.ТекстСлева  = ТекущаяДата();
КонецПроцедуры // ДобавитьВыводДатыФормирования()

// Процедура добавляет полное наименование организации в верхний колонтитул
//
// Параметры:
//  ДокументРезультат	 - ТабличныйДокумент - таб.док. куда будет добавлен номер страницы
//  Организация	 		- СправочникСсылка.Организации - Организация
Процедура ДобавитьВыводОрганизации(ДокументРезультат, Организация = Неопределено) Экспорт 
	ДокументРезультат.ВерхнийКолонтитул.Выводить 	= Истина;
	ДокументРезультат.ВерхнийКолонтитул.ТекстСлева  = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьПолноеНаименованиеОрганизации(Организация);
КонецПроцедуры // ДобавитьВыводНомераСтраницы()