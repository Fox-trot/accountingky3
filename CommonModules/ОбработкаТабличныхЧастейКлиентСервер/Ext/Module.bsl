...
//  СтранаВходитВЕАЭС - булево - признак, что страна входит в ЕАЭС
//  СчитатьОтДохода - булево - признак, что расчет идет от суммы дохода
//  Точность - число - точность округления цены
//
Процедура РассчитатьЦенуСтрокиТабличнойЧасти(СтрокаТабличнойЧасти, ЗначениеПустогоКоличества = 0, СтранаВходитВЕАЭС = Ложь, СчитатьОтДохода = Ложь, Точность = 2) Экспорт
	...
	Иначе	
		...
		ИначеЕсли СчитатьОтДохода Тогда
			СтрокаТабличнойЧасти.Цена = Окр(СтрокаТабличнойЧасти.СуммаДохода / РасчетноеКоличество, Точность);
			
		Иначе
			СтрокаТабличнойЧасти.Цена = Окр(СтрокаТабличнойЧасти.Сумма / РасчетноеКоличество, Точность);
		КонецЕсли; 
	КонецЕсли;
КонецПроцедуры
...
//  БезналичныйРасчет	 	- Булево - Признак безналичного расчета
//
Процедура ПересчитатьНалогиТабличнойЧасти(Объект, Период, Организация, ИмяТабличнойЧасти, СуммаВключаетНалоги = Ложь, СтавкаНДС = Неопределено, СтавкаНСП = Неопределено, БезналичныйРасчет = Ложь, СчитатьОтДохода = Ложь) Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект[ИмяТабличнойЧасти] Цикл
		// СуммаНДС.
		СтрокаТабличнойЧасти.СуммаНДС = УчетНДСКлиентСервер.РассчитатьСуммуНДС(
			?(СчитатьОтДохода, СтрокаТабличнойЧасти.СуммаДохода, СтрокаТабличнойЧасти.Сумма),
			СуммаВключаетНалоги,           
			ЗначениеСтавкиНДС,
			ЗначениеСтавкиНСП);
		...
		Если СтрокаТабличнойЧасти.Свойство("Всего") Тогда
			СтрокаТабличнойЧасти.Всего = ?(СчитатьОтДохода, СтрокаТабличнойЧасти.СуммаДохода, СтрокаТабличнойЧасти.Сумма) 
									+ ?(СуммаВключаетНалоги, 0, СтрокаТабличнойЧасти.СуммаНДС + СтрокаТабличнойЧасти.СуммаНСП);
		КонецЕсли;
		
		Если СтрокаТабличнойЧасти.Свойство("СуммаДохода") И НЕ СчитатьОтДохода Тогда
