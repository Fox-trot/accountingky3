...
Процедура ВыполнитьКомандуДополнительнойОбработки(ИдентификаторОбработки, ИдентификаторКоманды, ИдентификаторОперации, СообщитьМенеджеру = Ложь) Экспорт
	...
	Событие = Событие + НСтр("ru = 'Начало выполнения команды';
							|en = 'Command execution start'", ОбщегоНазначения.КодОсновногоЯзыка()); 
	...
	Если Выборка.Следующий() Тогда
		...
		Попытка
			...
			Событие = Событие + НСтр("ru = 'Завершение выполнения команды';
									|en = 'Command execution end'", ОбщегоНазначения.КодОсновногоЯзыка()); 
		...
		Исключение
			...
			Событие = НСтр("ru = 'Ошибка выполнения команды';
							|en = 'An error occurred when executing the command'", ОбщегоНазначения.КодОсновногоЯзыка()); 
	...
	Иначе
		...
		Событие = НСтр("ru = 'Обработка не найдена';
						|en = 'Data processor is not found'", ОбщегоНазначения.КодОсновногоЯзыка()); 
...
Функция ВыполнитьПередачуФайлаПриложению(ИмяФайла, КодПолучателя, БыстраяПередача = Ложь, ПараметрыВызова = Неопределено) Экспорт
	...
	Если НЕ Указатель.Существует() Тогда
		Комментарий = НСтр("ru = 'Файл с указанным именем не найден на файловой системе.';
							|en = 'File with the specified name is not found in the file system.'", КодЯзыка); 
	...
	Если НЕ ЗначениеЗаполнено(КодПолучателя) И НЕ ТипЗнч(КодПолучателя) = Тип("Число") Тогда
		Комментарий = НСтр("ru = 'Параметр КодПолучателя не содержит валидный код области данных.';
							|en = 'The КодПолучателя parameter does not contain a valid code of data area.'", КодЯзыка); 
		ВызватьИсключение Комментарий;
	КонецЕсли; 
...
Процедура ВыслатьКвитанциюПередачиФайла(ИдентификаторВызова, КодПолучателя, БыстраяПередача = Ложь, ПараметрыВызова = Неопределено) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(ИдентификаторВызова) ИЛИ НЕ ТипЗнч(ИдентификаторВызова) = Тип("УникальныйИдентификатор") Тогда
		Комментарий = НСтр("ru = 'Параметр ИдентификаторВызова не содержит валидный идентификатор.';
							|en = 'The ИдентификаторВызова parameter does not contain a valid identifier.'", КодЯзыка); 
		ВызватьИсключение Комментарий;
	КонецЕсли; 
...
Функция ИмяСобытияЖурналаРегистрации() Экспорт
	...
	Возврат НСтр("ru = 'Распределенное выполнение команд';
				|en = 'Distributed command execution'", ОбщегоНазначения.КодОсновногоЯзыка()); 
