...
Процедура СтраницыПриСменеСтраницы(Элемент, ТекущаяСтраница)
	//Если Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаНалоговаяВыверкаГлавная Тогда
	// 	ИмяАктивнойЗакладки = "НалоговаяВыверка";
	//ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаНалоговаяАмортизация Тогда
	//	ИмяАктивнойЗакладки = "НалоговаяАмортизация";
	//ИначеЕсли Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаЕдинаяНалоговаяДекларация Тогда
	//	ИмяАктивнойЗакладки = "ЕдинаяНалоговаяДекларация";				
	//КонецЕсли;
	//
	//УстановитьВидимостьДоступностьЭлементов();
КонецПроцедуры
...
&НаКлиенте
Процедура НалоговаяВыверкаПриАктивизацииСтроки(Элемент)
	СтрокаТаблицыНВ = Элементы.НалоговаяВыверка.ТекущиеДанные;
	Если СтрокаТаблицыНВ = Неопределено Тогда 
		НомерСтрокиНВ = Неопределено;
	Иначе
		НомерСтрокиНВ = СтрокаТаблицыНВ.НомерСтроки;
	КонецЕсли;	
	
	// УстановитьВидимостьДоступностьЭлементовНалоговойВыверки();
...
Процедура НалоговаяВыверкаИтоги11ПриАктивизацииСтроки(Элемент)
	...
	Иначе
		Элементы.НалоговаяВыверка11.ОтборСтрок = Новый ФиксированнаяСтруктура("Счет", СтрокаТабличнойЧасти.Счет);
	КонецЕсли;
КонецПроцедуры
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	//ИначеЕсли ИмяАктивнойЗакладки = "ЕдинаяНалоговаяДекларация" Тогда
	// 	Элементы.Страницы.ТекущаяСтраница = Элементы.СтраницаЕдинаяНалоговаяДекларация;		
	//Иначе                        
...
Процедура УстановитьУсловноеОформление()
	...
	ОтборЭлемента = Элемент.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ОтборЭлемента.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Объект.НалоговаяВыверка.НомерСтроки");
...
// Процедура устанавливает видимость и доступность элементов закладки Налоговая выверка.
//
&НаКлиенте
Процедура УстановитьВидимостьДоступностьЭлементовНалоговойВыверки()
	СтрокаТабличнойЧасти = Элементы.НалоговаяВыверка.ТекущиеДанные;
	
	Элементы.НалоговаяВыверкаИтоги01а.Видимость = Ложь;
	Элементы.НалоговаяВыверкаИтоги02а.Видимость = Ложь;
	Элементы.НалоговаяВыверкаИтоги04а.Видимость = Ложь;
	Элементы.НалоговаяВыверкаИтоги05а.Видимость = Ложь;
	Элементы.НалоговаяВыверкаИтоги13а.Видимость = Ложь;
	Если НЕ СтрокаТабличнойЧасти = Неопределено Тогда
		НомерСтрокиНВ = СтрокаТабличнойЧасти.НомерСтроки;
		Если НомерСтрокиНВ = 1 Тогда 
			Элементы.НалоговаяВыверкаИтоги01а.Видимость = Истина;	
		ИначеЕсли НомерСтрокиНВ = 2 Тогда 
			Элементы.НалоговаяВыверкаИтоги02а.Видимость = Истина;
		ИначеЕсли НомерСтрокиНВ = 4 Тогда 
			Элементы.НалоговаяВыверкаИтоги04а.Видимость = Истина;				
		ИначеЕсли НомерСтрокиНВ = 5 Тогда 
			Элементы.НалоговаяВыверкаИтоги05а.Видимость = Истина;
		ИначеЕсли НомерСтрокиНВ = 13 Тогда 
			Элементы.НалоговаяВыверкаИтоги13а.Видимость = Истина;				
		КонецЕсли;				
	КонецЕсли;
...
Процедура ЗаполнитьТабличныеЧастиПоНалоговойВыверке()
	...
	Запрос.Текст = 
		...
		|			Организация = &Организация
		|				И ВидУчета = ЗНАЧЕНИЕ(Перечисление.ВидыУчета.ВР)
		|				И Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПрочиеДоходыОтНеоперационнойДеятельности)) КАК КорректировкиНУОбороты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|			Организация = &Организация
		|				И ВидУчета = ЗНАЧЕНИЕ(Перечисление.ВидыУчета.ВР)
		|				И Счет = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасходыПоВыбытиюАктивов)) КАК КорректировкиНУОбороты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|			Организация = &Организация
		|				И ВидУчета = ЗНАЧЕНИЕ(Перечисление.ВидыУчета.ВР)
		|				И Счет.Вид = ЗНАЧЕНИЕ(ВидСчета.Пассивный)) КАК КорректировкиНУОбороты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|			Организация = &Организация
		|				И ВидУчета = ЗНАЧЕНИЕ(Перечисление.ВидыУчета.ВР)
		|				И Счет.Вид = ЗНАЧЕНИЕ(ВидСчета.Активный)) КАК КорректировкиНУОбороты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Если НЕ МассивРезультатовЗапроса[10].Пустой() Тогда 
		ВыборкаДетальныхЗаписей = МассивРезультатовЗапроса[10].Выбрать();
	...
	Если НЕ МассивРезультатовЗапроса[11].Пустой() Тогда 
		ВыборкаДетальныхЗаписей = МассивРезультатовЗапроса[11].Выбрать();
		Пока ВыборкаДетальныхЗаписей.Следующий() Цикл
			СтрокаНалоговойВыверки = Объект.НалоговаяВыверка19.Добавить();
			ЗаполнитьЗначенияСвойств(СтрокаНалоговойВыверки, ВыборкаДетальныхЗаписей);
	...
	Если НЕ МассивРезультатовЗапроса[4].Пустой() Тогда 
		...
		Пока ВыборкаДетальныхЗаписей.Следующий() Цикл
			...
			ЗаполнитьЗначенияСвойств(СтрокаНалоговойВыверки, ВыборкаДетальныхЗаписей);
		КонецЦикла;		
		СуммаАмортизацияОСБУ = Объект.НалоговаяВыверка11.Итог("Сумма");		
		ТаблицаЗначений = Объект.НалоговаяВыверка11.Выгрузить(,"Счет,Сумма");		
		ТаблицаЗначений.Свернуть("Счет", "Сумма");
		ТаблицаЗначений.Сортировать("Счет");
	...
	// 13.1 Реализация ОС - временные разницы - доходы/расходы.
	СуммаВР_Доходы = 0;
	СуммаВР_Расходы = 0;
	...
	Если НЕ МассивРезультатовЗапроса[8].Пустой() Тогда 
		...
		ВыборкаДетальныхЗаписей.Следующий();
		СуммаВР_ПрочиеДоходы = ВыборкаДетальныхЗаписей.Сумма - СуммаВР_Доходы;
	КонецЕсли;
	Если НЕ МассивРезультатовЗапроса[9].Пустой() Тогда 
		ВыборкаДетальныхЗаписей = МассивРезультатовЗапроса[9].Выбрать();
		ВыборкаДетальныхЗаписей.Следующий();
		СуммаВР_ПрочиеРасходы = ВыборкаДетальныхЗаписей.Сумма - СуммаВР_Расходы;
	КонецЕсли;	
	
	ТаблицаЗначений = Объект.НалоговаяВыверка13.Выгрузить(,"Счет,Сумма");		
	ТаблицаЗначений.Свернуть("Счет", "Сумма");
	ТаблицаЗначений.Сортировать("Счет");
	...
	СтрокаНалоговойВыверки.Сумма 				= СуммаВР_Доходы - СуммаВР_Расходы
													- ПревышениеНормыНаРемонт
													- (СуммаВР_ПрочиеДоходы + СуммаВР_ПрочиеРасходы);
	...
	// 18. Доходы/расходы по бухгалтерскому учету.
	СтрокаНалоговойВыверки 						= Объект.НалоговаяВыверка.Добавить();
	...
	СтрокаНалоговойВыверки 						= Объект.НалоговаяВыверка.Добавить();
	СтрокаНалоговойВыверки.КодСтроки            = "19";
	СтрокаНалоговойВыверки.НаименованиеСтроки 	= НСтр("ru = 'Курсовые разницы (См. закладку ""Строка 19"")'");	
	СтрокаНалоговойВыверки.Доходы				= -СуммаДоходы;
	СтрокаНалоговойВыверки.Расходы				= -СуммаРасходы;	
	...
	// 25 Превышение затрат на ремонт ОС ВР.
	СтрокаНалоговойВыверки 						= Объект.НалоговаяВыверка.Добавить();
	...
	// 26 Прочие доходы/расходы ВР.
	СтрокаНалоговойВыверки 						= Объект.НалоговаяВыверка.Добавить();
	СтрокаНалоговойВыверки.КодСтроки            = "26";
	СтрокаНалоговойВыверки.НаименованиеСтроки 	= НСтр("ru = 'Прочие доходы/расходы ВР (См. закладку ""Строка 13"")'");
	...
	//// 29. КР – доходы/расходы
	//СтрокаНалоговойВыверки 						= Объект.НалоговаяВыверка.Добавить();
	//СтрокаНалоговойВыверки.КодСтроки            = "29";
	//СтрокаНалоговойВыверки.НаименованиеСтроки 	= НСтр("ru = 'КР – доходы/расходы'");	
	////СтрокаНалоговойВыверки.Сумма				= СуммаКР;
	//СтрокаНалоговойВыверки.Доходы				= СуммаДоходы;
	//СтрокаНалоговойВыверки.Расходы				= СуммаРасходы;	
...
Процедура ЗаполнитьНастройки()
	...
	СтавкаНалогаНаПрибыльНастройкаНП = ДанныеУчетнойПолитики.СтавкаНалогаНаПрибыль;
...
Процедура ПроверитьНастройкиЕНД()
	...
	Запрос.Текст = 
		...
		|				ТОГДА ЛОЖЬ
		|			ИНАЧЕ ИСТИНА
		|		КОНЕЦ) КАК СчетУчавствуетВНастройке,
		|	КОЛИЧЕСТВО(ВЫБОР
		|			КОГДА ВременнаяТаблицаНастройкиСтрок.СчетУчета ЕСТЬ NULL
		|				ТОГДА ЛОЖЬ
		|			ИНАЧЕ ИСТИНА
	...
	Пока ВыборкаВременныеСчета.Следующий() Цикл
		...
		Если НЕ ВыборкаВременныеСчета.СчетУчавствуетВНастройке 
			...
			И ВыборкаВременныеСчета.Счет <> ПланыСчетов.Хозрасчетный.РасходыДоходыПоНалогуНаПрибыль Тогда
		...
		ИначеЕсли ВыборкаВременныеСчета.СчетУчавствуетВНастройке 
			И (ВыборкаВременныеСчета.Счет = ПланыСчетов.Хозрасчетный.ПрочиеДоходыОтНеоперационнойДеятельности 
			ИЛИ ВыборкаВременныеСчета.Счет = ПланыСчетов.Хозрасчетный.РасходыПоВыбытиюАктивов
			ИЛИ ВыборкаВременныеСчета.Счет = ПланыСчетов.Хозрасчетный.РасходыДоходыПоНалогуНаПрибыль) Тогда
