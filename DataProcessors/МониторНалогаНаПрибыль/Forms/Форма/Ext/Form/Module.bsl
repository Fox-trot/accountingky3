...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	Если Параметры.Свойство("СтруктураДанных") Тогда
		...
		Иначе
			Объект.Организация = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьЗначениеПоУмолчаниюПользователя(
	...
	ЗаполнитьТабличнуюЧастьНалоговаяАмортизация();
...
//
&НаСервере
Процедура ЗаполнитьТабличнуюЧастьНалоговаяАмортизация()
	...
	Объект.НалоговаяАмортизация.Загрузить(Запрос.Выполнить().Выгрузить());
...
Функция ПараметрыДокументаЗакрытиеМесяца(Дата)
	Параметры = Новый Структура("ЕстьДокументЗакрытиеМесяца, ссылка, ПредельнаяНормаНаРемонтОС", Истина, Неопределено, 0);
