...
Функция ПараметрыДокументаЗакрытиеМесяца(Дата)
	Параметры = Новый Структура("ЕстьДокументЗакрытиеМесяца, ссылка, ПредельнаяНормаНаРемонтОС, МинимальнаяСтоимостьГруппыОС", Ложь, Неопределено, 0, 0);
...
Процедура ЗаполнитьТабличныеЧастиПоНалоговойВыверке()
	...
	// 25 Реализация ОС (доходы/расходы) ВР.
	СтрокаНалоговойВыверки 						= Объект.НалоговаяВыверка.Добавить();
	СтрокаНалоговойВыверки.КодСтроки            = "25";
	СтрокаНалоговойВыверки.НаименованиеСтроки 	= НСтр("ru = 'Реализация ОС (доходы/расходы) ВР (СПРАВОЧНО!)'");
	СтрокаНалоговойВыверки.Доходы				= СуммаВР_Доходы;	
	СтрокаНалоговойВыверки.Расходы				= СуммаВР_Расходы;

	// 26 Превышение затрат на ремонт ОС ВР.
	СтрокаНалоговойВыверки 						= Объект.НалоговаяВыверка.Добавить();
