...
Процедура ЗаполнитьТабличныеЧастиПоНалоговойВыверке()
	...
	// 28 Проверка сумм 163 ЕНД и 22 НВ
	...
	СтрокаНалоговойВыверки.НаименованиеСтроки 	= СтрШаблон(НСтр("ru = 'Строка 163 ЕНД  должна быть равна строке 22 НВ %1'"), РезультатПроверки);
	СтрокаНалоговойВыверки.Доходы				= Сумма;	
	СтрокаНалоговойВыверки.Расходы				= СуммаДоходОтВыбытияОС;
	СтрокаНалоговойВыверки.ИзменитьЦветФона		= Истина;
	
	// 29 Проверка сумм 255 ЕНД и 24 НВ
	...
	СтрокаНалоговойВыверки.НаименованиеСтроки 	= СтрШаблон(НСтр("ru = 'Строка 255 ЕНД должна быть равна строке 24 НВ %1'"), РезультатПроверки);
	СтрокаНалоговойВыверки.Доходы				= Сумма;	
	СтрокаНалоговойВыверки.Расходы				= СуммаАмортизацияОСНУ;
	СтрокаНалоговойВыверки.ИзменитьЦветФона		= Истина;
	
	// 30 Проверка сумм 256 ЕНД минус 25 НВ и суммы затрат на ремонт РС "Налоговая амортизация"
	...
	СтрокаНалоговойВыверки.НаименованиеСтроки 	= СтрШаблон(НСтр("ru = 'Строка 256 ЕНД минус строка 25 НВ должна быть равна сумма затрат на ремонт регистра ""Налоговая амортизация"" %1'"), РезультатПроверки);
	СтрокаНалоговойВыверки.Доходы				= Сумма;	
	СтрокаНалоговойВыверки.Расходы				= СуммаЗатрат;
	СтрокаНалоговойВыверки.ИзменитьЦветФона		= Истина;
	
	// 31 Проверка сумм 154/298а ЕНД и 19 НВ
	...
	СтрокаНалоговойВыверки.НаименованиеСтроки 	= СтрШаблон(НСтр("ru = 'Строка 154 и 298а не должны быть обе ненулевыми. Если строка 154 не ноль, то она равна Доходы – Расходы сроки 19 НВ. Если строка 298а не ноль, то она равна Расходы – Доходы строки 19 НВ %1'"), РезультатПроверки);
	СтрокаНалоговойВыверки.Доходы				= Сумма;	
	СтрокаНалоговойВыверки.Расходы				= Сумма19НВ;
	СтрокаНалоговойВыверки.ИзменитьЦветФона		= Истина;
	
	// 32 (и далее 33, 34...) Проверка документов
	НомерСтроки = 32;
	...
	Если НЕ МассивРезультатовЗапроса[16].Пустой() Тогда 
		...
		Пока Выборка.Следующий() Цикл
			...
			СтрокаНалоговойВыверки.НаименованиеСтроки 	= НСтр("ru = 'Документ с проводкой 7160, у которого указано ОС либо не принятое к учету, либо не имеющее группы НУ'");
