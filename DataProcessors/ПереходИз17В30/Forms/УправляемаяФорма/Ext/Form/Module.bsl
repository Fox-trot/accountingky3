...
Процедура ПриОткрытии(Отказ)
	...
	Объект.ЗаписыватьВИнформационнуюБазуТолькоИзмененныеОбъекты = Ложь;
	Объект.ОбъектыПоСсылкеЗагружатьБезПометкиУдаления = Ложь;
	Объект.ОптимизированнаяЗаписьОбъектов = Ложь;
	Объект.ЗаписыватьРегистрыНаборамиЗаписей = Ложь;
	...
	ИмяФайлаОбмена = ПолноеИмя + "\Выгрузка.xml";
...
//&НаКлиенте
//Процедура ИмяФайлаИНННачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
//	//ВыборФайлаИНН(Элемент, ЭтотОбъект, "ИмяФайлаИНН", Ложь, "txt" , Ложь);
//КонецПроцедуры

//&НаКлиенте
//Процедура ВыборФайлаИНН(Элемент, ОбъектХранения, ИмяСвойства, ПроверятьСуществование, Знач РасширениеПоУмолчанию = "xml",
//	АрхивироватьФайлДанных = Истина, ВыборФайлаПравил = Ложь)
//	
//	ДиалогВыбораФайла = Новый ДиалогВыбораФайла(РежимДиалогаВыбораФайла.Открытие);
...
//	Если РасширениеПоУмолчанию = "txt" Тогда
//		
//		ДиалогВыбораФайла.Фильтр = "Файл ИНН (*.txt)|*.txt";
//		ДиалогВыбораФайла.Расширение = "txt";
//		
//	КонецЕсли;
//	
...
//	ДополнительныеПараметры.Вставить("Элемент",        Элемент);
//	
//	Оповещение = Новый ОписаниеОповещения("ДиалогВыбораФайлаОбработкаВыбораИНН", ЭтотОбъект, ДополнительныеПараметры);
//	ДиалогВыбораФайла.Показать(Оповещение);
//	
//КонецПроцедуры
...
Процедура ИНН() Экспорт
	...
	Файл = Новый ЧтениеТекста(ПолноеИмя + "\инн.txt");
	ВесьФайлОднойСтрокой = Файл.Прочитать();
	ЗаполнитьИНН(СтрЗаменить(ВесьФайлОднойСтрокой,Символы.ПС,""));
	Файл.Закрыть();
	
КонецПроцедуры

//&НаКлиенте
//Процедура ДиалогВыбораФайлаОбработкаВыбораИНН(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
//	
//	Если ВыбранныеФайлы = Неопределено Тогда
//		Возврат;
//	КонецЕсли;
//	
//	
//	ДополнительныеПараметры.ОбъектХранения.ИмяФайлаИНН = ВыбранныеФайлы[0];
//	
...
&НаСервере
Процедура ЗаполнитьИНН(СтрокаИНН) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		ОргОбъект = Выборка.Ссылка.ПолучитьОбъект();
		ОргОбъект.ИНН = СтрокаИНН;
		ОргОбъект.Записать();
	КонецЦикла;
