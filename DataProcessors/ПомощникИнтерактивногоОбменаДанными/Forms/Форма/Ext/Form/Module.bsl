...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	// с использованием правил конвертации объектов.
	...
	ПропускатьСтраницуТранспорта = ДополнениеВыгрузкиРасширенныйРежим Или ОбщегоНазначенияКлиентСервер.ЭтоВебКлиент() Или МодельСервиса;
	...
	Если КлиентскоеПриложение.ТекущийВариантИнтерфейса() = ВариантИнтерфейсаКлиентскогоПриложения.Версия8_2 Тогда
		...
		УстановитьШрифтЗаголовкаГруппы(Элементы.ЗаголовокСтраницыРегистрацияИзменений);
...
Процедура ВидТранспортаСообщенийОбменаПриИзмененииНаСервере()
	...
	ИспользоватьПрогрессБар = Не ОбменЧерезПодключениеККорреспонденту И Не МодельСервиса;
	...
	Элементы.НадписьWSПароль.Видимость   = Ложь;
	...
	Если ОбменЧерезВебСервис Тогда
		...
		Элементы.НадписьWSПароль.Видимость   = Не WSЗапомнитьПароль;
...
Функция Подключаемый_СтраницаНачала_ПриОткрытии(Отказ, ПропуститьСтраницу, ЭтоПереходДалее)
	...
	Если СледующаяСтраница
		И (ЗначениеЗаполнено(Объект.ВидТранспортаСообщенийОбмена) Или ПолученоСообщениеДляСопоставленияДанных) Тогда
...
Процедура ЗаполнитьТаблицуПереходов()
	...
	Если ПолучитьДанные Тогда
		...
		Если ОбменЧерезПодключениеККорреспонденту
