...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.ВидТранспортаСообщенийОбмена) Тогда
		...
		Объект.ВидТранспортаСообщенийОбмена = РегистрыСведений.НастройкиТранспортаОбменаДанными.ВидТранспортаСообщенийОбменаПоУмолчанию(
...
Процедура ВидТранспортаСообщенийОбменаПриИзмененииНаСервере()
	...
	СтруктураПараметровПодключенияВнешнегоСоединения = РегистрыСведений.НастройкиТранспортаОбменаДанными.НастройкиТранспорта(Объект.УзелИнформационнойБазы);
	...
	Если Параметры.Свойство("НастройкаРасписания") Тогда
		
		ОткрытьПомощникСозданияСценарияОбменаДанными = Параметры.НастройкаРасписания;
		 
	ИначеЕсли ЭтоЗапускИзДругогоПриложения Тогда
...
Процедура ОбновитьСтраницыНастроекТранспорта()
	...
	ВидТранспортаСообщенийОбменаПоУмолчанию = РегистрыСведений.НастройкиТранспортаОбменаДанными.ВидТранспортаСообщенийОбменаПоУмолчанию(Объект.УзелИнформационнойБазы);
...
Процедура ПриНачалеРегистрацииДанныхНаСервере(ПродолжитьОжидание)
	...
	СтруктураДополнение.Вставить("КомпоновщикОтбораВсехДокументов",
		Новый Структура("Настройки, ПользовательскиеНастройки, ФиксированныеНастройки"));
	ЗаполнитьЗначенияСвойств(СтруктураДополнение.КомпоновщикОтбораВсехДокументов, ДополнениеВыгрузки.КомпоновщикОтбораВсехДокументов);
	
	СтруктураДополнение.Вставить("ДополнительнаяРегистрация", ДополнениеВыгрузки.ДополнительнаяРегистрация.Выгрузить());
	
	НастройкиРегистрации.Вставить("ДополнениеВыгрузки", СтруктураДополнение);
...
Процедура ПолучениеДанныхЧерезОбычныеКаналыСвязи()
	...
	ТаблицаПереходовНоваяСтрокаДлительнаяОперация("СтраницаОжиданияСинхронизацииДанныхПрогрессБарЗагрузка", "СтраницаНавигацииОжидание", Истина, "ЗагрузкаДанных_ОбработкаДлительнойОперации", "ЗагрузкаДанных_ПриОткрытии");
...
Процедура ПолучениеДанныхЧерезВнешнееСоединениеИлиВебСервис()
	...
	ТаблицаПереходовНоваяСтрокаДлительнаяОперация(ИмяСтраницыСинхронизация, "СтраницаНавигацииОжидание", Истина, "ЗагрузкаДанных_ОбработкаДлительнойОперации", "ЗагрузкаДанных_ПриОткрытии");
...
Процедура ОтправкаДанныхЧерезОбычныеКаналыСвязи()
	...
	// Выгрузка данных.
	ИспользоватьПрогрессБар = Истина;
	ТаблицаПереходовНоваяСтрокаДлительнаяОперация("СтраницаОжиданияСинхронизацииДанныхПрогрессБарВыгрузка", "СтраницаНавигацииОжидание", Истина, "СтраницаОжиданияВыгрузкиДанных_ОбработкаДлительнойОперации", "ВыгрузкаДанных_ПриОткрытии");
...
Процедура ОтправкаДанныхЧерезВнешнееСоединениеИлиВебСервис()
	...
	ТаблицаПереходовНоваяСтрокаДлительнаяОперация(ИмяСтраницыСинхронизация, "СтраницаНавигацииОжидание", Истина, "СтраницаОжиданияВыгрузкиДанных_ОбработкаДлительнойОперации", "ВыгрузкаДанных_ПриОткрытии");
	ТаблицаПереходовНоваяСтрокаДлительнаяОперация(ИмяСтраницыСинхронизация, "СтраницаНавигацииОжидание", Истина, "СтраницаОжиданияВыгрузкиДанныхДлительнаяОперацияОкончание_ОбработкаДлительнойОперации");
