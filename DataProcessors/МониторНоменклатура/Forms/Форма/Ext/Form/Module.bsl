...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ОтборПартионныйУчет Тогда
		Элементы.ДекорацияПаритонныйУчет.Заголовок = НСтр("ru = 'Ведется партионный учет'");
	Иначе
		Элементы.ДекорацияПаритонныйУчет.Заголовок = НСтр("ru = 'Ведется учет по средней'");
	КонецЕсли;
...
&НаКлиенте
Процедура СвернутьПоСкладамПриИзменении(Элемент)
	УстановитьПараметрыДинамическихСписков();
	УстановитьВидимостьДоступностьЭлементов();
КонецПроцедуры

&НаКлиенте
Процедура СвернутьПоСчетамПриИзменении(Элемент)
	УстановитьПараметрыДинамическихСписков();
	УстановитьВидимостьДоступностьЭлементов();
КонецПроцедуры
...
Процедура ОткрытьОтчетОборотнаяВедомостьТоварыПоГруппам(Команда)
	...
	ПользовательскиеНастройки = УстановитьПользовательскиеНастройкиОтчета("ОборотнаяВедомостьТоваровПоГруппам",Новый Структура("СтПериод", стПериод));
	...
	ПараметрыФормы = Новый Структура("ПользовательскиеНастройки, Отбор, СформироватьПриОткрытии", ПользовательскиеНастройки, УсловияОтбора, Истина);
	ОткрытьФорму("Отчет.ОборотнаяВедомостьТоваровПоГруппам.Форма", ПараметрыФормы);	
КонецПроцедуры
...
Процедура ЗаполнитьСписокОбороты()
	...
	ТекстЗапроса = 	
		...
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(&ДатаНачала, &ДатаОкончания, , , , &ВидыСубконто, Организация = &Организация) КАК ХозрасчетныйОстаткиИОбороты";
...
Процедура ЗаполнитьРасшифровкуОборотов()
	...
	Запрос.Текст = 
		...
		|		КОНЕЦ) КАК Расход,
		|	СУММА(ХозрасчетныйОстаткиИОбороты.КоличествоКонечныйОстаток) КАК ОстатокКоличество,
		|	СУММА(ХозрасчетныйОстаткиИОбороты.СуммаКонечныйОстаток) КАК ОстатокСумма,
		...
		|	2 КАК Порядок
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНачала, &ДатаОкончания, Авто, , &ВидыСубконто) КАК ХозрасчетныйОбороты
...
&НаКлиенте
Процедура ЗаполнитьСписовВыбора()
...
&НаСервере
Функция ПолучитьВариантОтчета(ИмяВарианта)

	Возврат Справочники.ВариантыОтчетов.НайтиПоНаименованию(ИмяВарианта);	

КонецФункции // ПолучитьВариантОтчета()

&НаСервере
Функция ПоместитьСтруктуруВХранилище(Структура)

	Возврат Новый ХранилищеЗначения(Структура)	
