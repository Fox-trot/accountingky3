...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДекорацияКраткоеОписание.Заголовок = ЗначенияНастроекПланаОбмена.КраткаяИнформацияПоОбмену;
	...
	ЭтоПланОбменаРИБ  = ОбменДаннымиПовтИсп.ЭтоПланОбменаРаспределеннойИнформационнойБазы(Объект.ИмяПланаОбмена);
	...
	Объект.ЭтоНастройкаСтандартногоОбмена               = ОбменДаннымиПовтИсп.ЭтоУзелСтандартногоОбменаДанными(Объект.ИмяПланаОбмена);
...
Процедура ОбновитьНастройкиИИхОписанияЗначениямиПоУмолчанию(ДляОбменаЧерезПрямоеПодключение = Ложь)
	// 1. Для этой базы.
	ФормаНастройкиУзла = "ФормаНастройкиУзла";
	...
	НастройкаОтборовНаУзлеБазыКорреспондента = Новый Структура;
	
	ЗначенияПоУмолчаниюНаУзлеБазыКорреспондента = Новый Структура;
...
Функция СоздатьЗапросНаИспользованиеВнешнихРесурсов(Знач Объект, ЗапрашиватьCOM,
	...
	Запись.Корреспондент = Объект.СсылкаНового;
...
Функция ПолучитьОписаниеНастроекТранспортаОбмена()
	...
	ПредставленияНастроек = РегистрыСведений.НастройкиТранспортаОбменаДанными.ПредставленияНастроекТранспорта(Объект.ВидТранспортаСообщенийОбмена);
...
Процедура ПроверитьПодключениеWSНаКлиенте(Отказ, РасширеннаяПроверка = Ложь)
	...
	Если Не Отказ Тогда
		...
		Элементы.ПрефиксИнформационнойБазыПриемникаОбменССервисом.Видимость      = Не Объект.ПрефиксИнформационнойБазыПриемникаУстановлен; 
		Элементы.ПрефиксИнформационнойБазыПриемникаОбменССервисом.ТолькоПросмотр = Объект.ПрефиксИнформационнойБазыПриемникаУстановлен;
	КонецЕсли;
...
Процедура СтраницаМастераОжиданиеСозданияНастройкиОбменаДанными_ОбработкаДлительнойОперацииНаСервере(Отказ)
	...
	Если Объект.ЭтоНастройкаРаспределеннойИнформационнойБазы Тогда
		...
		Обработки.ПомощникСозданияОбменаДанными.ВыгрузитьНастройкиПодключенияДляПодчиненногоУзлаРИБ(ЭтотОбъект);
