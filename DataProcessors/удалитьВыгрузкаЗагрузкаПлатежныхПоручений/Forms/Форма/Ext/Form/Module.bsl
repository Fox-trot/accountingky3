...
Процедура ПоискБанка()
	...
	КлассификаторXML = Обработки.удалитьВыгрузкаЗагрузкаПлатежныхПоручений.ПолучитьМакет("СписокБанков").ПолучитьТекст();
...
Процедура ПродолжитьВыгрузку()
	...
	Если Режим = "Выгрузка" Тогда 
		...
		Если НайденныеСтроки.Количество() = 0 Тогда 
			...
			ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Банк",, Отказ);
		КонецЕсли;	
...
Функция СформироватьНаСервереОптимаБанк()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		Текст.ЗаписатьСтроку("Дата=" 				+ Формат(ВыборкаДетальныеЗаписи.Дата, "ДЛФ=D"));
		...
		Текст.ЗаписатьСтроку("СрокПлатежа=" 		+ Формат(ВыборкаДетальныеЗаписи.Дата, "ДЛФ=D"));
