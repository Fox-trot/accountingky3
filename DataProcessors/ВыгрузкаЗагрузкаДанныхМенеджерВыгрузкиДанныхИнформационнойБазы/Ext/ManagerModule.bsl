		...
		Если ИсключаемыеТипы.Найти(ОбъектМетаданных) <> Неопределено Тогда
			...
			ЗаписьЖурналаРегистрации(
				НСтр("ru = 'ВыгрузкаЗагрузкаДанных.ВыгрузкаОбъектаПропущена';
					|en = 'ВыгрузкаЗагрузкаДанных.ВыгрузкаОбъектаПропущена'", ОбщегоНазначения.КодОсновногоЯзыка()),
				...
				СтрШаблон(НСтр("ru = 'Выгрузка данных объекта метаданных %1 пропущена, т.к. он включен в
				 ...
				 |in the list of metadata objects excluded from data export and import '", ОбщегоНазначения.КодОсновногоЯзыка()),
