...
Процедура ПриОткрытии(Отказ)
	...
	Если СформироватьПриОткрытии Тогда	
		...
		//	Элементы.ГруппаСтраницыКонтрагенты.ТекущаяСтраница = Элементы.ГруппаИсторияАвансовНДС;
		//КонецЕсли;
	КонецЕсли;
КонецПроцедуры
...
#Область ОбработчикиСобытийЭлементовДинамическогоСписокРасшифровкаПоСчетамПодотчетников

&НаКлиенте
Процедура СписокРасшифровкаПоСчетамПодотчетниковПриАктивизацииСтроки(Элемент)
	ПолучитьСписокСчетовПодотчетника();
	УстановитьОтборДинамическихСписковПоПодотчетникам();
	СФормироватьСписокДокументовПоПодотчетнику();
КонецПроцедуры
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	Если ВСомах Тогда
		...
		Элементы.СписокРасшифровкаПоСчетамПодотчетниковВалюта.Видимость = Ложь;
	...
	Иначе
		...
		Элементы.СписокРасшифровкаПоСчетамПодотчетниковВалюта.Видимость = Истина;
...
Процедура ПроверкаДоговоров()
	...
	Для каждого СтрокаТабличнойЧасти Из Объект.ОбороткаСчетаДоговоры Цикл 
		Если Не СтрокаТабличнойЧасти.Контрагент = СтрокаТабличнойЧасти.Договор.Владелец Тогда 
			...
			ТБ.Контрагент = СтрокаТабличнойЧасти.Контрагент;
			ТБ.Договор = СтрокаТабличнойЧасти.Договор;
			ТБ.Счет = СтрокаТабличнойЧасти.Счет;
			ТБ.Содержание = "Контрагенту не соответствует текущий договор";
			ТБ.Регистратор = СтрокаТабличнойЧасти.Регистратор;
		...
		//Если НаимДоговора = СтрокаТабличнойЧасти.Договор И НаимКонтрагента = СтрокаТабличнойЧасти.Контрагент Тогда
		//	ТБ = ТаблицаЗначений.Добавить();
		...
		//КонецЕсли;
		//Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Договор) И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Контрагент) Тогда
		//	НаимДоговора = СтрокаТабличнойЧасти.Договор;
		//	НаимКонтрагента = СтрокаТабличнойЧасти.Контрагент;
		//КонецЕсли;
...
Процедура СформироватьОбороткаСчетаДоговоры();
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(&ДатаНачала, &ДатаОкончания, Авто, , Счет <> ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.НДСНААвансы), &ВидыСубконто, Организация = &Организация) КАК ХозрасчетныйОстаткиИОбороты
		|
		|УПОРЯДОЧИТЬ ПО
		|	Контрагент,
		|	Договор,
...
Процедура СформироватьОперации(ДополнительныеПараметры)
	...
	Результат = Запрос.Выполнить();
	Объект.Операции.Загрузить(Результат.Выгрузить());
	
	Для Каждого СтрокаТабличнойЧасти Из Объект.Операции Цикл 
		Если СтрокаТабличнойЧасти.ОборотДт = ДополнительныеПараметры.ОборотКт Тогда 
			СтрокаТабличнойЧасти.ВнутреннийОборот = ДополнительныеПараметры.ОборотДт; 			
		КонецЕсли; 
	КонецЦикла;
...
Процедура УстановитьОтборДинамическихСписковПоПодотчетникам()
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбораДинамическогоСписка(СписокРасшифровкаПоСчетамПодотчетников, "Подотчетник", ТекущийПодотчетник,,,ЗначениеЗаполнено(ТекущийПодотчетник)); 
...
Процедура УстановитьПараметрыДинамическихСписков()
	...
	// СписокРасшифровкаПоСчетамПодотчетников
	СписокРасшифровкаПоСчетамПодотчетников.Параметры.УстановитьЗначениеПараметра("ДатаНачала", НачалоДня(ДатаНачала));
	СписокРасшифровкаПоСчетамПодотчетников.Параметры.УстановитьЗначениеПараметра("ДатаОкончания", КонецДня(ДатаОкончания));
	СписокРасшифровкаПоСчетамПодотчетников.Параметры.УстановитьЗначениеПараметра("Организация", Объект.Организация);
	СписокРасшифровкаПоСчетамПодотчетников.Параметры.УстановитьЗначениеПараметра("ВСомах", ВСомах);
...
Процедура ПолучитьСписокСчетовПодотчетника()
	ТекущиеДанныеСписокРасшифровкаПоСчетамПодотчетников = Элементы.СписокРасшифровкаПоСчетамПодотчетников.ТекущиеДанные;
	Если ТекущиеДанныеСписокРасшифровкаПоСчетамПодотчетников = Неопределено Тогда
	 	Счет = ПредопределенноеЗначение("ПланСчетов.Хозрасчетный.ПустаяСсылка");
	Иначе	
	    Счет = ТекущиеДанныеСписокРасшифровкаПоСчетамПодотчетников.Счет;
	КонецЕсли;	
