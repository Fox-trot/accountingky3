...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Обработки.ПодборНоменклатуры.ПроверитьЗаполнениеПараметров(Параметры, Отказ, "КорзинаРеализации");
	...
	ТочностьЦены = Параметры.ТочностьЦены;
	...
	КешНастройкиПодбора.Вставить("ЗапрашиватьЦену", БухгалтерскийУчетПереопределяемый.ПолучитьЗначениеПоУмолчанию("ЗапрашиватьЦену"));
	КешНастройкиПодбора.Вставить("РазрешеноИзменятьЦену", Истина);
...
// Процедура - обработчик события ПриИзменении поля ввода КорзинаКоличество.
//
&НаКлиенте
Процедура КорзинаКоличествоПриИзменении(Элемент)
	РасчетСуммыВСтрокеКорзины(Элементы.Корзина.ТекущиеДанные);
КонецПроцедуры

// Процедура - обработчик события ПриИзменении поля ввода КорзинаЦена.
//
&НаКлиенте
...
&НаКлиенте
Процедура РасчетСуммыВСтрокеКорзины(СтрокаТабличнойЧасти)
...
&НаКлиенте
// Процедура - обработчик команды ПерейтиКРодителю (контекст. меню списка Номенклатура)
//
...
Процедура ИзменитьНастройки(Команда)
	...
	ПараметрКорзины.Вставить("Корзина", "КорзинаРеализации");
...
&НаКлиенте
// Функция ищет строки в корзине подбора с указанной номенклатурой
...
Процедура ДобавитьНоменклатуруВКорзину()
	...
	Если КешНастройкиПодбора.ЗапрашиватьКоличество
		...
		ДанныеСтрокиКорзины.Вставить("ТочностьЦены", 		ТочностьЦены);
		ДанныеСтрокиКорзины.Вставить("Корзина", 			"КорзинаРеализации");
	...
	Иначе
		Если ДанныеСтрокиКорзины.СтрокаКорзины = Неопределено Тогда
			...
			СтрокаКорзины.Цена = ДанныеСтрокиКорзины.Цена;
		Иначе
...
Процедура УстановитьПараметрыДинамическихСписков()
	...
	// Счета учета товаров
	МассивСчетовУчетаТоваров = БухгалтерскийУчетПовтИсп.СчетаУчетаТоваров();
	
	// Сортировка счетов учета
	НеИспользуемыеСчетаТоваров 		 	= Новый Массив; // Счета, которые не нужны в данном случае
	СчетаУчетаТоваровСуммовыеПоСкладу 	= Новый Массив; // счета с суммовым учетом по номенклатуре и складам
	СчетаУчетаТоваровНеСуммовыеПоСкладу = Новый Массив; // счета с суммовым учетом по номенклатуре и только количественным учетом по складам
	
	УчетТоваров.СортировкаСчетовУчета(МассивСчетовУчетаТоваров,
				                      НеИспользуемыеСчетаТоваров,
				                      СчетаУчетаТоваровСуммовыеПоСкладу,
				                      НеИспользуемыеСчетаТоваров,
				                      СчетаУчетаТоваровСуммовыеПоСкладу,
				                      НеИспользуемыеСчетаТоваров,
				                      НеИспользуемыеСчетаТоваров,
				                      СчетаУчетаТоваровСуммовыеПоСкладу,
				                      СчетаУчетаТоваровНеСуммовыеПоСкладу,
				                      СчетаУчетаТоваровНеСуммовыеПоСкладу,
				                      СчетаУчетаТоваровНеСуммовыеПоСкладу);

	// Счета учета МБП
	МассивСчетовУчетаМБП = БухгалтерскийУчетПовтИсп.СчетаУчетаМБП();
	
	// Сортировка счетов учета
	НеИспользуемыеСчетаМБП 		 	= Новый Массив; // Счета, которые не нужны в данном случае
	СчетаУчетаМБПСуммовыеПоСкладу 	= Новый Массив; // счета с суммовым учетом по номенклатуре и складам
	СчетаУчетаМБПНеСуммовыеПоСкладу = Новый Массив; // счета с суммовым учетом по номенклатуре и только количественным учетом по складам
	
	УчетМБП.СортировкаСчетовУчета(МассивСчетовУчетаМБП,
				                      НеИспользуемыеСчетаМБП,
				                      СчетаУчетаМБПСуммовыеПоСкладу,
				                      НеИспользуемыеСчетаМБП,
				                      СчетаУчетаМБПСуммовыеПоСкладу,
				                      СчетаУчетаМБПНеСуммовыеПоСкладу,
				                      СчетаУчетаМБПНеСуммовыеПоСкладу);	
	...
	// Товары
	ВидыСубконтоТоваровН = Новый Массив;
	ВидыСубконтоТоваровН.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура);
	Список.Параметры.УстановитьЗначениеПараметра("ВидыСубконтоТоваровН", ВидыСубконтоТоваровН);
	
	ВидыСубконтоТоваровНС = Новый Массив;
	ВидыСубконтоТоваровНС.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура);
	ВидыСубконтоТоваровНС.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Склады);
	Список.Параметры.УстановитьЗначениеПараметра("ВидыСубконтоТоваровНС", ВидыСубконтоТоваровНС);
	
	Список.Параметры.УстановитьЗначениеПараметра("СчетаУчетаТоваровСуммовыеПоСкладу", СчетаУчетаТоваровСуммовыеПоСкладу);
	Список.Параметры.УстановитьЗначениеПараметра("СчетаУчетаТоваровНеСуммовыеПоСкладу", СчетаУчетаТоваровНеСуммовыеПоСкладу);
	
	// МБП
	ВидыСубконтоМБПН = Новый Массив;
	ВидыСубконтоМБПН.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура);
	Список.Параметры.УстановитьЗначениеПараметра("ВидыСубконтоМБПН", ВидыСубконтоМБПН);
	
	ВидыСубконтоМБПНС = Новый Массив;
	ВидыСубконтоМБПНС.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Номенклатура);
	ВидыСубконтоМБПНС.Добавить(ПланыВидовХарактеристик.ВидыСубконтоХозрасчетные.Местонахождение);
	Список.Параметры.УстановитьЗначениеПараметра("ВидыСубконтоМБПНС", ВидыСубконтоМБПНС);
	
	Список.Параметры.УстановитьЗначениеПараметра("СчетаУчетаМБПСуммовыеПоСкладу", СчетаУчетаМБПСуммовыеПоСкладу);
	Список.Параметры.УстановитьЗначениеПараметра("СчетаУчетаМБПНеСуммовыеПоСкладу", СчетаУчетаМБПНеСуммовыеПоСкладу);
...
Процедура ЗаполнитьДанныеОбъекта()
	...
	Объект.ТипЦен = Параметры.ДоговорКонтрагента.ТипЦен;
КонецПроцедуры // ЗаполнитьДанныеОбъекта()

&НаСервере
// Процедура заполняет подобранные по переданным параметрам
//
...
Процедура УстановитьСвойстваЭлементовФормы()
	...
	//ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "КорзинаЦена", "Доступность", КешНастройкиПодбора.РазрешеноИзменятьЦену);
...
Процедура ОбновитьСписокПодобранных()
	...
	КешОтбораДанных = Новый Соответствие;
	
	Для Каждого СтрокаТабличнойЧасти Из Объект.Корзина Цикл 
		ОтборДанных = КешОтбораДанных[СтрокаТабличнойЧасти.Номенклатура];
		Если ОтборДанных = Неопределено
			Или НЕ ОтборДанных = СтрокаТабличнойЧасти.СчетУчета Тогда   // первое использование
			КешОтбораДанных.Вставить(СтрокаТабличнойЧасти.Номенклатура, СтрокаТабличнойЧасти.СчетУчета);
		Иначе 
			Продолжить;
		КонецЕсли;
...
Процедура УстановитьФорматЦены()
	...
	Если ТочностьЦены = 4 Тогда
		
		Элементы.СписокЦена.Формат = "ЧЦ=15; ЧДЦ=4";
		Элементы.КорзинаЦена.Формат = "ЧЦ=15; ЧДЦ=4";
		Элементы.КорзинаЦена.ФорматРедактирования = "ЧЦ=15; ЧДЦ=4";
		
	ИначеЕсли ТочностьЦены = 3 Тогда
		
		Элементы.СписокЦена.Формат = "ЧЦ=15; ЧДЦ=3";
		Элементы.КорзинаЦена.Формат = "ЧЦ=15; ЧДЦ=3";
		Элементы.КорзинаЦена.ФорматРедактирования = "ЧЦ=15; ЧДЦ=3";
		
	Иначе
		Элементы.СписокЦена.Формат = "ЧЦ=15; ЧДЦ=2";
		Элементы.КорзинаЦена.Формат = "ЧЦ=15; ЧДЦ=2";
		Элементы.КорзинаЦена.ФорматРедактирования = "ЧЦ=15; ЧДЦ=2";
	КонецЕсли;
