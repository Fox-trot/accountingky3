...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("МБПСклад") Тогда
		ОтборМБППоСкладу = Истина;
		Обработки.ПодборНоменклатуры.ПроверитьЗаполнениеПараметров(Параметры, Отказ, "КорзинаМБПСклад");
	Иначе
		ОтборМБППоСкладу = Ложь;
		Обработки.ПодборНоменклатуры.ПроверитьЗаполнениеПараметров(Параметры, Отказ, "КорзинаМБПМОЛ");
	КонецЕсли;
	...
	Если ОтборМБППоСкладу Тогда  
		Заголовок = СтрШаблон(НСтр("ru = 'Подбор товаров на дату %1 по складу %2'"), Объект.Дата, Объект.Склад);
	Иначе 
		Заголовок = СтрШаблон(НСтр("ru = 'Подбор товаров на дату %1 по МОЛ %2'"), Объект.Дата, Объект.ФизЛицо);
	КонецЕсли;
...
Процедура УстановитьПараметрыДинамическихСписков()
	...
	Если ОтборМБППоСкладу Тогда
		Список.Параметры.УстановитьЗначениеПараметра("Местонахождение", Объект.Склад);
	Иначе
		Список.Параметры.УстановитьЗначениеПараметра("Местонахождение", Объект.ФизЛицо);
	КонецЕсли;
