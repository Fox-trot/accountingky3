...
Функция СформироватьНаСервереОптимаБанк()
	...
	Запрос.Текст = 
		...
		|	ПлатежноеПоручениеИсходящее.КодНазначенияПлатежа.Код КАК КодПлатежа
		|ИЗ
...
Функция СформироватьНаСервереДемирКыргызИнтернэшнлБанк()
	...
	Запрос.Текст = 
		...
		|	ПлатежноеПоручениеИсходящее.БанковскийСчетПолучателя.Банк.Код КАК БанковскийСчетПолучателяБанкКод,
		...
		|	ПлатежноеПоручениеИсходящее.ВалютаДенежныхСредств КАК ВалютаДенежныхСредств,
		|	ПлатежноеПоручениеИсходящее.КодНазначенияПлатежа.Код КАК КодНазначенияПлатежаКод,
		|	ПлатежноеПоручениеИсходящее.ВалютаДенежныхСредств.Наименование КАК ВалютаДенежныхСредствНаименование,
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		СуммаПрописью = БухгалтерскийУчетСервер.СформироватьСуммуПрописью(ВыборкаДетальныеЗаписи.СуммаДокумента, ВыборкаДетальныеЗаписи.ВалютаДенежныхСредств);
		...
		Поле17 = Лев(ВыборкаДетальныеЗаписи.КодНазначенияПлатежаКод + Пробелы(20), 20);
		
		// 18. Currency Code - 10 - Код валюты.
		// Только в латинских буквах.
		Поле18 = Лев(ВыборкаДетальныеЗаписи.ВалютаДенежныхСредствНаименование + Пробелы(10), 10);
