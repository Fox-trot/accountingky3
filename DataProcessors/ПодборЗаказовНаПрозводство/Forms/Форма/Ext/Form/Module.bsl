...
Процедура УстановитьУсловноеОформление()
	...
	СписокКонтрагентов.УсловноеОформление.Элементы.Очистить();
	
	// Таблица СписокКонтрагентов.
	Элемент = СписокКонтрагентов.УсловноеОформление.Элементы.Добавить();
	// Поля.	
	КомпоновкаДанныхКлиентСервер.ДобавитьОформляемоеПоле(Элемент.Поля, "Контрагент");
	// Условие.	
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(Элемент.Отбор, "Контрагент", ВидСравненияКомпоновкиДанных.НеЗаполнено);
	// Оформление.
	Элемент.Оформление.УстановитьЗначениеПараметра("Текст", НСтр("ru = 'Полуфабрикат'"));
...
Процедура УстановитьПараметрыДинамическихСписков(ПервоначальноеЗаполнение = Ложь)
	...
	Если ПервоначальноеЗаполнение Тогда
		...
		СписокОпераций.Параметры.УстановитьЗначениеПараметра("Период", Объект.Период);
		СписокОпераций.Параметры.УстановитьЗначениеПараметра("Организация", Объект.Организация);
		СписокОпераций.Параметры.УстановитьЗначениеПараметра("Регистратор", Объект.Регистратор);
		СписокОпераций.Параметры.УстановитьЗначениеПараметра("Проведен", Объект.Проведен);
	КонецЕсли;
	...
	СписокОпераций.Параметры.УстановитьЗначениеПараметра("Заказ", Объект.ТекущийЗаказ);
	СписокОпераций.Параметры.УстановитьЗначениеПараметра("Номенклатура", Объект.ТекущаяПродукция);	
КонецПроцедуры

&НаСервере
Процедура ОтредактироватьУсловноеОформление(НаименованиеДействия, Операция = Неопределено)

	// Оформление подобранных товаров.
	ЭлементыУсловногоОформленияСписка = СписокОпераций.КомпоновщикНастроек.ФиксированныеНастройки.УсловноеОформление.Элементы;
	...
	Если НаименованиеДействия = "ЗаполнитьПоСтрокамИзДокумента" Тогда	
		...
		Для Каждого СтрокаТабличнойЧасти Из Объект.ПодобранныеСтрокиИзДокумента Цикл 
			...
			ЭлементОтбораДанных = ГруппаЭлементовОтбораДанных.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			ЭлементОтбораДанных.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Операция");
			ЭлементОтбораДанных.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
			ЭлементОтбораДанных.ПравоеЗначение = СтрокаТабличнойЧасти.Операция;
			ЭлементОтбораДанных.Использование  = Истина;
		КонецЦикла;		
	...
	ИначеЕсли НаименованиеДействия = "ЗаполнитьПоПодбираемойСтроке" Тогда
		...
		ЭлементОтбораДанных.ЛевоеЗначение  = Новый ПолеКомпоновкиДанных("Операция");
		ЭлементОтбораДанных.ВидСравнения   = ВидСравненияКомпоновкиДанных.Равно;
		ЭлементОтбораДанных.ПравоеЗначение = Операция;		
	КонецЕсли;
КонецПроцедуры
...
Процедура СписокВидовРаботВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	ЗаполнитьПоПодобраннойСтроке(Элемент.ТекущиеДанные.Операция);
КонецПроцедуры
...
&НаКлиенте
Процедура ЗаполнитьПоПодобраннойСтроке(Операция)
	...
	Иначе
		...
		ОтредактироватьУсловноеОформление("ЗаполнитьПоПодбираемойСтроке", Операция);
	КонецЕсли;
КонецПроцедуры
