...
Функция ЗагрузитьКурсыВалютПоПараметрамНБК(Валюты, НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки,	ПриЗагрузкеВозниклиОшибки)     	
	...
	Иначе
		...
		Если МассивОсновныхВалют.Количество() > 0 Тогда 			 			
			ЗагрузитьЕжедневныеКурсыОсновныхВалютИзЭксельФайла(СерверИсточник, МассивОсновныхВалют, 
				НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки, ПриЗагрузкеВозниклиОшибки, СостояниеЗагрузки);
		КонецЕсли;
		
		// Загрузка курсов валют из EXCEL файла allvalsrus.xls
		Если МассивНеОсновныхВалют.Количество() > 0 Тогда
			ЗагрузитьЕженедельныеКурсыНеОсновныхВалютИзЭксельФайла(СерверИсточник, МассивНеОсновныхВалют, 
				НачалоПериодаЗагрузки, ОкончаниеПериодаЗагрузки, ПриЗагрузкеВозниклиОшибки, СостояниеЗагрузки);
		КонецЕсли;
	КонецЕсли;     	
...
//
Процедура ЗагрузитьЕжедневныеКурсыОсновныхВалютИзЭксельФайла(СерверИсточник, 
		МассивВалют, 
	...
	ПутьДляСохранения = ПутьДляСохранения + "dailyrus.xls";
...
//
Процедура ЗагрузитьЕженедельныеКурсыНеОсновныхВалютИзЭксельФайла(СерверИсточник, 
		МассивВалют, 
		Знач НачалоПериодаЗагрузки, 
		Знач ОкончаниеПериодаЗагрузки, 
		ПриЗагрузкеВозниклиОшибки,
	...
	ПараметрыПолучения = Новый Структура;
	ПутьДляСохранения = КаталогВременныхФайлов();
	ПутьДляСохранения = ПутьДляСохранения + "allvalsrus.xls";
	ПараметрыПолучения.Вставить("ПутьДляСохранения", ПутьДляСохранения);
		...
		Для НомерСтроки = 1 По ТекущаяОбласть.ВысотаТаблицы Цикл 
			...
			Если НомерСтроки = 1 Тогда 
				...
				Если СтрДлина(Текст) = 3 Тогда 
					КратностьСтр = "1";
				Иначе
					// Символьный код вконце наименования
					КратностьСтр = СокрЛП(СтрЗаменить(Текст, СимвольныйКод, "")); 
					КратностьСтр = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(КратностьСтр);
			...
			Если НачалоПериодаЗагрузки = ОкончаниеПериодаЗагрузки Тогда
				...
				Курс = СтроковыеФункцииКлиентСервер.СтрокаВЧисло(КурсСтр);
			Иначе
