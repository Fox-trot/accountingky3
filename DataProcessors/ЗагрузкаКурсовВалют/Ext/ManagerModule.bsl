...
Функция ЗагрузитьКурсыВалютПоПараметрам(Знач Валюты, Знач НачалоПериодаЗагрузки, Знач ОкончаниеПериодаЗагрузки, ПриЗагрузкеВозниклиОшибки = Ложь)
	...
	Иначе
		...
		// БПКР
		СерверИсточник = "http://service.1ckato.kg:81/ak/hs/currencyrates/exchangerate/v1";
		Если НачалоПериодаЗагрузки = ОкончаниеПериодаЗагрузки Тогда
			ШаблонИмениФайла = СерверИсточник + "/%1" + ИмяФайлаДневногоКурса;
		Иначе
			ШаблонИмениФайла = СерверИсточник + "/%1";
		КонецЕсли;
		ПараметрыПолучения = ПараметрыПолученияСтатистика();
		// Конец БПКР
	КонецЕсли;
...
Функция ПараметрыПолученияСтатистика()
	
	ПараметрыПолучения = ПолучениеФайловИзИнтернетаКлиентСервер.ПараметрыПолученияФайла();
	
	ПараметрыПолучения.Заголовки.Вставить("ConfigurationName", ИнтернетПоддержкаПользователей.ИмяКонфигурации());
	ПараметрыПолучения.Заголовки.Вставить("ConfigurationVersion", ИнтернетПоддержкаПользователей.ВерсияКонфигурации());
	
	УстановитьПривилегированныйРежим(Истина);
	ДанныеАутентификации = ИнтернетПоддержкаПользователей.ДанныеАутентификацииПользователяИнтернетПоддержки();
	УстановитьПривилегированныйРежим(Ложь);
	ПараметрыПолучения.Заголовки.Вставить("login", ДанныеАутентификации.Логин);
	ПараметрыПолучения.Заголовки.Вставить("password", ДанныеАутентификации.Пароль);
	
	ОрганизацияПоУмолчанию = БухгалтерскийУчетПереопределяемый.ПолучитьЗначениеПоУмолчанию("ОсновнаяОрганизация");
	ПараметрыПолучения.Заголовки.Вставить("OrganizationName", ОрганизацияПоУмолчанию.НаименованиеПолное);
	ПараметрыПолучения.Заголовки.Вставить("OrganizationINN", ОрганизацияПоУмолчанию.ИНН);
