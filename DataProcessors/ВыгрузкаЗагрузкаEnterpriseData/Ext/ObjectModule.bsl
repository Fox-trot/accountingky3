...
Функция РезультатВыгрузкиВXML(СтруктураПараметров, АдресДляРазмещенияРезультата = Неопределено) Экспорт
	...
	Если МестоВыгрузки = 0 Тогда
		...
		ОбменДаннымиXDTOСервер.ОткрытьФайлВыгрузки(КомпонентыОбмена, АдресНаСервере);
	Иначе
		// в текст
	...
	Если СписокДополнениеКВыгрузке.Количество() > 0
		И Не ЕстьОшибкиПередКонвертацией Тогда
	...
	Если КомпонентыОбмена.ФлагОшибки Тогда
		ТекстСообщения = НСтр("ru = 'В ходе выполнения операции возникли ошибки (см. Журнал регистрации). Данные выгружены не полностью.'");
