...
Процедура ЗаполнитьСписокОС()
	...
	Запрос.Текст =
		...
		|		ПО ОсновныеСредства.Ссылка = ВременнаяТаблицаПараметрыУчетаОС.ОсновноеСредство
		...
		|		ПО ОсновныеСредства.Ссылка = СостоянияОССрезПоследних.ОсновноеСредство
		...
		|		ПО ОсновныеСредства.Ссылка = Износ.Субконто1
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(
		|				,
		|				&ДатаОкончания,
		|				,
		...
		|		ПО ОсновныеСредства.Ссылка = Стоимость.Субконто1";
...
Процедура ПолучитьТаблицуРасшифровкиПоСобытиямОСНаСервере()
	...
	Запрос.Текст =
		...
		|	ХозрасчетныйНакопленныйИзнос.НакопленныйИзнос КАК НакопленныйИзнос,
		...
		|		ПО ХозрасчетныйОсновнаяТаблица.Период >= УчетнаяПолитика.Период
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаДатаПринятияКУчетуОС КАК ДатаПринятияКУчетуОС
		|		ПО ВЫБОР
		|				КОГДА ХозрасчетныйОсновнаяТаблица.Регистратор ССЫЛКА Документ.МодернизацияОС
		|					ТОГДА ХозрасчетныйОсновнаяТаблица.Период >= ДОБАВИТЬКДАТЕ(ДатаПринятияКУчетуОС.Период, ДЕНЬ, 1)
		|				ИНАЧЕ ХозрасчетныйОсновнаяТаблица.Период >= ДатаПринятияКУчетуОС.Период
		|			КОНЕЦ
		|			И ХозрасчетныйОсновнаяТаблица.ОсновноеСредство >= ДатаПринятияКУчетуОС.ОсновноеСредство
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ХозрасчетныйНакопленныйИзнос.НакопленныйИзнос
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаДанныеСПериодами.Период КАК Период,
		|	ВременнаяТаблицаДанныеСПериодами.Регистратор КАК Регистратор,
		|	ВременнаяТаблицаДанныеСПериодами.Организация КАК Организация,
		|	ВременнаяТаблицаДанныеСПериодами.ОсновноеСредство КАК ОсновноеСредство,
		|	СУММА(ВременнаяТаблицаДанныеСПериодами.НакопленныйИзнос) КАК НакопленныйИзнос,
		|	ВременнаяТаблицаДанныеСПериодами.ПериодУП,
		|	ВременнаяТаблицаДанныеСПериодами.ДатаПринятияОС
		|ПОМЕСТИТЬ ВременнаяТаблицаДанныеНакопленнымИзносом
		|ИЗ
		|	ВременнаяТаблицаДанныеСПериодами КАК ВременнаяТаблицаДанныеСПериодами
		|СГРУППИРОВАТЬ ПО
		|	ВременнаяТаблицаДанныеСПериодами.Период,
		|	ВременнаяТаблицаДанныеСПериодами.Регистратор,
		|	ВременнаяТаблицаДанныеСПериодами.Организация,
		|	ВременнаяТаблицаДанныеСПериодами.ОсновноеСредство,
		|	ВременнаяТаблицаДанныеСПериодами.ПериодУП,
		|	ВременнаяТаблицаДанныеСПериодами.ДатаПринятияОС
		|;
		|
		...
		|ИЗ
		|	ВременнаяТаблицаДанныеНакопленнымИзносом КАК ТаблицаДанных
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаУчетнаяПолитика КАК УчетнаяПолитика
