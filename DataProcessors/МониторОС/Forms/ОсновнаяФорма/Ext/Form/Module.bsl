...
&НаКлиенте
Процедура ОткрытьОтчетВедомостьРасходаОС(Команда)
	
	стПериод = Новый СтандартныйПериод;
	стПериод.ДатаНачала = ДатаНачала;
	стПериод.ДатаОкончания = КонецДня(ДатаОкончания);
	
	ПользовательскиеНастройки = УстановитьПользовательскиеНастройкиОтчета("ВедомостьРасходаОС",Новый Структура("СтПериод", стПериод));
	...
	ПараметрыФормы = Новый Структура("ПользовательскиеНастройки, Отбор, СформироватьПриОткрытии", ПользовательскиеНастройки, УсловияОтбора, Истина);
	ОткрытьФорму("Отчет.ВедомостьРасходаОС.Форма", ПараметрыФормы);
КонецПроцедуры
...
&НаКлиенте
Процедура ОткрытьОтчетОборотнаяВедомостьПоКатегориямОС(Команда)
	
	стПериод = Новый СтандартныйПериод;
	стПериод.ДатаНачала = ДатаНачала;
	стПериод.ДатаОкончания = КонецДня(ДатаОкончания);
	
	ПользовательскиеНастройки = УстановитьПользовательскиеНастройкиОтчета("ОборотнаяВедомостьПоКатегориямОС",Новый Структура("СтПериод", стПериод));
	...
	ПараметрыФормы = Новый Структура("ПользовательскиеНастройки, Отбор, СформироватьПриОткрытии", ПользовательскиеНастройки, УсловияОтбора, Истина);
	ОткрытьФорму("Отчет.ОборотнаяВедомостьПоКатегориямОС.Форма", ПараметрыФормы);
КонецПроцедуры
...
&НаКлиенте
Процедура ОткрытьОтчетРеестрПоСписаниюОС(Команда)
	
	стПериод = Новый СтандартныйПериод;
	стПериод.ДатаНачала = ДатаНачала;
	стПериод.ДатаОкончания = КонецДня(ДатаОкончания);
	
	ПользовательскиеНастройки = УстановитьПользовательскиеНастройкиОтчета("РеестрПоСписаниюОС",Новый Структура("СтПериод", стПериод));
	...
	ПараметрыФормы = Новый Структура("ПользовательскиеНастройки, Отбор, СформироватьПриОткрытии", ПользовательскиеНастройки, УсловияОтбора, Истина);
	ОткрытьФорму("Отчет.РеестрПоСписаниюОС.Форма", ПараметрыФормы);
КонецПроцедуры
...
Процедура УстановитьВидимостьДоступностьЭлементов()	
	...
	Элементы.ДатаОкончания.Заголовок = "по";
	...
	ИначеЕсли Страница = "СтраницаНалогНаИмущество" Тогда
		...
		Элементы.ДатаНачала.Видимость 	 = Ложь;
		Элементы.ДатаОкончания.Видимость = Истина;
		Элементы.ДатаОкончания.Заголовок = "Период";
...
Процедура ПолучитьТаблицуРасшифровкиПоСобытиямОСНаСервере()
	...
	Запрос.Текст =
		...
		|		ПОЛНОЕ СОЕДИНЕНИЕ ВременнаяТаблицаПараметрыУчетаОСНаДатуОкончания КАК ТаблицаПараметровУчетаОС
		...
		|						ПараметрыУчетаОС.ОсновноеСредство
		|					ИЗ
		|						ВременнаяТаблицаПараметрыУчетаОСНаДатуОкончания КАК ПараметрыУчетаОС)) КАК ХозрасчетныйОстаткиИОбороты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Процедура УстановитьПараметрыДинамическихСписков()
	...
	//СобытияОСНалогНаИмущество.Параметры.УстановитьЗначениеПараметра("НачалоПериода", НачалоГода(ДатаНачала));
