...
Процедура СписокОСПриАктивизацииСтроки(Элемент)
	...
	Иначе 		
		...
		НадписьНакопленныйИзнос 		= СтрШаблон("Накопленный износ: %1", ?(ТекущиеДанныеСписокОС.СуммаАмортизация < 0, 
...
Процедура ОткрытьОтчетВедомостьПоАмортизацииОС(Команда)
	...
	ПользовательскиеНастройки = УстановитьПользовательскиеНастройкиОтчета("ВедомостьПоАмортизацииОС",Новый Структура("СтПериод", стПериод));
	...
	ПараметрыФормы = Новый Структура("ПользовательскиеНастройки, Отбор, СформироватьПриОткрытии", ПользовательскиеНастройки, УсловияОтбора, Истина);
	ОткрытьФорму("Отчет.ВедомостьПоАмортизацииОС.Форма", ПараметрыФормы);
КонецПроцедуры
...
Процедура ОткрытьОтчетВедомостьОС(Команда)
	...
	ПользовательскиеНастройки = УстановитьПользовательскиеНастройкиОтчета("ВедомостьОС",Новый Структура("СтПериод", стПериод));
	...
	Если ТекущиеДанные <> Неопределено Тогда
		УсловияОтбора.Вставить("ОсновноеСредство", ТекущиеДанные.ОсновноеСредство);	
	КонецЕсли;	
	
	ПараметрыФормы = Новый Структура("ПользовательскиеНастройки, Отбор, СформироватьПриОткрытии", ПользовательскиеНастройки, УсловияОтбора, Истина);
	ОткрытьФорму("Отчет.ВедомостьОС.Форма", ПараметрыФормы);
КонецПроцедуры
...
&НаКлиенте
Процедура СформироватьДокументыПоТранспорту(Команда)
	
	СтрокаТабличнойЧасти = Элементы.Транспорт.ТекущиеДанные;
	
	Если СтрокаТабличнойЧасти = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Не выбрана строка таблицы ""Транспорт"" по которой необходимо сформировать документы.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.Транспорт");
		Возврат;
	Иначе	
		ЗаполнитьДокументыНалогаНаИмущество(СтрокаТабличнойЧасти.ОсновноеСредство);		
	КонецЕсли;			
КонецПроцедуры

&НаКлиенте
Процедура СформироватьДокументыПоНедвижимости(Команда)
	
	СтрокаТабличнойЧасти = Элементы.Недвижимость.ТекущиеДанные;
	
	Если СтрокаТабличнойЧасти = Неопределено Тогда
		ТекстСообщения = НСтр("ru = 'Не выбрана строка таблицы ""Недвижимость"" по которой необходимо сформировать документы.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.Недвижимость");
		Возврат;
	Иначе	
		ЗаполнитьДокументыНалогаНаИмущество(СтрокаТабличнойЧасти.ОсновноеСредство);		
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьДокументыНалогаНаИмущество(ОсновноеСредствоНалогаНаИмущество)
	...
	Запрос.Текст = 
		...
		|			Авто,
		|			Организация = &Организация
		|				И ОсновноеСредство = &ОсновноеСредство) КАК ЗемельныйНалогОбороты
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ИнвентаризацияОСОС.ОсновноеСредство = &ОсновноеСредство
		...
		|	И (ЗакрытиеМесяца.Транспорт.ОсновноеСредство = &ОсновноеСредство
		|			ИЛИ ЗакрытиеМесяца.Недвижимость.ОсновноеСредство = &ОсновноеСредство)
	...
	Запрос.УстановитьПараметр("Организация", Организация);
	Запрос.УстановитьПараметр("ОсновноеСредство", ОсновноеСредствоНалогаНаИмущество);
	
	СобытияОСНалогНаИмущество.Загрузить(Запрос.Выполнить().Выгрузить());
КонецПроцедуры
...
Процедура ФормированиеНалогаНаИмущество()
	...
	Объект.Транспорт.Загрузить(МассивРезультатов[1].Выгрузить());
	Объект.Недвижимость.Загрузить(МассивРезультатов[2].Выгрузить());	
КонецПроцедуры
