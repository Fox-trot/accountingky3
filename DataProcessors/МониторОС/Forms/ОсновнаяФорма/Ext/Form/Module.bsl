...
Процедура СписокОС_ТЗПриАктивизацииСтроки(Элемент)
	...
	Если СтрокаТаблицыЗначений <> Неопределено Тогда
		...
		НадписьБазаАмортизации			= СтрШаблон("База амортизации: %1", СтрокаТаблицыЗначений.БазаАмортизации);
	Иначе
...
Процедура ОСПриАктивизацииСтроки(Элемент)
	...
	ТекущаяСтрокаСписка = Элементы.ОС.ТекущиеДанные;
	
	Если ТекущаяСтрокаСписка <> Неопределено Тогда
		СтруктураОтбора = Новый ФиксированнаяСтруктура("ОсновноеСредство", ТекущаяСтрокаСписка.Наименование);
	Иначе
		СтруктураОтбора = Неопределено;
	КонецЕсли;	

	Элементы.РасшифровкаНалогНаИмущество.ОтборСтрок = СтруктураОтбора;		
КонецПроцедуры
...
Процедура ЗаполнитьСписокОС()
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаПараметрыУчетаОС.БазаАмортизации КАК БазаАмортизации,
...
Процедура УстановитьПараметрыДинамическихСписков(ИзменилосьОС)
	...
	Если ИзменилосьОС Тогда
		...
		Если ВсеОсновныеСредства Тогда
			...
			МестонахождениеОС.Параметры.УстановитьЗначениеПараметра("ОсновноеСредство", МассивОС);
			СобытияОС.Параметры.УстановитьЗначениеПараметра("ОсновноеСредство", 		МассивОС);
			СостояниеОС.Параметры.УстановитьЗначениеПараметра("ОсновноеСредство", 		МассивОС);
			ПараметрыУчетаОС.Параметры.УстановитьЗначениеПараметра("ОсновноеСредство", 	МассивОС);	
	...
	Иначе
		...
		ОС.Параметры.УстановитьЗначениеПараметра("ДатаОкончания", 				КонецДня(ДатаОкончания));
		ОС.Параметры.УстановитьЗначениеПараметра("ДатаНачала", 					НачалоДня(ДатаНачала));
...
Процедура ФормированиеНалогаНаИмущество()
	...
	Запрос.Текст = 
		...
		|	СостоянияОССрезПоследних.Период КАК Дата,
		|	СостоянияОССрезПоследних.Регистратор КАК Документ
		|ИЗ
		...
		|		ПО ЗакрытиеМесяцаНедвижимость.ОсновноеСредство = СостоянияОССрезПоследних.ОсновноеСредство
		|ГДЕ
		|	ЗакрытиеМесяцаНедвижимость.Ссылка.Организация = &Организация
		|	И ЗакрытиеМесяцаНедвижимость.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	ЗакрытиеМесяцаТранспорт.ОсновноеСредство,
		|	ЗакрытиеМесяцаТранспорт.КоличествоМесяцевЭксплуатации,
		...
		|				Организация = &Организация
		|					И Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийОС.ПринятоКУчету)) КАК СостоянияОССрезПоследних
		|		ПО ЗакрытиеМесяцаТранспорт.ОсновноеСредство = СостоянияОССрезПоследних.ОсновноеСредство
		|ГДЕ
		|	ЗакрытиеМесяцаТранспорт.Ссылка.Организация = &Организация
		|	И ЗакрытиеМесяцаТранспорт.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания";
	Запрос.УстановитьПараметр("Организация", 	Организация);
	Запрос.УстановитьПараметр("ДатаНачала", 	НачалоДня(ДатаНачала));
	Запрос.УстановитьПараметр("ДатаОкончания", 	КонецДня(ДатаОкончания));
	
	РасшифровкаНалогНаИмущество.Очистить();
	РасшифровкаНалогНаИмущество.Загрузить(Запрос.Выполнить().Выгрузить());
	РасшифровкаНалогНаИмущество.Сортировать("Дата");
КонецПроцедуры
...
Процедура УстановитьОтборСпискаОС()
	...
	Если ЗначениеЗаполнено(ОтборСостояние) Тогда
		Если СтрокаКолонокФиксированнойСтруктуры = "" Тогда
			СтрокаКолонокФиксированнойСтруктуры = "СостояниеОС";
		Иначе
			СтрокаКолонокФиксированнойСтруктуры = СтрокаКолонокФиксированнойСтруктуры + ", СостояниеОС";
		КонецЕсли;	
