...
Функция СтраницаАдресСервисаДалее()
	...
	Если Не ЗначениеЗаполнено(АдресСервиса) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнен адрес сервиса';
	...
	Если ЧастиАдреса.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Некорректный адрес сервиса';
...
Функция СтраницаИдентификационныеДанныеДалее()
	...
	Если Не ЗначениеЗаполнено(РегистрацияНаименование) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Полное имя (Ф.И.О.)""';
														|en = 'The ""Full name"" field is not filled in'"), , "РегистрацияНаименование", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Логин) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Имя(логин)""';
														|en = 'The ""Name (login)"" field is not filled in'"), , "Логин", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(РегистрацияЭлектроннаяПочта) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Электронная почта""';
														|en = 'The Email field is not filled in'"), , "РегистрацияЭлектроннаяПочта", , Отказ);
	КонецЕсли;
	Если Не ЗначениеЗаполнено(Пароль) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Пароль""';
														|en = 'The Password field is not filled in'"), , "Пароль", , Отказ);
	ИначеЕсли Пароль <> ПодтверждениеПароля Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Подтверждение пароля не совпадает с паролем';
...
Функция СтраницаВыборАбонентаДалее()
	...
	Иначе
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выбран абонент';
...
Функция СтраницаСозданиеПриложениеДалее()
	...
	Если Не ЗначениеЗаполнено(Наименование) Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не заполнено поле ""Наименование""';
