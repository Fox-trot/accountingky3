...
Процедура ПодготовитьФормуНаСервере()
	...
	ДатаОплатыНачало 				= НачалоДня(ТекущаяДата());
	ДатаОплатыКонец  				= КонецДня(ТекущаяДата()); 	
	...
	Если ЗначениеЗаполнено(БанковскийСчет) Тогда
		Элементы.Декорация1.Заголовок = БанковскийСчет.ВалютаДенежныхСредств;
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	//Объект   	= Форма.Объект;
	//Элементы 	= Форма.Элементы;
	//Валюта 		= Форма.ВалютаДенежныхСредств;
	//ВалютаСом 	= Форма.ВалютаРегламентированногоУчета;	
...
Процедура ЗаполнитьВыписку()
	...
	ТекстЗапроса = "ВЫБРАТЬ
	               ...
	               |			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйДвиженияОборотыДтКт.Регистратор КАК Документ.РасходныйКассовыйОрдер).Операция
	               |		КОГДА ХозрасчетныйДвиженияОборотыДтКт.Регистратор ССЫЛКА Документ.ПриходныйКассовыйОрдер
	               |			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйДвиженияОборотыДтКт.Регистратор КАК Документ.ПриходныйКассовыйОрдер).Операция
	               |		КОГДА ХозрасчетныйДвиженияОборотыДтКт.Регистратор ССЫЛКА Документ.ПлатежноеПоручениеИсходящее
	               |			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйДвиженияОборотыДтКт.Регистратор КАК Документ.ПлатежноеПоручениеИсходящее).Операция
	               |		КОГДА ХозрасчетныйДвиженияОборотыДтКт.Регистратор ССЫЛКА Документ.ПлатежноеПоручениеВходящее
	               |			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйДвиженияОборотыДтКт.Регистратор КАК Документ.ПлатежноеПоручениеВходящее).Операция
	...
	//               |			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.Регистратор КАК Документ.РасходныйКассовыйОрдер).Операция
	//               |		КОГДА ХозрасчетныйДвиженияССубконто.Регистратор ССЫЛКА Документ.ПриходныйКассовыйОрдер
	//               |			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.Регистратор КАК Документ.ПриходныйКассовыйОрдер).Операция
	//               |		КОГДА ХозрасчетныйДвиженияССубконто.Регистратор ССЫЛКА Документ.ПлатежноеПоручениеИсходящее
	//               |			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.Регистратор КАК Документ.ПлатежноеПоручениеИсходящее).Операция
	//               |		КОГДА ХозрасчетныйДвиженияССубконто.Регистратор ССЫЛКА Документ.ПлатежноеПоручениеВходящее
	//               |			ТОГДА ВЫРАЗИТЬ(ХозрасчетныйДвиженияССубконто.Регистратор КАК Документ.ПлатежноеПоручениеВходящее).Операция
	...
	Пока ВыборкаСписокДокументов.Следующий() Цикл
		...
		Строка.Операция      			= ВыборкаСписокДокументов.Операция;
