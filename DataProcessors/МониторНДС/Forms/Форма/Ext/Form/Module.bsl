...
Процедура СформироватьНаСервере()
	...
	Запрос.Текст = 
		...
		|	СУММА(ЗакрытиеМесяцаАвансыРасшифровка.Сумма + ЗакрытиеМесяцаАвансыРасшифровка.СуммаНДС + ЗакрытиеМесяцаАвансыРасшифровка.СуммаНСП) КАК Всего,
		...
		|	СУММА(ЗакрытиеМесяцаДоотгрузкаРасшифровка.Сумма + ЗакрытиеМесяцаДоотгрузкаРасшифровка.СуммаНДС + ЗакрытиеМесяцаДоотгрузкаРасшифровка.СуммаНСП) КАК Всего
		|ИЗ
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("СуммаДоотгрузки", ПараметрыРасчета);
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("СуммаНДСДоотгрузки", ПараметрыРасчета);
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("СуммаНСПДоотгрузки", ПараметрыРасчета);
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("СуммаАвансы", ПараметрыРасчета);
	...
	Для Каждого СтрокаТаблицы Из СтруктураДанных.ТаблицаДетальныеЗаписи Цикл
		...
		Отбор.Владелец = "НДС КР не разрешенный";
		МассивСтрок = ТЗДетальныеЗаписи.НайтиСтроки(Отбор);
		...
		Отбор.Владелец = "НДС ЕАЭС не разрешенный";
		МассивСтрок = ТЗДетальныеЗаписи.НайтиСтроки(Отбор);
		...
		Отбор.Владелец = "НДС не КР не разрешенный";
		МассивСтрок = ТЗДетальныеЗаписи.НайтиСтроки(Отбор);
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("СуммаНДСАвансы", ПараметрыРасчета);
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("СуммаНСПАвансы", ПараметрыРасчета);
	...
	ТаблицаВсейРеализации	= УчетНДСВызовСервера.ВсеСведенияОРеализации(Организация, ДатаНачала, ДатаОкончания, 
									ДанныеУчетнойПолитики.НеУчитыватьЗакупкиБезНДС, ДанныеУчетнойПолитики.ОтчетПоНДСПоПоставке);
	ТаблицаВсехПоступлений	= УчетНДСВызовСервера.ВсеСведенияОПоступлении(Организация, ДатаНачала, ДатаОкончания, ДанныеУчетнойПолитики.НеУчитыватьЗакупкиБезНДС);	
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("НДСДляРаспределения", ПараметрыРасчета);	
	...
	СтрокаТабличнойЧасти.Содержание = НСтр("ru = 'НДС на приобретения с признаком ""Распределение"" или
													|весь НДС на пробретения, если указание признаков отключено в УП'");
	...
	ПараметрыРасчета.Вставить("Таблица", 									ТаблицаВсехПоступлений);
	ПараметрыРасчета.Вставить("УказыватьПризнакЗачетаНДСПриПоступлении", 	ДанныеУчетнойПолитики.УказыватьПризнакЗачетаНДСПриПоступлении);	
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("НДСДляРаспределенияКР", ПараметрыРасчета);	
	...
	//ПараметрыРасчета.Вставить("Таблица", 									ТаблицаВсехПоступлений);
	//ПараметрыРасчета.Вставить("УказыватьПризнакЗачетаНДСПриПоступлении", 	ДанныеУчетнойПолитики.УказыватьПризнакЗачетаНДСПриПоступлении);	
	//СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("НДСДляРаспределенияЕАЭС", ПараметрыРасчета);	
	//НДСДляРаспределенияЕАЭС = СтруктураДанных.РезультатРасчета;
	//СтрокаТабличнойЧасти = СтрокиОтчета.Добавить();
	//СтрокаТабличнойЧасти.Порядок = 24;
	//СтрокаТабличнойЧасти.Наименование = "ЕАЭС НДСР"; 
	...
	//	Отбор.Вставить("Регистратор", СтрокаТаблицы.Регистратор);
	//	Отбор.Вставить("ЗачетНДС", СтрокаТаблицы.ЗачетНДС);
	//	Отбор.Вставить("Владелец", "ЕАЭС НДСР");
	//	МассивСтрок = ТЗДетальныеЗаписи.НайтиСтроки(Отбор);
	//	
	//	Если МассивСтрок.Количество() = 0 Тогда
	//		НоваяСтрокаТаблицы = ТЗДетальныеЗаписи.Добавить();
	//		ЗаполнитьЗначенияСвойств(НоваяСтрокаТаблицы, СтрокаТаблицы);
	//		НоваяСтрокаТаблицы.Владелец = "ЕАЭС НДСР";
	//	КонецЕсли;
	//	
	//	Отбор.Владелец = "НДС ЕАЭС не разрешенный";
	//	МассивСтрок = ТЗДетальныеЗаписи.НайтиСтроки(Отбор);
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("НДСДляРаспределенияНеКР", ПараметрыРасчета);
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("НДСКРСебестоимость", ПараметрыРасчета);
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("НДСНеКРСебестоимость", ПараметрыРасчета);
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("Сумма050", ПараметрыРасчета);
	...
	Для Каждого СтрокаТаблицы Из СтруктураДанных.ТаблицаДетальныеЗаписи Цикл
		...
		Если МассивСтрок.Количество() = 0 Тогда
			НоваяСтрокаТаблицы = ТЗДетальныеЗаписи.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаТаблицы, СтрокаТаблицы);
			НоваяСтрокаТаблицы.Сумма = СтрокаТаблицы.СуммаБезНДС;
			НоваяСтрокаТаблицы.Владелец = "Строка 50";
		КонецЕсли;
		...
		Если СтрокаТаблицы.СуммаБезНДС > 0 Тогда
			СуммаРеализаций = СуммаРеализаций + СтрокаТаблицы.СуммаБезНДС;
		Иначе	
			СуммаВозвратов = СуммаВозвратов + СтрокаТаблицы.СуммаБезНДС;	
		КонецЕсли;	
	КонецЦикла;	
	...
	ПараметрыРасчета = Новый Структура;
	ПараметрыРасчета.Вставить("Таблица", ТаблицаВсейРеализации);
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("Сумма051", ПараметрыРасчета);
	...
	ПараметрыРасчета = Новый Структура;
	ПараметрыРасчета.Вставить("Таблица", ТаблицаВсейРеализации);
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("Сумма052", ПараметрыРасчета);		
	СтрокаТабличнойЧасти = СтрокиОтчета.Добавить();
	...
	ПараметрыРасчета.Вставить("Таблица", ТаблицаВсейРеализации);
	ПараметрыРасчета.Вставить("Период",  ДатаОкончания);
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("Сумма053", ПараметрыРасчета);
	...
	СтрокаТабличнойЧасти.Строка = "КфОП 
									|КфОПР";	
	СтрокаТабличнойЧасти.Содержание = НСтр("ru = 'Коэфициент освобожденных поставок
													|053
													|/
													|054 - (минус)
													|Авансы  - НДС на авансы – НСП на авансы
	...
	НоваяСтрокаТаблицы = Расшифровка.Добавить();
	НоваяСтрокаТаблицы.Строка = "КфОП
								|КфОПР";
	...
	ПараметрыРасчета.Вставить("Сумма051", 	Сумма051);
	ПараметрыРасчета.Вставить("Сумма050", 	Сумма050);
	КфНП = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("КфНП", ПараметрыРасчета).РезультатРасчета;		
	СтрокаТабличнойЧасти = СтрокиОтчета.Добавить();
	...
	ПараметрыРасчета = Новый Структура;
	ПараметрыРасчета.Вставить("Таблица", ТаблицаВсехПоступлений);
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("Сумма055", ПараметрыРасчета);
	...
	ПараметрыРасчета = Новый Структура;
	ПараметрыРасчета.Вставить("Таблица", ТаблицаВсехПоступлений);
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("Сумма056", ПараметрыРасчета);
	...
	ПараметрыРасчета = Новый Структура;
	ПараметрыРасчета.Вставить("Таблица", ТаблицаВсехПоступлений);
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("Сумма057", ПараметрыРасчета);		
	СтрокаТабличнойЧасти = СтрокиОтчета.Добавить();
	...
	ПараметрыРасчета.Вставить("СуммаНДСАвансы", 	СуммаНДСАвансы);
	ПараметрыРасчета.Вставить("СуммаНДСДоотгрузки", СуммаНДСДоотгрузки);
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("Сумма059", ПараметрыРасчета);
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("Сумма060", ПараметрыРасчета);
	...
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("Сумма061", ПараметрыРасчета);
	...
	ПараметрыРасчета.Вставить("Таблица", 						ТаблицаВсехПоступлений);
	ПараметрыРасчета.Вставить("НДСНеКРНеРазрешенныйКЗачету", 	НДСНеКРНеРазрешенныйКЗачету);
	СтруктураДанных = УчетНДСВызовСервера.РасчетСуммыОтчетаНДС("Сумма062", ПараметрыРасчета);		
	СтрокаТабличнойЧасти = СтрокиОтчета.Добавить();
...
Процедура РассчитатьАвансыДоотгрузки()	
	...
	Пока ВыборкаКонтрагент.Следующий() Цикл
		...
		Пока ВыборкаДоговор.Следующий() Цикл
			...
			// Заполнение отсортированнях данных в ТЗ на форме и высчитывание остатка аванса
			...
			ТекущаяОтгрузка 	= Макс(0,(Отгрузка - СуммаДоотгрузки));
...
Процедура ЗаполнитьЗакладкуНастроек()
	...
	Если ДанныеУчетнойПолитики.УказыватьПризнакЗачетаНДСПриПоступлении Тогда
		Элементы.УказыватьПризнкаЗачетаНДС.Заголовок = НСтр("ru = 'Указывать признак зачета НДС: Да'")
	Иначе
		Элементы.УказыватьПризнкаЗачетаНДС.Заголовок = НСтр("ru = 'Указывать признак зачета НДС: Нет'")
	КонецЕсли;															
	...
	Элементы.ЗначениеСтавкиНДССтандарт.Заголовок = СтрШаблон(НСтр("ru = 'Значение ставки НДС Стандарт на текущую дату: %1'"),
												УчетНДСВызовСервера.ПолучитьСтавкуНДС(ТекущаяДата, Справочники.СтавкиНДС.Стандарт));
															
	Элементы.ЗначениеСтавкиНСПТорговля.Заголовок = СтрШаблон(НСтр("ru = 'Значение ставки НСП Торговля: %1'"),
												УчетНДСВызовСервера.ПолучитьСтавкуНСП(ТекущаяДата, Организация, Справочники.СтавкиНСП.Торговля));
															
	Элементы.ЗначениеСтавкиНСППрочее.Заголовок = СтрШаблон(НСтр("ru = 'Значение ставки НСП Прочее : %1'"),
												УчетНДСВызовСервера.ПолучитьСтавкуНСП(ТекущаяДата, Организация, Справочники.СтавкиНСП.Прочее));	
КонецПроцедуры
