...
Процедура РассчитатьАвансыДоотгрузки()	
	...
	Запрос.Текст = 
		...
		|			ИЛИ ВременнаяТаблицаОстаткиАвансовНаНачалоМесяца.ДокументАванса ССЫЛКА Документ.ОперацияБух
		...
		|			ИЛИ ВременнаяТаблицаОстаткиНач.ДокументАванса ССЫЛКА Документ.ОперацияБух
	...
	ЗначениеСтавкиНСП = УчетНДСВызовСервера.ПолучитьСтавкуНСП(НачалоМесяца(Объект.Дата), Организация, ДанныеУчетнойПолитики.СтавкаНСПДляРасчетаНДСНаАвансы);
