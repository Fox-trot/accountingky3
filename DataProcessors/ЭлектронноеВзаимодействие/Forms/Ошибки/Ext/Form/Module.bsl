...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = ЭлектронноеВзаимодействиеОбработкаОшибокКлиент.ИмяСобытияИсправлениеОшибок() И ТипЗнч(Параметр) = Тип("Массив") Тогда
		
		Если Параметр.Количество() > 0 И ТипЗнч(Параметр[0]) = Тип("Строка") Тогда // переданы виды ошибок
			ВидыОшибок = Параметр;
...
Процедура ВывестиОшибки(ПараметрыВидовОшибок)
	...
	Для каждого Ошибка Из КонтекстОперации.Диагностика.Ошибки Цикл
		...
		Если СвойстваОшибокВидаДиагностики = Неопределено Тогда
			...
			СвойстваОшибок.Вставить("ЭлементОписаниеОшибки", Неопределено);
		...
		ЭлементыКолонки.ДекорацияКартинка.Картинка = БиблиотекаКартинок.КрасныйШарБЭД;
		ЭлементыКолонки.ДекорацияОписание.Заголовок = ПараметрыВидаОшибки.ЗаголовокПроблемы;
		ОбработанныеВидыОшибок[Ошибка.ВидОшибки].ЭлементОписаниеОшибки = ЭлементыКолонки.ДекорацияОписание;
	...
	Для каждого КлючИЗначение Из ОбработанныеВидыОшибок Цикл
		ЭлементОписаниеОшибки = КлючИЗначение.Значение.ЭлементОписаниеОшибки;
		Если КлючИЗначение.Значение.Количество > 1 Тогда
			ЭлементОписаниеОшибки.Заголовок = ЭлементОписаниеОшибки.Заголовок + СтрШаблон(" (%1)", КлючИЗначение.Значение.Количество);
		КонецЕсли;
	КонецЦикла;
...
Функция СоздатьКолонкуТаблицыОшибок(Родитель, ВидОшибки, ЭтоКолонкаОшибка)
	...
	ДекорацияОписание = Элементы.Добавить(ВидОшибки + "_ДекорацияОписание" + ПостфиксИмениЭлементов, Тип("ДекорацияФормы"), ГруппаЗаголовок);
...
Процедура Подключаемый_ОбработкаНавигационнойСсылки(Элемент, НавигационнаяСсылкаФорматированнойСтроки, СтандартнаяОбработка)
	...
	ЭлектронноеВзаимодействиеОбработкаОшибокКлиент.ВыполнитьОбработчикВидаОшибки(ВидОшибки, Обработчик, КонтекстОперации);
...
Процедура ОбновитьЭлементыФормыРешения(ВидОшибки) 
	...
	Если Ошибки.НайтиСтроки(Новый Структура("ВидОшибки", ВидОшибки)).Количество() = 0 Тогда
