...
Функция ВыгрузитьТестовыеДанные()
	...
	Если ЗначениеЗаполнено(БанковскийСчетОрганизации) Тогда
		...
		БанкОрганизацииОбъект.Родитель = Справочники.Банки.ПустаяСсылка();
	...
	Если ЗначениеЗаполнено(БанковскийСчетКонтрагента) Тогда
		...
		БанкКонтрагентаОбъект.Родитель = Справочники.Банки.ПустаяСсылка();
...
Процедура ЗаполнитьДанныеФормы(СтруктураДанных)
	...
	Если СтруктураДанных.Свойство("БанкОрганизации", БанкОрганизации)
		...
		БанкИзКлассификатора = Справочники.Банки.НайтиПоКоду(БанкОрганизации.Код);
	...
	Если СтруктураДанных.Свойство("БанкКонтрагента", БанкКонтрагента)
		...
		БанкИзКлассификатора = Справочники.Банки.НайтиПоКоду(БанкКонтрагента.Код);
		Если ЗначениеЗаполнено(БанкИзКлассификатора) Тогда
...
Процедура ПолучитьИдентификаторСессииБазоваяАутентификация()
	...
	БазоваяВерсияФорматаАсинхронногоОбмена = ОбменСБанкамиКлиентСервер.АктуальнаяВерсияФорматаАсинхронногоОбмена();
...
Процедура ЗагрузитьНастройкиОбменаНаСервере(ИдентификаторСессии, ЕстьОшибка)
	...
	Попытка
		НастройкаОбмена = ОбменСБанкамиСлужебный.СоздатьНастройкуОбменаИзФайла(
			АдресФайлаНастроекОбмена, Организация, Ложь, ДанныеСертификатов, Ложь);
	Исключение
		ЕстьОшибка = Истина;
		Возврат;
	КонецПопытки;
...
Процедура ОтправитьПлатежныйДокументНаСервере(ИдентификаторСессии, ЕстьОшибка)
	...
	ОбменСБанкамиСлужебныйВызовСервера.ОтправкаПакетовЭДО(МассивПакетов, СоотвНастроекОбменаИПараметровСертификатов, ТекстОшибки);
...
Процедура РезультатЗапросаВыпискиНаСервере(ИдентификаторСессии, ЕстьОшибка)
	...
	Если СтатусЗапроса <> Перечисления.СтатусыОбменСБанками.Подтвержден Тогда
		ТекстОшибки = НСтр("ru = 'Не выполнено условие: Document.extID = PayDocRu.ID'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
		ЕстьОшибка = Истина;
	КонецЕсли;
	...
	СтатусЗапроса = СтатусЗапросаВыписки();
	Если СтатусЗапроса <> Перечисления.СтатусыОбменСБанками.Обработан Тогда
		ТекстОшибки = НСтр("ru = 'Не выполнено условие: Statement.ExtIDStatementRequest = StatementRequest.ID'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки);
		ЕстьОшибка = Истина;
	КонецЕсли;
