...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Метаданные.ОпределяемыеТипы.ВладелецФайлов.Тип.СодержитТип(ТипВладельца) Тогда
		ПолноеИмяВладельца = Метаданные.НайтиПоТипу(ТипВладельца).ПолноеИмя();
		Если Метаданные.НайтиПоПолномуИмени(ПолноеИмяВладельца + "ПрисоединенныеФайлы") = Неопределено Тогда
	...
	ВозможностьСоздаватьВерсииФайлов = (ТипЗнч(Параметры.ВладелецФайла)= Тип("СправочникСсылка.Файлы"));
