...
Процедура ИзменитьТекстЗапросаДокументыСписок()
	Если ПоследующиеМесяцы Тогда
		СписокДокументы.ТекстЗапроса = СтрЗаменить(СписокДокументы.ТекстЗапроса, "Дата = &КонецПериода", "Дата >= &КонецПериода");
...
Процедура ЗаполнитьЗакладкуНастройки()
	...
	КонтрольСоответствияШтатномуРасписанию 	= Константы.ВвестиКонтрольЗапретаПоШтатномуРасписанию.Получить();
КонецПроцедуры
