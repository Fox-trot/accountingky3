...
Процедура ПроверитьЗаполнениеПараметров(ПараметрыПодбора, Отказ) Экспорт
	...
	Для каждого ЭлементСтруктуры Из СтруктураОбязательныхПараметров Цикл
		...
		// Контроль Счета Учета - только для документа ИнвентаризацияОС
		Если ЭлементСтруктуры.Ключ = "СчетУчета" 
			И НЕ ТипЗнч(ПараметрыПодбора.ДокументСсылка) = Тип("ДокументСсылка.ИнвентаризацияОС") Тогда 
			Продолжить;	
		КонецЕсли;
...
Функция СтруктураОбязательныхПараметров()
	...
	Возврат Новый Структура("ДокументСсылка, Дата, Организация, ЭтоДокументПоступления, КонтролироватьМестонахождение, УникальныйИдентификаторФормыВладельца, Подразделение, МОЛ, СчетУчета", 
