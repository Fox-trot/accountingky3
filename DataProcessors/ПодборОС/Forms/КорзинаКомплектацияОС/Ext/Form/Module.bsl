...
Процедура УстановитьТекстЗапросаСписокОС(ЭтоДокументПоступления)
	...
	Иначе 
		ТекстЗапроса = 
			...
			|ИЗ
			|	РегистрСведений.СоставОС.СрезПоследних(&Дата, ) КАК СоставОССрезПоследних
			|ГДЕ
			...
			|		ПО СправочникОсновныеСредства.Ссылка = ВременнаяТаблицаОСВКомплектах.ОсновноеСредствоВСоставеКомплекта
			|ГДЕ
			|	НЕ СправочникОсновныеСредства.ЭтоГруппа
			|	И НЕ СправочникОсновныеСредства.ЭтоКомплект
			...
			|	И ПараметрыУчетаОССрезПоследних.СчетУчета = &СчетУчета
			|	И ВременнаяТаблицаОСВКомплектах.ОсновноеСредствоВСоставеКомплекта ЕСТЬ NULL";
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Объект.СчетУчета) Тогда 
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПараметрыУчетаОССрезПоследних.СчетУчета = &СчетУчета", "ИСТИНА");		
	КонецЕсли;	
