...
// Процедура создает документы "Операцию Бух" с данными на дату свертки.
...
Процедура СформироватьДокументыВводаОстатковПоРегиструБухгалтерии(СтрокаСписка, Параметры)
	...
	ТекстЗапроса = 
		...
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.Остатки(&ДатаСвертки, , , Организация В (&ВыбранныеОрганизации)) КАК ХозрасчетныйОстатки
	...
	Если НЕ Параметры.ТолькоВыбранныеОрганизации Тогда 
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "Организация В (&ВыбранныеОрганизации)", "Истина"); 
	КонецЕсли;	
...
// Процедура создает документ "Операция Бух". Если у регистра есть
...
// Функция заполняет шапку документа "Операция Бух"
//
...
// Процедура удаляет документы "Операция Бух", которые содержат
	...
	Иначе
		...
		Для Каждого Орг Из ПараметрыОбработки.Организации Цикл
			...
			Если Не ДатаЗапретаДляВсехПользователей Тогда 
				...
				НоваяЗапись.Раздел = ДатыЗапретаИзмененияПереопределяемыйБП.РазделБухгалтерскийУчет();
				НоваяЗапись.Пользователь = Перечисления.ВидыНазначенияДатЗапрета.ДляВсехПользователей;
				НоваяЗапись.ДатаЗапрета = ДатаЗапрета;
				ОбновитьДатыЗапрета = Истина;
			КонецЕсли;
