...
Процедура ЗаполнитьВВидеПолотноИерархияНоменклатуры(НастройкиПрайсЛиста, ТабличныйДокумент, ДеревоИерархии)
	...
	Если НастройкиПрайсЛиста.ВыводитьКонтактнуюИнформацию Тогда
		...
		ЗначенияПараметров.Вставить("Адрес", 		НСтр("ru = 'Адрес: '") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ФактАдресОрганизации, ТекущаяДатаСеанса()));
		ЗначенияПараметров.Вставить("Телефон", 		НСтр("ru = 'Телефон, факс: '") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ТелефонОрганизации,ТекущаяДатаСеанса()) + ", " + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.ФаксОрганизации, ТекущаяДатаСеанса()));
		ЗначенияПараметров.Вставить("Email", 		НСтр("ru = 'E-mail: '") + УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(НастройкиПрайсЛиста.Организация, Справочники.ВидыКонтактнойИнформации.EmailОрганизации, ТекущаяДатаСеанса()));
...
Процедура ТранспонированиеТаблиц(НастройкиПрайсЛиста, ДеревоИерархии, ТаблицаЦен) 
	...
	Для каждого Строка Из ТаблицаЦен Цикл
		...
		Если РезультатПоиска.Количество() > 0 Тогда
			...
			РезультатПоиска[0][ИменаКолонок[Строка.ТипЦен.Ссылка]] = БухгалтерскийУчетСервер.ОкруглитьЦену(Строка.Цена, Строка.ПорядокОкругления, Строка.ОкруглятьВБольшуюСторону);
