...
Функция TestConnection(ИмяПланаОбмена, КодУзла, Результат)
	...
	Если ОбменДаннымиСервер.УзелПланаОбменаПоКоду(ИмяПланаОбмена, КодУзла) = Неопределено Тогда
		...
		Результат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			НСтр("ru = 'При проверке подключения к ""%1"" возникла ошибка.
			|Не найдена настройка синхронизации данных ""%2"" с идентификатором ""%3"".
			|
			|Убедитесь, что
			|  - настройка не была удалена в ""%1"";
			|  - указан правильный адрес для подключения к ""%1"" через Интернет.'"),
