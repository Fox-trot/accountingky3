...
Процедура УправлениеБлокомКурсыВалют(Форма)
	...
	ВГраницаИндекса = БлокиИнформационнойПанелиКлиентСервер.КоличествоВалютКурсыВалют() - 1;
	Для Индекс = 0 По ВГраницаИндекса Цикл
		ОтклонениеКурсы = БлокиИнформационнойПанелиКлиентСервер.СвойствоОтклонениеКурсыВалют(Индекс);
...
Процедура ОбновитьКурсыВалют()
	...
	ВГраницаИндекса = БлокиИнформационнойПанелиКлиентСервер.КоличествоВалютКурсыВалют() - 1;
	Для Индекс = 0 По ВГраницаИндекса Цикл
		ИмяРеквизита = БлокиИнформационнойПанелиКлиентСервер.СвойствоВалютаКурсыВалют(Индекс);
...
Процедура ОбновитьСостояниеДоступаКСервисам()
	...
	ДоступКСервисамПолучен = РаботаСКонтрагентами.ПроверитьДоступКСервисам();
