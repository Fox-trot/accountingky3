...
Процедура ВыполнитьСинхронизацию(Команда)
	...
	Иначе
		...
		ДополнительныеПараметры.Вставить("ИнтерактивнаяОтправка",
			ТекущиеДанные.ИнтерактивнаяОтправка И (Не МодельСервиса Или ТекущиеДанные.ПолученоСообщениеДляСопоставленияДанных));
		
		ДополнительныеПараметры.Вставить("ОтправитьДанные", Не ТекущиеДанные.ЗапускОбменаИзКорреспондента
			И Не ТекущиеДанные.ПолученоСообщениеДляСопоставленияДанных);
			
		ДополнительныеПараметры.Вставить("НастройкаРасписания", Ложь);
...
Процедура УдалитьНастройкуСинхронизации(Команда)
	...
	Если МодельСервиса Тогда
		...
		ИначеЕсли ТекущиеДанные.ЭтоОбменСПриложениемВСервисе Тогда
			...
			ОповещениеОЗавершении = Новый ОписаниеОповещения("УдалениеНастройкиСинхронизацииВМоделиСервисаЗавершение", ЭтотОбъект);
			
			ОткрытьФорму(ИмяФормыОтключенияСинхронизации,
				ПараметрыФормы, ЭтотОбъект, , , , ОповещениеОЗавершении, РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
...
Процедура НачальнаяВыгрузкаДанных(Команда)
	...
	Если Не НастройкаСинхронизацииЗавершена(ТекущиеДанные.УзелИнформационнойБазы) Тогда
		...
		ПоказатьВопрос(ОповещениеЗавершения,
			...
			|Открыть форму помощника настройки?'"),
			РежимДиалогаВопрос.ДаНет);
	ИначеЕсли Не ТекущиеДанные.ИнтерактивнаяОтправка Тогда
...
&НаКлиенте
Процедура УдалениеНастройкиСинхронизацииВМоделиСервисаЗавершение(РезультатЗакрытия, ДополнительныеПараметры) Экспорт
...
Процедура ПриНачалеОбновленияДанныхМонитора(Интерактивно = Истина)
	...
	Если ПродолжитьОжидание Тогда
		...
		Если Интерактивно Тогда
			ПодключитьОбработчикОжидания("ПриОжиданииОбновленияДанныхМонитораИнтерактивно",
...
&НаКлиенте
Процедура ПриОжиданииОбновленияДанныхМонитораИнтерактивно()
...
Процедура ПриОжиданииОбновленияДанныхМонитораВФоне()
	...
	Иначе
		ПараметрыОбработчикаОжиданияПолученияСпискаПриложений = Неопределено;
		ПриЗавершенииОбновленияДанныхМонитора(Ложь);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ПриЗавершенииОбновленияДанныхМонитора(Интерактивно = Истина)
...
Процедура СписокПриложенийПриАктивизацииСтроки(Элемент)
	...
	Элементы.СписокПриложенийНачальнаяВыгрузкаДанных.Доступность = ТекущиеДанные.ИнтерактивнаяОтправка
		И Не ТекущиеДанные.ЗапускОбменаИзКорреспондента
		И Не ТекущиеДанные.ПолученоСообщениеДляСопоставленияДанных;
