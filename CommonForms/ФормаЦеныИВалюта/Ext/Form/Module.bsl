...
Процедура ПолучитьЗначенияПараметровФормы()
	...
	// Ставка НСП (товары).
	Если Параметры.Свойство("СтавкаНСП") Тогда
		
		СтавкаНСП = Параметры.СтавкаНСП;
		СтавкаНСППриОткрытии = Параметры.СтавкаНСП;
		СтавкаНСПЕстьРеквизит = Истина;
		
		КлючПоложения = КлючПоложения + "СтавкаНСП";
		
	Иначе
		
		Элементы.СтавкаНСП.Видимость = Ложь;
		СтавкаНСПЕстьРеквизит = Ложь;
		
	КонецЕсли;
	
	// Ставка НСП (услуги).
	Если Параметры.Свойство("СтавкаНСПУслуги") Тогда
		
		СтавкаНСПУслуги = Параметры.СтавкаНСПУслуги;
		СтавкаНСПУслугиПриОткрытии = Параметры.СтавкаНСПУслуги;
		СтавкаНСПУслугиЕстьРеквизит = Истина;
		
		КлючПоложения = КлючПоложения + "СтавкаНСПУслуги";
		
	Иначе
		
		Элементы.СтавкаНСПУслуги.Видимость = Ложь;
		СтавкаНСПУслугиЕстьРеквизит = Ложь;
