...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ОбновлятьСПортала1СИТС Тогда
		...
		Если Не ЭтоМаксимальнаяВерсия Тогда
			...
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстОшибки, , "Объект.ОбновлятьСПортала1СИТС");
		...
		Если Не Отказ Тогда
			...
			Запрос.Текст = 
			    ...
			    |ГДЕ
			    |	ВнешниеКомпоненты.Идентификатор = &Идентификатор
			    |	И ВнешниеКомпоненты.ОбновлятьСПортала1СИТС
				|	И ВнешниеКомпоненты.Ссылка <> &Ссылка";
