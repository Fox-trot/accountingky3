...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ИННУказанНеВерно ИЛИ ЕстьДублиИНН Тогда
		ТекстСообщения = НСтр("ru = 'ИНН физ лица либо указан не верно, либо есть дубли. Запись отменена.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.ИНН",,Отказ);		
	КонецЕсли;	
КонецПроцедуры
...
Процедура Подключаемый_ВыполнитьКомандуНаСервере(Контекст, Результат) Экспорт
	ПодключаемыеКоманды.ВыполнитьКоманду(ЭтотОбъект, Контекст, ФизЛицо, Результат);
КонецПроцедуры
