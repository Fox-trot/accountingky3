...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе 
		ДатаУвольнения = ДатаУвольненияСотрудника(ТекущаяДата(),ФизическоеЛицоСсылка);		
	КонецЕсли;
...
&НаКлиенте
Процедура СведенияОбИнвалидностиФизическихЛицНаборЗаписейПриИзменении(Элемент)
	Модифицированность = Истина;
КонецПроцедуры
...
Процедура ФизлицоДатаРожденияПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ФизЛицо.ДатаРождения) Тогда 
		ГодМолодости = Год(ТекущаяДата()) - 15;
		ГодСтарости  = Год(ТекущаяДата()) - 150;
		
		Если Год(ФизЛицо.ДатаРождения) > ГодМолодости Или Год(ФизЛицо.ДатаРождения) < ГодСтарости Тогда 
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю("Дата рождения выходит за границы допустимого диапозона",ФизическоеЛицоСсылка,"ДатаРождения","ФизЛицо");
...
#Область ОбработчикиСобытийРедактированиеГражданства
...
Процедура ГражданствоПоУмолчанию()
	...
	ГражданствоФизическихЛиц.Период = ТекущаяДата(); 
	
	ДокументыФизическихЛиц.ВидДокумента = Справочники.ВидыДокументовФизическихЛиц.Паспорт;
	ДокументыФизическихЛиц.ДатаВыдачи = ТекущаяДата();
	ДокументыФизическихЛиц.СрокДействия = ТекущаяДата();
...
Процедура ДокументыФизическихЛицДатаВыдачиПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(ДатаВыдачи) Тогда 	                                                   
		Если Год(ДатаВыдачи) < Год(ТекущаяДата()) - 100 Тогда 
...
Процедура ДокументыФизическихЛицСрокДействияПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(СрокДействия) Тогда 	                                                   
		Если Год(СрокДействия) > Год(ТекущаяДата()) + 10 или  Год(СрокДействия) < Год(ТекущаяДата()) - 100 Тогда 
...
&НаКлиенте
Процедура КартСчетаСотрудниковНаборЗаписейПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
...
&НаКлиенте
Процедура СведенияОбИнвалидностиФизическихЛицНаборЗаписейПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	Элемент.ТекущиеДанные.ФизЛицо = ФизическоеЛицоСсылка;
...
Процедура СостоянияВБракеФизическихЛицСостояниеВБракеПриИзмененииНаСервере()
	Если СостоянияВБракеФизическихЛицПрежняя.СостояниеВБраке <> СостоянияВБракеФизическихЛиц.СостояниеВБраке Тогда
		...
		СостоянияВБракеФизическихЛиц.Период = ТекущаяДата();
	КонецЕсли;
КонецПроцедуры // ()
...
Процедура Подключаемый_ОбновитьКонтактнуюИнформацию(Результат)
	Если ОбщегоНазначения.ПодсистемаСуществует("СтандартныеПодсистемы.КонтактнаяИнформация") Тогда
		...
		МодульУправлениеКонтактнойИнформацией.ОбновитьКонтактнуюИнформацию(ЭтотОбъект, ФизЛицо, Результат);
	КонецЕсли;
КонецПроцедуры
// Конец СтандартныеПодсистемы.КонтактнаяИнформация
...
Процедура Подключаемый_РедактироватьСоставСвойств(Команда)
	...
	УправлениеСвойствамиКлиент.РедактироватьСоставСвойств(ЭтотОбъект, ФизЛицо.Ссылка);
