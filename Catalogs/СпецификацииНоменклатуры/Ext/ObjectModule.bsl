...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	// Контроль зацикливания уровней.
	ТаблицаЦиклов = Справочники.СпецификацииНоменклатуры.ЦиклыСпецификаций(
		Новый Структура("Ссылка, Владелец, ИсходныеКомплектующие", Ссылка, Владелец, ИсходныеКомплектующие.Выгрузить()));
	
	Если НЕ ТаблицаЦиклов.Количество() = 0 Тогда 
		ТекстСообщения = НСтр("ru = 'Найдено зацикливание спецификаций.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
		
		Для Каждого СтрокаТаблицы Из ТаблицаЦиклов Цикл 
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Предок: %1; Потомок: %2; Спецификация: %3'"),
				СтрокаТаблицы.Предок, СтрокаТаблицы.Потомок, СтрокаТаблицы.Спецификация);
