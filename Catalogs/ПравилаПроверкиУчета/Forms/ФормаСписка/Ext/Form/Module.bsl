...
Процедура ВыполнитьПроверку(Команда)
	...
	Если Проверки.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Необходимо выбрать в списке одну или несколько включенных проверок для выполнения.'");
	КонецЕсли;
...
Функция ВключенныеПроверки()
	...
	Для Каждого Проверка Из Элементы.Список.ВыделенныеСтроки Цикл
		...
		Если ДанныеПроверки <> Неопределено И Не ДанныеПроверки.ЭтоГруппа И ДанныеПроверки.Использование Тогда
