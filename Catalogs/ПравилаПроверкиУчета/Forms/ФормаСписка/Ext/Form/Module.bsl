...
Процедура ВыполнитьПроверку(Команда)
	...
	Проверки = ПроверкиБезГрупп();
	Если Проверки.Количество() = 0 Тогда
		ВызватьИсключение НСтр("ru = 'Необходимо выбрать в списке одну или несколько проверок для выполнения.'");
	КонецЕсли;
