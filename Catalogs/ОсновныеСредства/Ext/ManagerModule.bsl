...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	ТабличныйДокумент.КлючПараметровПечати = "КарточкаОС";
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		Если ИмяМакета = "КарточкаОС" Тогда
			ТабличныйДокумент.ИмяПараметровПечати = "КарточкаОС";
			...
			|	ПервоначальныеСведенияОбОсновныхСредствахОрганизацийСрезПоследних.ОсновноеСредство.НомерПаспорта,
			|	ПервоначальныеСведенияОбОсновныхСредствахОрганизацийСрезПоследних.ОсновноеСредство.ЗаводскойНомер,
			...
			Пока Выборка2.Следующий() Цикл
				Если Выборка2.Состояние   = Перечисления.ВидыСостоянийОС.ПринятоКУчету Тогда
					ОбластьМакета.Параметры.ДатаПриобретения = Формат(Выборка2.ДатаСостояния,"ДФ=dd.MM.yyyy");//именно поступления
				ИначеЕсли Выборка2.Состояние   = Перечисления.ВидыСостоянийОС.СнятоСУчета Тогда
					ОбластьМакета.Параметры.ДатаВыбытия = Формат(Выборка2.ДатаСостояния,"ДФ=dd.MM.yyyy");//именно выбытия
				КонецЕсли;
			КонецЦикла;
			...
			ОбластьМакета.Параметры.ДатаКапитализации 	= Формат(Выборка.ДатаКапитализации,"ДФ=dd.MM.yyyy");
			...
			ВыборкаЗаписей = РегистрыСведений.ПараметрыУчетаОС.ПолучитьПоследнее(ДатаСведений,Новый Структура("ОсновноеСредство",Ссылка));
			СчетНачисленияАмортизацииБУ = ?(ВыборкаЗаписей.Количество() > 0, ВыборкаЗаписей.СчетУчета.ПарныйСчет,0);
			//считаем
			ВыборкаЗаписей = РегистрыБухгалтерии.Хозрасчетный.Остатки(ДатаСведений,ВидСубконтоОС,Новый Структура("Субконто1,Организация,Счет",Ссылка,ТекОрганизация,СчетНачисленияАмортизацииБУ));
			//выводим
