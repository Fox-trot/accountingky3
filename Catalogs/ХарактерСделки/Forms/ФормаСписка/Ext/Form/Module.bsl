...
Процедура ЗаполнитьНаСервере()
	...
	КС4	 	= Новый КвалификаторыСтроки(4);
	КС150 	= Новый КвалификаторыСтроки(150);

	Массив = Новый Массив;
	Массив.Добавить(Тип("Строка"));
	ОписаниеТиповС4 = Новый ОписаниеТипов(Массив, , КС4);
	Массив = Новый Массив;
	Массив.Добавить(Тип("Строка"));
	ОписаниеТиповС150 = Новый ОписаниеТипов(Массив, , КС150);
	
	ТаблицаЗначений.Колонки.Добавить("Код", ОписаниеТиповС4);
	...
	Для Счетчик = 5 По ВысотаТаблицы Цикл
		...
		СтрокаКлассификатора.Код = Макет.Область(Счетчик, 2, ВысотаТаблицы, 2).Текст;
	...
	Запрос.Текст = 
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаКлассификатор.Код,
		|	ВременнаяТаблицаКлассификатор.Наименование
		|ИЗ
		|	ВременнаяТаблицаКлассификатор КАК ВременнаяТаблицаКлассификатор
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ХарактерСделки КАК ХарактерСделки
		|		ПО ВременнаяТаблицаКлассификатор.Код = ХарактерСделки.Код
		|ГДЕ
		|	ХарактерСделки.Код ЕСТЬ NULL";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		НовыйЭлемент = Справочники.ХарактерСделки.СоздатьЭлемент();
