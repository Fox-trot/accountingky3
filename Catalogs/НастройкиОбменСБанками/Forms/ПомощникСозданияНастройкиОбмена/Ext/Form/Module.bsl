...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Банк) И УпрощенныйРежимНастройки Тогда
		...
		Если ЗначениеЗаполнено(БанкиДляОбмена) И БанкиДляОбмена.Количество() > 0 Тогда
			...
			ОбщегоНазначенияКлиентСервер.УстановитьСвойствоЭлементаФормы(Элементы, "Банк", "РежимВыбораИзСписка", Истина);
			...
			Для Каждого СтрокаТаблицы Из БанкиДляОбмена Цикл
				...
				Если ЕстьОрганизация Тогда
					Если Элементы.Организация.СписокВыбора.НайтиПоЗначению(СтрокаТаблицы.Организация) = Неопределено Тогда
						Элементы.Организация.СписокВыбора.Добавить(СтрокаТаблицы.Организация);
					КонецЕсли;
				Иначе
				...
				Если ЕстьБанк Тогда
					Если Элементы.Банк.СписокВыбора.НайтиПоЗначению(СтрокаТаблицы.Банк) = Неопределено Тогда
						Элементы.Банк.СписокВыбора.Добавить(СтрокаТаблицы.Банк);
					КонецЕсли;
				Иначе
			...
			Если ЕстьОрганизация И Элементы.Организация.СписокВыбора.Количество() = 1 Тогда
				Организация = Элементы.Организация.СписокВыбора[0].Значение;
			КонецЕсли;
			
			Если ЕстьБанк И Элементы.Банк.СписокВыбора.Количество() = 1 Тогда
				Банк = Элементы.Банк.СписокВыбора[0].Значение;
			КонецЕсли;
...
Процедура ПослеСкачиванияВнешнейКомпонентыСбербанка(Результат, ДополнительныеПараметры) Экспорт
	...
	ИдетУстановкаРасширенияСбербанк = Истина;
	...
	#Если ВебКлиент Тогда
		ПодключитьОбработчикОжидания("ПроверитьУстановкуРасширенияСбербанк", 60, Истина);
	#КонецЕсли 
	
КонецПроцедуры

&НаКлиенте
Процедура ПроверитьУстановкуРасширенияСбербанк()
	
	Если ИдетУстановкаРасширенияСбербанк Тогда
		
		ЗаголовокПредупреждения = НСтр("ru = 'Кажется, что-то пошло не так.'");
		ТекстПредупреждения = НСтр("ru = 'Процесс загрузки параметров обмена слишком затянулся.
			|Возможно, не удается установить расширение для браузера.
			|Попробуйте запустить браузер под Администратором
			|и повторить подключение к сервису 1С:ДиректБанк.'");
...
Процедура ПослеБазовойАутентификацииСбербанк(Результат, ДополнительныеПараметры) Экспорт
	...
	ИдетУстановкаРасширенияСбербанк = Ложь;
	Если Результат.Успех Тогда
