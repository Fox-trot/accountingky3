...
Функция ПолучитьНастройкиОбмена(ПараметрыПолученияНастроек)
	...
	//ИмяФайлаApi = ПолучитьимяВременногоФайла("txt");
	//Файл = Новый ТекстовыйДокумент();
	//Файл.ДобавитьСтроку(ApiKey);
	//Файл.Записать(ИмяФайлаApi);
	//ApiKey = Новый ДвоичныеДанные(ИмяФайлаApi);
	...
	Результат = ОбменСБанкамиСлужебный.ОтправитьPOSTЗапрос(ПараметрыПолученияНастроек.АдресСервера, "v1/ms-api-1c/GetSettings", Заголовки, ТелоЗапроса);
	// Конец БПКР
