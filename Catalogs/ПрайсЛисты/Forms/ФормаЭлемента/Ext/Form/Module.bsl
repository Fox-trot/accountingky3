...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ ЗначениеЗаполнено(Объект.ТипЦены) И 
		(Объект.ПечатьПрайсЛиста = КэшЗначений.Диафильм ИЛИ Объект.ПечатьПрайсЛиста = КэшЗначений.ДвеКолонки) Тогда
		
		ТекстСообщения = НСтр("ru = 'Необходимо заполнить поле ""Тип цены"".'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ТипЦены",,Отказ)	
