...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта = ВариантыОтчетов.ОписаниеВарианта(Настройки, НастройкиОтчета, "АнализСчета");
	НастройкиВарианта.Описание = НСтр("ru = 'Анализ счета.'");
КонецПроцедуры
...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ПараметрыОтчета.БезИерархии Тогда 
		ЗначениеОтбора = БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(Структура.Отбор, "Счет.ЗапретитьИспользоватьВПроводках", Ложь);		
		ЗначениеОтбора.Применение = ТипПримененияОтбораКомпоновкиДанных.Элементы;
	ИначеЕсли Не ПараметрыОтчета.ПоСубсчетам Тогда
	...
	Если ПараметрыОтчета.БезИерархии Тогда 
		ЗначениеОтбора = БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(Структура.Отбор, "КорСчет.ЗапретитьИспользоватьВПроводках", Ложь);
		ЗначениеОтбора.Применение = ТипПримененияОтбораКомпоновкиДанных.Элементы;
	ИначеЕсли Не ПараметрыОтчета.ПоСубсчетамКорСчетов Тогда
...
Функция СохраняемыеОпции() Экспорт
	...
	КоллекцияНастроек.Добавить("БезИерархии");
...
Процедура ДобавитьУсловиеПоВыбраннымРесурсам(ПараметрыОтчета, Схема)
	...
	Для Каждого ИмяПоказателя Из ПолучитьНаборПоказателей() Цикл
		...
		Если ПараметрыОтчета["Показатель" + ИмяПоказателя] Тогда
			...
			ИначеЕсли ИмяПоказателя = "ВалютнаяСумма" ИЛИ ИмяПоказателя = "Количество" Тогда			
				Продолжить;
			КонецЕсли;
	...
	Если ТекстУсловияДт <> "" Тогда
		...
		ТекстЗапроса = СхемаЭталон.НаборыДанных.ОсновнойНаборДанных.Запрос;
		...
		Схема.НаборыДанных.ОсновнойНаборДанных.Запрос = ТекстЗапроса;
	КонецЕсли;
