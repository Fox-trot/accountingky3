...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта.Описание = НСтр("ru = 'Оборотно сальдовая ведомость'");
КонецПроцедуры
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	ЗаполнитьМакетыРесурсовПодвалаОтчета(ТелоМакета, МакетыРесурсовПодвалаОтчета, СоответствиеМакетовКолонкамОтчета);
	ЗаполнитьМакетыРесурсовГруппировки(ТелоМакета.Строки, МакетыРесурсовСчета, СоответствиеМакетовКолонкамОтчета, "Счет");
	...
	// Макеты группировки и ресурсов группировки по валюте будем использовать для определения принадлежности элемента Макет
	// макета компоновки.
...
Процедура ПередВыводомЭлементаРезультата(ПараметрыОтчета, МакетКомпоновки, ДанныеРасшифровки, ЭлементРезультата, Отказ = Ложь) Экспорт
	...
	Если ЭлементРезультата.ЗначенияПараметров.Количество() > 0 
		...
		ЗапретитьИспользоватьВПроводках = ПараметрыОтчета.ВременныеДанныеОтчета.ТекущийСчет.ЗапретитьИспользоватьВПроводках;
		...
		Если ПараметрыОтчета.ВременныеДанныеОтчета.МакетыРесурсовСчета.Найти(ЭлементРезультата.Макет) <> Неопределено 
			...
			Если ПараметрыОтчета.ПоПодразделам
				И Не ЗначениеЗаполнено(ПараметрыОтчета.ВременныеДанныеОтчета.ТекущийСчет.Родитель) 
				И МакетКомпоновки.Макеты[ЭлементРезультата.Макет].Макет.Количество() > 0
				Или 
				НЕ ПараметрыОтчета.ПоПодразделам
				...
				Для ИндексЭлемента = 0 По КоличествоПоказателей - 1 Цикл
					...
					Для Каждого Ячейка Из СтрокаМакета.Ячейки Цикл
						...
						Для Каждого Элемент Из Ячейка.Элементы Цикл
							...
							Если ПараметрРезультата <> Неопределено И ПараметрРезультата.Значение <> Null Тогда
		...
		Если Не ПараметрыОтчета.ПоПодразделам Тогда 
			Если ЗначениеЗаполнено(ПараметрыОтчета.ВременныеДанныеОтчета.ТекущийСчет) Тогда
				Отказ = ЗапретитьИспользоватьВПроводках;
				Если Не ЗапретитьИспользоватьВПроводках Тогда  
					// Убираем группировку строк и выравнивание.
					Если ТипЗнч(ЭлементРезультата.ЗначенияПараметров.П1.Значение) = Тип("Строка") Тогда 
						ЭлементРезультата.ЗначенияПараметров.П3.Значение = 0;
						// Если есть вывод дополнительных полей,
						// то изменяем значение параметра 5
						Если ЭлементРезультата.ЗначенияПараметров.Найти("П5") = Неопределено Тогда 
							ЭлементРезультата.ЗначенияПараметров.П4.Значение = 0;
						Иначе 
							ЭлементРезультата.ЗначенияПараметров.П5.Значение = 0;
						КонецЕсли;
						
						// Убримаем группировку при условии что дополнительные поля выводятся в отдельных колонках.
						Если НЕ ЭлементРезультата.ЗначенияПараметров.Найти("П6") = Неопределено Тогда 
							ЭлементРезультата.ЗначенияПараметров.П6.Значение = 0;
						КонецЕсли;
					КонецЕсли;	
				КонецЕсли;	
			КонецЕсли;
...
Процедура ДоработатьНаборДанных_ПоСубконто(ПараметрыОтчета, ПараметрыАлгоритма)
	...
	Для Каждого ОписаниеПодзапроса Из ЗапросыДляФормирования Цикл
		...
		Если ОписаниеПодзапроса.ПоСубсчетам ИЛИ ПараметрыОтчета.ПоСубсчетам Тогда
			...
			Для Каждого Счет Из БухгалтерскийУчетСервер.СформироватьМассивСубсчетов(СчетаПодзапроса) Цикл
		...
		СчетаГруппировки = БухгалтерскийУчетСервер.СформироватьМассивСубсчетов(СчетаПодзапроса);
...
Процедура ДоработатьНаборДанных_ПоСубконтоРазвернутое(ПараметрыОтчета, ПараметрыАлгоритма)
	...
	Для Каждого ОписаниеПодзапроса Из ЗапросыДляФормирования Цикл
		...
		СчетаРазвернутоеСальдо = БухгалтерскийУчетСервер.СформироватьМассивСубсчетов(СчетаПодзапроса);
