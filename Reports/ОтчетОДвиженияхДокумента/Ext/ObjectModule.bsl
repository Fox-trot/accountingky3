...
Процедура ПодготовкаДанныхПоРегистрамРасчета(ДвижениеДокумента, ОсновнаяИнформация, ИмяРегистра, ВидРегистра, ЛокализацияВидРегистра, ПредставлениеРегистра, ДополнительнаяНумерация = Ложь)
	...
	ИнформацияПоРегистру.Вставить("Ресурсы",                            Новый Структура);
	ИнформацияПоРегистру.Вставить("Реквизиты",                          Новый Структура);
	ИнформацияПоРегистру.Вставить("СпецифическиеДанныеРегистраРасчета", Новый Структура);
	ИнформацияПоРегистру.Вставить("СтандартныеРеквизиты",               Новый Структура);
	ИнформацияПоРегистру.Вставить("ИмяРегистра",                        ИмяРегистра);
	ИнформацияПоРегистру.Вставить("ВидРегистра",                        ВидРегистра);
	ИнформацияПоРегистру.Вставить("ЛокализацияВидРегистра",             ЛокализацияВидРегистра);
	...
	ИсключенияДляСтандартныхРеквизитов =
	"ПериодРегистрации, Регистратор, НомерСтроки, ВидРасчета, ПериодДействия,
	|ПериодДействияНачало, ПериодДействияКонец, БазовыйПериодНачало, БазовыйПериодКонец, Сторно";
	ИсключенияДляСпецифическихДанных   = "Активность, НомерСтроки, Регистратор, Период";
	
	Если Не ДвижениеДокумента.БазовыйПериод Тогда
		ИсключенияДляСпецифическихДанных = ИсключенияДляСпецифическихДанных + ", БазовыйПериодНачало, БазовыйПериодКонец";
	КонецЕсли;
	
	Если Не ДвижениеДокумента.ПериодДействия Тогда
		ИсключенияДляСпецифическихДанных = ИсключенияДляСпецифическихДанных + ", ПериодДействияНачало, ПериодДействияКонец";
	КонецЕсли;
	
	ИнформацияПоРегистру.СтандартныеРеквизиты               =
		СформироватьСтруктуруПолей(ДвижениеДокумента.СтандартныеРеквизиты, ИсключенияДляСтандартныхРеквизитов);
	ИнформацияПоРегистру.СпецифическиеДанныеРегистраРасчета =
		СформироватьСтруктуруПолей(ДвижениеДокумента.СтандартныеРеквизиты, ИсключенияДляСпецифическихДанных);
	ИнформацияПоРегистру.Измерения                          = СформироватьСтруктуруПолей(ДвижениеДокумента.Измерения);
	ИнформацияПоРегистру.Ресурсы                            = СформироватьСтруктуруПолей(ДвижениеДокумента.Ресурсы);
	ИнформацияПоРегистру.Реквизиты                          = СформироватьСтруктуруПолей(ДвижениеДокумента.Реквизиты);
	...
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.Ресурсы);
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.Реквизиты);
	ДобавитьПоля(ПоляВыбора, ИнформацияПоРегистру.СпецифическиеДанныеРегистраРасчета);
	...
	Если ДополнительнаяНумерация Тогда
		...
		МаксимумПолей = Макс(ИнформацияПоРегистру.СтандартныеРеквизиты.Количество(),
			...
			ИнформацияПоРегистру.Ресурсы.Количество(),
			ИнформацияПоРегистру.Реквизиты.Количество(),
			ИнформацияПоРегистру.СпецифическиеДанныеРегистраРасчета.Количество());
...
Процедура НастройкаСтруктурыРегистровРасчета_Горизонталь(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	СтруктураСтандартныхРеквизитов = ЭлементДанных.СтандартныеРеквизиты;
	СтруктураСпецифическихДанных   = ЭлементДанных.СпецифическиеДанныеРегистраРасчета;
	СтруктураИзмерений             = ЭлементДанных.Измерения;
	СтруктураРесурсов              = ЭлементДанных.Ресурсы;
	СтруктураРеквизитов            = ЭлементДанных.Реквизиты;
	...
	Если СтруктураСтандартныхРеквизитов.Количество() > 0 Тогда
		...
		РазместитьГруппуПолейСКД(СтруктураСтандартныхРеквизитов, НаборДанныхКД, ГруппировкаДетальныеЗаписи,
			"СтандартныеРеквизиты", ПользовательскийЗаголовок, РасположениеПоляКомпоновкиДанных.Вертикально);
	
	
	КонецЕсли;
	
	Если СтруктураСпецифическихДанных.Количество() > 0 Тогда
		
		ПользовательскийЗаголовок = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Данные расчета (%1)'"),
			ПредставлениеРегистра);
	
		РазместитьГруппуПолейСКД(СтруктураСпецифическихДанных, НаборДанныхКД, ГруппировкаДетальныеЗаписи, "ДанныеРасчета",
			ПользовательскийЗаголовок, , , НСтр("ru = 'Данные расчета'"));
...
Процедура НастройкаСтруктурыРегистровРасчета_Вертикаль(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	Если СтруктураСпецифическихДанных.Количество() > 0 Тогда
		...
		РазместитьГруппуПолейСКД(СтруктураСпецифическихДанных, НаборДанныхКД, ГруппировкаДетальныеЗаписи, "ДанныеРасчета", ,
			РасположениеПоляКомпоновкиДанных.Вертикально, , НСтр("ru = 'Данные расчета'"));
