...
Процедура ДобавитьДоступныеПоля(ТаблицаПоказателей, ПоказателиИсточник, УсловноеОформление, Приоритет)
	...
	Для каждого Показатель Из ПоказателиИсточник Цикл
		...
		Если ТаблицаПоказателей.Найти(ПоказательИмя, "Поле") <> Неопределено Тогда
			Продолжить;
		КонецЕсли;
		Если ПоказательИмя = "Валюта" Тогда
			Продолжить;
		КонецЕсли;
...
Функция ПодготовитьПараметрыОтчета()
	...
	ПараметрыОтчета.Вставить("ТипыДокументов",            ТипыДокументов.ВыгрузитьЗначения());
