...
Процедура НастройкиДинамическогоСписка(Форма, ИмяТаблицыФормы = "Список", Идентификатор = "") Экспорт
	...
	Если Идентификатор <> "" Тогда
		СтрокаТаблицы = ТаблицаКоманды.Найти(Идентификатор, "Идентификатор");
	Иначе
	...
	Если СтрокаТаблицы <> Неопределено Тогда
		...
		Если СтрНайти(Форма.ИмяФормы, "ЖурналОпераций") <> 0 Тогда
			ЗаголовокФормы = "Реестр документов";
		Иначе	
	...
	Для каждого ПараметрЗапроса Из ЭлементыПараметров Цикл
		...
		Если ИмяПараметра = "ОтборТипыДокументов" Тогда
			Если ПараметрЗапроса.Значение.Найти(Тип("ДокументСсылка.ПоступлениеТоваровУслуг")) <> Неопределено Тогда
	...
	// Если получившийся период ограничен только с одной строны (с начала или с конца), то для полного ограничения
	// действуем как в п.5. Основная идея полного ограничения периода состоит в том, чтобы пользователь случайно не сформировал
	...
	КоличествоДокументов = Элементы[ИмяТаблицыФормы].ВыделенныеСтроки.Количество();
	Если КоличествоДокументов = 1 Тогда
		ВыделеннаяСтрока = Элементы[ИмяТаблицыФормы].ВыделенныеСтроки[0];
	...
	ИначеЕсли НЕ ЗначениеЗаполнено(НастройкиСписка.Период) Тогда
		...
		ИначеЕсли КоличествоДокументов > 1 Тогда
			...
			СчетчикДокументов = 0;
			Если НастройкиКомпоновки.Структура.Количество() = 1
				И НастройкиКомпоновки.Структура[0].ПоляГруппировки.Элементы.Количество() > 0 Тогда
				Для каждого СтрокаТаблицы Из Элементы[ИмяТаблицыФормы].ВыделенныеСтроки Цикл
					Если ТипЗнч(СтрокаТаблицы) = Тип("СтрокаГруппировкиДинамическогоСписка") Тогда
						Продолжить;
					КонецЕсли;
					НоваяСтрока = ТаблицаДат.Добавить();
					НоваяСтрока.Дата = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТаблицы, "Дата");
					СчетчикДокументов = СчетчикДокументов + 1;
				КонецЦикла;
			Иначе
				Для каждого КлючИЗначение Из ОбщегоНазначения.ЗначениеРеквизитаОбъектов(Элементы[ИмяТаблицыФормы].ВыделенныеСтроки, "Дата") Цикл
					НоваяСтрока = ТаблицаДат.Добавить();
					НоваяСтрока.Дата = КлючИЗначение.Значение;
				КонецЦикла;
				СчетчикДокументов = КоличествоДокументов;
			КонецЕсли;
...
Функция НомерНаПечать(Номер, Ссылка) Экспорт
	...
	//	ПредставлениеНомера = ПрефиксацияОбъектовКлиентСервер.УдалитьПрефиксыИзНомераОбъекта(СокрЛП(Номер), Истина, Истина);
	//	ПредставлениеНомера = ПрефиксацияОбъектовКлиентСервер.УдалитьЛидирующиеНулиИзНомераОбъекта(ПредставлениеНомера);		
...
Процедура НастроитьСтруктуруОтчета(Настройки, ПараметрыОтчета, Схема, СхемаКомпоновкиДанныхЭталон)
	...
	Если НЕ ЕстьВалютаДокумента Тогда
