...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	СтавкаНСПДляРасчетаНДСНаАвансы = БухгалтерскийУчетВызовСервераПовтИсп.ПолучитьДанныеУчетнойПолитикиОрганизаций(
		?(Отчет.КонецПериода = Дата(0001, 01, 01), Дата(3999, 01, 01), Отчет.КонецПериода), Отчет.Организация).СтавкаНСПДляРасчетаНДСНаАвансы;
	...
	ПараметрыОтчета.СтавкаНСП							= СтавкаНСПДляРасчетаНДСНаАвансы;
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Название = ?(ЗначениеЗаполнено(Форма.ПредставлениеТекущегоВарианта), Форма.ПредставлениеТекущегоВарианта, "Реестр продаж");
