	...
	Если ЗаполняемыеНастройки.Свойство("Отбор") Тогда
		Если ЗаполняемыеНастройки.Отбор Тогда
			...
			Для Индекс = 1 По КоличествоСубконто Цикл
				...
				НовыйЭлементОтбора = БухгалтерскиеОтчетыКлиентСервер.ДобавитьОтбор(Отчет.КомпоновщикНастроек, "Субконто" + Индекс,ТипЗначенияСубконто.ПривестиЗначение(Неопределено), , Ложь);
