...
Процедура ПодготовитьГоризонтальныйВариантРегистровБухгалтерии(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	// БПКР
	// Выделение цветом движений РБ по Дт
	УсловноеОформление = НастройкиКД.УсловноеОформление;
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	ЭлементУО.Использование = Истина;
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветФона", ЦветаСтиля.ЦветФонаДвиженияДокументаДт);
	Для Каждого ЭлементСтруктуры Из СтруктураИзмеренийДт Цикл
		ОбщегоНазначенияБПКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, ЭлементСтруктуры.Ключ);
	КонецЦикла;
	Для Каждого ЭлементСтруктуры Из СтруктураСубконтоДт Цикл
		ОбщегоНазначенияБПКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, ЭлементСтруктуры.Ключ);
	КонецЦикла;
	Для Каждого ЭлементСтруктуры Из СтруктураРесурсовДт Цикл
		ОбщегоНазначенияБПКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, ЭлементСтруктуры.Ключ);
	КонецЦикла;
	
	// Выделение цветом движений РБ по Кт
	УсловноеОформление = НастройкиКД.УсловноеОформление;
	ЭлементУО = УсловноеОформление.Элементы.Добавить();
	ЭлементУО.Использование = Истина;
	ЭлементУО.Оформление.УстановитьЗначениеПараметра("ЦветФона", ЦветаСтиля.ИзмененноеЗначениеРеквизитаФон);
	Для Каждого ЭлементСтруктуры Из СтруктураИзмеренийКт Цикл
		ОбщегоНазначенияБПКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, ЭлементСтруктуры.Ключ);
	КонецЦикла;
	Для Каждого ЭлементСтруктуры Из СтруктураСубконтоКт Цикл
		ОбщегоНазначенияБПКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, ЭлементСтруктуры.Ключ);
	КонецЦикла;
	Для Каждого ЭлементСтруктуры Из СтруктураРесурсовКт Цикл
		ОбщегоНазначенияБПКлиентСервер.ДобавитьОформляемоеПоле(ЭлементУО.Поля, ЭлементСтруктуры.Ключ);
	КонецЦикла;
	// Конец БПКР
