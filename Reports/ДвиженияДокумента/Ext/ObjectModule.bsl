...
Процедура ПодготовкаДанныхПоРегистрамНакопленияСведений(ДвижениеДокумента, ОсновнаяИнформация, ИмяРегистра,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	КОЛИЧЕСТВО(*) КАК КоличествоЗаписейРегистра,
	|	""&ИмяРегистра"" КАК ИмяРегистра
	|ПОМЕСТИТЬ ВТ_Пакет
	|ИЗ
	|	ВТ_ПакетОсновной КАК ПакетОсновной
	|
	|ИМЕЮЩИЕ
	|	КОЛИЧЕСТВО(*) > 0
	|;
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	Пакет.КоличествоЗаписейРегистра КАК КоличествоЗаписейРегистра,
	|	""&ИмяРегистра"" КАК ИмяРегистра,
	|	&Поля
	|ИЗ
	|	&ТекущаяТаблица КАК ТекущаяТаблица
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВТ_Пакет КАК Пакет
	|		ПО (Пакет.ИмяРегистра = ""&ИмяРегистра"")
	|ГДЕ
	|	&УсловиеЗапроса
	|{ГДЕ
	|	(&УсловиеКомпоновки)}";
...
Процедура ДобавитьРесурсРасчетаКоличестваЗаписей(СхемаКД)
	...
	Если ПолеКоличестваЗаписей = Неопределено Тогда
		...
		ПолеИтога.Выражение   = "КоличествоЗаписейРегистра";
	КонецЕсли;
...
Процедура ЗафиксироватьГлобальныеНастройки(Регистратор, МассивИспользуемыхРегистров)
	...
	ПредаврительныйРасчетКоличества = ПредаврительныйРасчетКоличества(Регистратор, МассивИспользуемыхРегистров);
	...
	Для Каждого ИспользуемыйРегистр Из МассивИспользуемыхРегистров Цикл
		...
		СтруктураНастроек.Вставить("КоличествоЗаписей",      ПредаврительныйРасчетКоличества[СтрЗаменить(ИспользуемыйРегистр.ПолноеИмя(), ".", "_")]);
...
Функция ПредаврительныйРасчетКоличества(Регистратор, МассивИспользуемыхРегистров)
	
	РасчитанноеКоличество = Новый Соответствие;
	...
	Для Каждого РезультатЗапроса Из РезультатыЗапроса Цикл
		...
		РасчитанноеКоличество.Вставить(Результат.ПолноеИмяРегистра, Результат.Количество);
	КонецЦикла;
	
	Запрос.МенеджерВременныхТаблиц.Закрыть();
	Запрос = Неопределено;
	
	Возврат РасчитанноеКоличество;
...
Процедура ВосстановитьОтборПоГруппировкамРегистров(НастройкиКД)
	...
	Для Каждого ЭлементСтруктуры Из СтруктураОтчета Цикл
		ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(ЭлементСтруктуры.Отбор,
			...
			"Служебный отбор", Истина, РежимОтображенияЭлементаНастройкиКомпоновкиДанных.Недоступный);
	КонецЦикла;
