...
Функция ПодготовитьГоризонтальныйВариант(ДокументОтбор, НастройкиКД)
	...
	Для Каждого НаборДанных Из НаборыДанных Цикл
		...
		НастроитьСтруктурыНабора_Горизонталь(НастройкиКД, НаборДанных, НаборДанныхСКД);
...
Процедура НастроитьСтруктурыНабора_Горизонталь(НастройкиКД, ЭлементДанных, НаборДанныхКД)
	
	Если ЭлементДанных.ВидРегистра = "Накопления" Или ЭлементДанных.ВидРегистра = "Сведений" Тогда
		НастроитьСтруктурыРегистровНакопленияИСведений_Горизонталь(НастройкиКД, НаборДанныхКД, ЭлементДанных);
	ИначеЕсли ЭлементДанных.ВидРегистра = "Бухгалтерии" Тогда
		НастроитьСтруктурыРегистровБухгалтерии(НастройкиКД, НаборДанныхКД, ЭлементДанных);
	ИначеЕсли ЭлементДанных.ВидРегистра = "Расчета" Тогда
		НастроитьСтруктурыРегистровРасчета_Горизонталь(НастройкиКД, НаборДанныхКД, ЭлементДанных);
	КонецЕсли;
...
Процедура НастроитьСтруктурыРегистровНакопленияИСведений_Горизонталь(НастройкиКД, НаборДанныхКД, ЭлементДанных)
...
Процедура НастроитьСтруктурыРегистровРасчета_Горизонталь(НастройкиКД, НаборДанныхКД, ЭлементДанных)
...
Функция ПодготовитьВертикальныйВариант(ДокументОтбор, НастройкиКД, ДополнительнаяНумерация = Ложь)
	...
	Для Каждого НаборДанных Из НаборыДанных Цикл
		...
		НастройкаСтруктурыНабора_Вертикаль(НастройкиКД, НаборДанных, НаборДанныхСКД);
...
Процедура НастройкаСтруктурыНабора_Вертикаль(НастройкиКД, НаборДанных, НаборДанныхКД)
	
	Если НаборДанных.ВидРегистра = "Накопления" Или НаборДанных.ВидРегистра = "Сведений" Тогда
	
		НастройкаСтруктурыРегистровНакопленияИСведений_Вертикаль(НастройкиКД, НаборДанныхКД, НаборДанных);
	
	ИначеЕсли НаборДанных.ВидРегистра = "Бухгалтерии" Тогда
	
		НастроитьСтруктурыРегистровБухгалтерии(НастройкиКД, НаборДанныхКД, НаборДанных);
	
	ИначеЕсли НаборДанных.ВидРегистра = "Расчета" Тогда
	
		НастройкаСтруктурыРегистровРасчета_Вертикаль(НастройкиКД, НаборДанныхКД, НаборДанных);
...
Процедура НастроитьСтруктурыРегистровБухгалтерии(НастройкиКД, НаборДанныхКД, ЭлементДанных)
	...
	Для ИндексСтрок = 1 По ВысотаТаблицы Цикл
		...
		Для ИндексКолонок = 1 По ШиринаТаблицы Цикл
			...
			Если СтрокаГлобальныхНастроек <> Неопределено Тогда
				...
				СтруктураРасшифровки.Вставить("ВидРегистра", СтрокаГлобальныхНастроек.ПредставлениеВидаРегистра);
...
Процедура УстановитьГлобальныеНастройки(Регистратор, МассивИспользуемыхРегистров)
	
	ГлобальныеНастройки             = Новый Массив;
	КоличествоЗаписей = КоличествоЗаписейПоРегистратору(Регистратор, МассивИспользуемыхРегистров);
...
Процедура ДоопределитьМакетСКД(СхемаКД, ИндексНабора, ВидРегистра, ПредставлениеВидаРегистра, ИмяРегистра,
	...
	Если СтрокиГлобальныхНастроек.Количество() > 0 Тогда
		...
		НРег(ПредставлениеВидаРегистра), СтрокаГлобальныхНастроек.ПредставлениеРегистра);
...
Функция ПерехватитьГлобальныеНастройки()
	...
	КолонкиГлобальныхНастроек.Добавить("ПредставлениеВидаРегистра",
		Новый ОписаниеТипов("Строка", , , , Новый КвалификаторыСтроки(32)));
	...
	Для Каждого ЭлементГлобальныхНастроек Из ГлобальныеНастройки Цикл
		...
		НадписьРезультат = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru = 'Регистр %1 ""%2""'"),
			СокрЛП(НРег(ЭлементГлобальныхНастроек.ПредставлениеВидаРегистра)),
...
Процедура ПерерасчетКоличестваДвижений(СписокРегистров, ДокументОтбор, ГлобальныеНастройки)
	...
	Для Каждого ГлобальнаяНастройка Из ГлобальныеНастройки Цикл
		...
		ГлобальнаяНастройка.Вставить("КоличествоЗаписей", КоличествоЗаписей[ПолноеИмяРегистра]);
		ГлобальнаяНастройка.Вставить("Использование", ОтмеченныеРегистры.НайтиПоЗначению(ПолноеИмяРегистра) <> Неопределено);
