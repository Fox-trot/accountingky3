...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.Подразделение                   = Отчет.Подразделение;
	ПараметрыОтчета.Сотрудник                       = Отчет.Сотрудник;
	...
	ПараметрыОтчета.ДобавлятьДетальныеЗаписи       	= Истина;
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Название = ?(ЗначениеЗаполнено(Форма.ПредставлениеТекущегоВарианта), Форма.ПредставлениеТекущегоВарианта, "Кадровые изменения");
...
Процедура УстановитьНастройкиПоУмолчаниюНаСервере()
	...
	// Так как в отчете выводятся детальные записи
	// их нужно восстановить отдельно
	Для Каждого ВыбранноеПоле Из Отчет.КомпоновщикНастроек.Настройки.Выбор.Элементы Цикл
		Если ТипЗнч(ВыбранноеПоле) = Тип("ВыбранноеПолеКомпоновкиДанных") 
			...
			НоваяСтрока.Представление = БухгалтерскиеОтчетыКлиентСервер.ПолучитьСвойствоПоля(Отчет.КомпоновщикНастроек, НоваяСтрока.Поле);
		КонецЕсли;
	КонецЦикла;
КонецПроцедуры 
