...
Функция СчетаУчетаТоваров()
	...
	Для Каждого Счет Из СчетаУчетаТоваров Цикл
		Если БухгалтерскийУчетСервер.НаСчетеВедетсяПартионныйУчет(Счет) Тогда
			Если БухгалтерскийУчетСервер.ВедетсяУчетПоСкладам(Счет) Тогда
...
Функция СчетаУчетаТорговойНаценки()
	...
	СчетаУчета = БухгалтерскийУчетПовтИсп.СчетаВИерархии(ПланыСчетов.Хозрасчетный.НереализованнаяТорговаяНаценка);
...
// Обработчик обновления 3.0.53.10
// В команде "ВаловаяПрибыль" отчета ключ варианта изменился и теперь совпадает с именем варианта в схеме компоновки данных.
// После переименования ключа варианта отчета в команде "ВаловаяПрибыль", необходимо перенести сохраненные
// пользовательские настройки отчета из хранилища настроек со старым ключом варианта отчета в новый, чтобы настройки
// корректно восстанавливались при работе с отчетом.
//
Процедура ПеренестиТекущиеПользовательскиеНастройки() Экспорт
	
	СтарыйКлючОбъекта = "Отчет.ВаловаяПрибыль/Валовая прибыль/ТекущиеПользовательскиеНастройки";
	НовыйКлючОбъекта  = "Отчет.ВаловаяПрибыль/ВаловаяПрибыль/ТекущиеПользовательскиеНастройки";
	
	ПользователиИБ = ПользователиИнформационнойБазы.ПолучитьПользователей();
	Для Каждого ПользовательИБ Из ПользователиИБ Цикл
		
		СохраненныеНастройки = ОбщегоНазначения.ХранилищеСистемныхНастроекЗагрузить(СтарыйКлючОбъекта, Неопределено, Неопределено, , ПользовательИБ.Имя);
		Если СохраненныеНастройки = Неопределено Тогда
			Продолжить;
		КонецЕсли;
		
		ОбщегоНазначения.ХранилищеСистемныхНастроекСохранить(НовыйКлючОбъекта, Неопределено, СохраненныеНастройки, , ПользовательИБ.Имя);
		ОбщегоНазначения.ХранилищеСистемныхНастроекУдалить(СтарыйКлючОбъекта, Неопределено, ПользовательИБ.Имя);
