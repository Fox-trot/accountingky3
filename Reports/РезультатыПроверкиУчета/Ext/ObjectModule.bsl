	...
	ИначеЕсли ПараметрыФормы.Свойство("ДанныеКонтекста") Тогда
		...
		Если ТипЗнч(ПараметрыФормы.ДанныеКонтекста) = Тип("Структура") Тогда
			...
			Если ВыделенныеСтроки.Количество() > 0 Тогда
				...
				ПроблемныеОбъекты = КонтрольВеденияУчетаСлужебный.МассивПроблемныхОбъектов(ДанныеКонтекста.ВыделенныеСтроки);
...
Процедура ДоопределитьШапку(ДокументРезультат)
	...
	ОбластьИтоговПоГоризонтали          = ДокументРезультат.Область("R4C" + Формат(ДокументРезультат.ШиринаТаблицы, "ЧГ=0"));
	ОбластьИтоговПоГоризонтали.ЦветФона = Новый Цвет(255, 250, 217);
