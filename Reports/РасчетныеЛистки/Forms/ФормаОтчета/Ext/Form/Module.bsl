...
Процедура СформироватьВыполнить()
	...
	Пока ВыборкаПодразделение.Следующий() Цикл
		...
		Пока ВыборкаФизЛицо.Следующий() Цикл
			...
			Пока ВыборкаДетали.НайтиСледующий(СтруктураПоискиПоВыборкей) Цикл
				
				Если ВыборкаДетали.Удержание = NULL Тогда
					
					Если ВыборкаДетали.СуммаНачислено <> 0 Тогда
					...
					Иначе
						...
						СтрокаТаблицы.Начисление 		= ВыборкаДетали.Начисление;
						СтрокаТаблицы.ДатаНачала 		= ВыборкаДетали.ДатаНачала;
						СтрокаТаблицы.ДатаОкончания 	= ВыборкаДетали.ДатаОкончания;
						СтрокаТаблицы.ОтработаноДней 	= ВыборкаДетали.ОтработаноДней;
						СтрокаТаблицы.ОтработаноЧасов 	= ВыборкаДетали.ОтработаноЧасов;
						СтрокаТаблицы.Допдоход 			= ВыборкаДетали.Допдоход;
					КонецЕсли;
				...
				Иначе
					...
					Если ПоследнееУдержание < ПоследнееНачисление Тогда
						...
						ТабличныйДокумент.Область(ПоследнееУдержание + 1, 11, ПоследнееУдержание + 1, 12).Текст = Формат(ВыборкаДетали.ДатаОкончания , "ДФ=""МММ гг""");
					...
					Иначе
						...
						ОбластьДетали.Параметры.ПериодУдержание = Формат(ВыборкаДетали.ДатаОкончания , "ДФ=""МММ гг""");
				...
				ВсегоНачисления = ВсегоНачисления + ВыборкаДетали.СуммаНачислено;
				ВсегоУдержания = ВсегоУдержания + ВыборкаДетали.СуммаУдержано;
			КонецЦикла;
			...
			Для Каждого СтрокаТаблицы Из ТЗДопдоходы Цикл
				...
				ВсегоНачисления = ВсегоНачисления + ВыборкаДетали.Допдоход;
			КонецЦикла;	
			...
			Пока ВыборкаВыплатСотрудника.НАйтиСледующий(СтруктураПоискиПоВыборкей) Цикл
				...
				ОбластьДеталиВыплаты.Параметры.ТекстВыплаты = "" + ВыборкаВыплатСотрудника.ПредставлениеДокумента 
					+ ?(ВыборкаВыплатСотрудника.ПризнакАванса, "(аванс) №", " №") + 
					СокрЛП(ВыборкаВыплатСотрудника.Номер) + " от " + Формат(ВыборкаВыплатСотрудника.Дата, "ДФ=dd.MM.yyyy");
					
				Если ВыборкаВыплатСотрудника.ПризнакАванса Тогда 
					ОбластьДеталиВыплаты.Параметры.ПериодВыплат = Формат(ВыборкаВыплатСотрудника.Дата, "ДФ=""МММ гг""");
				Иначе 
					ОбластьДеталиВыплаты.Параметры.ПериодВыплат = Формат(ВыборкаВыплатСотрудника.ПериодРегистрации, "ДФ=""МММ гг""");
				КонецЕсли;	
