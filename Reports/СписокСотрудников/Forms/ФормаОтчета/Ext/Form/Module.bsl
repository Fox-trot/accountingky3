...
&НаКлиенте
Процедура НастройкаКратко(Команда)
	
	ЗаполнитьНастройкамиПоУмолчанию(НастрокиКратко());
...
&НаКлиенте
Процедура НастройкаРасширенная(Команда)
	
	ЗаполнитьНастройкамиПоУмолчанию(НастройкиРасширенные());
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Название = ?(ЗначениеЗаполнено(Форма.ПредставлениеТекущегоВарианта), Форма.ПредставлениеТекущегоВарианта, "Список сотрудников");
	ЗаголовокОтчета = Название + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.Период);
...
Процедура ЗаполнитьНастройкамиПоУмолчанию(ЗаполняемыеНастройки) Экспорт
	...
	// Сброс группировки по умолчанию.
	...
	// Дополнительные поля.
	Если ЗаполняемыеНастройки.НастройкаКратко Тогда
		
		Отчет.ДополнительныеПоля.Очистить();
		
		НоваяСтрока = Отчет.ДополнительныеПоля.Добавить();
		НоваяСтрока.Представление = НСтр("ru = 'Таб.№'");
		НоваяСтрока.Поле = "ФизЛицо.ТабельныйНомер";
		НоваяСтрока.Использование = Истина;
		
		НоваяСтрока = Отчет.ДополнительныеПоля.Добавить();
		НоваяСтрока.Представление = НСтр("ru = 'Должность'");
		НоваяСтрока.Поле          = "ФизЛицо.Должность";
		НоваяСтрока.Использование = Истина;
	КонецЕсли;

	Если ЗаполняемыеНастройки.НастройкаРасширенная Тогда
		
		Отчет.ДополнительныеПоля.Очистить();
		
		НастройкиКомпоновкиДанных = Отчет.КомпоновщикНастроек.ПолучитьНастройки();
		
		Для Каждого ПолеВыбора Из НастройкиКомпоновкиДанных.Выбор.Элементы Цикл
			НоваяСтрока = Отчет.ДополнительныеПоля.Добавить();
			НоваяСтрока.Представление = ПолеВыбора.Заголовок;
			НоваяСтрока.Поле = ПолеВыбора.Поле;
			НоваяСтрока.Использование = ПолеВыбора.Использование;
		КонецЦикла;
	КонецЕсли;
...
&НаКлиентеНаСервереБезКонтекста
Функция НастрокиКратко()
	
	Настройки = Новый Структура();
	
	Настройки.Вставить("НастройкаКратко", Истина);
	Настройки.Вставить("НастройкаРасширенная", Ложь);
...
&НаКлиентеНаСервереБезКонтекста
Функция НастройкиРасширенные()
	
	Настройки = Новый Структура();
	
	Настройки.Вставить("НастройкаКратко", Ложь);
	Настройки.Вставить("НастройкаРасширенная", Истина);
