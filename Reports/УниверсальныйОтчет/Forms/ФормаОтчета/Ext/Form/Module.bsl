...
Процедура УстановитьИсточникДанныхТекущегоВарианта(Вариант)
	...
	Исключение
		Событие = НСтр("ru = 'Варианты отчетов.Установка источника данных универсального отчета'");
		ШаблонКомментария = НСтр("ru = 'Не удалось установить источник данных варианта отчета:
			|	%1 по причине:
			|	%2'");
		Комментарий = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
			ШаблонКомментария, Вариант, ПодробноеПредставлениеОшибки(ИнформацияОбОшибке()));
		ЗаписьЖурналаРегистрации(
			Событие, УровеньЖурналаРегистрации.Предупреждение, Вариант.Метаданные(), Вариант, Комментарий);
	КонецПопытки;
КонецПроцедуры
