...
Процедура СформироватьТитульныйЛист(ЛистовЗаГод, ПараметрыОтчета, СписокСформированныхЛистов)
	...
	СведенияОбОрганизации = ОбщегоНазначенияБПВызовСервера.СведенияОЮрФизЛице(ПараметрыОтчета.Организация, КонецПериода);
	
	ТекстРНН_БИН 		  = "";
	
	ОбластьОбложка.Параметры.НазваниеОрганизации = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "ПолноеНаименование,");
	ОбластьОбложка.Параметры.РНН_БИН             = ОбщегоНазначенияБПВызовСервера.ОписаниеОрганизации(СведенияОбОрганизации, "БИН_ИИН,", Ложь, КонецДня(КонецПериода), "ru");
	...
	Руководители = БухгалтерскийУчетСервер.ОтветственныеЛицаОрганизаций(ПараметрыОтчета.Организация, КонецПериода);
...
Функция ПолучитьОтветственныхЛиц(ДатаЛиста, ПараметрыОтчета)
	...
	Руководители = БухгалтерскийУчетСервер.ОтветственныеЛицаОрганизаций(ПараметрыОтчета.Организация, ПараметрыОтчета.КонецПериода);
	...
	//Запрос.Текст = 
	//"ВЫБРАТЬ РАЗРЕШЕННЫЕ
	//|	ОтветственныеЛицаОрганизаций.Физлицо
	//|ИЗ
	//|	РегистрСведений.ОтветственныеЛицаОрганизаций.СрезПоследних(
	//|			&Период,
	//|			СтруктурнаяЕдиница = &СтруктурнаяЕдиница
	//|				И ОтветственноеЛицо = ЗНАЧЕНИЕ(Перечисление.ОтветственныеЛицаОрганизаций.Кассир)) КАК ОтветственныеЛицаОрганизаций";
	//
	...
	//	СтруктураРезультата.Кассир = ФизическиеЛицаКлиентСервер.ФамилияИнициалыФизЛица(СрезПоследних[0].Физлицо);
	//КонецЕсли;
