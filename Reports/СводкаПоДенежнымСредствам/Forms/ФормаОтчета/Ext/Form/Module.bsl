...
Процедура ПриЗагрузкеПользовательскихНастроекНаСервере(Настройки)
	...
	Иначе
		ОбщегоНазначенияКлиентСервер.УстановитьСостояниеПоляТабличногоДокумента(Элементы.Результат, "ФормированиеОтчета");
	КонецЕсли;
	Если НЕ (Отчет.ПоказательВалютнаяСумма И Отчет.ПоказательСумма) Тогда
...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.НаборПоказателей                  = Отчеты[ПараметрыОтчета.ИдентификаторОтчета].ПолучитьНаборПоказателей();
	ПараметрыОтчета.ПоказательСумма                   = Отчет.ПоказательСумма;
	ПараметрыОтчета.ПоказательВалютнаяСумма           = Отчет.ПоказательВалютнаяСумма;
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	Название = ?(ЗначениеЗаполнено(Форма.ПредставлениеТекущегоВарианта), Форма.ПредставлениеТекущегоВарианта, "Сводка по денежным средствам");
...
Функция ПолучитьПараметрыВыбораЗначенияОтбора()
	...
	СписокПараметров.Вставить("КассаБанковскийСчет", Неопределено);
	СписокПараметров.Вставить("Валюта", Неопределено);
