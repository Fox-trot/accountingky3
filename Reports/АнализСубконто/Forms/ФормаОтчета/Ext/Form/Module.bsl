...
Процедура ПроверкаСпискаВидовСубконто(Знач СписокВидовСубконто, Отказ)
	...
	СписокВидовСубконто = СписокВидовСубконто.Скопировать();
	СписокВидовСубконто.Добавить(Неопределено);
	ПроверяемыеЗначения = Новый Структура("СписокВидовСубконто", СписокВидовСубконто);
...
Процедура ЗаполнитьНастройкамиПоУмолчанию(ЗаполняемыеНастройки) Экспорт
	...
	Если ЗаполняемыеНастройки.Свойство("Группировка", ЗаполнятьКоллекцию) И ЗаполнятьКоллекцию Тогда
		...
		Для Индекс = 1 По Мин(МассивСубконто.Количество(), БухгалтерскийУчетСервер.МаксимальноеКоличествоСубконто()) Цикл
