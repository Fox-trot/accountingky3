...
Процедура ОбновитьТекстЗаголовка(Форма)
	ЗаголовокОтчета = НСтр("ru = 'Оборотно-сальдовая ведомость по паре счетов %1'");
...
Функция СформироватьОтчетНаСервере(ПараметрыОтчета)
	...
	РезультатВыполнения = ДлительныеОперации.ЗапуститьВыполнениеВФоне(
		...
		"Отчеты.ОСВПоПареСчетов.Сформировать",
		ПараметрыОтчета,
...
Функция ПодготовитьПараметрыОтчета() Экспорт
	...
	ПараметрыОтчета.Вставить("ПарныйСчет", 		ПарныйСчет);
