...
Процедура НастроитьВариантыОтчета(Настройки, НастройкиОтчета) Экспорт
	...
	НастройкиВарианта.Описание = НСтр("ru = 'Главная книга.'");

	НастройкиВарианта.НастройкиДляПоиска.НаименованияПолей =
		НСтр("ru = 'Оборот по дебету
		|Оборот по кредиту
		|Сальдо дебет
		|Сальдо кредит
		|С кредита счета'");
	НастройкиВарианта.НастройкиДляПоиска.НаименованияПараметровИОтборов =
		...
		|ВыводитьЗаголовок
		|ВыводитьПодписи
		|ВыводитьПодписиРуководителей
		|Использование
		|Счет
		|ПоСубсчетам'");
	НастройкиВарианта.НастройкиДляПоиска.КлючевыеСлова = НСтр("ru = 'Главная книга'");
КонецПроцедуры
...
Процедура СформироватьОтчет(Знач ПараметрыОтчета, АдресХранилища) Экспорт
	...
	Результат.КлючПараметровПечати = "ГлавнаяКнигаХозрасчетный_ГлавнаяКнига";
...
Процедура СформироватьГлавнуюКнигу(ДокументРезультат, ПараметрыОтчета)
	...
	Пока ВыборкаПоСчетам.Следующий() Цикл
		...
		Если ПараметрыОтчета.РазбитьПоЛистам Тогда
			...
			ДокументОтдельногоСчета.КлючПараметровПечати = "ГлавнаяКнигаХозрасчетный_РазвернутоеСальдо";
		...
		Если ПараметрыОтчета.РазбитьПоЛистам Тогда
			...
			ТабДок.КлючПараметровПечати = "ГлавнаяКнигаХозрасчетный_РазвернутоеСальдо";
	...
	СтруктураНастроекОтчета.Вставить("СписокОрганизаций", ПараметрыОтчета.Организация);
	...
	ДокументРезультат.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ГлавнаяКнигаХозрасчетный";
...
Функция ВыполнитьОбщийЗапрос(ПараметрыОтчета)
	...
	// установим параметры
	Запрос.УстановитьПараметр("СписокОрганизаций", ПараметрыОтчета.Организация);
...
Функция ПолучитьТекстОбщегоЗапроса(ПараметрыОтчета)
	...
	ДополнитьСтрокуОграниченийПоРеквизитам(СтрокаОграниченийПоРеквизитам, "СписокОрганизаций", ПараметрыОтчета.Организация, "Организация");
	...
	|	NULL КАК СуммаНачКт
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаНач, &ДатаКон, " + ПараметрыОтчета.ПредставленияПериодичности[ПараметрыОтчета.Периодичность] + ", , , " + СтрокаОграниченийПоРеквизитам + ", , ) КАК Обороты
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.Остатки(&ДатаНач, , , " + СтрокаОграниченийПоРеквизитам + ") КАК Остатки
	|		ЛЕВОЕ СОЕДИНЕНИЕ ПланСчетов.Хозрасчетный КАК Счета
...
Процедура ДополнитьСтрокуОграниченийПоРеквизитам(СтрокаОграничения, Знач ИмяОграниченияПоРеквизиту, Знач ОграничениеПоРеквизиту, ИмяРеквизита = "")
	...
	//Иначе		
		СтрокаНовогоОграничения = ИмяРеквизита + " = &" + ИмяОграниченияПоРеквизиту + " ";//ИмяОграниченияПоРеквизиту + " = &" + ИмяОграниченияПоРеквизиту + " ";
	//КонецЕсли;
...
Процедура ЗаполнитьТаблицуРазвернутыхОстатков(ПараметрыОтчета, СтруктураПараметров)
	...
	Если СтруктураПараметров.РазвернутоеСальдо Тогда
		...
		+ ТекстПолей + "
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.ОстаткиИОбороты(&ДатаНач, &ДатаКон, " + ПараметрыОтчета.ПредставленияПериодичности[ПараметрыОтчета.Периодичность] + ", , Счет В ИЕРАРХИИ (&Счет), , " + СтрокаОграниченийПоРеквизитам + ") КАК ОстаткиИОбороты
		|
...
Функция ПолучитьСписокРасшифровок(ПараметрыРасшифровки, ОтчетПоПроводкам = Истина)
	...
	Если ПараметрыРасшифровки <> Неопределено Тогда
		...
		КопияРасшифровки.Вставить("ИмяОбъекта", "удалитьОборотыСчетаХозрасчетный");
