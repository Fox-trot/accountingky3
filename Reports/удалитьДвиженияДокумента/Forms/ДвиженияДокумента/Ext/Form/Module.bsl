...
Процедура ОбработатьВыводДанныхПоМассиву(СписокПолей, ТаблицаРесурсов, ТаблицаИзмерений, ТаблицаРеквизитов, ТаблицаВидДвижений = Неопределено, Знач ИмяРегистра, СинонимРегистра)
	...
	Макет = Отчеты.удалитьДвиженияДокумента.ПолучитьМакет("Макет");
...
Процедура ВывестиЖурналПроводок()
	...
	Макет 				= Отчеты.удалитьДвиженияДокумента.ПолучитьМакет("Макет");
	МакетЖурналПроводок = Отчеты.удалитьДвиженияДокумента.ПолучитьМакет("МакетЖурналПроводок");
	ОбластьЗаголовок 	= Макет.ПолучитьОбласть("ЗаголовокОтчета");
	...
	|ИЗ
	|	РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(, , Регистратор = &ДокументОтчета, НомерСтроки, ) КАК ХозрасчетныйДвиженияССубконто";
	Запрос.УстановитьПараметр("ДокументОтчета", Отчет.Документ);	
	
	РезультатЗапроса = Запрос.Выполнить();
	
	Если НЕ РезультатЗапроса.Пустой() Тогда 
		ОбластьЗаголовок.Параметры.СинонимРегистра = "Регистр бухгалтерии ""Хозрасчетный""";
		ТабличныйДокумент.Вывести(ОбластьЗаголовок);
		ТабличныйДокумент.НачатьГруппуСтрок();
...
Процедура СформироватьОтчет()
	...
	Для Каждого СвойстваОбъекта Из ДвиженияДокумента Цикл
		...
		Если ВидРегистра = "Сведений" ИЛИ ВидРегистра = "Накопления" ИЛИ ВидРегистра = "Расчета" Тогда
			...
			ИначеЕсли ВидРегистра = "Расчета" И СвойстваОбъекта.Имя = "Начисления" Тогда
				СписокПолей = СписокПолей + ", ПериодРегистрации, ВидРасчета, ПериодДействияНачало, ПериодДействияКонец";
				ТаблицаИзмерений.Колонки.Добавить("ПериодРегистрации", , "Период регистрации");
				ТаблицаИзмерений.Колонки.Добавить("ВидРасчета", , "Вид расчета");
				ТаблицаИзмерений.Колонки.Добавить("ПериодДействияНачало", , "Период действия (начало)");
				ТаблицаИзмерений.Колонки.Добавить("ПериодДействияКонец", , "Период действия (конец)");
				//ТаблицаИзмерений.Колонки.Добавить("БазовыйПериодНачало", , "Базовый период (начало)");
				//ТаблицаИзмерений.Колонки.Добавить("БазовыйПериодКонец", , "Базовый период (конец)");
			ИначеЕсли ВидРегистра = "Расчета" И СвойстваОбъекта.Имя = "Удержания" Тогда
				СписокПолей = СписокПолей + ", ПериодРегистрации, ВидРасчета";
				ТаблицаИзмерений.Колонки.Добавить("ПериодРегистрации", , "Период регистрации");
				ТаблицаИзмерений.Колонки.Добавить("ВидРасчета", , "Вид расчета");
			Иначе
