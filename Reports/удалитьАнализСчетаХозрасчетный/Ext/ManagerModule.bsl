...
Процедура ПередКомпоновкойМакета(ПараметрыОтчета, Схема, КомпоновщикНастроек) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыОтчета.Организация) Тогда
		БухгалтерскиеОтчеты.ДобавитьОтборПоОрганизациям(КомпоновщикНастроек, ПараметрыОтчета, ПараметрыОтчета.Организация);
	КонецЕсли;
...
Процедура ПослеКомпоновкиМакета(ПараметрыОтчета, МакетКомпоновки) Экспорт
	...
	ЗавершающаяЧастьЗапроса = ") КАК ТиповойОбороты";
	...
	Для Каждого СтрокаТаблицы Из ПараметрыОтчета.ГруппировкаКор Цикл 
		Если СтрокаТаблицы.Использование И ЗначениеЗаполнено(СтрокаТаблицы.Счет) Тогда
			...
			Для ИндексКор = НачальныйИндексКор + 1 По 3 Цикл
				НачалоСтроки = Найти(ПромежуточныйТекстЗапроса, "ТиповойОбороты.КорСубконто" + ИндексКор);
	...
	Для Индекс = 1 По 3 Цикл
		ПостояннаяЧастьЗапроса = СтрЗаменить(ПостояннаяЧастьЗапроса, "ТиповойОбороты.КорСубконто" + Индекс + " КАК", "NULL КАК");
	КонецЦикла;
...
Процедура ЗаполнитьПравилаРасшифровки(Правила) Экспорт
		...
		Правило = Правила.Добавить();
		Правило.Отчет = "удалитьКарточкаСчетаХозрасчетный";
