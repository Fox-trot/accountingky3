...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ЦеныНоменклатуры.Основание = &Основание";
...
//  ДокументПоступление - Документ поступления, данными которого надо заполнить табличную часть.
//
Процедура ЗаполнитьТабличнуюЧастьПоПоступлениюСГТД(ДокументПоступление) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =	
		"ВЫБРАТЬ
		|	ГТДПоИмпортуТовары.Номенклатура КАК Номенклатура,
		|	СУММА(ГТДПоИмпортуТовары.СуммаАкциза + ГТДПоИмпортуТовары.СуммаПошлины + ГТДПоИмпортуТовары.СуммаСопровождения + ГТДПоИмпортуТовары.СуммаТаможенногоСбора) / СУММА(ГТДПоИмпортуТовары.Количество) КАК ЦенаГТД
		|ПОМЕСТИТЬ ВременнаяТаблицаГТДПоИмпорту
		|ИЗ
		|	Документ.ГТДПоИмпорту.Товары КАК ГТДПоИмпортуТовары
		|ГДЕ
		|	ГТДПоИмпортуТовары.ДокументПоступления = &ДокументПоступление
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ВременнаяТаблицаПоступление.Цена + ЕСТЬNULL(ВременнаяТаблицаГТДПоИмпорту.ЦенаГТД, 0) КАК Цена,
...
Процедура ЗаполнитьТабличнуюЧастьОстатками() Экспорт
	...
	|				Организация = &Организация
	|					И ТипЦен = &ТипЦен) КАК ЦеныНоменклатуры
	|		ПО ЗапасыОстатки.Номенклатура = ЦеныНоменклатуры.Номенклатура
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ТипыЦенНоменклатуры КАК ТипыЦенСправочник
	|		ПО (ЦеныНоменклатуры.ТипЦен = ТипыЦенСправочник.Ссылка)";
...
Процедура ВыполнитьПредварительныйКонтроль(Отказ)
	...
	Пока Выборка.Следующий() Цикл	
		ТекстСообщения = СтрШаблон(НСтр(
			"ru = 'В регистре сведений ""Цены номенклатуры"", на указанную дату, уже существует запись с номенклатурой - %1'"),
