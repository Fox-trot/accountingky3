...
Процедура ЗаполнитьТабличнуюЧастьОтчетПоНСП() Экспорт
	...
	Запрос.Текст =
		...
		|	ПродажиОбороты.СуммаДоходаОборот - ПродажиОбороты.СуммаСкидкиОборот КАК Выручка,
		...
		|	ПродажиОсновныхСредствОбороты.СуммаДоходаОборот - ПродажиОсновныхСредствОбороты.СуммаСкидкиОборот,
		...
		|ИЗ
		|	РегистрНакопления.ПродажиОсновныхСредств.Обороты(&НачалоПериода, &КонецПериода, , Организация = &Организация) КАК ПродажиОсновныхСредствОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаСтавкиНСПСрезПоследних КАК ВременнаяТаблицаСтавкиНСПСрезПоследних
...
Процедура ЗаполнитьТабличныеЧастиПриложениеКОтчетуПоНСП() Экспорт 
	...
	Запрос.Текст =	
		"ВЫБРАТЬ
		|	ПродажиОбороты.БезналичныйРасчет КАК БезналичныйРасчет,
		|	СУММА(ПродажиОбороты.СуммаОборот - ПродажиОбороты.СуммаСкидкиОборот) КАК Выручка,
		...
		|ВЫБРАТЬ
		|	ПродажиОсновныхСредствОбороты.БезналичныйРасчет,
		|	СУММА(ПродажиОсновныхСредствОбороты.СуммаОборот - ПродажиОсновныхСредствОбороты.СуммаСкидкиОборот),
		|	СУММА(ПродажиОсновныхСредствОбороты.СуммаНСПОборот),
		|	СтавкиНСПСрезПоследних.Ставка,
		|	ПродажиОсновныхСредствОбороты.СтавкаНСП
		|ИЗ
		|	РегистрНакопления.ПродажиОсновныхСредств.Обороты(&НачалоКвартала, КОНЕЦПЕРИОДА(&НачалоКвартала, МЕСЯЦ), , Организация = &Организация) КАК ПродажиОсновныхСредствОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтавкиНСП.СрезПоследних(КОНЕЦПЕРИОДА(&НачалоКвартала, МЕСЯЦ), Организация = &Организация) КАК СтавкиНСПСрезПоследних
		...
		|	ПродажиОсновныхСредствОбороты.СтавкаНСП
		|ИЗ
		|	РегистрНакопления.ПродажиОсновныхСредств.Обороты(НАЧАЛОПЕРИОДА(&ВторойМесяцКвартала, МЕСЯЦ), КОНЕЦПЕРИОДА(&ВторойМесяцКвартала, МЕСЯЦ), , Организация = &Организация) КАК ПродажиОсновныхСредствОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтавкиНСП.СрезПоследних(КОНЕЦПЕРИОДА(&ВторойМесяцКвартала, МЕСЯЦ), Организация = &Организация) КАК СтавкиНСПСрезПоследних
		|		ПО ПродажиОсновныхСредствОбороты.СтавкаНСП = СтавкиНСПСрезПоследних.СтавкаНСП
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ПродажиОсновныхСредствОбороты.СтавкаНСП
		|ИЗ
		|	РегистрНакопления.ПродажиОсновныхСредств.Обороты(НАЧАЛОПЕРИОДА(&КонецКвартала, МЕСЯЦ), КОНЕЦПЕРИОДА(&КонецКвартала, МЕСЯЦ), , Организация = &Организация) КАК ПродажиОсновныхСредствОбороты
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СтавкиНСП.СрезПоследних(КОНЕЦПЕРИОДА(&КонецКвартала, МЕСЯЦ), Организация = &Организация) КАК СтавкиНСПСрезПоследних
		|		ПО ПродажиОсновныхСредствОбороты.СтавкаНСП = СтавкиНСПСрезПоследних.СтавкаНСП
		|
		|СГРУППИРОВАТЬ ПО
