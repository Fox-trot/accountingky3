...
#Область ОбработчикиСобытийЭлементовТаблицыФормыВозвратЗаработнойПлаты
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ВозвратОтСотрудника") Тогда
		...
		Элементы.Получатель.Видимость 								= Ложь;
		Элементы.ВозвратЗаработнойПлатыВедомость.Видимость          = Истина;
		//Видимость ТЧ
...
Процедура УстановитьСчетаУчетаРасчетовСКонтрагентом(СтрокаТабличнойЧасти)
	...
	Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПКО.ОплатаОтНесколькихКонтрагентов") Тогда
		...
		СчетаУчета			= БухгалтерскийУчетВызовСервера.ПолучитьСчетаРасчетовСКонтрагентом(Объект.Организация, СтрокаТабличнойЧасти.Контрагент, СтрокаТабличнойЧасти.ДоговорКонтрагента);			
...
Процедура УстановитьКурсОбменаКонвертации()
	...
	Если Объект.КурсВалютыКонвертации >= Объект.Курс Тогда
		
		Если Объект.Курс = 0 Тогда
			Возврат;
		КонецЕсли;
		Объект.ПрямойКурс 		= Ложь;	
		Объект.КурсОбмена 		= Окр(Объект.КурсВалютыКонвертации / Объект.Курс, 4);
	...
	Иначе
		...
		Объект.КурсОбмена 		= Окр(Объект.Курс / Объект.КурсВалютыКонвертации, 4);
		Объект.КросскурсНБКР 	= Объект.КурсОбмена;
...
Процедура УстановитьКурсОбменаКонвертацииПриИзмененииСуммыДокумента()
	...
	Если Объект.КурсВалютыКонвертации >= Объект.Курс Тогда		
		...
		Объект.КросскурсНБКР 	= Окр(Объект.КурсВалютыКонвертации / Объект.Курс, 4);		
	Иначе
		Если Объект.КурсВалютыКонвертации = 0 Тогда
			Возврат;
		КонецЕсли;		
		...
		Объект.КросскурсНБКР 	= Окр(Объект.Курс / Объект.КурсВалютыКонвертации, 4);		
	КонецЕсли;
...
Процедура ПоказатьКР()
	...
	СуммаУчетнаяПрихода = Объект.СуммаДокумента * Объект.Курс;
	СуммаУчетнаяРасхода = Объект.СуммаКонвертации * Объект.КурсВалютыКонвертации;
	КурсПрихода 	= Объект.Курс;
