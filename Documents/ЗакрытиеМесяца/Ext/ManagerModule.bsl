...
Процедура СформироватьНалогНаПрибыльВТаблицеХозрасчетный(ДатаДокумента, Организация, ТаблицаХозрасчетный, ДокументСсылка)
	...
	Если ДокументСсылка.СуммаНалогНаПрибыль <> 0 Тогда
		...
		Если ДокументСсылка.НеФормироватьОтсроченныеПроводки Тогда 
			...
			СтрокаТЗ.СчетКт 		= ПланыСчетов.Хозрасчетный.НалогНаПрибыль;
			...
			// 2. Учитываем отсроченные проводки
		Иначе			
			
			// Отсроченная проводки 4310
			// 2.1 Если бухгатерская прибыль больше налоговой: 
			Если ДокументСсылка.СуммаНалогНаПрибыльБУ > ДокументСсылка.СуммаНалогНаПрибыль Тогда 
				...
				// 2.2 Если налоговая прибыль больше бухгалтерской: 
			...
			ИначеЕсли ДокументСсылка.СуммаНалогНаПрибыльБУ = ДокументСсылка.СуммаНалогНаПрибыль Тогда  
				// 2.3 Основная проводка налог на прибыль НУ 9910 - 3410				
				Если ДокументСсылка.СуммаНалогНаПрибыльБУ <> 0 Тогда
					СтрокаТЗ				= ТаблицаХозрасчетный.Добавить();
					СтрокаТЗ.Период 		= ДатаДокумента;
					СтрокаТЗ.Организация 	= Организация;
...
Функция ПолучитьДанныеПоНалогуНаИмущество(Организация, НачалоПериода, КонецПериода, ДополнительныйВариантНИ = Ложь, КонецПериодаДляОстатков = Неопределено) Экспорт
	...
	Если ДополнительныйВариантНИ Тогда
		Запрос.Текст =
			...
			|	ВЫБОР
			|		КОГДА СостоянияОС.ДатаНачисленияНалогаНаИмущество <> ДАТАВРЕМЯ(1,1,1)
			|			ТОГДА СостоянияОС.ДатаНачисленияНалогаНаИмущество
			|		ИНАЧЕ СостоянияОС.Период
			|	КОНЕЦ КАК ПериодПринятоКУчету
			...
			|	И СостоянияОС.ДатаНачисленияНалогаНаИмущество <= &КонецПериода
	...
	Иначе
		Запрос.Текст =
			...
			|	ВЫБОР
			|		КОГДА СостоянияОССрезПоследних.ДатаНачисленияНалогаНаИмущество <> ДАТАВРЕМЯ(1,1,1)
			|			ТОГДА СостоянияОССрезПоследних.ДатаНачисленияНалогаНаИмущество
			|		ИНАЧЕ СостоянияОССрезПоследних.Период
			|	КОНЕЦ КАК ПериодПринятоКУчету
			|ПОМЕСТИТЬ ВременнаяТаблицаСостоянияОСПринятоКУчету
			|ИЗ
			...
			|		И СостоянияОССрезПоследних.ДатаНачисленияНалогаНаИмущество <= &КонецПериода
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ
	...
	Запрос.Текст = Запрос.Текст +
		...
		|	ВЫБОР
		|		КОГДА СведенияОбИмуществеСрезПоследних.ОсновноеСредство.ДатаВыпускаНедвижимости <> ДАТАВРЕМЯ(1,1,1)
		|			ТОГДА ГОД(&КонецПериода) - ГОД(СведенияОбИмуществеСрезПоследних.ОсновноеСредство.ДатаВыпускаНедвижимости)
		|		ИНАЧЕ ГОД(&КонецПериода) - ГОД(ВременнаяТаблицаСостоянияОС.ПериодПринятоКУчету)
		|	КОНЕЦ КАК КоличествоЛетЭксплуатации,
		...
		|	ЕСТЬNULL(ЗемельныйНалогОбороты.СуммаЗемельногоНалогаПодОбъектомИмуществаОборот, 0) КАК ЗемНалог
		...
		|	ВременнаяТаблицаИмуществоГотовая.ЗемНалог КАК ЗемНалог,
		|	ЕСТЬNULL(ВременнаяТаблицаСостоянияОС.ПериодПринятоКУчету, ДАТАВРЕМЯ(1,1,1)) КАК ДатаНачисленияНалогаНаИмущество,
		|	ЕСТЬNULL(СостоянияОССрезПоследних.Период, ДАТАВРЕМЯ(1,1,1)) КАК ДатаПоступления
		|ИЗ
		...
		|	ВЫБОР
		|		КОГДА СведенияОТранспортеСрезПоследних.ОсновноеСредство.ДатаВыпускаТранспортногоСредства <> ДАТАВРЕМЯ(1,1,1)
		|			ТОГДА ГОД(&КонецПериода) - ГОД(СведенияОТранспортеСрезПоследних.ОсновноеСредство.ДатаВыпускаТранспортногоСредства)
		|		ИНАЧЕ ГОД(&КонецПериода) - ГОД(ВременнаяТаблицаСостоянияОС.ПериодПринятоКУчету)
		|	КОНЕЦ КАК КоличествоЛетЭксплуатации,
		|	ВременнаяТаблицаСостоянияОС.КоличествоМесяцевЭксплуатации КАК КоличествоМесяцевЭксплуатации,
		|	ВременнаяТаблицаСостоянияОС.ИнвентарныйНомер КАК ИнвентарныйНомер,
		|	ВременнаяТаблицаСостоянияОС.ПервоначальнаяСтоимость КАК ПервоначальнаяСтоимость,
		|	ВременнаяТаблицаСостоянияОС.ПериодПринятоКУчету КАК ПериодПринятоКУчету
		|ПОМЕСТИТЬ ВременнаяТаблицаТранспорт
		|ИЗ
		|	ВременнаяТаблицаСостоянияОС КАК ВременнаяТаблицаСостоянияОС
		...
		|	ВременнаяТаблицаТранспорт.ПериодПринятоКУчету КАК ПериодПринятоКУчету
		...
		|	ВременнаяТаблицаТранспортГотовая.ПериодПринятоКУчету КАК ДатаНачисленияНалогаНаИмущество
		|ИЗ
