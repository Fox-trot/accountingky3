...
Функция ПолучитьДанныеПоНалогуНаИмущество(Организация, НачалоПериода, КонецПериода, ДополнительныйВариантНИ = Ложь, КонецПериодаДляОстатков = Неопределено) Экспорт
	...
	Иначе
		Запрос.Текст =
			...
			//|ПОМЕСТИТЬ ВременнаяТаблицаСостоянияОСПринятоКУчету
			//|ИЗ
			//|	РегистрСведений.СостоянияОС.СрезПоследних(ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(&КонецПериода, МЕСЯЦ), СЕКУНДА, -1), Организация = &Организация) КАК СостоянияОССрезПоследних
			//|ГДЕ
			//|	СостоянияОССрезПоследних.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийОС.ПринятоКУчету)
			//|
			//|ОБЪЕДИНИТЬ ВСЕ
			...
			//|			ДОБАВИТЬКДАТЕ(НАЧАЛОПЕРИОДА(&КонецПериода, МЕСЯЦ), СЕКУНДА, -1),
			//|			Организация = &Организация
			//|				И Период >= НАЧАЛОПЕРИОДА(&КонецПериода, ГОД)) КАК СостоянияОССрезПоследних
			//|ГДЕ
	...
	Запрос.Текст = Запрос.Текст +
		...
		|ГДЕ
		|	СостоянияОССрезПервых.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийОС.СнятоСУчета) ИЛИ СостоянияОССрезПервых.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийОС.Передано)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		//|		КОГДА ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.ГодНачалаРасчета = ГОД(&НачалоПериода)
		//|				И ГОД(ЕСТЬNULL(ВременнаяТаблицаСостоянияОССнятоСУчета.ПериодСнятоСУчета, ДАТАВРЕМЯ(1, 1, 1))) = ГОД(&НачалоПериода)
		//|				И ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.МесяцНачалаРасчета = МЕСЯЦ(ЕСТЬNULL(ВременнаяТаблицаСостоянияОССнятоСУчета.ПериодСнятоСУчета, ДАТАВРЕМЯ(1, 1, 1)))
		//|			ТОГДА 0
		//// Если принято к учету и снято с учета в текущем году.
		//|		КОГДА ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.ГодНачалаРасчета = ГОД(&НачалоПериода)
		//|				И ГОД(ЕСТЬNULL(ВременнаяТаблицаСостоянияОССнятоСУчета.ПериодСнятоСУчета, ДАТАВРЕМЯ(1, 1, 1))) = ГОД(&НачалоПериода)
		//|			ТОГДА ВременнаяТаблицаСостоянияОССнятоСУчета.МесяцВыбытия - ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.МесяцНачалаРасчета - 1
		//// Если принято к учету в текущем году.
		//|		КОГДА ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.ГодНачалаРасчета = ГОД(&НачалоПериода)
		//|			ТОГДА 12 - ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.МесяцНачалаРасчета
		...
		|ГДЕ
		|	СведенияОТранспортеСрезПоследних.ГруппаИмущества = Значение(Справочник.ГруппыИмущества.ГИ4)				
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
