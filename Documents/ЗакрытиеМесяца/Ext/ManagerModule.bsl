...
Функция ПолучитьДанныеПоНалогуНаИмущество(Организация, НачалоПериода, КонецПериода, ДополнительныйВариантНИ = Ложь, КонецПериодаДляОстатков = Неопределено) Экспорт
	...
	Запрос.Текст = Запрос.Текст +
		...
		|	ВЫБОР
		|		КОГДА ПараметрыУчетаОССрезПоследних.ДатаНачисленияНалогаНаИмущество <> ДАТАВРЕМЯ(1,1,1)
		|			ТОГДА ПараметрыУчетаОССрезПоследних.ДатаНачисленияНалогаНаИмущество
		|		ИНАЧЕ ПараметрыУчетаОССрезПоследних.Период
		...
		|	4 КАК Порядок,
		|	ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.ОсновноеСредство КАК ОсновноеСредство,
		|	ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.ПериодПринятоКУчету КАК ПериодПринятоКУчету,
		// Расчет количетва месяцев эксплкатации.		
		|	ВЫБОР
		// Если принято к учету и снято с учета в текущем году в одном и том же месяце.
		|		КОГДА ГОД(ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.ПериодПринятоКУчету) = ГОД(&НачалоПериода)
		|				И ГОД(ЕСТЬNULL(ВременнаяТаблицаСостоянияОССнятоСУчета.ПериодСнятоСУчета, ДАТАВРЕМЯ(1, 1, 1))) = ГОД(&НачалоПериода)
		|				И МЕСЯЦ(ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.ПериодПринятоКУчету) = МЕСЯЦ(ЕСТЬNULL(ВременнаяТаблицаСостоянияОССнятоСУчета.ПериодСнятоСУчета, ДАТАВРЕМЯ(1, 1, 1)))
		|			ТОГДА 1
		// Если принято к учету и снято с учета в текущем году.
		|		КОГДА ГОД(ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.ПериодПринятоКУчету) = ГОД(&НачалоПериода)
		|				И ГОД(ЕСТЬNULL(ВременнаяТаблицаСостоянияОССнятоСУчета.ПериодСнятоСУчета, ДАТАВРЕМЯ(1, 1, 1))) = ГОД(&НачалоПериода)
		|			ТОГДА ВременнаяТаблицаСостоянияОССнятоСУчета.МесяцСнятоСУчета - МЕСЯЦ(ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.ПериодПринятоКУчету)
		// Если принято к учету в текущем году.
		|		КОГДА ГОД(ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.ПериодПринятоКУчету) = ГОД(&НачалоПериода)
		|			ТОГДА 13 - МЕСЯЦ(ВременнаяТаблицаСостоянияОСПринятоКУчетуСДатойНачисленияНИ.ПериодПринятоКУчету)
...
Процедура СформироватьТаблицаНалоговаяАмортизация(ДокументСсылка, СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		//|	ВременнаяТаблицаНалоговаяАмортизация.ПоступлениеПрошлыйГод КАК СуммаПоступленияОстаток,
		...
		//|	ВременнаяТаблицаНалоговаяАмортизация.ВыбытиеПрошлыйГод КАК СуммаВыбытияОстаток,
		...
		|	ВременнаяТаблицаНалоговаяАмортизация.БалансоваяСтоимостьНаКонецГода КАК БалансоваяСтоимостьНаКонецГода,
...
Процедура СформироватьТаблицаПараметрыУчетаОС(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаПараметрыУчетаОС.КоэффициентАмортизации КАК КоэффициентАмортизации,
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ВременнаяТаблицаАмортизацияОС.Сумма < 0
		|			ТОГДА 0
		|		ИНАЧЕ ВременнаяТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации - ВременнаяТаблицаАмортизацияОС.Сумма
		...
		|ИЗ
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаПараметрыУчетаОС КАК ВременнаяТаблицаПараметрыУчетаОС
		|			ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаАмортизацияОС КАК ВременнаяТаблицаАмортизацияОС
		|			ПО ВременнаяТаблицаПараметрыУчетаОС.ОсновноеСредство = ВременнаяТаблицаАмортизацияОС.ОсновноеСредство
		|		ПО (ИСТИНА)";
	РезультатЗапроса = Запрос.Выполнить();
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт 		
	...
	Запрос.Текст =
		...
		|	ПараметрыУчетаОССрезПоследних.КоэффициентАмортизации КАК КоэффициентАмортизации,
