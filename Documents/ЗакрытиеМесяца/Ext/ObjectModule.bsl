...
Процедура ВыполнитьЗакрытиеМесяца(СтруктураПараметров) Экспорт
	...
	Если РасчетСтоимостиПродукции Тогда 
		РасчетСебестоимостиПродукции(ДанныеУчетнойПолитики, Сообщения, Ошибки);
	КонецЕсли;
	...
	Если ЗакрытьВременныеСчетаУчета
		Или ЗакрытьПарныеСчетаУчета Тогда 
		// Попытка проведения- так как данные нужны для закрытия парных и временных счетов.
		Попытка
...
Процедура РасчитатьНалоговуюВыверку(ДанныеУчетнойПолитики)
	...
	// 6. Получение суммы временной разницы (доходы - расходы).
	Запрос = Новый Запрос();
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	1 КАК Порядок,
...
//
Процедура РасчетСебестоимостиПродукции(ДанныеУчетнойПолитики, Сообщения, Ошибки)
	...
	ТекстЗапроса =
		...
		|	ХозрасчетныйОстатки.СуммаОстаток КАК СуммаРасходов
		|ИЗ
