...
Процедура ВыполнитьЗакрытиеМесяца(Знач ПараметрыПроцедуры, АдресХранилища = Неопределено) Экспорт
	...
	Если РасчетАмортизацииОС Тогда
		ДлительныеОперации.СообщитьПрогресс(, НСтр("ru = 'Начисление амортизации ОС и НМА'"));
	...
	Если РасчетПереоценкиВалютныхСредств Тогда 
		ДлительныеОперации.СообщитьПрогресс(, НСтр("ru = 'Переоценка валютных счетов'"));
	...
	Если РасчетСписанияРасходовБудущихПериодов Тогда 
		ДлительныеОперации.СообщитьПрогресс(, НСтр("ru = 'Списание расходов будущих периодов'"));
	...
	Если РасчетНДС Тогда 
		ДлительныеОперации.СообщитьПрогресс(, НСтр("ru = 'Расчет НДС'"));
	...
	Если РасчетСтоимостиПродукцииПоЗаказам Тогда 
		ДлительныеОперации.СообщитьПрогресс(, НСтр("ru = 'Закрытие производства'"));
	...
	Если РасчетНалогаНаИмущество Тогда 
		ДлительныеОперации.СообщитьПрогресс(, НСтр("ru = 'Расчет налога на имущество'"));
	...
	Если ЗакрытьВременныеСчетаУчета Тогда 
		ДлительныеОперации.СообщитьПрогресс(, НСтр("ru = 'Закрытие временных счетов (5999)'"));
	...
	Если РасчетНалогаНаПрибыль Тогда 
		ДлительныеОперации.СообщитьПрогресс(, НСтр("ru = 'Расчет налога на прибыль'"));
	...
	Если РасчетЕдиногоНалога Тогда
		ДлительныеОперации.СообщитьПрогресс(, НСтр("ru = 'Расчет единого налога'"));
	...
	Если ЗакрытьПарныеСчетаУчета Тогда
		ДлительныеОперации.СообщитьПрогресс(, НСтр("ru = 'Закрытие парных счетов'"));
	...
	ПоместитьВоВременноеХранилище(Ссылка, АдресХранилища);
КонецПроцедуры 
...
Процедура РассчитатьНДСНаАвансыДоотгрузку(ДанныеУчетнойПолитики, Ошибки)
	...
	// Остатки по договорам (по которым были движения).
	ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		...
		|				ИЗ
		|					ВременнаяТаблицаСписокКонтрагентов КАК ВременнаяТаблицаСписокКонтрагентов)) КАК ХозрасчетныйОстатки";
	ТекстЗапроса = ТекстЗапроса + ОбщегоНазначенияБПСервер.ТекстРазделителяЗапросовПакета();
...
Процедура РассчитатьНалоговуюВыверку(ДанныеУчетнойПолитики)
	...
	//СуммаКР = СуммаДоходы - СуммаРасходы;
	...
	СуммаДоходы_2_1 		= 0;
	СуммаРасходы_2_1		= 0;
	//СуммаДоходы_2_1_минус 	= 0;
	//СуммаРасходы_2_1_минус	= 0;	
	...
	Если НЕ МассивРезультатовЗапроса[5].Пустой() Тогда 
		...
		Пока ВыборкаДетальныхЗаписей.Следующий() Цикл
			СуммаВременныеРазницы = СуммаВременныеРазницы + ВыборкаДетальныхЗаписей.Сумма;
		КонецЦикла;		
	КонецЕсли;
	...
	// 13.1 Прочие временные разницы - доходы/расходы.
	...
	//СтрокаНалоговойВыверки.КодСтроки            = "2.1";
	//СтрокаНалоговойВыверки.НаименованиеСтроки 	= НСтр("ru = 'Превышение КР'");
	//СтрокаНалоговойВыверки.Сумма				= СуммаКР;	
	...
	//СтрокаНалоговойВыверки.Расходы           	= -СуммаАмортизацияОСБУ;
	...
	СтрокаНалоговойВыверки.Сумма 				= - ПревышениеНормыНаРемонт + (СуммаВР_ПрочиеДоходы - СуммаВР_ПрочиеРасходы);
...
// Рассчитать налоговую выверку до 12 декабря 2019 г. (до версии 3.1.6.17 включительно)
Процедура старыйРассчитатьНалоговуюВыверку(ДанныеУчетнойПолитики)
