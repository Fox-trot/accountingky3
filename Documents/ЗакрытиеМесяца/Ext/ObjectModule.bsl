...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	БухгалтерскийУчетСервер.ОтразитьПараметрыУчетаОС(ДополнительныеСвойства, Движения, Ложь);
...
Процедура РассчитатьАмортизациюОсновныхСредств(ДанныеУчетнойПолитики, Ошибки)
	...
	//	*По месяцам, для всех методов за исключением производственного
	//	*По дням, для всех методов за исключением производственного
	...
	// Данные по выработке за текущий месяц.
	ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		|	5 КАК Порядок,
	...
	Пока Выборка.Следующий() Цикл
		...
		ДоступнаяСуммаАмортизации = Выборка.СтоимостьДляРасчетаАмортизации - Выборка.НакопленныйИзнос;
...
Процедура РасчитатьНалоговуюВыверку(ДанныеУчетнойПолитики)
	...
	СуммаОстаткаНераспределеннаяПрибыль = 0;
	
	// Получение положительного остатка по счету 5300 при сброшенном флаге. 
	Если НЕ ПрибыльБезУчетаНераспределеннойПрибыли Тогда
		...
		Если НЕ РезультатЗапроса.Пустой() Тогда 
			...
			ВыборкаДетальныхЗаписей.Следующий();
			Если ВыборкаДетальныхЗаписей.СуммаОстаток > 0 Тогда 
				СуммаОстаткаНераспределеннаяПрибыль = ВыборкаДетальныхЗаписей.СуммаОстаток;
			КонецЕсли;	
		КонецЕсли;	
	КонецЕсли;
	
	БазаРасчета = ?(СуммаПрибыльСУчетомПостоянныхРазниц > 0, СуммаПрибыльСУчетомПостоянныхРазниц, 0) + СуммаОстаткаНераспределеннаяПрибыль;
	СуммаНалогНаПрибыльБУ = Окр(БазаРасчета * СтавкаНалогаНаПрибыль / 100, 0);
...
Процедура ДобавитьКомментарийПоНалоговойАмортизации(ВидКомментария, КомментарийПоНалоговойАмортизации, СтрокаТабличнойЧасти, ВыборкаДетальныеЗаписи)
	...
	Если ВидКомментария = "КомментарийНА_1" Тогда
		Строка = СтрШаблон(НСтр("ru = 'Балансовая ст-ть = %1, т.е. меньше 0. Налоговая амортизация = 0, Увеличение СОД = %2, Балансовая ст-ть = 0'"), 
	...
	ИначеЕсли ВидКомментария = "КомментарийНА_2" Тогда
		Строка = СтрШаблон(НСтр("ru = 'Балансовая ст-ть = %1, т.е. меньше мин. стоимости группы ОС %2. Налоговая амортизация = %1, Балансовая ст-ть = 0'"), 					
					СтрокаТабличнойЧасти.БалансоваяСтоимостьНаКонецГода,
					МинимальнаяСтоимостьГруппыОС);
	    Строка = Строка + Символы.ПС;	
				
	ИначеЕсли ВидКомментария = "КомментарийНА_3" Тогда
		Строка = СтрШаблон(НСтр("ru = 'Количество ОС = 0. Налоговая амортизация = %1, Балансовая ст-ть = 0'"), 
					СтрокаТабличнойЧасти.БалансоваяСтоимостьНаКонецГода);
	    Строка = Строка + Символы.ПС;
						
	//ИначеЕсли ВидКомментария = "КомментарийНА_4" Тогда
	//	Строка = СтрШаблон(НСтр("ru = 'Балансовая ст-ть итого = 0, Сумма выбытия = %1'"), 
