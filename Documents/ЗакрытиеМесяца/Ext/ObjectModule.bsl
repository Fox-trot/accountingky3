...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Если РасчетСтоимостиПродукции Тогда
		СуммаГотоваяПродукцияКорректировка = 0;
		УчетПроизводства.КорректировкаСебестоимостиПродукции(ДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаРеквизиты, 
			Движения, Отказ, СуммаГотоваяПродукцияКорректировка);		
	КонецЕсли;		
...
Процедура ВыполнитьЗакрытиеМесяца(СтруктураПараметров) Экспорт
	...
	Если РасчетСебестоимостиГотовойПродукции Тогда 
		РасчетСтоимостиПродукцииПоЗаказам();
	КонецЕсли;
	...
	Если ЗакрытьВременныеСчетаУчета
		Или ЗакрытьПарныеСчетаУчета
		Или РасчетСтоимостиПродукции Тогда 
		// Попытка проведения- так как данные нужны для закрытия парных, временных счетов, распеределения накладных расходов.
		Попытка
	...
	Если РасчетСтоимостиПродукции Тогда 
		РасчетСтоимостиПродукцииПоНоменклатурнымГруппам(ДанныеУчетнойПолитики, Сообщения, Ошибки);
	КонецЕсли;
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.Линейный Тогда 
			...
			Иначе 
				Если Выборка.ДатаМодернизации = 0 Тогда 				
					...
					СуммаАмортизации = (Выборка.СтоимостьДляРасчетаАмортизации - Выборка.ЛиквидационнаяСтоимость) / Выборка.СрокСлужбы * Выборка.КоэффициентНачислениеАмортизацииОСПоМесяцам;  // КЭ ***АТ от 22.05.2018 >>
		...
		ИначеЕсли Выборка.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.Производственный Тогда
			...
			Иначе 
				...
				СуммаАмортизации = Выборка.СтоимостьДляРасчетаАмортизации * Выборка.КоличествоПродукции / Выборка.ОбъемПродукции;   // КЭ ***АТ от 22.05.2018 >>																															 
			КонецЕсли;	
		...
		ИначеЕсли Выборка.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.УменьшаемогоОстатка Тогда
			...
			Иначе 
				...
				СуммаАмортизации = (Выборка.СтоимостьДляРасчетаАмортизации - Выборка.НакопленныйИзнос) / Выборка.СрокСлужбы * Выборка.КоэффициентУскорения; // КЭ ***АТ от 22.05.2018 >>
			КонецЕсли;	
		...
		ИначеЕсли Выборка.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.ПоСуммеЧиселЛет Тогда
			...
			СуммаГодовойАмортизации	= Выборка.СтоимостьДляРасчетаАмортизации * ПроцентГодовойАмортизации; // КЭ ***АТ от 22.05.2018 >>
		...
		ДоступнаяСуммаАмортизации = Выборка.СтоимостьДляРасчетаАмортизации - Выборка.ЛиквидационнаяСтоимость - Выборка.НакопленныйИзнос; // КЭ ***АТ от 22.05.2018 >>
...
Процедура РассчитатьНалогНаИмущество(Ошибки)
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе 
			...
			Результат = Стоимость * Выборка.Ставка / 100;
	...
	// Транспорт.
	Выборка = МассивРезультатов[14].Выбрать();
...
Процедура РассчитатьНалоговуюАмортизацию(ДанныеУчетнойПолитики)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ДвижениеОС.ГруппаНУ КАК ГруппаНУ,
		|	ВЫБОР
		|		КОГДА ДвижениеОС.ГруппаНУ = &Группа5
		|			ТОГДА ДвижениеОС.ОсновноеСредство
		|		ИНАЧЕ &ПустоеОС
		|	КОНЕЦ КАК ОсновноеСредство,
		|	ДвижениеОС.ПоступлениеВыбытие КАК ПоступлениеВыбытие,
		|	ДвижениеОС.МесяцПоступления КАК МесяцПоступления,
		|	ДвижениеОС.МесяцВыбытия КАК МесяцВыбытия,
		|	ДвижениеОС.СуммаПоступления КАК СуммаПоступления,
		|	ДвижениеОС.СуммаВыбытия КАК СуммаВыбытия,
		|	ВЫБОР
		...
		|	ВЫБОР
		|		КОГДА ПараметрыУчетаОС.ГруппаНУ = &Группа5
		|			ТОГДА Ремонт.Субконто1
		|		ИНАЧЕ &ПустоеОС
		|	КОНЕЦ,
		|	""Р"",
		...
		|	Ремонт.СуммаОборот
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.Обороты(&ДатаН, &ДатаК, , Счет В (&СчетаЗатратНаРемонтОС), &СубконтоОС, Организация = &Организация, , ) КАК Ремонт
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.ПараметрыУчетаОС.СрезПоследних(&ДатаК, ) КАК ПараметрыУчетаОС
		|		ПО (ПараметрыУчетаОС.ОсновноеСредство = Ремонт.Субконто1)";	
	...
	Запрос.Текст = 
		...
		|	СУММА(СписокГруппИОС.СуммаПоступленияОстатокПГ) КАК СуммаПоступленияОстатокПГ,
		|	СУММА(СписокГруппИОС.СуммаВыбытияОстатокПГ) КАК СуммаВыбытияОстатокПГ,
...
Процедура ВыполнитьЗакрытиеПарныхСчетовУчета()
	...
	ТекстЗапроса =
		...
		|	И НЕ Хозрасчетный.ВидыСубконто.ВидСубконто В(&ВидыСубконто)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
...
//
Процедура РасчетСтоимостиПродукцииПоЗаказам()
...
//
Процедура РасчетСтоимостиПродукцииПоНоменклатурнымГруппам(ДанныеУчетнойПолитики, Сообщения, Ошибки)
