...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	БухгалтерскийУчетСервер.ЗаписатьНаборыЗаписей(ЭтотОбъект, СтруктураИсключений);
...
Процедура РассчитатьАмортизациюОсновныхСредств(ДанныеУчетнойПолитики, Ошибки)
	...
	ТекстЗапроса = 		
		...
		|			ТОГДА (ДЕНЬ(&КонецПериода) - ДЕНЬ(ПараметрыУчетаОССрезПоследних.ДатаВводаВЭксплуатацию) + 1) / ДЕНЬ(&КонецПериода)
		|		ИНАЧЕ 1
		|	КОНЕЦ
		|ИЗ
		|	РегистрСведений.ПараметрыУчетаОС.СрезПоследних(
		...
		|	ТаблицаПараметрыУчетаОС.СтоимостьДляРасчетаАмортизации,
		|	ВЫБОР
	...
	Запрос.УстановитьПараметр("СписокСчетовНакопленнойАмортизации",
		БухгалтерскийУчетВызовСервераПовтИсп.СчетаУчетаНакопленнойАмортизацииОсновныхСредствИНематериальныхАктивов());
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.Линейный Тогда 
			...
			Иначе 
				Если Выборка.ДатаМодернизации = 0 Тогда 				
					СуммаАмортизации = (Выборка.СтоимостьДляРасчетаАмортизации - Выборка.ЛиквидационнаяСтоимость) / Выборка.СрокСлужбы * Выборка.КоэффициентНачислениеАмортизацииОСПоМесяцам;
				...
				Иначе
					...
					СуммаАмортизации = ((Выборка.СтоимостьДляРасчетаАмортизации - Выборка.ЛиквидационнаяСтоимость) / Выборка.СрокСлужбы)
		...
		ИначеЕсли Выборка.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.УменьшаемогоОстатка Тогда
			...
			Иначе 
				СуммаАмортизации = (Выборка.СтоимостьДляРасчетаАмортизации - Выборка.НакопленныйИзнос) / Выборка.СрокСлужбы * Выборка.КоэффициентУскорения;
			КонецЕсли;	
		...
		ИначеЕсли Выборка.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.ПоСуммеЧиселЛет Тогда
			...
			ГодИспользования = Год(Дата) - Год(ДатаВводаВЭксплуатацию);
			ГодовщинаНачалаИспользования = Дата(Год(Дата), Месяц(ДатаВводаВЭксплуатацию), День(ДатаВводаВЭксплуатацию));
		...
		ДоступнаяСуммаАмортизации = Выборка.СтоимостьДляРасчетаАмортизации - Выборка.ЛиквидационнаяСтоимость - Выборка.НакопленныйИзнос;
...
Процедура РассчитатьНДС(ДанныеУчетнойПолитики)
	...
	//
	ТаблицаВсейРеализации	= УчетНДСВызовСервера.ВсеСведенияОРеализации(Организация, НачалоПериода, КонецПериода, ДанныеУчетнойПолитики.НеУчитыватьЗакупкиБезНДС);
...
Процедура РассчитатьСтоимостиПродукцииПоНоменклатурнымГруппам(ДанныеУчетнойПолитики, Сообщения, Ошибки)
	...
	БазаРаспределенияКорректировкиСебестоимости = ДанныеУчетнойПолитики.БазаРаспределенияКорректировкиСебестоимости;
	Если НЕ ЗначениеЗаполнено(БазаРаспределенияКорректировкиСебестоимости) Тогда 
		ТекстСообщения = НСтр("ru = 'Расчет себестоимости продукции. Не заполнен метод распределения корректировки себестоимости готовой продукции.
			|Проверьте заполнение ""Учетной политики по организации"" - Раздел ""Производство"".'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки, Неопределено, ТекстСообщения, Неопределено);                              		
		Возврат;
	КонецЕсли;	
	...
	// Вслучае отсутствия подразделения, вся сумма затрат будет распределена на все подразделения.
...
Процедура РассчитатьНалогНаИмущество(Ошибки)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.КоличествоМесяцевЭксплуатации = 0 Тогда 
			...
			Продолжить;	
		Иначе 
			Стоимость = Выборка.ЖилаяПлощадь * Выборка.ОценочнаяСтоимость * Выборка.РегиональныйКоэффициент * Выборка.ЗональныйКоэффициент * Выборка.ОтраслевойКоэффициент;
	...
	Пока Выборка.Следующий() Цикл
		...
		// Для групп "НесамоходныеТС" и "ДругиеТС" вместо ставки указаны проценты, поэтому при расчете делим на 100.
		// Для расчета суммы налога этих же групп берется балансовая стоимость (ПервоначальнаяСтоимость - НакопленныйИзнос).
...
Процедура РассчитатьНалоговуюАмортизацию(ДанныеУчетнойПолитики)
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		...
		// Проверяем ОС группы 5 больше 20 лет.
		СтрокаТабличнойЧасти.КоличествоЛетЭксплуатации = 0;	
		Если ВыборкаДетальныеЗаписи.ГруппаНУ = Справочники.ГруппыНалоговогоУчетаОС.Группа5 Тогда 
			
			// Проверка указаия даты принятия к учету.
			...
			// Расчет количества лет экплуатации.
			...
			БазаПрямолинейнойАмортизацииПоршлыйГод = 0;
			Если СтрокаТабличнойЧасти.КоличествоЛетЭксплуатации > 20 
				И ВыборкаДетальныеЗаписи.БазаПрямолинейнойАмортизацииПоршлыйГод = 0 Тогда 
				БазаПрямолинейнойАмортизацииПоршлыйГод = СтрокаТабличнойЧасти.БалансоваяСтоимостьНаНачалоГода;
			КонецЕсли;		
			
			// БазаПрямолинейнойАмортизации = База прошлого года + СуммаПоступления + ПревышениеЗатратНаРемонт
			СтрокаТабличнойЧасти.БазаПрямолинейнойАмортизации = БазаПрямолинейнойАмортизацииПоршлыйГод
		...
		ИначеЕсли НЕ СтрокаТабличнойЧасти.ОсновноеСредство.Пустая()
			И СтрокаТабличнойЧасти.СуммаВыбытия > 0 Тогда 
		...
		ДобавитьКомментарийПоНалоговойАмортизации(КомментарийПоНалоговойАмортизации, СтрокаТабличнойЧасти, ВыборкаДетальныеЗаписи);
	КонецЦикла;
КонецПроцедуры
...
Процедура РасчитатьНалоговуюВыверку(ДанныеУчетнойПолитики)
	...
	Запрос.Текст = 
		...
		|	СУММА(ВЫБОР
		|			КОГДА КорректировкиНУОбороты.ДоходРасход = ЗНАЧЕНИЕ(Перечисление.ПризнакСчета.Расходный)
...
Процедура ДобавитьКомментарийПоНалоговойАмортизации(КомментарийПоНалоговойАмортизации, СтрокаТабличнойЧасти, ВыборкаДетальныеЗаписи)
	Строка = "";
	Если СтрокаТабличнойЧасти.ГруппаНУ = Справочники.ГруппыНалоговогоУчетаОС.Группа5 Тогда
		ОсновноеСредство = СтрШаблон(НСтр("ru = ', ОС: %1'"), СтрокаТабличнойЧасти.ОсновноеСредство);
	Иначе
		ОсновноеСредство = "";
	
	КонецЕсли;
	Если СтрокаТабличнойЧасти.БалансоваяСтоимостьНаКонецГода < 0 Тогда
		Строка = СтрШаблон(НСтр("ru = 'Группа НУ: %1%2. Балансовая ст-ть = %3, т.е. меньше 0. Налоговая амортизация = 0, Увеличение СОД = %4, Балансовая ст-ть = 0'"), 
					СтрокаТабличнойЧасти.ГруппаНУ,
					ОсновноеСредство,
	...
	ИначеЕсли СтрокаТабличнойЧасти.БалансоваяСтоимостьНаКонецГода < МинимальнаяСтоимостьГруппыОС Тогда
		Строка = СтрШаблон(НСтр("ru = 'Группа НУ: %1%2. Балансовая ст-ть = %3, т.е. меньше мин. стоимости группы ОС %4. Налоговая амортизация = %3, Балансовая ст-ть = 0'"), 
					СтрокаТабличнойЧасти.ГруппаНУ,
					ОсновноеСредство,					
					СтрокаТабличнойЧасти.БалансоваяСтоимостьНаКонецГода,
	...
	ИначеЕсли ВыборкаДетальныеЗаписи.КоличествоВГруппах = 0 
			И НЕ СтрокаТабличнойЧасти.ГруппаНУ = Справочники.ГруппыНалоговогоУчетаОС.Группа5 Тогда
		Строка = СтрШаблон(НСтр("ru = 'Группа НУ: %1. Количество ОС = 0. Налоговая амортизация = %2, Балансовая ст-ть = 0'"), 
					СтрокаТабличнойЧасти.ГруппаНУ,
					СтрокаТабличнойЧасти.БалансоваяСтоимостьНаКонецГода);
	    Строка = Строка + Символы.ПС;		
...
Процедура ОтразитьНалогНаИмущество(СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ТаблицаДокумента.ЗемНалог КАК ЗемНалог,
...
Процедура ОтразитьНалогНаПрибыль(СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =		
		...
		|	ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ОтсроченныеНалоговыеТребованияНП),
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	&СуммаНалогНаПрибыльБУ - &СуммаНалогНаПрибыль,
		|	&СодержаниеОтсроченныеНалоговыеТребования
		|ИЗ
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|ГДЕ
		|	НЕ &НеФормироватьОтсроченныеПроводки
...
Процедура РаспределениеНакладныхРасходов(МассивРезультатов, ИндексЗапроса, ТаблицаВыпускПродукцииЗатраты, ТаблицаМатериальныеЗатраты, Сообщения, Ошибки)
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		...
		// Накладные расходы, в котрых указано подразделение, распределяются внутри этого подразделения по номенклатурным группам.
		...
		Если БазаРаспределения = Перечисления.БазыРаспределенияКосвенныхРасходов.ПлановаяСебестоимость Тогда 
			...
			Если ЗначениеЗаполнено(ПодразделениеЗатрат) Тогда 
				...
				Для Каждого НайденныяСтрока Из НайденныеСтроки Цикл 
					МассивКоэффициентов.Добавить(НайденныяСтрока.ПлановаяСтоимостьОборот);
				КонецЦикла;
			Иначе 
				...
				КонецЦикла;
			КонецЕсли;
			// Не удалось найти подразделение затрат- распределение по всем подраделениям.
		...
		ИначеЕсли БазаРаспределения = Перечисления.БазыРаспределенияКосвенныхРасходов.ОбъемВыпуска Тогда	
			...
			Если ЗначениеЗаполнено(ПодразделениеЗатрат) Тогда 
				НайденныеСтроки = ТаблицаЗатрат.НайтиСтроки(Новый Структура("Подразделение", ПодразделениеЗатрат));
				Для Каждого НайденныяСтрока Из НайденныеСтроки Цикл 
					МассивКоэффициентов.Добавить(НайденныяСтрока.КоличествоОборот);
				КонецЦикла;
			Иначе 
				Для Каждого СтрокаТаблицы Из ТаблицаЗатрат Цикл 
		...
		ИначеЕсли БазаРаспределения = Перечисления.БазыРаспределенияКосвенныхРасходов.МатериальныеЗатраты Тогда	
			...
			Если ЗначениеЗаполнено(ПодразделениеЗатрат) Тогда 
				НайденныеСтроки = ТаблицаЗатрат.НайтиСтроки(Новый Структура("Подразделение", ПодразделениеЗатрат));
				Для Каждого НайденныяСтрока Из НайденныеСтроки Цикл 
					МассивКоэффициентов.Добавить(НайденныяСтрока.Сумма);
				КонецЦикла;
			Иначе 
				Для Каждого СтрокаТаблицы Из ТаблицаЗатрат Цикл 
...
Функция ПолучитьБазуРаспределения(СтруктураОтбора) 
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		//|	МетодыРаспределенияКосвенныхРасходовОрганизацийСрезПоследних.СчетЗатрат КАК СчетЗатрат,
		...
		|
		|////////////////////////////////////////////////////////////////////////////////
		// 1. По Счету, Подразделению, Статье.
		|ВЫБРАТЬ
		...
		|ГДЕ
		//|	ВременнаяТаблицаМетоды.СчетЗатрат = &СчетЗатрат
		|	ВременнаяТаблицаМетоды.Подразделение = &Подразделение
		//|	И ВременнаяТаблицаМетоды.СтатьяЗатрат = &СтатьяЗатрат
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		//// 2. По Счету, Подразделению, пустной Статье.
		//|ВЫБРАТЬ
		//|	ВременнаяТаблицаМетоды.БазаРаспределения
		//|ИЗ
		//|	ВременнаяТаблицаМетодыРаспределенияКосвенныхРасходовОрганизаций КАК ВременнаяТаблицаМетоды
		//|ГДЕ
		//|	ВременнаяТаблицаМетоды.СчетЗатрат = &СчетЗатрат
		//|	И ВременнаяТаблицаМетоды.Подразделение = &Подразделение
		//|	И ВременнаяТаблицаМетоды.СтатьяЗатрат = ЗНАЧЕНИЕ(Справочник.СтатьиЗатрат.ПустаяСсылка)
		//|
		//|ОБЪЕДИНИТЬ ВСЕ
		//|
		//// 3. По Счету, пустому Подразделению, пустной Статье.
		//|ВЫБРАТЬ
		//|	ВременнаяТаблицаМетоды.БазаРаспределения
		//|ИЗ
		//|	ВременнаяТаблицаМетодыРаспределенияКосвенныхРасходовОрганизаций КАК ВременнаяТаблицаМетоды
		//|ГДЕ
		//|	ВременнаяТаблицаМетоды.СчетЗатрат = &СчетЗатрат
		//|	И ВременнаяТаблицаМетоды.Подразделение = ЗНАЧЕНИЕ(Справочник.ПодразделенияОрганизаций.ПустаяСсылка)
		////|	И ВременнаяТаблицаМетоды.СтатьяЗатрат = ЗНАЧЕНИЕ(Справочник.СтатьиЗатрат.ПустаяСсылка)
		//|
		//|ОБЪЕДИНИТЬ ВСЕ
		//|
		// 4. По пустому Счету, пустому Подразделению, пустной Статье.
		|ВЫБРАТЬ
		|	ВременнаяТаблицаМетоды.БазаРаспределения КАК БазаРаспределения
		|ИЗ
		|	ВременнаяТаблицаМетодыРаспределенияКосвенныхРасходовОрганизаций КАК ВременнаяТаблицаМетоды
		|ГДЕ
		//|	ВременнаяТаблицаМетоды.СчетЗатрат = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ПустаяСсылка)
	...
	//Запрос.УстановитьПараметр("СчетЗатрат", СтруктураОтбора.СчетЗатрат);
	Запрос.УстановитьПараметр("Подразделение", СтруктураОтбора.Подразделение);
	//Запрос.УстановитьПараметр("СтатьяЗатрат", СтруктураОтбора.СтатьяЗатрат);
...
// не все затраты.
// Кроме того, этими "незакрытыми" затратами могут оказаться остатки НЗП, введеные 
