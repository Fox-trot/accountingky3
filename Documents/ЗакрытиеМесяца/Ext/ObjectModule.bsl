...
Процедура ВыполнитьЗакрытиеМесяца(СтруктураПараметров) Экспорт
	...
	Если РасчетПереоценкиВалютныхСредств Тогда 
		...
		Если ЗакрытьПарныеСчетаУчета Тогда 
			// Попытка проведения- так как данные после переоценки нужня для закрытия парных счетов
			Попытка
...
Процедура РассчитатьНалогНаИмущество(Ошибки)
	...
	МассивРезультатов = Документы.ЗакрытиеМесяца.ПолучитьДанныеПоНалогуНаИмущество(Организация, Дата);
