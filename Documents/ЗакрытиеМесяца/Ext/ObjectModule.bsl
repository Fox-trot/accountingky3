...
Процедура РассчитатьАмортизациюОсновныхСредств(ДанныеУчетнойПолитики, Ошибки)
	...
	Пока Выборка.Следующий() Цикл
		...
		ИначеЕсли Выборка.СпособНачисленияАмортизации = Перечисления.СпособыНачисленияАмортизацииОС.УменьшаемогоОстатка Тогда
			...
			Иначе 
				ЧислоЛет = Цел(Выборка.СрокСлужбы / 12);
				
				Если ЧислоЛет <> 0 Тогда
					ГодоваяНормаАО 	 = 100 / ЧислоЛет * Выборка.КоэффициентУскорения;
					СуммаАмортизации = (Выборка.СтоимостьДляРасчетаАмортизации - Выборка.НакопленныйИзнос) * ГодоваяНормаАО;
				КонецЕсли;
