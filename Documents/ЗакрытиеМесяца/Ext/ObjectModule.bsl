...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Дата) Тогда
		...
		Если ЕстьДокументВУказанныйПериод() Тогда
			Дата = Дата('00010101');
		КонецЕсли;		
	КонецЕсли;
...
Процедура РассчитатьАмортизациюОсновныхСредств(УПП)
	...
	// 4. Получение параметров ОС по настройке РасчетАмортизационныхОтчисленийПоМесяцам
	...
	ТекстЗапроса = 		
		...
		|						ТаблицаСостоянияОС.ОсновноеСредство
		|					ИЗ
		|						ТаблицаСостоянияОС КАК ТаблицаСостоянияОС)) КАК ВыработкаОСОбороты
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ОсновноеСредство
		|;
		...
		|						КОНЕЦ - РАЗНОСТЬДАТ(&КонецПериода, ТаблицаСостоянияОС.ДатаПринятияКУчету, ГОД) + 1 = 0
		|						ТОГДА 1
		|					ИНАЧЕ ВЫБОР
		|							КОГДА (ВЫРАЗИТЬ(ТаблицаПараметрыУчетаОС.СрокСлужбы / 12 - 0.5 КАК ЧИСЛО(15, 0))) = 0
		|								ТОГДА 1
		|							ИНАЧЕ ВЫРАЗИТЬ(ТаблицаПараметрыУчетаОС.СрокСлужбы / 12 - 0.5 КАК ЧИСЛО(15, 0))
		|						КОНЕЦ - РАЗНОСТЬДАТ(&КонецПериода, ТаблицаСостоянияОС.ДатаПринятияКУчету, ГОД) + 1
		|				КОНЕЦ / ((ВЫБОР
		|					КОГДА (ВЫРАЗИТЬ(ТаблицаПараметрыУчетаОС.СрокСлужбы / 12 - 0.5 КАК ЧИСЛО(15, 0))) = 0
		|						ТОГДА 1
		|					ИНАЧЕ ВЫРАЗИТЬ(ТаблицаПараметрыУчетаОС.СрокСлужбы / 12 - 0.5 КАК ЧИСЛО(15, 0))
...
Процедура Старый_РассчитатьНДС(УПП)
	...
	Если УПП.УказыватьПризнакЗачетаНДСПриПоступлении Тогда
		...
		МассивСуммРаспределения = Неопределено;
		Если НДСРаспределениеЗачет > 0 Тогда
...
Процедура СтарыйРабочий_РассчитатьНДС(УПП)
	НачалоПериода 	= НачалоМесяца(Дата);
	КонецПериода 	= КонецМесяца(Дата);
	...
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ТабличнаяЧастьДоотгрузка.Сумма КАК Сумма,
	|	ТабличнаяЧастьДоотгрузка.СуммаНДС КАК СуммаНДС,
	|	ТабличнаяЧастьДоотгрузка.СуммаНСП КАК СуммаНСП,
	//|	ТабличнаяЧастьДоотгрузка.СтавкаНДС КАК СтавкаНДС
	|	Значение(Справочник.СтавкиНДС.Стандарт) КАК СтавкаНДС
	|ПОМЕСТИТЬ ТабличнаяЧастьДоотгрузка
	|ИЗ
	|	&Доотгрузка КАК ТабличнаяЧастьДоотгрузка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТабличнаяЧастьАвансы.Сумма КАК Сумма,
	|	ТабличнаяЧастьАвансы.СуммаНДС КАК СуммаНДС,
	|	ТабличнаяЧастьАвансы.СуммаНСП КАК СуммаНСП,
	//|	ТабличнаяЧастьАвансы.СтавкаНДС КАК СтавкаНДС
	|	Значение(Справочник.СтавкиНДС.Стандарт) КАК СтавкаНДС
	|ПОМЕСТИТЬ ТабличнаяЧастьАвансы
	|ИЗ
	|	&Авансы КАК ТабличнаяЧастьАвансы
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ЗакрытиеМесяцаДоотгрузка.Сумма - ЗакрытиеМесяцаДоотгрузка.СуммаНДС - ЗакрытиеМесяцаДоотгрузка.СуммаНСП) КАК СуммаДоотгрузки,
	|	СУММА(ЗакрытиеМесяцаДоотгрузка.СуммаНДС) КАК СуммаНДСДоотгрузки
	|ПОМЕСТИТЬ ВременнаяТаблицаДоотгрузка
	|ИЗ
	|	ТабличнаяЧастьДоотгрузка КАК ЗакрытиеМесяцаДоотгрузка
	|ГДЕ
	|	ЗакрытиеМесяцаДоотгрузка.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Нулевая)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ЗакрытиеМесяцаДоотгрузка.Сумма - ЗакрытиеМесяцаДоотгрузка.СуммаНДС - ЗакрытиеМесяцаДоотгрузка.СуммаНСП) КАК СуммаДоотгрузки,
	|	СУММА(ЗакрытиеМесяцаДоотгрузка.СуммаНДС) КАК СуммаНДСДоотгрузки
	|ПОМЕСТИТЬ ВременнаяТаблицаДоотгрузкаНулеваяСтавка
	|ИЗ
	|	ТабличнаяЧастьДоотгрузка КАК ЗакрытиеМесяцаДоотгрузка
	|ГДЕ
	|	ЗакрытиеМесяцаДоотгрузка.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Нулевая)
	...
	|	СУММА(ЗакрытиеМесяцаДоотгрузка.Сумма - ЗакрытиеМесяцаДоотгрузка.СуммаНДС - ЗакрытиеМесяцаДоотгрузка.СуммаНСП) КАК СуммаДоотгрузки,
	|	СУММА(ЗакрытиеМесяцаДоотгрузка.СуммаНДС) КАК СуммаНДСДоотгрузки
	|ПОМЕСТИТЬ ВременнаяТаблицаДоотгрузкаОсвобожденнаяСтавка
	|ИЗ
	|	ТабличнаяЧастьДоотгрузка КАК ЗакрытиеМесяцаДоотгрузка
	|ГДЕ
	|	ЗакрытиеМесяцаДоотгрузка.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Освобожденная)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ЗакрытиеМесяцаАвансы.Сумма - ЗакрытиеМесяцаАвансы.СуммаНДС - ЗакрытиеМесяцаАвансы.СуммаНСП) КАК СуммаАванса,
	|	СУММА(ЗакрытиеМесяцаАвансы.СуммаНДС) КАК СуммаНДСАванса
	|ПОМЕСТИТЬ ВременнаяТаблицаАвансы
	|ИЗ
	|	ТабличнаяЧастьАвансы КАК ЗакрытиеМесяцаАвансы
	|ГДЕ
	|	ЗакрытиеМесяцаАвансы.СтавкаНДС <> ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Нулевая)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ЗакрытиеМесяцаАвансы.Сумма - ЗакрытиеМесяцаАвансы.СуммаНДС - ЗакрытиеМесяцаАвансы.СуммаНСП) КАК СуммаАванса,
	|	СУММА(ЗакрытиеМесяцаАвансы.СуммаНДС) КАК СуммаНДСАванса
	|ПОМЕСТИТЬ ВременнаяТаблицаАвансыНулеваяСтавка
	|ИЗ
	|	ТабличнаяЧастьАвансы КАК ЗакрытиеМесяцаАвансы
	|ГДЕ
	|	ЗакрытиеМесяцаАвансы.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Нулевая)
	...
	|	СУММА(ЗакрытиеМесяцаАвансы.Сумма - ЗакрытиеМесяцаАвансы.СуммаНДС - ЗакрытиеМесяцаАвансы.СуммаНСП) КАК СуммаАванса,
	|	СУММА(ЗакрытиеМесяцаАвансы.СуммаНДС) КАК СуммаНДСАванса
	|ПОМЕСТИТЬ ВременнаяТаблицаАвансыОсвобожденнаяСтавка
	|ИЗ
	|	ТабличнаяЧастьАвансы КАК ЗакрытиеМесяцаАвансы
	|ГДЕ
	|	ЗакрытиеМесяцаАвансы.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Освобожденная)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ВременнаяТаблицаСчетаФактурыВыписанные.СуммаБезНДС) КАК СуммаПродажОбщая
	|ПОМЕСТИТЬ ВременнаяВсяРеализация
	|ИЗ
	|	ВременнаяТаблицаСчетаФактурыВыписанные КАК ВременнаяТаблицаСчетаФактурыВыписанные
	|ГДЕ
	|	ВременнаяТаблицаСчетаФактурыВыписанные.ДатаПоставки МЕЖДУ &НачалоМесяца И &КонецМесяца
	|	И ВременнаяТаблицаСчетаФактурыВыписанные.Организация = &Организация
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ЕСТЬNULL(ВременнаяВсяРеализация.СуммаПродажОбщая, 0) + ЕСТЬNULL(ВременнаяТаблицаАвансы.СуммаАванса, 0) - ЕСТЬNULL(ВременнаяТаблицаДоотгрузка.СуммаДоотгрузки, 0)) КАК СуммаПродажОбщая
	|ИЗ
	|	ВременнаяВсяРеализация КАК ВременнаяВсяРеализация
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВременнаяТаблицаДоотгрузка КАК ВременнаяТаблицаДоотгрузка
	|		ПО (ИСТИНА)
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВременнаяТаблицаАвансы КАК ВременнаяТаблицаАвансы
	|		ПО (ИСТИНА)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ВременнаяТаблицаСчетаФактурыВыписанные.СуммаБезНДС) КАК СуммаПродажОсвобожденная
	|ИЗ
	|	ВременнаяТаблицаСчетаФактурыВыписанные КАК ВременнаяТаблицаСчетаФактурыВыписанные
	|ГДЕ
	|	ВременнаяТаблицаСчетаФактурыВыписанные.ДатаПоставки МЕЖДУ &НачалоМесяца И &КонецМесяца
	|	И ВременнаяТаблицаСчетаФактурыВыписанные.Организация = &Организация
	|	И ВременнаяТаблицаСчетаФактурыВыписанные.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Освобожденная)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ВременнаяТаблицаДоотгрузка.СуммаНДСДоотгрузки) КАК СуммаНДСДоотгрузки
	|ИЗ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СУММА(ВременнаяТаблицаАвансы.СуммаНДСАванса) КАК СуммаНДСАванса
	|ИЗ
	|	ВременнаяТаблицаАвансы КАК ВременнаяТаблицаАвансы";
	
	Запрос.Параметры.Вставить("Ссылка", 	Ссылка);
	Запрос.Параметры.Вставить("Доотгрузка", Доотгрузка.Выгрузить());
	Запрос.Параметры.Вставить("Авансы", 	Авансы.Выгрузить());
	РезультатЗапроса = Запрос.ВыполнитьПакет();
	
	Выборка = РезультатЗапроса[10].Выбрать();
	СуммаПродажОбщая = 0;
	Если Выборка.Следующий() И Выборка.СуммаПродажОбщая <> Null Тогда
		СуммаПродажОбщая = Выборка.СуммаПродажОбщая;
	КонецЕсли;
	
	Выборка = РезультатЗапроса[11].Выбрать();
	СуммаПродажОсвобожденная = 0;
	Если Выборка.Следующий() И Выборка.СуммаПродажОсвобожденная <> Null Тогда
		СуммаПродажОсвобожденная = Выборка.СуммаПродажОсвобожденная;
	КонецЕсли;
	
	Выборка = РезультатЗапроса[12].Выбрать();
	НДСДоотгрузки = 0;
	Если Выборка.Следующий() И Выборка.СуммаНДСДоотгрузки <> Null Тогда
		НДСДоотгрузки = Выборка.СуммаНДСДоотгрузки;
	КонецЕсли;	
	
	Выборка = РезультатЗапроса[13].Выбрать();
	НДСНаАвансы = 0;
	Если Выборка.Следующий() И Выборка.СуммаНДСАванса <> Null Тогда
		НДСНаАвансы = Выборка.СуммаНДСАванса;
	КонецЕсли;	
	
	//КоэффициентОсвобожденныхПоставок = 0;	
	Если НЕ УПП.ВводКоэффициентаОсвобожденныхПоставокВЗакрытииМесяца Тогда
		Если СуммаПродажОбщая = 0 Тогда
			КоэффициентОсвобожденныхПоставок = 0;
		Иначе
			КоэффициентОсвобожденныхПоставок = Окр(СуммаПродажОсвобожденная / СуммаПродажОбщая, 4);
		КонецЕсли;			
	...
	|	""050"" КАК Строка,
	|	""ОБЛАГАЕМЫЕ ПОСТАВКИ НА ТЕРРИТОРИИ КР"" КАК Содержание,
	|	СУММА(ЕСТЬNULL(СчетаФактурыВыписанные.СуммаБезНДС, 0)) КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблица050СГруппировкой
	|ИЗ
	|	ВременнаяТаблицаСчетаФактурыВыписанные КАК СчетаФактурыВыписанные
	|ГДЕ
	|	НЕ СчетаФактурыВыписанные.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Освобожденная)
	|	И НЕ СчетаФактурыВыписанные.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Нулевая)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	""050"" КАК Строка,
	|	""ОБЛАГАЕМЫЕ ПОСТАВКИ НА ТЕРРИТОРИИ КР"" КАК Содержание,
	|	ЕСТЬNULL(ВременнаяТаблица050СГруппировкой.Сумма, 0) + ЕСТЬNULL(ВременнаяТаблицаАвансы.СуммаАванса, 0) - ЕСТЬNULL(ВременнаяТаблицаДоотгрузка.СуммаДоотгрузки, 0) КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблица050
	|ИЗ
	|	ВременнаяТаблица050СГруппировкой КАК ВременнаяТаблица050СГруппировкой
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВременнаяТаблицаАвансы КАК ВременнаяТаблицаАвансы
	|		ПО (ИСТИНА)
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВременнаяТаблицаДоотгрузка КАК ВременнаяТаблицаДоотгрузка
	|		ПО (ИСТИНА)
	...
	|	""051"" КАК Строка,
	|	""ПОСТАВКИ, ОБЛАГАЕМЫЕ ПО НУЛЕВОЙ СТАВКЕ"" КАК Содержание,
	|	СУММА(СчетаФактурыВыписанные.СуммаБезНДС) КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблица051СГруппировкой
	|ИЗ
	|	ВременнаяТаблицаСчетаФактурыВыписанные КАК СчетаФактурыВыписанные
	|ГДЕ
	|	СчетаФактурыВыписанные.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Нулевая)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	""051"" КАК Строка,
	|	""ПОСТАВКИ, ОБЛАГАЕМЫЕ ПО НУЛЕВОЙ СТАВКЕ"" КАК Содержание,
	|	ЕСТЬNULL(ВременнаяТаблица051СГруппировкой.Сумма, 0) + ЕСТЬNULL(ВременнаяТаблицаАвансыНулеваяСтавка.СуммаАванса, 0) - ЕСТЬNULL(ВременнаяТаблицаДоотгрузкаНулеваяСтавка.СуммаДоотгрузки, 0) КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблица051
	|ИЗ
	|	ВременнаяТаблица051СГруппировкой КАК ВременнаяТаблица051СГруппировкой
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВременнаяТаблицаАвансыНулеваяСтавка КАК ВременнаяТаблицаАвансыНулеваяСтавка
	|		ПО (ИСТИНА)
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВременнаяТаблицаДоотгрузкаНулеваяСтавка КАК ВременнаяТаблицаДоотгрузкаНулеваяСтавка
	|		ПО (ИСТИНА)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	...
	|	""ОСВОБОЖДЕННЫЕ И НЕОБЛАГАЕМЫЕ ПОСТАВКИ"" КАК Содержание,
	|	СУММА(ЕСТЬNULL(СчетаФактурыВыписанные.СуммаБезНДС, 0)) КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблица053СГруппировкой
	|ИЗ
	|	ВременнаяТаблицаСчетаФактурыВыписанные КАК СчетаФактурыВыписанные
	|ГДЕ
	|	СчетаФактурыВыписанные.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Освобожденная)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	""053"" КАК Строка,
	|	""ОСВОБОЖДЕННЫЕ И НЕОБЛАГАЕМЫЕ ПОСТАВКИ"" КАК Содержание,
	|	ЕСТЬNULL(ВременнаяТаблица053СГруппировкой.Сумма, 0) + ЕСТЬNULL(ВременнаяТаблицаАвансыОсвобожденнаяСтавка.СуммаАванса, 0) - ЕСТЬNULL(ВременнаяТаблицаДоотгрузкаОсвобожденнаяСтавка.СуммаДоотгрузки, 0) КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблица053
	|ИЗ
	|	ВременнаяТаблица053СГруппировкой КАК ВременнаяТаблица053СГруппировкой
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВременнаяТаблицаАвансыОсвобожденнаяСтавка КАК ВременнаяТаблицаАвансыОсвобожденнаяСтавка
	|		ПО (ИСТИНА)
	|		ПОЛНОЕ СОЕДИНЕНИЕ ВременнаяТаблицаДоотгрузкаОсвобожденнаяСтавка КАК ВременнаяТаблицаДоотгрузкаОсвобожденнаяСтавка
	|		ПО (ИСТИНА)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВременнаяТаблица050.Сумма КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблица054
	|ИЗ
	|	ВременнаяТаблица050 КАК ВременнаяТаблица050
	|
	...
	|	""050"" КАК Строка,
	|	""ОБЛАГАЕМЫЕ ПОСТАВКИ НА ТЕРРИТОРИИ КР"" КАК Содержание,
	|	ЕСТЬNULL(ВременнаяТаблица050.Сумма, 0) КАК Сумма
	|ИЗ
	|	ВременнаяТаблица050 КАК ВременнаяТаблица050
	|;
	|
	...
	|	СведенияОПоступлении.СтавкаНДС КАК СтавкаНДС,
	|	ВЫБОР
	|		КОГДА СведенияОПоступлении.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|			ТОГДА СведенияОПоступлении.СуммаБезНДС + СведенияОПоступлении.СуммаНДС
	|		ИНАЧЕ СведенияОПоступлении.СуммаБезНДС
	|	КОНЕЦ КАК СуммаБезНДС,
	|	ВЫБОР
	|		КОГДА СведенияОПоступлении.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|			ТОГДА 0
	|		ИНАЧЕ СведенияОПоступлении.СуммаНДС
	|	КОНЕЦ КАК СуммаНДС,
	...
	|ПОМЕСТИТЬ ВременнаяТаблицаПервичная
	|ИЗ
	...
	|	И (НЕ СведенияОПоступлении.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.БезНДС)
	|			ИЛИ СведенияОПоступлении.Патентник <> """")
	...
	|	""055"" КАК Строка,
	|	""ПРИОБРЕТЕННЫЕ МАТЕРИАЛЬНЫЕ РЕСУРСЫ В КР"" КАК Содержание,
	|	СУММА(ЕСТЬNULL(ВременнаяТаблицаПервичная.СуммаБезНДС, 0)) КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблица055
	|ИЗ
	|	ВременнаяТаблицаПервичная КАК ВременнаяТаблицаПервичная
	|ГДЕ
	|	НЕ ВременнаяТаблицаПервичная.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ИмпортЭкспорт)
	|	И НЕ ВременнаяТаблицаПервичная.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ЕАЭС)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВременнаяТаблицаПервичная КАК ВременнаяТаблицаПервичная
	|ГДЕ
	|	ВременнаяТаблицаПервичная.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ИмпортЭкспорт)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВременнаяТаблицаПервичная КАК ВременнаяТаблицаПервичная
	|ГДЕ
	|	ВременнаяТаблицаПервичная.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ЕАЭС)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВременнаяТаблицаПервичная.СуммаНДСЗачет КАК СуммаНДСЗачет
	|ИЗ
	|	ВременнаяТаблицаПервичная КАК ВременнаяТаблицаПервичная";
	...
	Выборка = РезультатЗапроса[9].Выбрать();
	Выборка.Следующий();
	СуммаНДСЗачет = Выборка.СуммаНДСЗачет;	
	...
	|	""059"" КАК Строка,
	|	""ОБЩАЯ СУММА НДС ЗА ПРОИЗВЕДЕННЫЕ ПОСТАВКИ ( =050 х ставку / 100%)"" КАК Содержание,
	|	СУММА(ЕСТЬNULL(ВременнаяТаблицаСчетаФактурыВыписанные.СуммаНДС, 0)) КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблица059
	|ИЗ
	|	ВременнаяТаблицаСчетаФактурыВыписанные КАК ВременнаяТаблицаСчетаФактурыВыписанные
	|
	...
	|	СУММА(ЕСТЬNULL(ВременнаяТаблицаАвансы.СуммаНДСАванса, 0))
	|ИЗ
	|	ВременнаяТаблицаАвансы КАК ВременнаяТаблицаАвансы
	|
	|ОБЪЕДИНИТЬ ВСЕ
	...
	|	""059"",
	|	""ОБЩАЯ СУММА НДС ЗА ПРОИЗВЕДЕННЫЕ ПОСТАВКИ ( =050 х ставку / 100%)"",
	|	СУММА(-ЕСТЬNULL(ВременнаяТаблицаДоотгрузка.СуммаНДСДоотгрузки, 0))
	|ИЗ
	|	ВременнаяТаблицаДоотгрузка КАК ВременнаяТаблицаДоотгрузка
	|;
	|
	...
	|	""065"" КАК Строка,
	|	""В ТОМ ЧИСЛЕ НДС ЗА ПРИОБРЕТЕННЫЕ МАТЕРИАЛЬНЫЕ РЕСУРСЫ,  ИСПОЛЬЗУЕМЫЕ ДЛЯ ПОСТАВОК С НУЛЕВОЙ СТАВКОЙ"" КАК Содержание,
	|	СУММА(ЕСТЬNULL(ВременнаяТаблицаПервичная.СуммаНДС, 0)) КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблица065
	|ИЗ
	|	ВременнаяТаблицаПервичная КАК ВременнаяТаблицаПервичная
	|ГДЕ
	|	(ВременнаяТаблицаПервичная.ЗачетНДС = ЗНАЧЕНИЕ(Перечисление.ВидыЗачетаНДС.Зачет)
	|			ИЛИ ВременнаяТаблицаПервичная.ЗачетНДС = ЗНАЧЕНИЕ(перечисление.ВидыЗачетаНДС.ПустаяСсылка))
	|	И ВременнаяТаблицаПервичная.СтавкаНДС = ЗНАЧЕНИЕ(Справочник.СтавкиНДС.Нулевая)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВременнаяТаблица059.Строка КАК Строка,
	|	ВременнаяТаблица059.Содержание КАК Содержание,
	|	СУММА(ВременнаяТаблица059.Сумма) КАК Сумма
	|ПОМЕСТИТЬ ВременнаяТаблица059СГруппировкойСумм
	|ИЗ
	|	ВременнаяТаблица059 КАК ВременнаяТаблица059
	|
	|СГРУППИРОВАТЬ ПО
	|	ВременнаяТаблица059.Содержание,
	|	ВременнаяТаблица059.Строка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	""059"" КАК Строка,
	|	""ОБЩАЯ СУММА НДС ЗА ПРОИЗВЕДЕННЫЕ ПОСТАВКИ ( =050 х ставку / 100%)"" КАК Содержание,
	|	ЕСТЬNULL(ВременнаяТаблица059СГруппировкойСумм.Сумма, 0) КАК Сумма
	|ИЗ
	|	ВременнаяТаблица059СГруппировкойСумм КАК ВременнаяТаблица059СГруппировкойСумм
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	// 065 - Начало. Продолжение после 064
	Выборка065 = РезультатЗапроса[4].Выбрать();
	Выборка065.Следующий();	
	
	// Строки 060 - 063 	
	СуммаНДСРаспределениеИмпорт		= 0;
	СуммаНДСРаспределениеИмпортТС	= 0;
	СуммаНДСРаспределениеКР 		= 0;
	СуммаНДССебестоимостьИмпорт 	= 0;
	СуммаНДССебестоимостьИмпортТС	= 0;
	СуммаНДССебестоимостьКР			= 0;
	СуммаНДСИмпорт					= 0;
	СуммаНДСИмпортТС				= 0;
	СуммаНДСКР						= 0;
	
	СуммаНДСНеРазрешенныйТС 	= 0;
	СуммаНДСНеРазрешенныйИмпорт = 0;
    СуммаНДСНеРазрешенныйКР 	= 0;		
	
	Если УПП.УказыватьПризнакЗачетаНДСПриПоступлении Тогда	
		Запрос.Текст =				
		"ВЫБРАТЬ
		|	""Распределение"" КАК Содержание,
		|	ВременнаяТаблицаПервичная.ПризнакСтраны КАК ПризнакСтраны,
		|	ВременнаяТаблицаПервичная.ЗачетНДС КАК ЗачетНДС,
		|	ВременнаяТаблицаПервичная.СуммаНДС КАК Сумма
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	СУММА(ЕСТЬNULL(ВременнаяТаблицаРаспределение.Сумма, 0)) КАК Сумма
		|ИЗ
		|	ВременнаяТаблицаРаспределение КАК ВременнаяТаблицаРаспределение
		|ГДЕ
		|	ВременнаяТаблицаРаспределение.ЗачетНДС = ЗНАЧЕНИЕ(Перечисление.ВидыЗачетаНДС.Распределение)
		|	И ВременнаяТаблицаРаспределение.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ИмпортЭкспорт)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ВременнаяТаблицаРаспределение.ЗачетНДС = ЗНАЧЕНИЕ(Перечисление.ВидыЗачетаНДС.Распределение)
		|	И ВременнаяТаблицаРаспределение.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ЕАЭС)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ВременнаяТаблицаРаспределение.ЗачетНДС = ЗНАЧЕНИЕ(Перечисление.ВидыЗачетаНДС.Распределение)
		|	И НЕ ВременнаяТаблицаРаспределение.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ИмпортЭкспорт)
		|	И НЕ ВременнаяТаблицаРаспределение.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ЕАЭС)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВременнаяТаблицаРаспределение КАК ВременнаяТаблицаРаспределение
		|ГДЕ
		|	ВременнаяТаблицаРаспределение.ЗачетНДС = ЗНАЧЕНИЕ(Перечисление.ВидыЗачетаНДС.Себестоимость)
		|	И ВременнаяТаблицаРаспределение.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ИмпортЭкспорт)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		...
		|	ВременнаяТаблицаРаспределение КАК ВременнаяТаблицаРаспределение
		|ГДЕ
		|	ВременнаяТаблицаРаспределение.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ИмпортЭкспорт)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВременнаяТаблицаРаспределение КАК ВременнаяТаблицаРаспределение
		|ГДЕ
		|	ВременнаяТаблицаРаспределение.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ЕАЭС)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВременнаяТаблицаРаспределение КАК ВременнаяТаблицаРаспределение
		|ГДЕ
		|	НЕ ВременнаяТаблицаРаспределение.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ИмпортЭкспорт)
		|	И НЕ ВременнаяТаблицаРаспределение.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ЕАЭС)";		
		...
		Выборка = РезультатЗапроса[1].Выбрать();
		Если Выборка.Следующий() Тогда
			СуммаНДСРаспределениеИмпорт = ?(Выборка.Сумма = Null, 0, Выборка.Сумма);
		КонецЕсли;
		
		Выборка = РезультатЗапроса[2].Выбрать();
		Если Выборка.Следующий() Тогда
			СуммаНДСРаспределениеИмпортТС = ?(Выборка.Сумма = Null, 0, Выборка.Сумма);
		КонецЕсли;
		
		Выборка = РезультатЗапроса[3].Выбрать();
		Если Выборка.Следующий() Тогда
			СуммаНДСРаспределениеКР = ?(Выборка.Сумма = Null, 0, Выборка.Сумма);
		КонецЕсли;
		
		Выборка = РезультатЗапроса[4].Выбрать();
		Если Выборка.Следующий() Тогда
			СуммаНДССебестоимостьИмпорт = ?(Выборка.Сумма = Null, 0, Выборка.Сумма);
		КонецЕсли;
		
		Выборка = РезультатЗапроса[5].Выбрать();
		Если Выборка.Следующий() Тогда
			СуммаНДССебестоимостьИмпортТС = ?(Выборка.Сумма = Null, 0, Выборка.Сумма);
		КонецЕсли;
		
		Выборка = РезультатЗапроса[6].Выбрать();
		Если Выборка.Следующий() Тогда
			СуммаНДССебестоимостьКР = ?(Выборка.Сумма = Null, 0, Выборка.Сумма);
		КонецЕсли;
		
		Выборка = РезультатЗапроса[7].Выбрать();
		Если Выборка.Следующий() Тогда
			СуммаНДСИмпорт = ?(Выборка.Сумма = Null, 0, Выборка.Сумма);
		КонецЕсли;
		
		Выборка = РезультатЗапроса[8].Выбрать();
		Если Выборка.Следующий() Тогда
			СуммаНДСИмпортТС = ?(Выборка.Сумма = Null, 0, Выборка.Сумма);
		КонецЕсли;
				
		Выборка = РезультатЗапроса[9].Выбрать();
		Если Выборка.Следующий() Тогда
			СуммаНДСКР 		= ?(Выборка.Сумма = Null, 0, Выборка.Сумма);
		КонецЕсли;
		
		СуммаНДСНеРазрешенныйТС 	= Окр(СуммаНДССебестоимостьИмпортТС + СуммаНДСРаспределениеИмпортТС * КоэффициентОсвобожденныхПоставок, 2);
		СуммаНДСНеРазрешенныйИмпорт = Окр(СуммаНДССебестоимостьИмпорт 	+ СуммаНДСРаспределениеИмпорт	* КоэффициентОсвобожденныхПоставок, 2);
	    СуммаНДСНеРазрешенныйКР 	= Окр(СуммаНДССебестоимостьКР 		+ СуммаНДСРаспределениеКР 		* КоэффициентОсвобожденныхПоставок, 2);			
		
		// 060
		СтрокаТабличнойЧасти = НДС.Добавить();
		СтрокаТабличнойЧасти.Строка 	= 	"060";		
		СтрокаТабличнойЧасти.Содержание = 	"НДС ЗА ПРИОБРЕТЕННЫЕ МАТЕРИАЛЬНЫЕ РЕСУРСЫ, ПОДЛЕЖАЩИЙ ЗАЧЕТУ";
		СтрокаТабличнойЧасти.Сумма 		= 	СуммаНДСКР - СуммаНДСНеРазрешенныйКР;
		НДС_060 = СтрокаТабличнойЧасти.Сумма;
		// 061
		СтрокаТабличнойЧасти = НДС.Добавить();
		СтрокаТабличнойЧасти.Строка 	= 	"061";		
		СтрокаТабличнойЧасти.Содержание = 	"НДС НА ИМПОРТ, ПОДЛЕЖАЩИЙ ЗАЧЕТУ";
		СтрокаТабличнойЧасти.Сумма 		= 	СуммаНДСИмпорт + СуммаНДСИмпортТС - СуммаНДСНеРазрешенныйИмпорт - СуммаНДСНеРазрешенныйТС;		
		НДС_061 = СтрокаТабличнойЧасти.Сумма;
		НДСИмпорт	= СтрокаТабличнойЧасти.Сумма;
		// 062
		СтрокаТабличнойЧасти = НДС.Добавить();
		СтрокаТабличнойЧасти.Строка 	= 	"062";		
		СтрокаТабличнойЧасти.Содержание = 	"В ТОМ ЧИСЛЕ ИЗ ЕВРАЗИЙСКОГО ЭКОНОМИЧЕСКОГО СОЮЗА";
		СтрокаТабличнойЧасти.Сумма 		= 	СуммаНДСИмпортТС - СуммаНДСНеРазрешенныйТС;
		НДС_062 = СтрокаТабличнойЧасти.Сумма;
		// 063
		СтрокаТабличнойЧасти = НДС.Добавить();
		СтрокаТабличнойЧасти.Строка 	= 	"063";		
		СтрокаТабличнойЧасти.Содержание = 	"НДС ЗА ПРИОБРЕТЕННЫЕ МАТЕРИАЛЬНЫЕ РЕСУРСЫ, НЕ ПОДЛЕЖАЩИЙ ЗАЧЕТУ";
		СтрокаТабличнойЧасти.Сумма 		= 	СуммаНДСНеРазрешенныйТС + СуммаНДСНеРазрешенныйИмпорт + СуммаНДСНеРазрешенныйКР;
		
		НДСНеЗачет 		= 	СтрокаТабличнойЧасти.Сумма;		
		НДСРаспределениеНеЗачет = НДСНеЗачет - СуммаНДССебестоимостьКР - СуммаНДССебестоимостьИмпорт - СуммаНДССебестоимостьИмпортТС; 
	...
	Иначе
		...
		|	""060"" КАК Строка,
		|	""НДС ЗА ПРИОБРЕТЕННЫЕ МАТЕРИАЛЬНЫЕ РЕСУРСЫ, ПОДЛЕЖАЩИЙ ЗАЧЕТУ"" КАК Содержание,
		|	СУММА(ЕСТЬNULL(ВременнаяТаблицаПервичная.СуммаНДСЗачет, 0)) КАК Сумма
		...
		НДСНеЗачет 	= 	НДСКР_НеЗачет + НДСТолькоИмпорт_НеЗачет + НДСЕАЭС_НеЗачет;
		НДСРаспределениеНеЗачет = НДСНеЗачет;
	...
	СтрокаТабличнойЧасти = НДС.Добавить();
	СтрокаТабличнойЧасти.Строка 	= 	"067";		
	СтрокаТабличнойЧасти.Содержание = 	"НАЛОГОВОЕ ОБЯЗАТЕЛЬСТВО ПО ПРОМЫШЛЕННОЙ ПЕРЕРАБОТКЕ СЕЛЬСКОХОЗЯЙСТВЕННОЙ ПРОДУКЦИИ, ПОДЛЕЖАЩЕЕ УМЕНЬШЕНИЮ НА 35 % (УКАЖИТЕ ЗНАК МИНУС, ЕСЛИ ЗНАЧЕНИЕ ОТРИЦАТЕЛЬНОЕ)";
	
	// 068
	СтрокаТабличнойЧасти = НДС.Добавить();
	СтрокаТабличнойЧасти.Строка 	= 	"068";		
	СтрокаТабличнойЧасти.Содержание = 	"НАЛОГОВОЕ ОБЯЗАТЕЛЬСТВО ВСЕГО (=066-067) (УКАЖИТЕ ЗНАК МИНУС, ЕСЛИ ЗНАЧЕНИЕ ОТРИЦАТЕЛЬНОЕ)";
	СтрокаТабличнойЧасти.Сумма 		= 	ВсегоНДСПоставки - НДС_064;
	...
	СальдоНДС = - ВыбНДС.Итог("СуммаОстатокДт") + ВыбНДС.Итог("СуммаОстатокКт") + НДСНаАвансы + (-НДСДоотгрузки); 
	...
	Если НЕ (Сальдо1 = 0 И Сальдо2 = 0 
		...
		СтрокаТабличнойЧасти.Содержание            = "Зачет сальдо НДС к возмещению";
	...
	Если НДСРаспределениеНеЗачет > 0 Тогда
		Если Справочники.СпискиСчетов.РасходыПоНезачтенномуНДС.СписокСчетов.Количество() > 0 Тогда
			РасходыПоНДСнеПринимаемомуКЗачету = Справочники.СпискиСчетов.РасходыПоНезачтенномуНДС.СписокСчетов[0].Счет;
		Иначе
			РасходыПоНДСнеПринимаемомуКЗачету = ПланыСчетов.Хозрасчетный.РасходыПоНДСнеПринимаемомуКЗачету;
		КонецЕсли;
		
		СтрокаТабличнойЧасти = КорректировкаНДС.Добавить();
		СтрокаТабличнойЧасти.Содержание            = "НДС не разрешенный к зачету";
...
Процедура РассчитатьНДС(УПП)
	...
	//
	ПараметрыРасчета = Новый Структура;
	ПараметрыРасчета.Вставить("ВводКоэффициентаОсвобожденныхПоставокВЗакрытииМесяца",	УПП.ВводКоэффициентаОсвобожденныхПоставокВЗакрытииМесяца);
	...
	ПараметрыРасчета.Вставить("УказыватьПризнакЗачетаНДСПриПоступлении",				УПП.УказыватьПризнакЗачетаНДСПриПоступлении);
	...
	СтрокаТабличнойЧасти.Содержание = "ОБЛАГАЕМЫЕ ПОСТАВКИ НА ТЕРРИТОРИИ КР";
	...
	СтрокаТабличнойЧасти.Содержание = "ПОСТАВКИ, ОБЛАГАЕМЫЕ ПО НУЛЕВОЙ СТАВКЕ";
	...
	СтрокаТабличнойЧасти.Содержание = "В ТОМ ЧИСЛЕ В ЕВРАЗИЙСКИЙ ЭКОНОМИЧЕСКИЙ СОЮЗ";
	...
	СтрокаТабличнойЧасти.Содержание = "ОСВОБОЖДЕННЫЕ И НЕОБЛАГАЕМЫЕ ПОСТАВКИ";
	...
	СтрокаТабличнойЧасти.Содержание = "ОБЩАЯ СТОИМОСТЬ  ПОСТАВОК (=050+051+053)";
	...
	СтрокаТабличнойЧасти.Содержание = "ПРИОБРЕТЕННЫЕ МАТЕРИАЛЬНЫЕ РЕСУРСЫ В КР";
	...
	СтрокаТабличнойЧасти.Содержание = "ИМПОРТ МАТЕРИАЛЬНЫХ РЕСУРСОВ В КР";
	...
	СтрокаТабличнойЧасти.Содержание = "В ТОМ ЧИСЛЕ ИЗ ГОСУДАРСТВ-ЧЛЕНОВ ЕВРАЗИЙСКОГО ЭКОНОМИЧЕСКОГО СОЮЗА";
	...
	СтрокаТабличнойЧасти.Содержание = "ОБЩАЯ СТОИМОСТЬ ПРИОБРЕТЕННЫХ МАТЕРИАЛЬНЫХ РЕСУРСОВ (=055+056)";
	...
	СтрокаТабличнойЧасти.Содержание = "ОБЩАЯ СУММА НДС ЗА ПРОИЗВЕДЕННЫЕ ПОСТАВКИ ( =050 х ставку / 100%)";
	...
	СтрокаТабличнойЧасти = НДС.Добавить();
	СтрокаТабличнойЧасти.Строка = "065";	
	СтрокаТабличнойЧасти.Содержание = "В ТОМ ЧИСЛЕ НДС ЗА ПРИОБРЕТЕННЫЕ МАТЕРИАЛЬНЫЕ РЕСУРСЫ, ИСПОЛЬЗУЕМЫЕ ДЛЯ ПОСТАВОК С НУЛЕВОЙ СТАВКОЙ";
	...
	СтрокаТабличнойЧасти = НДС.Добавить();
	СтрокаТабличнойЧасти.Строка = "068";	
	СтрокаТабличнойЧасти.Содержание = "НАЛОГОВОЕ ОБЯЗАТЕЛЬСТВО ВСЕГО (=066-067-068) (УКАЖИТЕ ЗНАК МИНУС, ЕСЛИ ЗНАЧЕНИЕ ОТРИЦАТЕЛЬНОЕ)";
...
Функция ЕстьДокументВУказанныйПериод()
