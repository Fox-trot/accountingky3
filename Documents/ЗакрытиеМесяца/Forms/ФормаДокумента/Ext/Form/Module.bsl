...
Процедура ДатаПриИзменении(Элемент)
	...
	Если ЕстьДокументВУказанныйПериод(Объект.Дата) Тогда		
		...
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения,, "Объект.Дата",,Истина);
...
Функция РезультатЗаполненияВДлительнойОперации()
	...
	Иначе
		...
		РезультатВыполнения.Вставить("ЗаданиеВыполнено", ЗаданиеВыполнено(
			?(ПараметрыВыполнения.Свойство("ИдентификаторЗадания"), ПараметрыВыполнения.ИдентификаторЗадания, Истина)));
	КонецЕсли;
