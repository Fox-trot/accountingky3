...
Функция ПечатнаяФормаИнвентаризационнаяОписьИНВ19(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	ТабличныйДокумент.КлючПараметровПечати = "ПФ_MXL_ИнвентаризационнаяОписьИНВ19";
	...
	Пока ВыборкаЗаголовок.Следующий()  Цикл
		...
		Для Каждого ИмяОбласти Из МассивОбластейМакета Цикл
			...
			ИначеЕсли ИмяОбласти = "ШапкаСличВедомость" Тогда 
				ОбластьМакета.Параметры.ДокументНомер = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаЗаголовок.НомерДокумента);
			...
			ИначеЕсли ИмяОбласти = "Строка" Тогда 
				Для Каждого СтрокаТаблицы Из ВременнаяТаблицаТовары Цикл
					Если СтрокаТаблицы.ИзлишекКоличество	= 0
						...
						И СтрокаТаблицы.НедостачаСумма		= 0
						Тогда  Продолжить;	
					КонецЕсли;
...
Функция ПечатьИнвентаризацияТоваров_ИНВ3(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ИнвентаризацияТоваров_ИНВ3";
	...
	Пока ВыборкаДок.Следующий() Цикл
		...
		Если ВременнаяТаблицаТовары.Количество() Тогда
			ВидЦенностей = "Товары (материалы)";	
		КонецЕсли;
		Если ВременнаяТаблицаМБП.Количество() Тогда
			ВидЦенностей = ВидЦенностей + ?(ЗначениеЗаполнено(ВидЦенностей), ", ", "") + "МБП";	
		КонецЕсли;
		...
		ДанныеПечати.Вставить("НомерДокумента", 			ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаДок.НомерДокумента));		
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	НужноПечататьМакет = УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_ИнвентаризационнаяОписьИНВ19");
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
		КоллекцияПечатныхФорм,
		"ПФ_MXL_ИнвентаризационнаяОписьИНВ19",
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПФ_MXL_ИнвентаризационнаяОписьИНВ19";
