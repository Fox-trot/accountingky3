...
Функция ПечатнаяФормаИнвентаризационнаяОписьИНВ19(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаЗаголовок.Следующий()  Цикл
		...
		ДанныеФизЛица = БухгалтерскийУчетСервер.ДанныеФизЛица(ВыборкаЗаголовок.Организация, ВыборкаЗаголовок.ФизЛицо, ВыборкаЗаголовок.ДатаДокумента);
		ДанныеПечати.Вставить("МОЛДолжность", 	ДанныеФизЛица.Должность);
...
Функция ПечатьИнвентаризацияТоваров_ИНВ3(МассивОбъектов, ОбъектыПечати)
	...
	Пока ВыборкаДок.Следующий() Цикл
		...
		ДанныеФизЛица = БухгалтерскийУчетСервер.ДанныеФизЛица(ВыборкаДок.Организация, ВыборкаДок.ФизЛицо, ВыборкаДок.ДатаДокумента);
		ДанныеПечати.Вставить("ДолжностьФизЛица", 	ДанныеФизЛица.Должность);
		ДанныеПечати.Вставить("ФИОФизЛица", 		ДанныеФизЛица.Представление);
			...
			Если ИмяОбласти = "Комиссия" Тогда
				...
				Пока ВыборкаСтрокКомиссия.Следующий() Цикл
					Если ВыборкаСтрокКомиссия.Председатель Тогда
						...
						ОбластьМакета.Параметры.ДолжностьПредседателя = ДанныеФизЛица.Должность;
						ОбластьМакета.Параметры.ФИОПредседателя       = ДанныеФизЛица.Представление;
						ТабличныйДокумент.Вывести(ОбластьМакета);
						ПредседательЗаполнен = Истина;
						
					Иначе
						Если ЧленКомисии = 4 Тогда
							Продолжить;	
						КонецЕсли;
						
						ДанныеФизЛица = БухгалтерскийУчетСервер.ДанныеФизЛица(ВыборкаДок.Организация, ВыборкаСтрокКомиссия.ФизЛицо, ВыборкаДок.ДатаДокумента);
						ОбластьМакета.Параметры["ДолжностьЧленаКомисси" + ЧленКомисии] = ДанныеФизЛица.Должность;
						ОбластьМакета.Параметры["ФИОЧленаКомисси" + ЧленКомисии]       = ДанныеФизЛица.Представление;
						ТабличныйДокумент.Вывести(ОбластьМакета);
						ЧленКомисии = ЧленКомисии + 1;
					КонецЕсли;
					
					Если ПредседательЗаполнен И ЧленКомисии = 4 Тогда
						Прервать;	
					КонецЕсли;	
				КонецЦикла;	
			Иначе
