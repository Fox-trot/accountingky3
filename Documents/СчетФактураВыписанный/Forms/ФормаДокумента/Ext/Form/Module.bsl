...
// Процедура - обработчик события ПриИзменении поля ввода Безналичный расчет.
//
&НаКлиенте
...
Процедура ТоварыПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	ТекстСообщения = СтрШаблон(НСтр("ru = 'Добавлени строк не предуспотрено. Воспользуйтесь командой ""Заполнить по документам"".'"));
...
Процедура Заполнить(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.СтавкаНДС) Тогда
		ТекстСообщения = НСтр("ru = 'Для заполнения табличной части необходимо указать ставку НДС.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.СтавкаНДС",,Отказ)		
	КонецЕсли;
	
	Если НЕ ЗначениеЗаполнено(Объект.СтавкаНСП) И НЕ Объект.БезналичныйРасчет Тогда
		ТекстСообщения = НСтр("ru = 'Для заполнения табличной части необходимо указать ставку НСП.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.СтавкаНСП",,Отказ)		
	КонецЕсли;
	...
	Если НЕ (Объект.ЗаполнятьТоварами ИЛИ Объект.ЗаполнятьУслугами) Тогда
		ТекстСообщения = НСтр("ru = 'Для заполнения табличной части необходимо установить хотя бы одну из настроек ""Заполнять товарами"" или ""Заполнять услугами"".'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,, "Объект.ЗаполнятьТоварами",,Отказ)		
	КонецЕсли;
...
Процедура ЗаполнитьПоДокументам(Команда)
	...
	Если НЕ Объект.ЗаполнятьТоварами И НЕ Объект.ЗаполнятьУслугами Тогда
		ТекстСообщения = НСтр("ru = 'Не установлены настройки заполнения! Операция отменена.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Объект.ЗаполнятьТоварами",,Отказ);		
	КонецЕсли;
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	Элементы.ДокументыСуммаНСП.Видимость = НЕ Объект.БезналичныйРасчет;
...
// Процедура обнуления сумм НСП всех строк ТЧ "ДокументыОснования".
//
&НаКлиенте
Процедура ОбнулитьСуммыНСП()
...
Процедура ОбработатьИзменениеДоговора(МодифицированДоговор = Ложь)
	...
	Объект.СтавкаНДС 		= ?(ЗначениеЗаполнено(СтруктураДанные.СтавкаНДС),
		СтруктураДанные.СтавкаНДС, ПредопределенноеЗначение("Справочник.СтавкиНДС.Стандарт"));
