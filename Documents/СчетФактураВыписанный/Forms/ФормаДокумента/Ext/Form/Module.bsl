...
Функция ФормированиеФайлаXML()
	...
	ТекущаяДата 	= Формат(ТекущаяДатаСеанса(), "ДФ=yyyy-MM-dd");
	...
	Пока ВыборкаФормаОплаты.Следующий() Цикл
		...
		Пока ВыборкаПризнакУслуги.Следующий() Цикл
			...
			Если ДанныеПоТоварам.ЕстьЗаписи Тогда
				...
				ОбъектXDTOcreatedDate = СоздатьОбъектXDTO("createdDate"); // Дата создания
				ОбъектXDTOcreatedDate.Text = ТекущаяДата;
				ОбъектXDTOreceipt.createdDate.Добавить(ОбъектXDTOcreatedDate);
...
Функция goods(ТаблицаДокументы, ТаблицаНомеклатура, КодФормыОплаты, Услуги)
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.Товары Цикл
		...
		КодСтавкиНДС = ?(СтрокаТабличнойЧасти.СтавкаНДС = Справочники.СтавкиНДС.Стандарт, 10, 20);
