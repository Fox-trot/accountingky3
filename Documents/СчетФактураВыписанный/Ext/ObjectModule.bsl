...
Процедура ЗаполнитьПоВозвратуОтПокупателя(ДанныеЗаполнения) Экспорт
	...
	СтрокаТабличнойЧастиДокументы.СуммаДохода = ДанныеЗаполнения.Товары.Итог("СуммаДохода");
КонецПроцедуры
...
Процедура ВыполнитьПредварительныйКонтроль(Отказ)
	...
	Если ЗначениеЗаполнено(СерияБланкаСФ)Тогда 
		Запрос = Новый Запрос;
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	БланкиСчетовФактур.СерияБланкаСФ КАК СерияБланкаСФ
			|ИЗ
			|	РегистрНакопления.БланкиСчетовФактур КАК БланкиСчетовФактур
			|ГДЕ
			|	БланкиСчетовФактур.Организация = &Организация
			|	И БланкиСчетовФактур.СерияБланкаСФ = &СерияБланкаСФ
			|	И БланкиСчетовФактур.НомерБланкаСФ = &НомерБланкаСФ";
		...
		Если Результат.Пустой() Тогда
			ТекстСообщения = НСтр("ru = 'Указанных серии или номера БС нет в базе. Необходимо оформить документ ""Поступление бланков счетов-фактур"".'");
...
Процедура ЗаполнитьДокументыОснования() Экспорт
	...
	Запрос.Текст = 
		...
		|	ПродажиОбороты.СуммаДоходаОборот КАК СуммаДохода,
		...
		|						ГДЕ
		|							СчетаФактурыВыписанные.Период МЕЖДУ &НачалоПериода И &КонецПериода
	...
	Если ВозвратТоваров Тогда
		...
		Запрос.Текст = 		
			...
			|	ВременнаяТаблицаДокументы.СуммаДохода КАК СуммаДохода
			|ИЗ
