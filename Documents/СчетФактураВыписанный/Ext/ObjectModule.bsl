		...
		Иначе
			...
			Если НЕ РезультатЗапроса[1].Пустой() Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'Для контрагента %1 обнаружены документы возврата %2.'"), Контрагент, 
					?(ЗначениеЗаполнено(ДоговорКонтрагента), "с другим договором", "с другими ставками"));
...
Процедура ЗаполнитьПоПодобраннымДокументам() Экспорт
	...
	Если ВозвратТоваров Тогда
		Запрос.Текст = 
			...
			|	ВозвратТоваровОтПокупателяТовары.Ссылка.СтавкаНДС КАК СтавкаНДС,
			|	ВозвратТоваровОтПокупателяТовары.Ссылка.СтавкаНСП КАК СтавкаНСП,
			...
			|	-ВозвратТоваровОтПокупателяТовары.СуммаНДСВВалютеРеглУчета КАК СуммаНДСВВалютеРеглУчета,
			|	-ВозвратТоваровОтПокупателяТовары.СуммаНСПВВалютеРеглУчета КАК СуммаНСПВВалютеРеглУчета,
	...
	Иначе
		Запрос.Текст =		
			...
			|	РеализацияТоваровУслугУслуги.СуммаВВалютеРеглУчета КАК СуммаВВалютеРеглУчета,
			|	РеализацияТоваровУслугУслуги.СуммаНДСВВалютеРеглУчета КАК СуммаНДСВВалютеРеглУчета,
			|	РеализацияТоваровУслугУслуги.СуммаНСПВВалютеРеглУчета КАК СуммаНСПВВалютеРеглУчета,
			|	РеализацияТоваровУслугУслуги.ВсегоВВалютеРеглУчета КАК ВсегоВВалютеРеглУчета,
			|	РеализацияТоваровУслугУслуги.СуммаДоходаВВалютеРеглУчета КАК СуммаДоходаВВалютеРеглУчета,
	...
	Пока Выборка.Следующий() Цикл
		...
		СтрокаТабличнойЧастиДокументы = ДокументыОснования.Найти(Выборка.ДокументОснования, "ДокументОснование");
