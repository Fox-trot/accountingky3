...
Процедура СформироватьТаблицаСчетаФактурыВыписанные(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		//|	КОНЕЦ КАК Сумма,
		//|	ВременнаяТаблицаДокументыОснования.СуммаДохода + (ВЫРАЗИТЬ(ВременнаяТаблицаДокументыОснования.СуммаСкидкиВВалютеРеглУчета * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))) КАК Сумма,		
