...
Процедура СформироватьТаблицаСчетаФактурыВыписанные(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаДокументыОснования.СуммаДохода - ВЫБОР
		|		КОГДА ВременнаяТаблицаДокументыОснования.Всего = ВременнаяТаблицаДокументыОснования.СуммаСкидкиВВалютеРеглУчета
		|			ТОГДА ВременнаяТаблицаДокументыОснования.СуммаСкидкиВВалютеРеглУчета
		|		ИНАЧЕ ВЫРАЗИТЬ(ВременнаяТаблицаДокументыОснования.СуммаСкидкиВВалютеРеглУчета * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		...
		|	ВременнаяТаблицаОбщая.ВидПоставкиНДС КАК ВидПоставкиНДС
		|ИЗ
						...
						Если ОбластьМакетаДопСтрока = Неопределено Тогда
							...
							Иначе
								ЗначениеСтавкиНСП = ?(ДанныеУчетнойПолитики.ПлательщикНСП, 
													УчетНДСВызовСервера.ПолучитьСтавкуНСП(Шапка.ДатаСФ, Шапка.Организация, СтрокаТаблицы.СтавкаНСП), 
							...
							ЗначениеСтавкиНДС = ?(ДанныеУчетнойПолитики.ПлательщикНДС, 
												УчетНДСВызовСервера.ПолучитьСтавкуНДС(Шапка.ДатаСФ, СтрокаТаблицы.СтавкаНДС), 
						...
						Иначе
							...
							Если Строки.Количество() > 0 Тогда
								...
								Иначе	
									...
									Иначе
										ЗначениеСтавкиНСП = ?(ДанныеУчетнойПолитики.ПлательщикНСП, 
															УчетНДСВызовСервера.ПолучитьСтавкуНСП(Шапка.ДатаСФ, Шапка.Организация, Данные.СтавкаНСП), 
															ПредопределенноеЗначение("Справочник.СтавкиНСП.ПустаяСсылка"));
										ДанныеПечати.СтавкаНСП = ?(ЗначениеЗаполнено(ЗначениеСтавкиНСП), Строка(ЗначениеСтавкиНСП) + "%","");
									КонецЕсли;	
									
									ЗначениеСтавкиНДС = ?(ДанныеУчетнойПолитики.ПлательщикНДС, 
														УчетНДСВызовСервера.ПолучитьСтавкуНДС(Шапка.ДатаСФ, Данные.СтавкаНДС), 
														ПредопределенноеЗначение("Справочник.СтавкиНДС.ПустаяСсылка"));
...
Функция ПечатьСчетФактураБезНДС(МассивОбъектов, ОбъектыПечати, ПараметрыПечати)
	...
	Если НЕ ПараметрыПечати.ПлательщикНДС Тогда
		...
		ТекстЗапроса =
			...
			|	СчетФактураВыписанныйТовары.ДокументОснования.Курс КАК Курс,
			|	СчетФактураВыписанныйТовары.ДокументОснования.Кратность КАК Кратность,
			...
			|	СчетФактураВыписанныйТовары.Номенклатура КАК Номенклатура,
			|	СчетФактураВыписанныйТовары.Номенклатура.НаименованиеПолное КАК НоменклатураПредставление,
			|   СРЕДНЕЕ(СчетФактураВыписанныйТовары.ДокументОснования.Курс) КАК Курс,
			|   СРЕДНЕЕ(СчетФактураВыписанныйТовары.ДокументОснования.Кратность) КАК Кратность,
				...
				ИначеЕсли ИмяОбласти = "СтрокаТаблицы" Тогда 
					...
					Для Каждого СтрокаМассива Из МассивСтрок Цикл
						...
						Если ЗначениеСтавкиНДС = Неопределено Тогда
							ЗначениеСтавкиНДС = УчетНДСВызовСервера.ПолучитьСтавкуНДС(Шапка.Дата, СтрокаМассива.СтавкаНДС);	
						...
						Если ЗначениеСтавкиНСП = Неопределено Тогда
							ЗначениеСтавкиНСП = ?(ДанныеУчетнойПолитики.ПлательщикНСП,
										УчетНДСВызовСервера.ПолучитьСтавкуНСП(Шапка.Дата, Шапка.Организация, СтрокаМассива.СтавкаНСП), 0);
...
Функция ПолучитьТекстЗапросаДокументовОснования()
	...
	ТекстЗапроса =
		...
		|	ТаблицаДокумента.ДокументОснования.СуммаВключаетНалоги КАК СуммаВключаетНалоги,
		|	ТаблицаДокумента.ДокументОснования.Дата КАК Дата,
		|	ТаблицаДокумента.ДокументОснования.БезналичныйРасчет КАК БезналичныйРасчет,
