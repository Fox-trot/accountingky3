...
Процедура СформироватьТаблицаСчетаФактурыВыписанные(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаТоварыКонтрагенты.СтавкаНДС КАК СтавкаНДС,
...
Функция ПечатьСчетФактура(МассивОбъектов, ОбъектыПечати, КоличествоСтрокСчетаФактуры)
	...
	Пока Шапка.Следующий() Цикл
		...
		Если Шапка.ЭтоГоловнаяОрганизация Тогда 
		    ...
		    ГНС = Шапка.ГНСОрганизаци;
		    КодГНС = Шапка.КодГНСОрганизаци;
		...
		АдресПоставщика = УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(Организация, Справочники.ВидыКонтактнойИнформации.ЮрАдресКонтрагента); 
