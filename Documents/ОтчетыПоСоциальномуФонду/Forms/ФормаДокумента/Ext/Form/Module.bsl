...
&НаКлиенте
Процедура ЕжемесячныйСоцфонд_Excel_UBR(Команда)
	
	Если ТребуетсяНастройкаАвторизацияИнтернетПоддержки() Тогда
		ТекстВопроса = НСтр("ru='Для формирования отчета в электронной форме
			|необходимо подключиться к Интернет-поддержке пользователей.
			|Подключиться сейчас?'");
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриПодключенииИнтернетПоддержки", ЭтотОбъект, Новый Структура("НаправлениеВыгрузки", "ЕжемесячныйСоцфонд_Excel_UBR"));
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Подключиться'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки);
	Иначе
		ПродолжитьВыгрузку("ЕжемесячныйСоцфонд_Excel_UBR");
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура ЕжемесячныйСоцфонд_Excel_Достек(Команда)
	
	Если ТребуетсяНастройкаАвторизацияИнтернетПоддержки() Тогда
		ТекстВопроса = НСтр("ru='Для формирования отчета в электронной форме
			|необходимо подключиться к Интернет-поддержке пользователей.
			|Подключиться сейчас?'");
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриПодключенииИнтернетПоддержки", ЭтотОбъект, Новый Структура("НаправлениеВыгрузки", "ЕжемесячныйСоцфонд_Excel_Достек"));
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Подключиться'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки);
	Иначе
		ПродолжитьВыгрузку("ЕжемесячныйСоцфонд_Excel_Достек");
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ЕжемесячныйСоцфонд_xml_UBR(Команда)
	
	Если ТребуетсяНастройкаАвторизацияИнтернетПоддержки() Тогда
		ТекстВопроса = НСтр("ru='Для формирования отчета в электронной форме
			|необходимо подключиться к Интернет-поддержке пользователей.
			|Подключиться сейчас?'");
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриПодключенииИнтернетПоддержки", ЭтотОбъект, Новый Структура("НаправлениеВыгрузки", "ЕжемесячныйСоцфонд_xml_UBR"));
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Подключиться'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки);
	Иначе
		ПродолжитьВыгрузку("ЕжемесячныйСоцфонд_xml_UBR");
	КонецЕсли;
КонецПроцедуры
...
Процедура ОткрытьЛичныйКабинет(Команда)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка)
		...
		ТекстСообщения = НСтр("ru='Для формирования json файла необходимо записать документ.'");
		ОбщегоНазначенияКлиент.СообщитьПользователю(ТекстСообщения);
		Возврат;
	КонецЕсли;	
	
	Если ТребуетсяНастройкаАвторизацияИнтернетПоддержки() Тогда
		ТекстВопроса = НСтр("ru='Для формирования отчета json файла
			|необходимо подключиться к Интернет-поддержке пользователей.
			|Подключиться сейчас?'");
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриПодключенииИнтернетПоддержки", ЭтотОбъект, Новый Структура("НаправлениеВыгрузки", "СохранитьФайл_json"));
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Подключиться'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки);
	Иначе
		СформироватьJosnИОткрытьЛичныйКабинет();
	КонецЕсли;
...
Процедура ПослеПодключенияИнтернетПоддержки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.НаправлениеВыгрузки = "ЕжемесячныйСоцфонд_xml"
		ИЛИ ДополнительныеПараметры.НаправлениеВыгрузки = "ЕжемесячныйСоцфонд_xml_UBR" Тогда
...
Процедура ДиалогОткрытияФайлаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ВидДиалога = "ЕжемесячныйСоцфонд_Excel"
		ИЛИ ДополнительныеПараметры.ВидДиалога = "ЕжемесячныйСоцфонд_Excel_UBR" 
		ИЛИ ДополнительныеПараметры.ВидДиалога = "ЕжемесячныйСоцфонд_Excel_Достек" Тогда
...
Процедура ФормированиеФайла(КаталогФайлаВыгрузки, ДополнительныеПараметры)
	...
	#Если ТонкийКлиент Тогда
		...
		Если ДополнительныеПараметры.ВидДиалога = "ЕжемесячныйСоцфонд_Excel_UBR" Тогда
			ПриставкаНаименования = "(UBR) ";
			
		ИначеЕсли ДополнительныеПараметры.ВидДиалога = "ЕжемесячныйСоцфонд_Excel_Достек" Тогда
			ПриставкаНаименования = "(Достек) ";	
			
		Иначе
			ПриставкаНаименования = "";
		КонецЕсли;
...
Функция ОтчетЕжемесячныйСоцфонд(ВидДИалога)
	...
	Если ВидДИалога = "ЕжемесячныйСоцфонд_Excel_UBR" Тогда
		Макет = Документы.ОтчетыПоСоциальномуФонду.ПолучитьМакет("ПФ_MXL_СФЕжемесячный_Excel_UBR");
		
	ИначеЕсли ВидДИалога = "ЕжемесячныйСоцфонд_Excel_Достек" Тогда	
		Макет = Документы.ОтчетыПоСоциальномуФонду.ПолучитьМакет("ПФ_MXL_СФЕжемесячный_Excel_Достек");
	...
	Если ВидДИалога <> "ЕжемесячныйСоцфонд_Excel_Достек" Тогда
...
Функция ФормированиеФайлаXML(ДополнительныеПараметры, Ошибки, ВернутьВ_Base64 = Ложь)
	...
	Если ДополнительныеПараметры.ВидДиалога = "ЕжемесячныйСоцфонд_xml_UBR" Тогда
		PayerInfo.TariffType 		= Тариф;
		PayerInfo.PayerID			= СведенийОбОрганизации.РегНомерСоцФонда;
	Иначе
	...
	Если ДополнительныеПараметры.ВидДиалога = "ЕжемесячныйСоцфонд_xml_UBR" Тогда
		ОбъектXDTO.SalaryInfoCollection.Добавить(SalaryInfoCollection(Истина));
		ОбъектXDTO.PayerBalanceOfArrearsCollection.Добавить(PayerBalanceOfArrearsCollection(Истина));
	Иначе
...
&НаСервере
Функция SalaryInfoCollection(UBR = Ложь)
	
	ОбъектXDTO = СоздатьОбъектXDTO("SalaryInfoCollection");
	
	Для Каждого СтрокаТабличнойЧасти Из Объект.СведенияОЗанятостиИЗаработнойПлате Цикл
		ОбъектXDTO.SalaryInfo.Добавить(SalaryInfo(UBR,СтрокаТабличнойЧасти));	
	КонецЦикла;
...
&НаСервере
Функция SalaryInfo(UBR, СтрокаТабличнойЧасти)
	// Создание объекта.
...
&НаСервере
Функция PayerBalanceOfArrearsCollection(UBR = Ложь)
	...
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Страховые взносы (кроме НПФ)" ,"ФОНД"),  
		Объект.ОбязательстваПоСтраховымВзносам));
		
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Накопительный пенсионный фонд" ,"НПФ"),   
		Объект.ОбязательстваПоПенсионномуФонду));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Пени и штрафы" ,"САНК"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Отсроченные страховые взносы (кроме НПФ)" ,"ОТСФ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Отсроченные страховые взносы - НПФ" ,"ОНПФ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Отсроченные пени и штрафы" ,"ОСАНК"), 
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Преемственная задолженность по взносам (кроме НПФ)" ,"ПРЕФ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Преемственная задолженность по взносам - НПФ" ,"ПНПФ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Преемственная задолженность по пеням и штрафам" ,"ПНПФ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Проценты по предоставлению отсрочки" ,"ПСАНК"), 
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Переплата по страховым взносам(кроме НПФ)" ,"АВПФ"),  
		Объект.ПереплатаПоСтраховымВзносам));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Переплата по санкциям" ,"ПРОЦ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(UBR, "Переплата по НПФ" ,"АВНПФ"), 
...
// Процедура открытия личного кабинета налогоплательщика.
//
&НаКлиенте
Процедура СформироватьJosnИОткрытьЛичныйКабинет()

	ДанныеОтправки = ФормированиеФайла_json();
	
	Если ДанныеОтправки = Неопределено Тогда
		Возврат;		     	
	КонецЕсли;	
	
	РаботаСЭлектроннымиСчетамиФактурамиСлужебный.ОткрытьЛичныйКабинет(ДанныеОтправки);	
КонецПроцедуры

// Процедура создания Json файла
//
&НаСервере
Функция ФормированиеФайла_json()
	
	СведенияОбОрганизации = БухгалтерскийУчетСервер.ПолучитьСведенияОбОрганизации(Объект.Организация);
	
	Адрес = ?(СведенияОбОрганизации.Индекс  = "", "", СведенияОбОрганизации.Индекс + ", ")
		+ ?(СведенияОбОрганизации.Город  = "", СведенияОбОрганизации.НаселенныйПункт + ", ", СведенияОбОрганизации.Город + ", ")
		+ ?(СведенияОбОрганизации.Улица = "", "", СведенияОбОрганизации.Улица + ", ")
		+ ?(СведенияОбОрганизации.Дом = "", "", СведенияОбОрганизации.Дом + ", ")
		+ ?(СведенияОбОрганизации.Корпус = "", "", СведенияОбОрганизации.Корпус + ", ")
		+ ?(СведенияОбОрганизации.Квартира = "", "", СведенияОбОрганизации.Квартира);
	
	ОтветственныеЛица = БухгалтерскийУчетСервер.ОтветственныеЛицаОрганизацийРуководители(Объект.Организация, ДатаДокумента);
	
	taxpayerInfo = Новый Структура();
	taxpayerInfo.Вставить("name", 				СведенияОбОрганизации.НаименованиеПолное);
	taxpayerInfo.Вставить("tin", 				СведенияОбОрганизации.ИНН);
	taxpayerInfo.Вставить("okpo", 				СведенияОбОрганизации.ОКПО);
	taxpayerInfo.Вставить("sfRegNum", 			СведенияОбОрганизации.РегНомерСоцФонда);
	taxpayerInfo.Вставить("address", 			Адрес);
	taxpayerInfo.Вставить("economicActivity", 	СведенияОбОрганизации.НаименованиеГКЭД);
	taxpayerInfo.Вставить("bankName", 			СведенияОбОрганизации.Банк.Наименование);
	taxpayerInfo.Вставить("bankAccount", 		СведенияОбОрганизации.НомерСчета);
	taxpayerInfo.Вставить("phoneNum", 			СведенияОбОрганизации.Тел);
	taxpayerInfo.Вставить("region", 			СведенияОбОрганизации.ГНСКод);
	taxpayerInfo.Вставить("tariff", 			1);
	taxpayerInfo.Вставить("directorFullName", 	ОтветственныеЛица.Руководитель);
	taxpayerInfo.Вставить("accountFullName", 	ОтветственныеЛица.ГлавныйБухгалтер);
	
	ДанныеАвторизации = ИнтернетПоддержкаПользователей.ДанныеАутентификацииПользователяИнтернетПоддержки();
	taxpayerInfo.Вставить("RegNumberITS", 		ДанныеАвторизации.Логин);
	                                              
	reportInfo = Новый Структура();
	
	// Отчета по НДС в xml конвертируется в формат Base64 и добавляется в json файл.
	ОтчетXML_Base64 = ФормированиеФайлаXML(Новый Структура("ВидДиалога", "ЕжемесячныйСоцфонд_xml_UBR"), Новый Массив(), Истина);
	reportInfo.Вставить("xml", ОтчетXML_Base64);
	
	// Печатная форма отчета по НДС сохраняется в формате PDF, затем конвертируется в формат Base64 и добавляется в json файл.
	Массив = Новый Массив(1);
	Массив[0] = Объект.Ссылка;
	ТабличныйДокумент = Документы.ОтчетыПоСоциальномуФонду.ПечатьЕжемесячногоПоСФ(Массив, Новый СписокЗначений);
	ИмяВременногоФайла = ПолучитьИмяВременногоФайла("pdf");
	ТабличныйДокумент.Записать(ИмяВременногоФайла, ТипФайлаТабличногоДокумента.PDF);
	Двоичное = Новый ДвоичныеДанные(ИмяВременногоФайла);
	ОтчетPDF_Base64 = Base64Строка(Двоичное);
	reportInfo.Вставить("pdf",			ОтчетPDF_Base64);
	
	reportInfo.Вставить("formName", 	"Отчеты по социальному фонду");
	reportInfo.Вставить("formVer", 		"");
	reportInfo.Вставить("whenCreated", 	Формат(ДатаДокумента, "ДФ='yyyy-MM-dd HH:mm:ss'"));    
	
	ФайловаяСистема.УдалитьВременныйФайл(ИмяВременногоФайла);
	
	Структура_json = Новый Структура();
	Структура_json.Вставить("taxpayerInfo", taxpayerInfo);
	Структура_json.Вставить("reportInfo", reportInfo);
		
	Дата = Формат(ДатаДокумента, "ДФ=yyyy-MM-dd");

	ШаблонИмениФайла = СтрШаблон(НСтр("ru = 'Json_%1.'"), Дата);
	ИмяФайла = ШаблонИмениФайла + ".json";
	...
	Иначе                                                      
		...
		ЗаписатьJSON(ЗаписьJson, Структура_json);
		...
		СтруктураДанных = Новый Структура;		
		СтруктураДанных.Вставить("ИНН", СведенияОбОрганизации.ИНН);
		СтруктураДанных.Вставить("СтрJSON", СтрJSON);
		СтруктураДанных.Вставить("Код_service_id", 1);
