...
Функция ОтчетЕжемесячныйСоцфонд()
	...
	ДанныеПечати.Вставить("НомерСчета", ДанныеОрганизации.БанковскийСчет);
...
Функция PayerBalanceOfArrearsCollection()
	...
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears("АВПФ",  Объект.ПереплатаПоСтраховымВзносам));
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears("ПРОЦ",  0));
