...
&НаКлиенте
Процедура ЕжемесячныйСоцфонд_Excel_ЛК(Команда)
	
	Если ТребуетсяНастройкаАвторизацияИнтернетПоддержки() Тогда
		ТекстВопроса = НСтр("ru='Для формирования отчета в электронной форме
			|необходимо подключиться к Интернет-поддержке пользователей.
			|Подключиться сейчас?'");
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриПодключенииИнтернетПоддержки", ЭтотОбъект, Новый Структура("НаправлениеВыгрузки", "ЕжемесячныйСоцфонд_Excel_ЛК"));
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Подключиться'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки);
	Иначе
		ПродолжитьВыгрузку("ЕжемесячныйСоцфонд_Excel_ЛК");
	КонецЕсли;
КонецПроцедуры
...
&НаКлиенте
Процедура ЕжемесячныйСоцфонд_xml_ЛК(Команда)
	
	Если ТребуетсяНастройкаАвторизацияИнтернетПоддержки() Тогда
		ТекстВопроса = НСтр("ru='Для формирования отчета в электронной форме
			|необходимо подключиться к Интернет-поддержке пользователей.
			|Подключиться сейчас?'");
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриПодключенииИнтернетПоддержки", ЭтотОбъект, Новый Структура("НаправлениеВыгрузки", "ЕжемесячныйСоцфонд_xml_ЛК"));
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Подключиться'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки);
	Иначе
		ПродолжитьВыгрузку("ЕжемесячныйСоцфонд_xml_ЛК");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПослеПодключенияИнтернетПоддержки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.НаправлениеВыгрузки = "ЕжемесячныйСоцфонд_xml"
		ИЛИ ДополнительныеПараметры.НаправлениеВыгрузки = "ЕжемесячныйСоцфонд_xml_ЛК" Тогда
...
Процедура ДиалогОткрытияФайлаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.ВидДиалога = "ЕжемесячныйСоцфонд_Excel"
		ИЛИ ДополнительныеПараметры.ВидДиалога = "ЕжемесячныйСоцфонд_Excel_ЛК" 
...
Процедура ФормированиеФайла(КаталогФайлаВыгрузки, ДополнительныеПараметры)
	...
	#Если ТонкийКлиент Тогда
		...
		Если ДополнительныеПараметры.ВидДиалога = "ЕжемесячныйСоцфонд_Excel_ЛК" Тогда
			ПриставкаНаименования = "(ЛК) ";
...
Функция ОтчетЕжемесячныйСоцфонд(ВидДИалога)
	...
	Если ВидДИалога = "ЕжемесячныйСоцфонд_Excel_ЛК" Тогда
		Макет = Документы.ОтчетыПоСоциальномуФонду.ПолучитьМакет("ПФ_MXL_СФЕжемесячный_Excel_ЛК");
...
Функция ФормированиеФайлаXML(ДополнительныеПараметры, Ошибки, ВернутьВ_Base64 = Ложь)
	...
	Если ДополнительныеПараметры.ВидДиалога = "ЕжемесячныйСоцфонд_xml_ЛК" Тогда
...
&НаСервере
Функция SalaryInfoCollection(ЛК = Ложь)
	
	ОбъектXDTO = СоздатьОбъектXDTO("SalaryInfoCollection");
	
	Для Каждого СтрокаТабличнойЧасти Из Объект.СведенияОЗанятостиИЗаработнойПлате Цикл
		ОбъектXDTO.SalaryInfo.Добавить(SalaryInfo(ЛК,СтрокаТабличнойЧасти));	
	КонецЦикла;
...
&НаСервере
Функция SalaryInfo(ЛК, СтрокаТабличнойЧасти)
	// Создание объекта.
...
&НаСервере
Функция PayerBalanceOfArrearsCollection(ЛК = Ложь)
	...
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Страховые взносы (кроме НПФ)" ,"ФОНД"),  
		Объект.ОбязательстваПоСтраховымВзносам));
		
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Накопительный пенсионный фонд" ,"НПФ"),   
		Объект.ОбязательстваПоПенсионномуФонду));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Пени и штрафы" ,"САНК"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Отсроченные страховые взносы (кроме НПФ)" ,"ОТСФ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Отсроченные страховые взносы - НПФ" ,"ОНПФ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Отсроченные пени и штрафы" ,"ОСАНК"), 
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Преемственная задолженность по взносам (кроме НПФ)" ,"ПРЕФ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Преемственная задолженность по взносам - НПФ" ,"ПНПФ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Преемственная задолженность по пеням и штрафам" ,"ПНПФ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Проценты по предоставлению отсрочки" ,"ПСАНК"), 
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Переплата по страховым взносам(кроме НПФ)" ,"АВПФ"),  
		Объект.ПереплатаПоСтраховымВзносам));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Переплата по санкциям" ,"ПРОЦ"),  
		0));
	
	ОбъектXDTO.PayerBalanceOfArrears.Добавить(PayerBalanceOfArrears(
		?(ЛК, "Переплата по НПФ" ,"АВНПФ"), 
