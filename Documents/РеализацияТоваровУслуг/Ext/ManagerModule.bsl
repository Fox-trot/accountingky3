...
Процедура СформироватьТаблицаХозрасчетный(ДокументСсылка,СтруктураДополнительныеСвойства, Отказ)
	...
	Запрос.Текст =
		...
		|			ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаТовары.ВсегоВРегВалюте - ВременнаяТаблицаТовары.СуммаСкидкиВРегВалюте - ВременнаяТаблицаТовары.СуммаНДСВРегВалюте - ВременнаяТаблицаТовары.СуммаНСПВРегВалюте + ВременнаяТаблицаТовары.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|		ИНАЧЕ ВременнаяТаблицаТовары.СуммаДоходаВРегВалюте
		|	КОНЕЦ КАК Сумма,
		...
		|ГДЕ
		|	НЕ ВременнаяТаблицаТовары.ВсегоВРегВалюте = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	ВременнаяТаблицаТовары.СуммаНДСВРегВалюте,
		...
		|ГДЕ
		|	&ПлательщикНДС
		|	И НЕ ВременнаяТаблицаТовары.СуммаНДСВРегВалюте = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	ВременнаяТаблицаТовары.СуммаНСПВРегВалюте,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		...
		|		ПО (ИСТИНА)
		|ГДЕ
		|	НЕ ВременнаяТаблицаТовары.СуммаНСПВРегВалюте = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ВременнаяТаблицаШапка.ДоговорКонтрагента,
		|	НЕОПРЕДЕЛЕНО,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаТовары.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2)),
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		...
		|		ПО ВременнаяТаблицаШапка.СтавкаНСП = ВременнаяТаблицаСтавкиНСП.СтавкаНСП
		|ГДЕ
		|	НЕ ВременнаяТаблицаТовары.СуммаСкидкиВРегВалюте = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаУслуги.ВсегоВРегВалюте - ВременнаяТаблицаУслуги.СуммаСкидкиВРегВалюте - ВременнаяТаблицаУслуги.СуммаНДСВРегВалюте - ВременнаяТаблицаУслуги.СуммаНСПВРегВалюте + ВременнаяТаблицаУслуги.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|		ИНАЧЕ ВременнаяТаблицаУслуги.СуммаДоходаВРегВалюте
		|	КОНЕЦ,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВЫБОР
		...
		|ГДЕ
		|	НЕ ВременнаяТаблицаУслуги.ВсегоВРегВалюте = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	ВременнаяТаблицаУслуги.СуммаНДСВРегВалюте,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		...
		|ГДЕ
		|	&ПлательщикНДС
		|	И НЕ ВременнаяТаблицаУслуги.СуммаНДСВРегВалюте = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	ВременнаяТаблицаУслуги.СуммаНСПВРегВалюте,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		...
		|		ПО (ИСТИНА)
		|ГДЕ
		|	НЕ ВременнаяТаблицаУслуги.СуммаНСПВРегВалюте = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ВременнаяТаблицаШапка.ДоговорКонтрагента,
		|	НЕОПРЕДЕЛЕНО,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаУслуги.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2)),
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		...
		|		ПО ВременнаяТаблицаШапка.СтавкаНСПУслуги = ВременнаяТаблицаСтавкиНСП.СтавкаНСП
		|ГДЕ
		|	НЕ ВременнаяТаблицаУслуги.СуммаСкидкиВРегВалюте = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|			ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаОС.ВсегоВРегВалюте - ВременнаяТаблицаОС.СуммаСкидкиВРегВалюте - ВременнаяТаблицаОС.СуммаНДСВРегВалюте - ВременнаяТаблицаОС.СуммаНСПВРегВалюте + ВременнаяТаблицаОС.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|		ИНАЧЕ ВременнаяТаблицаОС.СуммаДоходаВРегВалюте
		|	КОНЕЦ,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВЫБОР
		...
		|ГДЕ
		|	НЕ ВременнаяТаблицаОС.ВсегоВРегВалюте = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	ВременнаяТаблицаОС.СуммаНДСВРегВалюте,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		...
		|ГДЕ
		|	&ПлательщикНДС
		|	И НЕ ВременнаяТаблицаОС.СуммаНДСВРегВалюте = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	ВременнаяТаблицаОС.СуммаНСПВРегВалюте,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		...
		|		ПО (ИСТИНА)
		|ГДЕ
		|	НЕ ВременнаяТаблицаОС.СуммаНСПВРегВалюте = 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|	ВременнаяТаблицаШапка.ДоговорКонтрагента,
		|	НЕОПРЕДЕЛЕНО,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаОС.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2)),
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		...
		|		ПО ВременнаяТаблицаШапка.СтавкаНСПОС = ВременнаяТаблицаСтавкиНСП.СтавкаНСП
		|ГДЕ
		|	НЕ ВременнаяТаблицаОС.СуммаСкидкиВРегВалюте = 0";
...
Процедура СформироватьТаблицаПродажи(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	СУММА(ВЫРАЗИТЬ(ВременнаяТаблицаТовары.ВсегоВРегВалюте - ВременнаяТаблицаТовары.СуммаНДСВРегВалюте - ВременнаяТаблицаТовары.СуммаНСПВРегВалюте КАК ЧИСЛО(15, 2))) КАК Сумма,
		|	СУММА(ВременнаяТаблицаТовары.СуммаНДСВРегВалюте) КАК СуммаНДС,
		|	СУММА(ВременнаяТаблицаТовары.СуммаНСПВРегВалюте) КАК СуммаНСП,
		|	СУММА(ВЫРАЗИТЬ(ВременнаяТаблицаТовары.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))) КАК СуммаСкидки,
		|	СУММА(ВЫБОР
		|			КОГДА ВременнаяТаблицаТовары.СуммаСкидкиВРегВалюте <> 0
		|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаТовары.ВсегоВРегВалюте - ВременнаяТаблицаТовары.СуммаСкидкиВРегВалюте - ВременнаяТаблицаТовары.СуммаНДСВРегВалюте - ВременнаяТаблицаТовары.СуммаНСПВРегВалюте + ВременнаяТаблицаТовары.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|			ИНАЧЕ ВременнаяТаблицаТовары.СуммаДоходаВРегВалюте
		...
		|	СУММА(ВременнаяТаблицаУслуги.ВсегоВРегВалюте - ВременнаяТаблицаУслуги.СуммаНДСВРегВалюте - ВременнаяТаблицаУслуги.СуммаНСПВРегВалюте),
		|	СУММА(ВременнаяТаблицаУслуги.СуммаНДСВРегВалюте),
		|	СУММА(ВременнаяТаблицаУслуги.СуммаНСПВРегВалюте),
		|	СУММА(ВЫРАЗИТЬ(ВременнаяТаблицаУслуги.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))),
		|	СУММА(ВЫБОР
		|			КОГДА ВременнаяТаблицаУслуги.СуммаСкидкиВРегВалюте <> 0
		|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаУслуги.ВсегоВРегВалюте - ВременнаяТаблицаУслуги.СуммаСкидкиВРегВалюте - ВременнаяТаблицаУслуги.СуммаНДСВРегВалюте - ВременнаяТаблицаУслуги.СуммаНСПВРегВалюте + ВременнаяТаблицаУслуги.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|			ИНАЧЕ ВременнаяТаблицаУслуги.СуммаДоходаВРегВалюте
		|		КОНЕЦ),
		|	СУММА(0)
...
Процедура СформироватьТаблицаПродажиОсновныхСредств(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаШапка.БезналичныйРасчет КАК БезналичныйРасчет,
		|	ВременнаяТаблицаОС.ОсновноеСредство КАК ОсновноеСредство,
		|	СУММА(ВЫРАЗИТЬ(ВременнаяТаблицаОС.ВсегоВРегВалюте - ВременнаяТаблицаОС.СуммаНДСВРегВалюте - ВременнаяТаблицаОС.СуммаНСП КАК ЧИСЛО(15, 2))) КАК Сумма,
		|	СУММА(ВременнаяТаблицаОС.СуммаНДСВРегВалюте) КАК СуммаНДС,
		|	СУММА(ВременнаяТаблицаОС.СуммаНСПВРегВалюте) КАК СуммаНСП,
		|	СУММА(ВЫРАЗИТЬ(ВременнаяТаблицаОС.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))) КАК СуммаСкидки,
		|	СУММА(ВЫБОР
		|			КОГДА ВременнаяТаблицаОС.СуммаСкидкиВРегВалюте <> 0
		|				ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаОС.ВсегоВРегВалюте - ВременнаяТаблицаОС.СуммаСкидкиВРегВалюте - ВременнаяТаблицаОС.СуммаНДСВРегВалюте - ВременнаяТаблицаОС.СуммаНСПВРегВалюте + ВременнаяТаблицаОС.СуммаСкидкиВРегВалюте * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|			ИНАЧЕ ВременнаяТаблицаОС.СуммаДоходаВРегВалюте
		|		КОНЕЦ) КАК СуммаДохода,
		|	СУММА(0) КАК Себестоимость
...
Процедура СформироватьТаблицаНДС(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =	
		...
		|	ВременнаяТаблицаШапка.СтавкаНДС КАК СтавкаНДС,
		|	ВременнаяТаблицаТовары.СуммаНДСВРегВалюте КАК СуммаНДС,
		|	ВременнаяТаблицаТовары.СуммаНСПВРегВалюте КАК СуммаНСП,
		|	ВременнаяТаблицаТовары.ВсегоВРегВалюте - ВременнаяТаблицаТовары.СуммаНДСВРегВалюте - ВременнаяТаблицаТовары.СуммаНСПВРегВалюте КАК СуммаБезНДС
		|ИЗ
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаТовары КАК ВременнаяТаблицаТовары
		|		ПО (ИСТИНА)
		...
		|	ВременнаяТаблицаУслуги.СуммаНДСВРегВалюте,
		|	ВременнаяТаблицаУслуги.СуммаНСПВРегВалюте,
		|	ВременнаяТаблицаУслуги.ВсегоВРегВалюте - ВременнаяТаблицаУслуги.СуммаНДСВРегВалюте - ВременнаяТаблицаУслуги.СуммаНСПВРегВалюте
		|ИЗ
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаУслуги КАК ВременнаяТаблицаУслуги
		|		ПО (ИСТИНА)
		...
		|	ВременнаяТаблицаОС.СуммаНДСВРегВалюте,
		|	ВременнаяТаблицаОС.СуммаНСПВРегВалюте,
		|	ВременнаяТаблицаОС.ВсегоВРегВалюте - ВременнаяТаблицаОС.СуммаНДСВРегВалюте - ВременнаяТаблицаОС.СуммаНСПВРегВалюте
		|ИЗ
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаОС КАК ВременнаяТаблицаОС
		|		ПО (ИСТИНА)
...
Процедура СформироватьТаблицаДвижениеОСНУ(ДокументСсылка, СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаОС.ОсновноеСредство КАК ОсновноеСредство,
		|	ВременнаяТаблицаОС.ВсегоВРегВалюте - ВременнаяТаблицаОС.СуммаНДСВРегВалюте - ВременнаяТаблицаОС.СуммаНСПВРегВалюте КАК Выбытие,
		|	ВЫРАЗИТЬ((ВременнаяТаблицаОС.ВсегоВРегВалюте - ВременнаяТаблицаОС.СуммаНДСВРегВалюте - ВременнаяТаблицаОС.СуммаНСПВРегВалюте) * (12 - МЕСЯЦ(ВременнаяТаблицаШапка.Дата) + 0.5) / 12 КАК ЧИСЛО(15, 2)) КАК ТекущееВыбытие
		|ИЗ
		|	ВременнаяТаблицаОС КАК ВременнаяТаблицаОС
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|		ПО (ИСТИНА)
...
Процедура СформироватьТаблицаСчетаФактурыВыписанные(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаТовары.ВсегоВРегВалюте - ВременнаяТаблицаТовары.СуммаНДСВРегВалюте - ВременнаяТаблицаТовары.СуммаНСПВРегВалюте КАК СуммаБезНДС,
	|	ВременнаяТаблицаТовары.СуммаНДСВРегВалюте КАК СуммаНДС,
	|	ВременнаяТаблицаТовары.СуммаНСПВРегВалюте КАК СуммаНеоблагаемая
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, СтруктураДополнительныеСвойства, Отказ) Экспорт 
	...
	СтруктураТаблицЗначений = ПолучитьТаблицыПересчитанныхДанныхТабличныхЧастей(ДокументСсылка);
	...
	Запрос.Текст = 
		...
		|	ТаблицаДокумента.ВсегоВРегВалюте КАК ВсегоВРегВалюте,
		|	ТаблицаДокумента.СуммаВРегВалюте КАК СуммаВРегВалюте,
		|	ТаблицаДокумента.СуммаДоходаВРегВалюте КАК СуммаДоходаВРегВалюте,
		|	ТаблицаДокумента.СуммаНДСВРегВалюте КАК СуммаНДСВРегВалюте,
		|	ТаблицаДокумента.СуммаНСПВРегВалюте КАК СуммаНСПВРегВалюте,
		|	ТаблицаДокумента.СуммаСкидкиВРегВалюте КАК СуммаСкидкиВРегВалюте,
		...
		|ВЫБРАТЬ
		|	ТаблицаДокумента.ОсновноеСредство КАК ОсновноеСредство,
		|	ТаблицаДокумента.ЭтоКомплект КАК ЭтоКомплект,
		|	ТаблицаДокумента.Всего КАК Всего,
		|	ТаблицаДокумента.Сумма КАК Сумма,
		|	ТаблицаДокумента.СуммаДохода КАК СуммаДохода,
		|	ТаблицаДокумента.СуммаНДС КАК СуммаНДС,
		...
		|ИЗ
		|	&ТаблицаОС КАК ТаблицаДокумента
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("Период", 					 СтруктураДополнительныеСвойства.ДляПроведения.Дата);
	Запрос.УстановитьПараметр("Организация", 				 СтруктураДополнительныеСвойства.ДляПроведения.Организация);
	Запрос.УстановитьПараметр("ТаблицаТовары", 				 СтруктураТаблицЗначений.ТаблицаТовары);
	Запрос.УстановитьПараметр("ТаблицаУслуги", 				 СтруктураТаблицЗначений.ТаблицаУслуги);
	Запрос.УстановитьПараметр("ТаблицаОС", 					 СтруктураТаблицЗначений.ТаблицаОС);
	РезультатЗапроса = Запрос.Выполнить();
				...
				Для Каждого Выборка Из МассивВыборок Цикл
					...
					Пока Выборка.Следующий() Цикл
						...
						Если Шапка.ПечатьВРегламентированнойВалюте Тогда
							...
							СтруктураСтроки.Вставить("Сумма", 	 Выборка.Сумма * Шапка.Курс / Шапка.Кратность);
							СтруктураСтроки.Вставить("СуммаНДС", Выборка.СуммаНДС * Шапка.Курс / Шапка.Кратность);
							СтруктураСтроки.Вставить("СуммаНСП", Выборка.СуммаНСП * Шапка.Курс / Шапка.Кратность);
							СтруктураСтроки.Вставить("Всего", 	 Выборка.Всего * Шапка.Курс / Шапка.Кратность);
							
							Всего = Всего + СтруктураСтроки.Всего;
							Сумма = Сумма + СтруктураСтроки.Сумма;
							ВсегоНДС = ВсегоНДС + СтруктураСтроки.СуммаНСП;
						...
						Иначе
							...
							ВсегоНДС = ВсегоНДС + Выборка.СуммаНСП;
...
Функция ПечатьТТН(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		ДанныеПечати.Вставить("МассаГрузаНеттоПрописью", ЧислоПрописью(Шапка.МассаГруза, ,"тонна, тонны, тонн, ж,,,,,0") 
...
// Возвращает структуру таблиц значений с пересчитанными суммами в валюте регламентированного учета.
//
// Параметры:
//  ДокументСсылка - ДокументСсылка.РеализацияТоваровУслуг - ссылка на документ реализации.
//
// Возвращаемое значение:
//  Структура - структура содержащая 3 таблицы значений: по товарам, по услугам и по ОС.
// 
Функция ПолучитьТаблицыПересчитанныхДанныхТабличныхЧастей(ДокументСсылка)
	...
	КвалификаторЧисла_15_2 = Новый КвалификаторыЧисла(15, 2);	
	
	ТаблицаТовары = ДокументСсылка.Товары.Выгрузить().СкопироватьКолонки();
	ТаблицаТовары.Колонки.Добавить("НоменклатурнаяГруппа", 	Новый ОписаниеТипов("СправочникСсылка.НоменклатурныеГруппы"));
	ТаблицаТовары.Колонки.Добавить("ВсегоВРегВалюте", 		Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаТовары.Колонки.Добавить("СуммаВРегВалюте", 		Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаТовары.Колонки.Добавить("СуммаНДСВРегВалюте", 	Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаТовары.Колонки.Добавить("СуммаНСПВРегВалюте", 	Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаТовары.Колонки.Добавить("СуммаСкидкиВРегВалюте", Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаТовары.Колонки.Добавить("СуммаДоходаВРегВалюте", Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	
	ТаблицаУслуги = ДокументСсылка.Услуги.Выгрузить().СкопироватьКолонки();
	ТаблицаУслуги.Колонки.Добавить("НоменклатурнаяГруппа", 	Новый ОписаниеТипов("СправочникСсылка.НоменклатурныеГруппы"));
	ТаблицаУслуги.Колонки.Добавить("ВсегоВРегВалюте", 		Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаУслуги.Колонки.Добавить("СуммаВРегВалюте", 		Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаУслуги.Колонки.Добавить("СуммаНДСВРегВалюте", 	Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаУслуги.Колонки.Добавить("СуммаНСПВРегВалюте", 	Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаУслуги.Колонки.Добавить("СуммаСкидкиВРегВалюте", Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаУслуги.Колонки.Добавить("СуммаДоходаВРегВалюте", Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	
	ТаблицаОС = ДокументСсылка.ОС.Выгрузить().СкопироватьКолонки();
	ТаблицаОС.Колонки.Добавить("ЭтоКомплект",				Новый ОписаниеТипов("Булево"));
	ТаблицаОС.Колонки.Добавить("ВсегоВРегВалюте", 			Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаОС.Колонки.Добавить("СуммаВРегВалюте", 			Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаОС.Колонки.Добавить("СуммаНДСВРегВалюте", 		Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаОС.Колонки.Добавить("СуммаНСПВРегВалюте", 		Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаОС.Колонки.Добавить("СуммаСкидкиВРегВалюте", 	Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	ТаблицаОС.Колонки.Добавить("СуммаДоходаВРегВалюте", 	Новый ОписаниеТипов(МассивЧисло, КвалификаторЧисла_15_2));
	...
	Для Каждого ТабличнаяЧасть Из МассивТабличныхЧастей Цикл
		...
		Если ТабличнаяЧасть = "Товары" Тогда
			СтавкаНСП = ДокументСсылка.СтавкаНСП;
		ИначеЕсли ТабличнаяЧасть = "Услуги" Тогда
			СтавкаНСП = ДокументСсылка.СтавкаНСПУслуги;
		Иначе
			СтавкаНСП = ПредопределенноеЗначение("Справочник.СтавкиНСП.Прочее");
		КонецЕсли;
		
		Для Каждого СтрокаТабличнойЧасти Из ДокументСсылка[ТабличнаяЧасть] Цикл
			...
			Структура.Вставить("СтавкаНДС", 			ДокументСсылка.СтавкаНДС);
			Структура.Вставить("СтавкаНСП", 			СтавкаНСП);
			
			Если ДокументСсылка.ВидСкидкиНаценки = Перечисления.ВидыСкидокНаценок.СуммаПоСтроке Тогда
				Структура.Вставить("СуммаСкидки", Окр(СтрокаТабличнойЧасти.СуммаСкидки * ДокументСсылка.Курс / ДокументСсылка.Кратность, 2));	
			Иначе	
				Структура.Вставить("СуммаСкидки", СтрокаТабличнойЧасти.СуммаСкидки);
			КонецЕсли;
			
			Если ТабличнаяЧасть = "ОС" Тогда
				Если ДокументСсылка.СуммаВключаетНалоги Тогда
					Структура.Вставить("Сумма", 	 	Окр(СтрокаТабличнойЧасти.Сумма * ДокументСсылка.Курс / ДокументСсылка.Кратность, 2));
					Структура.Вставить("СуммаДохода", 	СтрокаТабличнойЧасти.СуммаДохода);	
				Иначе
					Структура.Вставить("Сумма", 	 	СтрокаТабличнойЧасти.Сумма);
					Структура.Вставить("СуммаДохода", 	Окр(СтрокаТабличнойЧасти.СуммаДохода * ДокументСсылка.Курс / ДокументСсылка.Кратность, 2));	
				КонецЕсли;	
				
			Иначе
				Структура.Вставить("Цена", 	 		Окр(СтрокаТабличнойЧасти.Цена * ДокументСсылка.Курс / ДокументСсылка.Кратность, 2));
			...
			Если ДокументСсылка.СуммаВключаетНалоги Тогда
				...
				ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммыНалоговСтрокиТабличнойЧасти(
					...
					Истина, 
					?(ДанныеУчетнойПолитики.ПлательщикНДС, ДокументСсылка.СтавкаНДС, ПредопределенноеЗначение("Справочник.СтавкиНДС.ПустаяСсылка")),
					?(ДанныеУчетнойПолитики.ПлательщикНСП, СтавкаНСП, ПредопределенноеЗначение("Справочник.СтавкиНСП.ПустаяСсылка")), 
					ДокументСсылка.БезналичныйРасчет);
			...
			Иначе
				...
				СтруктураДопПараметров.Вставить("Период", 			 ДокументСсылка.Дата);
				СтруктураДопПараметров.Вставить("Организация", 		 ДокументСсылка.Организация);
				СтруктураДопПараметров.Вставить("БезналичныйРасчет", ДокументСсылка.БезналичныйРасчет);
				СтруктураДопПараметров.Вставить("СтавкаНДС", 		 ДокументСсылка.СтавкаНДС);
			...
			Если ТабличнаяЧасть = "Товары" Тогда
				...
				СтрокаТаблицы.НоменклатурнаяГруппа 	= СтрокаТабличнойЧасти.Номенклатура.НоменклатурнаяГруппа;
				СтрокаТаблицы.ВсегоВРегВалюте 		= Структура.Всего;
				СтрокаТаблицы.СуммаВРегВалюте 		= Структура.Сумма;
				СтрокаТаблицы.СуммаНДСВРегВалюте 	= Структура.СуммаНДС;
				СтрокаТаблицы.СуммаНСПВРегВалюте 	= Структура.СуммаНСП;
				СтрокаТаблицы.СуммаСкидкиВРегВалюте = Структура.СуммаСкидки;
				СтрокаТаблицы.СуммаДоходаВРегВалюте = Структура.СуммаДохода;
				
			ИначеЕсли ТабличнаяЧасть = "Услуги" Тогда
				СтрокаТаблицы = ТаблицаУслуги.Добавить();
			...
			Иначе
				...
				ЗаполнитьЗначенияСвойств(СтрокаТаблицы, СтрокаТабличнойЧасти);
				СтрокаТаблицы.ЭтоКомплект 			= СтрокаТабличнойЧасти.ОсновноеСредство.ЭтоКомплект;
				СтрокаТаблицы.ВсегоВРегВалюте 		= Структура.Всего;
				СтрокаТаблицы.СуммаВРегВалюте 		= Структура.Сумма;
				СтрокаТаблицы.СуммаНДСВРегВалюте 	= Структура.СуммаНДС;
				СтрокаТаблицы.СуммаНСПВРегВалюте 	= Структура.СуммаНСП;
	...
	СтруктураТаблицЗначений = Новый Структура();
	СтруктураТаблицЗначений.Вставить("ТаблицаТовары", 	ТаблицаТовары);
	СтруктураТаблицЗначений.Вставить("ТаблицаУслуги", 	ТаблицаУслуги);
	СтруктураТаблицЗначений.Вставить("ТаблицаОС", 		ТаблицаОС);
	
	Возврат СтруктураТаблицЗначений;
КонецФункции
