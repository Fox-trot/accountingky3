...
//// Процедура определяет готовность заказов и в зависимости от
//// этого делает проводку в РС "СостояниеЗаказов".
//Процедура СформироватьТаблицуСостоянияЗаказов(СтруктураДополнительныеСвойства) Экспорт

//	Если СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаСписанныеТовары.Количество() Тогда
//		
//		СчетГотовойПродукции = ПланыСчетов.Хозрасчетный.ГотоваяПродукция;
...
//		
//		Для Каждого СтрокаТаблицыТовары Из СтруктураДополнительныеСвойства.ТаблицыДляДвижений.ТаблицаСписанныеТовары Цикл
//			Если СтрокаТаблицыТовары.СчетУчета = СчетГотовойПродукции Тогда
//				СтрокаТаблицыЗаказов = ТаблицаСостояниеЗаказов.Добавить();
//				СтрокаТаблицыЗаказов.Период = Дата;
//				СтрокаТаблицыЗаказов.Организация = Организация;
//				СтрокаТаблицыЗаказов.Заказ = СтрокаТаблицыТовары.Партия;
//				СтрокаТаблицыЗаказов.Состояние = Перечисления.СостояниеРаботПоПроизводству.Реализован;
//			КонецЕсли;				
//		КонецЦикла;
//		
//		СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаСостояниеЗаказов", ТаблицаСостояниеЗаказов);
//	КонецЕсли;
//КонецПроцедуры
...
Функция ПечатьСчетФактура(МассивОбъектов, ОбъектыПечати, КоличествоСтрокСчетаФактуры)
	...
	ТекстЗапроса =
		...
		|	РеализацияТоваровУслуг.Организация.ФайлФаксимильнаяПечать КАК ФайлФаксимильнаяПечать,
		...
		|	РеализацияТоваровУслуг.Контрагент.НаименованиеПолное КАК КонтрагентПредставление,
		|	РеализацияТоваровУслуг.Контрагент.ИНН КАК ИННКонтрагента,
		|	РеализацияТоваровУслуг.Контрагент.ГНС КАК ГНСКонтрагента,
		|	РеализацияТоваровУслуг.Контрагент.ГНС.Код КАК КодГНСКонтрагента,
		|	РеализацияТоваровУслуг.Контрагент.ОсновнойБанковскийСчет КАК БанковскийСчетКонтрагента,
		|	РеализацияТоваровУслуг.Контрагент.ОсновнойБанковскийСчет.НомерСчета КАК НомерБанковскогоСчетаКонтрагента,
		|	РеализацияТоваровУслуг.Контрагент.ОсновнойБанковскийСчет.Банк.Код КАК БикБанкаКонтрагента,
		|	РеализацияТоваровУслуг.Контрагент.ОсновнойБанковскийСчет.Банк.Наименование КАК НаименованиеБанкаКонтрагента,
		...
		|	МАКСИМУМ(ТаблицаТовары.Номенклатура.ЕдиницаИзмерения.Наименование),
		|	СУММА(ТаблицаТовары.Количество),
		|	СРЕДНЕЕ(ТаблицаТовары.СуммаДохода / ТаблицаТовары.Количество),
		|	СУММА(ТаблицаТовары.Сумма),
		|	СУММА(ТаблицаТовары.СуммаНДС),
		|	СУММА(ТаблицаТовары.СуммаНСП),
		|	СУММА(ТаблицаТовары.Всего),
		|	СУММА(ТаблицаТовары.СуммаДохода),
		|	СУММА(ТаблицаТовары.СуммаСкидки),
		|	МАКСИМУМ(ТаблицаТовары.Номенклатура.КодТНВЭД)
		|ИЗ
		|	Документ.РеализацияТоваровУслуг.Товары КАК ТаблицаТовары
		|ГДЕ
		|	ТаблицаТовары.Ссылка В(&СписокДокументов)
		...
		|	ТаблицаУслуги.Номенклатура.ЕдиницаИзмерения.Наименование КАК ЕдиницаИзмерения,
		...
		|	СРЕДНЕЕ(ТаблицаУслуги.СуммаДохода / ТаблицаУслуги.Количество),
		...
		//|	ВЫБОР
		//|		КОГДА ВременнаяТаблицаДокументыСИсходнымиСуммами.СуммаВключаетНалоги
		//|			ТОГДА ВременнаяТаблицаСтроки.Сумма - ВременнаяТаблицаСтроки.СуммаНДС - ВременнаяТаблицаСтроки.СуммаНСП
		...
		//|	ВЫБОР
		//|		КОГДА ВременнаяТаблицаДокументыДляПересчета.СуммаВключаетНалоги
		//|			ТОГДА ВЫРАЗИТЬ((ВременнаяТаблицаСтроки.Сумма - ВременнаяТаблицаСтроки.СуммаНДС - ВременнаяТаблицаСтроки.СуммаНСП) * ВременнаяТаблицаДокументыДляПересчета.Курс / ВременнаяТаблицаДокументыДляПересчета.Кратность КАК ЧИСЛО(15, 2))
		//|		ИНАЧЕ ВЫРАЗИТЬ(ВременнаяТаблицаСтроки.Сумма * ВременнаяТаблицаДокументыДляПересчета.Курс / ВременнаяТаблицаДокументыДляПересчета.Кратность КАК ЧИСЛО(15, 2))
		//|	КОНЕЦ КАК Стоимость,
		...
		|	ВременнаяТаблицаШапка.Контрагент КАК Контрагент,
		|	ВременнаяТаблицаШапка.КонтрагентПредставление КАК КонтрагентПредставление,
		|	ВременнаяТаблицаШапка.ИННКонтрагента КАК ИННКонтрагента,
		|	ВременнаяТаблицаШапка.ГНСКонтрагента КАК ГНСКонтрагента,
		|	ВременнаяТаблицаШапка.КодГНСКонтрагента КАК КодГНСКонтрагента,
		|	ВременнаяТаблицаШапка.БанковскийСчетКонтрагента КАК БанковскийСчетКонтрагента,
		|	ВременнаяТаблицаШапка.НомерБанковскогоСчетаКонтрагента КАК НомерБанковскогоСчетаКонтрагента,
		|	ВременнаяТаблицаШапка.БикБанкаКонтрагента КАК БикБанкаКонтрагента,
		|	ВременнаяТаблицаШапка.НаименованиеБанкаКонтрагента КАК НаименованиеБанкаКонтрагента,
	...
	Для Каждого ТаблицаЗначений Из МассивТаблицЗначений Цикл
		Для Каждого СтрокаТаблицы Из ТаблицаЗначений Цикл
			...
			Если СтруктураПеременных.Свойство("Курс") Тогда
				//ДанныеСтроки.Сумма = Окр((СтрокаТаблицы.Сумма + ?(СтруктураПеременных.СуммаВключаетНалоги, 0, СтрокаТаблицы.СуммаНДС)) * СтруктураПеременных.Курс, 2);
			...
			Иначе
				//ДанныеСтроки.Сумма = Окр(СтрокаТаблицы.Сумма + ?(СтруктураПеременных.СуммаВключаетНалоги, 0, СтрокаТаблицы.СуммаНДС), 2);
