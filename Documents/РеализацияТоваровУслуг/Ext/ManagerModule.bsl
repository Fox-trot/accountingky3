...
Процедура СформироватьТаблицаХозрасчетный(ДокументСсылка,СтруктураДополнительныеСвойства, Отказ)
	...
	Запрос.Текст =
		...
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	ВременнаяТаблицаОС.ОстаточнаяСтоимость,
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	0,
		|	0,
		...
		|		ПО (ИСТИНА)
		|ГДЕ
		|	ВременнаяТаблицаОС.ОстаточнаяСтоимость > 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, СтруктураДополнительныеСвойства, Отказ) Экспорт 
	...
	Запрос.Текст = 
		...
		|	ТаблицаДокумента.НакопленнаяАмортизацияДокумента КАК НакопленнаяАмортизация,
...
Функция ТаблицаОСВходящихВКомплекты(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	0 КАК НакопленнаяАмортизацияДокумента,
