...
Функция ПечатьСчетФактура(МассивОбъектов, ОбъектыПечати, КоличествоСтрокСчетаФактуры)
	...
	ТекстЗапроса =
		...
		|	ТаблицаДокумента.Контрагент.ОсновнойБанковскийСчет КАК БанковскийСчетКонтрагента,
		...
		|	ТаблицаДокумента.ВидПлатежа КАК ВидПлатежа,
		...
		|		Всего КАК Всего,
		|		ВЫБОР
		|			КОГДА ТаблицаДокумента.Товары.Ссылка.СуммаВключаетНалоги
		|				ТОГДА ТаблицаДокумента.Товары.Сумма - ТаблицаДокумента.Товары.СуммаНДС - ТаблицаДокумента.Товары.СуммаНСП
		|			ИНАЧЕ ТаблицаДокумента.Товары.Сумма
		|		КОНЕЦ КАК Стоимость,
		...
		|		Всего КАК Всего,
		|		ВЫБОР
		|			КОГДА ТаблицаДокумента.Услуги.Ссылка.СуммаВключаетНалоги
		|				ТОГДА ТаблицаДокумента.Услуги.Сумма - ТаблицаДокумента.Услуги.СуммаНДС - ТаблицаДокумента.Услуги.СуммаНСП
		|			ИНАЧЕ ТаблицаДокумента.Услуги.Сумма
		|		КОНЕЦ КАК Стоимость,
		|		СуммаНДС КАК СуммаНДС,
		|		СуммаНСП КАК СуммаНСП,
		|		СуммаСкидки КАК СуммаСкидки,
	...
	Пока Шапка.Следующий() Цикл
		...
		// Итоги
		ВсегоСтоимость = ТаблицаТовары.Итог("Стоимость") + ТаблицаУслуги.Итог("Стоимость");
...
Функция ПечатьТоварныйЧек(МассивОбъектов, ОбъектыПечати)
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РеализацияТоваровУслуг.удалитьПФ_MXL_ТоварныйЧек");
...
Функция ПечатьТТН(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		Для Каждого СтрокаТовары Из ТаблицаТовары Цикл
			...
			ДанныеСтроки.Номер = НомерСтроки;
			ДанныеСтроки.Сумма = Окр((СтрокаТовары.Сумма + ?(Шапка.СуммаВключаетНалоги, 0, СтрокаТовары.СуммаНДС)) * Шапка.Курс, 2);
			ДанныеСтроки.Цена = Окр(СтрокаТовары.Цена * Шапка.Курс, 2);
			ДанныеСтроки.Количество = Окр(СтрокаТовары.Количество, 2);
		...
		ДанныеПечати.Вставить("КоличествоПорядковыхНомеровЗаписейПрописью", ЧислоПрописью(НомерСтроки, ,",,,,,,,,0"));
		ДанныеПечати.Вставить("СуммаПрописью", БухгалтерскийУчетСервер.СформироватьСуммуПрописью(ИтоговыеСуммы.ИтогоСумма, Шапка.ВалютаДокумента));
		...
		ДанныеПечати.Вставить("ВсегоНаименованийПрописью", ЧислоПрописью(КоличествоСтрок, ,",,,,,,,,0"));
		// Подписи.
...
Функция ПечатьТТНВВалюте(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		Для Каждого СтрокаТовары Из ТаблицаТовары Цикл		
			...
			ДанныеСтроки = СтруктураДанныеСтрокиТТН();
			НомерСтроки = НомерСтроки + 1;
			ДанныеСтроки.Номер = НомерСтроки + 1;
			ДанныеСтроки.Сумма = Окр((СтрокаТовары.Сумма + ?(Шапка.СуммаВключаетНалоги, 0, СтрокаТовары.СуммаНДС)), 2);
			ДанныеСтроки.Количество = Окр(СтрокаТовары.Количество, 2);
