...
Процедура СформироватьТаблицаХозрасчетный(ДокументСсылка,СтруктураДополнительныеСвойства, Отказ)
	...
	Запрос.Текст =
		...
		|	ВЫРАЗИТЬ(ВременнаяТаблицаТовары.Доход * ВременнаяТаблицаШапка.Курс / ВременнаяТаблицаШапка.Кратность КАК ЧИСЛО(15, 2)) КАК Сумма,
		|	ВременнаяТаблицаШапка.ВалютаДокумента КАК ВалютаДт,
		|	ВременнаяТаблицаШапка.ВалютаДокумента КАК ВалютаКт,
		|	ВременнаяТаблицаТовары.Доход КАК ВалютнаяСуммаДт,
		|	ВременнаяТаблицаТовары.Доход КАК ВалютнаяСуммаКт,
		...
		|	КОНЕЦ,
		|	НЕОПРЕДЕЛЕНО,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаУслуги.Доход * ВременнаяТаблицаШапка.Курс / ВременнаяТаблицаШапка.Кратность КАК ЧИСЛО(15, 2)),
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаУслуги.Доход,
		|	ВременнаяТаблицаУслуги.Доход,
		...
		|	НЕОПРЕДЕЛЕНО,
		|	НЕОПРЕДЕЛЕНО,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаОС.Доход * ВременнаяТаблицаШапка.Курс / ВременнаяТаблицаШапка.Кратность КАК ЧИСЛО(15, 2)),
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаШапка.ВалютаДокумента,
		|	ВременнаяТаблицаОС.Доход,
		|	ВременнаяТаблицаОС.Доход,
		|	&СодержаниеОСДоход,
		|	0,
		|	0,
...
Процедура СформироватьТаблицаПродажи(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	СУММА(ВЫРАЗИТЬ(ВременнаяТаблицаУслуги.СуммаСкидки * ВременнаяТаблицаШапка.Курс / ВременнаяТаблицаШапка.Кратность КАК ЧИСЛО(15, 2))) КАК СуммаСкидки,
		|	СУММА(0)
		|ИЗ
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаУслуги КАК ВременнаяТаблицаУслуги
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, СтруктураДополнительныеСвойства, Отказ) Экспорт 
	...
	Запрос.Текст = 
		...
		|	ТаблицаДокумента.Доход КАК Доход,
...
Функция ПечатьСчетФактура(МассивОбъектов, ОбъектыПечати, КоличествоСтрокСчетаФактуры)
	...
	ТекстЗапроса =
		...
		|	ВременнаяТаблицаСтроки.Доход КАК Стоимость,
		...
		|	ВЫРАЗИТЬ(ВременнаяТаблицаСтроки.Доход * ВременнаяТаблицаДокументыДляПересчета.Курс / ВременнаяТаблицаДокументыДляПересчета.Кратность КАК ЧИСЛО(15, 2)) КАК Стоимость,
