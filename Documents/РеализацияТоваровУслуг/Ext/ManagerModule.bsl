...
Процедура СформироватьТаблицаПродажи(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	СУММА(ВЫРАЗИТЬ(ВременнаяТаблицаТовары.СуммаСкидкиВВалютеРеглУчета * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))) КАК СуммаСкидки,
		|	СУММА(ВЫБОР
		...
		|	СУММА(ВЫРАЗИТЬ(ВременнаяТаблицаУслуги.СуммаСкидкиВВалютеРеглУчета * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))),
		|	СУММА(ВЫБОР
					...
					Для Каждого СтрокаТаблицы Из ИсходнаяТаблица Цикл
						...
						Иначе
							...
							Если СчетФактураВВалюте Тогда
								...
								СтрутктураЦены.Вставить("Цена", Строка(СтрокаТаблицы.Цена) + " (" + Шапка.ВалютаДокумента + ")");
							...
							Если Строки.Количество() > 0 Тогда
								...
								Если СчетФактураВВалюте Тогда
									...
									СтрутктураСуммВРеглВалюте.Вставить("ЦенаВРеглВалюте", 		Строка(Данные.Цена) + " (" + ВалютаРегламентированногоУчета + ")");
