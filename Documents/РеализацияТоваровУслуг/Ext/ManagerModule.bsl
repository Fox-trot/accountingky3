...
Функция ПечатьСчетФактура(МассивОбъектов, ОбъектыПечати, КоличествоСтрокСчетаФактуры)
	...
	ТекстЗапроса =
		...
		|		Всего КАК Всего,
		|		ВЫБОР
		|			КОГДА ТаблицаДокумента.ОС.Ссылка.СуммаВключаетНалоги
		|				ТОГДА ТаблицаДокумента.ОС.Сумма - ТаблицаДокумента.ОС.СуммаНДС - ТаблицаДокумента.ОС.СуммаНСП
		|			ИНАЧЕ ТаблицаДокумента.ОС.Сумма
		|		КОНЕЦ КАК Стоимость,
		|		СуммаНДС КАК СуммаНДС,
		|		СуммаНСП КАК СуммаНСП,
		|		СуммаСкидки КАК СуммаСкидки,
	...
	Пока Шапка.Следующий() Цикл
		...
		// Итоги
		ВсегоСтоимость = ТаблицаТовары.Итог("Стоимость") + ТаблицаУслуги.Итог("Стоимость") + ТаблицаОС.Итог("Стоимость");
		Всего = ТаблицаТовары.Итог("Всего") + ТаблицаУслуги.Итог("Всего") + ТаблицаОС.Итог("Всего");
		ВсегоНДС = ТаблицаТовары.Итог("СуммаНДС") + ТаблицаУслуги.Итог("СуммаНДС") + ТаблицаОС.Итог("СуммаНДС");
		ВсегоНСП = ТаблицаТовары.Итог("СуммаНСП") + ТаблицаУслуги.Итог("СуммаНСП") + ТаблицаОС.Итог("СуммаНСП");
		ВсегоСуммаСкидки = ТаблицаТовары.Итог("СуммаСкидки") + ТаблицаУслуги.Итог("СуммаСкидки") + ТаблицаОС.Итог("СуммаСкидки");
		КоличествоСтрок = ТаблицаТовары.Количество() + ТаблицаУслуги.Количество() + ТаблицаОС.Количество();
