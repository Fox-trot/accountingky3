...
Процедура СформироватьТаблицаСчетаФактурыВыписанные(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	|	ВременнаяТаблицаШапка.ДоговорКонтрагента КАК ДоговорКонтрагента,
	|	ВременнаяТаблицаШапка.СтавкаНДС КАК СтавкаНДС,
	|	ВременнаяТаблицаТовары.СуммаДоходаВВалютеРеглУчета КАК СуммаБезНДС,
	|	ВременнаяТаблицаТовары.СуммаНДСВВалютеРеглУчета КАК СуммаНДС,
...
Процедура СформироватьТаблицаКорректировкиНУ(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	ТекстЗапроса = 
		...
		|	И НЕ Хозрасчетный.Ссылка = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.ДоходыОтРеализацииНеамортизируемыхАктивов)
		|	И НЕ Хозрасчетный.Ссылка = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.РасходыПоВыбытиюНеамортизируемыхАктивов)
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	Счет";
	ТекстЗапроса = ТекстЗапроса + ОбщегоНазначенияБПСервер.ТекстРазделителяЗапросовПакета();
	
	// Таблица для движения в регистр Хозрасчетный.
	// см. СформироватьТаблицаХозрасчетный
	ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
	...
	// Отбор записей по Основным средствам с заполненой группой НУ.
	ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		...
		|ПОМЕСТИТЬ ВременнаяТаблицаХозрасчетныйОС
		|ИЗ
	...
	// Определение временных счетов в записях по Основным средствам по Дт и Кт.
	ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		|	ВременнаяТаблицаХозрасчетныйОС.Период КАК Период,
		|	ВременнаяТаблицаХозрасчетныйОС.Организация КАК Организация,
		|	ВременнаяТаблицаХозрасчетныйОС.СчетДт КАК Счет,
		|	ЗНАЧЕНИЕ(Перечисление.ВидыУчета.ВР) КАК ВидУчета,
		|	ВременнаяТаблицаХозрасчетныйОС.Сумма КАК Сумма
		|ПОМЕСТИТЬ ВременнаяТаблицаКорректировкиНУ
		|ИЗ
		|	ВременнаяТаблицаХозрасчетныйОС КАК ВременнаяТаблицаХозрасчетныйОС
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаСчетов КАК ВременнаяТаблицаСчетов
		|		ПО ВременнаяТаблицаХозрасчетныйОС.СчетДт = ВременнаяТаблицаСчетов.Счет
		|ГДЕ
		|	ВременнаяТаблицаХозрасчетныйОС.СчетДт В
		|			(ВЫБРАТЬ
		...
		|ИЗ
		|	ВременнаяТаблицаХозрасчетныйОС КАК ВременнаяТаблицаХозрасчетный
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаСчетов КАК ВременнаяТаблицаСчетов
	...
	// Результат для ТаблицаКорректировкиНУ. 
	ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
