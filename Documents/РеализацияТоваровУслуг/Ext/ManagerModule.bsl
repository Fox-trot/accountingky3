...
Процедура СформироватьТаблицаХозрасчетный(ДокументСсылка,СтруктураДополнительныеСвойства, Отказ)
	...
	ТекстЗапроса =
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаТовары.СуммаСкидки <> 0
		|			ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаТовары.ВсегоВВалютеРеглУчета - ВременнаяТаблицаТовары.СуммаНДСВВалютеРеглУчета - ВременнаяТаблицаТовары.СуммаНСПВВалютеРеглУчета - ВременнаяТаблицаТовары.СуммаСкидкиВВалютеРеглУчета * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|		ИНАЧЕ ВременнаяТаблицаТовары.СуммаДоходаВВалютеРеглУчета
		|	КОНЕЦ КАК Сумма,
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаТовары.СуммаСкидки <> 0
		|			ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаТовары.Всего - ВременнаяТаблицаТовары.СуммаНДС - ВременнаяТаблицаТовары.СуммаНСП - ВременнаяТаблицаТовары.СуммаСкидки * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|		ИНАЧЕ ВременнаяТаблицаТовары.СуммаДохода
		|	КОНЕЦ КАК ВалютнаяСуммаДт,
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|	ВременнаяТаблицаШапка.ДоговорКонтрагента КАК СубконтоДт2,
		|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
		|	ВременнаяТаблицаУслуги.СтатьяДохода КАК СубконтоКт1,
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаУслуги.СуммаСкидки <> 0
		|			ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаУслуги.ВсегоВВалютеРеглУчета - ВременнаяТаблицаУслуги.СуммаНДСВВалютеРеглУчета - ВременнаяТаблицаУслуги.СуммаНСПВВалютеРеглУчета - ВременнаяТаблицаУслуги.СуммаСкидкиВВалютеРеглУчета * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|		ИНАЧЕ ВременнаяТаблицаУслуги.СуммаДоходаВВалютеРеглУчета
		|	КОНЕЦ КАК Сумма,
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаУслуги.СуммаСкидки <> 0
		|			ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаУслуги.Всего - ВременнаяТаблицаУслуги.СуммаНДС - ВременнаяТаблицаУслуги.СуммаНСП - ВременнаяТаблицаУслуги.СуммаСкидки * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|		ИНАЧЕ ВременнаяТаблицаУслуги.СуммаДохода
		|	КОНЕЦ КАК ВалютнаяСуммаДт,
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|	ВременнаяТаблицаШапка.ДоговорКонтрагента КАК СубконтоДт2,
		|	НЕОПРЕДЕЛЕНО КАК СубконтоДт3,
		|	ВременнаяТаблицаОС.СтатьяДохода КАК СубконтоКт1,
		|	НЕОПРЕДЕЛЕНО КАК СубконтоКт2,
		|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаОС.СуммаСкидки <> 0
		|			ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаОС.ВсегоВВалютеРеглУчета - ВременнаяТаблицаОС.СуммаНДСВВалютеРеглУчета - ВременнаяТаблицаОС.СуммаНСПВВалютеРеглУчета - ВременнаяТаблицаОС.СуммаСкидкиВВалютеРеглУчета * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|		ИНАЧЕ ВременнаяТаблицаОС.СуммаДоходаВВалютеРеглУчета
		|	КОНЕЦ КАК Сумма,
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаОС.СуммаСкидки <> 0
		|			ТОГДА ВЫРАЗИТЬ(ВременнаяТаблицаОС.Всего - ВременнаяТаблицаОС.СуммаНДС - ВременнаяТаблицаОС.СуммаНСП - ВременнаяТаблицаОС.СуммаСкидки * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
		|		ИНАЧЕ ВременнаяТаблицаОС.СуммаДохода
		|	КОНЕЦ КАК ВалютнаяСуммаДт,
...
Процедура СформироватьТаблицаПродажиОсновныхСредств(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаШапка.ДоговорКонтрагента КАК Договор,
		|	ВременнаяТаблицаШапка.СтавкаНДС КАК СтавкаНДС,
		|	ВременнаяТаблицаШапка.СтавкаНСПОС КАК СтавкаНСП,
		|	ВременнаяТаблицаШапка.БезналичныйРасчет КАК БезналичныйРасчет,
		|	ВременнаяТаблицаОС.ОсновноеСредство КАК ОсновноеСредство,
...
Процедура СформироватьТаблицаСчетаФактурыВыписанные(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	//|	ВременнаяТаблицаТовары.СуммаДоходаВВалютеРеглУчета
	//|	-ВЫБОР
	//|		КОГДА ВременнаяТаблицаТовары.ВсегоВВалютеРеглУчета = ВременнаяТаблицаТовары.СуммаСкидкиВВалютеРеглУчета
	//|			ТОГДА ВременнаяТаблицаТовары.СуммаСкидкиВВалютеРеглУчета
	//|		ИНАЧЕ ВЫРАЗИТЬ(ВременнаяТаблицаТовары.СуммаСкидкиВВалютеРеглУчета * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
	...
	//|	ВременнаяТаблицаОС.СуммаДоходаВВалютеРеглУчета
	//|	-ВЫБОР
	//|		КОГДА ВременнаяТаблицаОС.ВсегоВВалютеРеглУчета = ВременнаяТаблицаОС.СуммаСкидкиВВалютеРеглУчета
	//|			ТОГДА ВременнаяТаблицаОС.СуммаСкидкиВВалютеРеглУчета
	//|		ИНАЧЕ ВЫРАЗИТЬ(ВременнаяТаблицаОС.СуммаСкидкиВВалютеРеглУчета * 100 / (100 + ЕСТЬNULL(ВременнаяТаблицаСтавкиНДС.ЗначениеСтавки, 0) + ЕСТЬNULL(ВременнаяТаблицаСтавкиНСП.ЗначениеСтавки, 0)) КАК ЧИСЛО(15, 2))
...
Функция ПечатьНакладной(МассивОбъектов, ОбъектыПечати)
	...
	ТекстЗапроса = 
		...
		|				ТОГДА ВЫРАЗИТЬ(Всего / Количество КАК ЧИСЛО(15, 2))
		|			ИНАЧЕ Цена
		|		КОНЕЦ КАК Цена,
		|		Сумма КАК Сумма,
...
//  Табличный документ - печатная форма
//
Функция ПечатьСчетФактура008(МассивОбъектов, ОбъектыПечати)
	Перем Ошибки, ПервыйДокумент, НомерСтрокиНачало;
	
	ТабличныйДокумент = Новый ТабличныйДокумент;		
	ТабличныйДокумент.КлючПараметровПечати = "РеализацияТоваровУслуг_СчетФактура";
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.РеализацияТоваровУслуг.ПФ_MXL_СчетФактура008");
	...
	Пока Шапка.Следующий() Цикл
		...
		КоличествоЛистов = КоличествоЛистовСчетФатуры008(Макет, Шапка);		
		...
		ДанныеПечати.СтавкаНСП = ЗначениеСтавкиНСПУслуги;
		...
		ДанныеПечати.СтавкаНСП = ЗначениеСтавкиНСПОС;
		...
		Пока ВыборкаОС.Следующий() Цикл
			...
			ОбластьСтрокаТаблицы.Параметры.Заполнить(ДанныеПечати);
			ОбластьСтрокаТаблицы.Параметры.Заполнить(ВыборкаОС);
			...
			Если ТабличныйДокумент.ПроверитьВывод(МассивОбластей) Тогда			
				...
				ДанныеПечати.СтоимостьПоЛисту 			= ДанныеПечати.СтоимостьПоЛисту + ВыборкаОС.Стоимость;
				ДанныеПечати.СуммаНДСПоЛисту 			= ДанныеПечати.СуммаНДСПоЛисту + ВыборкаОС.СуммаНДС;
				ДанныеПечати.СуммаНСППоЛисту 			= ДанныеПечати.СуммаНСППоЛисту + ВыборкаОС.СуммаНСП;
				ДанныеПечати.ВсегоПоЛисту 				= ДанныеПечати.ВсегоПоЛисту + ВыборкаОС.Всего;
				ДанныеПечати.СтоимостьПоСчетуФактуре 	= ДанныеПечати.СтоимостьПоСчетуФактуре + ВыборкаОС.Стоимость;
				ДанныеПечати.СуммаНДСПоСчетуФактуре 	= ДанныеПечати.СуммаНДСПоСчетуФактуре + ВыборкаОС.СуммаНДС;
				ДанныеПечати.СуммаНСППоСчетуФактуре 	= ДанныеПечати.СуммаНСППоСчетуФактуре + ВыборкаОС.СуммаНСП;
				ДанныеПечати.ВсегоПоСчетуФактуре 		= ДанныеПечати.ВсегоПоСчетуФактуре + ВыборкаОС.Всего;
...
Функция ПечатьСчетФактураБезНДС(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		ДанныеПечати.Вставить("ЗначениеСтавкиНСПУслуги", ?(ПлательщикНСП,
		...
		Для Каждого ИмяОбласти Из МассивОбластейМакета Цикл
			...
			ИначеЕсли ИмяОбласти = "СтрокаТаблицы" Тогда 
				...
				Индекс = 0;
				СтавкиНСПУслуги = Новый Структура("ЗначениеСтавкиНСП", ДанныеПечати.ЗначениеСтавкиНСПУслуги);
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "СчетФактура008") Тогда
		// Формируем табличный документ и добавляем его в коллекцию печатных форм.
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм,
		"СчетФактура008", НСтр("ru = 'Счет-фактура (008)'"), ПечатьСчетФактура008(МассивОбъектов, ОбъектыПечати));
	КонецЕсли;
...
Функция КоличествоЛистовСчетФатуры008(Макет, Шапка)
...
КонецФункции // КоличествоЛистовСчетФатуры()
