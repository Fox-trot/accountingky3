...
Процедура УстановитьВидимостьДоступностьЭлементов()	
	...
	Если НЕ ЗначениеЗаполнено(Объект.ВидСкидкиНаценки) Тогда 
		...
		Элементы.ТоварыСуммаСкидки1.Видимость = Ложь;
		Элементы.ТоварыСуммаСкидки2.Видимость = Ложь;
	...
	ИначеЕсли Объект.ВидСкидкиНаценки = Перечисления.ВидыСкидокНаценок.ПроцентПоСтроке Тогда 
		...
		Элементы.ТоварыСуммаСкидки1.Видимость = Объект.СуммаВключаетНалоги;
		Элементы.ТоварыСуммаСкидки1.ТолькоПросмотр = Истина;
		Элементы.ТоварыСуммаСкидки2.Видимость = НЕ Объект.СуммаВключаетНалоги;
		Элементы.ТоварыСуммаСкидки2.ТолькоПросмотр = Истина;
	...
	ИначеЕсли Объект.ВидСкидкиНаценки = Перечисления.ВидыСкидокНаценок.СуммаПоСтроке Тогда 
		...
		Элементы.ТоварыПроцентСкидкиНаценки.Видимость = Ложь;
		Элементы.ТоварыСуммаСкидки1.Видимость = Объект.СуммаВключаетНалоги;
		Элементы.ТоварыСуммаСкидки1.ТолькоПросмотр = Ложь;
		Элементы.ТоварыСуммаСкидки2.Видимость = НЕ Объект.СуммаВключаетНалоги;
		Элементы.ТоварыСуммаСкидки2.ТолькоПросмотр = Ложь;
...
Функция ПолучитьДанныеНоменклатураПриИзменении(СтруктураДанные)
	...
	// Цены документов
	Если Цена = 0 Тогда
		СтруктураДанные.СтавкаНДС = УчетНДСПовтИсп.ПолучитьСтавкуНДС(СтруктураДанные.Дата, СтруктураДанные.СтавкаНДС);
		СтруктураДанные.СтавкаНСП = УчетНДСПовтИсп.ПолучитьСтавкуНСП(СтруктураДанные.Дата, СтруктураДанные.Организация, СтруктураДанные.СтавкаНСП);
...
Процедура ЗагрузитьТоварыИзФайлаНаСервере(АдресЗагруженныхДанных)
	...
	Для каждого СтрокаТаблицы Из ЗагруженныеДанные Цикл 
		...
		Если Объект.ВидСкидкиНаценки <> ПредопределенноеЗначение("Перечисление.ВидыСкидокНаценок.СуммаПоСтроке") Тогда
			ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСкидкуСтрокиТабличнойЧасти(НоваяСтрокаТабличнойЧасти);
		КонецЕсли;	
		
		ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммыНалоговСтрокиТабличнойЧасти(НоваяСтрокаТабличнойЧасти, ПараметрыРасчета);
		ОбработкаТабличныхЧастейКлиентСервер.РассчитатьВсегоСтрокиТабличнойЧасти(НоваяСтрокаТабличнойЧасти);
		ОбработкаТабличныхЧастейКлиентСервер.РассчитатьДоходСтрокиТабличнойЧасти(НоваяСтрокаТабличнойЧасти, Объект.СуммаВключаетНалоги);
		ОбработкаТабличныхЧастейКлиентСервер.РассчитатьДоходСкидкиСтрокиТабличнойЧасти(НоваяСтрокаТабличнойЧасти);
