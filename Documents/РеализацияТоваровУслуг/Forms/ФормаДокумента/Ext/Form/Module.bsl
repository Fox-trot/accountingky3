...
Процедура УслугиКоличествоПриИзменении(Элемент)
	...
	ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммуСтрокиТабличнойЧасти(СтрокаТабличнойЧасти,,СуммаСкидкиПоСтроке);
	ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммыНалоговСтрокиТабличнойЧасти(
		СтрокаТабличнойЧасти, ДатаДокумента, Объект.Организация, Объект.СуммаВключаетНалоги, Объект.СтавкаНДС, Объект.СтавкаНСПУслуги, Объект.БезналичныйРасчет);
...
Процедура ОССуммаПриИзменении(Элемент)
	...
	ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммыНалоговСтрокиТабличнойЧасти(
		СтрокаТабличнойЧасти, ДатаДокумента, Объект.Организация, Объект.СуммаВключаетНалоги, Объект.СтавкаНДС, ПредопределенноеЗначение("Справочник.СтавкиНСП.Прочее"), Объект.БезналичныйРасчет);
...
Процедура УстановитьВидимостьДоступностьЭлементов()	
	...
	Если Объект.СуммаВключаетНалоги Тогда
		...
		Элементы.ТоварыДоход1.Видимость = Ложь;
		Элементы.УслугиДоход1.Видимость = Ложь;
		Элементы.ОСДоход1.Видимость 	= Ложь;
		
		Элементы.ТоварыВсего.Видимость 	= Ложь;
		Элементы.УслугиВсего.Видимость 	= Ложь;
		Элементы.ОСВсего.Видимость 		= Ложь;
		
		Элементы.ТоварыДоход2.Видимость = Истина;
		Элементы.УслугиДоход2.Видимость = Истина;
		Элементы.ОСДоход2.Видимость 	= Истина;
	Иначе
		Элементы.ТоварыСумма.Видимость 	= Ложь;
		Элементы.УслугиСумма.Видимость 	= Ложь;
		Элементы.ОССумма.Видимость 		= Ложь;
		
		Элементы.ТоварыДоход1.Видимость = Истина;
		Элементы.УслугиДоход1.Видимость = Истина;
		Элементы.ОСДоход1.Видимость 	= Истина;
		
		Элементы.ТоварыВсего.Видимость 	= Истина;
		Элементы.УслугиВсего.Видимость 	= Истина;
		Элементы.ОСВсего.Видимость 		= Истина;
		
		Элементы.ТоварыДоход2.Видимость = Ложь;
		Элементы.УслугиДоход2.Видимость = Ложь;
		Элементы.ОСДоход2.Видимость 	= Ложь;
	КонецЕсли;	
...
Процедура ОбработатьИзменениеДоговора(МодифицированДоговор = Ложь)
	...
	ОбработкаТабличныхЧастейКлиентСервер.ПересчитатьНалогиТабличнойЧасти(
		Объект, ДатаДокумента, Объект.Организация, "ОС", Объект.СуммаВключаетНалоги, Объект.СтавкаНДС, ПредопределенноеЗначение("Справочник.СтавкиНСП.Прочее"));
...
Функция СформироватьНадписьЦеныИВалюта(СтруктураНадписи)
	...
	// Ставка НДС
	Если ЗначениеЗаполнено(СтруктураНадписи.СтавкаНДС) Тогда
		Если ПустаяСтрока(ТекстНадписи) Тогда
...
Процедура ПолучитьТоварыИзХранилища(АдресЗапасовВХранилище, ИмяТабличнойЧасти)
	...
	Для Каждого СтрокаЗагрузки Из ТаблицаДляЗагрузки Цикл
		...
		ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммуСтрокиТабличнойЧасти(СтрокаТабличнойЧасти,,СуммаСкидкиПоСтроке);	
		ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммыНалоговСтрокиТабличнойЧасти(
			СтрокаТабличнойЧасти, ДатаДокумента, Объект.Организация, Объект.СуммаВключаетНалоги, Объект.СтавкаНДС, СтавкаНСП, Объект.БезналичныйРасчет);
	КонецЦикла;
