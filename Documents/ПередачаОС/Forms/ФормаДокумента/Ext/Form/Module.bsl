...
// Процедура - Устанавливает счета расчетов с контрагентами
//
&НаСервере
Процедура УстановитьСчетаРасчетовСКонтрагентами()
	СчетаРасчетовСКонтрагентом = БухгалтерскийУчетСервер.ПолучитьСчетаРасчетовСКонтрагентом(Объект.Организация, Объект.Контрагент, Объект.ДоговорКонтрагента);
	Объект.СчетРасчетовПокупателя = СчетаРасчетовСКонтрагентом.СчетРасчетовПокупателя;		
КонецПроцедуры
...
// Процедура - обработчик события ПриИзменении флага Авизо.
//
//&НаКлиенте
//Процедура АвизоПриИзменении(Элемент)
//	УстановитьВидимостьДоступностьЭлементов();
//КонецПроцедуры
