...
Функция ПолучитьДанныеДоговорПриИзменении(Период, ВалютаДокумента, ДоговорКонтрагента, ВидОперации)
	...
	Если ВидОперации = Перечисления.ВидыОперацийВозвратТоваровПоставщику.ВозвратПоставщику Тогда 
		СчетУчетаРасчетовСКонтрагентом = СчетаУчета.СчетРасчетовПоставщика;		
	Иначе 
...
Процедура ОбработатьИзменениеДоговора()
	...
	Если ДоговорПередИзменением <> Объект.ДоговорКонтрагента Тогда
		...
		Объект.СчетРасчетов = СтруктураДанные.СчетУчетаРасчетовСКонтрагентом;
...
//&НаСервере
//Процедура ЗаполнитьДокументПоОснованию(ДокументОснование)
...
//												Объект.СуммаВключаетНалоги,
//												УчетНДСПовтИсп.ПолучитьСтавкуНДС(СтрокаТабличнойЧасти.СтавкаНДС),
//												УчетНДСПовтИсп.ПолучитьСтавкуНСП(СтрокаТабличнойЧасти.СтавкаНСП));

//			ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммуСтрокиТабличнойЧасти(СтрокаТабличнойЧасти);
//			ОбработкаТабличныхЧастейКлиентСервер.РассчитатьСуммыНалоговСтрокиТабличнойЧасти(СтрокаТабличнойЧасти, ДатаДокумента, Объект.Организация, Объект.СуммаВключаетНалоги, Объект.СтавкаНДС, Объект.СтавкаНСП);
