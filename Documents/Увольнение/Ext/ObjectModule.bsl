...
Процедура ВыполнитьКонтроль(ДополнительныеСвойства, Отказ) 
	...
	Запрос = Новый Запрос(
		...
		|	ПлановыеНачисленияСрезПервых.Период КАК Период
		|ИЗ
		...
		|					ИЗ
		|						ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка)
		|				И НЕ Регистратор = &Ссылка) КАК ПлановыеНачисленияСрезПервых
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ПлановыеУдержанияСрезПервых.Период КАК Период
		|ИЗ
		...
		|					ИЗ
		|						ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка)
		|				И НЕ Регистратор = &Ссылка) КАК ПлановыеУдержанияСрезПервых");
	...
	// По сотруднику есть движения после увольнения по регистру "ПлановыеУдержания".
	Если НЕ МассивРезультатов[4].Пустой() Тогда
		ВыборкаИзРезультатаЗапроса = МассивРезультатов[3].Выбрать();
		ВыборкаИзРезультатаЗапроса.Следующий();
		ТекстСообщения = СтрШаблон(НСтр("ru = 'По сотруднику %1 есть кадровые движения %2 после даты увольнения. По регистру ""Плановые удержания"".'"), 
						ВыборкаИзРезультатаЗапроса.ФизЛицо, Формат(ВыборкаИзРезультатаЗапроса.Период, "ДФ=dd.MM.yy"));
		БухгалтерскийУчетСервер.СообщитьОбОшибке(
			ЭтотОбъект,
			ТекстСообщения,
