...
Процедура СформироватьТаблицаХозрасчетный(ДокументСсылка, СтруктураДополнительныеСвойства, Отказ)
	...
	// 2. Принятие к учету.
	// 3. По результатам инвентаризации.
	...
	Запрос.Текст =
		...
		|	И ВременнаяТаблицаШапка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПринятияКУчетуОсновныхСредств.ОсновныеСредства)
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
	...
	Запрос.УстановитьПараметр("СодержаниеПринятиеКУчету", НСтр("ru = 'Принятие к учету основного средства'")); 
