...
Процедура ВыполнитьПредварительныйКонтроль(Отказ)
	...
	Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
		Если (ВыборкаИзРезультатаЗапроса.ГруппаИмущества = Справочники.ГруппыИмущества.ГИ2 
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'У основного средства ""%1"" с группой иумщества %2 значение ""Начисления НИ с даты"" должно быть пустым или началом квартала текущего года.'"), 
		...
		ИначеЕсли ВыборкаИзРезультатаЗапроса.ГруппаИмущества = Справочники.ГруппыИмущества.ГИ4 
			...
			ТекстСообщения = СтрШаблон(НСтр("ru = 'У основного средства ""%1"" с группой имущества %2 значение ""Начисления НИ с даты"" должно быть пустым или началом следующего года.'"), 
													ВыборкаИзРезультатаЗапроса.ОсновноеСредство, ВыборкаИзРезультатаЗапроса.ГруппаИмущества);
			БухгалтерскийУчетСервер.СообщитьОбОшибке(ЭтотОбъект, ТекстСообщения,,,, Отказ);
		КонецЕсли;			
	КонецЦикла;			
