...
Процедура ВыполнитьПредварительныйКонтроль(Отказ)
	...
	Если Ссылка.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПринятияКУчетуОсновныхСредств.Оборудование") Тогда
		Если Ссылка.ВыбранноеКоличествоНоменклатуры <> ОС.Количество() Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Количество строк записи должно совпадать с выбранным количеством номенклатуры!'"));
		...
		ИначеЕсли ОС.Итог("ПервоначальнаяСтоимость") <> Ссылка.СуммаВыбраннойНоменклатуры Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Сумма первоначальной стоимости по всем строкам не совпадает с суммой выбранной номенклатуры!'"));
