...
Процедура ЗаполнитьПоПоступлениюТоваровУслуг(ДанныеЗаполнения) Экспорт
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	ТаблицаДокумента.Ссылка.Дата <= &Дата
		|	И ТаблицаДокумента.ОсновноеСредство В
		|			(ВЫБРАТЬ
		...
		|	ТаблицаПоступления.ПервоначальнаяСтоимость + ЕСТЬNULL(ТаблицаДопрасходы.СуммаРасходов, 0) КАК ПервоначальнаяСтоимость,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаДопрасходы КАК ТаблицаДопрасходы
		|		ПО ТаблицаПоступления.ОсновноеСредство = ТаблицаДопрасходы.ОсновноеСредство";
