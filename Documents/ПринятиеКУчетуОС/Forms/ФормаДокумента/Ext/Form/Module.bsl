...
Процедура ДополнитьСтрокиНаСервере(МассивОС = Неопределено)
	...
	Запрос.Текст = 
		...
		|	СУММА(ТаблицаДокумента.СуммаРасходов) КАК СуммаРасходов
		|ПОМЕСТИТЬ ВременнаяТаблицаДопРасходы
		|ИЗ
		|	Документ.ДополнительныеРасходы.ОС КАК ТаблицаДокумента
		|ГДЕ
		|	ТаблицаДокумента.Ссылка.Дата <= &Период
		|	И ТаблицаДокумента.ОсновноеСредство В(&МассивОС)
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ПараметрыУчетаОССрезПоследних.ПервоначальнаяСтоимость + ЕСТЬNULL(ВременнаяТаблицаДопРасходы.СуммаРасходов, 0) КАК ПервоначальнаяСтоимость,
		|	ПараметрыУчетаОССрезПоследних.ПервоначальнаяСтоимость + ЕСТЬNULL(ВременнаяТаблицаДопРасходы.СуммаРасходов, 0) КАК СтоимостьДляРасчетаАмортизации,
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаДопРасходы КАК ВременнаяТаблицаДопРасходы
		|		ПО ПараметрыУчетаОССрезПоследних.ОсновноеСредство = ВременнаяТаблицаДопРасходы.ОсновноеСредство";
