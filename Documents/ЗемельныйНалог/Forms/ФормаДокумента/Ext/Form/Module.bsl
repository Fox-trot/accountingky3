...
Процедура Заполнить(Команда)
	...
	Если Объект.Налоги.Количество() > 0 Тогда
		Вопрос = НСтр("ru = 'Перед загрузкой табличная часть будет очищена! Продолжить?'");
		Ответ = Неопределено;
...
Процедура ЗаполнитьНаСервере()
	...
	Для Каждого СТЧ Из Объект.Налоги Цикл 
		Если СТЧ.СрокЭксплуатации >= 30 Тогда
			СТЧ.СрокЭксплуатацииСтрока = НСтр("ru = 'Более 30 лет'");
		Иначе 
			СТЧ.СрокЭксплуатацииСтрока = НСтр("ru = 'До 30 лет'");
		КонецЕсли;
		
		ОтборКр = Новый Структура("Классификация", СТЧ.КлассКр);
		СТЧ.Кр  = РегистрыСведений.КоэффициентыНалогаНаИмущество.ПолучитьПоследнее(Объект.Дата, ОтборКр).Коэффициент;
		
		ОтборКз = Новый Структура("Классификация", СТЧ.КлассКз);
		СТЧ.Кз = РегистрыСведений.КоэффициентыНалогаНаИмущество.ПолучитьПоследнее(Объект.Дата, ОтборКз).Коэффициент;
		
		ОтборКо = Новый Структура("Классификация", СТЧ.КлассКо);
		СТЧ.Ко = РегистрыСведений.КоэффициентыНалогаНаИмущество.ПолучитьПоследнее(Объект.Дата, ОтборКо).Коэффициент;
