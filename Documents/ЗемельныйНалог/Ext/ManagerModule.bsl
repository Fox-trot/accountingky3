...
Процедура ПечатьЛистаОтчета_Группа3(ТабДок, Макет, ТЗ, ОтчетныйПериод, Организация, Дата)
	...
	СведенияОбОрганизации = ОбщегоНазначенияБПВызовСервера.СведенияОЮрФизЛице(Организация, Дата);
	//ИНН Организации
	...
	Для Каждого СТЗ Из ТЗ Цикл
		...
		Для НомерКолонки = 0 ПО 12 Цикл 
			Выполнить("Обл.Параметры.ТС" + НомерКолонки + " = " + Строка(300 + НомерКолонки + НомерСтроки));	
		КонецЦикла;	
...
Функция ПечатнаяФорма(МассивОбъектов, ОбъектыПечати, ИмяМакета)
	...
	ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ЗемельныйНалог";
	...
	Для Каждого ТекущийДокумент Из МассивОбъектов Цикл
		...
		ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_НалогНаИмущество";
