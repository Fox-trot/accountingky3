...
//			СформироватьСписокПродукции(СтрокаТабличнойЧасти.Заказ);	
//		Иначе
//			Элементы.ГотоваяПродукцияНоменклатура.СписокВыбора.Очистить();
//		КонецЕсли;
//		
//		Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Заказ) И ЗначениеЗаполнено(СтрокаТабличнойЧасти.Номенклатура) Тогда
//			СформироватьСписокВидовРабот(СтрокаТабличнойЧасти.Заказ, СтрокаТабличнойЧасти.Номенклатура, СтрокаТабличнойЧасти.Количество);
//		Иначе
//			Элементы.ГотоваяПродукцияВидРаботы.СписокВыбора.Очистить();
//		КонецЕсли;
//	КонецЕсли;
//КонецПроцедуры
...
//	
//	Если ЗначениеЗаполнено(СтрокаТабличнойЧасти.Заказ) Тогда
//		ТекущийЗаказПоСтрокеГП = СтрокаТабличнойЧасти.Заказ;
//		СформироватьСписокПродукции(СтрокаТабличнойЧасти.Заказ);
//	Иначе
//		ОчиститьДанныеСтрокиГП(СтрокаТабличнойЧасти.ВидРаботы, СтрокаТабличнойЧасти.Количество, 
...
//&НаСервере
//Процедура ОчиститьДанныеСтрокиГП(ВидРаботы, Количество, Номенклатура = Неопределено)
...
//// Процедура формирования списка продукции заказа.
////
//&НаКлиенте
//Процедура СформироватьСписокПродукции(Заказ)
//		
//	Элементы.ГотоваяПродукцияНоменклатура.СписокВыбора.Очистить();
//	МассивНоменклатуры = ПолучитьМассивДляСписка(Заказ);
//	Элементы.ГотоваяПродукцияНоменклатура.СписокВыбора.ЗагрузитьЗначения(МассивНоменклатуры);
//	
//КонецПроцедуры

//// Процедура формирования списка видов работ по продукции и заказу.
////
//&НаКлиенте
//Процедура СформироватьСписокВидовРабот(Заказ, Номенклатура, Количество)
//		
//	Элементы.ГотоваяПродукцияВидРаботы.СписокВыбора.Очистить();
//	СтруктураРаботИКоличества = ПолучитьМассивДляСписка(Заказ, Номенклатура);
//	
//	Элементы.ГотоваяПродукцияВидРаботы.СписокВыбора.ЗагрузитьЗначения(СтруктураРаботИКоличества.ВидыРабот);
//	Количество = СтруктураРаботИКоличества.Количество;
//КонецПроцедуры

//&НаСервере
//Функция ПолучитьМассивДляСписка(Заказ, Номенклатура = Неопределено)
...
//			|				И Заказ = &Заказ
//			|				И Состояние = ЗНАЧЕНИЕ(Перечисление.СостояниеРаботПоПроизводству.Заказ)) КАК СостояниеЗаказовСрезПоследних";
//		ТЗНоменклатуры = Запрос.Выполнить().Выгрузить();
//		
//		СтруктураВозврата = Новый Структура();
//		СтруктураВозврата.Вставить("Склад", ТЗНоменклатуры[0].Склад);
//		СтруктураВозврата.Вставить("Номенклатура", ТЗНоменклатуры.ВыгрузитьКолонку("Номенклатура"));
//				
...
//		Запрос.Текст =	
//			"ВЫБРАТЬ
//			|	СостояниеЗаказовСрезПоследних.ВидРабот КАК ВидРабот,
//			|	СостояниеЗаказовСрезПоследних.Количество КАК Количество
//			|ИЗ
//			|	РегистрСведений.СостояниеЗаказов.СрезПоследних(
//			|			,
//			|			Организация = &Организация
...
//		
//		СтруктураВозврата = Новый Структура();
//		СтруктураВозврата.Вставить("Количество", ТЗРабот[0].Количество);
//		СтруктураВозврата.Вставить("ВидыРабот", ТЗРабот.ВыгрузитьКолонку("ВидРабот"));
//				
//		Возврат СтруктураВозврата;
//	КонецЕсли;	
//КонецФункции
