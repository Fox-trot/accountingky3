...
Процедура СформироватьВыполненныеРаботы(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаГотоваяПродукция.ВидРаботы КАК ВидРаботы,
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, СтруктураДополнительныеСвойства, Отказ) Экспорт
	...
	Запрос.Текст =
		...
		|	ОтчетПоПроизводствуГотоваяПродукция.ВидРаботы,
...
Процедура СформироватьТаблицуГотовыхЗаказов(ДополнительныеСвойства) Экспорт
	...
	Запрос.Текст = 
		...
		|	ТаблицаГотоваяПродукция.ВидРаботы,
		...
		|   	СпецификацияНоменклатурыВидыРабот.ВидРаботы,
		|   	ВременнаяТаблицаЗаказы.Спецификация,
		|   	ВременнаяТаблицаЗаказы.Полуфабрикаты
		|ПОМЕСТИТЬ ВременнаяТаблицаВидыРабот
		|ИЗ
		|	ВременнаяТаблицаЗаказы КАК ВременнаяТаблицаЗаказы
		|	ПОЛНОЕ СОЕДИНЕНИЕ Справочник.СпецификацияНоменклатуры.ВидыРабот КАК СпецификацияНоменклатурыВидыРабот
		|	ПО ВременнаяТаблицаЗаказы.Спецификация = СпецификацияНоменклатурыВидыРабот.Ссылка
		|;
		|
		|///////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаВидыРабот.Заказ,
		|   ВременнаяТаблицаВидыРабот.Номенклатура,
		|   ВременнаяТаблицаВидыРабот.ВидРаботы,
		|   ВременнаяТаблицаВидыРабот.Спецификация,
		|   ВременнаяТаблицаВидыРабот.Полуфабрикаты,
		|	ВыполненныеРаботыСрезПоследних.Исполнитель
		|ПОМЕСТИТЬ ВременнаяТаблицаСостоянийРабот
		|ИЗ
		|	ВременнаяТаблицаВидыРабот КАК ВременнаяТаблицаВидыРабот
		...
		|	ПО ВременнаяТаблицаВидыРабот.Заказ = ВыполненныеРаботыСрезПоследних.Заказ
		|	И ВременнаяТаблицаВидыРабот.Номенклатура = ВыполненныеРаботыСрезПоследних.Номенклатура
		|	И ВременнаяТаблицаВидыРабот.ВидРаботы = ВыполненныеРаботыСрезПоследних.ВидРаботы
		|	И ВременнаяТаблицаВидыРабот.Спецификация = ВыполненныеРаботыСрезПоследних.Спецификация
		|ГДЕ
		|	НЕ ВременнаяТаблицаВидыРабот.Заказ ЕСТЬ NULL
		|;
		|
		|///////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|   ВременнаяТаблицаСостоянийРабот.ВидРаботы,
	...
	Если МассивГотовыхЗаказовГП.Количество() > 0 ИЛИ МассивГотовыхЗаказовПФ.Количество() > 0 Тогда
		...
		Если МассивГотовыхЗаказовГП.Количество() > 0 Тогда
			Для Каждого СтрокаМассива Из МассивГотовыхЗаказовГП Цикл
				...
				СтрокаТаблицы.Состояние = Перечисления.СостояниеРаботПоПроизводству.Изготовлен;
			КонецЦикла;	
		КонецЕсли;
		...
		Если МассивГотовыхЗаказовПФ.Количество() > 0 Тогда
			Для Каждого СтрокаМассива Из МассивГотовыхЗаказовПФ Цикл
				...
				СтрокаТаблицы.Период = Дата;
				СтрокаТаблицы.Организация = Организация;
				СтрокаТаблицы.Заказ = СтрокаМассива.Заказ;;
				СтрокаТаблицы.Состояние = Перечисления.СостояниеРаботПоПроизводству.Изготовлен;
			КонецЦикла;	
		КонецЕсли;
