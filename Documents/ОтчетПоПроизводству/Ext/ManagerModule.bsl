...
Процедура СформироватьСостояниеЗаказов(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаГотоваяПродукция.ВидРаботы КАК ВидРабот,
		|	ВременнаяТаблицаГотоваяПродукция.Склад КАК Склад,
		|	ВременнаяТаблицаГотоваяПродукция.Полуфабрикаты КАК Полуфабрикаты,
		|	ЗНАЧЕНИЕ(Перечисление.СостояниеРаботПоПроизводству.Изготовлен) КАК Состояние
		|ИЗ
...
Процедура СформироватьТаблицуГотовыхЗаказов(ДополнительныеСвойства) Экспорт
	...
	Запрос.Текст = 
		...
		|	СостояниеЗаказовСрезПоследних.Полуфабрикаты КАК Полуфабрикаты,
		...
		|					ИЗ
		|						ВременнаяТаблицаЗаказы КАК ВременнаяТаблицаЗаказы)) КАК СостояниеЗаказовСрезПоследних
		|ИТОГИ
	...
	Пока ВыборкаПоГруппировкам.Следующий() Цикл
		...
		СтруктураОтбора.Вставить("Заказ", ВыборкаПоГруппировкам.Заказ);
		...
		Пока ДетальныеЗаписи.Следующий() Цикл
			Если ДетальныеЗаписи.Состояние = Перечисления.СостояниеРаботПоПроизводству.Изготовлен Тогда
				ЗаказГотов = Истина;
	...
	Если МассивГотовыхЗаказовГП.Количество() > 0 ИЛИ МассивГотовыхЗаказовПФ.Количество() > 0 Тогда
		...
		Запрос.Текст =
			...
			|			&Период,
			|			Организация = &Организация
			|			И Заказ В (&МассивГотовыхЗаказовГП)) КАК СостояниеЗаказовСрезПоследних
			|
			|ОБЪЕДИНИТЬ ВСЕ
			|
			|ВЫБРАТЬ РАЗЛИЧНЫЕ
			|	СостояниеЗаказовСрезПоследних.Количество,
			|	СостояниеЗаказовСрезПоследних.Склад,
			|	СостояниеЗаказовСрезПоследних.Номенклатура,
			|	СостояниеЗаказовСрезПоследних.Заказ,
			|	СостояниеЗаказовСрезПоследних.Полуфабрикаты
			|ИЗ
			|	РегистрСведений.СостояниеЗаказов.СрезПоследних(
			|			&Период,
			|			Организация = &Организация
			|			И Заказ В (&МассивГотовыхЗаказовПФ)) КАК СостояниеЗаказовСрезПоследних
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
...
Функция ПолучитьМассивСубконтоНоменклатура(МассивГотовыхЗаказов, ГраницаКонтроля, Организация)
	...
	Запрос.Текст =
		...
		|			&Период,
		|			Организация = &Организация
		|			И Заказ В (&МассивГотовыхЗаказов)) КАК СостояниеЗаказовСрезПоследних";
	Запрос.УстановитьПараметр("Период", ГраницаКонтроля);
	Запрос.УстановитьПараметр("Организация", Организация);
