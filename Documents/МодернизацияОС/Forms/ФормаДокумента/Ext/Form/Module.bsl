...
Процедура СтоимостьМодернизацииПриИзменении(Элемент)
	...
	//Объект.БазаАмортизации = ПервоначальнаяСтоимость + Объект.СтоимостьМодернизации - Объект.НакопленнаяАмортизация;
КонецПроцедуры
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	Если Объект.Событие = Перечисления.ВидыСобытийОС.Модернизация Тогда 
		...
		Элементы.ГруппаОстаточнаяСтоимость.Видимость 		= ДанныеУчетнойПолитики.НачислениеАмортизацииОСПоМесяцам;
		Элементы.ГруппаБазаАмортизации.Видимость 			= ДанныеУчетнойПолитики.НачислениеАмортизацииОСПоМесяцам;
		Элементы.РассчитатьСтоимостьМодернизации.Видимость 	= Истина;
	Иначе 
		Элементы.СчетУчетаМодернизации.Видимость 			= Ложь;
		Элементы.СтоимостьМодернизации.Видимость 			= Ложь;
		Элементы.ГруппаСтоимость.Видимость 					= Ложь;
		Элементы.ГруппаАмортизацияЗаМесяц.Видимость 		= Ложь;
		Элементы.ГруппаОстаточнаяСтоимость.Видимость 		= Ложь;
		Элементы.ГруппаБазаАмортизации.Видимость 			= Ложь;
		Элементы.РассчитатьСтоимостьМодернизации.Видимость 	= Ложь;
	КонецЕсли;	
	
	Элементы.ГруппаНакопленнаяАмортизация.Видимость = ДанныеУчетнойПолитики.НачислениеАмортизацииОСПоМесяцам;
...
Процедура РасчитатьСтоимостьМодернизации()	
	...
	Объект.БазаАмортизацииПослеМодернизации 		= Объект.СтоимостьПослеМодернизации 
					 									- ЛиквидационнаяСтоимость - Объект.НакопленнаяАмортизацияПослеМодернизации;
КонецПроцедуры
...
Процедура РасчетАмортизацииИОстаточнойСтоимости()
	...
	Запрос.Текст = 
		...
		|					ИЛИ Регистратор ССЫЛКА Документ.ВводНачальныхОстатков
		|					ИЛИ Регистратор ССЫЛКА Документ.ЗакрытиеМесяца)
		|				И Регистратор <> &Ссылка) КАК ПараметрыУчетаОССрезПоследних";
	Запрос.УстановитьПараметр("Период", ДатаДокумента);
	Запрос.УстановитьПараметр("Организация", Объект.Организация);
	Запрос.УстановитьПараметр("ОсновноеСредство", Объект.ОсновноеСредство);
	...
	Объект.АмортизацияЗаМесяц = УправлениеВнеоборотнымиАктивамиВызовСервера.РасчетАмортизацииОсновногоСредства(ДатаПринятияОС, 
									ДатаДокумента, Объект.Организация, Объект.ОсновноеСредство, 
									ДанныеУчетнойПолитики.НачислениеАмортизацииОСПоМесяцам);
																		
	//Объект.АмортизацияЗаМесяцПослеМодернизации = УправлениеВнеоборотнымиАктивамиВызовСервера.РасчетАмортизацииОсновногоСредства(
	//							ДатаПринятияОС, ДатаДокумента, Объект.Организация, Объект.ОсновноеСредство, 
