...
Процедура КонтрольДублейИПараметровУчета(Отказ)
	...
	|ИЗ
	|	РегистрСведений.СоставОС.СрезПоследних(&МоментВремени, НЕ ОсновноеСредство = &ОсновноеСредство) КАК СоставОССрезПоследних
	|ГДЕ
...
Процедура КонтрольПриВыводеИзКомплекта(Отказ)
	...
	|ПОМЕСТИТЬ ВременнаяТаблицаОСВКомплектах
	|ИЗ
	|	РегистрСведений.СоставОС.СрезПоследних(&МоментВремени, ОсновноеСредство = &ОсновноеСредство) КАК СоставОССрезПоследних
	|ГДЕ
	|	СоставОССрезПоследних.СостояниеВСоставеОС В (ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКомплектацияОС.Комплектация), ЗНАЧЕНИЕ(Перечисление.ВидыОперацийКомплектацияОС.Добавление))
	|;
	|
