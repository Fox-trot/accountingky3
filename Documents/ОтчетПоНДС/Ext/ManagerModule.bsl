...
Процедура ВыводСтрокиРеестра(ТабличныйДокумент, СтрокаТаблицы, ОбластьСтрокаРеестра, МассивПроверки, ИмяМакета)
	...
	ИначеЕсли ИмяМакета = "ОтчетРеестрГТД" Тогда
...
Процедура СформироватьПустойБланк(СтруктураПараметров, ТабличныйДокумент, ИмяМакета)
	...
	ИначеЕсли ИмяМакета = "ОтчетРеестрГТД" Тогда
		Макет = ПолучитьМакет("ПФ_MXL_РеестрГТД");
	КонецЕсли;
...
//  МассивПроверки				 - 	 - 
//
Процедура ПодвалСИтогамиПоследнейСтраницы(ИтогоПоРеестру, Макет, МассивПроверки, СтруктураОбластейМакета, ТабличныйДокумент, 
											КоличествоТекушегоЛиста, ИмяМакета, ИтогоПоРееструЕАЭС = Неопределено, ОдинЛист = Ложь)
	
	Если ИмяМакета = "ОтчетРеестрПриобретенных" Или ИмяМакета = "ОтчетРеестрПриобретенныхНаОсновеНалоговогоКонтракта" Тогда
		СтруктураОбластейМакета.ОбластьИтогиПоРеестру.Параметры.СтоимостьБезНДС				= ИтогоПоРеестру.ИтогоПоРееструБНДС;
		СтруктураОбластейМакета.ОбластьИтогиПоРеестру.Параметры.СуммаНДС 					= ИтогоПоРеестру.ИтогоПоРееструНДС;
		СтруктураОбластейМакета.ОбластьИтогиПоРеестру.Параметры.СуммаПодлежащаяКЗачетуНДС 	= ИтогоПоРеестру.ИтогоПоРееструНДСЗачет;
...
//  НомерЛиста					 - 	 - 
//
Процедура ПодвалПромежуточнойСтраницыСИтогами(ИтогоПоЛисту, СтруктураОбластейМакета, ТабличныйДокумент, МассивПроверки, ИмяМакета, 
													НомерЛиста, КоличествоОпераций, КоличествоВывода, Макет = Неопределено)
	...
	Пока Шапка.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			ИначеЕсли КоличествоОпераций = КоличествоВывода и КоличесвтоТекушегоЛиста <= 13 Тогда 
				...
				ПодвалСИтогамиПоследнейСтраницы(ИтогоПоРеестру, Макет, МассивПроверки, СтруктураОбластейМакета, ТабличныйДокумент, КоличесвтоТекушегоЛиста, ИмяМакета,,Истина);
				Прервать;
			ИначеЕсли КоличествоОпераций = КоличествоВывода Тогда 
				ПодвалСИтогамиПоследнейСтраницы(ИтогоПоРеестру, Макет, МассивПроверки, СтруктураОбластейМакета, ТабличныйДокумент, КоличесвтоТекушегоЛиста, ИмяМакета);
				Прервать;
			КонецЕсли;
			...
			Иначе 
				...
				ПодвалПромежуточнойСтраницыСИтогами(ИтогоПоЛисту, СтруктураОбластейМакета, ТабличныйДокумент, МассивПроверки, 
														ИмяМакета, НомерЛиста, КоличествоОпераций, КоличествоВывода, Макет);
				КоличесвтоТекушегоЛиста = 0;
			КонецЕсли;
...
//  Табличный документ - печатная форма
//
Функция ПечатьОтчетаРеестрВвезенных(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	Запрос.Текст = 
		...
		|		ПДСуммаБезНДС,
		//|		ВЫБОР
		//|			КОГДА НЕ ОтчетПоНДС.ОтчетРеестрГТД.Контрагент.СтранаРезидентства = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
		//|				ТОГДА ОтчетПоНДС.ОтчетРеестрГТД.Контрагент.СтранаРезидентства.ЕАЭС
		//|			ИНАЧЕ ЛОЖЬ
		//|		КОНЕЦ 
		|		ЛОЖЬ КАК СтранаРезидентстваЕАЭС,
		|		ВЫБОР
		|			КОГДА НЕ ОтчетПоНДС.ОтчетРеестрГТД.Контрагент.СтранаРезидентства = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
		|				ТОГДА ОтчетПоНДС.ОтчетРеестрГТД.Контрагент.СтранаРезидентства.Код
		|			ИНАЧЕ """"
		|		КОНЕЦ КАК КодСтраны
		|	)
		|ИЗ
	...
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ОтчетПоНДС.ПФ_MXL_РеестрГТД");
	ОбластьШапка = Макет.ПолучитьОбласть("Шапка");
	ИмяМакета = "ОтчетРеестрГТД";
	...
	Пока Шапка.Следующий() Цикл
		...
		ВыборкаДанных = Шапка.ОтчетРеестрГТД.Выбрать();
		...
		Пока ВыборкаДанных.Следующий() Цикл
			...
			ИначеЕсли КоличествоОпераций = КоличествоВывода И КоличествоВывода <= 8 Тогда 
				//4. вывод итогов по реестру
				ПодвалСИтогамиПоследнейСтраницы(ИтогоПоРеестру, Макет, МассивПроверки, СтруктураОбластейМакета, ТабличныйДокумент, 
				КоличествоТекушегоЛиста, ИмяМакета, ИтогоПоРееструЕАЭС, Истина);
				Прервать;
				
			ИначеЕсли КоличествоОпераций = КоличествоВывода Тогда 
				//4. вывод итогов по реестру
				ПодвалСИтогамиПоследнейСтраницы(ИтогоПоРеестру, Макет, МассивПроверки, СтруктураОбластейМакета, ТабличныйДокумент, 
				КоличествоТекушегоЛиста, ИмяМакета, ИтогоПоРееструЕАЭС);
				Прервать;
			КонецЕсли;
			...
			Иначе 
				//5. вывод подвала
				ПодвалПромежуточнойСтраницыСИтогами(ИтогоПоЛисту, СтруктураОбластейМакета, ТабличныйДокумент, МассивПроверки, 
														ИмяМакета, НомерЛиста, КоличествоОпераций, КоличествоВывода);
			КонецЕсли;
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "РеестрПриобретенных") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, 
		"РеестрПриобретенных", 
		"Реестр приобретенных материальных ресурсов", 
		ПечатьОтчетаРеестрПриобретенных(МассивОбъектов, ОбъектыПечати),
		,
		"Документ.ОтчетПоНДС.ПФ_MXL_РеестрПриобретенных");
	КонецЕсли;
	
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "РеестрГТД") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, 
		"РеестрГТД", 
		"Реестр ввезенных материальных ресурсов", 
		ПечатьОтчетаРеестрВвезенных(МассивОбъектов, ОбъектыПечати),
		,
		"Документ.ОтчетПоНДС.ПФ_MXL_РеестрГТД");
	КонецЕсли;
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "РеестрПоставокНаОсновеНалоговогоКонтракта") Тогда
		...
		"Реестр поставок на основе налогового контракта", 
	...
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "РеестрПриобретенныхНаОсновеНалоговогоКонтракта") Тогда
		...
		"Реестр приобретенных материальных ресурсов на основе налогового контракта", 
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "РеестрГТД";
	КомандаПечати.Представление = НСтр("ru = 'Реестр ГТД'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
