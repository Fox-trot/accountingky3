...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПриобретенных(НалоговыйКонтракт, НеУчитыватьЗакупкиБезНДС) Экспорт
	...
	Запрос.Текст =  
		...
		|			ТОГДА 0
		|		ИНАЧЕ РеестрПриобретенныхМатериальныхРесурсов.СуммаНДСОборот
		|	КОНЕЦ КАК СуммаНДС,
		|	ВЫБОР
		...
		|ИЗ
		|	РегистрНакопления.РеестрПриобретенныхМатериальныхРесурсов.Обороты(&НачалоПериода, &КонецПериода, , Организация = &Организация) КАК РеестрПриобретенныхМатериальныхРесурсов
		|ГДЕ
		|	ВЫБОР
		|			КОГДА &НеУчитыватьЗакупкиБезНДС
