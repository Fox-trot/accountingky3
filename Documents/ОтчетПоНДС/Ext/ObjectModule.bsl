...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПоставок() Экспорт	
	...
	|ПОМЕСТИТЬ ВременнаяТаблицаСФВыписанныеОбщие
	|ИЗ
	|	РегистрСведений.СчетаФактурыВыписанные КАК СчетаФактурыВыписанные
	|ГДЕ
	|	СчетаФактурыВыписанные.ДатаПоставки МЕЖДУ &НачалоПериода И &КонецПериода
	|	И СчетаФактурыВыписанные.Общая
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	СчетаФактурыВыписанные.ДоговорКонтрагента КАК Договор,
	|	СчетаФактурыВыписанные.СтавкаНДС КАК СтавкаНДС,
	|	ЕСТЬNULL(ВременнаяТаблицаСФВыписанныеОбщие.СерияБланкаСФ, СчетаФактурыВыписанные.СерияБланкаСФ) КАК СерияСФ,
	|	ЕСТЬNULL(ВременнаяТаблицаСФВыписанныеОбщие.НомерБланкаСФ, СчетаФактурыВыписанные.НомерБланкаСФ) КАК НомерСФ,
	|	СчетаФактурыВыписанные.Контрагент.ИНН КАК ИНН,
	...
	|	НАЧАЛОПЕРИОДА(СчетаФактурыВыписанные.ДатаПоставки, ДЕНЬ) КАК ДатаПоставки,
	|	СУММА(СчетаФактурыВыписанные.СуммаБезНДС) КАК Отгрузка,
	|	СУММА(СчетаФактурыВыписанные.СуммаНДС) КАК СуммаНДС,
	|	СУММА(СчетаФактурыВыписанные.СуммаНеоблагаемая) КАК СуммаНСП,
	|	НАЧАЛОПЕРИОДА(СчетаФактурыВыписанные.ДатаСФ, ДЕНЬ) КАК Дата,
	|	СУММА(СчетаФактурыВыписанные.СуммаБезНДС + СчетаФактурыВыписанные.СуммаНДС + ВЫБОР
	|			КОГДА &НСПВСумме
	|				ТОГДА СчетаФактурыВыписанные.СуммаНеоблагаемая
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК ОбщаяСтоимостьСНДС
	|ПОМЕСТИТЬ ВременнаяТаблицаОтгрузкаТекущегоМесяца
	|ИЗ
	|	РегистрСведений.СчетаФактурыВыписанные КАК СчетаФактурыВыписанные
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаСФВыписанныеОбщие КАК ВременнаяТаблицаСФВыписанныеОбщие
	|		ПО СчетаФактурыВыписанные.Документ = ВременнаяТаблицаСФВыписанныеОбщие.Документ
	|ГДЕ
	|	СчетаФактурыВыписанные.ДатаПоставки МЕЖДУ &НачалоПериода И &КонецПериода
	|	И НЕ СчетаФактурыВыписанные.Общая
	|
	|СГРУППИРОВАТЬ ПО
	...
	|	НАЧАЛОПЕРИОДА(СчетаФактурыВыписанные.ДатаПоставки, ДЕНЬ),
	|	НАЧАЛОПЕРИОДА(СчетаФактурыВыписанные.ДатаСФ, ДЕНЬ),
	...
	|		ИНАЧЕ СчетаФактурыВыписанные.Контрагент.СтранаРезидентства.Код
	|	КОНЕЦ,
	|	ЕСТЬNULL(ВременнаяТаблицаСФВыписанныеОбщие.СерияБланкаСФ, СчетаФактурыВыписанные.СерияБланкаСФ),
	|	ЕСТЬNULL(ВременнаяТаблицаСФВыписанныеОбщие.НомерБланкаСФ, СчетаФактурыВыписанные.НомерБланкаСФ)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПриобретенных() Экспорт
	...
	Запрос.Текст =  
	"ВЫБРАТЬ
	|	СведенияОПоступлении.ДокументСсылка КАК ДокументПоступления,
	|	СведенияОПоступлении.СерияБланкаСФ КАК СерияБланкаСФ,
	|	СведенияОПоступлении.НомерБланкаСФ КАК НомерБланкаСФ
	|ПОМЕСТИТЬ ВременнаяТаблицаСФПолученныеОбщие
	|ИЗ
	|	РегистрСведений.СведенияОПоступлении КАК СведенияОПоступлении
	|ГДЕ
	|	СведенияОПоступлении.ДатаПоставки МЕЖДУ &НачалоПериода И &КонецПериода
	|	И СведенияОПоступлении.Организация = &Организация
	|	И СведенияОПоступлении.Общая
	|	И СведенияОПоступлении.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.КР)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	СведенияОПоступлении.Контрагент.НаименованиеПолное КАК КонтрагентНаименование,
	|	СведенияОПоступлении.Контрагент.ИНН КАК ИННКонтрагента,
	|	СведенияОПоступлении.ДатаПоставки КАК ДатаПоставки,
	|	СведенияОПоступлении.Контрагент.ГНС.Код КАК КодГНС,
	|	СведенияОПоступлении.СуммаНДС КАК СуммаНДС,
	|	СведенияОПоступлении.СуммаБезНДС КАК СтоимостьБезНДС,
	|	ВЫБОР
	...
	|	ЕСТЬNULL(ВременнаяТаблицаСФПолученныеОбщие.СерияБланкаСФ, СведенияОПоступлении.СерияБланкаСФ) КАК СерияСФ,
	|	ЕСТЬNULL(ВременнаяТаблицаСФПолученныеОбщие.НомерБланкаСФ, ВЫБОР
	|			КОГДА СведенияОПоступлении.СерияБланкаСФ = ""ДПБУ""
	|				ТОГДА ВЫБОР
	...
	|						ТОГДА СведенияОПоступлении.НомерБланкаСФ
	...
	|ИЗ
	|	РегистрСведений.СведенияОПоступлении КАК СведенияОПоступлении
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаСФПолученныеОбщие КАК ВременнаяТаблицаСФПолученныеОбщие
	|		ПО СведенияОПоступлении.ДокументСсылка = ВременнаяТаблицаСФПолученныеОбщие.ДокументПоступления
	|ГДЕ
	|	СведенияОПоступлении.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.КР)
	|	И СведенияОПоступлении.ДатаПоставки МЕЖДУ &НачалоПериода И &КонецПериода
	|	И СведенияОПоступлении.Организация = &Организация
	|	И НЕ СведенияОПоступлении.Общая
	|
	|УПОРЯДОЧИТЬ ПО
	|	ДатаПоставки";
