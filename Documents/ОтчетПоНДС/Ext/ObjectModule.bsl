...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПоставок() Экспорт	
	...
	|	ВЫБОР
	|		КОГДА СчетаФактурыВыписанные.Контрагент.СтранаРезидентства = ЗНАЧЕНИЕ(Справочник.СтраныМира.Киргизия)
	...
	|	1 КАК КоличествоСтрок,
	|	ВЫБОР
	|		КОГДА РеализацияОбороты.Контрагент.СтранаРезидентства = ЗНАЧЕНИЕ(Справочник.СтраныМира.Киргизия)
	|			ТОГДА РеализацияОбороты.Контрагент.ГНС.Код
	|		ИНАЧЕ РеализацияОбороты.Контрагент.СтранаРезидентства.Код
	|	КОНЕЦ КАК КодГНС,
	|	РеализацияИтоги.ВидПоставкиНДС.Код КАК КодПоставки,
	...
	|		ИНАЧЕ РеализацияОбороты.Контрагент.СтранаРезидентства.Код
	|	КОНЕЦ,
	|	РеализацияИтоги.ВидПоставкиНДС.Код,
	...
	|	ВременнаяТаблицаАвансы.ДокументАванса,
	...
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаАвансы.Контрагент.СтранаРезидентства = ЗНАЧЕНИЕ(Справочник.СтраныМира.Киргизия)
	|			ТОГДА ""АВАНС-1""
	|		КОГДА ВременнаяТаблицаАвансы.Контрагент.СтранаРезидентства.ЕАЭС
	|			ТОГДА ""АВАНС-2""
	|		ИНАЧЕ ""АВАНС-3""
	...
	|	ВременнаяТаблицаОтгрузкаТекущегоМесяцаФинал.СтавкаНДС,
	|	ВЫБОР
	|		КОГДА ВременнаяТаблицаОтгрузкаТекущегоМесяцаФинал.Контрагент.СтранаРезидентства = ЗНАЧЕНИЕ(Справочник.СтраныМира.ПустаяСсылка)
	|			ТОГДА ЛОЖЬ
	|		ИНАЧЕ ВременнаяТаблицаОтгрузкаТекущегоМесяцаФинал.Контрагент.СтранаРезидентства.ЕАЭС
...
Процедура РассчитатьИтогиОтчетаРеестрПоставок() Экспорт	
	...
	Для каждого СтрокаТабличнойЧасти Из ОтчетРеестрПоставок Цикл
		...
		Если НЕ СтрокаТабличнойЧасти.КодПоставки = "111" И НЕ СтрокаТабличнойЧасти.КодПоставки = "112" Тогда
			...
			Если СтрокаТабличнойЧасти.КонтрагентСтранаРезидентстваЕАЭС Тогда
				ИтогоСуммаБНДСОблТС 	= ИтогоСуммаБНДСОблТС + СтрокаТабличнойЧасти.СтоимостьБезНДС;			
			КонецЕсли;			
	...
	НоваяСтрокаИтогов = ОтчетРеестрПоставокИтоги.Добавить();
	НоваяСтрокаИтогов.Содержание 			= "из них НУЛЕВЫЕ ПОСТАВКИ";
	...
	НоваяСтрокаИтогов = ОтчетРеестрПоставокИтоги.Добавить();
	НоваяСтрокаИтогов.Содержание 			= "в том числе в ТАМОЖЕННЫЙ СОЮЗ";
	НоваяСтрокаИтогов.СтоимостьБезНДС 		= ИтогоСуммаБНДСОблТС;
	НоваяСтрокаИтогов.СуммаНДС 				= 0;
	НоваяСтрокаИтогов.ОбщаяСтоимостьСНДС 	= ИтогоСуммаБНДСОблТС;
...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПриобретенных() Экспорт
	...
	|		ПО ПоступленияОбороты.ДокументПоступления = ПоступленияИтоги.ДокументПоступления
	|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СчетаФактурыПолученные КАК СчетаФактурыПолученные
	|		ПО ПоступленияОбороты.ДокументПоступления = СчетаФактурыПолученные.Документ
	|ГДЕ
