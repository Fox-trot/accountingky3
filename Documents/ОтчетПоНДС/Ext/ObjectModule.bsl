...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПриобретенных(НалоговыйКонтракт, НеУчитыватьЗакупкиБезНДС) Экспорт
	...
	Запрос.Текст =  
		...
		|	МАКСИМУМ(ВременнаяТаблицаГотовыеДанные.КонтрагентНаименование) КАК КонтрагентНаименование,
		|	МАКСИМУМ(ВременнаяТаблицаГотовыеДанные.ИННКонтрагента) КАК ИННКонтрагента,
		|	МАКСИМУМ(ВременнаяТаблицаГотовыеДанные.КодГНС) КАК КодГНС,
		|	МАКСИМУМ(ВременнаяТаблицаГотовыеДанные.СерияСФ) КАК СерияСФ,
		|	МАКСИМУМ(ВременнаяТаблицаГотовыеДанные.НомерСФ) КАК НомерСФ,
		|	МАКСИМУМ(ВременнаяТаблицаГотовыеДанные.КорСерияСФ) КАК КорСерияСФ,
		|	МАКСИМУМ(ВременнаяТаблицаГотовыеДанные.КорНомерСФ) КАК КорНомерСФ,
		|	МАКСИМУМ(ВременнаяТаблицаГотовыеДанные.ДатаПоставки) КАК ДатаПоставки,
