...
// Процедура заполняет табличную часть
//
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПоставок(НалоговыйКонтракт) Экспорт
	...
	Запрос.Текст = 
		...
		|	КОНЕЦ КАК НомерСФ,
		|	ВЫБОР
		|		КОГДА АвансыДоотгрузкаОбороты.Договор.СуммаВключаетНалоги
		|			ТОГДА АвансыДоотгрузкаОбороты.СуммаПриход - АвансыДоотгрузкаОбороты.СуммаНДСПриход - АвансыДоотгрузкаОбороты.СуммаНСППриход
		|		ИНАЧЕ АвансыДоотгрузкаОбороты.СуммаПриход
		|	КОНЕЦ КАК СуммаПриход,
		|	ВЫБОР
		|		КОГДА АвансыДоотгрузкаОбороты.Договор.СуммаВключаетНалоги
		|			ТОГДА АвансыДоотгрузкаОбороты.СуммаРасход - АвансыДоотгрузкаОбороты.СуммаНДСРасход - АвансыДоотгрузкаОбороты.СуммаНСПРасход
		|		ИНАЧЕ АвансыДоотгрузкаОбороты.СуммаРасход
		...
		|	ВременнаяТаблицаАвансыДоотгрузка.Дата КАК ДатаПоставки,
		...
		|
		|УПОРЯДОЧИТЬ ПО
		|	ВременнаяТаблицаАвансыДоотгрузка.ДокументОтгрузки
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Процедура РассчитатьИтогиОтчетаРеестрПоставок() Экспорт	
	...
	НоваяСтрокаИтогов = ОтчетРеестрПоставокИтоги.Добавить();
	НоваяСтрокаИтогов.Содержание 			= НСтр("ru = 'ОБЛАГАЕМЫЕ ПОСТАВКИ'");
	...
	НоваяСтрокаИтогов = ОтчетРеестрПоставокИтоги.Добавить();
	НоваяСтрокаИтогов.Содержание 			= НСтр("ru = 'ОСВОБОЖДЕННЫЕ И НЕОБЛАГАЕМЫЕ ПОСТАВКИ'");
...
// Процедура заполняет табличную часть
//
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПриобретенных(НалоговыйКонтракт) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст =  
		"ВЫБРАТЬ
		...
		|	РеестрПриобретенныхМатериальныхРесурсов.Сумма КАК СтоимостьБезНДС,
		|	РеестрПриобретенныхМатериальныхРесурсов.СуммаНДС КАК СуммаНДС
		|ПОМЕСТИТЬ ВременнаяТаблицаРеестрПриобретенных
		|ИЗ
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаРеестрПриобретенных.КонтрагентНаименование КАК КонтрагентНаименование,
		|	ВременнаяТаблицаРеестрПриобретенных.ИННКонтрагента КАК ИННКонтрагента,
		|	ВременнаяТаблицаРеестрПриобретенных.КодГНС КАК КодГНС,
		|	ВременнаяТаблицаРеестрПриобретенных.СерияСФ КАК СерияСФ,
		|	ВременнаяТаблицаРеестрПриобретенных.НомерСФ КАК НомерСФ,
		|	ВременнаяТаблицаРеестрПриобретенных.КорСерияСФ КАК КорСерияСФ,
		|	ВременнаяТаблицаРеестрПриобретенных.КорНомерСФ КАК КорНомерСФ,
		|	ВременнаяТаблицаРеестрПриобретенных.ДатаПоставки КАК ДатаПоставки,
		|	ВременнаяТаблицаРеестрПриобретенных.СтоимостьБезНДС КАК СтоимостьБезНДС,
		|	ВременнаяТаблицаРеестрПриобретенных.СуммаНДС КАК СуммаНДС
		|ИЗ
		|	ВременнаяТаблицаРеестрПриобретенных КАК ВременнаяТаблицаРеестрПриобретенных
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ЕСТЬNULL(СУММА(ВременнаяТаблицаРеестрПриобретенных.СуммаНДС), 0) КАК СуммаНДС
		|ИЗ
		|	ВременнаяТаблицаРеестрПриобретенных КАК ВременнаяТаблицаРеестрПриобретенных
		|;
		|
		...
		|	ВременнаяТаблицаРеестрПриобретенных КАК ВременнаяТаблицаРеестрПриобретенных
		|ГДЕ
		|	ВременнаяТаблицаРеестрПриобретенных.СуммаНДС > 0";
	Запрос.УстановитьПараметр("НачалоПериода", 	НачалоМесяца(Дата));            
	Запрос.УстановитьПараметр("КонецПериода", 	КонецМесяца(Дата));
	Запрос.УстановитьПараметр("Организация", 	Организация); 
	...
	СтрокаОтчета = ОтчетОсновной.Найти("060", "Строка");
	
	Коэффициент = 0;
	
	Если СтрокаОтчета <> Неопределено И СуммаНДС > 0 Тогда
		Коэффициент = СтрокаОтчета.Сумма / СуммаНДС;
	КонецЕсли;
	...
	Если НалоговыйКонтракт Тогда
		...
		Пока Выборка.Следующий() Цикл		
			...
			СтрокаТабличнойЧасти.СуммаПодлежащаяКЗачетуНДС = Выборка.СуммаНДС * Коэффициент;		
		КонецЦикла;	
		...
		Для Каждого СтрокаТабличнойЧасти Из ОтчетРеестрПриобретенныхНаОсновеНалоговогоКонтракта Цикл
			
			Если СтрокаТабличнойЧасти.СтоимостьБезНДС < 0 Тогда				
				Отбор = Новый Структура();
				Отбор.Вставить("СерияСФ", СтрокаТабличнойЧасти.КорСерияСФ);
				Отбор.Вставить("НомерСФ", СтрокаТабличнойЧасти.КорНомерСФ);
				
				МассивСтрок = ОтчетРеестрПриобретенныхНаОсновеНалоговогоКонтракта.НайтиСтроки(Отбор);
				
				Если МассивСтрок.Количество() > 0 Тогда					
					НайденнаяСтрока = МассивСтрок[0];
					Если НайденнаяСтрока.СуммаНДС <> 0 Тогда
						СтрокаТабличнойЧасти.СуммаПодлежащаяКЗачетуНДС = СтрокаТабличнойЧасти.СуммаНДС / НайденнаяСтрока.СуммаНДС 
																			* НайденнаяСтрока.СуммаПодлежащаяКЗачетуНДС;
					КонецЕсли;
				КонецЕсли;	
			КонецЕсли;				
		КонецЦикла;	
	...
	Иначе
		...
		Для Каждого СтрокаТабличнойЧасти Из ОтчетРеестрПриобретенныхМатРессурсов Цикл
			...
			Если СтрокаТабличнойЧасти.СтоимостьБезНДС < 0 Тогда				
				...
				МассивСтрок = ОтчетРеестрПриобретенныхМатРессурсов.НайтиСтроки(Отбор);
...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрГТД() Экспорт
	...
	ТаблицаРеестрВвезенных = Запрос.Выполнить().Выгрузить();
	
	СуммаНДС 		= ТаблицаРеестрВвезенных.Итог("ГТДСуммаНДС");	
	СтрокаОтчета 	= ОтчетОсновной.Найти("061", "Строка");	
	Коэффициент 	= 0;
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРеестрВвезенных Цикл		
		СтрокаТабличнойЧасти = ОтчетРеестрГТД.Добавить();		
		ЗаполнитьЗначенияСвойств(СтрокаТабличнойЧасти, СтрокаТаблицы);
		СтрокаТабличнойЧасти.НДСЗачет = СтрокаТабличнойЧасти.ГТДСуммаНДС * Коэффициент;
	КонецЦикла;	
	
	Если ОтчетРеестрГТД.Количество() = 0 Тогда
