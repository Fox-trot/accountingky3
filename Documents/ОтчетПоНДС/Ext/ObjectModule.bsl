...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПоставок(НалоговыйКонтракт) Экспорт	
			...
			Пока ПродолжитьОбходТаблицы Цикл
				Для каждого СтрокаТаблицы Из ТаблицаОтгрузокКонтрагента Цикл
					Если ТекущаяОплата > 0 Тогда
						Если НалоговыйКонтракт Тогда
							НоваяСтрокаДоотгрузки = ОтчетРеестрПоставокНаОсновеНалоговогоКонтракта.Добавить();
						Иначе
	...
	Если НалоговыйКонтракт Тогда
		Если ОтчетРеестрПоставокНаОсновеНалоговогоКонтракта.Количество() = 0 Тогда
			ТекстСообщения = СтрШаблон(НСтр("ru = 'В период ""%1"" нет данных для заполнения отчета ""Реестр поставок на осн. налогового контракта""!'"), 
											ПредставлениеПериода(НачалоМесяца(Дата), КонецМесяца(Дата)));
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);		
		КонецЕсли;
	Иначе
...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПриобретенных(НалоговыйКонтракт) Экспорт
	...
	Запрос.Текст =  
		...
		|	РеестрПриобретенныхМатериальныхРесурсов.СуммаПодлежащаяКЗачетуНДС КАК СуммаПодлежащаяКЗачетуНДС
		|ИЗ
	...
	Если НалоговыйКонтракт Тогда
		ОтчетРеестрПриобретенныхНаОсновеНалоговогоКонтракта.Загрузить(Запрос.Выполнить().Выгрузить());
...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрГТД() Экспорт
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА РеестрВвезенныхОбороты.Регистратор ССЫЛКА Документ.ГТДПоИмпорту
		|			ТОГДА РеестрВвезенныхОбороты.Номер
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК ГТДНомер,
		|	ВЫБОР
		|		КОГДА НЕ РеестрВвезенныхОбороты.Регистратор ССЫЛКА Документ.ГТДПоИмпорту
		|			ТОГДА РеестрВвезенныхОбороты.Номер
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК ПДНомер,
		|	ВЫБОР
		|		КОГДА РеестрВвезенныхОбороты.Регистратор ССЫЛКА Документ.ГТДПоИмпорту
		|			ТОГДА РеестрВвезенныхОбороты.ПериодДень
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК ГТДДата,
		|	ВЫБОР
		|		КОГДА НЕ РеестрВвезенныхОбороты.Регистратор ССЫЛКА Документ.ГТДПоИмпорту
		...
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК ГТДСуммаНДС,
		|	ВЫБОР
		|		КОГДА НЕ РеестрВвезенныхОбороты.Регистратор ССЫЛКА Документ.ГТДПоИмпорту
		|			ТОГДА РеестрВвезенныхОбороты.СуммаНДСОборот
		|		ИНАЧЕ НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК ПДСуммаНДС,
		|	РеестрВвезенныхОбороты.СуммаОборот КАК ГТДСуммаБезНДС,
		|	РеестрВвезенныхОбороты.СуммаПодлежащаяКЗачетуНДСОборот КАК НДСЗачет
		|ИЗ
