...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПриобретенных(НалоговыйКонтракт) Экспорт
	...
	Если СтрокаОтчета <> Неопределено И СуммаНДС > 0 Тогда
		ТаблицаСумм = МассивРезультатов[3].Выгрузить();
		МассивКоэффициентов = ТаблицаСумм.ВыгрузитьКолонку("СуммаНДС");
		
		МассивСумм = ОбщегоНазначенияКлиентСервер.РаспределитьСуммуПропорциональноКоэффициентам(СтрокаОтчета.Сумма, МассивКоэффициентов);
	КонецЕсли;
	...
	Если НалоговыйКонтракт Тогда
		...
		Пока Выборка.Следующий() Цикл		
			...
			Если СтрокаОтчета <> Неопределено Тогда
				Если СтрокаОтчета.Сумма <> 0 И СуммаНДС > 0 И Выборка.СуммаНДС > 0 Тогда						
					СтрокаТабличнойЧасти.СуммаПодлежащаяКЗачетуНДС = МассивСумм[Индекс];
					Индекс = Индекс + 1;
				КонецЕсли;
			КонецЕсли;			
		КонецЦикла;	
