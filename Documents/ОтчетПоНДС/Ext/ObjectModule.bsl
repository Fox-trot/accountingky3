...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПриобретенных(НалоговыйКонтракт) Экспорт
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	ЗакрытиеМесяцаДоотгрузка.Ссылка.Дата = &Дата
		...
		|ГДЕ
		|	ЗакрытиеМесяцаАвансы.Ссылка.Дата = &Дата
	...
	КоэффициентОблагаемыхПоставок = 1 - КоэффициентОсвобожденныхПоставокРасчетный;
	...
	Запрос.Текст =  
		...
		|	СУММА(РеестрПриобретенныхМатериальныхРесурсов.СуммаНДС) КАК СуммаНДС,
		|	СУММА(ВЫБОР
...
//
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрВвезенных() Экспорт
	...
	Для Каждого СтрокаТаблицы Из ТаблицаРеестрВвезенных Цикл		
		СтрокаТабличнойЧасти = ОтчетРеестрВвезенныхМатериальныхРесурсовВКР.Добавить();		
