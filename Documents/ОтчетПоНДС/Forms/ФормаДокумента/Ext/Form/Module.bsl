...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПоставокНаСервере()
	Документ = РеквизитФормыВЗначение("Объект");
	Документ.ЗаполнитьТабличнуюЧастьОтчетРеестрПоставок(ДанныеУчетнойПолитики.Контракт, ДанныеУчетнойПолитики.НеУчитыватьЗакупкиБезНДС);
	ЗначениеВРеквизитФормы(Документ, "Объект");
КонецПроцедуры
...
Функция ОтчетНДСОсновнаяФорма()
	...
	ОбластьМакета.Параметры.Приложение03 = ?(Объект.ОтчетРеестрВвезенныхМатериальныхРесурсовВКР.Количество() = 0, "0", "1");
...
Функция record(СтрокаДанных, НомерПриложения)
	...
	ИначеЕсли НомерПриложения = 3 Тогда
		ОбъектXDTO.column.Добавить(column(1, СтрокаДанных.Контрагент));
