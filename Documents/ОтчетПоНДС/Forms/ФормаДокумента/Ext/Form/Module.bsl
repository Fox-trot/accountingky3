...
Процедура ЗаполнитьДокумент(Команда)
	...
	Если Объект.ОтчетОсновной.Количество() > 0 
		...
		ИЛИ Объект.ОтчетРеестрГТД.Количество() > 0
...
Функция ОтчетНДСПриложение03(КоличествоОпераций)
	...
	Для каждого СтрокаТабличнойЧасти Из Объект.ОтчетРеестрГТД Цикл
		Если СтрокаТабличнойЧасти.Контрагент.ПризнакСтраны = Перечисления.ПризнакиСтраны.ЕАЭС Тогда 
...
Функция ФормированиеФайлаXML(ДополнительныеПараметры, Ошибки)
	...
	Запрос.Текст = 
		...
		|	ТаблицаДокумента.ОтчетРеестрГТД.(
		|		НомерСтроки КАК НомерСтроки
		|	) КАК ОтчетРеестрГТД,
	...
	ОбъектXDTO.data.Добавить(data("sti_062_001", ?(ВыборкаШапка.ОтчетРеестрПоставок.Выбрать().Количество() = 0, 0, 1)));
	ОбъектXDTO.data.Добавить(data("sti_062_002", ?(ВыборкаШапка.ОтчетРеестрПриобретенныхМатРессурсов.Выбрать().Количество() = 0, 0, 1)));
	ОбъектXDTO.data.Добавить(data("sti_062_003", ?(ВыборкаШапка.ОтчетРеестрГТД.Выбрать().Количество() = 0, 0, 1)));
	ОбъектXDTO.data.Добавить(data("sti_062_004", ?(ВыборкаШапка.ОтчетРеестрПоставокНаОсновеНалоговогоКонтракта.Выбрать().Количество() = 0, 0, 1)));
	ОбъектXDTO.data.Добавить(data("sti_062_005", ?(ВыборкаШапка.ОтчетРеестрПриобретенныхНаОсновеНалоговогоКонтракта.Выбрать().Количество() = 0, 0, 1)));
