...
Процедура ЗаполнитьДокумент(Команда)
	...
	Если Объект.ОтчетОсновной.Количество() > 0 
		...
		ИЛИ Объект.ОтчетРеестрПриобретенныхМатРессурсовИтоги.Количество() > 0
...
&НаКлиенте
Процедура СохранитьВЭлектроннойФорме_UBR(Команда)
	...
	Если ТребуетсяНастройкаАвторизацияИнтернетПоддержки() Тогда
		ТекстВопроса = НСтр("ru='Для формирования отчета по НДС в электронной форме
			|необходимо подключиться к Интернет-поддержке пользователей.
			|Подключиться сейчас?'");
		ОписаниеОповещения = Новый ОписаниеОповещения("ПриПодключенииИнтернетПоддержки", ЭтотОбъект, Новый Структура("НаправлениеВыгрузки", "СохранитьВЭлектроннойФорме_UBR"));
		Кнопки = Новый СписокЗначений;
		Кнопки.Добавить(КодВозвратаДиалога.Да, НСтр("ru = 'Подключиться'"));
		Кнопки.Добавить(КодВозвратаДиалога.Отмена);
		...
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, Кнопки);
	Иначе
		ПродолжитьСохранение("СохранитьВЭлектроннойФорме_UBR");
	КонецЕсли;
КонецПроцедуры
...
Процедура ПослеПодключенияИнтернетПоддержки(Результат, ДополнительныеПараметры) Экспорт
	...
	Если ДополнительныеПараметры.НаправлениеВыгрузки = "СохранитьВЭлектроннойФорме" Тогда
		ПродолжитьСохранение("СохранитьВЭлектроннойФорме");	
	ИначеЕсли ДополнительныеПараметры.НаправлениеВыгрузки = "СохранитьВЭлектроннойФорме_UBR" Тогда
		ПродолжитьСохранение("СохранитьВЭлектроннойФорме_UBR");	
	Иначе
...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПоставокНаСервере()
	...
	Документ.ЗаполнитьТабличнуюЧастьОтчетРеестрПоставок(ДанныеУчетнойПолитики.Контракт, 
...
Процедура ЗаполнитьТабличнуюЧастьОтчетРеестрПриобретенныхНаСервере()
	Документ = РеквизитФормыВЗначение("Объект");
	Документ.ЗаполнитьТабличнуюЧастьОтчетРеестрПриобретенных(ДанныеУчетнойПолитики.Контракт, ДанныеУчетнойПолитики.НеУчитыватьЗакупкиБезНДС);
	ЗначениеВРеквизитФормы(Документ, "Объект");
КонецПроцедуры
...
Функция ОтчетНДСОсновнаяФорма()
	...
	// Вид отчета
	Если Объект.ВидОтчета = Перечисления.ВидыОтчетаПоНДС.Уточненный Тогда
		ДанныеПечати.Вставить("ВидОтчета", 1);
		
	ИначеЕсли Объект.ВидОтчета = Перечисления.ВидыОтчетаПоНДС.Ликвидационный Тогда
	...
	ДанныеПечати.Вставить("Приложение02", ?(Объект.ОтчетРеестрПриобретенныхМатРессурсов.Количество() = 0, "0", "1"));
	...
	Иначе	      		
		Если ВыбранныеФайлы <> Неопределено И ВыбранныеФайлы.Количество() > 0 Тогда
			Попытка
				...
				Если ДополнительныеПараметры.ВидДиалога = "СохранитьВЭлектроннойФорме_UBR" Тогда
					СтруктураДанных = ФормированиеФайлаXML_UBR(ДополнительныеПараметры, Ошибки);
...
Функция ФормированиеФайлаXML(ДополнительныеПараметры, Ошибки)
	...
	Запрос.Текст = 
		...
		|	ТаблицаДокумента.ОтчетРеестрПриобретенныхМатРессурсов.(
		|		НомерСФ,
		|		СерияСФ,
		|		КонтрагентНаименование,
		|		ИННКонтрагента,
		...
		|		КорСерияСФ,
		|		КорНомерСФ
		|	) КАК ОтчетРеестрПриобретенныхМатРессурсов,
	...
	ТаблицаРеестрПриобретенныхМатРессурсов 	= ВыборкаШапка.ОтчетРеестрПриобретенныхМатРессурсов.Выгрузить();
	...
	ОбъектXDTO.data.Добавить(data("sti_062_002", ?(ТаблицаРеестрПриобретенныхМатРессурсов.Количество() = 0, 0, 1)));
	...
	ОбъектXDTO.attachment.Добавить(attachment(2, ТаблицаРеестрПриобретенныхМатРессурсов));
...
&НаСервере
Функция СоздатьОбъектXDTO(ТипОбъекта, UBR = Ложь)
...
//
&НаСервере
Функция ФормированиеФайлаXML_UBR(ДополнительныеПараметры, Ошибки)
	...
	Если Объект.ВидОтчета = Перечисления.ВидыОтчетаПоНДС.Первоначальный Тогда 
		FTYPE = 0;
	ИначеЕсли Объект.ВидОтчета = Перечисления.ВидыОтчетаПоНДС.Уточненный Тогда 
		FTYPE = 1;
	ИначеЕсли Объект.ВидОтчета = Перечисления.ВидыОтчетаПоНДС.Ликвидационный Тогда 
		FTYPE = 3;
	Иначе
	...
	ТаблицаПриложение2 	= ВыборкаШапка.ОтчетРеестрПриобретенныхМатРессурсов.Выгрузить();
