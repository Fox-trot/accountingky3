...
Процедура УстановитьПараметрыВыбора()
	...
	МассивПараметровВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ПризнакиСтраны.ЕАЭС"));
	МассивПараметровВыбора.Добавить(ПредопределенноеЗначение("Перечисление.ПризнакиСтраны.ИмпортЭкспорт"));

	// Формирование параметра выбора.
	НовыйПараметр = Новый ПараметрВыбора("Отбор.ПризнакСтраны", Новый ФиксированныйМассив(МассивПараметровВыбора));
...
Функция ФормированиеФайла(ДополнительныеПараметры, Ошибки)
	...
	Иначе 
		...
		КонецЕсли;
	КонецЕсли;
	Если НЕ ЗначениеЗаполнено(ВыборкаШапка.КурсДокумента) Тогда 
		ТекстДляОднойОшибки = НСтр("ru = 'Поле ""Курс документа"" не заполнено.'");
		ОбщегоНазначенияКлиентСервер.ДобавитьОшибкуПользователю(Ошибки,	"Объект.КурсДокумента", ТекстДляОднойОшибки, "Объект",, ТекстДляОднойОшибки);
		Отказ = Истина;
	КонецЕсли;
...
Функция Commodity(ВыборкаШапка, СтрокаТабличнойЧасти)
	...
	// Курс валюты.
	ОбъектXDTO.currency_rate = ВыборкаШапка.КурсДокумента;
