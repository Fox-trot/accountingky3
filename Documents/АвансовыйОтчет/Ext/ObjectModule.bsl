...
Процедура ЗаполнитьТаблицуДокументов(Валюта, СуммаОстатка, ДатаДокумента, КлючСвязи)
	...
	Запрос.Текст =
		...
		|		КОНЕЦ
		|	И (ВЫРАЗИТЬ(АвансыПодотчетника.Документ КАК Документ.АвансовыйОтчет).СчетУчета = &СчетУчета
		|			ИЛИ ВЫРАЗИТЬ(АвансыПодотчетника.Документ КАК Документ.ПлатежноеПоручениеИсходящее).СчетРасчетов = &СчетУчета
		|			ИЛИ ВЫРАЗИТЬ(АвансыПодотчетника.Документ КАК Документ.ПриходныйКассовыйОрдер).СчетРасчетов = &СчетУчета
		|			ИЛИ ВЫРАЗИТЬ(АвансыПодотчетника.Документ КАК Документ.РасходныйКассовыйОрдер).СчетРасчетов = &СчетУчета)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Дата УБЫВ";
