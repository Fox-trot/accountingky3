...
Процедура СформироватьТаблицаХозрасчетный(СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА ТаблицаПрочее.Содержание = """"
		|			ТОГДА ""Поступления по прочим операциям""
		|		ИНАЧЕ ТаблицаПрочее.Содержание
		|	КОНЕЦ,
		|	ВЫРАЗИТЬ(ТаблицаПрочее.Сумма * ТаблицаПрочее.Курс / ТаблицаПрочее.Кратность КАК ЧИСЛО(15,2)),
...
Процедура СформироватьТаблицаСведенияОПоступлении(СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		|	ВладельцыПатентов.Паспорт КАК Патентник,
		...
		|	ВременнаяТаблицаДанныеТЧ.ПризнакСтраны КАК ПризнакСтраны,
		|	ВременнаяТаблицаДанныеТЧ.Патентник КАК Патентник,
		|	ВременнаяТаблицаШапка.Дата КАК ДатаПоставки,
		|	ВременнаяТаблицаШапка.Дата КАК ДатаСФ,
		|	СУММА(ВременнаяТаблицаДанныеТЧ.Сумма) КАК Сумма
		|ИЗ
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаДанныеТЧ КАК ВременнаяТаблицаДанныеТЧ
		|		ПО (ИСТИНА)
		|ГДЕ
		|	ВременнаяТаблицаДанныеТЧ.Сумма <> 0
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ВременнаяТаблицаДанныеТЧ.ПризнакСтраны,
		|	ВременнаяТаблицаДанныеТЧ.Патентник,
		|	ВременнаяТаблицаШапка.Дата,
		|	ВременнаяТаблицаШапка.Дата";	
...
Процедура СформироватьТаблицаРеестрПриобретенных(СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		|	ВладельцыПатентов.Контрагент КАК Контрагент,
		|	ВЫБОР
		|		КОГДА ВладельцыПатентов.Контрагент.НаименованиеПолное = """"
		|			ТОГДА ВладельцыПатентов.Контрагент.Наименование
		|		ИНАЧЕ ВладельцыПатентов.Контрагент.НаименованиеПолное
		|	КОНЕЦ КАК КонтрагентНаименование,
		|	ВладельцыПатентов.Контрагент.ИНН КАК ИННКонтрагента,
		|	ВладельцыПатентов.Контрагент.ГНС.Код КАК КодГНСКонтрагента,
		|	ВладельцыПатентов.ВидДокумента КАК СерияБланкаСФ,
		|	ВладельцыПатентов.НомерДокумента КАК НомерБланкаСФ,
		|	ВладельцыПатентов.ДатаДокумента КАК ДатаПоставки,
		...
		|		ПО ВладельцыПатентов.КлючСвязи = ТоварыУслугиПатентника.КлючСвязи
		|ГДЕ
		|	ВладельцыПатентов.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.КР)
		|	И ТоварыУслугиПатентника.Сумма <> 0
		|
		|СГРУППИРОВАТЬ ПО
		|	ВременнаяТаблицаШапка.Организация,
		|	ВременнаяТаблицаШапка.Ссылка,
		...
		|		ИНАЧЕ ВладельцыПатентов.Контрагент.НаименованиеПолное
		|	КОНЕЦ,
		|	ВладельцыПатентов.Контрагент.ГНС.Код,
		|	ВладельцыПатентов.Контрагент.ИНН,
...
Процедура СформироватьТаблицаКорректировкиНУ(СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА ТаблицаПрочее.СчетУчета.Вид = ЗНАЧЕНИЕ(ВидСчета.Активный)
		|			ТОГДА ЗНАЧЕНИЕ(Перечисление.ПризнакСчета.Расходный)
		|		ИНАЧЕ ЗНАЧЕНИЕ(Перечисление.ПризнакСчета.Доходный)
...
Функция ПечатьАвансовогоОтчета(МассивОбъектов, ОбъектыПечати)
	...
	ТекстЗапроса =
		...
		|	АвансовыйОтчет.Документов,
		|	АвансовыйОтчет.Листов
		|ИЗ
		|	Документ.АвансовыйОтчет КАК АвансовыйОтчет
		|ГДЕ
	...
	Пока Шапка.Следующий() Цикл
		...
		Пока ВыборкаТабличныхЧастей.Следующий() Цикл
			Если ЗначениеЗаполнено(ВыборкаТабличныхЧастей.Выдано) 
				...
				ИЛИ ЗначениеЗаполнено(ВыборкаТабличныхЧастей.ИзрасходованоВВалюте) Тогда
		...
		Если ВыборкаПоСсылкеПроводки.Следующий() Тогда
			...
			ОбластьМакета.Параметры.КоличествоДокументовВПриложени = Шапка.Документов;
		...
		Если ВыборкаПоСсылке.Следующий() Тогда
			...
			Для Каждого СТЧ Из Ссылка.ОплатаПоставщикам Цикл
				...
				Для Каждого СТЧТ Из СТЧ.ВидДокВходящий.Товары Цикл
					...
					Сумма = СТЧТ.Сумма
					+ ?(СТЧ.ВидДокВходящий.СуммаВключаетНалоги, 0, СТЧТ.СуммаНДС);
				...
				Для Каждого СТЧТ Из СТЧ.ВидДокВходящий.Услуги Цикл
					...
					ОбластьМакета.Параметры.ДокументДата        = СТЧ.ВидДокВходящий.Дата;
					ОбластьМакета.Параметры.НаименованиеРасхода = СокрЛП(СТЧТ.Номенклатура.Наименование);
					ОбластьМакета.Параметры.ДебетСубСчета 		= СТЧТ.СчетРасхода;
					
					Сумма = СТЧТ.Сумма + ?(СТЧ.ВидДокВходящий.СуммаВключаетНалоги, 0, СТЧТ.СуммаНДС);
...
Функция ПечатьАвансовогоОтчетаАО1(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст =
		...
		|	АвансовыйОтчетОплатаПоставщикам.Сумма КАК ПоОтчету,
		|	АвансовыйОтчетОплатаПоставщикам.Сумма * АвансовыйОтчетОплатаПоставщикам.Курс / АвансовыйОтчетОплатаПоставщикам.Кратность КАК ПоОтчетуВВалюте,
		|	АвансовыйОтчетОплатаПоставщикам.Сумма КАК ПоУчету,
		|	АвансовыйОтчетОплатаПоставщикам.Сумма * АвансовыйОтчетОплатаПоставщикам.Курс / АвансовыйОтчетОплатаПоставщикам.Кратность КАК ПоУчетуВВалюте,
		|	АвансовыйОтчетОплатаПоставщикам.СчетРасчетов КАК ДебетСубСчета
		|ПОМЕСТИТЬ ВременнаяТаблицаДанныеТЧ
		|ИЗ
		|	Документ.АвансовыйОтчет.ОплатаПоставщикам КАК АвансовыйОтчетОплатаПоставщикам
		|ГДЕ
		...
		|	ВременнаяТаблицаДанныеТЧ.ДебетСубСчета КАК ДебетСубСчета
		|ИЗ
		|	ВременнаяТаблицаДанныеТЧ КАК ВременнаяТаблицаДанныеТЧ
		|
		|СГРУППИРОВАТЬ ПО
		...
		|	ВременнаяТаблицаДанныеТЧ.ДебетСубСчета";
	...
	Пока Шапка.Следующий() Цикл	
		...
		СтрокаТЗ = ТаблицаСуммыДокументов.Найти(Шапка.Ссылка, "Ссылка");
		
		Если СтрокаТЗ <> Неопределено Тогда
			ДанныеПечати.Вставить("СуммаОтчетаПрописью", БухгалтерскийУчетСервер.СформироватьСуммуПрописью(СтрокаТЗ.Сумма, ВалютаРегламентированногоУчета));
			ДанныеПечати.Вставить("СуммаОтчетаЧисломЦел", Цел(СтрокаТЗ.Сумма));
			ДанныеПечати.Вставить("СуммаОтчетаЧисломДроб", Окр((СтрокаТЗ.Сумма - Цел(СтрокаТЗ.Сумма)) * 100, 2));
		КонецЕсли;
			
		ДанныеФизЛица = БухгалтерскийУчетСервер.ДанныеФизЛица(Шапка.Организация, Шапка.ПодотчетноеЛицо, Шапка.Дата, Ложь);
		
		ДанныеПечати.Вставить("ПредставлениеПодотчетногоЛица", ДанныеФизЛица.Представление);
		ДанныеПечати.Вставить("ПредставлениеПодразделения", ДанныеФизЛица.Подразделение);
		ДанныеПечати.Вставить("ПредставлениеДолжность", ДанныеФизЛица.Должность);
		
		Запрос = Новый Запрос;
		Запрос.Текст =
			"ВЫБРАТЬ
			|	ТиповойОстатки.Счет,
			|	ЕСТЬNULL(ТиповойОстатки.СуммаОстаток, 0) СуммаОстаток
			|ИЗ
			|	РегистрБухгалтерии.Хозрасчетный.Остатки(
			|		&ДатаИсключенияДок, 
			|		Счет = &СчетРасчета, 
			...
			|			Организация = &Организация
			|				И Субконто1 = &Сотрудник) КАК ТиповойОстатки";
	   	Запрос.УстановитьПараметр("ДатаИсключенияДок", 	Шапка.Дата);
		Запрос.УстановитьПараметр("ДатаВключенияДок", 	Шапка.Дата + 1);
		Запрос.УстановитьПараметр("СчетРасчета", 		Шапка.СчетУчета);
		Запрос.УстановитьПараметр("Организация", 		Шапка.Организация);
		Запрос.УстановитьПараметр("Сотрудник", 			Шапка.ПодотчетноеЛицо);
		МассивРезультатов = Запрос.ВыполнитьПакет();
			
		ОстатокНаНачало = МассивРезультатов[0].Выбрать();
		
		Если ОстатокНаНачало.Следующий() Тогда
			Сумма = ОстатокНаНачало.СуммаОстаток;
		...
		ОстатокНаКонец = МассивРезультатов[1].Выбрать();
		
		Если ОстатокНаКонец.Следующий() Тогда		
			Сумма = ОстатокНаКонец.СуммаОстаток;
			
			Если Сумма > 0  Тогда
				ДанныеПечати.Вставить("КонечныйОстаток", Сумма);
			Иначе
				ДанныеПечати.Вставить("КонечныйПерерасход", -Сумма);
			КонецЕсли;
		КонецЕсли;
		...
		Для Каждого СтрокаМассива Из МассивСтрокПроводок Цикл
			...
			Если Индекс < 10 Тогда
				ОбластьМакета.Параметры["СубСчетДебет" 	+ Индекс] = СтрокаМассива.СчетДт;
				ОбластьМакета.Параметры["СуммаДебет" 	+ Индекс] = СтрокаМассива.Сумма;
				ОбластьМакета.Параметры["СубСчетКредит" + Индекс] = СтрокаМассива.СчетКт;
				...
				КонецЕсли;	
			Иначе
				ОбластьМакетаСтрока.Параметры.СубСчетДебет 	= СтрокаМассива.СчетДт;
				ОбластьМакетаСтрока.Параметры.СуммаДебет 	= СтрокаМассива.Сумма;
				ОбластьМакетаСтрока.Параметры.СубСчетКредит = СтрокаМассива.СчетКт;
