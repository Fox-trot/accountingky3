...
Функция ПечатьАвансовогоОтчетаАО1(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл	
		...
		Если СуммаОтчета <> 0 Тогда
			...
			ДанныеПечати.Вставить("СуммаОтчетаЧисломДроб", Окр((СуммаОтчета - Цел(СуммаОтчета)) * 100, 2));
		...
		//	ДанныеПечати.Вставить("Израсходовано", Цел(СтрокаТЗ.Израсходовано));
		//КонецЕсли;
		...
		Запрос.УстановитьПараметр("ПарныйСчет", 		Шапка.СчетУчета.ПарныйСчет);
		...
		Для Каждого СтрокаМассива Из МассивСтрокПроводок Цикл
			...
			ОбластьМакета.Параметры.НаименованиеРасхода = "" + СтрокаМассива.НаименованиеРасхода + " " + СтрокаМассива.Содержание;
			ТабличныйДокумент.Вывести(ОбластьМакета);
