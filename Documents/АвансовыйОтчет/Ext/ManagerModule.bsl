...
Функция ПечатьАвансовогоОтчетаУпрощенный(МассивОбъектов, ОбъектыПечати)
	...
	ТекстЗапроса =
		...
		|	ХозрасчетныйДвиженияССубконто.Содержание КАК Содержание
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.ДвиженияССубконто(, , Регистратор В (&МассивОбъектов) И (Не СчетКт В (&СчетаКР) И Не СчетКт В (&СчетаКР)), , ) КАК ХозрасчетныйДвиженияССубконто
		|
		|УПОРЯДОЧИТЬ ПО
		|	Ссылка,
		|	ХозрасчетныйДвиженияССубконто.НомерСтроки
