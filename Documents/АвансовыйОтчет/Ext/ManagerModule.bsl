...
Процедура СформироватьТаблицаХозрасчетный(СтруктураДополнительныеСвойства)
	...
	ТекстЗапроса = ТекстЗапроса + 
		...
		|	&СодержаниеВыплатаЗПCотрудникам КАК Содержание
		|ИЗ
	...
	Запрос.УстановитьПараметр("СодержаниеВыплатаЗПCотрудникам", НСтр("ru = 'Выплата ЗП сотрудникам'"));
...
Процедура СформироватьТаблицаСведенияОПоступлении(СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаВладельцыПатентов.ПризнакСтраны КАК ПризнакСтраны,
		...
		|	ВременнаяТаблицаВладельцыПатентов.ПризнакСтраны,
...
Процедура СформироватьТаблицаРеестрПриобретенныхМатериальныхРесурсов(СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		|ГДЕ
		|	НЕ ТоварыУслугиПатентника.Сумма = 0
		|
		|СГРУППИРОВАТЬ ПО
		|	ВременнаяТаблицаШапка.Дата,
		|	ВременнаяТаблицаШапка.Организация,
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаРеестЗакупок(СтруктураДополнительныеСвойства) 
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	СформироватьТаблицаРеестЗакупок(СтруктураДополнительныеСвойства);
