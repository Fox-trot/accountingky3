...
Функция ПечатьАвансовогоОтчета(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		ДанныеПечати.Вставить("СуммаОтчетаЧислом", СтрШаблон("%1 %2", СуммаОтчета, ВалютаРегламентированногоУчета));	
				...
				Пока ВыборкаПоСсылкеПроводки.Следующий() Цикл
					...
					Пока ВыборкаПроводок.Следующий() Цикл
						...
						Если ВыборкаПроводок.СчетДт = ПланыСчетов.Хозрасчетный.РасчетыСПодотчетнымиЛицами 
							...
							СтруктураСтроки.Вставить("КорСчетКредит1", "");
							...
							СтруктураСтроки.Вставить("Валюта", ВыборкаПроводок.ВалютаДт);
							СтруктураСтроки.Вставить("Сумма", ВыборкаПроводок.ВалютнаяСуммаДт);
							
							ЗаполнитьЗначенияСвойств(ОбластьМакета.Параметры, СтруктураСтроки);
							ТабличныйДокумент.Вывести(ОбластьМакета);
							
							СтруктураСтроки.Очистить();
						
							СтруктураСтроки.Вставить("КорСчетДебет1", "");
	...
	Пока Шапка.Следующий() Цикл	
		...
		Для Каждого СтрокаМассива Из МассивСтрокПроводок Цикл
			...
			Если Индекс < 10 Тогда
				ОбластьМакета.Параметры["СчетДТ" 	+ Индекс] = СтрокаМассива.СчетДт;
				ОбластьМакета.Параметры["СчетКТ" 	+ Индекс] = СтрокаМассива.СчетКт;
				ОбластьМакета.Параметры["Сумма" 	+ Индекс] = СтрокаМассива.Сумма;
		...
		Для Каждого СтрокаМассива Из МассивСтрокПроводок Цикл
			...
			СуммыВВалюте.ПоОтчетуВВалюте = Строка(СтрокаМассива.ПоОтчетуВВалюте) + " " + СтрокаМассива.Валюта;
			СуммыВВалюте.ПоУчетуВВалюте  = Строка(СтрокаМассива.ПоУчетуВВалюте) + " " + СтрокаМассива.Валюта;
