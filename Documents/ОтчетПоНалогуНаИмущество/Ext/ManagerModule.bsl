...
Процедура ЗаполнитьШапкуОтчета(СтруктураПараметров, ОбластьМакета)
	...
	Если НЕ СтруктураПараметров.Дата = КонецГода(СтруктураПараметров.Дата) Тогда
		ОбластьМакета.Параметры.Квартал = Цел(Месяц(СтруктураПараметров.Дата) / 3);	
	КонецЕсли;
КонецПроцедуры
...
Функция ОтчетПоНалогуНаИмущество2Группы(МассивОбъектов, ОбъектыПечати)
	...
	ВтораяГруппаИмущества = Справочники.ГруппыИмущества.НайтиПоНаименованию("2 группа");
	ПервыйНомер = 300;
	...
	Пока Шапка.Следующий() Цикл		
		...
		Если Шапка.Дата = КонецГода(Шапка.Дата) Тогда
		    ОбластьМакета = Макет.ПолучитьОбласть("Заголовок");
		Иначе
			ОбластьМакета = Макет.ПолучитьОбласть("ЗаголовокКвартальный");
		КонецЕсли;
		...
		Пока ВыборкаНедвижимость.Следующий() Цикл
			...
			Если ПервыйНомер = 444 ИЛИ КоличествоЗаписей = ТекущаяЗапись Тогда
				...
				Пока ВыборкаПриложение.Следующий() Цикл
					Если ВыборкаПриложение.ГруппаИмущества = ВтораяГруппаИмущества Тогда
...
Функция ОтчетПоНалогуНаИмущество3Группы(МассивОбъектов, ОбъектыПечати)
	...
	ТретьяГруппаИмущества = Справочники.ГруппыИмущества.НайтиПоНаименованию("3 группа");
	ПервыйНомер = 300;
	КоличествоСтрокНаЛисте = 1;
	...
	Пока Шапка.Следующий() Цикл		
		...
		Пока ВыборкаНедвижимость.Следующий() Цикл
			...
			Если ПервыйНомер = 435 ИЛИ КоличествоЗаписей = ТекущаяЗапись Тогда
				...
				Пока ВыборкаПриложение.Следующий() Цикл
					Если ВыборкаПриложение.ГруппаИмущества = ТретьяГруппаИмущества Тогда
						ОбластьМакета.Параметры.Заполнить(ВыборкаПриложение);
					КонецЕсли;	
				КонецЦикла;	
...
Функция ОтчетПоНалогуНаИмущество4Группы(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст = 
		...
		|		ВЫБОР
		|			КОГДА ГОД(ОсновноеСредство.ДатаВыпускаТранспортногоСредства) <> 1
		|				ТОГДА ГОД(ОсновноеСредство.ДатаВыпускаТранспортногоСредства)
		|			ИНАЧЕ 0
		...
		|		ВЫБОР
		|			КОГДА ВидТранспорта = &НесамоходныеТС ИЛИ ВидТранспорта = &ДругиеТС
		|				ТОГДА БалансоваяСтоимость
		|			ИНАЧЕ ОбъемДвигателя
		...
		|		ВЫБОР
		|			КОГДА ОтчетНалогНаИмуществоТранспорт.ВидТранспорта = &НесамоходныеТС ИЛИ ОтчетНалогНаИмуществоТранспорт.ВидТранспорта = &ДругиеТС
		|				ТОГДА ОтчетНалогНаИмуществоТранспорт.БалансоваяСтоимость
		|			ИНАЧЕ ОтчетНалогНаИмуществоТранспорт.ОбъемДвигателя
		|		КОНЕЦ КАК ОбъемДвигателя,
			...
			Если ВыборкаТранспорт.АБВ = "А" Тогда
				...
				Если НЕ Группа_А_Заполнена Тогда
					...
					Иначе
						...
						ОбластьМакета.Параметры.ГодВыпуска = Формат(ВыборкаТранспорт.ГодВыпуска, "ЧГ=0");
