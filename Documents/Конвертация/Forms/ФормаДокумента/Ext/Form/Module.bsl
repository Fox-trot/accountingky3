...
Процедура ПоказатьКР()	
	...
	СуммаПриходаВСомах = Объект.СуммаПрихода * Объект.КурсПрихода;
	СуммаРасходаВСомах = Объект.СуммаРасхода * Объект.КурсРасхода;
	...
	ИначеЕсли СчетУчетаВалютный(СчетПрихода) И СчетУчетаВалютный(СчетРасхода) И СуммаПриходаВСомах < СуммаРасходаВСомах Тогда
	...
	Если Текст3 = "" Тогда
		Текст4 = "";
	Иначе
		Сумма = СуммаРасходаВСомах - СуммаПриходаВСомах;
		Текст4 = " (" + СуммаРасходаВСомах + " - " + СуммаПриходаВСомах + " = " + Сумма + ")";
	КонецЕсли;
	
	Элементы.ВалютыКР.Заголовок = Текст1;
	
	Элементы.КурсоваяРазница.Заголовок = ?(Текст3 = "", "", "Операционная КР ") + Текст3 + Текст4;
	
	//Элементы.КурсоваяРазница.Заголовок = Текст2 + ?(Текст2 = "", "", ". ") + ?(Текст3 = "", "", "Операционная КР ") + Текст3 + Текст4;
