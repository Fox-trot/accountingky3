...
Процедура УслугиСчетРасходаПриИзменении(Элемент)
	...
	ПоляФормы = Новый Структура("Субконто1, Субконто2, Субконто3", "УслугиСубконто1", "УслугиСубконто2", "УслугиСубконто3");
	БухгалтерскийУчетКлиентСервер.ПриВыбореСчета(СтрокаТабличнойЧасти.СчетРасхода, ЭтаФорма, ПоляФормы, Неопределено, Истина);

	// Восстановление значений при совпадении типов субконто.
	ПоляОбъекта = Новый Структура("Субконто1, Субконто2, Субконто3", "Субконто1", "Субконто2", "Субконто3");
	БухгалтерскийУчетКлиентСервер.ПриИзмененииСчета(СтрокаТабличнойЧасти.СчетРасхода, СтрокаТабличнойЧасти, ПоляОбъекта, Истина);
	
	// Изменение параметров выбора полей субконто.
	ПараметрыДокумента = БухгалтерскийУчетКлиентСервер.СписокПараметровВыбораСубконто(ЭтаФорма, СтрокаТабличнойЧасти, "Субконто%Индекс%", "СчетРасхода");
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	Иначе // КР или не заполнен Контрагент	
		Элементы.БезналичныйРасчет.Видимость = ДанныеУчетнойПолитики.ПлательщикНСП И НЕ Объект.НДСНеПодтвержден;
		// Сумма
		...
		КонецЕсли;	
		// Ставки
		Если ДанныеУчетнойПолитики.ПлательщикНДС И НЕ ЗначениеЗаполнено(Объект.СерияБланкаСФ) Тогда
			Элементы.НДСНеПодтвержден.Видимость = Истина;
		Иначе
			Элементы.НДСНеПодтвержден.Видимость = Ложь;
		КонецЕсли;	
		Элементы.ЗначениеСтавкиНДС.Видимость = ДанныеУчетнойПолитики.ПлательщикНДС И НЕ Объект.НДСНеПодтвержден;
		// Счет-фактура
		...
		Элементы.ОСИмпортОсвобожденныйОтНДС.Видимость = Ложь;
		// Сумма НДС
		Элементы.ТоварыСуммаНДС.Видимость = ДанныеУчетнойПолитики.ПлательщикНДС И Объект.ЗначениеСтавкиНДС > 0 И НЕ Объект.НДСНеПодтвержден;
		Элементы.УслугиСуммаНДС.Видимость = ДанныеУчетнойПолитики.ПлательщикНДС И Объект.ЗначениеСтавкиНДС > 0 И НЕ Объект.НДСНеПодтвержден;
		Элементы.ОССуммаНДС.Видимость = ДанныеУчетнойПолитики.ПлательщикНДС И Объект.ЗначениеСтавкиНДС > 0 И НЕ Объект.НДСНеПодтвержден;
		// Сумма НСП
		Если Объект.БезналичныйРасчет Тогда
			Элементы.ЗначениеСтавкиНСП.Видимость = Ложь;
			Элементы.ТоварыСуммаНСП.Видимость = Ложь;
			...
			Элементы.ОССуммаНСП.Видимость = Ложь;
		Иначе 
			Элементы.ЗначениеСтавкиНСП.Видимость = ДанныеУчетнойПолитики.ПлательщикНСП И НЕ Объект.НДСНеПодтвержден;
			Элементы.ТоварыСуммаНСП.Видимость = ДанныеУчетнойПолитики.ПлательщикНСП И НЕ Объект.НДСНеПодтвержден;
			Элементы.УслугиСуммаНСП.Видимость = ДанныеУчетнойПолитики.ПлательщикНСП И НЕ Объект.НДСНеПодтвержден;
			Элементы.ОССуммаНСП.Видимость = ДанныеУчетнойПолитики.ПлательщикНСП И НЕ Объект.НДСНеПодтвержден;
		КонецЕсли;
		// Зачет НДС
		Элементы.ТоварыЗачетНДС.Видимость = ДанныеУчетнойПолитики.ПлательщикНДС И Объект.ЗначениеСтавкиНДС > 0 И НЕ Объект.НДСНеПодтвержден И ПризнакЗачетаНДСП;
		Элементы.УслугиЗачетНДС.Видимость = ДанныеУчетнойПолитики.ПлательщикНДС И Объект.ЗначениеСтавкиНДС > 0 И НЕ Объект.НДСНеПодтвержден И ПризнакЗачетаНДСП;
		Элементы.ОСЗачетНДС.Видимость = ДанныеУчетнойПолитики.ПлательщикНДС И Объект.ЗначениеСтавкиНДС > 0 И НЕ Объект.НДСНеПодтвержден И ПризнакЗачетаНДСП;
		// Подвал
		Если Объект.БезналичныйРасчет Тогда
			Элементы.ИтогСуммаНСП.Видимость = Ложь;
		Иначе
			Элементы.ИтогСуммаНСП.Видимость = ДанныеУчетнойПолитики.ПлательщикНСП И НЕ Объект.НДСНеПодтвержден;
		КонецЕсли;	
		Элементы.ИтогСуммаНДС.Видимость = ДанныеУчетнойПолитики.ПлательщикНДС И НЕ Объект.НДСНеПодтвержден;
	КонецЕсли;	
...
Процедура ОбработатьИзменениеДоговора(МодифицированДоговор = Ложь)
	...
	Если Объект.ПризнакСтраны = ПредопределенноеЗначение("Перечисление.ПризнакиСтраны.ИмпортЭкспорт") ИЛИ Объект.НДСНеПодтвержден Тогда		
		Объект.ЗначениеСтавкиНДС 	= 0;
		Объект.ЗначениеСтавкиНСП 	= 0;
	Иначе	
...
Процедура ОбработатьИзменениеНоменклатуры(СтрокаТабличнойЧасти, ИмяТабличнойЧасти)
	...
	// Нет смысла пересчитывать- так как никаие цифры при изменении номенклатуры не меняются
	//СтранаВходитВЕАЭС = Объект.ПризнакСтраны = ПредопределенноеЗначение("Перечисление.ПризнакиСтраны.ЕАЭС");
