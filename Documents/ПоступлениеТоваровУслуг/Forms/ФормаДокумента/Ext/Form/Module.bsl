...
Процедура ВидОперацииПриИзменении(Элемент)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПоступлениеТоваровУслуг.ЗакупочныйАкт") Тогда
		Объект.Товары.Очистить();
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	ЭтоЗакупочныйАкт = Объект.ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ЗакупочныйАкт;
	ЭтоКонтрагентСЭЗ = Объект.Контрагент.СЭЗ;
	
	Если ЭтоЗакупочныйАкт Тогда 
	...
	Иначе // КР или не заполнен Контрагент	
		Элементы.СтраницаСопроводительнаяНакладная.Видимость = Ложь;
		Если Не ЭтоЗакупочныйАкт Тогда 
			...
			Элементы.ЗначениеСтавкиНСП.Видимость = ДанныеУчетнойПолитики.ПлательщикНСП И НЕ Объект.БезналичныйРасчет;
		...
		//	Элементы.УслугиСуммаНСП.Видимость = Ложь;
		//	Элементы.ОССуммаНСП.Видимость = Ложь;
		//ИначеЕсли НЕ ЭтоЗакупочныйАкт Тогда 
	...
	Если НЕ Элементы.ЗначениеСтавкиНСП.Видимость
		И Объект.ЗначениеСтавкиНСП <> 0 Тогда
	...
	// Видимость сумм НСП зависит от значения НСП.
	Если Объект.ЗначениеСтавкиНСП = 0 Тогда 
		Элементы.ТоварыСуммаНСП.Видимость = Ложь;
		Элементы.УслугиСуммаНСП.Видимость = Ложь;
	Иначе 
		Элементы.ТоварыСуммаНСП.Видимость = Истина;
	...
	Если Объект.ЗначениеСтавкиНСП = 0
		И Объект.ЗначениеСтавкиНСПДляОС = 0 Тогда 
...
Процедура ОбработатьИзменениеДоговора(МодифицированДоговор = Ложь)
	...
	// Переопределение для ЕАЭС.
	Если ПараметрыРасчета.ПризнакСтраныЕАЭС Тогда 
...
Процедура ПолучитьТоварыИзХранилища(АдресЗапасовВХранилище)
	...
	Для Каждого СтрокаЗагрузки Из ТаблицаДляЗагрузки Цикл
		...
		// Расчет налогов будет произведен для всей табличной части
...
Процедура УстановитьМаскуНомераБланкаСФ()
	...
	ИначеЕсли ПризнакСтраны = ПредопределенноеЗначение("Перечисление.ПризнакиСтраны.КР") Тогда
		Элементы.НомерБланкаСФ.Маска = "@@@@@@";
...
Функция ПодготовитьПараметрыРасчета(ДляОС = Ложь)
	...
	ПараметрыРасчета.Вставить("ЗначениеСтавкиНСП", 			?(ДляОС, Объект.ЗначениеСтавкиНСПДляОС, Объект.ЗначениеСтавкиНСП));
