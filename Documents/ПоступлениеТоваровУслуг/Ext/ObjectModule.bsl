...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если ВидОперации = Перечисления.ВидыОперацийПоступлениеТоваровУслуг.ЗакупочныйАкт
		И НЕ ЗначениеЗаполнено(НомерБланкаСФ) Тогда
		
		ТекстСообщения = НСтр("ru = 'В поступлении с видом операции ""Закупочный акт"" необходимо заполнить поле ""Номер бланка СФ"".'");
		ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,, "Объект.НомерБланкаСФ",, Отказ);
	КонецЕсли;	
