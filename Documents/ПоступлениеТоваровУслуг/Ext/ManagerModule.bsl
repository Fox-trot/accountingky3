...
Процедура СформироватьТаблицаХозрасчетный(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|		ИНАЧЕ
		|			ВременнаяТаблицаТовары.Сумма + ВременнаяТаблицаТовары.СуммаНСП
	...
	Запрос.Текст =
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаШапка.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ЕАЭС)
		|			ТОГДА ВременнаяТаблицаОС.Сумма - ВременнаяТаблицаОС.СуммаАкциза
...
Процедура СформироватьТаблицаСведенияОПоступлении(ДокументСсылка, СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		|				И ВременнаяТаблицаШапка.НДСНеПодтвержден = ЛОЖЬ
		|				И ВременнаяТаблицаШапка.НеВключатьВРеестрСФ = ЛОЖЬ
		...
		|		КОГДА ВременнаяТаблицаШапка.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ИмпортЭкспорт)
		|			ТОГДА ЛОЖЬ
		|		ИНАЧЕ ВременнаяТаблицаШапка.НДСНеПодтвержден
		|	КОНЕЦ КАК НДСНеПодтвержден
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаСтроки.Период КАК Период,
...
// Таблицы значений сохраняет в свойствах структуры "ДополнительныеСвойства".
//
Процедура СформироватьТаблицаЗначенияСтавокНалоговНДСиНСП(ДокументСсылка, СтруктураДополнительныеСвойства) 
	
	Запрос = Новый Запрос;
	Запрос.МенеджерВременныхТаблиц = СтруктураДополнительныеСвойства.ДляПроведения.СтруктураВременныеТаблицы.МенеджерВременныхТаблиц;
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ЗначенияСтавокНалоговНДСиНСПСрезПоследних.Контрагент КАК Контрагент
		|ПОМЕСТИТЬ ВременнаяТаблицаЗначенияСтавок
		|ИЗ
		|	РегистрСведений.ЗначенияСтавокНалоговНДСиНСП.СрезПоследних(
		|			&Период,
		|			Период = НАЧАЛОПЕРИОДА(&Период, МЕСЯЦ)
		|				И (Контрагент, ДоговорКонтрагента) В
		|					(ВЫБРАТЬ
		|						ВременнаяТаблицаШапка.Контрагент,
		|						ВременнаяТаблицаШапка.ДоговорКонтрагента
		|					ИЗ
		|						ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка)) КАК ЗначенияСтавокНалоговНДСиНСПСрезПоследних
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|ИЗ
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаЗначенияСтавок КАК ВременнаяТаблицаЗначенияСтавок
		|		ПО (ИСТИНА)
		|ГДЕ
		|	ВременнаяТаблицаЗначенияСтавок.Контрагент ЕСТЬ NULL";
	Запрос.УстановитьПараметр("Период", СтруктураДополнительныеСвойства.ДляПроведения.Дата);	
	РезультатЗапроса = Запрос.Выполнить();
	
	СтруктураДополнительныеСвойства.ТаблицыДляДвижений.Вставить("ТаблицаЗначенияСтавокНалоговНДСиНСП", РезультатЗапроса.Выгрузить());
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.Текст =
		...
		|	ТаблицаДокумента.НДСНеПодтвержден,
		|	ТаблицаДокумента.НеВключатьВРеестрСФ,
		|	&ПлательщикНДС КАК ПлательщикНДС,
		|	ТаблицаДокумента.ЗначениеСтавкиНДС,
		|	ТаблицаДокумента.ЗначениеСтавкиНСП,
	...
	СформироватьТаблицаЗначенияСтавокНалоговНДСиНСП(ДокументСсылка, СтруктураДополнительныеСвойства);
