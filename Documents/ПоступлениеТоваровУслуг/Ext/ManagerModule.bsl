...
Процедура СформироватьТаблицаХозрасчетный(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|					ИНАЧЕ ВЫРАЗИТЬ((ВременнаяТаблицаТовары.Сумма + ВременнаяТаблицаТовары.СуммаНДС + ВременнаяТаблицаТовары.СуммаНСП) * ВременнаяТаблицаШапка.Курс / ВременнаяТаблицаШапка.Кратность КАК ЧИСЛО(15, 2))
		|				КОНЕЦ
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ВременнаяТаблицаШапка.ПризнакСтраны = ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ЕАЭС)
		...
		|	ИЛИ ВременнаяТаблицаШапка.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийПоступлениеТоваровУслуг.ЗакупочныйАкт))
	...
	// НДС товары (Признак страны ЕАЭС и НЕ плательщик НДС).
	ТекстЗапроса = ТекстЗапроса +
		"ВЫБРАТЬ
		|	4 КАК Порядок,
		|	ВременнаяТаблицаШапка.Дата КАК Период,
...
Процедура СформироватьТаблицаРеестрВвезенных(ДокументСсылка, СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		|	ВложенныйЗапрос.БазаНДС КАК БазаНДС,
		...
		|		ЕСТЬNULL(СУММА(ВременнаяТаблицаУслуги.БазаНДС), 0) КАК БазаНДС,
		|		ЕСТЬNULL(СУММА(ВременнаяТаблицаУслуги.СуммаАкциза), 0) КАК СуммаАкциза
		|	ИЗ
		|		ВременнаяТаблицаУслуги КАК ВременнаяТаблицаУслуги
		|
		|	СГРУППИРОВАТЬ ПО
		...
		|		ЕСТЬNULL(СУММА(ВременнаяТаблицаОС.БазаНДС), 0) КАК БазаНДС,
		|		ЕСТЬNULL(СУММА(ВременнаяТаблицаОС.СуммаАкциза), 0) КАК СуммаАкциза
		|	ИЗ
		|		ВременнаяТаблицаОС КАК ВременнаяТаблицаОС
		|
		|	СГРУППИРОВАТЬ ПО
...
Процедура СформироватьТаблицаЗакупочныхАктов(ДокументСсылка, СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		|	ИЛИ ВременнаяТаблицаТовары.Сумма <> 0)";
	РезультатЗапроса = Запрос.Выполнить();
