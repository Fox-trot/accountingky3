...
Процедура СформироватьТаблицаХозрасчетный(ДокументСсылка, СтруктураДополнительныеСвойства, Отказ)	
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаПродукция.Сумма КАК Сумма,
		|	ВременнаяТаблицаПродукция.Количество КАК Количество,
		|	ВременнаяТаблицаПродукция.СчетОприходования КАК КорСчет,
		|	ВременнаяТаблицаПродукция.СубконтоОприходования1 КАК КорСубконто1,
		|	ВременнаяТаблицаПродукция.СубконтоОприходования2 КАК КорСубконто2,
		|	ВременнаяТаблицаПродукция.СубконтоОприходования3 КАК КорСубконто3,
		|	&СодержаниеПродукция КАК Содержание
		|ИЗ
		|	ВременнаяТаблицаПродукция КАК ВременнаяТаблицаПродукция
		|ГДЕ
		|	НЕ ВременнаяТаблицаПродукция.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.МБП)
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки
		|;
		...
		|	ВременнаяТаблицаПродукция КАК ВременнаяТаблицаПродукция
		|ГДЕ
		|	ВременнаяТаблицаПродукция.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Хозрасчетный.МБП)
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
	Запрос.УстановитьПараметр("СодержаниеПродукция", НСтр("ru = 'Оприходование товаров'")); 
