...
Функция ПечатнаяФормаИнвентаризационнаяОписьИНВ19(МассивОбъектов, ОбъектыПечати)
	...
	ТекстЗапроса = 
		"ВЫБРАТЬ
		|	ИнвентаризацияМБП.Ссылка КАК Ссылка,
		|	ИнвентаризацияМБП.Номер КАК НомерДокумента,
		|	ИнвентаризацияМБП.Дата КАК ДатаДокумента,
		|	ИнвентаризацияМБП.Организация КАК Организация,
		|	"""" КАК Подразделение,
		|	ИнвентаризацияМБП.ДатаНачала КАК ДатаНачалаИнвентаризации,
		|	ИнвентаризацияМБП.ДатаОкончания КАК ДатаОкончанияИнвентаризации,
		|	ИнвентаризацияМБП.НаОсновании КАК ДокументОснованиеВид,
		|	ИнвентаризацияМБП.ДатаОснования КАК ДокументОснованиеДата,
		|	ИнвентаризацияМБП.НомерОснования КАК ДокументОснованиеНомер,
		|	ИнвентаризацияМБП.МБП.(
		...
		|		Цена КАК Цена,
		|		ВЫРАЗИТЬ(ВЫБОР
		|				КОГДА ИнвентаризацияМБП.МБП.Отклонение > 0
		|					ТОГДА ИнвентаризацияМБП.МБП.Отклонение
		|				ИНАЧЕ 0
		|			КОНЕЦ КАК ЧИСЛО) КАК РезультатИзлишекКол,
		|		ВЫРАЗИТЬ(ВЫБОР
		|				КОГДА ИнвентаризацияМБП.МБП.Отклонение < 0
		|					ТОГДА -1 * ИнвентаризацияМБП.МБП.Отклонение
		|				ИНАЧЕ 0
		|			КОНЕЦ КАК ЧИСЛО) КАК РезультатНедостачаКол,
		|		ВЫРАЗИТЬ(ВЫБОР
		|				КОГДА ИнвентаризацияМБП.МБП.УчетнаяСумма - ИнвентаризацияМБП.МБП.Сумма > 0
		|					ТОГДА ИнвентаризацияМБП.МБП.УчетнаяСумма - ИнвентаризацияМБП.МБП.Сумма
		|				ИНАЧЕ 0
		|			КОНЕЦ КАК ЧИСЛО) КАК РезультатИзлишекСумма,
		|		ВЫРАЗИТЬ(ВЫБОР
		|				КОГДА ИнвентаризацияМБП.МБП.УчетнаяСумма - ИнвентаризацияМБП.МБП.Сумма < 0
		|					ТОГДА -1 * (ИнвентаризацияМБП.МБП.УчетнаяСумма - ИнвентаризацияМБП.МБП.Сумма)
		|				ИНАЧЕ 0
		|			КОНЕЦ КАК ЧИСЛО) КАК РезультатНедостачаСумма
		|	) КАК МБП,
		|	ИнвентаризацияМБП.ФизЛицо КАК ФизЛицо
		|ИЗ
		|	Документ.ИнвентаризацияМБП КАК ИнвентаризацияМБП
		|ГДЕ
		|	ИнвентаризацияМБП.Ссылка В(&СписокДокументов)
		|
		|УПОРЯДОЧИТЬ ПО
	...
	ТабличныйДокумент.КлючПараметровПечати = "ИнвентаризацияМБП_ИнвентаризационнаяОписьИНВ19";
	
	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ИнвентаризацияМБП.ПФ_MXL_ИнвентаризационнаяОписьИНВ19");		
	...
	Пока ВыборкаЗаголовок.Следующий()  Цикл
		...
		ОтветственныеЛица = БухгалтерскийУчетСервер.ОтветственныеЛицаОрганизаций(ВыборкаЗаголовок.Организация, ВыборкаЗаголовок.ДатаДокумента);
...
//  Табличный документ - печатная форма
//
Функция ПечатьИнвентаризацияМБП_ИНВ3(МассивОбъектов, ОбъектыПечати)
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати = "ИнвентаризацияМБП_ИнвентаризацияТоваров_ИНВ3";
	...
	|	ИнвентаризацияМБП.Дата КАК ДатаДокумента,
	|	ИнвентаризацияМБП.Организация КАК Организация,
	|	ИнвентаризацияМБП.Организация.НаименованиеПолное КАК ОрганизацияНаименованиеПолное,
	|	ИнвентаризацияМБП.Организация.КодПоОКПО КАК КодПоОКПО,
	|	"""" КАК Склад,
	|	ИнвентаризацияМБП.ДатаНачала КАК ДатаНачалаИнвентаризации,
	|	ИнвентаризацияМБП.ДатаОкончания КАК ДатаОкончанияИнвентаризации,
	|	ИнвентаризацияМБП.НаОсновании КАК ДокументОснованиеВид,
	...
	Пока ВыборкаДок.Следующий() Цикл
		...
		ОбщееКоличествоСтрок 	= ВременнаяТаблицаМБП.Количество();
		ОбщееКоличествоФакт 	= ВременнаяТаблицаМБП.Итог("ФактКоличество");
		ОбщееСуммаФакт 			= ВременнаяТаблицаМБП.Итог("ФактСумма");
		ОбщееБухКоличество 		= ВременнаяТаблицаМБП.Итог("БухКоличество");
		ОбщееБухСумма 			= ВременнаяТаблицаМБП.Итог("БухСумма");
		...
		Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ИнвентаризацияМБП.ПФ_MXL_ИнвентаризацияТоваров_ИНВ3");
		...
		КоличествоСтрок = ВыборкаСтрокМБП.Количество();
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	Если УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ИнвентаризацияТоваров_ИНВ3") Тогда
		...
		ПечатьИнвентаризацияМБП_ИНВ3(МассивОбъектов, ОбъектыПечати),
		,
		"Документ.ИнвентаризацияМБП.ПФ_MXL_ИнвентаризацияТоваров_ИНВ3");
	КонецЕсли;
	...
	Если НужноПечататьМакет Тогда
		...
		,
		"Документ.ИнвентаризацияМБП.ПФ_MXL_ИнвентаризационнаяОписьИНВ19");
	КонецЕсли; 
