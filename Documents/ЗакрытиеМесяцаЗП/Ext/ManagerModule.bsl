...
Процедура СформироватьТаблицуДляОтчетаПоПН(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст = 
		...
		|	ВременнаяТаблицаНалогиПоЗарплате.ПН - ВременнаяТаблицаНалогиПоЗарплате.ДоплатаПН КАК ПН,
		|	ВременнаяТаблицаНалогиПоЗарплате.ДоплатаПН КАК ДоплатаПН,
		//|	ВременнаяТаблицаНалогиПоЗарплате.ПН + ВременнаяТаблицаНалогиПоЗарплате.ДоплатаПН КАК ВсегоПН,
		|	ВременнаяТаблицаНалогиПоЗарплате.СуммаНачислено КАК ВсегоНачислено,
		//|	ВременнаяТаблицаНалогиПоЗарплате.СуммаНачислено - ВременнаяТаблицаНалогиПоЗарплате.ОДПН КАК Необлагаемая,
		|	1 КАК Количество,
		|	ВременнаяТаблицаНалогиПоЗарплате.ОтработаноДней КАК ОтработаноДней,
		...
		|		КОГДА ВременнаяТаблицаНалогиПоЗарплате.Статус.Категория = ЗНАЧЕНИЕ(Справочник.КатегорииСотрудников.Арендодатели)
		|			ТОГДА 4
		|		КОГДА ВременнаяТаблицаНалогиПоЗарплате.Статус.Категория = ЗНАЧЕНИЕ(Справочник.КатегорииСотрудников.ИностранныеГраждане)
		|				ИЛИ ВременнаяТаблицаНалогиПоЗарплате.Статус.Категория = ЗНАЧЕНИЕ(Справочник.КатегорииСотрудников.ТрудовыеМигранты)
		|			ТОГДА 3
		|		КОГДА ЕСТЬNULL(ВременнаяТаблицаНалогиПоЗарплате.ПрименятьВычет, ИСТИНА)
		|			ТОГДА 1
		|		ИНАЧЕ 2
		|	КОНЕЦ КАК НомерСтроки,
		|	ВременнаяТаблицаНалогиПоЗарплате.Вычеты * 100 КАК Вычеты,
		|	ВременнаяТаблицаНалогиПоЗарплате.ПФР КАК ПФР,
		|	ВременнаяТаблицаНалогиПоЗарплате.ГНПФР КАК ГНПФР
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаСборкаПН.Период КАК Период,
		|	ВременнаяТаблицаСборкаПН.Организация КАК Организация,
		|	ВременнаяТаблицаСборкаПН.Подразделение КАК Подразделение,
		|	СтрокиОтчетаПН.Ссылка КАК СтрокаОтчета,
		|	ВременнаяТаблицаСборкаПН.Количество КАК Количество,
		|	ВременнаяТаблицаСборкаПН.ВсегоНачислено КАК ВсегоНачислено,
		//|	ВременнаяТаблицаСборкаПН.Необлагаемая КАК Необлагаемая,
		|	ВременнаяТаблицаСборкаПН.Вычеты КАК Вычеты,
		|	ВременнаяТаблицаСборкаПН.ОДПН КАК ОДПН,
		|	ВременнаяТаблицаСборкаПН.ПН КАК ПНСотрудника,
		|	ВременнаяТаблицаСборкаПН.ДоплатаПН КАК ДоплатаПН,
		// |	ВременнаяТаблицаСборкаПН.ВсегоПН КАК ВсегоПН,
		|	ВременнаяТаблицаСборкаПН.ФизЛицо КАК ФизЛицо,
		...
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник.СтрокиОтчетаПН КАК СтрокиОтчетаПН
		|		ПО ВременнаяТаблицаСборкаПН.НомерСтроки = СтрокиОтчетаПН.НомерСтроки
		|ГДЕ
		|	ВременнаяТаблицаСборкаПН.ВсегоНачислено > 0";
	РезультатЗапроса = Запрос.Выполнить();
