...
Процедура СформироватьТаблицуДляОтчетаПоЕжемесячномуОтчетуПоСФ(ДокументСсылка, СтруктураДополнительныеСвойства);
	...
	Запрос.Текст = 
		...
		|	ВременнаяТаблицаНалогиПоЗарплате.ПФР + ВременнаяТаблицаНалогиПоЗарплате.ПФФ + ВременнаяТаблицаНалогиПоЗарплате.МСФ + ВременнаяТаблицаНалогиПоЗарплате.ФОТФ + ВременнаяТаблицаНалогиПоЗарплате.МСР КАК НачисленыеВзносы,
...
Процедура СформироватьТаблицуДляОтчетаПоКвартальномуОтчетуПоСФ(ДокументСсылка, СтруктураДополнительныеСвойства);
	...
	Запрос.Текст = 
		...
		|	ВременнаяТаблицаСотрудники.Подразделение КАК Подразделение,
		|	ВременнаяТаблицаНалогиПоЗарплате.ФизЛицо КАК ФизЛицо,
		|	ВременнаяТаблицаНалогиПоЗарплате.Физлицо.Пол КАК ФизлицоПол,
		|	ВременнаяТаблицаНалогиПоЗарплате.Физлицо.ДатаРождения КАК ФизлицоДатаРождения,
		|	ВременнаяТаблицаНалогиПоЗарплате.Статус.Категория КАК Категория,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаНалогиПоЗарплате.Статус.Категория = ЗНАЧЕНИЕ(Справочник.КатегорииСотрудников.Арендодатель)
		|			ТОГДА &СреднемесячнаяЗПСФ
		...
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ДопФОТ
		//|	ВременнаяТаблицаНалогиПоЗарплате.Статус.ПоказательВОтчетеПоКвартальномуСоцфонду КАК ПоказательВОтчетеПоКвартальномуСоцфонду
		...
		|		ПО (ИСТИНА)
		|ГДЕ
		|	ВременнаяТаблицаШапка.Дата = &КонецКвартала
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		|	&Период,
		|	ДанныеДляОтчетаПоСФЕжемесячномуОбороты.Организация,
		|	ДанныеДляОтчетаПоСФЕжемесячномуОбороты.Подразделение,
		|	ДанныеДляОтчетаПоСФЕжемесячномуОбороты.Физлицо,
		|	ДанныеДляОтчетаПоСФЕжемесячномуОбороты.Физлицо.Пол КАК ФизлицоПол,
		|	ДанныеДляОтчетаПоСФЕжемесячномуОбороты.Физлицо.ДатаРождения КАК ФизлицоДатаРождения,
		...
		|	ДанныеДляОтчетаПоСФЕжемесячномуОбороты.ГНПФРОборот,
		|	ДанныеДляОтчетаПоСФЕжемесячномуОбороты.НачисленыеВзносыОборот,
		|	ДанныеДляОтчетаПоСФЕжемесячномуОбороты.ФактДнейОборот,
		|	ДанныеДляОтчетаПоСФЕжемесячномуОбороты.НормаДнейОборот,
		...
		|				И &Период = &КонецКвартала) КАК ДанныеДляОтчетаПоСФЕжемесячномуОбороты
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		|	ВременнаяТаблицаСоцфонд.ФизлицоПол,
		|	ВременнаяТаблицаСоцфонд.ФизлицоДатаРождения,
		|	ВременнаяТаблицаСоцфонд.Категория,
		|	СУММА(ВременнаяТаблицаСоцфонд.ПриведеннаяСЗП) КАК ПриведеннаяСЗП,
		|	СУММА(ВременнаяТаблицаСоцфонд.ФОТ) КАК ФОТ,
		|	СУММА(ВременнаяТаблицаСоцфонд.ГНПФР) КАК ГНПФР,
		|	СУММА(ВременнаяТаблицаСоцфонд.НачисленыеВзносы) КАК НачисленыеВзносы,
		|	СУММА(ВременнаяТаблицаСоцфонд.ФактДней) КАК ФактДней,
		|	СУММА(ВременнаяТаблицаСоцфонд.НормаДней) КАК НормаДней,
		|	СУММА(ВременнаяТаблицаСоцфонд.ДопФОТ) КАК ДопФОТ
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаСоцфонд.ПриведеннаяСЗП > ВременнаяТаблицаСоцфонд.ФОТ
		|			ТОГДА ВременнаяТаблицаСоцфонд.ФОТ
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаСоцфонд.ПриведеннаяСЗП <= ВременнаяТаблицаСоцфонд.ФОТ
		...
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаСоцфонд.ПриведеннаяСЗП > ВременнаяТаблицаСоцфонд.ФОТ
		|			ТОГДА ВременнаяТаблицаСоцфонд.ДопФОТ
		|		ИНАЧЕ 0
		|	КОНЕЦ,
		|	0
		|ИЗ
		|	ВременнаяТаблицаСоцфонд КАК ВременнаяТаблицаСоцфонд
		|ГДЕ
		|	НЕ ВременнаяТаблицаСоцфонд.ДопФОТ = 0
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ВременнаяТаблицаСборка.Период,
		|	ВременнаяТаблицаСборка.Организация,
		|	ВременнаяТаблицаСборка.Подразделение,
		|	СтрокиОтчетаПоСФКвартальному.Ссылка КАК Показатель,
		|	ВременнаяТаблицаСборка.ФизЛицо,
		|	ВременнаяТаблицаСборка.ЗначениеМеньшеСЗП,
		|	ВременнаяТаблицаСборка.ЗначениеБольшеСЗП
		|ИЗ
	...
	Запрос.УстановитьПараметр("НачалоПериода", НачалоКвартала(СтруктураДополнительныеСвойства.ДляПроведения.Дата));
	Запрос.УстановитьПараметр("КонецПериода", КонецМесяца(ДобавитьМесяц(НачалоКвартала(СтруктураДополнительныеСвойства.ДляПроведения.дата),1)));
	Запрос.УстановитьПараметр("Период", КонецМесяца(СтруктураДополнительныеСвойства.ДляПроведения.Дата));
	Запрос.УстановитьПараметр("КонецКвартала", КонецКвартала(СтруктураДополнительныеСвойства.ДляПроведения.Дата));
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, СтруктураДополнительныеСвойства, ДокументОбъект) Экспорт
	...
	Запрос.Текст =
		...
		|	ТаблицаДокумента.ПрименятьВычет,
		|	ТаблицаДокумента.СуммаНачислено,
		|	ТаблицаДокумента.ПФР,
		|	ТаблицаДокумента.ГНПФР,
		|	ТаблицаДокумента.ПН,
		|	ТаблицаДокумента.ПрофВзнос,
		|	ТаблицаДокумента.ПФФ,
		|	ТаблицаДокумента.МСФ,
		|	ТаблицаДокумента.ФОТФ,
		|	ТаблицаДокумента.МСР,
		|	ТаблицаДокумента.ФОТР,
		|	ТаблицаДокумента.Вычеты,
		|	ТаблицаДокумента.ОДСФ,
		|	ТаблицаДокумента.ОДПН,
		|	ТаблицаДокумента.ОДПрофВзнос,
		|	ТаблицаДокумента.СуммаДопДоход,
		|	ТаблицаДокумента.ДоплатаПН,
		|	ТаблицаДокумента.МРД,
