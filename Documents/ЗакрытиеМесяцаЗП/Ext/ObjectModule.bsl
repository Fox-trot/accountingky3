...
Процедура РассчитатьНалогиПоЗарплате()
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК СтатусАрендодатель,
		|	ВЫБОР
		|		КОГДА СтатусыСотрудниковСрезПоследних.Статус.Категория = ЗНАЧЕНИЕ(Справочник.КатегорииСотрудников.МОП) 
		|			ИЛИ СтатусыСотрудниковСрезПоследних.Статус.Категория = ЗНАЧЕНИЕ(Справочник.КатегорииСотрудников.РаботающиеПенсионерыМОП)
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК СтатусМОП		
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|	И НЕ ВременнаяТаблицаСтатусы.СтатусАрендодатель
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
		|ВЫБРАТЬ
		...
		|ГДЕ
		|	ВременнаяТаблицаНачисленияЗаМесяц.ОДСФ > 0
		|	И ВременнаяТаблицаСтатусы.СтатусАрендодатель
		|
		|ИНДЕКСИРОВАТЬ ПО
		|	ФизЛицо";
	ТекстЗапроса = ТекстЗапроса + ОбщегоНазначенияБПВызовСервера.ТекстРазделителяЗапросовПакета();
	...
	Если ДанныеУчетнойПолитикиПоПерсоналу.РасчетПНБезНакопления Тогда
		ТекстЗапроса = ТекстЗапроса +
			...
			|	ВЫРАЗИТЬ(ВЫБОР
			|		КОГДА ЕСТЬNULL(ВременнаяТаблицаСтатусы.СтавкаПН, 0) = 0 ИЛИ ЕСТЬNULL(ВременнаяТаблицаСтатусы.СтатусМОП, ЛОЖЬ)
			|			ТОГДА 0			
			|		ИНАЧЕ &МРД  * ВременнаяТаблицаСтатусы.СтавкаПН / 100
	...
	Иначе	
		ТекстЗапроса = ТекстЗапроса +
			...
			|			ИНАЧЕ (ВременнаяТаблицаНачисленияЗаМесяц.ОДПН + ЕСТЬNULL(ПодоходныйНалогОбороты.ОДПНОборот, 0) 
			|						- ЕСТЬNULL(ВременнаяТаблицаНалогиСФ.ПФРДляВычетаПН, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ПФРДляРасчетаПНОборот, 0) 
			|						- ЕСТЬNULL(ВременнаяТаблицаНалогиСФ.ГНПФРДляВычетаПН, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ГНПФРДляРасчетаПНОборот, 0) 
			|						- (ВременнаяТаблицаСтатусы.Вычеты * 100 + (ЕСТЬNULL(ПодоходныйНалогОбороты.ПиИВычетыОборот, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ПФРОборот, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ГНПФРОборот, 0)))) 
			|						* ВременнаяТаблицаСтатусы.СтавкаПН / 100 - ЕСТЬNULL(ПодоходныйНалогОбороты.ПНОборот, 0)
			|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК ПН,
			|	ВременнаяТаблицаСтатусы.Вычеты КАК Вычеты,			
			|	ВЫРАЗИТЬ(ВЫБОР
			|		КОГДА ЕСТЬNULL(ВременнаяТаблицаСтатусы.СтавкаПН, 0) = 0 ИЛИ ЕСТЬNULL(ВременнаяТаблицаСтатусы.СтатусМОП, ЛОЖЬ)
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|	КОНЕЦ КАК ПН,
		|	ВЫБОР
		|		КОГДА ВременнаяТаблицаНалогиПН.ПНотМРД > ВременнаяТаблицаНалогиПН.ПН
		|			ТОГДА ВременнаяТаблицаНалогиПН.ПНотМРД - ВременнаяТаблицаНалогиПН.ПН
		|		ИНАЧЕ 0
		|	КОНЕЦ КАК ДоплатаПН,
