...
Процедура РассчитатьНалогиПоЗарплате()
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|	&СреднемесячнаяЗПСФ КАК СреднемесячнаяЗПСФ,
		|	ВЫБОР
		|		КОГДА ЕСТЬNULL(ВременнаяТаблицаНормыДней.НормаДней, 0) = 0
		|			ТОГДА &СреднемесячнаяЗПСФ / 100 * &ПроцентОтСЗПСФ
		|		ИНАЧЕ ВЫБОР
		|				КОГДА ВременнаяТаблицаНачисленияЗаМесяц.ОтработаноДней > ЕСТЬNULL(ВременнаяТаблицаНормыДней.НормаДней, 0)
		|					ТОГДА &СреднемесячнаяЗПСФ / 100 * &ПроцентОтСЗПСФ
		|				ИНАЧЕ ВЫРАЗИТЬ(&СреднемесячнаяЗПСФ / 100 * &ПроцентОтСЗПСФ * ВременнаяТаблицаНачисленияЗаМесяц.ОтработаноДней / ЕСТЬNULL(ВременнаяТаблицаНормыДней.НормаДней, 0) КАК ЧИСЛО(15, 2))
		|			КОНЕЦ
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|	&СреднемесячнаяЗПСФ * ВременнаяТаблицаСотрудники.ЗанимаемыхСтавок,
		|	ВЫРАЗИТЬ(ВЫБОР
		|			КОГДА ВременнаяТаблицаСтатусы.СтавкаПФР = 0
		|				ТОГДА 0
		|			ИНАЧЕ &СреднемесячнаяЗПСФ * ВременнаяТаблицаСтатусы.СтавкаПФР / 100
		|		КОНЕЦ КАК ЧИСЛО(15, 2)),
		|	ВЫРАЗИТЬ(ВЫБОР
		|			КОГДА ВременнаяТаблицаСтатусы.СтавкаГНПФР = 0
		|				ТОГДА 0
		|			ИНАЧЕ &СреднемесячнаяЗПСФ * ВременнаяТаблицаСтатусы.СтавкаГНПФР / 100
		|		КОНЕЦ КАК ЧИСЛО(15, 2)),
		|	ВЫРАЗИТЬ(ВЫБОР
		|			КОГДА ВременнаяТаблицаСтатусы.СтавкаПФР = 0
		|					ИЛИ ВременнаяТаблицаСтатусы.НеВычитатьПФ
		...
		|			КОГДА ВременнаяТаблицаСтатусы.СтавкаМСР = 0
		|				ТОГДА 0
		|			ИНАЧЕ &СреднемесячнаяЗПСФ * ВременнаяТаблицаСтатусы.СтавкаМСР / 100
		|		КОНЕЦ КАК ЧИСЛО(15, 2)),
		|	ВЫРАЗИТЬ(ВЫБОР
		|			КОГДА ВременнаяТаблицаСтатусы.СтавкаПФФ = 0
		|				ТОГДА 0
		|			ИНАЧЕ &СреднемесячнаяЗПСФ * ВременнаяТаблицаСтатусы.СтавкаПФФ / 100
		|		КОНЕЦ КАК ЧИСЛО(15, 2)),
		|	ВЫРАЗИТЬ(ВЫБОР
		|			КОГДА ВременнаяТаблицаСтатусы.СтавкаМСФ = 0
		|				ТОГДА 0
		|			ИНАЧЕ &СреднемесячнаяЗПСФ * ВременнаяТаблицаСтатусы.СтавкаМСФ / 100
		|		КОНЕЦ КАК ЧИСЛО(15, 2)),
		|	ВЫРАЗИТЬ(ВЫБОР
		|			КОГДА ВременнаяТаблицаСтатусы.СтавкаФОТФ = 0
		|				ТОГДА 0
		|			ИНАЧЕ &СреднемесячнаяЗПСФ * ВременнаяТаблицаСтатусы.СтавкаФОТФ / 100
		|		КОНЕЦ КАК ЧИСЛО(15, 2))
		|ИЗ
		|	ВременнаяТаблицаНачисленияЗаМесяц КАК ВременнаяТаблицаНачисленияЗаМесяц
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаСтатусы КАК ВременнаяТаблицаСтатусы
