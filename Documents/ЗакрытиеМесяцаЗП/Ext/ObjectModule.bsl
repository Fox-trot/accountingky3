...
Процедура РассчитатьНалогиПоЗарплате()
	...
	Если ДанныеУчетнойПолитикиПоПерсоналу.РасчетПНБезНакопления Тогда
		ТекстЗапроса = ТекстЗапроса +
			...
			|				ТОГДА 0
			|			КОГДА &ПлательщикЕН
			|					И НЕ (ВременнаяТаблицаСтатусы.Категория В (&МассивКатегорийБезПримененияМРД)
			|						ИЛИ ВременнаяТаблицаСтатусы.ВидЗанятости В ИЕРАРХИИ (&МассивВидовЗанятостиБезПримененияМРД))
			|				ТОГДА ВЫБОР
			...
			|	ВЫРАЗИТЬ(ВЫБОР
			|			КОГДА ЕСТЬNULL(ВременнаяТаблицаСтатусы.СтавкаПН, 0) = 0
			|					ИЛИ ВременнаяТаблицаСтатусы.Категория = ЗНАЧЕНИЕ(Справочник.КатегорииСотрудников.МОП)
			|					ИЛИ ВременнаяТаблицаСтатусы.Категория = ЗНАЧЕНИЕ(Справочник.КатегорииСотрудников.РаботающиеПенсионерыМОП)
			|				ТОГДА 0
	...
	МассивКатегорийБезПримененияМРД = Новый Массив;
	МассивКатегорийБезПримененияМРД.Добавить(Справочники.КатегорииСотрудников.Сотрудник); // 100
	МассивКатегорийБезПримененияМРД.Добавить(Справочники.КатегорииСотрудников.ИнвалидВОВ1_2гр); // 101
	МассивКатегорийБезПримененияМРД.Добавить(Справочники.КатегорииСотрудников.ИнвалидВОВ3гр); // 102
	МассивКатегорийБезПримененияМРД.Добавить(Справочники.КатегорииСотрудников.МОП); // 106
	МассивКатегорийБезПримененияМРД.Добавить(Справочники.КатегорииСотрудников.РаботающиеПенсионерыМОП); // 107
	Запрос.УстановитьПараметр("МассивКатегорийБезПримененияМРД", МассивКатегорийБезПримененияМРД);
	
	МассивВидовЗанятостиБезПримененияМРД = Новый Массив;
	Запрос.УстановитьПараметр("МассивВидовЗанятостиБезПримененияМРД", Справочники.ВидыЗанятости.Совместители);
