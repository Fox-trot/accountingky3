...
Процедура РассчитатьНалогиПоЗарплате()
	...
	ТекстЗапроса =
		...
		|	СотрудникиПоТрудовымСоглашениямСрезПоследних.Подразделение КАК Подразделение,
	...
	Иначе	
		ТекстЗапроса = ТекстЗапроса +
			...
			|			ИНАЧЕ (ВременнаяТаблицаНачисленияЗаМесяц.ОДПН + ЕСТЬNULL(ПодоходныйНалогОбороты.ОДПНОборот, 0) - ЕСТЬNULL(ВременнаяТаблицаНалогиСФ.ПФРДляВычетаПН, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ПФРДляРасчетаПНОборот, 0) - ЕСТЬNULL(ВременнаяТаблицаНалогиСФ.ГНПФРДляВычетаПН, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ГНПФРДляРасчетаПНОборот, 0) - (ВременнаяТаблицаСтатусы.Вычеты * 100 + (ЕСТЬNULL(ПодоходныйНалогОбороты.ПиИВычетыОборот, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ПФРОборот, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ГНПФРОборот, 0)))) * ВременнаяТаблицаСтатусы.СтавкаПН / 100 - (ЕСТЬNULL(ПодоходныйНалогОбороты.ПНОборот, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ДоплатаПНОборот, 0))
			|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК ПН,
			|	ВЫРАЗИТЬ(ВЫБОР
			|			КОГДА ВременнаяТаблицаНачисленияЗаМесяц.ОтработаноДней = 0
			|				ТОГДА 0
