...
Процедура ВыполнитьПредварительныйКонтроль(Отказ)
	...
	Если НЕ МассивРезультатов[2].Пустой() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
			ТекстСообщения = СтрШаблон(НСтр("ru = 'Не заполнен счет учета (затраты) Подразделение: %1, Вид расчета: %2. Проверьте настройки в регистре сведений ""Счета учета отражения заработной платы"".'"), 
...
Процедура РаспределитьНалогиСОрганизации()
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
		...
		СтрокаТабличнойЧасти.СуммаНачислено = СтрокаТабличнойЧасти.СуммаНачислено + ВыборкаДетальныеЗаписи.Результат;
	КонецЦикла;	
	
	Если НалогиСОрганизации.Количество() = 0 Тогда 
		Возврат;
	КонецЕсли;	
	
	// Распределение расчитанных налогов.
