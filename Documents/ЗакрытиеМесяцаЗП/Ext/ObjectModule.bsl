...
Процедура РассчитатьНалогиПоЗарплате()
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|	ВЫБОР
		|		КОГДА &ПлательщикЕН
		|				И (ВременнаяТаблицаСтатусы.Категория В (&МассивКатегорийБезПримененияМРД)
		|					ИЛИ ВременнаяТаблицаСтатусы.ВидЗанятости В ИЕРАРХИИ (&МассивВидовЗанятостиБезПримененияМРД))
		|			ТОГДА 0
		|		КОГДА &ПННарастающимИтогом
		|				И ВременнаяТаблицаНалогиПН.ПН < 0
	...
	МассивКатегорийБезПримененияМРД.Добавить(Справочники.КатегорииСотрудников.ИностранныеГраждане); // 010
