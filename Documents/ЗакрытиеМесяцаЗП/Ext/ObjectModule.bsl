...
Процедура РассчитатьНалогиПоЗарплате()
	...
	// а затем были уволины и снова приняты.
	ТекстЗапроса = 
		"ВЫБРАТЬ
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|ИЗ
		// Убираем секунду, так как записьуже попала в верхнем пакете.
		|	РегистрСведений.Сотрудники.СрезПоследних(ДОБАВИТЬКДАТЕ(&НачалоПериода, СЕКУНДА, -1),
		|			Организация = &Организация
		|				И НЕ ФизЛицо В
		|						(ВЫБРАТЬ
		...
		|ИЗ
		// Убираем секунду, так как записьуже попала в верхнем пакете.
		|	РегистрСведений.СотрудникиПоТрудовымСоглашениям.СрезПоследних(ДОБАВИТЬКДАТЕ(&НачалоПериода, СЕКУНДА, -1),
		|			Организация = &Организация
		|				И НЕ ФизЛицо В
		|						(ВЫБРАТЬ
		|							ВременнаяТаблицаСотрудникиПринятыеНаРаботу.ФизЛицо
	...
	ТекстЗапроса = ТекстЗапроса +
		...
		|ГДЕ
		|	ВременнаяТаблицаСреднемесячнаяЗПСФПриведенная.СреднемесячнаяЗПСФ * ВременнаяТаблицаСотрудники.ЗанимаемыхСтавок > 0
	...
	Иначе	
		ТекстЗапроса = ТекстЗапроса +
			...
			|			ИНАЧЕ (ВременнаяТаблицаНачисленияЗаМесяц.ОДПН + ЕСТЬNULL(ПодоходныйНалогОбороты.ОДПНОборот, 0) - ЕСТЬNULL(ВременнаяТаблицаНалогиСФ.ПФРДляВычетаПН, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ПФРДляРасчетаПНОборот, 0) - ЕСТЬNULL(ВременнаяТаблицаНалогиСФ.ГНПФРДляВычетаПН, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ГНПФРДляРасчетаПНОборот, 0) - (ВременнаяТаблицаСтатусы.Вычеты * 100 + ЕСТЬNULL(ПодоходныйНалогОбороты.ВычетыОборот, 0))) * ВременнаяТаблицаСтатусы.СтавкаПН / 100 - (ЕСТЬNULL(ПодоходныйНалогОбороты.ПНОборот, 0) - ЕСТЬNULL(ПодоходныйНалогОбороты.ДоплатаПНОборот, 0))
			|		КОНЕЦ КАК ЧИСЛО(15, 2)) КАК ПН,
			|	ВЫРАЗИТЬ(ВЫБОР
			|			КОГДА ВременнаяТаблицаНачисленияЗаМесяц.ОтработаноДней = 0
			|				ТОГДА 0
