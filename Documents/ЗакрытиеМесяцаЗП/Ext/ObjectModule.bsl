...
Процедура РаспределитьНалогиСОрганизации()
	...
	Пока ВыборкаФизЛицо.Следующий() Цикл 
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
			...
			НайденныеСтроки = НалогиСОрганизации.НайтиСтроки(Новый Структура("СчетУчетаЗатрат, СтатьяЗатрат, НоменклатурнаяГруппа", 
				ДанныеСчетаУчетаЗП.СчетУчетаЗатратСоцОтчисления, 
				ДанныеСчетаУчетаЗП.СтатьяЗатратСоцОтчисления,
				ДанныеСчетаУчетаЗП.НоменклатурнаяГруппа));
			...
			Если НайденныеСтроки.Количество() = 0 Тогда 
				...
				СтрокаТабличнойЧасти.НоменклатурнаяГруппа = ДанныеСчетаУчетаЗП.НоменклатурнаяГруппа;
			Иначе 
...
Процедура РаспределитьНалогиСОрганизацииКорректировки()
	...
	Пока ВыборкаФизЛицо.Следующий() Цикл 
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
			...
			НайденныеСтроки = НалогиСОрганизацииКорректировки.НайтиСтроки(Новый Структура("СчетУчетаЗатрат, СтатьяЗатрат, НоменклатурнаяГруппа", 
				ДанныеСчетаУчетаЗП.СчетУчетаЗатратСоцОтчисления, 
				ДанныеСчетаУчетаЗП.СтатьяЗатратСоцОтчисления,
				ДанныеСчетаУчетаЗП.НоменклатурнаяГруппа));
