...
Функция ПечатьПриказа(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст = 
		...
		|			И КадровоеПеремещение.Дата >= ВременнаяТаблицаСотрудники.Период
		|ГДЕ
		...
		|	КадровоеПеремещение.Подразделение,
	...
	Пока Выборка.Следующий()  Цикл
		...
		ДанныеПечати.Вставить("ДатаТрудовогоДоговора", Выборка.Дата);
		ДанныеПечати.Вставить("НомерТрудовогоДоговора", НомерНаПечать);
		...
		Руководители = БухгалтерскийУчетСервер.ОтветственныеЛицаОрганизаций(Выборка.Организация, Выборка.Дата);
