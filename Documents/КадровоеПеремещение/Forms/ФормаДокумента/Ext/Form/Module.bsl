...
&НаКлиенте
Процедура УдержанияВидДействияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НовыйМассив.Добавить(НовыйПараметр);
	НовыеПараметры = Новый ФиксированныйМассив(НовыйМассив);
	Элементы.УдержанияВидДействия.ПараметрыВыбора = НовыеПараметры;
КонецПроцедуры
...
Процедура ЗаполнитьПоСотруднику(ЗаполнитьРеквизиты = Ложь)
	...
	Объект.ЗанимаемыхСтавок = ВыборкаДетальныеЗаписи.ЗанимаемыхСтавок;
	Объект.Размер = ВыборкаДетальныеЗаписи.Размер;
	Объект.ВидРасчета = ВыборкаДетальныеЗаписи.ВидРасчета;
	Объект.Валюта = ВыборкаДетальныеЗаписи.Валюта;
...
Процедура ЗаполнитьУдержанияНаСервере()
	...
	Запрос.Текст =
		...
		|
		|УПОРЯДОЧИТЬ ПО
		|	ПлановыеУдержанияНачалоСрезПоследних.ВидРасчета.Порядок";
	Запрос.УстановитьПараметр("Период", Объект.Период);
	Запрос.УстановитьПараметр("Ссылка", Объект.Ссылка);
	Запрос.УстановитьПараметр("Организация", Объект.Организация);
	Запрос.УстановитьПараметр("ФизЛицо", Объект.ФизЛицо);
