...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	МожноРедактировать = ПравоДоступа("Редактирование", Метаданные.Документы.ЭлектронныйСчетФактура);
...
Процедура ЗаполнитьНомерСФДокумента(НомераСФПоGUID);// УникальныйИдентификатор, НомерСФ)
	...
	Для Каждого ДанныеНомераСФ Из НомераСФПоGUID Цикл
		ДокументСсылка = Документы.ЭлектронныйСчетФактура.ПолучитьСсылку(Новый УникальныйИдентификатор(ДанныеНомераСФ.Ключ));
		ДокументОбъект = ДокументСсылка.ПолучитьОбъект();
		
		ДокументОбъект.НомерЭлектроннойСчетФактуры = ДанныеНомераСФ.Значение;	
...
Процедура ДиалогОткрытияФайлаЗавершение(ВыбранныеФайлы, ДополнительныеПараметры) Экспорт
	...
	Если ВыделенныеСтроки.Количество() = 0 Тогда
		ОбщегоНазначенияКлиент.СообщитьПользователю(НСтр("ru = 'Не выделен ни один документ в списке, формирование файл xml отменено.'"));
		Возврат;
