	...
	Иначе
		Пока ВыборкаИзРезультатаЗапроса.Следующий() Цикл
			Если (ВыборкаИзРезультатаЗапроса.ГруппаИмущества = Справочники.ГруппыИмущества.ГИ2 
				...
				И Год(ВыборкаИзРезультатаЗапроса.ДатаНачисленияНалогаНаИмущество) = Год(Дата)) Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'У основного средства ""%1"" с с группой имущества %2 значение ""Начисления НИ с даты"" должно быть пустым или началом квартала текущего года.'"), 
				...
				БухгалтерскийУчетСервер.СообщитьОбОшибке(
					...
					"ОсновноеСредство",
					Отказ);
			ИначеЕсли ВыборкаИзРезультатаЗапроса.ГруппаИмущества = Справочники.ГруппыИмущества.ГИ4 
				И НЕ ВыборкаИзРезультатаЗапроса.ДатаНачисленияНалогаНаИмущество = КонецГода(Дата) + 1 Тогда
				ТекстСообщения = СтрШаблон(НСтр("ru = 'У основного средства ""%1"" с с группой имущества %2 значение ""Начисления НИ с даты"" должно быть пустым или началом следующего года.'"), 
														ВыборкаИзРезультатаЗапроса.ОсновноеСредствоПредставление, ВыборкаИзРезультатаЗапроса.ГруппаИмущества);
				БухгалтерскийУчетСервер.СообщитьОбОшибке(
					ЭтотОбъект,
					ТекстСообщения,
