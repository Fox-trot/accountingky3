...
Процедура ПрочееСчетУчетаПриИзменении(Элемент)
	...
	ПоляФормы = Новый Структура("Субконто1, Субконто2, Субконто3", "ПрочееСубконто1", "ПрочееСубконто2", "ПрочееСубконто3");
	БухгалтерскийУчетКлиентСервер.ПриВыбореСчета(СтрокаТабличнойЧасти.СчетУчета, ЭтаФорма, ПоляФормы, Неопределено, Истина);
	...
	ПараметрыДокумента = БухгалтерскийУчетКлиентСервер.СписокПараметровВыбораСубконто(ЭтаФорма, СтрокаТабличнойЧасти, "Субконто%Индекс%", "СчетУчета");
...
Процедура УстановитьПараметрыВыбора()
	// Основные средства.
	МассивСчетов = БухгалтерскийУчетВызовСервераПовтИсп.СчетаУчетаОсновныхСредств();
