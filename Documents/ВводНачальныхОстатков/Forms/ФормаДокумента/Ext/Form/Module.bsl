...
// Процедура - обработчик события ПриИзменении поля ввода НеПринятыеКУчету.
//
&НаКлиенте
Процедура НеПринятыеКУчетуПриИзменении(Элемент)
	Если Объект.НеПринятыеКУчету 
		И НЕ Объект.ОС.Количество() = 0 Тогда 
		ОписаниеОповещения = Новый ОписаниеОповещения("ОтветНаВопросНеПринятыеКУчетуПриИзменении", ЭтотОбъект);
		ТекстВопроса = СтрШаблон(НСтр("ru = 'Некоторые колонки табличной части ""%1"" будут очищены. Продолжить?'"), "Основные средства");
	
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 60);
	Иначе
		НеПринятыеКУчетуПриИзмененииФрагмент();	
		УстановитьВидимостьДоступностьЭлементов();
	КонецЕсли;
КонецПроцедуры
...
// Процедура-обработчик результата вопроса об изменении флана "Не принятые к учету".
//
&НаКлиенте
Процедура ОтветНаВопросНеПринятыеКУчетуПриИзменении(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		НеПринятыеКУчетуПриИзмененииФрагмент();	
		УстановитьВидимостьДоступностьЭлементов();
	Иначе 
...
&НаКлиенте
Процедура НеПринятыеКУчетуПриИзмененииФрагмент()
...
Процедура ОбработатьТаблицу(ИмяТЧ)
	...
	Если ИмяТЧ = "МБП" Тогда
		ВыполненноеДействие = МБПИзменениеСтрокДействие;
	КонецЕсли;
