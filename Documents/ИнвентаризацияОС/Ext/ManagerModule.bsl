...
Функция ПечатнаяФормаИнвентаризационнаяОписьИНВ1(МассивОбъектов, ОбъектыПечати) Экспорт
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати = "ПФ_MXL_ИнвентаризационнаяОписьИНВ1";
	...
	ТекстЗапроса = 
		...
		|	ИнвентаризацияОС.МОЛОрганизации КАК МОЛ,
		...
		|	ИнвентаризацияОС.ПодразделениеОрганизации,
		|	ИнвентаризацияОС.ПодразделениеОрганизации.Наименование,
		...
		|		ФизЛицо КАК ФизЛицо
		|	),
		|	ИнвентаризацияОС.МОЛОрганизации.Представление КАК МОЛПредставление
		|ИЗ
	...
	Пока ВыборкаЗаголовок.Следующий() Цикл
		...
		ОбластьМакета.Параметры.ДокументНомер = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ВыборкаЗаголовок.Номер);
		...
		Пока ВыборкаТЧ.Следующий() Цикл 		
			...
			Для Каждого Строка Из ВыборкаРегистра Цикл 
				Если ТипЗнч(Строка.Регистратор) = Тип("ДокументСсылка.ВводНачальныхОстатков") Тогда 
					...
					НомерДокумента = ?(Не ЗначениеЗаполнено(Строка.НомерДокумента), ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Строка.Регистратор.Номер),Строка.НомерДокумента);
				ИначеЕсли ТипЗнч(Строка.Регистратор) = Тип("ДокументСсылка.ПринятиеКУчетуОС") Тогда
					...
					НомерДокумента = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Строка.Регистратор.Номер);
...
Функция ПечатнаяФормаИнвентаризационнаяОписьИНВ18(МассивОбъектов, ОбъектыПечати) Экспорт
	
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати = "ПФ_MXL_ИнвентаризационнаяОписьИНВ18";
	...
	ТекстЗапроса = 
		...
		|	МАКСИМУМ(МБПЭксплуатация.Период) КАК Период
		|ПОМЕСТИТЬ ВТИНВМБП
		|ИЗ
		|	РегистрНакопления.МБПЭксплуатация КАК МБПЭксплуатация
		|ГДЕ
		|	МБПЭксплуатация.Регистратор.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийДвижениеМБП.ВводМБПВЭксплуатацию)
		|
		|СГРУППИРОВАТЬ ПО
		|	МБПЭксплуатация.Номенклатура,
		|	МБПЭксплуатация.ИнвНомер
		...
		|		НедостачаСумма,
		|		НедостачаКоличество,
		|		ВТИНВМБП.ИнвНомер КАК ИнвентарныйНомер,
		|		ВТИНВМБП.Период КАК ДатаВвода,
		|		Номенклатура.Наименование КАК Наименование
		|	),
		|	ЕСТЬNULL(ПроверкаИзлНедостачаМБП.ПроверкаМБП, ЛОЖЬ) КАК ПроверкаМБП,
		|	ЕСТЬNULL(ПроверкаИзлНедостачаОС.ПроверкаОС, ЛОЖЬ) КАК ПроверкаОС
		|ИЗ
		|	Документ.ИнвентаризацияОС КАК ИнвентаризацияОС
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВТИНВМБП КАК ВТИНВМБП
		|		ПО ИнвентаризацияОС.МБП.Номенклатура = ВТИНВМБП.Номенклатура
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
		|			ИнвентаризацияОСМБП.Ссылка КАК Ссылка,
		|			ВЫБОР
		|				КОГДА СУММА(ИнвентаризацияОСМБП.ИзлишекСумма) > 0
		|						ИЛИ СУММА(ИнвентаризацияОСМБП.ИзлишекКоличество) > 0
		|						ИЛИ СУММА(ИнвентаризацияОСМБП.НедостачаСумма) > 0
		|						ИЛИ СУММА(ИнвентаризацияОСМБП.НедостачаКоличество) > 0
		|					ТОГДА ИСТИНА
		|				ИНАЧЕ ЛОЖЬ
		|			КОНЕЦ КАК ПроверкаМБП
		|		ИЗ
		|			Документ.ИнвентаризацияОС.МБП КАК ИнвентаризацияОСМБП
		|		
		|		СГРУППИРОВАТЬ ПО
		|			ИнвентаризацияОСМБП.Ссылка) КАК ПроверкаИзлНедостачаМБП
		|		ПО ИнвентаризацияОС.Ссылка = ПроверкаИзлНедостачаМБП.Ссылка
		|ГДЕ
		|	ИнвентаризацияОС.Ссылка В(&СписокДокументов)";
	Запрос = Новый Запрос(ТекстЗапроса);
	Запрос.УстановитьПараметр("СписокДокументов", МассивОбъектов);
	...
	Пока ВыборкаЗаголовок.Следующий()  Цикл
		Если НЕ ВыборкаЗаголовок.ПроверкаМБП   
			и   Не ВыборкаЗаголовок.ПроверкаОС  Тогда
			Прервать;
		КонецЕсли;
		...
		ВыборкаСтрок  = Макет.ПолучитьОбласть("Строка");	
		ВыборкаТЧ  = ВыборкаЗаголовок.МБП.Выбрать();
...
Функция ДолжностьСотрудника(Фирма,Сотр,Дат)
	ОтборСотрудники = Новый Структура;
	ОтборСотрудники.Вставить("Организация", Фирма);
	ОтборСотрудники.Вставить("ФизЛицо", Сотр);
	
	Ресурсы =РегистрыСведений.Сотрудники.ПолучитьПоследнее(Дат,ОтборСотрудники);
	
	Возврат Ресурсы.Должность;
КонецФункции
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	НужноПечататьМакет = УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_ИнвентаризационнаяОписьИНВ1");
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
		КоллекцияПечатныхФорм,
		"ПФ_MXL_ИнвентаризационнаяОписьИНВ1",
	...
	НужноПечататьМакет = УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_ИнвентаризационнаяОписьИНВ18");
	Если НужноПечататьМакет Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(
		КоллекцияПечатныхФорм,
		"ПФ_MXL_ИнвентаризационнаяОписьИНВ18",
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "ПФ_MXL_ИнвентаризационнаяОписьИНВ1";
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПФ_MXL_ИнвентаризационнаяОписьИНВ18";
