...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	СуммаДокумента = Зарплата.Итог("СуммаПоБанковскомуСчету") + Зарплата.Итог("СуммаПоКартСчету")
					+ Зарплата.Итог("СуммаПоКассе")+ Зарплата.Итог("СуммаПоАвансовомуОтчету");
КонецПроцедуры
...
Процедура ЗаполнитьТабличнуюЧасть() Экспорт 
	...
	ТекстЗапроса =
		...
		|ГДЕ
		|	&СотрудникиПоКассе
		|	И (&СотрудникиПоЗППроекту ИЛИ &СотрудникиПоБанковскимСчетам)
		...
		|	ВременнаяТаблицаПолныйСписокСотрудников КАК ВременнаяТаблицаПолныйСписокСотрудников
		|ГДЕ
		|	(&СотрудникиПоКассе ИЛИ &СотрудникиПоАО)
		|	И НЕ &СотрудникиПоЗППроекту
		|	И НЕ &СотрудникиПоБанковскимСчетам";		
	...
	Запрос.УстановитьПараметр("СотрудникиПоКассе", ПоКассе);
	Запрос.УстановитьПараметр("СотрудникиПоЗППроекту", ПоЗППроект);
	Запрос.УстановитьПараметр("СотрудникиПоБанковскимСчетам", ПоБанковскимСчетам);
	Запрос.УстановитьПараметр("СотрудникиПоАО", ПоАвансовомуОтчету);
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ЗначениеЗаполнено(Выборка.КартСчет) Тогда
			ЕстьКартСчет = Истина;
		Иначе 
			ЕстьКартСчет = Ложь;
		КонецЕсли;
		...
		ИначеЕсли ЕстьКартСчет Тогда
			СтрокаТабличнойЧасти.СуммаПоКартСчету = СуммаВыборки;
			
		ИначеЕсли ПоКассе Тогда
			СтрокаТабличнойЧасти.СуммаПоКассе = СуммаВыборки;
			ОбщаяСуммаПоКассе = ОбщаяСуммаПоКассе + СуммаВыборки;
			
		Иначе
			СтрокаТабличнойЧасти.СуммаПоАвансовомуОтчету = СуммаВыборки;
			ОбщаяСуммаПоАО = ОбщаяСуммаПоАО + СуммаВыборки;
		КонецЕсли;
		...
		ИначеЕсли ЕстьБанковскийСчет <> "" Тогда
		...
		Иначе
			СтрокаТабличнойЧасти.СуммаПоАвансовомуОтчету = БухгалтерскийУчетСервер.ОкруглитьЦену(СтрокаТабличнойЧасти.СуммаПоАвансовомуОтчету, ПорядокОкругления, Истина);
		КонецЕсли;
	КонецЦикла; 
	...
	Если ПоАвансовомуОтчету Тогда
		Если ОбщаяСуммаПоАО <> 0 Тогда
			СтрокаТабличнойЧасти = ТаблицаВыплат.Добавить();
			СтрокаТабличнойЧасти.Наименование = "Авансовый отчет";
			СтрокаТабличнойЧасти.Сумма 		  = ОбщаяСуммаПоАО;
		КонецЕсли;
	КонецЕсли;
	...
	Пока ВыборкаПоГруппировкам.Следующий() Цикл
		Если ВыборкаПоГруппировкам.Сумма <> 0 Тогда
			...
			Если КоличествоЗпПроектов = 1 Тогда
				Наименование = "ЗП проект";
			Иначе
				Наименование = "ЗП проект" + НомерПроекта;
			КонецЕсли;
...
Процедура ЗаполнитьТаблицуВыплатПоБанковскимСчетам(Сумма) Экспорт 
	...
	Если Сумма <> 0 Тогда	
		СтрокаТабличнойЧасти = ТаблицаВыплат.Добавить();
		СтрокаТабличнойЧасти.Наименование = "Банковские счета";
