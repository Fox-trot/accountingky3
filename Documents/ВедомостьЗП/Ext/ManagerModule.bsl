...
Функция ПечатьФормаТ53(МассивОбъектов, ОбъектыПечати)
	...
	ТекстЗапроса =
		...
		|	ВедомостьЗП.Номер КАК Номер,
		|	ВедомостьЗП.Дата КАК Дата,
		|	ВедомостьЗП.Организация КАК Организация,
		|	ВедомостьЗП.Организация.НаименованиеПолное КАК ОрганизацияНаименованиеПолное,
		|	ВедомостьЗП.Организация.КодПоОКПО КАК ОрганизацияКодПоОКПО,
		|	ВедомостьЗП.Подразделение КАК Подразделение,
		|	ВедомостьЗП.Касса КАК Касса,
		|	ВедомостьЗП.ПериодРегистрации КАК ПериодРегистрации,
		|	ВедомостьЗП.СуммаДокумента КАК СуммаДокумента,
		|	ВЫБОР
		|		КОГДА ВедомостьЗП.ВидВедомости = ЗНАЧЕНИЕ(Перечисление.ВидыВедомости.Все)
		|				ИЛИ ВедомостьЗП.ВидВедомости = ЗНАЧЕНИЕ(Перечисление.ВидыВедомости.Касса)
		|			ТОГДА ВедомостьЗП.Касса.СчетУчета
		|		КОГДА ВедомостьЗП.ВидВедомости = ЗНАЧЕНИЕ(Перечисление.ВидыВедомости.Банк)
		|			ТОГДА ВедомостьЗП.БанковскийСчет.СчетУчета
		|		ИНАЧЕ
		|			НЕОПРЕДЕЛЕНО
		|	КОНЕЦ КАК КорСчет,
		|	ВедомостьЗП.Зарплата.(
		|		НомерСтроки КАК НомерСтроки,
		|		ФизЛицо КАК ФИОФизЛица,
		|		ВЫБОР
		|			КОГДА ВедомостьЗП.ВидВедомости = ЗНАЧЕНИЕ(Перечисление.ВидыВедомости.Все)
		|					ИЛИ ВедомостьЗП.ВидВедомости = ЗНАЧЕНИЕ(Перечисление.ВидыВедомости.Касса)
		|				ТОГДА ЕСТЬNULL(ВедомостьЗП.Зарплата.СуммаПоКассе, 0)
		|			КОГДА ВедомостьЗП.ВидВедомости = ЗНАЧЕНИЕ(Перечисление.ВидыВедомости.Банк)
		|				ТОГДА ЕСТЬNULL(ВедомостьЗП.Зарплата.СуммаПоБанковскомуСчету, 0)
		|			КОГДА ВедомостьЗП.ВидВедомости = ЗНАЧЕНИЕ(Перечисление.ВидыВедомости.ЗППроект)
		|				ТОГДА ЕСТЬNULL(ВедомостьЗП.Зарплата.СуммаПоКартСчету, 0)
		|			ИНАЧЕ
		|				0	
		|		КОНЕЦ КАК Сумма
		|	) КАК Зарплата
		...
		|	Документ.ВедомостьЗП КАК ВедомостьЗП
		|ГДЕ
		|	ВедомостьЗП.Ссылка В(&СписокДокументов)";		
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати = "ВедомостьЗП_ФормаТ53";

	Макет = УправлениеПечатью.МакетПечатнойФормы("Документ.ВедомостьЗП.ПФ_MXL_Т53");
