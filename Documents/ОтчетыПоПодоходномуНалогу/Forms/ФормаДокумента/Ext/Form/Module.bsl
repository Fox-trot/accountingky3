...
Функция ФормированиеФайлаXML_UBR(ДополнительныеПараметры, Ошибки)
	...
	Если Объект.ВидСубъекта = 2 Тогда
	...
	ТаблицаПодоходныйНалог.Свернуть("НомерСтрокиОтчета",
		"Количество,СуммаНачислено,СуммаНеоблагаемая,Вычеты,СуммаПНсСотрудника,СуммаПНсМРД,СальдоК,СуммаПН,СуммаВыплачен");
	...
	ОбъектXDTOMORE15PAIDINCOME.Text = ?(Объект.НеЗаполнятьВыплатыПоЗП, 0, 
										?(СтрокаТаблицы = Неопределено, 0, СтрокаТаблицы.СуммаВыплачен));
	...
	ОбъектXDTOTOTALPAIDINCOME.Text = ?(Объект.НеЗаполнятьВыплатыПоЗП, 0, Объект.ПодоходныйНалог.Итог("СуммаВыплачен"));
...
Процедура ЗаполнитьПервыйМесяцФайлаXML(ОбъектXDTO)
	...
	ОбъектXDTOSTI133X198.Text = ?(Объект.НеЗаполнятьВыплатыПоЗП, 0, 
									Объект.ПодоходныйНалогПервыйМесяцКвартала.Итог("СуммаВыплачен"));
...
Процедура ЗаполнитьВторойМесяцФайлаXML(ОбъектXDTO)
	...
	ОбъектXDTOSTI133X248.Text = ?(Объект.НеЗаполнятьВыплатыПоЗП, 0, 
									Объект.ПодоходныйНалогВторойМесяцКвартала.Итог("СуммаВыплачен"));
...
Процедура ЗаполнитьТретийМесяцФайлаXML(ОбъектXDTO)
	...
	ОбъектXDTOSTI133X298.Text = ?(Объект.НеЗаполнятьВыплатыПоЗП, 0, 
									Объект.ПодоходныйНалогТретийМесяцКвартала.Итог("СуммаВыплачен"));
