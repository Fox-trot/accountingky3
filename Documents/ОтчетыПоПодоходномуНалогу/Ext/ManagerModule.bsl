...
Функция ПечатьПоПН(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст = 
		...
		|		Вычеты КАК Вычеты,
		|		ОтчетыПоПодоходномуНалогу.ПодоходныйНалог.СуммаНачислено - ОтчетыПоПодоходномуНалогу.ПодоходныйНалог.СуммаНеоблагаемая - ОтчетыПоПодоходномуНалогу.ПодоходныйНалог.Вычеты КАК Облагаемая,
		|		СуммаПНсСотрудника КАК ПодоходныйСотрудника,
		|		СуммаПНсМРД КАК СМРД,
		|		СальдоК КАК ПодоходныйКОплате,
		|		СуммаПН КАК ВсегоПодоходный,
		|		ВЫБОР
		|			КОГДА ОтчетыПоПодоходномуНалогу.НеЗаполнятьВыплатыПоЗП
		|				ТОГДА 0
		|			ИНАЧЕ ОтчетыПоПодоходномуНалогу.ПодоходныйНалог.СуммаВыплачен
		|		КОНЕЦ КАК Выплачено,
...
Функция ПечатьПриложенияКОтчетуПН(МассивОбъектов, ОбъектыПечати)	
	...
	Запрос.Текст =  
		...
		|		ЕСТЬNULL(ОтчетыПоПодоходномуНалогу.ПодоходныйНалогПервыйМесяцКвартала.СуммаВыплачен, 0) КАК Выплачено,
		...
		|		ЕСТЬNULL(ОтчетыПоПодоходномуНалогу.ПодоходныйНалогВторойМесяцКвартала.СуммаВыплачен, 0) КАК Выплачено,
		...
		|		ЕСТЬNULL(ОтчетыПоПодоходномуНалогу.ПодоходныйНалогТретийМесяцКвартала.СуммаВыплачен, 0) КАК Выплачено,
