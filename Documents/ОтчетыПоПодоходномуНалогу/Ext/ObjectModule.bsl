...
Процедура ЗаполнитьПодоходныйНалог(ДанныеУчетнойПолитики) Экспорт
	...
	Запрос.Текст = 
		...
		|	СУММА(ДанныеОтчетаПоПНОбороты.КоличествоОборот) КАК КоличествоОборот,
		|	СУММА(ДанныеОтчетаПоПНОбороты.ВсегоНачисленоОборот) КАК ВсегоНачисленоОборот,
		|	СУММА(ДанныеОтчетаПоПНОбороты.ВсегоНачисленоОборот - ДанныеОтчетаПоПНОбороты.ОДПНОборот) КАК СуммаНеоблагаемая,
		|	СУММА(ДанныеОтчетаПоПНОбороты.ВычетыОборот + ДанныеОтчетаПоПНОбороты.ПФРОборот + ДанныеОтчетаПоПНОбороты.ГНПФРОборот) КАК ПиИВычетыОборот,
		|	СУММА(ДанныеОтчетаПоПНОбороты.ОДПНОборот - ДанныеОтчетаПоПНОбороты.ВычетыОборот - ДанныеОтчетаПоПНОбороты.ПФРОборот - ДанныеОтчетаПоПНОбороты.ГНПФРОборот) КАК СуммаОблагаемая,
		...
		|	СУММА(ДанныеОтчетаПоПНОбороты.ПНСотрудникаОборот + ДанныеОтчетаПоПНОбороты.ДоплатаПНОборот) КАК ВсегоПНОборот,
		...
		|	4,
		|	1,
		|	ВЫРАЗИТЬ(ХозрасчетныйОбороты.СуммаОборотКт * 10 КАК ЧИСЛО(15, 2)),
		|	0,
		|	0,
		|	ВЫРАЗИТЬ(ХозрасчетныйОбороты.СуммаОборотКт * 10 КАК ЧИСЛО(15, 2)),
		|	ВЫРАЗИТЬ(ХозрасчетныйОбороты.СуммаОборотКт КАК ЧИСЛО(15, 2)),
		|	0,
		|	ВЫРАЗИТЬ(ХозрасчетныйОбороты.СуммаОборотКт КАК ЧИСЛО(15, 2)),
		|	0,
		|	0
		|ИЗ
		|	РегистрБухгалтерии.Хозрасчетный.Обороты(&НачалоПериода, &КонецПериода, , Счет = &СчетУчетаПоПНДляПрочихФизическихЛиц, , Организация = &Организация, , ) КАК ХозрасчетныйОбороты
		|ГДЕ
		...
		|	СУММА(ВЫРАЗИТЬ(ВременнаяТаблицаВсеСтроки.КоличествоОборот / &КоличествоМесяцев КАК ЧИСЛО(7, 2))) КАК Количество,
		...
		|	СУММА(ВременнаяТаблицаВсеСтроки.ВсегоПНОборот) КАК СальдоК,
		|	СУММА(ВЫБОР
		|			КОГДА &НеЗаполнятьВыплатыПоЗП
		|				ТОГДА 0
		|			ИНАЧЕ ВременнаяТаблицаВсеСтроки.ВсегоНачисленоОборот - ВременнаяТаблицаВсеСтроки.ПФРОборот - ВременнаяТаблицаВсеСтроки.ГНПФРОборот - ВременнаяТаблицаВсеСтроки.ВсегоПНОборот
...
Процедура ЗаполнитьПодоходныйНалогПриложение(ДанныеУчетнойПолитики) Экспорт
	...
	Запрос.Текст = 
		...
		|	ДанныеОтчетаПоПНОбороты.ВсегоНачисленоОборот - ДанныеОтчетаПоПНОбороты.ОДПНОборот КАК СуммаНеоблагаемая,
		|	ДанныеОтчетаПоПНОбороты.ВычетыОборот + ДанныеОтчетаПоПНОбороты.ПФРОборот + ДанныеОтчетаПоПНОбороты.ГНПФРОборот КАК Вычеты,
		|	ДанныеОтчетаПоПНОбороты.ОДПНОборот - ДанныеОтчетаПоПНОбороты.ВычетыОборот - ДанныеОтчетаПоПНОбороты.ПФРОборот - ДанныеОтчетаПоПНОбороты.ГНПФРОборот КАК СуммаОблагаемая,
		|	ДанныеОтчетаПоПНОбороты.ПНСотрудникаОборот КАК СуммаПНсСотрудника,
		|	ДанныеОтчетаПоПНОбороты.ДоплатаПНОборот КАК СуммаПНсМРД,
		|	ДанныеОтчетаПоПНОбороты.ПНСотрудникаОборот + ДанныеОтчетаПоПНОбороты.ДоплатаПНОборот КАК СуммаПН,
		|	ДанныеОтчетаПоПНОбороты.ПНСотрудникаОборот + ДанныеОтчетаПоПНОбороты.ДоплатаПНОборот КАК СальдоК,
		|	ВЫБОР
		|		КОГДА &НеЗаполнятьВыплатыПоЗП
		|			ТОГДА 0
		|		ИНАЧЕ ДанныеОтчетаПоПНОбороты.ВсегоНачисленоОборот - ДанныеОтчетаПоПНОбороты.ПФРОборот - ДанныеОтчетаПоПНОбороты.ГНПФРОборот - ДанныеОтчетаПоПНОбороты.ПНСотрудникаОборот - ДанныеОтчетаПоПНОбороты.ДоплатаПНОборот
		|	КОНЕЦ КАК СуммаВыплачен
