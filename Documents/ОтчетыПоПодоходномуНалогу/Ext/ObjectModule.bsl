...
Процедура ЗаполнитьПодоходныйНалог(ДанныеУчетнойПолитики) Экспорт
	...
	Запрос.Текст = 
		...
		|	КОЛИЧЕСТВО(ДанныеОтчетаПоПНОбороты.ФизЛицо) КАК КоличествоОборот,
		|	КОЛИЧЕСТВО(РАЗЛИЧНЫЕ ДанныеОтчетаПоПНОбороты.ПериодМесяц) КАК ПериодМесяц,
		...
		|	ВЫРАЗИТЬ(ВременнаяТаблицаВсеСтроки.КоличествоОборот / &КоличествоМесяцев КАК ЧИСЛО(5, 0)) КАК Количество,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаВсеСтроки.ВсегоНачисленоОборот КАК ЧИСЛО(15, 2)) КАК СуммаНачислено,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаВсеСтроки.НеоблагаемаяОборот КАК ЧИСЛО(15, 2)) КАК СуммаНеоблагаемая,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаВсеСтроки.ПиИВычетыОборот КАК ЧИСЛО(15, 2)) КАК Вычеты,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаВсеСтроки.ОблагаемаяОборот КАК ЧИСЛО(15, 2)) КАК СуммаОблагаемая,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаВсеСтроки.ПНСотрудникаОборот КАК ЧИСЛО(15, 2)) КАК СуммаПНсСотрудника,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаВсеСтроки.ДоплатаПНОборот КАК ЧИСЛО(15, 2)) КАК СуммаПНсМРД,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаВсеСтроки.ВсегоПНОборот КАК ЧИСЛО(15, 2)) КАК СуммаПН,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаВсеСтроки.ВсегоПНОборот КАК ЧИСЛО(15, 2)) КАК СальдоК,
		|	ВЫБОР
		|		КОГДА &НеЗаполнятьВыплатыПоЗП
		|			ТОГДА 0
		|		ИНАЧЕ ВЫРАЗИТЬ(ВременнаяТаблицаВсеСтроки.ВсегоНачисленоОборот - ВременнаяТаблицаВсеСтроки.ПФРОборот - ВременнаяТаблицаВсеСтроки.ГНПФРОборот - ВременнаяТаблицаВсеСтроки.ВсегоПНОборот КАК ЧИСЛО(15, 2))
...
Процедура ЗаполнитьПодоходныйНалогПриложение(ДанныеУчетнойПолитики) Экспорт
	...
	Запрос.Текст = 
		...
		|	ВЫРАЗИТЬ(ДанныеОтчетаПоПНОбороты.ВычетыОборот + ДанныеОтчетаПоПНОбороты.ПФРОборот + ДанныеОтчетаПоПНОбороты.ГНПФРОборот КАК ЧИСЛО(15, 2)) КАК Вычеты,
		...
		|	ВЫРАЗИТЬ(ДанныеОтчетаПоПНОбороты.ПНСотрудникаОборот + ДанныеОтчетаПоПНОбороты.ДоплатаПНОборот КАК ЧИСЛО(15, 2)) КАК СуммаПН,
		|	ВЫРАЗИТЬ(ДанныеОтчетаПоПНОбороты.ПНСотрудникаОборот + ДанныеОтчетаПоПНОбороты.ДоплатаПНОборот КАК ЧИСЛО(15, 2)) КАК СальдоК,
		|	ВЫБОР
		|		КОГДА &НеЗаполнятьВыплатыПоЗП
		|			ТОГДА 0
		|		ИНАЧЕ ВЫРАЗИТЬ(ДанныеОтчетаПоПНОбороты.ВсегоНачисленоОборот - ДанныеОтчетаПоПНОбороты.ПФРОборот - ДанныеОтчетаПоПНОбороты.ГНПФРОборот - ДанныеОтчетаПоПНОбороты.ПНСотрудникаОборот - ДанныеОтчетаПоПНОбороты.ДоплатаПНОборот КАК ЧИСЛО(15, 2))
		|	КОНЕЦ КАК СуммаВыплачен
