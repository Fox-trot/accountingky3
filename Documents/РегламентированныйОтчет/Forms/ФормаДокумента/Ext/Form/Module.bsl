...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	ВидимостьПП = ГруппаНастройки(Объект.Настройка) = Справочники.НастройкиРегламентированныхОтчетов.ОПУ;
...
Процедура ПроставитьНомераПроходов()
	...
	// 1. Список групп
	ТЗСГ = Объект.СтрокиОтчета.Выгрузить();
	...
	Пока КЦ < 20 Цикл
		...
		Для Каждого СТЗ Из ТЗСГ Цикл
			Если СТЗ.Проход = КЦ Тогда
				...
				СтрокиГруппы  = Объект.СтрокиОтчета.НайтиСтроки(Отбор);
	...
	// 5. Простановка проходов в СтрокиОтчета
	
	Для Каждого СТЧ Из Объект.СтрокиОтчета Цикл
		СТЧ.Проход = 0;
...
Процедура РассчитатьОтчет()
	...
	Запрос.Текст = 
		...
		|	СтрокиОтчета.Картинка КАК Картинка,
	...
	Объект.СтрокиОтчета.Загрузить(РезультатЗапроса[8].Выгрузить());
	...
	Пока КЦ <= КоличествоПроходов Цикл
		ТЗГ = Объект.СтрокиОтчета.Выгрузить();
		...
		// 7.3. Перенос групповых Сумм и СуммПП из ТЗГ в ТЧ СтрокиОтчета
		Для Каждого СТЧ Из Объект.СтрокиОтчета Цикл
...
Процедура РасчитатьОПУ()
	...
	|ИЗ
	|	Документ.РегламентированныйОтчет.СтрокиОтчета КАК ОтчетР
	|ГДЕ
...
Процедура РассчитатьОИКНаСервере()
	...
	ТЗ = Объект.Настройка.СтрокиОтчета.Выгрузить();
	Объект.СтрокиОтчетаОИК.Загрузить(ТЗ);
	...
	Пока Выборка.Следующий() Цикл 
		...
		МассивСТЧ =  Объект.СтрокиОтчетаОИК.НайтиСтроки(ПараметрыОтбора);
	...
	ОтчетЕГ = Объект.СтрокиОтчетаОИК.Выгрузить();
	...
	Объект.СтрокиОтчетаОИК.Загрузить(ОтчетЕГ);
	
	ОтчетЕГ.Очистить();	
	Для Счетчик = 0 По 2 Цикл
		НоваяСтрока = ОтчетЕГ.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, Объект.СтрокиОтчетаОИК[Счетчик]);
	КонецЦикла;
	ОтчетЕГ.Свернуть(,
	"АкционерныйКапитал,ДополнительноОплаченныйКапитал,ОбщийРезерв,РезервПоПереоценкеОсновныхСредств,РезервПоПереоценкеИнвестиционногоИмущества,РезервПоХеджированиюПотокоаДенежныхСредств,
	|РезеврПоПересчетуЗарубежнойДеятельности,РезеврПоПереоценкеФинансовыхАктивов,НераспределеннаяПрибыль,ИтогоКапиталОтосящийсяКСобственникамКомпании,НеконтрольнаяДоляУчастия,Итого");
	ЗаполнитьЗначенияСвойств(Объект.СтрокиОтчетаОИК[3], ОтчетЕГ[0],	"АкционерныйКапитал,ДополнительноОплаченныйКапитал,ОбщийРезерв,РезервПоПереоценкеОсновныхСредств,РезервПоПереоценкеИнвестиционногоИмущества,РезервПоХеджированиюПотокоаДенежныхСредств,
	|РезеврПоПересчетуЗарубежнойДеятельности,РезеврПоПереоценкеФинансовыхАктивов,НераспределеннаяПрибыль,ИтогоКапиталОтосящийсяКСобственникамКомпании,НеконтрольнаяДоляУчастия,Итого");
	...
	"АкционерныйКапитал,ДополнительноОплаченныйКапитал,ОбщийРезерв,РезервПоПереоценкеОсновныхСредств,РезервПоПереоценкеИнвестиционногоИмущества,РезервПоХеджированиюПотокоаДенежныхСредств,
	|РезеврПоПересчетуЗарубежнойДеятельности,РезеврПоПереоценкеФинансовыхАктивов,НераспределеннаяПрибыль,ИтогоКапиталОтосящийсяКСобственникамКомпании,НеконтрольнаяДоляУчастия,Итого");
	ЗаполнитьЗначенияСвойств(Объект.СтрокиОтчетаОИК[14], ОтчетЕГ[0],	"АкционерныйКапитал,ДополнительноОплаченныйКапитал,ОбщийРезерв,РезервПоПереоценкеОсновныхСредств,РезервПоПереоценкеИнвестиционногоИмущества,РезервПоХеджированиюПотокоаДенежныхСредств,
	|РезеврПоПересчетуЗарубежнойДеятельности,РезеврПоПереоценкеФинансовыхАктивов,НераспределеннаяПрибыль,ИтогоКапиталОтосящийсяКСобственникамКомпании,НеконтрольнаяДоляУчастия,Итого");
	...
	"АкционерныйКапитал,ДополнительноОплаченныйКапитал,ОбщийРезерв,РезервПоПереоценкеОсновныхСредств,РезервПоПереоценкеИнвестиционногоИмущества,РезервПоХеджированиюПотокоаДенежныхСредств,
	|РезеврПоПересчетуЗарубежнойДеятельности,РезеврПоПереоценкеФинансовыхАктивов,НераспределеннаяПрибыль,ИтогоКапиталОтосящийсяКСобственникамКомпании,НеконтрольнаяДоляУчастия,Итого");
	ЗаполнитьЗначенияСвойств(Объект.СтрокиОтчетаОИК[21], ОтчетЕГ[0],	"АкционерныйКапитал,ДополнительноОплаченныйКапитал,ОбщийРезерв,РезервПоПереоценкеОсновныхСредств,РезервПоПереоценкеИнвестиционногоИмущества,РезервПоХеджированиюПотокоаДенежныхСредств,
	|РезеврПоПересчетуЗарубежнойДеятельности,РезеврПоПереоценкеФинансовыхАктивов,НераспределеннаяПрибыль,ИтогоКапиталОтосящийсяКСобственникамКомпании,НеконтрольнаяДоляУчастия,Итого");	
