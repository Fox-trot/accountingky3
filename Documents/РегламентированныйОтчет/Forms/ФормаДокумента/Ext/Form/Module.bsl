...
Процедура РасшифровкаДДС(Команда)
	...
	Если СтрокаТабличнойЧасти = Неопределено Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"Объект.СтрокиОтчета",, Отказ);
	КонецЕсли;
	Если Отказ Тогда
		Возврат;		
	КонецЕсли;
	...
	Если СтрокиНастройкиСтрокОДДСПоСтатьям.Количество() = 0 Тогда
		...
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"Объект.НастройкиСтрокОДДСПоСтатьям",, Отказ);		
		Возврат;
	КонецЕсли;
	...
	ОткрытьОтчет("Отчет.РасшифровкаДвиженияДенежныхСредств.Форма",, ОтборыОтчета,,,, НачалоДня(Объект.НачалоПериода), КонецДня(Объект.КонецПериода));
	
КонецПроцедуры

&НаКлиенте
Процедура КонтрольЗаполненныхСтатейДДС(Команда)

	ОткрытьОтчет("Отчет.КонтрольЗаполненныхСтатейДвиженияДенежныхСредств.Форма",,,,,, НачалоДня(Объект.НачалоПериода), КонецДня(Объект.КонецПериода));
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	ИначеЕсли Объект.ВидОтчета = Перечисления.ВидыРегламентированныхОтчетов.ОДДС Тогда
		...
		Элементы.СтрокиОтчетаКонтрольЗаполненныхСтатейДДС.Видимость = ПолучитьФункциональнуюОпцию("ИспользоватьСтатьиДвиженияДенежныхСредств");
