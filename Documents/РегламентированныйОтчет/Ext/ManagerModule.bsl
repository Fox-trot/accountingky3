...
Функция ПечатьОИК(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст = 
		...
		|	РегламентированныйОтчет.Организация.КодГКЭД КАК ГКЭД,
		|	РегламентированныйОтчет.Организация.ОрганУправления КАК ОрганУправления,
		|	РегламентированныйОтчет.Организация.КодПравовойФормы КАК ФормаСобст,
		|	РегламентированныйОтчет.Организация.КодПравовойФормы КАК ГКФС,
		|	РегламентированныйОтчет.СтрокиОтчета.(
		|		КодСтроки КАК КодСтроки,
		|		НаименованиеСтроки КАК НаименованиеСтроки,
		|		НаименованиеСтрокиКырг КАК НаименованиеСтрокиКырг,
		...
		|	) КАК СтрокиОтчета,
		|	РегламентированныйОтчет.СтрокиОтчетаОИК.(
		|		Ссылка КАК Ссылка,
		|		НомерСтроки КАК НомерСтроки,
		|		НаименованиеСтроки КАК НаименованиеСтроки,
		|		ЭтоЗаголовок КАК ЭтоЗаголовок,
		|		Группа КАК Группа,
		|		ЗнакГруппы КАК ЗнакГруппы,
		|		АкционерныйКапитал КАК АкционерныйКапитал,
		|		ДополнительноОплаченныйКапитал КАК ДополнительноОплаченныйКапитал,
		|		ОбщийРезерв КАК ОбщийРезерв,
		|		РезервПоПереоценкеОсновныхСредств КАК РезервПоПереоценкеОсновныхСредств,
		|		РезервПоПереоценкеИнвестиционногоИмущества КАК РезервПоПереоценкеИнвестиционногоИмущества,
		|		РезервПоХеджированиюПотокоаДенежныхСредств КАК РезервПоХеджированиюПотокоаДенежныхСредств,
		|		РезеврПоПересчетуЗарубежнойДеятельности КАК РезеврПоПересчетуЗарубежнойДеятельности,
		|		РезеврПоПереоценкеФинансовыхАктивов КАК РезеврПоПереоценкеФинансовыхАктивов,
		|		НераспределеннаяПрибыль КАК НераспределеннаяПрибыль,
		|		ИтогоКапиталОтосящийсяКСобственникамКомпании КАК ИтогоКапиталОтосящийсяКСобственникамКомпании,
		|		НеконтрольнаяДоляУчастия КАК НеконтрольнаяДоляУчастия,
		|		Итого КАК Итого,
		|		ИмяКолонки КАК ИмяКолонки,
		|		КодСтроки КАК КодСтроки
		|	) КАК СтрокиОтчетаОИК
		|ИЗ
		|	Документ.РегламентированныйОтчет КАК РегламентированныйОтчет
