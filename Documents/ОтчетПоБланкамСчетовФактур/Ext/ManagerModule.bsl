...
Процедура ОбработкаРезультатаЗапросаЛандшафт(ТабличныйДокумент, ПараметрыВыборки)
	...
	Пока ВыборкаСерии.Следующий() Цикл
		...
		//СерияБланкаСФСтр = СерияБланкаСФ + "-" + ВыборкаСерии.КоличествоВсего;
	КонецЦикла; 
...
Функция ПечатьОтчетаОбИспользованииБланков(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст =		
		...
		//|	СУММА(ВЫБОР
		//|			КОГДА Обороты.КоличествоРасход) КАК ИспользованоРА5,
		//|	СУММА(ВЫБОР
		//|			КОГДА Обороты.КоличествоРасход) КАК ИспользованоПА5,
...
Функция ПечатьОтчетаОбИспользованииБланковПодробный(МассивОбъектов, ОбъектыПечати)
	Для каждого ДокументСсылка Из МассивОбъектов Цикл	
		...
		АдресОрганизации	= УправлениеКонтактнойИнформацией.КонтактнаяИнформацияОбъекта(Организация, Справочники.ВидыКонтактнойИнформации.ЮрАдресОрганизации, КонецПериода);
