...
Процедура РазовоеНачислениеПриИзменении(Элемент)	
	...
	Если Объект.Начисления.Количество() > 0 Тогда 
		ОписаниеОповещения = Новый ОписаниеОповещения("ОтветНаВопросОчиститьТабличнуюЧастьНачисленияПриИзмененииРазовоеНачисление", ЭтотОбъект);
		ТекстВопроса = НСтр("ru = 'Табличная часть документа будет очищена. Продолжить выполнение операции?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 60);
	Иначе 
		РазовоеНачисление = Объект.РазовоеНачисление;
...
Процедура НачисленияВидРасчетаПриИзменении(Элемент)
	...
	СпособРасчета = ПолучитьСпособРасчетаПоВидуНачисления(СтрокаТабличнойЧасти.ВидНачисления);
...
&НаКлиенте
Процедура ОтветНаВопросОчиститьТабличнуюЧастьНачисленияПриИзмененииПериодаРегистрации(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ПериодРегистрации = Объект.ПериодРегистрации;
		Объект.Начисления.Очистить();
	Иначе
...
&НаКлиенте
Процедура ОтветНаВопросОчиститьТабличнуюЧастьНачисленияПриИзмененииРазовоеНачисление(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		РазовоеНачисление = Объект.РазовоеНачисление;
		Объект.Начисления.Очистить();
		УстановитьВидимостьДоступностьЭлементов();
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	Если ЭтоРазовоеНачисление Тогда 
		...
		Элементы.НачисленияРазмер.ТолькоПросмотр = Ложь;
	Иначе 	
		...
		Элементы.НачисленияРазмер.ТолькоПросмотр = Истина;
	КонецЕсли;
КонецПроцедуры 
...
Процедура ВводМесяцаОбработкаВыбораЗавершение(ВыбранноеЗначение, ДополнительныеПараметры) Экспорт
	...
	// Значение реквизита не изменилось
	Если Объект.ПериодРегистрации = ПериодРегистрации Тогда 
		Возврат;
	КонецЕсли;	

	Если Объект.Начисления.Количество() > 0 Тогда 
		ОписаниеОповещения = Новый ОписаниеОповещения("ОтветНаВопросОчиститьТабличнуюЧастьНачисленияПриИзмененииПериодаРегистрации", ЭтотОбъект);
		ТекстВопроса = НСтр("ru = 'Табличная часть документа будет очищена. Продолжить выполнение операции?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 60);
	Иначе 
		ПериодРегистрации = Объект.ПериодРегистрации;
