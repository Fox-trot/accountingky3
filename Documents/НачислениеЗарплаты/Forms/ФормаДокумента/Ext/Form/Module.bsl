...
//
&НаКлиенте
Процедура РассчитатьТабличнуюЧастьНаКлиенте(РасчетПоКоманде = Ложь)
	Отказ = Ложь;
...
&НаКлиенте
Процедура ЗаполнитьПоВидуНачисления(ВидРасчета, Размер)
	Отказ = Ложь;
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.Начисления Цикл
		...
		СтрокаТабличнойЧасти.Размер = Размер;
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	Элементы.ГруппаВидРасчетаОтражение.Видимость= Не ЭтоРазовоеНачисление;
	...
	Элементы.НачисленияРазмер.Видимость 		= Не ЭтоРазовоеНачисление;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ВыборНачисления" Тогда
	 	ЗаполнитьПоВидуНачисления(Параметр.ВидНачисления, Параметр.СуммаНачисления);
	КонецЕсли;
КонецПроцедуры
...
Процедура Заполнить(Команда)
	Если Объект.Начисления.Количество() > 0 Тогда 
		...
		ТекстВопроса = НСтр("ru = 'Табличная часть документа будет перезаполнена и рассчитана автоматически! Продолжить выполнение операции?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 60);
	Иначе
...
Процедура Рассчитать(Команда)
	РассчитатьТабличнуюЧастьНаКлиенте(Истина);
КонецПроцедуры

&НаКлиенте
Процедура ЗаполнитьПоНачислениям(Команда)
	
	Отказ = Ложь;
	
	Если Объект.Начисления.Количество() > 0 Тогда 
		ОписаниеОповещения = Новый ОписаниеОповещения("ОтветНаВопросПерезаполнитьКолонки", ЭтотОбъект);
		ТекстВопроса = НСтр("ru = 'Колонки Вид начисления, Размер и Результат будут очищены! Продолжить выполнение операции?'");
		ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса, РежимДиалогаВопрос.ДаНет, 60);     
	Иначе
		ТекстСообщения = НСтр("ru = 'Не заполнена табличная часть ""Начисления""! Расчет документа отменен.'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Объект.Начисления",,Отказ);	
...
&НаКлиенте 
Процедура ОтветНаВопросПерезаполнитьКолонки(РезультатВопроса, ДополнительныеПараметры) Экспорт
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		ФормаНач = ОткрытьФорму("Документ.НачислениеЗарплаты.Форма.ФормаВыбораВидаНачислений",,ЭтаФорма);
	КонецЕсли;
КонецПроцедуры	
