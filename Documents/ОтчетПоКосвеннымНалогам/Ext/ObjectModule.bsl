...
Процедура ЗаполнитьПриложение8()
	...
	//Запрос.Текст = 
	//	"ВЫБРАТЬ
	//	|	СведенияОПодакционыхТоварах.ДатаУведомления КАК ДатаУведомления,
	//	|	СведенияОПодакционыхТоварах.НомерУведомления КАК НомерУведомления,
	//	|	СведенияОПодакционыхТоварах.ДатаФактическогоВвоза КАК ДатаФактическогоВвоза,
	//	|	СведенияОПодакционыхТоварах.КодСтраны КАК КодСтраны,
	//	|	СведенияОПодакционыхТоварах.ЕдиницаИзмерения КАК ЕдИзмер,
	//	|	СведенияОПодакционыхТоварах.ОблагаемыйОбъем КАК ОблагаемыйОбъем
	//	|ИЗ
	//	|	РегистрСведений.СведенияОПодакционыхТоварах КАК СведенияОПодакционыхТоварах
	//	|ГДЕ
	//	|	СведенияОПодакционыхТоварах.Организация = &Организация
	//	|	И СведенияОПодакционыхТоварах.ДатаУведомления МЕЖДУ &ДатаНачала И &ДатаОкончания";
	//
	//Запрос.УстановитьПараметр("ДатаНачала", НачалоМесяца(Дата));
	//Запрос.УстановитьПараметр("ДатаОкончания", КонецМесяца(Дата));
	//Запрос.УстановитьПараметр("Организация", Организация);
