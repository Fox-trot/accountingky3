...
Процедура ЗаполнитьПриложение1()
	...
	СТЧ.Содержание		 = НСтр("ru = 'Облагаемый импорт предметов лизинга'");
	
	СТЧ 				 = Объект.Приложение1.Добавить();
	СТЧ.НомерГрафыОтчета = "151";
	СТЧ.Содержание		 = НСтр("ru = 'Облагаемый импорт товаров, являющихся продуктами переработки давальческого сырья'");
	
	СТЧ 				 = Объект.Приложение1.Добавить();
	СТЧ.НомерГрафыОтчета = "152";
	СТЧ.Содержание		 = НСтр("ru = 'Импорт транспортных средств'");
	
	СТЧ 				 = Объект.Приложение1.Добавить();
	СТЧ.НомерГрафыОтчета = "153";
	СТЧ.Содержание		 = НСтр("ru = 'Прочий облагаемый импорт товаров , не указанных в строках 150, 151 и 152'");
	
	СТЧ 				 = Объект.Приложение1.Добавить();
	СТЧ.НомерГрафыОтчета = "154";
	СТЧ.Содержание		 = НСтр("ru = 'Итого облагаемый импорт (=150+151+152+153)'");
