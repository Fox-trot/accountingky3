...
Процедура УстановитьПараметрыДинамическихСписков()
	...
	Если НЕ Объект.Дата = '00010101' Тогда
		...
		Запрос.Текст =
			"ВЫБРАТЬ ПЕРВЫЕ 1
			|	удалитьОтчетыПоНалогамЗП.Ссылка,
			|	ВЫБОР
			|		КОГДА удалитьОтчетыПоНалогамЗП.ДатаСдачиОтчета = ДАТАВРЕМЯ(1, 1, 1)
			|			ТОГДА удалитьОтчетыПоНалогамЗП.Дата
			|		ИНАЧЕ удалитьОтчетыПоНалогамЗП.ДатаСдачиОтчета
			|	КОНЕЦ КАК ДатаНачалаЕжемесячныхПлатежей
			|ИЗ
			|	Документ.удалитьОтчетыПоНалогамЗП КАК удалитьОтчетыПоНалогамЗП
			|ГДЕ
			|	удалитьОтчетыПоНалогамЗП.Дата <= &ДатаНачалаОтчета
			|	И НЕ удалитьОтчетыПоНалогамЗП.ПометкаУдаления
			|
			|УПОРЯДОЧИТЬ ПО
			|	удалитьОтчетыПоНалогамЗП.Дата УБЫВ";
...
Функция ОтчетЕжемесячныйСоцфонд()
	...
	Макет = Документы.удалитьОтчетыПоНалогамЗП.ПолучитьМакет("ПФ_MXL_СФЕжемесячный_Excel");
