...
Процедура СотрудникиФизЛицоПриИзменении(Элемент)
	...
	СтрокаТабличнойЧасти.Размер = СведенияОСотруднике.РазмерОсновногоВидаРасчета;
...
Функция ПолучитьСведенияОСотруднике(Период, Организация, ФизЛицо)
	...
	Если НЕ СведенияОСотруднике.ВидРасчета.СпособРасчета = Перечисления.СпособыРасчетаНачислений.ПоЧасовойСтавке Тогда 
		...
		СведенияОСотруднике.РазмерОсновногоВидаРасчета = ?(НормаЧасов = 0, 0, СведенияОСотруднике.РазмерОсновногоВидаРасчета / НормаЧасов);
	КонецЕсли;	
...
Процедура РассчитатьНаСервере()
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.Сотрудники Цикл
		...
		Если КоличествоЧасов > 0 Тогда 
			Результат1 = ?(Коэффициент1 = 0, 0, СтрокаТабличнойЧасти.Размер * КоличествоЧасов * Коэффициент1);
		КонецЕсли;
		Результат2 = ?(Коэффициент2 = 0, 0, СтрокаТабличнойЧасти.Размер * ОстатокЧасов * Коэффициент2);
