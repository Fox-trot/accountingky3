...
Функция ПечатьПоПН(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст = 
		...
		|	ОтчетыПоНалогамЗП.ВидСубъекта КАК ВидСубъекта,
		|	ОтчетыПоНалогамЗП.ПодоходныйНалог.(
		|		Количество КАК Количество,
		|		СуммаНачислено КАК Начислено,
		|		СуммаНеоблагаемая КАК Необлагаемая,
		|		Вычеты КАК Вычеты,
		|		ОтчетыПоНалогамЗП.ПодоходныйНалог.СуммаНачислено - ОтчетыПоНалогамЗП.ПодоходныйНалог.СуммаНеоблагаемая - ОтчетыПоНалогамЗП.ПодоходныйНалог.Вычеты КАК Облагаемая,
		...
		|				ТОГДА 0
		|			ИНАЧЕ ОтчетыПоНалогамЗП.ПодоходныйНалог.СуммаВыплачен
		|		КОНЕЦ КАК Выплачено,
		|		СальдоК КАК СальдоК,
...
Функция ПечатьПриложенияКОтчетуПН(МассивОбъектов, ОбъектыПечати)	
	...
	Запрос.Текст =  
		...
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.Количество, 0) КАК Количество,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.СуммаНачислено, 0) КАК Начислено,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.СуммаНеоблагаемая, 0) КАК Необлагаемая,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.Вычеты, 0) КАК Вычеты,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.СуммаНачислено, 0) - ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.СуммаНеоблагаемая, 0) - ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.Вычеты, 0) КАК Облагаемая,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.СуммаПНсСотрудника, 0) КАК ПодоходныйСотрудника,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.СуммаПНсМРД, 0) КАК СМРД,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.СуммаПН, 0) КАК ПодоходныйКОплате,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.СуммаПН, 0) КАК ВсегоПодоходный,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.СуммаВыплачен, 0) КАК Выплачено,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.СальдоК, 0) КАК СальдоК,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.НомерСтрокиОтчета, 0) КАК НомерСтрокиОтчета
		|	) КАК ПодоходныйНалогПервыйМесяцКвартала,
		|	ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.(
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.Количество, 0) КАК Количество,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.СуммаНачислено, 0) КАК Начислено,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.СуммаНеоблагаемая, 0) КАК Необлагаемая,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.Вычеты, 0) КАК Вычеты,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.СуммаНачислено, 0) - ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.СуммаНеоблагаемая, 0) - ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.Вычеты, 0) КАК Облагаемая,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.СуммаПНсСотрудника, 0) КАК ПодоходныйСотрудника,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.СуммаПНсМРД, 0) КАК СМРД,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.СуммаПН, 0) КАК ПодоходныйКОплате,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.СуммаПН, 0) КАК ВсегоПодоходный,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.СуммаВыплачен, 0) КАК Выплачено,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.СальдоК, 0) КАК СальдоК,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.НомерСтрокиОтчета, 0) КАК НомерСтрокиОтчета
		|	) КАК ПодоходныйНалогВторойМесяцКвартала,
		|	ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.(
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.Количество, 0) КАК Количество,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.СуммаНачислено, 0) КАК Начислено,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.СуммаНеоблагаемая, 0) КАК Необлагаемая,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.Вычеты, 0) КАК Вычеты,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.СуммаНачислено, 0) - ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.СуммаНеоблагаемая, 0) - ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.Вычеты, 0) КАК Облагаемая,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.СуммаПНсСотрудника, 0) КАК ПодоходныйСотрудника,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.СуммаПНсМРД, 0) КАК СМРД,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.СуммаПН, 0) КАК ПодоходныйКОплате,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.СуммаПН, 0) КАК ВсегоПодоходный,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.СуммаВыплачен, 0) КАК Выплачено,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.СальдоК, 0) КАК СальдоК,
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.НомерСтрокиОтчета, 0) КАК НомерСтрокиОтчета
...
Функция ПечатьЕжемесячногоПоСФ(МассивОбъектов, ОбъектыПечати)
	...
	Запрос.Текст =	
		...
		|	ОтчетыПоНалогамЗП.Организация.НаименованиеГКЭД КАК ОрганизацияНаименованиеГКЭД,
