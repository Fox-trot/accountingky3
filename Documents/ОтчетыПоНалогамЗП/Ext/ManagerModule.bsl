...
Функция ПечатьПриложенияКОтчетуПН(МассивОбъектов, ОбъектыПечати)	
	...
	Запрос.Текст =  
		...
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогПервыйМесяцКвартала.СальдоК, 0) КАК ПодоходныйКОплате,
		...
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогВторойМесяцКвартала.СальдоК, 0) КАК ПодоходныйКОплате,
		...
		|		ЕСТЬNULL(ОтчетыПоНалогамЗП.ПодоходныйНалогТретийМесяцКвартала.СальдоК, 0) КАК ПодоходныйКОплате,
