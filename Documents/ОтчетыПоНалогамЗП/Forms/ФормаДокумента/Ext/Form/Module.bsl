...
Процедура ЕжемесячныйСоцфонд_Excel(Команда)
	...
	ВыгрузитьВExcelНаСервере(ИмяФайла);
КонецПроцедуры
...
&НаСервере
Процедура ВыгрузитьВExcelНаСервере(ИмяФайла)

	Попытка
		Макет = Документы.ОтчетыПоНалогамЗП.ПолучитьМакет("ОтчетЕжемесячныйПоСоцфонду");
		Оригинал = Макет.Получить();  
		
		РабочийФайл = Оригинал;
		
		Лист = РабочийФайл.WorkSheets(1);
		
		ДанныеОрганизации = ПолучитьДанныеОрганизации(Объект.Организация);
		
		Попытка
			Лист.Range("M5").Value 	= Формат(ДатаДокумента,"ДФ='ММММ гггг ''г.'''");
			Лист.Range("M6").Value 	= Сред(Формат(КонецМесяца(ДатаДокумента)+1,"ДЛФ=DD" ),3);
			Лист.Range("C8").Value 	= ДанныеОрганизации.НаименованиеПолное;
			Лист.Range("C9").Value 	= ДанныеОрганизации.ИНН;
			Лист.Range("C10").Value = ДанныеОрганизации.РегНомерСоцФонда;
			Лист.Range("C11").Value = ДанныеОрганизации.ОКПО;
			
			Лист.Range("I8").Value 	= ?(Объект.Контрагент.НаименованиеПолное <> "", Объект.Контрагент.НаименованиеПолное, Объект.Контрагент.Наименование);
			Лист.Range("I9").Value 	= ДанныеОрганизации.АдресОрганизации;
			Лист.Range("I10").Value = ДанныеОрганизации.Телефон;
			Лист.Range("I11").Value = ДанныеОрганизации.НаименованиеБанка;
			Лист.Range("I12").Value = ДанныеОрганизации.БанковскийСчет;
			Лист.Range("I13").Value = ДанныеОрганизации.ВидДеятельности;
		Исключение
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		КонецПопытки;
		
		// Вывод сведений о занятости.
		СуммаФот 	= 0;
		СуммаДопФот = 0;
		СуммаНСВ	= 0;
		СуммаНВ_НПФ	= 0;
		...
		Для Каждого СтрокаТабличнойЧасти Из Объект.СведенияОЗанятостиИЗаработнойПлате Цикл
			...
			Позиция   		  =  Формат(НачальнаяПозиция,"ЧГ=0");
			ПозицияИНН		  = "B" + Позиция;
			ПозияФИО  	      = "C" + Позиция;
			ПозияДатаН		  = "E" + Позиция;
			ПозияДатаK 		  = "F" + Позиция;
			ПозицияНормаДней  = "G" + Позиция;
			ПозицияФакДней 	  = "H" + Позиция;
			ПозицияФОТДней 	  = "I" + Позиция;
			ПозицияДопФОТДней = "J" + Позиция;
			ПозицияНачислено  = "K" + Позиция;
			ПозицияНачГНПФР   = "L" + Позиция;
			ПозицияКатегория  = "M" + Позиция;
			
			// Рамки
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(5).LineStyle  = -4142;
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(6).LineStyle  = -4142;
			
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(7).LineStyle  = 1;
            Лист.Range("A" + Позиция + ":N" + Позиция).Borders(7).Weight 	 = 2;
			
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(8).LineStyle  = 1;
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(8).Weight 	 = 2;
			
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(9).LineStyle  = 1;
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(9).Weight 	 = 2;
			
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(10).LineStyle = 1;
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(10).Weight 	 = 2;
			
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(11).LineStyle = 1;
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(11).Weight 	 = 2;
			
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(12).LineStyle = 1;
			Лист.Range("A" + Позиция + ":N" + Позиция).Borders(12).Weight 	 = 2;

			Лист.Range("A" + Позиция).Value 	= Счетчик;
			Лист.Range(ПозияФИО).Value 			= СтрокаТабличнойЧасти.Физлицо.Наименование;
			Лист.Range(ПозицияИНН).Value 		= СтрокаТабличнойЧасти.ФизЛицо.ИНН;
			Лист.Range(ПозияДатаН).Value 		= Формат(СтрокаТабличнойЧасти.ДатаНачалаРаботы,"ДЛФ=D");
			Лист.Range(ПозияДатаK).Value 		= Формат(СтрокаТабличнойЧасти.ДатаОкончанияРаботы,"ДЛФ=D");
			Лист.Range(ПозицияНормаДней).Value  = СтрокаТабличнойЧасти.Дней;
			Лист.Range(ПозицияФакДней).Value    = СтрокаТабличнойЧасти.ФактическиДней;
			Лист.Range(ПозицияФОТДней).Value 	= СтрокаТабличнойЧасти.ФондОплатыТруда;
			Лист.Range(ПозицияДопФОТДней).Value = СтрокаТабличнойЧасти.ДополнительныйФондОплатыТруда;
			Лист.Range(ПозицияНачислено).Value  = СтрокаТабличнойЧасти.НачисленыеСтраховыеВзносы;
			Лист.Range(ПозицияНачГНПФР).Value   = СтрокаТабличнойЧасти.НачсиленыеВзносыПоНПФ;
			Лист.Range(ПозицияКатегория).Value 	= СтрокаТабличнойЧасти.Категория.Код;		
			
			СуммаФот 	= СуммаФот 		+ СтрокаТабличнойЧасти.ФондОплатыТруда;
			СуммаДопФот = СуммаДопФот 	+ СтрокаТабличнойЧасти.ДополнительныйФондОплатыТруда;
			СуммаНСВ	= СуммаНСВ 		+ СтрокаТабличнойЧасти.НачисленыеСтраховыеВзносы;
			СуммаНВ_НПФ	= СуммаНВ_НПФ 	+ СтрокаТабличнойЧасти.НачсиленыеВзносыПоНПФ;
			Если Не Счетчик = Объект.СведенияОЗанятостиИЗаработнойПлате.Количество() Тогда 
				Лист.Rows(НачальнаяПозиция + 1).insert(1);
			КонецЕсли;
		КонецЦикла;
		
		НачальнаяПозиция = НачальнаяПозиция + 1;
		Лист.Range("I" + НачальнаяПозиция).Value = СуммаФот;
		Лист.Range("J" + НачальнаяПозиция).Value = СуммаДопФот;
		Лист.Range("K" + НачальнаяПозиция).Value = СуммаНСВ;
		Лист.Range("L" + НачальнаяПозиция).Value = СуммаНВ_НПФ;

		НачальнаяПозицияФОТПоКатегориям = НачальнаяПозиция + 6;
		ПозицияИтоговФОТПоКатегориям 	= НачальнаяПозицияФОТПоКатегориям + 4;
		Счетчик 						= 0;		
		СуммаЧисленностьРаботников		= 0;
		СуммаФотМеньше40 				= 0;
		СуммаФотБольше40 				= 0;
		СуммаДопФОТ 					= 0;
		СуммаИтог 						= 0;
		...
		Для Каждого СтрокаТабличнойЧасти Из Объект.ФондОплатыТрудаПоКатегориям Цикл
			Счетчик = Счетчик +1;
			НачальнаяПозицияФОТПоКатегориям = НачальнаяПозицияФОТПоКатегориям + 1;
			Позиция = Формат(НачальнаяПозицияФОТПоКатегориям,"ЧГ=0");
			ПозицияКатегории  			 = "B" + Формат(НачальнаяПозицияФОТПоКатегориям,"ЧГ=0");
			ПозицияЧисленности 			 = "C" + Формат(НачальнаяПозицияФОТПоКатегориям,"ЧГ=0");
			ПозицияФотМеньше40 			 = "D" + Формат(НачальнаяПозицияФОТПоКатегориям,"ЧГ=0");
			ПозицияФотБольше40 			 = "E" + Формат(НачальнаяПозицияФОТПоКатегориям,"ЧГ=0");
			ПозицияДопФОТ	  			 = "F" + Формат(НачальнаяПозицияФОТПоКатегориям,"ЧГ=0");
			ПозицияИтоговФОТПоКатегориям = "G" + Формат(НачальнаяПозицияФОТПоКатегориям,"ЧГ=0");
			
			Лист.Range("A" + Позиция).Value 				= Счетчик;
			Лист.Range(ПозицияКатегории).Value 				= СокрЛП(СтрокаТабличнойЧасти.Категория);
			Лист.Range(ПозицияЧисленности).Value 			= СтрокаТабличнойЧасти.Численость;
			Лист.Range(ПозицияФотМеньше40).Value			= СтрокаТабличнойЧасти.ФОТМенее;
			Лист.Range(ПозицияФотБольше40).Value			= СтрокаТабличнойЧасти.ФОТБолее;
			Лист.Range(ПозицияДопФОТ).Value					= СтрокаТабличнойЧасти.ДопФОТ;
			Лист.Range(ПозицияИтоговФОТПоКатегориям).Value	= СтрокаТабличнойЧасти.ДопФОТ + СтрокаТабличнойЧасти.ФОТБолее + СтрокаТабличнойЧасти.ФОТМенее;
			
			СуммаЧисленностьРаботников 	= СуммаЧисленностьРаботников + СтрокаТабличнойЧасти.Численость;
			СуммаФотМеньше40 			= СуммаФотМеньше40 + СтрокаТабличнойЧасти.ФОТМенее;
			СуммаФотБольше40 			= СуммаФотБольше40 + СтрокаТабличнойЧасти.ФОТБолее;
			СуммаДопФОТ 				= СуммаДопФОТ + СтрокаТабличнойЧасти.ДопФОТ;
			СуммаИтог 					= СуммаИтог	+ СтрокаТабличнойЧасти.ДопФОТ + СтрокаТабличнойЧасти.ФОТБолее + СтрокаТабличнойЧасти.ФОТМенее;

			Если Не Счетчик = Объект.ФондОплатыТрудаПоКатегориям.Количество() Тогда 
				Лист.Rows(Формат(НачальнаяПозицияФОТПоКатегориям + 1,"ЧГ=0")).insert(1);
			КонецЕсли;
		КонецЦикла;
		
		НачальнаяПозицияФОТПоКатегориям = НачальнаяПозицияФОТПоКатегориям + 1;
		Лист.Range("C" + НачальнаяПозицияФОТПоКатегориям).Value = СуммаЧисленностьРаботников;
		Лист.Range("D" + НачальнаяПозицияФОТПоКатегориям).Value = СуммаФотМеньше40;
		Лист.Range("E" + НачальнаяПозицияФОТПоКатегориям).Value = СуммаФотБольше40;
		Лист.Range("F" + НачальнаяПозицияФОТПоКатегориям).Value = СуммаДопФОТ;
		Лист.Range("G" + НачальнаяПозицияФОТПоКатегориям).Value = СуммаИтог;
		
		ПозицияРВ = НачальнаяПозицияФОТПоКатегориям + 18;
		Лист.Range("D" + Формат(ПозицияРВ,"ЧГ=0")).Value 		= Объект.ОбязательстваПоСтраховымВзносам;
		Лист.Range("D" + Формат(ПозицияРВ + 11,"ЧГ=0")).Value 	= Объект.ПереплатаПоСтраховымВзносам;
		Лист.Range("D" + Формат(ПозицияРВ + 1,"ЧГ=0")).Value 	= Объект.ОбязательстваПоПенсионномуФонду;
		Лист.Range("D" + Формат(ПозицияРВ + 12,"ЧГ=0")).Value 	= Объект.ПереплатаПоПенсионномуФонду;
		Лист.Range("D" + Формат(ПозицияРВ + 10,"ЧГ=0")).Value 	= Объект.ОбязательстваПоСтраховымВзносам + Объект.ОбязательстваПоПенсионномуФонду;
		Лист.Range("D" + Формат(ПозицияРВ + 14,"ЧГ=0")).Value 	= Объект.ПереплатаПоСтраховымВзносам + Объект.ПереплатаПоПенсионномуФонду;
		
		Руководители = БухгалтерскийУчетСервер.ОтветственныеЛицаОрганизаций(Объект.Организация, ДатаДокумента);
		ПозицияРук = ПозицияРВ + 19;
		Лист.Range("D" + Формат(ПозицияРук,"ЧГ=0")).Value 		= Руководители.Руководитель;		
		Лист.Range("D" + Формат(ПозицияРук + 6,"ЧГ=0")).Value 	= Руководители.ГлавныйБухгалтер;
		
		РабочийФайл.Windows(РабочийФайл.Name).Visible = True;
		
		РабочийФайл.SaveAs(ИмяФайла);
		ТекстСообщения = СтрШаблон(НСтр("ru = 'Расчетная ведомость по средствам государственного социального страхования сохранена по адресу: %1.'"), СокрЛП(ИмяФайла));
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения);					
		
	Исключение
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(КраткоеПредставлениеОшибки(ИнформацияОбОшибке()));
		РабочийФайл.Application.Quit();		
	КонецПопытки;	
КонецПроцедуры
...
Функция ОтчетЕжемесячныйСоцфонд()
	...
	ДанныеПечати.Вставить("Адрес", 		 ДанныеОрганизации.АдресОранизации);
