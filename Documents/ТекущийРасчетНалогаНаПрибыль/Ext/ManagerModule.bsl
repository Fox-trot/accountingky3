...
Функция ПечатьНалогаНаПрибыль(МассивОбъектов, ОбъектыПечати)	
	...
	Пока Шапка.Следующий() Цикл
		...
		СуммаЯчейка205 = Шапка.СуммаДоходов - Шапка.СуммаРасходов; // ?(Шапка.СуммаДоходов - Шапка.СуммаРасходов > 0, Шапка.СуммаДоходов - Шапка.СуммаРасходов, 0);
		
		ОбластьШапка.Параметры.Ячейка205 = СуммаЯчейка205;
		ОбластьШапка.Параметры.Ячейка206 = Строка(Шапка.СтавкаНалогаНаПрибыль) + "%";
		
		СуммаЯчейка207 = СуммаЯчейка205 / 100 * Шапка.СтавкаНалогаНаПрибыль; // ?(СуммаЯчейка205 > 0, СуммаЯчейка205 / 100 * Шапка.СтавкаНалогаНаПрибыль, 0);
