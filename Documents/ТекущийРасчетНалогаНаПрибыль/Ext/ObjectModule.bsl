...
Процедура ОтразитьДвижения(Отказ, РежимПроведения)
	...
	БухгалтерскийУчетСервер.ОтразитьПлатежиПоНалогуНаПрибль(ДополнительныеСвойства, Движения, Отказ);
...
Процедура ВыполнитьПредварительныйКонтроль(Отказ) 
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПлатежиПоНалогуНаПрибль.Регистратор КАК Регистратор
		|ИЗ
		|	РегистрСведений.ПлатежиПоНалогуНаПрибль КАК ПлатежиПоНалогуНаПрибль
		|ГДЕ
		|	ПлатежиПоНалогуНаПрибль.Организация = &Организация
		|	И ПлатежиПоНалогуНаПрибль.Период = &Период
		|	И НЕ ПлатежиПоНалогуНаПрибль.Регистратор = &Ссылка";
...
// Процедура производит расчет суммы начисления налога на прибль
//
Процедура Рассчитать() Экспорт 
	...
	СуммаНП = База * СтавкаНалогаНаПрибыль / 100;
	...
	Если НЕ НачалоГода(Дата) = НачалоКвартала(Дата) Тогда 
		...
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	ПлатежиПоНалогуНаПрибльСрезПоследних.Сумма КАК Сумма
			|ИЗ
			|	РегистрСведений.ПлатежиПоНалогуНаПрибль.СрезПоследних(
			|			&Период,
			|			Организация = &Организация
			|				И НАЧАЛОПЕРИОДА(Период, ГОД) = НАЧАЛОПЕРИОДА(&Период, ГОД)
			|				И НЕ Регистратор = &Ссылка) КАК ПлатежиПоНалогуНаПрибльСрезПоследних";
		Запрос.УстановитьПараметр("Организация", Организация);
		Запрос.УстановитьПараметр("Период", КонецКвартала(Дата));
		Запрос.УстановитьПараметр("Ссылка", Ссылка);
