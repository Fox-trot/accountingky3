...
Функция ПечатьПриказаКомандировка(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент.КлючПараметровПечати 	= "ПАРАМЕТРЫ_ПЕЧАТИ_Командировка_ПриказКомандировка";
	ТабличныйДокумент.ИмяПараметровПечати 	= "Командировка_Приказ";
	...
	Пока Шапка.Следующий()  Цикл
		...
		Для каждого СтрокаТаблицыЗначений Из МассивСтрок Цикл
			...
			ОбластьМакета.Параметры.ДатаНачала		= Формат(Шапка.ДатаНачала,"ДФ=dd.MM.yyyy");
			ОбластьМакета.Параметры.ДатаОкончания	= Формат(Шапка.ДатаОкончания,"ДФ=dd.MM.yyyy");
...
Функция ПечатьКомандировочногоУдостоверения(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати 	= "ПАРАМЕТРЫ_ПЕЧАТИ_Командировка_КомандировочноеУдостоверение";
	ТабличныйДокумент.ИмяПараметровПечати 	= "Командировка_КомандировочноеУдостоверение";
	...
	Пока Шапка.Следующий()  Цикл
		...
		Для каждого СтрокаТаблицыЗначений Из МассивСтрок Цикл
			...
			ОбластьМакета.Параметры.Заполнить(СтрокаТаблицыЗначений);
			ОбластьМакета.Параметры.Заполнить(Шапка);
			ОбластьМакета.Параметры.НомерДокумента 		= ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Шапка.Номер) + "/" + СтрокаТаблицыЗначений.НомерСтроки;
			ОбластьМакета.Параметры.ДатаДокумента		= Формат(Шапка.Дата,"ДЛФ=DD");
			ОбластьМакета.Параметры.ТабельныйНомер		= ПрефиксацияОбъектовКлиентСервер.УдалитьЛидирующиеНулиИзНомераОбъекта(СтрокаТаблицыЗначений.ТабНомер);	
...
Функция ПечатьПлатежнаяВедомость(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати 	= "ПАРАМЕТРЫ_ПЕЧАТИ_Командировка_ПлатежнаяВедомость";
	ТабличныйДокумент.ИмяПараметровПечати 	= "Командировка_ПлатежнаяВедомость";
	...
	Пока Шапка.Следующий()  Цикл
		...
		ОбластьМакета.Параметры.Заполнить(Шапка);
		ОбластьМакета.Параметры.Номер = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Шапка.Номер);
		...
		ОбластьМакета.Параметры.РКО				= ПолучитьСписокРКО(ТаблицаРКО);
...
Функция ПечатьРаспоряжениеНаВыполнениеЗадания(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати 	= "ПАРАМЕТРЫ_ПЕЧАТИ_Командировка_РаспоряжениеНаВыполнениеЗадания";
	ТабличныйДокумент.ИмяПараметровПечати 	= "Командировка_РаспоряжениеНаВыполнениеЗадания";
...
Функция ПечатьПриказаОКомандировании(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати 	= "ПАРАМЕТРЫ_ПЕЧАТИ_Командировка_ПриказОКомандировании";
	ТабличныйДокумент.ИмяПараметровПечати 	= "Командировка_ПриказОКомандировании";
...
Функция ПолучитьСписокРКО(ТаблицаРКО)
	Первый = Истина;
	Стр = "";
	Для каждого СтрокаТаблицыЗначений Из ТаблицаРКО Цикл
		Стр = Стр + ?(Первый, "", ", ") + СтрокаТаблицыЗначений.НомерРКО;
		Первый = Ложь;
	КонецЦикла;	
