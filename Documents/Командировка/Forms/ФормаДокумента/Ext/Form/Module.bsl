...
Процедура ПодразделениеПриИзменении(Элемент)
	Если ЗначениеЗаполнено(Объект.Подразделение) И Подразделение <> Объект.Подразделение Тогда
		Подразделение = Объект.Подразделение;
		КоличествоСтрок = Объект.Сотрудники.Количество();
		Для Счетчик = 0 По КоличествоСтрок - 1 Цикл
			СтрокаТабличнойЧасти = Объект.Сотрудники[КоличествоСтрок - 1 - Счетчик];
			Если СтрокаТабличнойЧасти.Подразделение <> Подразделение Тогда
				Объект.Сотрудники.Удалить(СтрокаТабличнойЧасти);
			КонецЕсли; 
		КонецЦикла;
	КонецЕсли;
КонецПроцедуры
...
#Область ОбработчикиТабличнойЧастиСотрудники
