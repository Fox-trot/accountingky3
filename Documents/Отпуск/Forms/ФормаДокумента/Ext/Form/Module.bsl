...
Процедура РасчитатьСреднийЗаработок(СообщатьОбОшибке = Ложь, Отказ = Ложь)
	// Коэффициент расчета
	КоэффициентРасчета = КоэффициентРасчета(Объект.МетодРасчета);
	...
	Если КоличествоМесяцевСреднийЗаработок = 0 Тогда 
		...
		КонецЕсли;	
	Иначе 
		РазмерСреднийЗаработок = РезультатСреднийЗаработок / КоличествоМесяцевСреднийЗаработок / КоэффициентРасчета;
	КонецЕсли;
КонецПроцедуры
...
Функция ПроверитьЗаполнениеМетодаРасчета()
	Если НЕ (ЗначениеЗаполнено(Объект.МетодРасчета.НеполныеМесяцы)
		Или ЗначениеЗаполнено(Объект.МетодРасчета.ВидРасчета)
		Или ЗначениеЗаполнено(Объект.МетодРасчета.КоэффициентРасчета)) Тогда
		Возврат Ложь;
	КонецЕсли;	
...
Процедура ФизЛицоПриИзмененииНаСервере()
	...
	// Использование отдельного графика работа
	ДанныеУчетнойПолитики = БухгалтерскийУчетСервер.ПолучитьДанныеУчетнойПолитикиПоПерсоналу(Объект.Дата, Объект.Организация);
	Если ЗначениеЗаполнено(ДанныеУчетнойПолитики.ГрафикДляРасчетаОтпусков) Тогда 
		Объект.ГрафикРаботы = ДанныеУчетнойПолитики.ГрафикДляРасчетаОтпусков;
	КонецЕсли;	
КонецПроцедуры
