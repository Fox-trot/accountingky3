...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Иначе 
		МассивНепроверяемыхРеквизитов.Добавить("СпособОтражения");	
	КонецЕсли;	
...
Процедура ЗаполнитьТабличнуюЧасть() Экспорт 
	...
	// Дозаполнение коэффициента расчета.
	КоэффициентРасчета = МетодРасчета.КоэффициентРасчета;
	
	Для Каждого СтрокаТабличнойЧасти Из СреднийЗаработок Цикл 
		СтрокаТабличнойЧасти.КоэффициентРасчета = КоэффициентРасчета;
	КонецЦикла;	
...
Процедура РассчитатьТабличнуюЧасть(РазмерСреднийЗаработок) Экспорт
	...
	Иначе 
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "&РасчетПоРабочимДням", 
			"НЕ ДанныеПроизводственногоКалендаря.ВидДня = ЗНАЧЕНИЕ(Перечисление.ВидыДнейПроизводственногоКалендаря.Праздник)");
	КонецЕсли;
