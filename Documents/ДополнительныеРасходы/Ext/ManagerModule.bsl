...
Процедура СформироватьТаблицаПоступлениеТоваров(ДокументСсылка, СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		|	ВЫРАЗИТЬ((ВременнаяТаблицаТовары.СуммаРасходов - ВременнаяТаблицаТовары.СуммаРасходовНДС) * ВременнаяТаблицаШапка.Курс / ВременнаяТаблицаШапка.Кратность КАК ЧИСЛО(15, 2)) КАК СуммаСебестоимости,
