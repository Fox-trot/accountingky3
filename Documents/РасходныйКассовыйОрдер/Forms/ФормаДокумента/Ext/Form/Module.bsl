...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	Если ТипЗнч(ВыбранноеЗначение) = ТИП("ДокументСсылка.ВедомостьЗП") 
		ИЛИ ТипЗнч(ВыбранноеЗначение) = ТИП("ДокументСсылка.ВыплатаЗП") Тогда
...
Процедура ПеречислениеЗПБезПодтчетникаНаСервере(Ведомость)
	...
	Запрос.Текст = 
		...
		|	ВыплатаЗПЗарплата.СуммаКВыплате - ЕСТЬNULL(УжеВыплаченоПоЭтойВедомости.Сумма, 0) КАК СуммаПлатежа,
		|	&Ведомость,
		|	ВыплатаЗПЗарплата.Ссылка.СтатьяДвиженияДенежныхСредств
		|ИЗ
		|	Документ.ВыплатаЗП.Зарплата КАК ВыплатаЗПЗарплата
		|		ЛЕВОЕ СОЕДИНЕНИЕ УжеВыплаченоПоЭтойВедомости КАК УжеВыплаченоПоЭтойВедомости
		|		ПО ВыплатаЗПЗарплата.ФизЛицо = УжеВыплаченоПоЭтойВедомости.Физлицо
		|ГДЕ
		|	ВыплатаЗПЗарплата.Ссылка = &Ведомость";
...
// СтандартныеПодсистемы.ЗапретРедактированияРеквизитов
&НаКлиенте
...
КонецПроцедуры
// Конец СтандартныеПодсистемы.ЗапретРедактированияРеквизитов
