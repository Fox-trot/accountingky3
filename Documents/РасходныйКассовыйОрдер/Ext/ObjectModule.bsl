...
Функция ПересчетСуммыДокумента() Экспорт
...
Процедура ЗаполнитьПоВедомостиЗП(ДанныеЗаполнения, ВидДокументаВыплаты = 1) Экспорт
	...
	Если НЕ ТипЗнч(ДанныеЗаполнения)= Тип("Структура") Тогда 
		...
		Операция = ПредопределенноеЗначение("Справочник.ОперацииРКО.ВыплатаЗаработнойПлатыПоВедомостям"); 
		...
		НазваниеДокумента = "Ведомость ЗП";
		НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ДанныеЗаполнения.Номер);
	...
	Иначе 
		...
		Операция = ПредопределенноеЗначение("Справочник.ОперацииРКО.ВыплатаЗаработнойПлатыСотруднику"); 
		...
		НазваниеДокумента = "Ведомость ЗП";
		НомерНаПечать = ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(ДанныеЗаполнения.Ведомость.Номер);
		...
		Если ВыплатаЗаработнойПлаты.Количество() > 0 И ЗначениеЗаполнено(ВыплатаЗаработнойПлаты[0].Ведомость) Тогда
			Для каждого СтрокаТабличнойЧасти Из ВыплатаЗаработнойПлаты Цикл
				Если НЕ ЗначениеЗаполнено(СтрокаТабличнойЧасти.Ведомость) Тогда
					...
					Сообщение.Текст = "Необходимо заполнить Ведомость";
