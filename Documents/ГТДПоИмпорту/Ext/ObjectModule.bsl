	...
	Для Каждого СтрокаТабличнойЧасти Из Разделы Цикл
		...
		Для Каждого СтрокаМассива Из МассивСтрокТоваров Цикл
			...
			Если СтрокаТабличнойЧасти.НДСТолькоНаПошлину Тогда
				БазаНДСИтог = БазаНДСИтог + (ФактурнаяСтоимость + СтрокаМассива.Пошлина + СтрокаМассива.ДопРасходы + СтрокаМассива.Акциз);
			Иначе
				БазаНДСИтог = БазаНДСИтог + (ФактурнаяСтоимость + СтрокаМассива.ДопРасходы + СтрокаМассива.Акциз);	
			КонецЕсли; 
		...
		Для Каждого СтрокаМассива Из МассивСтрокОС Цикл
			...
			СтрокаМассива.ТаможенныйСбор	 = Окр(ТаможеннаяСтоимость * СтрокаТабличнойЧасти.СтавкаСбора / 100, 2);
			СтрокаМассива.Пошлина			 = Окр(ТаможеннаяСтоимость * СтрокаТабличнойЧасти.СтавкаПошлины / 100, 2);
			БазаНДСИтог 			 		 = БазаНДСИтог + (СтрокаМассива.Пошлина + СтрокаМассива.ДопРасходы);
		...
		СтрокаТабличнойЧасти.БазаНДС = БазаНДСИтог;
		СтрокаТабличнойЧасти.НДС = Окр(БазаНДСИтог * СтрокаТабличнойЧасти.СтавкаНДС / 100, 2);
