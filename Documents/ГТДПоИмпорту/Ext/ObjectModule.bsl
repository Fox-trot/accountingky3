...
Процедура ПересчетСборовПошлинНалогов()
	...
	Для Каждого СтрокаТабличнойЧасти Из Разделы Цикл
		...
		Для Каждого СтрокаМассива Из МассивСтрокТоваров Цикл
			...
			Если РаспределениеПошлины = Перечисления.СпособыРаспределенияПошлины.ПоСумме Тогда
				СтрокаМассива.Пошлина 		 = Окр(ТаможеннаяСтоимость * СтрокаТабличнойЧасти.СтавкаПошлины / 100, 2);
			Иначе
				СтрокаМассива.Пошлина 		 = Окр(СтрокаМассива.Количество * СтрокаТабличнойЧасти.СтавкаПошлины / 100, 2);
			КонецЕсли;	
			...
			ПошлинаИтог = ПошлинаИтог + СтрокаМассива.Пошлина;
		КонецЦикла;	
		...
		Для Каждого СтрокаМассива Из МассивСтрокОС Цикл
			...
			Если РаспределениеПошлины = Перечисления.СпособыРаспределенияПошлины.ПоСумме Тогда
				СтрокаМассива.Пошлина 		 = Окр(ТаможеннаяСтоимость * СтрокаТабличнойЧасти.СтавкаПошлины / 100, 2);
			Иначе
				СтрокаМассива.Пошлина 		 = Окр(СтрокаТабличнойЧасти.СтавкаПошлины / 100, 2);
			КонецЕсли;
		...
		Если СтрокаТабличнойЧасти.НДСТолькоНаПошлину Тогда
			СтрокаТабличнойЧасти.БазаНДС = ПошлинаИтог;
		Иначе
			СтрокаТабличнойЧасти.БазаНДС = ФактурнаяСтоимостьИтог + ПошлинаИтог + ДопрасходыИтог + АкцизИтог;
		КонецЕсли;
