...
Процедура СформироватьТаблицаСведенияОПоступлении(СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		|	ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ИмпортЭкспорт) КАК ПризнакСтраны,
		...
		|	ВременнаяТаблицаСтроки.ДатаПоставки,
		|	ВременнаяТаблицаСтроки.ДатаСФ,
		|	ЗНАЧЕНИЕ(Перечисление.ПризнакиСтраны.ИмпортЭкспорт),
		|	ВременнаяТаблицаСтроки.ЗачетНДС";
...
Процедура СформироватьТаблицаРеестрВвезенных(СтруктураДополнительныеСвойства) 
	...
	Запрос.Текст =
		...
		|	СУММА(ВЫБОР
		|			КОГДА &УказыватьПризнакЗачетаНДС
		|					ИЛИ ВременнаяТаблицаРазделы.ЗачетНДС <> ЗНАЧЕНИЕ(Перечисление.ВидыЗачетаНДС.Себестоимость)
		|				ТОГДА ВременнаяТаблицаРазделы.НДС
		|			ИНАЧЕ 0
