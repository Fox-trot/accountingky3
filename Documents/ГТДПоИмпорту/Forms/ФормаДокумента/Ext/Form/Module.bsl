...
Функция ПроверитьЗаполнениеПередРасчетом()
	...
	Если Объект.Разделы.Количество() = 1 Тогда
		...
		Если СтрокаТабличнойЧасти.СтавкаПошлины <> 0 И 
			Объект.РаспределениеПошлины = Перечисления.СпособыРаспределенияПошлины.ПустаяСсылка() Тогда
				
			ТекстСообщения = НСтр("ru = 'В разделе заполнена ставка пошлины, но не указан способ распределения! Рассчет отменен.'");
			ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения,,"Объект.РаспределениеПошлины",,Отказ);
		КонецЕсли;
