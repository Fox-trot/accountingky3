...
Процедура ДобавитьПоступление(Команда)	
	...
	Для Каждого СтрокаТабличнойЧасти Из Объект.Товары Цикл
		МассивПоступлений.Добавить(СтрокаТабличнойЧасти.ДокументПоступления);		
	КонецЦикла;
...
Функция ЗаполнитьДопрасходыИВернутьСумму(МассивДокументов, КлючСвязи)
	...
	Пока Выборка.Следующий() Цикл
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Если ДоговорОтбор.ВалютаРасчетов <> ВалютаРегламентированногоУчета Тогда
				Отбор.Вставить("ФактурнаяСтоимость", ВыборкаДетальныеЗаписи.Сумма);
			Иначе	
...
Функция ПолучитьТекстЗапросаДопРасходы()
	...
	|	ВЫБОР
	|		КОГДА ДополнительныеРасходыТовары.Ссылка.ЗачетНДС = ЗНАЧЕНИЕ(Перечисление.ВидыЗачетаНДС.Себестоимость)
	|			ТОГДА ДополнительныеРасходыТовары.СуммаРасходов + ДополнительныеРасходыТовары.СуммаРасходовНДС
	|		ИНАЧЕ ДополнительныеРасходыТовары.СуммаРасходов
	...
	|	ВЫБОР
	|		КОГДА ДополнительныеРасходыОС.Ссылка.ЗачетНДС = ЗНАЧЕНИЕ(Перечисление.ВидыЗачетаНДС.Себестоимость)
	|			ТОГДА ДополнительныеРасходыОС.СуммаРасходов + ДополнительныеРасходыОС.СуммаРасходовНДС
	|		ИНАЧЕ ДополнительныеРасходыОС.СуммаРасходов
	|	КОНЕЦ,
