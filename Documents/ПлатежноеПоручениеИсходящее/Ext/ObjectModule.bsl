...
Процедура ЗаполнитьПоВедомостиЗП(ДанныеЗаполнения, ВидДокументаВыплаты = 1) Экспорт
	...
	Если НЕ ТипЗнч(ДанныеЗаполнения)= Тип("Структура") Тогда 
		...
		Операция = Справочники.ОперацииППИ.ПеречислениеЗаработнойПлаты; 
	...
	Иначе 
		...
		Операция = Справочники.ОперацииППИ.ПеречислениеЗаработнойПлатыСотруднику; 
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ ЗначениеЗаполнено(ВидОперации) Тогда 
		ТекстСообщения = НСтр("ru = 'Не заполнен вид операции в поле ""Операция"".'");
		ОбщегоНазначенияКлиентСервер.СообщитьПользователю(ТекстСообщения, , "Операция", , Отказ);
		
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийППИ.ОплатаПоставщику") 
