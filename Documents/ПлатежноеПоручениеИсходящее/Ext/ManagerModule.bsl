...
Процедура СформироватьТаблицаВыплаченнаяЗарплата(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =		     
		...
		|	ВременнаяТаблицаШапка.Организация,
		|	ВременнаяТаблицаВыплатаЗаработнойПлаты.ФизЛицо,
		|	ВременнаяТаблицаВыплатаЗаработнойПлаты.Ведомость КАК Ведомость,
...
Функция ПечатьПлатежноеПоручение(МассивОбъектов, ОбъектыПечати)
	...
	ТекстЗапроса = 
		...
		// Реквизиты платежа.
		|	ПлатежноеПоручениеИсходящее.КодНазначенияПлатежа.Код КАК КодНазначенияПлатежа,
	...
	Пока Шапка.Следующий() Цикл
		...
		ДанныеПечати.Вставить("КодНазначенияПлатежа", Шапка.КодНазначенияПлатежа);
		...
		// Подписи
		ДанныеПечати.Вставить("ВтораяПодпись", ?(Шапка.ВтораяПодписьНеПредусмотрена, 
			НСтр("ru = 'Вторая подпись не предусмотрена'"), 
