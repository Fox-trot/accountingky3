...
Процедура СформироватьТаблицаХозрасчетный(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|		ЛЕВОЕ СОЕДИНЕНИЕ ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|		ПО ВременнаяТаблицаНалоги.Ссылка = ВременнаяТаблицаШапка.Ссылка
		|ГДЕ
		...
		|	ВедомостьНаВыплатуЗарплатыЗарплата.Ссылка,
		//|	ЗНАЧЕНИЕ(ПланВидовРасчета.ВидыУдержаний.УдержанияДляПеречисления),
		|	ЗНАЧЕНИЕ(ПланВидовРасчета.ВидыУдержаний.ПустаяСсылка),
		|	ВременнаяТаблицаШапка.БанковскийСчет,
		...
		|	ВременнаяТаблицаШапка.ДокументОснование,
		//|	ЗНАЧЕНИЕ(ПланВидовРасчета.ВидыУдержаний.УдержанияДляПеречисления),
		|	ЗНАЧЕНИЕ(ПланВидовРасчета.ВидыУдержаний.ПустаяСсылка),
		|	ВременнаяТаблицаШапка.БанковскийСчет,
		|	ВременнаяТаблицаЗП.СтатьяДвиженияДенежныхСредств,
		...
		|	ВременнаяТаблицаШапка.Организация,
		|	ВременнаяТаблицаШапка.Операция,
		|	ВременнаяТаблицаЗП.СуммаПлатежа * ВременнаяТаблицаШапка.Курс,
		|	0,
		|	0
		|ИЗ
		|	ВременнаяТаблицаЗП КАК ВременнаяТаблицаЗП
		...
		|ГДЕ
		|	ВременнаяТаблицаШапка.Операция.ВидОперации = ЗНАЧЕНИЕ(Перечисление.ВидыОперацийППИ.ВыплатаЗП)
		|	И (НЕ ВременнаяТаблицаШапка.ПоВедомости
		|			ИЛИ ВременнаяТаблицаШапка.ПоВедомости
		|				И НЕ ТИПЗНАЧЕНИЯ(ВременнаяТаблицаЗП.Ведомость) = ТИП(Документ.ВедомостьЗП))
		|	И ВременнаяТаблицаЗП.СуммаПлатежа <> 0
		|
		|ОБЪЕДИНИТЬ ВСЕ
		|
...
Процедура СформироватьТаблицаВыплаченнаяЗарплата(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =		
		...
		|	ВЫРАЗИТЬ(ВременнаяТаблицаШапка.ДокументОснование КАК Документ.ВедомостьЗП).ПериодРегистрации КАК ПериодРегистрации,
		...
		|	ВременнаяТаблицаШапка.Организация,
		|	ВедомостьНаВыплатуЗарплатыЗарплата.ФизЛицо,
		|	ВременнаяТаблицаЗП.Ведомость,
		|	ВЫРАЗИТЬ(ВременнаяТаблицаЗП.Ведомость КАК Документ.ВедомостьЗП).ПериодРегистрации,
		|	ЗНАЧЕНИЕ(Перечисление.ВидыВыплатыЗарплаты.ЧерезБанк),
...
Процедура ИнициализироватьДанныеДокумента(ДокументСсылка, СтруктураДополнительныеСвойства) Экспорт
	...
	Запрос.Текст =
		...
		|	ПлатежноеПоручениеИсходящее.ВерсияДанных,
		...
		|	ПлатежноеПоручениеИсходящее.ПоВедомости,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПлатежноеПоручениеИсходящееНалоги.Ссылка,
		|	ПлатежноеПоручениеИсходящееНалоги.НомерСтроки,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПлатежноеПоручениеИсходящееПрочиеРасходы.Ссылка,
		|	ПлатежноеПоручениеИсходящееПрочиеРасходы.НомерСтроки,
		...
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		|	ПлатежноеПоручениеИсходящееВыдачаВПодотчет.Ссылка,
		|	ПлатежноеПоручениеИсходящееВыдачаВПодотчет.НомерСтроки,
...
Функция ПечатьППИ(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПлатежноеПоручениеИсходящее";
	...
	Пока Шапка.Следующий()  Цикл
		...
		Для Сч = 1 По КоличествоЭкземплярНаСтранице Цикл
			...
			Если Шапка.Операция.ВидОперации = Перечисления.ВидыОперацийППИ.ВыплатаЗП
				И НЕ Шапка.ПоВедомости И ЗначениеЗаполнено(Шапка.КартСчет) Тогда
			...
			ОбластьМакета.Параметры.НаименованиеНомер = "ПЛАТЕЖНОЕ ПОРУЧЕНИЕ ИСХОДЯЩЕЕ № " + ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(Шапка.Номер, Истина, Истина);
