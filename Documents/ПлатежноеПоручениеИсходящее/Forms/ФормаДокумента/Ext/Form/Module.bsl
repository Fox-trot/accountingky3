...
// Установка отбора (по контрагенту / по организации / все банковские счета) для поля Банковский Счет Получателя в зависимости от вида операции
//
&НаСервере
...
&НаКлиенте
Процедура УстановитьПараметрВыбораБанковскогоСчетаПолучателяДляВидаОперацииПрочийРасход()

	Если НЕ ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийППИ.ПрочийРасход") Тогда
		Возврат;
	КонецЕсли;
		
	Если Объект.ПрочиеРасходы.Количество() > 0 Тогда
		СтрокаПрочиеРасходы = Объект.ПрочиеРасходы[0];
		Если ТипЗнч(СтрокаПрочиеРасходы.Субконто1) = ТИП("СправочникСсылка.Контрагенты") Тогда
			ПараметрВыбораБС = СтрокаПрочиеРасходы.Субконто1;
			Если ЗначениеЗаполнено(СтрокаПрочиеРасходы.Субконто1) Тогда
				Объект.БанковскийСчетПолучателя	= ПолучитьОсновнойБанковскийСчет(СтрокаПрочиеРасходы.Субконто1);			
			КонецЕсли;
			
		ИначеЕсли ТипЗнч(СтрокаПрочиеРасходы.Субконто2) = ТИП("СправочникСсылка.Контрагенты") Тогда	
			ПараметрВыбораБС = СтрокаПрочиеРасходы.Субконто2;
			Если ЗначениеЗаполнено(СтрокаПрочиеРасходы.Субконто2) Тогда
				Объект.БанковскийСчетПолучателя	= ПолучитьОсновнойБанковскийСчет(СтрокаПрочиеРасходы.Субконто2);	
			КонецЕсли;
			
		ИначеЕсли ТипЗнч(СтрокаПрочиеРасходы.Субконто1) = ТИП("СправочникСсылка.ФизическиеЛица") Тогда	
			ПараметрВыбораБС = СтрокаПрочиеРасходы.Субконто1;
			Если ЗначениеЗаполнено(СтрокаПрочиеРасходы.Субконто1) Тогда
				Объект.БанковскийСчетПолучателя	= ПолучитьОсновнойБанковскийСчет(СтрокаПрочиеРасходы.Субконто1);			
			КонецЕсли
			
		ИначеЕсли ТипЗнч(СтрокаПрочиеРасходы.Субконто2) = ТИП("СправочникСсылка.ФизическиеЛица") Тогда	
			ПараметрВыбораБС = СтрокаПрочиеРасходы.Субконто2;				
			Если ЗначениеЗаполнено(СтрокаПрочиеРасходы.Субконто2) Тогда
				Объект.БанковскийСчетПолучателя	= ПолучитьОсновнойБанковскийСчет(СтрокаПрочиеРасходы.Субконто2);	
			КонецЕсли;
...
КонецПроцедуры // УстановитьПараметрВыбораБанковскогоСчетаПолучателяДляВидаОперацииПрочийРасход()
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	ИначеЕсли ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийППИ.ВыплатаЗП") Тогда
		...
		Элементы.ВыплатаЗаработнойПлатыКартСчет.Видимость           = НЕ Объект.ПоВедомости;
		Если Объект.ПоВедомости Тогда
			Элементы.БанковскийСчетПолучателя.Заголовок				= "Б/счет ЗП";
			Элементы.ВыплатаЗаработнойПлаты.Видимость 				= Истина;
			Элементы.Получатель.Видимость 							= Истина;
		    Элементы.Получатель.Заголовок							= "Банк";
			Элементы.ВыплатаЗаработнойПлатыВедомость.Видимость 		= Истина;
			Элементы.ВыплатаЗаработнойПлатыФизЛицо.Видимость 		= Истина;//Ложь
		Иначе
			Элементы.ВыплатаЗаработнойПлаты.Видимость 				= Истина;
			Элементы.Получатель.Видимость 							= Ложь;
...
Процедура БанковскийСчетПолучателяПриИзменении(Элемент)
	Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийППИ.ПереводНаДругойСчет") 
		И Объект.БанковскийСчетПолучателя = Объект.БанковскийСчет Тогда
			...
			БухгалтерскийУчетСервер.СообщитьОбОшибке(
			Объект.Ссылка,
			"Банковские счета должны быть разные!",
			,
			,
...
Процедура ПрочиеРасходыСубконто1ПриИзменении(Элемент)
	ИзменитьПараметрыВыбораПолейСубконто(ЭтаФорма, "", "СчетУчета");
	УстановитьПараметрВыбораБанковскогоСчетаПолучателяДляВидаОперацииПрочийРасход();
КонецПроцедуры
