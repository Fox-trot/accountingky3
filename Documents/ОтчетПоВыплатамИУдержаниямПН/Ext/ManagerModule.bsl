...
Функция ПечатьSTI_017_1Страница(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		ТЗ = Шапка.Отчет.Выгрузить();
		КолвоСтрок = ТЗ.Количество();	
		КолвоЛистов = КолвоСтрок / 16;
		...
		ДанныеПечати.Вставить("СуммаВыплаченная", ТЗ.Итог("СуммаВыплаченная"));
		ДанныеПечати.Вставить("СуммаПН", ТЗ.Итог("СуммаПН"));
	...
	Пока Шапка.Следующий() Цикл
		...
		Для Каждого СтрокаТаблицы Из ТЗ Цикл
			...
			Если Счетчик <= 15 Тогда
				...
				СуммаВыплаченная = СуммаВыплаченная + СтрокаТаблицы.СуммаВыплаченная;
			...
			ИначеЕсли Счетчик > 14 Тогда 
				Счетчик = 0;
				СтрокНаЛисте = 0;
				НомерЛиста = НомерЛиста + 1;
				
				СтруктураПараметров = Новый Структура;
				СтруктураПараметров.Вставить("СуммаВыплаченная", СуммаВыплаченная);
		...
		НехваткаНаЛист = 15 - СтрокНаЛисте;
...
Функция ПечатьSTI_058_1Страница(МассивОбъектов, ОбъектыПечати)
	...
	|	ТаблицаДокумента.Организация.ГНС.Код КАК ГНСКод,
	|	ТаблицаДокумента.Организация.НаименованиеПолное КАК ОрганизацияНаименованиеПолное,
	|	ТаблицаДокумента.ОтчетГодовой058.(
	|		НомерСтроки,
	|		ФизЛицо,
	|		ИНН,
	|		КодДохода,
	...
	Пока Шапка.Следующий() Цикл
		...
		ТЗ = Шапка.ОтчетГодовой058.Выгрузить();
		КолвоСтрок = ТЗ.Количество();	
		...
		ДанныеПечати.Вставить("СуммаВыплаченная", ТЗ.Итог("СуммаВыплаченная"));
		ДанныеПечати.Вставить("СуммаПН", ТЗ.Итог("СуммаПН"));
		ДанныеПечати.Вставить("СуммаМатериальнойВыгоды", ТЗ.Итог("СуммаМатериальнойВыгоды"));
		ДанныеПечати.Вставить("СуммаПНСМатериальнойВыгоды", ТЗ.Итог("СуммаПНСМатериальнойВыгоды"));
