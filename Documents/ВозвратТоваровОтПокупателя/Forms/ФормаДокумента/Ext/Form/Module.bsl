...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ПодборДокументаРеализации" 
		...
		Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
			...
			Объект.РасчитатьСуммыВВалютеРеглУчета = Ложь;
		КонецЕсли;	
		УстановитьВидимостьДоступностьЭлементов();
		ОбновитьПодвалФормы();
...
Процедура БезналичныйРасчетПриИзменении(Элемент)
	...
	Объект.СтавкаНСП = ?(Объект.БезналичныйРасчет, Неопределено, ДанныеУчетнойПолитики.СтавкаНСПРеализацииТовары);
	Объект.СтавкаНСПУслуги = ?(Объект.БезналичныйРасчет, Неопределено, ДанныеУчетнойПолитики.СтавкаНСПРеализацииУслуги);
...
// Процедура - обработчик события ПриИзменении поля таблицы Товары.
//
&НаКлиенте
Процедура ТоварыПриИзменении(Элемент)
	Объект.РасчитатьСуммыВВалютеРеглУчета = Истина;
КонецПроцедуры
...
// Процедура - обработчик события ПриИзменении поля таблицы Услуги.
//
&НаКлиенте
...
Процедура ДобавитьСтрокиТабличнойЧасти(ИмяТабличнойЧасти, ДокументРеализации, Выборка)
	...
	СтавкаНСП = ?(ИмяТабличнойЧасти = "Товары", Объект.СтавкаНСП, Объект.СтавкаНСПУслуги);
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СтрокаТабличнойЧасти.Цена = УчетНДСКлиентСервер.ПересчитатьЦенуПриИзмененииФлаговНалогов(Цена,
															 ...
															 УчетНДСВызовСервера.ПолучитьСтавкуНСП(ДатаДокумента, Объект.Организация, СтавкаНСП));
