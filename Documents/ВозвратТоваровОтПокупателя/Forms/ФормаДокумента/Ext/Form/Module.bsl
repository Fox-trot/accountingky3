...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ПодборДокументаРеализации" 
		...
		Если ЗначениеЗаполнено(Объект.ДокументОснование) Тогда
			...
			Объект.РассчитатьСуммыВВалютеРеглУчета = Ложь;
		КонецЕсли;	
		УстановитьВидимостьДоступностьЭлементов();
		ОбновитьПодвалФормы();
...
Процедура ТоварыПриИзменении(Элемент)
	Объект.РассчитатьСуммыВВалютеРеглУчета = Истина;
КонецПроцедуры
...
Процедура ПолучитьПредставлениеЭСФ()
	Если БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяУчетЭСФ() Тогда 
		ПредставлениеЭСФ = ЭСФСервер.ПолучитьПредставлениеЭСФ(Объект.Ссылка);
	КонецЕсли;	
КонецПроцедуры
	...
	Пока Выборка.Следующий() Цикл
		...
		Иначе
			...
			СтрокаТабличнойЧасти.Цена = УчетНДСКлиентСервер.ПересчитатьЦенуПриИзмененииФлаговНалогов(Цена,
			                                                 ...
			                                                 УчетНДСВызовСервера.ПолучитьСтавкуНДС(ДатаДокумента, СтавкаНДС),
