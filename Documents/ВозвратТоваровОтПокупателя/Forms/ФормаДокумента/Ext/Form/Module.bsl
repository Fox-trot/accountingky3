...
Процедура ОбработатьИзменениеУчетнойПолитики()	
	...
	Если НЕ ДанныеУчетнойПолитики.ПлательщикНСП Тогда
		...
		Объект.СтавкаНСПУслуги 	= ПредопределенноеЗначение("Справочник.СтавкиНСП.ПустаяСсылка");
	КонецЕсли;	
КонецПроцедуры
...
Процедура ЗаполнитьТоварыПоОснованию(ДокументОснование)
	...
	// Замена ставки НСП на ставку НСП (услуги)
	СтруктураДанные.СтавкаНСП = Объект.СтавкаНСПУслуги;
