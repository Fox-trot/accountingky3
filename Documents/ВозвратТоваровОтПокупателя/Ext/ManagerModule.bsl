...
Процедура СформироватьТаблицаХозрасчетный(ДокументСсылка,СтруктураДополнительныеСвойства, Отказ)
	...
	Запрос.Текст =
		...
		|	-ВЫБОР
		|		КОГДА НЕ ВременнаяТаблицаШапка.СуммаВключаетНалоги И ВременнаяТаблицаТовары.ВсегоВВалютеРеглУчета = ВременнаяТаблицаТовары.СуммаСкидкиВВалютеРеглУчета
		|			ТОГДА ВременнаяТаблицаТовары.ВсегоВВалютеРеглУчета
		...
		|	-ВЫБОР
		|		КОГДА НЕ ВременнаяТаблицаШапка.СуммаВключаетНалоги И ВременнаяТаблицаТовары.Всего = ВременнаяТаблицаТовары.СуммаСкидки
		...
		|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,
		|	-ВЫБОР
		|		КОГДА НЕ ВременнаяТаблицаШапка.СуммаВключаетНалоги И ВременнаяТаблицаУслуги.ВсегоВВалютеРеглУчета = ВременнаяТаблицаУслуги.СуммаСкидкиВВалютеРеглУчета
		|			ТОГДА ВременнаяТаблицаУслуги.ВсегоВВалютеРеглУчета
		...
		|	ВременнаяТаблицаШапка.ВалютаДокумента КАК ВалютаКт,
		|	-ВЫБОР
		|		КОГДА НЕ ВременнаяТаблицаШапка.СуммаВключаетНалоги И ВременнаяТаблицаУслуги.Всего = ВременнаяТаблицаУслуги.СуммаСкидки
		...
		|	НЕОПРЕДЕЛЕНО КАК СубконтоКт3,	
		|	-ВЫБОР
		|		КОГДА НЕ ВременнаяТаблицаШапка.СуммаВключаетНалоги И ВременнаяТаблицаОС.ВсегоВВалютеРеглУчета = ВременнаяТаблицаОС.СуммаСкидкиВВалютеРеглУчета
		|			ТОГДА ВременнаяТаблицаОС.ВсегоВВалютеРеглУчета
		...
		|	ВременнаяТаблицаШапка.ВалютаДокумента КАК ВалютаКт,
		|	-ВЫБОР
		|		КОГДА НЕ ВременнаяТаблицаШапка.СуммаВключаетНалоги И ВременнаяТаблицаОС.Всего = ВременнаяТаблицаОС.СуммаСкидки
				...
				Если (ИсходнаяТаблица <> Неопределено И ПересчитаннаяТаблица <> Неопределено)
					...
					Для Каждого СтрокаТаблицы Из ИсходнаяТаблица Цикл
						...
						Если ОбластьМакетаДопСтрока = Неопределено Тогда
							...
							ДанныеДокумента.СтавкаНСП = ?(СтрокаТаблицы.ЭтоУслуга, Шапка.СтавкаНСПУслуги, Шапка.СтавкаНСП);
							РассчитатьСуммыСчетФактуры(СтрокаТаблицы, ДанныеДокумента, ДанныеПечати,, НЕ ЕстьСкидка);
							ОбластьМакета.Параметры.Заполнить(СтрокаТаблицы);
						...
						Иначе
							...
							Если НЕ СчетФактураВВалюте Тогда
								ДанныеДокумента.СтавкаНСП = ?(СтрокаТаблицы.ЭтоУслуга, Шапка.СтавкаНСПУслуги, Шапка.СтавкаНСП);
								РассчитатьСуммыСчетФактуры(СтрокаТаблицы, ДанныеДокумента, ДанныеПечати, Истина, НЕ ЕстьСкидка);
							КонецЕсли;	
							...
							Если Строки.Количество() > 0 Тогда
								...
								Иначе
									...
									ДанныеДокумента.СтавкаНСП = ?(СтрокаТаблицы.ЭтоУслуга, Шапка.СтавкаНСПУслуги, Шапка.СтавкаНСП);
									РассчитатьСуммыСчетФактуры(Данные, ДанныеДокумента, ДанныеПечати,, НЕ ЕстьСкидка);
...
Процедура РассчитатьСуммыСчетФактуры(СтрокаТаблицы, ДанныеДокумента, ДанныеПечати, ИсходныеСуммы = Ложь, ТолькоРасчетИтогов = Ложь)
	...
	Иначе	
		...
		СтруктураСтроки.Вставить("СтавкаНДС", 	ДанныеДокумента.СтавкаНДС);
		СтруктураСтроки.Вставить("СтавкаНСП", 	ДанныеДокумента.СтавкаНСП);
		...
		Иначе
			...
			СтруктураДопПараметров.Вставить("СтавкаНДС", 		 ДанныеДокумента.СтавкаНДС);
			СтруктураДопПараметров.Вставить("СтавкаНСП", 		 ДанныеДокумента.СтавкаНСП);
