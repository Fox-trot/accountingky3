...
Процедура СформироватьТаблицаСотрудникиПоТрудовымСоглашениям(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ЗНАЧЕНИЕ(Справочник.Календари.Основной) КАК ГрафикРаботы,
		...
		|	ТаблицаДокумента.Подразделение,
		|	ТаблицаДокумента.Должность,
		|	ЗНАЧЕНИЕ(Справочник.Календари.Основной),
...
Процедура СформироватьТаблицаВзаиморасчетыССотрудниками(ДокументСсылка, СтруктураДополнительныеСвойства)
	...
	Запрос.Текст =
		...
		|	ВременнаяТаблицаШапка.Размер КАК Сумма
		|ИЗ
		|	ВременнаяТаблицаШапка КАК ВременнаяТаблицаШапка
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ВременнаяТаблицаНачисленияПоПериодам КАК ТаблицаДокумента
		|		ПО (ИСТИНА)";
...
Функция ПечатьТрудовогоСоглашения(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПриемНаРаботу";     
	...
	Для каждого  ТекущийДокумент Из МассивОбъектов Цикл
		...
		ТабличныйДокумент.ИмяПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ТрудовоеСоглашение_ТрудовойДоговор";     
		...
		Если Не ЗначениеЗаполнено(Выборка.ДатаОкончанияРаботы) Тогда 
			ОбластьМакета.Параметры.СрокОпределенныйНеопределенный = "- на неопределенный срок;";
		Иначе 
			ОбластьМакета.Параметры.СрокОпределенныйНеопределенный = "- на определённый срок;";
		КонецЕсли;
...
Функция ПечатьПриказОПриемеНаРаботу(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати = "ПАРАМЕТРЫ_ПЕЧАТИ_ПриказОПриемеНаРаботу";     
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_ПриказОПриемеНаРаботу") Тогда
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, 
		"ПФ_MXL_ПриказОПриемеНаРаботу", 
		"Приказ о приеме на рабту", 
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПФ_MXL_ПриказОПриемеНаРаботу";
