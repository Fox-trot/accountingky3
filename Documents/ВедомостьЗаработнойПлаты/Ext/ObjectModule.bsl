...
Процедура ЗаполнитьТабличнуюЧасть() Экспорт 
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВидВедомости = 4 Тогда
			СтрокаТабличнойЧасти.СуммаПоКартСчету = БухгалтерскийУчетСервер.ОкруглитьЦену(СтрокаТабличнойЧасти.СуммаПоКартСчету, ПорядокОкругления, Ложь);
		ИначеЕсли ВидВедомости = 3 Тогда
			СтрокаТабличнойЧасти.СуммаПоБанковскомуСчету = БухгалтерскийУчетСервер.ОкруглитьЦену(СтрокаТабличнойЧасти.СуммаПоБанковскомуСчету, ПорядокОкругления, Ложь);
		ИначеЕсли ВидВедомости = 2 ИЛИ ВидВедомости = 1 Тогда
			СтрокаТабличнойЧасти.СуммаПоКассе = БухгалтерскийУчетСервер.ОкруглитьЦену(СтрокаТабличнойЧасти.СуммаПоКассе, ПорядокОкругления, Ложь);
		КонецЕсли;
	КонецЦикла; 
...
Процедура ПодготовитьДанныеДляДокументовВыплаты() Экспорт 
	...
	Пока ВыборкаИндивидуальныеППИ.Следующий() Цикл
		...
		СтруктураППИ.Вставить("Банк", 							ВыборкаИндивидуальныеППИ.БанкБанковскогоСчета);
