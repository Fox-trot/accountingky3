	...
	Пока Шапка.Следующий() Цикл
		...
		Для Каждого ИмяОбласти Из МассивОбластейМакета Цикл
			...
			Если ИмяОбласти = "ШапкаУтверждаю" И Шапка.СШапкойУтверждаю Тогда 
				Руков = БухгалтерскийУчетСервер.ОтветственныеЛицаОрганизацийРуководители(Шапка.Организация, Шапка.Дата);
...
Функция ПечатьАктаСверкиСФ(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		Иначе
			...
			ФСККт 	= -ФСК;
		КонецЕсли;		
	    ДанныеПечати.Вставить("ФинсанкцККтПлательщик", ?(ФСККт = 0, "", ФСККт));
		ДанныеПечати.Вставить("ФинсанкцКДтПлательщик", ?(ФСКДт = 0, "", ФСКДт));
		ДанныеПечати.Вставить("ФинсанкцКДтСФ", ?(ФСККт = 0, "", ФСККт));	
		ДанныеПечати.Вставить("ФинсанкцККтСФ", ?(ФСКДт = 0, "", ФСКДт));	   
