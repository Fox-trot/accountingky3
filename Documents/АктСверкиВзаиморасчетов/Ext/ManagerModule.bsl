...
Функция СформироватьЗаголовокДокумента(Шапка, Знач НазваниеДокумента = "") Экспорт
	...
	ДанныеДокумента = Новый Структура("Организация,Рег,ОКПО,ИНН,День,Месяц,Год,Квартал");
	ЗаполнитьЗначенияСвойств(ДанныеДокумента, Шапка);

	ДанныеДокумента.День = День(Шапка.Дата);
	ДанныеДокумента.Месяц = Формат(Шапка.Дата,"ДФ= ММММ"); 
	ДанныеДокумента.Год = Формат(Шапка.ДатаНачала,"ДФ= yyyy");
	ТекущийНомерМесяца=Месяц(Шапка.ДатаОкончания);
	
	Если ТекущийНомерМесяца >= 1 И ТекущийНомерМесяца <= 3 Тогда 
		Квартал 	= "I";
	ИначеЕсли ТекущийНомерМесяца >= 4 И ТекущийНомерМесяца <= 6 Тогда 	
		Квартал 	= "II";
	ИначеЕсли ТекущийНомерМесяца >= 7 И ТекущийНомерМесяца <= 9 Тогда 	
		Квартал 	= "III";
	ИначеЕсли ТекущийНомерМесяца >= 10 И ТекущийНомерМесяца <= 12 Тогда 	
		Квартал 	= "IV";
	КонецЕсли;		
	
	ДанныеДокумента.Квартал = Квартал;
...
Функция ОписаниеРасчетногоДокументаКонтрагента(ДокументСсылка, Знач НомерДокумента, Знач ДатаДокумента) Экспорт
	
	// Номер и дату обработаем сразу, так как понадобиться и для пустой ссылки
	НомерДокумента	= ?(ПустаяСтрока(НомерДокумента), Нстр("ru = '_______'"), ПрефиксацияОбъектовКлиентСервер.ПолучитьНомерНаПечать(НомерДокумента, Ложь, Истина));
	...
	Возврат ОписаниеДокумента + ДобавитьКОписанию 
		...
		+ НСтр("ru=' от '") + ДатаДокумента + НСтр("ru=' г.'");
...
Функция ПечатьАктаСверки(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() цикл
		...
		ТекстЗаголовка = "сверки взаиморасчетов между " + СокрЛП(НазваниеОрганизации) + Символы.ПС + "и " + СокрЛП(НаименованиеКонтрагента)
		+ " с " + Формат(Шапка.ДатаНачала, "ДФ=dd.MM.yyyy") + " по " + Формат(Шапка.ДатаОкончания, "ДФ=dd.MM.yyyy");
		Если ЗначениеЗаполнено(Шапка.ДоговорКонтрагента) Тогда
			ТекстЗаголовка = ТекстЗаголовка + Символы.ПС + "по договору " + СокрЛП(Шапка.ДоговорКонтрагента.Наименование);
		КонецЕсли;
		...
		Если Шапка.СверкаСогласована Тогда
			Если ТаблицаПоДаннымОрганизации.Количество() = 0 Тогда
				РезультатыСверки = "Задолженность "+ СокрЛП(НазваниеОрганизации) +" перед "+НаименованиеКонтрагента+ " на "+ Формат(Шапка.ДатаОкончания,"ДФ=дд.ММ.гггг") + " составляет " + Формат(ОстатокНаКонецК, "ЧЦ=21; ЧДЦ=2")+" "+ строка(ВалютаРезультата);
				Сумма = БухгалтерскийУчетСервер.СформироватьСуммуПрописью(ОстатокНаКонецК, ВалютаРезультата);
				СуммаПрописью = "( "+Сумма+" )";
			Иначе                                                                                                                                                                                         
				Если ОстатокНаКонецК < 0  Тогда
					РезультатыСверки = "Задолженность "+ СокрЛП(НаименованиеКонтрагента) + " перед " + СокрЛП(НазваниеОрганизации)+" на "+ Формат(Шапка.ДатаОкончания,"ДФ=дд.ММ.гггг") + " составляет " + Формат(Шапка.ОстатокНаКонец, "ЧЦ=21; ЧДЦ=2")+" "+ строка(ВалютаРезультата);
...
Функция ПечатьАктаСверкиСФ(МассивОбъектов, ОбъектыПечати)
	...
	ТабличныйДокумент = Новый ТабличныйДокумент;
	ТабличныйДокумент.КлючПараметровПечати = "ПараметрыПечати_АктСверкиСФ";
...
Процедура Печать(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ОбъектыПечати, ПараметрыВывода) Экспорт
	...
	ИначеЕсли УправлениеПечатью.НужноПечататьМакет(КоллекцияПечатныхФорм, "ПФ_MXL_АктСверкиСФ") Тогда	
		УправлениеПечатью.ВывестиТабличныйДокументВКоллекцию(КоллекцияПечатныхФорм, 
		"ПФ_MXL_АктСверкиСФ", 
		"Акт сверки соцФонда", 
		ПечатьАктаСверкиСФ(МассивОбъектов, ОбъектыПечати),
		,
...
Процедура ДобавитьКомандыПечати(КомандыПечати) Экспорт
	...
	КомандаПечати.Идентификатор = "ПФ_MXL_АктСверки";
	КомандаПечати.Представление = НСтр("ru = 'Акт сверки'");
	КомандаПечати.СписокФорм = "ФормаДокумента,ФормаСписка";
	КомандаПечати.Порядок = 1;
	
	КомандаПечати = КомандыПечати.Добавить();
	КомандаПечати.Идентификатор = "ПФ_MXL_АктСверкиСФ";
