...
Функция ПечатьАктаСверки(МассивОбъектов, ОбъектыПечати)
	...
	Пока Шапка.Следующий() Цикл
		...
		СуммаДтКонтр = ?(Шапка.Разногласие = "", Формат(ТаблицаПоДаннымКонтрагента.Итог("СуммаКтКонтрагент"), "ЧЦ=15; ЧДЦ=2; ЧН=0,00"), Формат(ТаблицаПоДаннымКонтрагента.Итог("СуммаКтКонтрагент"), "ЧЦ=15; ЧДЦ=2; ЧН=' '"));
		ДанныеПечати.Вставить("СуммаДтКонтр", ?(СуммаДтКонтр = 0, "", СуммаДтКонтр));
		
		СуммаКтКонтр = ?(Шапка.Разногласие = "", Формат(ТаблицаПоДаннымКонтрагента.Итог("СуммаДтКонтрагент"), "ЧЦ=15; ЧДЦ=2; ЧН=0,00"), Формат(ТаблицаПоДаннымКонтрагента.Итог("СуммаДтКонтрагент"), "ЧЦ=15; ЧДЦ=2; ЧН=' '"));
			...
			ИначеЕсли ИмяОбласти = "Обороты" Тогда
				...
				Для Каждого СтрокаТабличнойЧасти Из ТаблицаПоДаннымОрганизации Цикл
					...
					ИначеЕсли ЗначениеЗаполнено(СтрокаТабличнойЧасти.ДокументСверки) И Не Шапка.ИсключитьИзПредставленияДокумента Тогда	
						НомерДокумента = ОписаниеРасчетногоДокументаКонтрагента(СтрокаТабличнойЧасти.ДокументСверки, СтрокаТабличнойЧасти.ДокументСверки.Номер, СтрокаТабличнойЧасти.ДокументСверки.Дата);
						ДанныеЗаполнения.Вставить("РегистраторПредставление", НомерДокумента + ?(СтрокаТабличнойЧасти.ДокументСверки.Комментарий = "", "", "
						|" + СтрокаТабличнойЧасти.ДокументСверки.Комментарий));
					КонецЕсли;
					...
					Если Не СтрокаТабличнойЧасти = Неопределено Тогда 
						Если НЕ Шапка.СверкаПоСоцФонду И ЗначениеЗаполнено(СтрокаТабличнойЧасти.ДокументСверки)
							...
							И ЗначениеЗаполнено(СтрокаТабличнойЧасти.ДокументСверки.СерияБланкаСФ) Тогда 
							
							РеквизитыСФ = "Счет-фактура " + СтрокаТабличнойЧасти.ДокументСверки.СерияБланкаСФ + " № " + СтрокаТабличнойЧасти.ДокументСверки.НомерБланкаСФ
							+ " от " + Формат(СтрокаТабличнойЧасти.ДокументСверки.ДатаСФ,"ДЛФ=D"); 
						Иначе
							РеквизитыСФ = "";
						КонецЕсли;				
					КонецЕсли;
