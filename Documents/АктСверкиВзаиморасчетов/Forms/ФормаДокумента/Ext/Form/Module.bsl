...
Процедура ДатаНачалаПриИзменении(Элемент)
	...
	Если ЗначениеЗаполнено(Объект.ДатаНачала) Тогда
		...
		КорректностьДат = ПроверитьКорректностьУказанныхДат("ДатаНачала");
...
Процедура ДатаОкончанияПриИзменении(Элемент)
	...
	КорректностьДат = ПроверитьКорректностьУказанныхДат("ДатаОкончания");
...
Процедура СверкаПоСоцФондуПриИзменении(Элемент)
	Объект.ПоГоловномуКонтрагенту = Ложь;
	Объект.СписокСчетов.Очистить();
	ЗаполнитьСписокСчетовНаСервере();
...
Процедура ПоГоловномуКонтрагентуПриИзменении(Элемент)
	Если Объект.ПоГоловномуКонтрагенту Тогда 
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	Если Объект.ПоГоловномуКонтрагенту 
	...
	Если Объект.СверкаПоСоцФонду Тогда  
		...
		Элементы.ПоГоловномуКонтрагенту.Видимость = Ложь;
	Иначе 
		Элементы.Контрагент.Видимость = Истина;
		Элементы.ПоГоловномуКонтрагенту.Видимость = Истина;
	КонецЕсли;
...
Процедура ЗаполнитьСписокСчетовНаСервере()
	...
	Запрос.Текст = 
		...
		|ИЗ
		|	РегистрСведений.УчетнаяПолитикаПоПерсоналу.СрезПоследних(&КонецПериода, Организация = &Организация) КАК УчетнаяПолитикаПоПерсоналуСрезПоследних
		|ГДЕ
		|	&СверкаПоСоцФонду
		|;
		|
	...
	Если Объект.ДоговорКонтрагента.Пустая() Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "Субконто2 = &Договор", "Истина");
	КонецЕсли;
...
//
&НаКлиенте
Функция ПроверитьКорректностьУказанныхДат(Дата)
