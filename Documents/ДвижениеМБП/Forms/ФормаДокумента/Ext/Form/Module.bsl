...
Процедура ТоварыСчетЗатратПриИзменении(Элемент)
	...
	ПоляФормы = Новый Структура("Субконто1, Субконто2, Субконто3", "ТоварыСубконто1", "ТоварыСубконто2", "ТоварыСубконто3");
	БухгалтерскийУчетКлиентСервер.ПриВыбореСчета(СтрокаТабличнойЧасти.СчетЗатрат, ЭтаФорма, ПоляФормы, Неопределено, Истина);
	...
	ПараметрыДокумента = БухгалтерскийУчетКлиентСервер.СписокПараметровВыбораСубконто(ЭтаФорма, СтрокаТабличнойЧасти, "Субконто%Индекс%", "СчетЗатрат");
	БухгалтерскийУчетКлиентСервер.ИзменитьПараметрыВыбораПолейСубконто(ЭтаФорма, СтрокаТабличнойЧасти, "Субконто%Индекс%", "ТоварыСубконто%Индекс%", ПараметрыДокумента);	
КонецПроцедуры
