	...
	Попытка
		...
		Если ПустаяСтрока(НайденныйЭлемент.ВерсияДанных) Тогда // Новый объект
			...
			Если ОбъектХДТО.DeletionMark = Истина Тогда
				...
				СтрокаЛогаЗагрузки = "Пропуск (не найден помеченный на удаление): " + СокрЛП(ОбъектХДТО.Code);
				Записывать = Ложь;
			Иначе
		...
		Иначе // Объект есть в базе данных
			...
			СтрокаЛогаЗагрузки = "Изменение (по УИН): " + СокрЛП(НайденныйЭлемент.Код) + ", " + СокрЛП(НайденныйЭлемент.Наименование) + "";
		КонецЕсли;
